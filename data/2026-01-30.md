...
TL;DR: 该研究针对对话式图像生成中的非马尔可夫问题，提出数据构建、训练框架和优化技术，显著提升多轮交互的连贯性和指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在对话式图像生成中存在局限性，大多采用马尔可夫方法，仅依赖最近图像而忽略长期历史，导致无法处理用户回溯早期状态、撤销更改或引用多轮前实体等复杂交互场景。

Method: 提出三方面方法：1) 非马尔可夫多轮数据构建策略，包括回滚式编辑和基于名称的多轮个性化；2) 历史条件训练与推理框架，采用令牌级缓存防止多轮身份漂移；3) 高保真图像重建和可编辑个性化优化技术，包括基于重建的DiT解令牌器和多阶段微调课程。

Result: 实验表明，针对非马尔可夫交互的显式训练显著提升了多轮一致性和指令遵循能力，同时保持了强大的单轮编辑和个性化性能。

Conclusion: 该研究通过系统解决对话式图像生成中的非马尔可夫挑战，为复杂多轮交互场景提供了有效的解决方案，推动了该领域的发展。
...
TL;DR: 提出一种基于文本引导的PET图像去噪方法，使用预训练CLIP模型特征结合U-Net架构，能够在单一模型中处理多种计数水平的PET图像去噪任务。


<details>
  <summary>Details</summary>
Motivation: PET成像在医学诊断中至关重要，但PET图像常受复杂噪声影响，噪声会掩盖关键诊断信息。图像质量受多种因素影响，包括扫描仪硬件、图像重建、示踪剂特性、剂量/计数水平和采集时间等。
...
Conclusion: 该方法展示了文本引导去噪在PET图像增强中的潜力，单一模型能够处理多种计数水平的去噪任务，为医学影像处理提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: CT探测器中的低性能像素会导致环状和条纹伪影，现有监督学习方法需要昂贵的专用数据集，且只关注图像域或投影域，忽略了CT几何前向操作的内在关联

Method: 提出基于合成数据的展开式双域方法，利用自然图像生成合成数据，通过CT几何前向操作的内在关联，在投影域和图像域之间建立联系进行伪影校正

Result: 在模拟1-2%探测器缺陷的实验中，该方法大幅优于现有最先进方法，能够有效校正低性能像素伪影

Conclusion: 该方法无需收集真实临床数据进行模型训练，可适应不同扫描仪设置的软件应用，具有实际应用价值
...
TL;DR: 基于前列腺活检病理切片训练AI模型预测根治性前列腺切除术后生化复发风险，在多个外部队列中验证了泛化能力，相比传统临床评分系统有增量改善。


<details>
  <summary>Details</summary>
Motivation: 根治性前列腺切除术后生化复发是侵袭性前列腺癌的替代标志物，但现有预后工具不够精确，需要更准确的预测模型来支持临床决策。

Method: 使用STHLM3队列（n=676）的诊断性前列腺活检切片训练AI模型，采用基础模型和基于注意力的多实例学习方法，预测患者特异性生化复发风险。在三个外部队列（LEOPARD、CHIMERA、TCGA-PRAD）验证泛化能力，并整合临床变量进行多模态分析。

Result: 图像方法在三个外部队列的5年时间依赖性AUC分别为0.64、0.70和0.70。整合临床变量增加了补充预后价值，实现了统计学显著的风险分层。相比指南推荐的CAPRA-S评分，AI模型在术后预后方面有增量改善。

Conclusion: 基于活检训练的组织病理学AI模型能够跨样本类型泛化，支持术前和术后决策，但AI多模态方法相比简单预测模型的附加价值需要在进一步研究中严格评估。
...
<details>
  <summary>Details</summary>
Motivation: 后门攻击对深度学习构成严重威胁，但现有针对目标检测的攻击方法存在关键缺陷：依赖不现实的假设、缺乏物理验证，需要更统一和鲁棒的框架。

Method: 提出BadDet+惩罚框架，统一区域误分类攻击(RMA)和物体消失攻击(ODA)，使用log-barrier惩罚机制抑制触发输入的真实类别预测，实现位置尺度不变性和物理鲁棒性。

Result: 在真实世界基准测试中，BadDet+在合成到物理转移方面优于现有RMA和ODA基线，同时保持干净性能；理论分析证实惩罚在触发特定特征子空间中起作用。

Conclusion: 目标检测存在显著漏洞，需要专门的防御措施；BadDet+框架为理解后门攻击在目标检测中的影响提供了统一方法。
...
TL;DR: 提出ActionVLM框架，通过去偏重加权模块和残差聚合策略，在时序动作定位任务中系统性地缓解视觉-语言模型的模态偏差问题，保持视觉主导信号的同时自适应利用语言信息。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在时序动作定位中过度依赖语言先验而忽视视觉证据，导致显著的模态偏差问题，需要一种能保持视觉主导地位同时自适应利用语言优势的解决方案。

Method: 提出ActionVLM框架，包含两个核心组件：1) 去偏重加权模块，估计语言相对于纯视觉预测的优势增量，并动态调整语言模态权重；2) 残差聚合策略，将语言作为补充细化而非主要驱动因素。

Result: 在THUMOS14数据集上的实验表明，该方法优于现有最先进方法，mAP提升最高达3.2%，有效缓解了模态偏差，减少了语言先验的过度自信，并增强了时序推理能力。

Conclusion: ActionVLM通过系统性地缓解视觉-语言模型中的模态偏差，在保持视觉主导地位的同时自适应利用语言信息，为时序动作定位任务提供了一种有效的多模态融合方案。
...
Conclusion: SoT为透明、过程监督的组合生成建立了新范式，显著提高了文本到图像生成的结构完整性。
...
<details>
  <summary>Details</summary>
Motivation: 微血管吻合手术需要极高精度和技术，传统评估依赖专家主观判断，存在评分者间差异、标准化不足、认知偏见和时间消耗等问题，亟需客观、可靠、自动化的评估系统

Method: 提出包含四个核心组件的AI框架：1)基于YOLO的器械检测模块；2)基于DeepSORT的器械跟踪模块；3)使用形状描述符的器械尖端定位模块；4)基于专家标注数据的监督分类模块用于评估操作熟练度

Result: 实验结果显示框架有效，器械检测精度达97%，平均精度(mAP)为96%，IoU阈值范围为50%到95%(mAP50-95)

Conclusion: 该AI框架为微血管吻合器械操作技能提供了客观、可靠、自动化的评估解决方案，有望改善传统主观评估方法的局限性
...
TL;DR: 提出SDCI框架，通过双向注意力融合、双向图扩散和超像素优化，解决高分辨率遥感图像中训练免费开放词汇语义分割的几何定位和语义预测难题。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像具有地物分布密集、边界复杂的特点，对几何定位和语义预测要求更高。现有的训练免费开放词汇语义分割方法通常采用"单向注入"和"浅层后处理"策略，难以满足这些要求。

Method: 提出SDCI框架：1) 交叉模型注意力融合模块，通过双向注入自注意力图指导协同推理；2) 双向交叉图扩散细化模块，通过迭代随机游走扩散增强双分支分割分数的可靠性；3) 基于凸优化的超像素协同预测机制，利用低层超像素结构进一步细化对象边界。

Result: 在多个遥感语义分割基准测试中，该方法比现有方法取得了更好的性能。消融研究进一步证实，利用超像素结构的传统基于对象的遥感图像分析方法在深度学习框架中仍然有效。

Conclusion: SDCI框架通过双向协同推理和超像素结构优化，有效解决了高分辨率遥感图像中训练免费开放词汇语义分割的挑战，展示了传统遥感分析方法与深度学习框架的有效结合。
...
TL;DR: GeoDiff-LF是一个基于SD-Turbo的扩散模型框架，通过利用光场的空间-角度结构来增强水下4-D光场成像质量
...
Result: 实验表明，该框架在视觉保真度和定量性能方面均优于现有方法，有效缓解了水下场景的颜色失真问题，推进了水下成像增强的技术水平

Conclusion: 通过整合扩散先验和光场几何信息，GeoDiff-LF能够有效提升水下4-D光场成像质量，为高质量水下图像获取提供了新方法
...
TL;DR: GRDR提出了一种新的生成式检索方法，通过多语义ID分配和共享码本训练，解决了生成式检索在视频召回中的语义模糊和跨模态对齐问题，在保持准确性的同时大幅提升检索效率和存储效率。


<details>
  <summary>Details</summary>
Motivation: 当前两阶段视频检索中，召回模型性能受限。生成式检索虽然具有恒定推理和存储复杂度，适合快速召回，但存在语义模糊（每个视频只能对应一个语义ID）和跨模态对齐不足（语义ID仅从视觉特征生成）的问题。

Method: GRDR设计了基于查询引导的多视图分词器，为每个视频分配多个语义ID，提供多样化的语义访问路径。通过共享码本联合训练分词器和生成式检索器，使语义ID成为文本和视频之间的语义桥梁。推理时使用树约束解码生成紧凑候选集，再由密集模型进行精细重排序。

Result: 在TVR基准测试中，GRDR在准确性上与强大的密集检索器相当，同时将索引存储减少了一个数量级，在全库检索中加速高达300倍。

Conclusion: GRDR通过创新的生成式检索方法有效解决了语义模糊和跨模态对齐问题，为大规模实时视频检索提供了高效准确的解决方案，实现了召回质量和检索效率的平衡。
...
TL;DR: 提出Thinker模型解决机器人领域视觉语言模型的问题，通过构建大规模数据集和联合关键帧与完整视频序列的方法，在任务规划基准上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型应用于机器人领域时存在简单但易错的问题，包括第三人称与第一人称视角混淆，以及在时序推理中容易忽略视频结尾信息

Method: 1. 构建针对机器人感知和推理的大规模数据集，包含自我视角视频、视觉定位、空间理解和思维链数据；2. 提出简单有效的方法，联合使用关键帧和完整视频序列作为输入，增强模型视频理解能力

Result: 在两个最常用的任务规划基准数据集上取得了最先进的结果

Conclusion: 提出的Thinker模型通过专门的数据集构建和创新的输入方法，有效解决了机器人领域中视觉语言模型的关键问题，提升了模型在任务规划任务上的性能
...
TL;DR: LAMP是一种针对多模态大语言模型的黑盒攻击方法，通过学习通用对抗扰动来破坏模型跨图像信息聚合能力，具有传染性和位置不变性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多图像处理方面取得了显著进展，但其安全漏洞尚未被充分研究。现有攻击方法主要针对单图像场景且多为白盒攻击，这在现实场景中不实用。

Method: LAMP采用黑盒学习方法生成通用对抗扰动，包含：1）基于注意力的约束防止模型有效聚合跨图像信息；2）跨图像传染约束使扰动标记影响干净标记；3）索引注意力抑制损失实现位置不变攻击。

Result: 实验结果表明，LAMP在多个视觉语言任务和模型上超越了现有最佳基线方法，取得了最高的攻击成功率。

Conclusion: LAMP揭示了多图像MLLM的安全漏洞，为黑盒攻击提供了有效方法，强调了多模态AI系统安全性的重要性。
...
TL;DR: 本文提出了PTQ4ARVG框架，专门解决自回归视觉生成模型的量化难题，包含三个关键技术：GPS处理通道级异常值，STWQ解决令牌级动态激活，DGC消除样本级分布不匹配，实现了8位和6位量化。


<details>
  <summary>Details</summary>
Motivation: 自回归视觉生成模型虽然与语言模型架构兼容且性能接近扩散模型，但现有量化方法无法有效应用于这类模型。本文旨在解决ARVG模型量化中的三个关键挑战：通道级严重异常值、令牌级高度动态激活、样本级分布信息不匹配。

Method: 提出了PTQ4ARVG训练后量化框架，包含三个核心技术：1) GPS通过泰勒展开量化缩放增益，推导最优缩放因子处理通道异常值；2) STWQ利用ARVG固定令牌长度和位置不变分布特性，避免动态校准开销；3) DGC选择对分布熵贡献最大的样本，消除样本级分布不匹配。
...
TL;DR: NFCDS提出了一种通过控制噪声频率来改善扩散采样中保真度-感知质量权衡的方法，通过傅里叶域滤波器抑制低频噪声、保留高频噪声，无需额外训练即可提升图像恢复的质量。


<details>
  <summary>Details</summary>
Motivation: 基于扩散采样的Plug-and-Play方法虽然能生成感知质量高的图像，但通常会牺牲数据保真度，这主要是由于反向扩散过程中引入的噪声造成的。作者发现保真度与感知质量之间的冲突本质上可以通过噪声频率来理解。

Method: 提出了噪声频率控制扩散采样（NFCDS），这是一种用于反向扩散噪声的频谱调制机制。设计了一个傅里叶域滤波器，逐步抑制低频噪声成分（这些成分会导致模糊和保真度下降），同时保留高频噪声成分（这些成分驱动细节生成）。

Result: NFCDS能够快速收敛到既具有高保真度又具有感知说服力的结果，无需额外训练。作为PnP模块，它可以无缝集成到现有的基于扩散的恢复框架中，并在各种零样本任务中改善了保真度-感知平衡。

Conclusion: 通过控制噪声频率，NFCDS有效解决了扩散采样中保真度与感知质量之间的权衡问题，为图像恢复提供了一种无需训练的高效解决方案。
...
TL;DR: Hypersolid是一种自监督学习方法，将表示学习重新解释为离散打包问题，通过短程硬球排斥防止局部碰撞，从而避免表示崩溃。


<details>
  <summary>Details</summary>
Motivation: 自监督学习中防止表示崩溃是一个反复出现的挑战。现有方法通常依赖全局正则化（如最大化距离、去相关维度或强制执行特定分布），但作者认为可以从不同角度重新思考这个问题。

Method: 将表示学习重新解释为离散打包问题，其中保持信息简化为保持单射性。Hypersolid方法使用短程硬球排斥来防止局部碰撞，这种约束产生了一个高分离的几何机制。

Result: 该方法能够保持增强多样性，在细粒度和低分辨率分类任务上表现出色。

Conclusion: 通过将表示学习重新解释为离散打包问题，并使用局部排斥约束，Hypersolid提供了一种防止表示崩溃的有效方法，特别适用于需要保持细节信息的任务。
...
<details>
  <summary>Details</summary>
Motivation: 当前沉浸式交互通信需求推动3D视频会议发展，但传统2D视频压缩技术无法保留细粒度几何和外观细节，而NeRF等隐式神经渲染方法计算成本过高，难以在低比特率下实现高保真3D说话人脸表示。

Method: 整合FLAME参数化建模与3DGS神经渲染，仅传输关键面部元数据，使用高斯头部模型进行高效重建。引入紧凑表示和压缩方案，包括高斯属性压缩和MLP优化，提升传输效率。

Result: 实验结果表明，该方法在极低比特率下实现了优异的率失真性能，能够提供高质量的面部渲染效果。

Conclusion: 该框架特别适合实时3D视频会议应用，解决了高保真3D说话人脸表示在低比特率传输中的挑战。
...
TL;DR: 该论文提出了首个地理定位推理链基准测试，发现虽然大型视觉语言模型在位置预测上接近人类专家水平，但在生成可审计的推理链方面远落后于人类，特别是开源模型表现极差。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在地理定位预测上表现出色，但其推理链经常产生幻觉，无法解释预测依据。需要建立基准来评估VLM生成地理定位推理链的能力，揭示其与人类专家的差距。

Method: 1. 基于GeoGuessr游戏构建基准数据集，包含500个查询场景；2. 与专家玩家（包括世界冠军）合作生成800个真实推理链；3. 评估LLM-as-a-judge和VLM-as-a-judge策略对VLM生成推理链的评分效果；4. 比较不同VLM（闭源和开源）在推理链生成上的表现。

Result: 1. Qwen 3 LLM-as-a-judge评分与人类评分相关性最好；2. 闭源VLM（Gemini、GPT 5）在位置预测上接近人类专家，但在推理链生成上仍落后；3. 开源VLM（Llama、Qwen）表现极差，仅略优于无视觉信息的幻觉基线；4. 揭示了VLM在从高分辨率图像中提取细粒度视觉属性方面的局限性。

Conclusion: 该研究建立了首个地理定位推理链基准，揭示了VLM在生成可解释推理链方面的严重不足，特别是开源模型表现极差。这表明VLM在细粒度视觉属性提取方面存在根本性限制，需要进一步改进。
...
TL;DR: WorldBench是一个新的视频基准测试，专门用于解耦评估世界模型的物理推理能力，通过分离测试单个物理概念或定律来提供更精确的诊断。
...
Method: 设计了两个层次的基准测试：1) 直觉物理理解评估（如物体恒存性、尺度/透视）；2) 低级物理常数和材料属性评估（如摩擦系数、流体粘度）。通过概念特定的解耦评估来隔离测试单个物理概念。

Result: 当在WorldBench上评估最先进的视频世界模型时，发现所有测试模型在特定物理概念上存在失败模式，缺乏生成可靠真实世界交互所需的物理一致性。

Conclusion: WorldBench通过概念特定的解耦评估，为视频生成和世界模型的物理推理能力提供了更细致和可扩展的评估框架，为更稳健和可泛化的世界模型驱动学习铺平了道路。
...
TL;DR: 提出Mam-App模型，一种基于Mamba架构的参数高效苹果叶病分类模型，在保持高精度的同时大幅减少参数量至0.051M，适用于资源受限设备部署。


<details>
  <summary>Details</summary>
Motivation: 全球人口增长和技术进步加剧了粮食生产需求，苹果作为重要水果面临严重病害损失。现有深度学习模型参数量大，训练和推理时间长，而轻量级模型性能下降，需要在效率和性能之间取得平衡。

Method: 提出Mam-App模型，基于Mamba架构设计参数高效的叶病分类模型。使用PlantVillage苹果叶病数据集进行训练和评估，并进一步在玉米和马铃薯叶病数据集上验证模型的鲁棒性和泛化能力。

Result: 在苹果叶病数据集上达到99.58%准确率、99.30%精确率、99.14%召回率和99.22% F1分数，仅使用0.051M参数。在玉米数据集上达到99.48%准确率，在马铃薯数据集上达到98.46%准确率，证明了模型的优秀性能和泛化能力。

Conclusion: Mam-App模型在保持高精度的同时大幅减少参数量，解决了效率与性能的权衡问题，适合部署在无人机、移动设备等资源受限平台，为农业病害监测提供了实用解决方案。
...
TL;DR: LANE通过紧凑自回归依赖实现6倍序列长度提升，AdaGraph策略通过时空解耦加速推理，在3D网格生成中取得速度、细节和几何一致性优势
...
TL;DR: 视觉基础模型在离散采样的病理图像上训练后，其表征能隐式编码疾病的连续进展过程，这为评估表征质量提供了新的度量标准。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究视觉基础模型是否能在表征空间中编码疾病的连续进展过程。在计算病理学中，如果模型的潜在表征能隐式捕捉疾病进展的连续性，可能更好地反映生物学基础，支持更稳健的泛化，并实现对疾病转变相关特征的定量分析。

Method: 使用扩散伪时间（一种从单细胞转录组学推断发育轨迹的方法）来探测基础模型是否在表征空间中沿着连贯的进展方向组织疾病状态。研究分析了四种癌症进展和六个模型，评估模型恢复轨迹排序的能力。

Result: 所有病理特异性模型恢复的轨迹排序都显著超过零基线，其中纯视觉模型达到最高的保真度（CRC-Serrated上τ>0.78）。模型在参考疾病上的轨迹保真度排名能强有力地预测在保留疾病上的少样本分类性能（ρ=0.92）。探索性分析显示细胞类型组成沿着推断的轨迹平滑变化，模式与已知的基质重塑一致。
...
TL;DR: DAF提出动态适配器融合方法，通过PAC-Bayes理论推导最优融合系数，构建单一全局适配器来解决类增量学习中的灾难性遗忘问题
...
Method: 提出动态适配器融合（DAF）方法，基于PAC-Bayes理论推导融合机制，显式整合三个组件：优化的任务特定适配器参数、先前的全局适配器参数和初始化参数。利用损失函数的泰勒展开推导最优融合系数，动态平衡稳定性和可塑性。同时提出鲁棒初始化策略来有效捕获全局知识模式

Result: 在多个类增量学习基准测试中，DAF实现了最先进的性能表现

Conclusion: DAF通过理论推导的动态融合机制，成功构建了单一鲁棒的全局适配器，有效解决了类增量学习中的灾难性遗忘问题，在稳定性和可塑性之间取得了良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 现有的CIL方法通常冻结预训练模型并使用轻量级适配器，通过强制参数正交来防止任务间干扰。但作者认为这种参数约束方法会损害模型的可塑性，需要更灵活的解决方案。
...
Conclusion: SGDS通过在激活空间中塑造类特定稀疏子空间，有效缓解了干扰问题，同时避免了参数空间的刚性约束，实现了更好的可塑性与性能平衡。
...
TL;DR: 提出GIQT方法解决航拍-地面行人重识别中极端视角和距离差异导致的几何失真问题，通过几何诱导的查询-键变换显式校正相似性空间
...
Method: 提出几何诱导查询-键变换(GIQT)，这是一个轻量级低秩模块，通过将查询-键交互条件化于相机几何来显式校正相似性空间。此外还结合了几何条件提示生成机制，直接从相机几何中提取全局、视图自适应的表示先验

Result: 在四个航拍-地面行人重识别基准测试上的实验表明，该框架在极端和先前未见几何条件下持续提高鲁棒性，同时相比最先进方法引入最小计算开销

Conclusion: 通过显式校正相似性空间而非修改特征表示或注意力公式本身，GIQT方法有效解决了航拍-地面行人重识别中的几何失真问题，提高了模型在极端几何条件下的鲁棒性
...
TL;DR: UniMRG提出了一种通过生成任务增强统一多模态模型理解能力的后训练方法，通过生成像素、深度和分割等多种内在表示来提升模型的视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型（UMMs）旨在实现视觉理解和生成的相互促进循环。虽然已有研究利用理解来增强生成，但利用生成来提升理解的方向仍未被充分探索。

Method: 提出UniMRG（统一多表示生成）方法，这是一种架构无关的后训练方法。通过训练UMMs生成输入图像的多种内在表示：像素（重建）、深度（几何）和分割（结构），同时结合标准视觉理解目标。

Result: 实验表明，该方法显著提升了UMMs的细粒度感知能力，减少了幻觉现象，改善了空间理解，同时增强了生成能力。这些改进在不同UMM架构上均得到验证。

Conclusion: 通过生成多种内在表示，UMMs能够捕捉外观、空间关系和结构布局的互补信息，从而实现对视觉输入更深入、更全面的理解，证明了生成任务对提升理解能力的有效性。
...
TL;DR: 该论文提出了一种检测AI生成视频的双路径框架，通过分析视频中的流形投影波动来区分真实视频和高质量伪造视频。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成模型（如Veo和Wan）的快速发展，合成内容的视觉质量已经达到很高水平，宏观语义错误和时间不一致性不再明显。然而，这并不意味着真实视频与前沿高保真伪造视频之间的区别无法追踪。作者认为AI生成视频本质上是流形拟合过程的产物而非物理记录，因此其连续相邻帧的像素组成逻辑表现出结构化和同质化特征。

Method: 提出分层双路径框架作为顺序过滤过程：1) 静态流形偏差分支：利用大规模视觉基础模型的精细感知边界捕获偏离自然真实世界流形的空间异常或物理违规；2) 微时间波动分支：作为二级精细过滤器，分析即使在视觉完美序列中仍存在的结构化流形投影波动。

Result: 该框架能够检测伪造视频，无论其表现为全局真实世界流形偏差还是微小的计算指纹，确保即使成功驻留在流形上并逃避空间检测的高保真视频也能被识别。

Conclusion: AI生成视频本质上是流形拟合过程的产物，其像素组成逻辑表现出结构化和同质化特征（流形投影波动）。通过分层双路径框架分析这些特征，可以有效区分真实视频和高质量AI生成视频，即使后者在视觉上已非常逼真。
...
Result: 在合成数据集实验中，现有mask监督的NeRF在密集场景中实例恢复率约89%，而提出的SVRaster方法达到95.8%恢复率。在分割掩码退化情况下，显式几何方法比隐式基线多恢复43%的实例。

Conclusion: 显式几何先验是高度自遮挡3D场景中可靠定量分析的前提条件，能够有效克服隐式密度场在遮挡下的内部几何退化问题。
...
Method: 1. 使用多模态大语言模型将单模态数据集转换为多模态数据集，生成合成图像描述；2. 设计包含类别标签和领域上下文的提示来生成高质量分类任务描述；3. 引入监督对比损失函数，在微调期间显式鼓励同类表示聚类；4. 提出新的推理技术，利用每张图像多个合成描述的类别平均文本嵌入。

Result: 在13个图像分类基准测试上的广泛实验表明，该方法优于基线方法，在少样本学习场景中改进尤为显著。

Conclusion: 该工作建立了一个新的数据集增强范式，有效弥合了多模态预训练与微调之间的差距，为利用多模态预训练表示提供了有效途径。
...
TL;DR: Spava是一个序列并行框架，通过优化的注意力机制在多GPU上加速长视频推理，解决了大型多模态模型预填充阶段计算密集的瓶颈问题。
...
Method: 提出Spava序列并行框架，通过在多GPU上分布近似注意力，减少计算并增加并行性，无需压缩即可高效处理更多视觉嵌入。系统级优化包括负载均衡和融合前向传播。

Result: 相比FlashAttn、ZigZagRing和APB，Spava分别实现了12.72倍、1.70倍和1.18倍的加速，且没有明显的性能损失。

Conclusion: Spava通过序列并行和优化的注意力机制，有效解决了长视频推理的计算瓶颈，实现了显著的加速效果，同时保持了模型性能。
...
Method: 提出了VARIANCE（类内/类间方差）和GREEDINESS（活跃比率和梯度范数）诊断框架，比较了7种代表性损失函数：Contrastive、Triplet、N-pair、InfoNCE、ArcFace、SCL和CCL，在5个图像检索数据集上进行实验分析。

Result: Triplet和SCL能保持更高的类内方差和更清晰的类间边界，在细粒度设置中实现更强的top-1检索性能。Contrastive和InfoNCE通过许多小更新快速压缩嵌入，加速收敛但可能过度简化类别结构。N-pair实现了较大的平均分离但间距不均匀。

Conclusion: 研究揭示了效率与粒度之间的权衡：当需要保持多样性和困难样本区分时，应优先选择Triplet/SCL；当需要更快的嵌入压缩时，应选择Contrastive/InfoNCE。这些见解为实际应用提供了指导。
...
TL;DR: RT-DeepLoc是一个弱监督的时序深度伪造定位框架，通过重建误差识别伪造片段，使用仅用真实数据训练的MAE模型，并引入非对称视频内对比损失来增强定位性能。
...
Method: 提出基于重建的时序深度伪造定位框架RT-DeepLoc，使用仅用真实数据训练的掩码自编码器学习时空模式，通过重建差异识别伪造片段。引入非对称视频内对比损失，利用重建线索指导真实特征的紧凑性，建立稳定的决策边界。
...
TL;DR: SimGraph：一个统一的场景图驱动框架，将图像生成和编辑集成在一起，通过结构化场景图控制对象关系和空间布局，提升空间一致性和语义连贯性。


<details>
  <summary>Details</summary>
Motivation: 当前生成式人工智能在图像生成和编辑方面存在分离处理的问题，导致效率低下、空间一致性和语义连贯性难以保持，同时缺乏对对象关系和空间布局的结构化控制。

Method: 提出SimGraph统一框架，集成基于场景图的图像生成和编辑，结合基于token的生成和基于扩散的编辑方法，通过单一场景图驱动模型实现精确的对象交互、布局和空间一致性控制。

Result: 通过大量实验验证，该方法在图像生成和编辑任务上优于现有的最先进方法，能够产生高质量且一致的结果。

Conclusion: SimGraph框架成功解决了图像生成和编辑分离处理的问题，通过场景图提供结构化控制，实现了更好的空间一致性和语义连贯性，为生成式人工智能的图像处理提供了统一有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决All-in-One图像修复中不同退化类型之间的优化目标冲突问题。现有方法受限于粗粒度控制机制或固定映射调度，导致次优适应。

Method: 提出不确定性感知扩散桥模型(UDBM)：1) 将AiOIR重新定义为随机传输问题；2) 引入松弛扩散桥公式，用松弛约束替代严格终端约束；3) 设计双重调制策略：噪声调度对齐退化到共享高熵潜在空间，路径调度自适应调节传输轨迹。

Result: 通过有效修正传输几何和动力学，UDBM在单一推理步骤中实现了跨多种修复任务的最先进性能。

Conclusion: UDBM通过像素级不确定性引导的随机传输框架，解决了多任务图像修复中的优化冲突问题，为异构退化处理提供了理论保证和实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 全球水危机需要可负担、准确、实时的水质监测解决方案。传统的手动采样或昂贵的商业系统无法解决资源受限环境中的可访问性问题。

Method: 采用创新的物联网框架，集成pH、溶解氧、温度、总溶解固体、估算氮含量和水位六个参数。使用双微控制器架构：Arduino Uno用于精确模拟测量和五点校准算法，ESP32用于无线连接、边缘处理和云集成。应用先进的信号处理技术，包括TDS测量的中值滤波、温度补偿算法和鲁棒错误处理。
...
<details>
  <summary>Details</summary>
Motivation: 现有水印评估方法存在显著局限：缺乏残差和语义水印的统一评估框架、结果缺乏可解释性、忽视全面的安全考量、对语义水印使用不恰当的评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在病理学应用中存在局限性，通常直接输出诊断结论而缺乏可验证的证据链推理，这严重限制了临床信任度并阻碍专家纠错。需要开发具有透明推理能力的病理诊断系统。

Method: 1) 构建PathReasoner数据集：利用医学知识图谱开发严格的知识引导生成流程，将结构化病理发现和临床推理与诊断对齐，生成超过20K高质量教学样本；2) 开发PathReasoner-R1模型：结合轨迹掩码监督微调与推理导向强化学习，植入结构化思维链能力；3) 设计知识感知多粒度奖励函数，包含与知识图谱严格对齐的实体奖励机制。

Result: PathReasoner-R1在PathReasoner数据集和公共基准测试中均达到最先进性能，在不同图像尺度上表现优异，为病理模型提供了透明、临床基础的推理能力。

Conclusion: PathReasoner数据集和模型成功解决了病理诊断中缺乏可验证推理的问题，通过知识引导的数据生成和强化学习优化，实现了逻辑一致而非仅结果匹配的稳健推理，显著提升了临床可信度。
...
TL;DR: 该研究探讨不同视觉模型在训练过程中是否收敛到相同的中间表示和操作，而不仅仅是最终表示相似。通过量化不同模型在各处理阶段的表示距离，发现尽管相似位置的层具有最相似的表示，但模型之间仍存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明大型模型倾向于收敛到相似的"通用"表示，但尚不清楚它们是否通过相同的中间步骤和操作达到这些表示。本研究旨在探索不同视觉模型在达到收敛表示过程中的具体路径和机制。

Method: 首先量化不同模型在不同处理阶段的表示距离，然后跟踪表示距离在整个处理过程中的演变，识别模型间差异最大的处理步骤。特别关注分类器模型、CNN和transformer模型的行为差异。

Result: 发现相似位置的层具有最相似的表示，但模型间仍存在显著差异。分类器模型在最终层会丢弃低层图像统计信息；CNN和transformer模型行为不同，transformer模型在层间表示变化更平滑。

Conclusion: 这些发现阐明了模型表示收敛的层次和性质，使得能够对图像模型底层过程进行更定性的描述。研究揭示了不同架构模型在达到相似最终表示时采用的不同处理路径。
...
TL;DR: 论文指出在潜在扩散模型中，自编码器评估存在系统性偏差：过度关注生成友好性指标（如gFID）而忽视重建质量，这种偏差在可控生成任务中会导致条件漂移问题，影响可控性。


<details>
  <summary>Details</summary>
Motivation: 研究发现当前ImageNet规模的自编码器研究中存在系统性评估偏差，过度偏向生成友好性指标（如gFID），而重建质量指标被低估。这种偏差在扩展到可控扩散模型时会产生风险，因为自编码器可能引起条件漂移，限制条件对齐能力。
...
Result: 研究发现gFID仅能弱预测条件保持能力，而重建导向的指标与条件保持显著更相关。ControlNet实验证实可控性跟踪条件保持而非gFID。揭示了ImageNet中心的自编码器评估与可扩展可控扩散需求之间的差距。

Conclusion: 论文暴露了当前自编码器评估标准与可控扩散实际需求之间的不匹配，为更可靠的基准测试和模型选择提供了实践指导，强调需要平衡重建保真度和生成友好性，特别是在可控生成任务中。
...
Method: 提出RSGround-R1框架：1）使用合成遥感推理数据进行链式思维监督微调建立位置感知；2）应用强化微调并设计位置奖励提供连续距离感知指导；3）引入空间一致性引导优化方案确保稳定收敛

Result: 在遥感视觉定位基准测试中表现出优越性能和泛化能力

Conclusion: 通过推理引导、位置感知的后训练框架有效提升了多模态大语言模型在遥感场景中的空间理解和定位能力
...
TL;DR: OCRVerse是首个端到端的统一OCR方法，同时支持文本中心OCR（识别图像/扫描文档中的文字）和视觉中心OCR（识别图表、网页、科学图表等视觉元素），通过两阶段SFT-RL多域训练方法实现跨域融合。


<details>
  <summary>Details</summary>
Motivation: 现有OCR方法主要关注从图像或扫描文档中识别文本元素（文本中心OCR），而忽略了从图表、网页、科学图表等视觉信息密集的图像源中识别视觉元素（视觉中心OCR）。这类视觉信息密集的图像在互联网上广泛存在，在数据可视化和网页分析等领域具有重要应用价值。

Method: 1. 构建全面的数据工程，覆盖文本中心文档（报纸、杂志、书籍）和视觉中心渲染复合体（图表、网页、科学图表）；2. 提出两阶段SFT-RL多域训练方法：SFT阶段直接混合跨域数据训练建立初始域知识，RL阶段针对每个域的特点设计个性化奖励策略，为不同域定制灵活的奖励信号，提高跨域融合并避免数据冲突。

Result: 实验结果表明OCRVerse的有效性，在文本中心和视觉中心数据类型上都取得了有竞争力的结果，甚至可与大规模开源和闭源模型相媲美。

Conclusion: OCRVerse是首个端到端的统一OCR方法，成功解决了现有方法只关注文本识别而忽视视觉元素识别的问题，通过创新的训练策略实现了跨域融合，为处理视觉信息密集的图像提供了有效解决方案。
...
TL;DR: CAF-Mamba：基于Mamba的跨模态自适应注意力融合框架，用于抑郁症检测，通过显式和隐式捕捉跨模态交互并动态调整模态贡献，在LMVD和D-Vlog数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测的深度学习方法大多依赖有限特征类型，忽视显式跨模态交互，并采用简单拼接或静态加权融合，存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然现有冰川崩解前缘分割模型在基准测试中达到接近人类水平，但在实际新研究点应用时精度不足，无法满足后续科学分析需求，这主要是由于域偏移问题

Method: 采用少量样本域适应策略，结合空间静态先验知识，并在输入时间序列中包含夏季参考图像，无需修改模型架构

Result: 将新研究点的分割误差从1131.6米显著降低到68.7米，实现了从基准测试到实际应用的性能提升

Conclusion: 建立了一个将深度学习冰川崩解前缘分割应用于新研究点的框架，为实现全球尺度冰川崩解前缘监测提供了可行方案
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习旨在整合异构模态的互补信息，但单纯优化不能保证良好的表示结构。即使经过精心平衡的训练方案，多模态模型仍常出现几何病理，包括模态内表示坍缩和样本级跨模态不一致，这会降低单模态鲁棒性和多模态融合效果。

Method: 提出RegName框架，这是一种轻量级的几何感知正则化方法。它通过两种互补约束作用于中间嵌入：1) 模态内分散正则化，促进表示多样性；2) 模态间锚定正则化，在不强制刚性对齐的情况下限制样本级跨模态漂移。该方法即插即用，无需架构修改，兼容多种训练范式。

Result: 在多个多模态基准测试上的广泛实验表明，该方法在多模态和单模态性能上均带来一致改进，证明显式调节表示几何能有效缓解模态权衡问题。

Conclusion: 将表示几何作为多模态学习中的控制维度，通过几何感知正则化框架能有效解决模态内坍缩和跨模态不一致问题，提升模型整体性能。
...
TL;DR: ChartE³：首个端到端图表编辑基准，直接评估模型能力，无需中间自然语言或代码表示，包含1200+高质量样本，聚焦局部和全局编辑任务。


<details>
  <summary>Details</summary>
Motivation: 现有图表编辑方法多采用基于流水线的设计，依赖自然语言或代码作为中间表示，难以忠实执行复杂编辑任务，需要更直接的端到端评估基准。

Method: 提出ChartE³基准，包含两个互补编辑维度：局部编辑（字体、颜色等细粒度外观调整）和全局编辑（数据过滤、趋势线添加等整体数据转换）。通过精心设计的数据流水线和人工筛选构建了1200多个高质量样本，每个样本包含图表图像、底层代码和多模态编辑指令。

Result: 对最先进的多模态大语言模型进行广泛基准测试，发现存在显著的性能差距，特别是在全局编辑任务上，突显了当前端到端图表编辑能力的严重局限性。

Conclusion: ChartE³基准揭示了现有模型在端到端图表编辑方面的不足，特别是处理全局结构一致性编辑的能力，为未来研究提供了重要的评估框架和方向。
...
Method: 采用两阶段范式：1）通过将参考外观和运动线索融合到统一的潜在空间，弥合输入模态差距，利用基础模型的生成先验共同推理空间身份和时间动态；2）引入自引导数据合成管道，策划伪跨身份训练对，实现从姿态依赖控制到直接端到端RGB驱动动画的无缝过渡。

Result: DreamActor-M2实现了最先进的性能，提供卓越的视觉保真度和强大的跨域泛化能力。还引入了AW Bench基准测试，涵盖广泛的角色类型和运动场景。

Conclusion: DreamActor-M2通过重新构想运动条件为上下文学习问题，解决了现有角色动画方法的根本限制，实现了更好的身份保持与运动一致性平衡，并显著提升了跨不同角色和运动场景的泛化能力。
...
TL;DR: 该论文提出了一种新的图像质量评估（IQA）模型评价方法——粒度调制相关性（GMC），用于解决传统全局相关性指标（如PLCC和SRCC）无法捕捉局部质量谱上排名一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统IQA评估主要依赖PLCC和SRCC等全局相关性指标，这些指标将性能简化为单一标量，无法反映模型在不同质量区域（如高质量图像vs低质量图像）的排名一致性差异。此外，这些指标对测试样本质量分布敏感，导致跨测试集比较不稳定。

Method: 提出了粒度调制相关性（GMC）方法，包含两个核心组件：1）粒度调制器——基于绝对MOS值和成对MOS差异（|ΔMOS|）应用高斯加权相关性，以检查局部性能变化；2）分布调节器——正则化相关性以减轻非均匀质量分布带来的偏差。最终生成相关性曲面，将相关性值映射为MOS和|ΔMOS|的联合函数。

Result: 在标准基准测试上的实验表明，GMC能够揭示标量指标无法看到的性能特征，为分析、比较和部署IQA模型提供了更丰富和可靠的范式。

Conclusion: GMC提供了一种结构化、细粒度的IQA性能分析方法，通过相关性曲面提供3D性能表示，克服了传统全局指标的局限性，为IQA模型评估提供了更全面和稳定的框架。
...
TL;DR: DGNav提出动态拓扑导航框架，通过场景感知自适应策略和动态图Transformer解决VLN-CE中拓扑规划的"粒度刚性"问题，实现按需密度调整和拓扑噪声过滤。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑规划方法存在"粒度刚性"问题：依赖固定几何阈值采样节点，无法适应环境复杂性变化。这导致在简单区域过度采样造成计算冗余，而在高不确定性区域采样不足增加碰撞风险并降低精度。

Method: 提出DGNav框架，包含两个核心创新：1) 场景感知自适应策略，基于预测路径点离散度动态调整图构建阈值，在挑战性环境中实现"按需密集化"；2) 动态图Transformer，融合视觉、语言和几何线索为动态边权重，重建图连接性，过滤拓扑噪声。

Result: 在R2R-CE和RxR-CE基准测试中，DGNav展现出优越的导航性能和强大的泛化能力。消融研究证实该框架在导航效率和安全探索之间实现了最优权衡。

Conclusion: DGNav通过动态拓扑导航框架有效解决了VLN-CE中的"粒度刚性"问题，实现了环境自适应的地图密度和连接性调整，提升了导航精度和安全性。
...
Result: 基于MMFineReason微调的模型在其规模类别中达到新的SOTA。MMFineReason-4B超越Qwen3-VL-8B-Thinking，MMFineReason-8B甚至超越Qwen3-VL-30B-A3B-Thinking并接近Qwen3-VL-32B-Thinking。发现"少即是多"现象：仅7%样本（12.3万）就能达到完整数据集相当的性能。
...
Result: 无需训练，与现有扩散模型兼容，能够生成视觉连贯、前景保留的结果，在多页面复杂文档中保持风格一致性，无需重复基于提示的风格指定

Conclusion: 通过将扩散重新定义为潜在空间中的轨迹设计，提供了一种原理性的方法来实现一致且结构化的生成建模，为文档中心背景生成提供了几何和物理解释
...
TL;DR: 论文提出了一种基于优化视角的Classifier-free Guidance（CFG）理论解释，将CFG采样重新表述为具有流形约束的同伦优化问题，并通过流形投影和改进的梯度下降方案提高生成质量、提示对齐和稳定性。


<details>
  <summary>Details</summary>
Motivation: 尽管CFG在可控生成中广泛应用且经验上成功，但它依赖于启发式的线性外推方法，对引导尺度敏感。作者希望从优化角度为CFG提供理论解释，解决其启发式方法的局限性。

Method: 1. 从优化视角解释CFG：证明流匹配中的速度场对应一系列平滑距离函数的梯度；2. 将CFG采样重新表述为具有流形约束的同伦优化问题；3. 通过增量梯度下降实现流形投影；4. 使用Anderson Acceleration改进计算效率和稳定性，无需额外模型评估。

Result: 提出的方法是免训练的，能持续改进生成保真度、提示对齐和对引导尺度的鲁棒性。在DiT-XL-2-256、Flux和Stable Diffusion 3.5等大规模模型上的多样化基准测试中验证了有效性，展示了显著改进。

Conclusion: 该工作为CFG提供了理论基础，揭示了预测差距（条件与无条件输出之间的差异）控制引导敏感性的机制。通过优化视角重新表述CFG，开发了更稳定、高效的采样方法，提升了扩散和流模型的生成质量。
...
TL;DR: 提出PaFu-KV方法，通过轻量级显著性估计头来优化KV缓存策略，解决长视频生成中因忽略token重要性差异导致的关键时空信息丢失问题，在保持生成质量的同时提升推理效率。
...
Method: 提出PaFu-KV方法：1）观察到token贡献具有高度时间异质性；2）引入轻量级显著性估计头，从双向教师模型蒸馏学习显著性分数；3）基于显著性分数保留信息丰富的token，丢弃相关性较低的token，优化KV缓存容量和内存占用。

Result: 在基准测试上的大量实验表明，该方法在保持高保真视频生成质量的同时，实现了加速推理，为更高效的长时域视频生成提供了解决方案。

Conclusion: PaFu-KV方法通过智能KV缓存策略，在质量和效率之间取得了更好的权衡，解决了长视频生成中的关键挑战，代码将在论文接受后发布。
...
TL;DR: TraceRouter是一个路径级防御框架，通过追踪和断开有害语义的因果传播电路来增强大型基础模型对抗攻击的鲁棒性，相比现有基于"局部性假设"的方法更有效。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要依赖"局部性假设"，通过抑制孤立的神经元或特征来对抗攻击，但有害语义实际上是分布式、跨层的电路，这使得局部干预方法既脆弱又损害模型效用。

Method: TraceRouter采用三阶段路径级框架：1) 通过注意力差异分析定位敏感起始层；2) 使用稀疏自编码器和差异激活分析来解耦和隔离恶意特征；3) 通过基于零输出干预的特征影响分数将特征映射到下游因果路径，选择性抑制这些因果链。

Result: 大量实验表明，TraceRouter显著优于现有最先进的基线方法，在对抗鲁棒性和通用效用之间实现了更好的平衡。

Conclusion: TraceRouter通过物理上切断有害信息流同时保持正交计算路径完整，为大型基础模型提供了一种更有效的对抗防御解决方案。
...
TL;DR: 提出基于金字塔式Shapley-Taylor学习框架的细粒度运动-语言检索方法，通过渐进式关节级和片段级对齐，显著提升跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有运动-语言检索方法主要关注全局序列与文本的对齐，忽略了局部运动片段与身体关节、文本标记之间的细粒度交互，导致检索性能受限。
...
Conclusion: 提出的金字塔式细粒度对齐框架有效解决了运动-语言检索中的语义鸿沟问题，为跨模态智能分析提供了新思路。
...
TL;DR: VideoAesBench：首个全面评估大语言模型视频美学质量理解能力的基准，包含1804个多样化视频、多种问题格式和全面的美学维度，测试发现当前模型仅具备基本感知能力


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在各种视觉感知任务中表现出色，但其视频美学质量评估能力尚未得到充分探索，而这是人类的基本能力。需要建立一个全面的基准来评估模型在这方面的能力。

Method: 创建VideoAesBench基准，包含1804个来自多个来源的视频（用户生成、AI生成、压缩、机器人生成、游戏视频），采用多种问题格式（单选、多选、判断、开放式描述），涵盖视觉形式、视觉风格、视觉感染力三个维度的12个美学方面。

Result: 对23个开源和商业大语言模型进行基准测试，发现当前模型仅具备基本的视频美学感知能力，其表现仍然不完整且不精确。
...
TL;DR: 提出首个利用视频扩散模型辅助图像方法实现零样本视频恢复与增强的框架，解决时间一致性闪烁问题


<details>
  <summary>Details</summary>
Motivation: 基于扩散的零样本图像恢复和增强方法在视频应用中会导致严重的时间闪烁问题，需要保持时间一致性

Method: 提出同源潜在融合、异源潜在融合和COT融合比策略，利用文本到视频扩散模型补充图像方法；提出时间强化后处理，利用图像到视频扩散模型进一步提升时间一致性

Result: 实验结果表明所提方法的优越性，能够有效解决时间闪烁问题

Conclusion: 该方法无需训练，可应用于任何基于扩散的图像恢复和增强方法，是首个利用视频扩散模型辅助图像方法保持时间一致性的框架
...
TL;DR: FeatJND：一种面向任务的深度视觉特征JND（恰可察觉差异）方法，用于预测保持下游任务性能的最大可容忍特征扰动，并在分类、检测和分割任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 深度视觉特征作为视觉系统接口越来越普遍，需要描述特征特性并控制特征质量。将JND概念扩展到深度特征空间，可以为机器感知提供任务对齐的容忍边界，在资源受限条件下控制特征质量。

Method: 提出FeatJND方法，预测保持下游任务性能的最大可容忍每特征扰动图；在标准化分割点构建FeatJND估计器；在图像分类、检测和实例分割任务中验证；应用于token-wise动态量化。

Result: 在相同失真强度下，FeatJND失真比非结构化高斯扰动能更好地保持任务性能；归因可视化显示FeatJND能抑制非关键特征区域；在动态量化应用中，FeatJND引导的步长分配优于随机步长排列和全局均匀步长。

Conclusion: FeatJND为深度视觉特征提供了任务对齐的JND表述，能够有效预测保持任务性能的最大特征扰动，在特征质量控制和资源优化方面具有实用价值。
...
TL;DR: BookNet是首个专为双页书籍图像校正设计的端到端深度学习框架，通过双分支架构和跨页注意力机制，能够同时估计单页和整本书页的扭曲流，显著优于现有方法。
...
Method: BookNet采用双分支架构，配备跨页注意力机制，能够同时估计单页和整本书页的扭曲流，明确建模左右页之间的相互影响。为支持训练和评估，还创建了Book3D合成数据集和Book100真实世界基准。

Result: 大量实验表明，BookNet在书籍图像校正方面优于现有的最先进方法。

Conclusion: BookNet是首个专门针对双页书籍图像校正的深度学习框架，通过建模页面间的耦合关系，显著提升了校正效果，并提供了专门的数据集支持该领域的研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏有效评估统一模型（能同时生成图像和文本）性能的基准测试，传统的LLM-as-a-judge方法难以捕捉多模态生成的细微差别，需要更精细的评估框架。

Method: UEval包含1000个专家策划的问题，要求模型输出图像和文本。采用基于量规的评分系统：首先由MLLM根据参考图像和文本生成初始评估量规，然后由人类专家进行细化和验证，最终包含10,417个已验证的量规标准。

Result: 当前统一模型在UEval上表现不佳：GPT-5-Thinking仅得66.4分（满分100），最佳开源模型仅得49.1分。研究发现推理模型通常优于非推理模型，将推理轨迹从推理模型转移到非推理模型能显著缩小性能差距。

Conclusion: UEval是一个具有挑战性的统一模型评估基准，结果表明推理能力对于需要复杂多模态理解和生成的任务至关重要，为未来统一模型的发展提供了重要评估工具。
...
Method: 1) 升级PaddleOCR-VL-1.5模型；2) 提出Real5-OmniDocBench基准测试，专门评估五种物理失真情况；3) 扩展模型功能，加入印章识别和文本检测任务；4) 保持0.9B参数的超紧凑视觉语言模型架构。

Result: 1) 在OmniDocBench v1.5上达到94.5%的SOTA准确率；2) 在新提出的Real5-OmniDocBench基准测试上也取得SOTA性能；3) 模型保持高效，同时增加了印章识别和文本检测能力。

Conclusion: PaddleOCR-VL-1.5在文档理解任务中实现了新的SOTA性能，并通过Real5-OmniDocBench基准测试为物理失真鲁棒性评估提供了新标准，同时以紧凑架构扩展了功能范围。
...
<details>
  <summary>Details</summary>
Motivation: 视频世界建模结合视觉语言预训练为机器人学习提供了新的基础，视频世界模型能够通过理解动作与视觉动态之间的因果关系来预测近未来。

Method: 提出LingBot-VA自回归扩散框架，包含三个关键设计：1) 共享潜在空间，整合视觉和动作token，采用混合Transformer架构；2) 闭环rollout机制，持续获取环境反馈和真实观测；3) 异步推理管道，并行化动作预测和运动执行以实现高效控制。

Result: 在仿真基准和真实世界场景中评估，模型在长时程操作、后训练数据效率和新型配置泛化能力方面表现出显著潜力。

Conclusion: 视频世界建模与视觉语言预训练为机器人学习建立了新的独立基础，LingBot-VA框架展示了在机器人控制任务中的有效性和潜力。
...
Method: 1. 将V-JEPA适配到端到端驾驶，在大规模驾驶视频上预训练ViT编码器以产生与轨迹规划对齐的预测表示；2. 引入以提议为中心的规划器，蒸馏模拟器生成的多条轨迹与人类轨迹，采用动量感知选择机制促进稳定安全行为
...
TL;DR: PLANING是一个高效的在线重建框架，通过混合表示松散耦合显式几何基元和神经高斯，实现几何与外观解耦建模，在保持高质量渲染的同时获得准确几何，重建速度比2D高斯泼溅快5倍以上。
...
Result: PLANING在稠密网格Chamfer-L2上比PGSR提升18.52%，PSNR比ARTDECO高1.31 dB，ScanNetV2场景重建时间小于100秒，比2D高斯泼溅快5倍以上，同时达到离线逐场景优化的质量。

Conclusion: PLANING不仅重建质量高，其结构清晰性和计算效率使其适用于大规模场景建模和具身AI的仿真就绪环境等广泛下游应用。
...
<details>
  <summary>Details</summary>
Motivation: 尽管缩放推动了视觉基础模型的进展，但将其扩展到度量深度估计仍面临挑战，包括异构传感器噪声、相机相关偏差以及跨源3D数据中的度量模糊性。需要一种能够从噪声多样的3D数据中学习度量深度的方法。

Method: 提出稀疏度量提示方法，通过随机掩码深度图创建通用接口，将空间推理与传感器和相机偏差解耦。使用约2000万张图像-深度对，涵盖重建、捕获和渲染的3D数据，跨越10000个相机模型。

Result: 首次展示了度量深度领域的清晰缩放趋势。预训练模型在深度补全、超分辨率和雷达-相机融合等提示驱动任务中表现出色，其蒸馏的无提示学生模型在单目深度估计、相机内参恢复、单/多视角度量3D重建和VLA规划中达到最先进水平。作为视觉编码器还能显著提升多模态大语言模型的空间智能能力。

Conclusion: 度量深度估计能够受益于驱动现代基础模型的相同缩放定律，为可扩展和高效的真实世界度量感知开辟了新路径。
...
TL;DR: 本文提出了一种通过对抗训练改进扩散模型中无监督因子分解的方法，使用判别器区分单源样本与跨源因子重组样本，从而提升因子发现和组合生成质量。
...
Method: 引入对抗训练信号：训练一个判别器来区分单源样本和跨源因子重组生成的样本。通过优化生成器来欺骗这个判别器，从而鼓励重组结果在物理和语义上的一致性。该方法应用于扩散模型，在图像和机器人视频数据上进行验证。

Result: 在CelebA-HQ、Virtual KITTI、CLEVR和Falcor3D数据集上优于先前基线方法，获得了更低的FID分数和更好的解缠结性能（通过MIG和MCC衡量）。在机器人视频轨迹应用中，通过重组学习到的动作组件，生成了显著增加状态空间覆盖范围的多样化序列。

Conclusion: 提出的对抗训练方法有效提升了无监督因子分解的质量和组合生成能力，在多个视觉数据集上表现出色，并展示了在机器人视频轨迹生成中的创新应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要大量事实信息的任务上存在局限性，通常采用简单的"推理-工具调用"模式，假设单一图像查询和少量文本查询就能获取关键证据，这在现实场景中存在视觉噪声的情况下是不现实的。此外，现有方法在推理深度和搜索广度上受限，难以解决需要聚合多样化视觉和文本证据的复杂问题。

Method: 提出Vision-DeepResearch，采用多轮、多实体、多尺度的视觉和文本搜索策略，能够在重度噪声下稳健地使用真实搜索引擎。该方法支持数十个推理步骤和数百个引擎交互，通过冷启动监督和强化学习训练将深度研究能力内化到MLLM中，构建端到端的多模态深度研究MLLM。

Result: Vision-DeepResearch显著优于现有的多模态深度研究MLLMs，以及在GPT-5、Gemini-2.5-pro和Claude-4-Sonnet等强大闭源基础模型上构建的工作流程。

Conclusion: Vision-DeepResearch通过创新的多模态深度研究范式，有效解决了现有MLLMs在复杂现实场景中的局限性，为多模态大语言模型在需要深度研究和证据聚合的任务上提供了新的解决方案。
...
TL;DR: BLO-Inst是一个通过双层优化统一检测和分割目标的框架，解决SAM依赖手动提示和现有自动化方法中目标不匹配、对齐过拟合的问题。


<details>
  <summary>Details</summary>
Motivation: SAM虽然具有零样本分割能力，但依赖手动提示阻碍了全自动部署。现有使用目标检测器作为提示生成器的自动化方法存在两个根本限制：1）目标不匹配 - 检测器优化的几何定位目标与SAM所需的最佳提示上下文不对应；2）对齐过拟合 - 标准联合训练中检测器只是记忆特定训练样本的提示调整，而非学习可泛化的策略。

Method: 提出BLO-Inst框架，通过双层优化对齐检测和分割目标。将对齐问题形式化为在不相交数据分割上的嵌套优化问题：下层在子集D1上微调SAM以最大化给定当前检测提议的分割保真度；上层在单独子集D2上更新检测器，使其生成明确最小化微调SAM验证损失的边界框。这使检测器成为分割感知的提示生成器，不仅优化定位精度，还优化下游掩码质量。

Result: 大量实验表明，BLO-Inst在一般和生物医学领域的任务上都实现了卓越性能，超越了标准基线方法。

Conclusion: BLO-Inst通过双层优化有效解决了检测器与SAM之间的目标不匹配问题，将检测器转化为分割感知的提示生成器，实现了更优的自动化分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于参考的图像生成方法仅限于单图像参考，无法利用3D资产，限制了实际应用多样性。需要开发能够整合3D资产到图像扩散模型的方法。

Method: 采用跨域扩散模型与双分支感知架构，利用3D资产的多视角RGB图像和点云图联合建模颜色和规范空间坐标。通过空间对齐的双分支生成架构和域解耦生成机制，同时生成空间对齐但内容解耦的RGB图像和点云图。

Result: 实验表明该方法能有效利用3D资产作为参考，生成与给定资产一致的图像，为扩散模型与3D内容创作的结合开辟了新可能性。

Conclusion: 提出的3D资产参考扩散模型成功解决了现有方法无法利用3D资产的限制，通过双分支感知和跨域建模实现了2D图像属性与3D资产属性的关联，为3D内容创作与扩散模型的结合提供了新途径。
...
Method: SINA集成了深度学习进行准确的元件检测、连通域标记(CCL)进行精确的连接性提取、光学字符识别(OCR)获取元件参考标识符，并使用视觉语言模型(VLM)进行可靠的参考标识符分配。
...
<details>
  <summary>Details</summary>
Motivation: 现有创意图像生成方法通常依赖手动混合概念或排除子类别，缺乏系统性、原则性的创造力定义和量化方法。需要一种能够自动生成新颖、高质量、富有想象力图像的新框架。

Method: 提出基于扩散模型的创意生成框架：1) 将创造力定义为CLIP嵌入空间中图像存在概率的倒数；2) 计算生成图像的概率分布并驱动其向低概率区域移动；3) 引入回拉机制，在保持高创造力的同时不牺牲视觉保真度。

Result: 在文本到图像扩散模型上的大量实验表明，该框架能够有效且高效地生成独特、新颖、发人深省的图像，在创造力和视觉质量之间取得了良好平衡。

Conclusion: 该工作为生成模型中的创造力提供了新视角，提供了一种原则性的方法来促进视觉内容合成的创新，推动了创意图像生成领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型擅长从文本和图像提示生成新内容，但在编辑现有预录制视频方面存在关键空白，特别是当需要修改口头脚本时，必须保持运动、时间一致性、说话人身份和准确的唇部同步。

Method: 基于通用视频扩散模型，EditYourself通过音频条件化和区域感知、编辑聚焦的训练扩展来增强其视频到视频（V2V）能力。采用时空修复技术实现精确的唇部同步和时间一致的重组，包括在新添加的片段中合成逼真的人类运动。

Result: 该框架能够实现精确的唇部同步和时间一致的重组现有表演，包括在新添加的片段中合成逼真的人类运动，同时在长时间内保持视觉保真度和身份一致性。

Conclusion: 这项工作代表了生成视频模型作为专业视频后期制作实用工具的基础性一步，为基于脚本的视频编辑提供了创新解决方案。
...
TL;DR: 开发了名为ePAI的AI系统，用于早期检测胰腺导管腺癌（PDAC），在内部测试中AUC达0.939-0.999，外部测试中AUC达0.918-0.945，能检测到直径小至2-5mm的病灶，并在临床诊断前3-36个月识别出放射科医生漏诊的病变。


<details>
  <summary>Details</summary>
Motivation: 胰腺导管腺癌（PDAC）是最致命的实体恶性肿瘤之一，通常在晚期和不可手术阶段才被发现。回顾性研究发现，当专家放射科医生知道患者后来发展为PDAC时，经常能在之前的CT扫描中发现被忽视的病变。因此需要开发自动化系统来帮助更早地检测这些病变。

Method: 开发了名为ePAI（早期胰腺癌检测人工智能）的自动化系统，使用来自单个医疗中心的1,598名患者数据进行训练。系统在内部测试（1,009名患者）和外部测试（6个中心的7,158名患者）中进行评估，并与30名认证放射科医生进行多读者研究比较。

Result: 内部测试中，ePAI对小于2cm的PDAC检测AUC为0.939-0.999，敏感性95.3%，特异性98.7%，能精确定位小至2mm的病灶。外部测试中AUC为0.918-0.945，敏感性91.5%，特异性88.0%，能定位小至5mm的病灶。ePAI在临床诊断前3-36个月的CT扫描中检测到放射科医生漏诊的PDAC，在159名患者中成功检测到75例，中位提前时间为347天。多读者研究显示ePAI敏感性显著优于30名放射科医生50.3%，同时保持95.4%的可比特异性。

Conclusion: ePAI系统在早期检测胰腺癌方面表现出卓越性能，能够识别放射科医生漏诊的微小病变，并在临床诊断前数月检测到PDAC。这些发现表明ePAI有潜力作为辅助工具改善胰腺癌的早期检测。
...
TL;DR: π-Light是一个两阶段物理启发扩散框架，用于全图像重照明，通过批量感知注意力、物理引导神经渲染和物理启发损失，在有限数据下实现物理合理的光传输和真实场景泛化。


<details>
  <summary>Details</summary>
Motivation: 解决全图像重照明面临的三大挑战：大规模结构化配对数据收集困难、物理合理性难以保持、数据驱动先验导致的泛化能力有限。现有方法在缩小合成到真实场景的差距方面仍不理想。

Method: 提出两阶段物理启发扩散框架：1) 批量感知注意力提升跨图像内在预测一致性；2) 物理引导神经渲染模块强制物理合理的光传输；3) 物理启发损失正则化训练动态；4) 精心策划的多样化对象和场景数据集。

Result: π-Light能够合成各种材质上的镜面高光和漫反射，相比先前方法在真实场景泛化方面表现更优，为下游评估提供了可靠的基准。

Conclusion: 该框架通过物理启发组件实现了高效微调预训练扩散模型，在有限数据下实现了物理合理的光传输和优秀的真实场景泛化能力。
...
TL;DR: 本文通过VI-Probe框架系统研究大视觉语言模型对视觉错觉的感知能力，发现模型对错觉图像的反应持续性源于多种机制而非单一原因


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在原始错觉图像上能"正确"回答，但在错觉因素反转后仍坚持相同回答，尽管视觉变化对人类显而易见。这引发了一个根本问题：VLMs是感知视觉变化还是仅仅回忆记忆模式？现有研究虽注意到此现象，但根本原因尚不清楚

Method: 引入VI-Probe框架，包含分级扰动和匹配的视觉控制（无错觉诱导因素），通过极性翻转一致性、模板固定指数和相对于匹配控制的错觉乘数等指标，分离视觉基础感知与语言驱动回忆

Result: 实验发现不同模型家族的反应持续性源于异质原因：GPT-5表现出记忆覆盖，Claude-Opus-4.1显示感知-记忆竞争，而Qwen变体则暗示视觉处理限制。这些发现挑战了单一原因观点

Conclusion: 研究结果表明需要基于探测的评估方法，同时测量模型的知识和对受控视觉变化的敏感性，而非仅关注平均准确率
...
<details>
  <summary>Details</summary>
Motivation: 改进语音识别评估方法，特别是针对非拉丁语言、构词丰富的语言以及杂乱或长语音的标注问题。同时解决模型可能适应特定数据集标注而导致度量改进假象的问题。

Method: 1. 提出支持多参考标注、任意长度插入和更好词对齐的字符串对齐算法；2. 收集DiverseSpeech-Ru俄语长语音数据集并进行多参考标注；3. 对流行的俄语测试集进行多参考重新标注；4. 开发流式语音识别评估工具和多转录对齐可视化工具；5. 提供离线与流式语音识别模型的统一封装。

Result: 展示了模型经常适应数据集特定标注，导致度量改进的假象。基于改进的词对齐，开发了评估流式语音识别和比较多个转录的工具，并提供了统一模型封装。

Conclusion: 该研究提供了改进的语音识别评估框架，特别适用于非拉丁语言和长语音场景，有助于更准确地评估模型性能，避免数据集特定标注带来的评估偏差。
...
TL;DR: 研究发现SSL模型微调时MSE损失会过度利用位置嵌入而非内容信息，提出两种位置不变微调策略，其中soft-DTW方法在收敛速度和下游任务性能上表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前将前端语音增强模型与基于自监督学习的语音模型结合时，通常使用MSE损失在增强语音和干净语音之间进行微调。但MSE容易利用SSL模型中的位置嵌入，导致目标函数通过位置相关性而非内容相关信息最小化，这是一个普遍的自监督表示微调限制。

Method: 研究了两种位置不变微调策略：1）零填充（在SSL预训练中已有探索，但在微调场景下研究）；2）速度扰动结合soft-DTW损失。通过表示引导的语音增强框架来探讨这一问题。

Result: 实验表明，基于soft-DTW的方法实现了更快的收敛速度和改进的下游任务性能，突显了在基于SSL的语音建模中进行位置不变微调的重要性。

Conclusion: 位置不变微调对SSL语音模型至关重要，soft-DTW方法相比传统MSE损失能更好地避免位置嵌入的过度利用，提升模型对内容信息的关注，从而改善下游任务表现。
...
TL;DR: ChunkWise LoRA：一种动态自适应低秩适配方法，通过基于令牌复杂度划分序列为变长块，为每个块分配定制化的低秩配置，显著降低延迟和内存使用
...
Method: 提出ChunkWise LoRA：1)运行时调度器估计令牌难度并进行自适应分块；2)使用秩梯机制为每个块选择LoRA秩和缩放；3)边界安全组合模块保持输出一致性；4)集成策略驱动的KV缓存策略

Result: 在Wikitext-103和SQuAD等基准数据集上，相比基线LoRA实现了34%延迟降低和38%内存减少，同时保持或改进了BLEU、EM和困惑度等任务性能指标
...
TL;DR: 比较数据混合与模型合并两种创建小型多任务代码LLM的方法，发现7B规模下模型合并效果最佳，2B规模下数据混合更优，最佳合并模型在HumanEval上达到92.7% Pass@1


<details>
  <summary>Details</summary>
Motivation: 当前研究提倡在智能体框架中部署小型专业化代码LLM，需要探索高效的多任务学习策略以平衡性能、约束和成本

Method: 比较数据混合与模型合并两种方法，在Qwen Coder和DeepSeek Coder两个模型系列的2B和7B规模上进行实验，针对代码生成和代码摘要任务进行微调，使用HumanEval、MBPP和CodeXGlue基准进行评估，并引入权重分析技术理解不同任务对模型参数的影响

Result: 模型合并在大规模（7B）下表现最佳，在代码生成任务上保留96%的专业模型性能，同时保持摘要能力；最佳配置的Qwen Coder 2.5 7B模型在HumanEval上达到92.7% Pass@1，优于任务特定微调模型的90.9%；小规模（2B）下数据混合是更优策略
...
TL;DR: LLM可从姓名推断种族，准确率超传统BISG方法，无需额外训练数据，适用于美国以外地区，准确率达84.7%，减少收入偏见，验证多国数据集有效


<details>
  <summary>Details</summary>
Motivation: 传统种族推断方法如BISG存在局限性：仅适用于美国，需要额外训练数据，对富裕社区少数族裔存在系统性误分类偏见。需要一种更通用、准确且无偏见的种族推断方法

Method: 使用大型语言模型（包括Gemini 3 Flash、GPT-4o、DeepSeek v3.2、GLM-4.7等6个模型）从姓名推断种族/民族。采用分层抽样验证，比较LLM与BISG准确率。测试扩展推理能力提升效果，加入党派注册等元数据。进一步在黎巴嫩、印度等多国数据集验证，并微调小型Transformer模型用于大规模部署

Result: LLM分类在平衡样本上达到84.7%准确率，显著优于BISG的68.2%。扩展推理可提升1-3个百分点，加入元数据后达86.7%。LLM减少了BISG的收入偏见。在黎巴嫩宗教派别分类达64.3%，印度保留选区议员达99.2%，种姓分类达74.0%。多国验证显示方法能恢复已知人口分布。微调后的小模型超越BISG准确率且可本地部署

Conclusion: LLM提供了一种超越传统BISG的种族推断方法，具有更高准确性、更少偏见、更广泛适用性（包括美国以外地区），且可通过微调小型模型实现低成本大规模部署
...
Method: EnsembleLink利用预训练语言模型学习语义关系（如地理位置、组织名称等），无需任何训练标签，使用开源模型在本地运行，无需外部API调用，能在几分钟内完成典型链接任务。

Result: 在涵盖城市名称、人名、组织、多语言政党和书目记录等多个基准测试中，EnsembleLink的表现与需要大量标注的方法相当或更优。

Conclusion: EnsembleLink提供了一种无需标注数据的高精度记录链接解决方案，解决了社会科学研究中记录链接方法不足的问题，能够量化链接不确定性并提高下游分析的可靠性。
...
TL;DR: OS-Search将LLM生成转化为端点搜索，通过在冻结编码器定义的3D输出空间中选择目标点，使用检索增强策略生成接近该目标的输出，实现并行扫描和黑盒优化。


<details>
  <summary>Details</summary>
Motivation: 传统LLM生成存在路径依赖的token/程序搜索问题，无法进行高效的并行扫描和黑盒优化。作者希望开发一种方法，能够在输出空间中进行系统性的探索和优化，提高生成多样性和目标导向性。

Method: 提出输出空间搜索(OS-Search)：1) 在冻结编码器定义的3D输出空间Z中选择目标点z*；2) 使用基于检索的策略，通过序列级强化学习训练，生成坐标接近z*的输出；3) 支持在Z空间进行并行扫描和贝叶斯优化，避免路径依赖的token搜索。

Result: 在故事生成中，在Z空间扫描比提示链方法获得3.1倍的LLM评分多样性提升；在代码生成中，对Z空间进行贝叶斯优化，在匹配推理预算下改进了控制器未知的目标函数，同时保持代码有效性。

Conclusion: OS-Search将LLM生成重新定义为输出空间搜索问题，实现了高效的并行探索和黑盒优化，在文本和代码生成任务中展示了显著优势，为可控生成提供了新范式。
...
TL;DR: 本文提出嵌入缩放作为混合专家架构的替代稀疏扩展维度，通过系统分析发现嵌入缩放在特定场景下比专家缩放具有更好的帕累托前沿，并开发了LongCat-Flash-Lite模型验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 混合专家架构作为大型语言模型稀疏扩展的标准方法，面临收益递减和系统级瓶颈问题。作者探索嵌入缩放作为正交的稀疏扩展维度，以解决现有方法的局限性。

Method: 通过全面分析和实验，识别嵌入缩放优于专家缩放的具体场景；系统表征影响效能的架构因素（参数预算、模型宽度与深度交互）；结合定制系统优化和推测解码，将稀疏性转化为实际推理加速；开发LongCat-Flash-Lite模型（685亿参数，约30亿激活参数）验证方法。

Result: 嵌入缩放在特定场景下比专家缩放具有更好的帕累托前沿；LongCat-Flash-Lite模型在分配超过300亿参数给嵌入的情况下，不仅超越了参数相当的混合专家基线，还在智能体和编码领域表现出与同规模现有模型的卓越竞争力。

Conclusion: 嵌入缩放是混合专家架构的有效替代稀疏扩展维度，通过系统优化可以转化为实际推理加速，为大型语言模型扩展提供了新的方向。
...
TL;DR: 提出一个多语言音素产出评估框架，结合通用音素识别和语言特定音素解释，用于跨语言的构音障碍语音可懂度评估
...
Method: 提出多语言音素产出评估框架，整合通用音素识别与语言特定音素解释，使用对比音系特征距离进行音素到音位映射和序列对齐。框架产生三个指标：音素错误率(PER)、音系特征错误率(PFER)和新提出的无对齐度量音素覆盖度(PhonCov)。

Result: 在英语、西班牙语、意大利语和泰米尔语上的分析表明：PER受益于映射和对齐的组合，PFER仅受益于对齐，PhonCov受益于映射。进一步分析显示该框架能捕捉与构音障碍语音已知观察一致的临床有意义可懂度退化模式。

Conclusion: 该多语言音素产出评估框架能够有效评估跨语言的构音障碍语音可懂度，捕捉语言特定的音系特征，为临床评估提供有价值的自动化工具。
...
TL;DR: 该论文研究了大型语言模型在推理步骤泛化中的性能下降问题，发现错误集中在少数关键错误类型的token位置，并提出了一种轻量级的推理时校正方法。


<details>
  <summary>Details</summary>
Motivation: 尽管链式思维推理已成为LLMs解决复杂问题的标准范式，但研究发现当所需推理步骤超出训练分布时，模型性能会急剧下降。这种推理步骤泛化失败的内部机制尚不清楚，需要深入探究。

Method: 通过对多领域任务的系统研究，发现错误集中在特定token位置而非均匀分布。进一步分析揭示这些错误源于内部竞争机制：某些注意力头（称为错误处理头）通过放大错误推理轨迹并抑制正确轨迹来影响结果。基于此，提出了推理时校正方法，动态识别并停用推理过程中的错误处理头。

Result: 实验表明，移除单个错误处理头通常能恢复正确预测。提出的推理时校正方法在不同任务和LLMs上都能一致地改善推理步骤泛化性能。

Conclusion: 该研究揭示了LLMs推理步骤泛化失败的内部机制，并提出了一种有效的轻量级干预方法，为理解和改进LLMs的推理能力提供了新视角。
...
TL;DR: 该论文提出了一个更稳健的大语言模型诚实性评估基准数据集，并利用Pythia模型的公开预训练数据开发了一种提高LLM诚实性的新方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然擅长回答问题，但往往不清楚自己的知识边界，导致在缺乏足够知识的主题上产生事实错误的回答（幻觉）。现有提高LLM诚实性的方法评估不够稳健，因为它们没有考虑LLM在预训练阶段已经吸收的知识。

Method: 1. 利用Pythia（一个真正开源且预训练数据公开可用的LLM）构建更稳健的LLM诚实性评估基准数据集；2. 提出一种利用预训练数据构建更诚实LLM的新方法。

Result: 论文提出了一个更稳健的评估基准，能够更好地评估LLM的诚实性，同时开发了一种利用预训练数据提高模型诚实性的新方法。

Conclusion: 通过利用公开可用的预训练数据，可以构建更稳健的LLM诚实性评估基准，并开发更有效的技术来提高语言模型的诚实性，使其在缺乏足够知识时能够诚实地回答"我不知道"。
...
TL;DR: SHARP框架通过多维度、分布感知的方法评估LLMs的社会危害风险，重点关注最坏情况行为，发现相似平均风险的模型在尾部风险和波动性上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs评估基准通常将复杂的社会风险简化为均值中心的标量分数，掩盖了分布结构、跨维度交互和最坏情况行为，无法全面评估高风险领域部署中的潜在危害。

Method: 提出SHARP框架，将危害建模为多元随机变量，整合偏见、公平性、伦理和认知可靠性的显式分解，采用风险敏感的分布统计方法，以条件风险价值(CVaR95)为主要指标，评估901个社会敏感提示下的11个前沿LLMs。

Result: 相似平均风险的模型在尾部风险和波动性上存在超过两倍的差异；不同危害维度的尾部行为呈现系统性差异：偏见维度尾部严重性最强，认知和公平风险居中，伦理错配风险最低；揭示了标量基准无法区分的异质性、模型依赖的失败结构。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型协作研究分散在不同研究社区，缺乏系统比较和统一框架，需要整合现有研究并建立模型协作作为一个学派

Method: 开发MoCo Python库，集成26种模型协作方法（路由、文本、logit、模型参数等不同信息交换层级），整合25个评估数据集，支持用户自定义数据

Result: 大多数协作策略在61.0%的（模型，数据）设置中优于无协作模型，最有效方法提升达25.8%；分析了协作策略的扩展性、训练/推理效率

Conclusion: MoCo作为工具包促进开放、模块化、去中心化和协作的AI未来，协作系统能解决单个语言模型难以处理的问题
...
TL;DR: Qwen3-ASR系列包含两个全功能语音识别模型和一个非自回归语音强制对齐模型，支持多语言识别，在开源模型中达到SOTA性能，并具有优秀的效率表现。


<details>
  <summary>Details</summary>
Motivation: 现有ASR模型在开源基准测试中差异不大，但在实际场景中质量差异显著，需要更全面的评估。同时，社区需要更高效、多功能的语音识别和对齐工具来加速音频理解研究。

Method: 基于Qwen3-Omni基础模型的强大音频理解能力，利用大规模语音训练数据开发了Qwen3-ASR-1.7B和Qwen3-ASR-0.6B两个ASR模型，支持52种语言和方言的识别。同时开发了基于LLM的非自回归时间戳预测模型Qwen3-ForcedAligner-0.6B，用于11种语言的文本-语音对齐。

Result: 1.7B版本在开源ASR模型中达到SOTA性能，与最强专有API竞争；0.6B版本提供最佳精度-效率权衡，平均TTFT低至92ms，并发128时1秒可转录2000秒语音。强制对齐模型在时间戳精度上优于三个最强对齐模型，效率和通用性更优。

Conclusion: Qwen3-ASR系列模型在性能、效率和多功能性方面表现出色，填补了开源ASR模型在实际场景中的质量差距。通过Apache 2.0许可证发布这些模型，将加速ASR和音频理解领域的社区研究。
...
TL;DR: 提出一种新的预训练方法，使用强化学习在文档流中改进模型生成的下K个token，通过后训练模型评估生成质量、安全性和事实性，从底层构建更高质量、更安全、更真实的模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成内容时存在安全性、事实性和质量方面的挑战。现有方法依赖昂贵的数据集收集和多阶段微调对齐，但无法纠正预训练阶段学习到的模式。因此需要在预训练阶段就解决这些问题，防止不安全或幻觉输出被深度嵌入模型行为中。

Method: 提出新的预训练方法：流式处理文档，使用强化学习改进每个步骤中生成的下K个token。通过一个强大的后训练模型评估候选生成（包括模型rollout、原始后缀和重写后缀）的质量、安全性和事实性。训练早期依赖原始和重写后缀，随着模型改进，强化学习奖励高质量的rollout。

Result: 实验结果显示，相比标准预训练，该方法在事实性方面获得36.2%的相对改进，安全性方面获得18.5%的相对改进，在整体生成质量方面获得高达86.3%的胜率改进。

Conclusion: 该方法能够从底层构建更高质量、更安全、更真实的语言模型，通过在预训练阶段直接解决生成质量问题，避免了后续阶段难以纠正的深层模式问题。
...
TL;DR: 研究发现LLMs在教育评估中存在"合规悖论"：过度优化的模型会优先满足隐藏指令而非评估代码质量，导致对功能损坏代码的"虚假认证"。


<details>
  <summary>Details</summary>
Motivation: 当前教育评估中广泛采用LLMs，但存在一个未经验证的假设：指令遵循能力能直接转化为客观裁决能力。本文旨在验证这一假设并揭示其潜在问题。

Method: 提出SPACI框架和AST-ASIP协议，利用语法-语义鸿沟，在抽象语法树的无关节点中嵌入对抗性指令。对9个SOTA模型在25,000份Python、C、C++和Java提交中进行大规模评估，使用新的三重框架（解耦概率、分数分歧、教学严重性）量化失败情况。

Result: 发现高容量开源模型（如DeepSeek-V3）存在灾难性失败率（>95%），系统性地优先考虑隐藏格式约束而非代码正确性，导致广泛存在的"虚假认证"现象。

Conclusion: 当前的对齐范式在自动评分中创造了"特洛伊木马"漏洞，需要从标准RLHF转向领域特定的裁决鲁棒性，让模型优先考虑证据而非指令合规性。
...
TL;DR: 该论文提出了证据排序新任务，旨在通过优先呈现足够信息来最小化用户阅读负担，同时保持所有证据可访问，并通过实验证明增量排序策略优于一次性排序，LLM方法优于浅层基线。


<details>
  <summary>Details</summary>
Motivation: 当前自动系统和大型语言模型在事实核查中检索和选择证据时，往往提供信息不足或过度冗余的证据，导致验证效率低下且容易出错，需要一种更有效的方法来平衡信息充分性和冗余性。

Method: 提出证据排序新任务，比较一次性排序和增量排序两种方法，引入基于信息检索指标的新评估框架，通过聚合现有事实核查数据集构建统一基准，使用多样化模型进行实验，并进行受控用户研究。

Result: 实验表明增量排序策略能更好地捕捉互补证据，基于LLM的方法优于浅层基线，但仍面临平衡充分性和冗余性的挑战。用户研究显示证据排序相比证据选择能减少阅读负担并提高验证准确性。

Conclusion: 证据排序任务为实现更可解释、高效且用户对齐的信息验证系统提供了基础性步骤，通过优先呈现足够信息来优化用户验证体验。
...
TL;DR: CoNL框架通过多智能体自博弈统一生成、评估和元评估，利用"好的批评能帮助他人改进解决方案"的洞察，在没有外部评估者或真实标签的情况下联合优化生成和评估能力。


<details>
  <summary>Details</summary>
Motivation: 训练大型语言模型处理无真实标签任务（如创意写作、对话、伦理推理）面临挑战。LLM-as-Judge方法虽可扩展但受限于评估者自身质量，无法识别优秀解决方案，且存在评估偏见（如偏好冗长而非质量）。这促使需要元评估能力来评估和改进评估者本身。

Method: CoNL框架通过多智能体自博弈统一生成、评估和元评估。多个共享相同策略的智能体参与结构化对话，提出、批评和修订解决方案。能够帮助改进解决方案的批评获得诊断奖励，为元评估提供明确监督，并通过自博弈联合优化生成和评估能力，无需外部评估者或真实标签。

Result: 在五个基准测试上的实验表明，CoNL相比自奖励基线实现了持续改进，同时保持训练稳定性。

Conclusion: CoNL框架通过多智能体自博弈成功解决了无真实标签任务中LLM训练的评估挑战，实现了生成和评估能力的联合优化，为LLM训练提供了有效的元评估机制。
...
TL;DR: SOUP提出了一种在token级别统一离线和在线策略学习的框架，通过在生成序列的前缀使用历史策略采样，后续部分使用在线策略生成，有效利用离线数据同时保持训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统的在线RL方法（如GRPO）在语言模型后训练中存在探索有限和早期饱和的问题，而现有混合整个轨迹的离线策略方法会导致严重的策略失配和不稳定性。

Method: 提出SOUP框架，在单个样本的token级别统一离线和在线策略学习：序列前缀从历史策略中采样（离线），后续部分由当前策略生成（在线），通过token级别的重要性比率有效利用离线信息。

Result: 大量实验表明，SOUP在性能上一致优于标准的在线训练和现有的离线扩展方法，能够同时改善探索能力和最终性能。

Conclusion: 细粒度的单样本混合策略训练能够有效解决语言模型RL中的探索限制和稳定性问题，为离线和在线策略的统一学习提供了新范式。
...
TL;DR: DimStance是首个维度立场资源，使用效价-唤醒度框架标注多语言文本，支持细粒度立场分析


<details>
  <summary>Details</summary>
Motivation: 传统立场检测仅使用分类标签（支持、中立、反对），无法捕捉立场表达背后的细微情感状态。需要更细粒度的维度方法来分析立场的情感基础

Method: 引入DimStance资源，包含11,746个目标方面在7,365篇文本中，涵盖5种语言和2个领域。使用效价-唤醒度框架进行标注，并建立维度立场回归任务来评估模型性能

Result: 微调的大型语言模型回归器表现竞争性，但在低资源语言中仍存在挑战，基于token的生成方法存在局限性。发现了跨语言的效价-唤醒度模式
...
TL;DR: 研究者创建了MURAD数据集，包含96,243个阿拉伯语单词-定义对，覆盖多个专业领域，旨在支持阿拉伯语自然语言处理和词典学研究。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语词汇丰富，涵盖科学、宗教和文学等多个领域，但大规模连接阿拉伯语单词与精确定义的词汇数据集仍然有限，需要构建一个全面的阿拉伯语词典资源。

Method: 采用混合提取流程，整合了直接文本解析、光学字符识别和自动重建技术，从可信的参考著作和教育资源中提取数据，确保准确性和清晰度。

Result: 成功构建了MURAD数据集，包含96,243个单词-定义对，每个记录包含目标单词、标准化的阿拉伯语定义以及标识源领域的元数据，覆盖语言学、伊斯兰研究、数学、物理、心理学和工程学等多个领域。

Conclusion: MURAD数据集为阿拉伯语自然语言处理和词典学研究提供了重要资源，支持反向词典建模、语义检索和教育工具等应用，有助于推动阿拉伯语词汇语义学的可重复研究。
...
TL;DR: 本文提出Landmark (LMK) pooling方法，通过将序列分块并在块间插入地标标记，然后对地标标记嵌入进行平均池化，解决了传统池化策略在长上下文任务中的系统性弱点。


<details>
  <summary>Details</summary>
Motivation: 现有序列编码器通常使用[CLS]标记或平均池化将变长标记序列压缩为单个向量，但这些策略存在系统性弱点：[CLS]倾向于将信息集中在序列初始位置，可能无法充分表示分布式证据；平均池化可能稀释显著的局部信号，在短上下文任务中表现不佳。

Method: 提出Landmark (LMK) pooling方法：1) 将序列划分为多个块；2) 在块之间插入地标标记；3) 对地标标记的嵌入进行平均池化形成最终表示。这种方法以引入少量特殊标记为代价，改善了长上下文外推能力而不牺牲局部显著特征。
...
TL;DR: 提出inversedMixup框架，将Mixup的可控性与LLM生成的可解释性结合，通过三阶段训练对齐任务模型与LLM的嵌入空间，生成可读的增强文本并解决流形入侵问题。
...
Method: 采用三阶段训练程序：1) 对齐任务特定模型输出嵌入空间与LLM输入嵌入空间；2) 利用LLM反演技术将混合嵌入重构为可读文本；3) 提出缓解流形入侵问题的简单策略。

Result: 实验证明该方法在少样本和全监督场景下都有效且具有泛化性，首次为文本Mixup中的流形入侵现象提供了实证证据。

Conclusion: inversedMixup成功结合了Mixup的可控性和LLM生成的可解释性，通过嵌入空间对齐实现了可读的文本增强，并解决了流形入侵问题，提升了增强性能。
...
<details>
  <summary>Details</summary>
Motivation: 临床病史采集是临床推理的基础但研究不足，现有LLM在动态多轮诊断场景中表现不佳，需要迭代提问和假设精炼
...
Conclusion: Note2Chat框架通过利用广泛可得的医疗记录，有效解决了LLM在动态临床推理中的局限性，为临床病史采集提供了可解释、高效且适应性强的解决方案
...
TL;DR: KromHC使用Kronecker积的小型双随机矩阵参数化残差矩阵，在保证双随机性的同时将参数复杂度从O(n³C)或O(nC·n!)降低到O(n²C)，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有超连接方法存在训练不稳定和可扩展性受限的问题。mHC通过将残差连接空间投影到Birkhoff多面体来缓解这些问题，但仍面临两个挑战：1) SK算法不一定产生精确的双随机残差矩阵；2) 参数复杂度O(n³C)过高。mHC-lite虽然保证双随机性但面临阶乘爆炸的参数复杂度O(nC·n!)。

Method: 提出KromHC方法，使用小型双随机矩阵的Kronecker积来参数化mHC中的残差矩阵。通过对张量化残差流的每个模态上的因子残差矩阵施加流形约束，KromHC保证残差矩阵的精确双随机性，同时将参数复杂度降低到O(n²C)。

Result: 综合实验表明，KromHC匹配甚至优于最先进的mHC变体，同时需要显著更少的可训练参数。

Conclusion: KromHC通过Kronecker积参数化方法有效解决了现有超连接方法的双随机性和参数复杂度问题，在保持性能的同时大幅降低了参数需求。
...
TL;DR: 使用语言模型作为受控统计学习器来系统模拟跨语言影响，研究L1语言主导性和L2语言熟练度对跨语言影响的作用机制。


<details>
  <summary>Details</summary>
Motivation: 人类研究中关于跨语言影响的结果常常相互矛盾，主要原因是实验方差难以控制。需要一种更系统的方法来研究跨语言影响的驱动因素。

Method: 使用语言模型作为受控统计学习器，通过控制L2接触年龄来操纵L2熟练度，研究不同句法距离的L1预训练对L2的影响，采用跨语言启动范式分析L1结构激活对L2处理的影响。

Result: 语言主导性和熟练度是跨语言影响的强预测因子；语法结构启动是双向的，但不合语法结构的启动对语言主导性敏感；L1在L2处理过程中被共同激活并直接影响L2的神经回路。

Conclusion: 语言模型可以作为计算框架来为人类跨语言影响理论提供信息，证实了语言主导性和熟练度在跨语言影响中的核心作用，并揭示了跨语言影响的神经机制。
...
TL;DR: 本文提出ILRR（迭代潜在表示细化），一种无需学习的框架，用于通过单个参考序列引导离散扩散语言模型的文本生成，实现推理时控制。
...
Method: 提出ILRR框架，通过动态对齐生成序列和参考序列的内部激活来引导去噪过程。引入空间调制引导扩展，允许使用较短参考引导长文本生成，通过调节序列上的引导强度实现。

Result: ILRR在LLaDA和MDLM架构上实现了有效的属性引导，计算开销小（每个去噪步骤仅需一次额外并行前向传播）。在相同计算预算下，ILRR将属性准确性比基线提高了10%到60%，同时保持高生成质量。

Conclusion: ILRR为离散扩散语言模型提供了一种有效、计算高效的推理时控制框架，能够通过单个参考序列实现灵活的属性引导，为可控文本生成开辟了新途径。
...
TL;DR: 提出一种后训练自适应策略，通过选择性替换低效用标记来优化子词分词，使大语言模型在特定领域或语言中更高效。


<details>
  <summary>Details</summary>
Motivation: 标准BPE分词方法使用通用标记集处理所有文本数据，但在应用到特定领域或语言时会产生效率低下问题，需要针对性的优化方案。
...
Conclusion: 该方法作为一种轻量级自适应机制，类似于词汇表微调过程，能够为特定领域或任务优化分词效果，提高大语言模型的性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 语言通过一系列语义状态演进，但其底层动态特性尚不清楚。研究旨在理解文本语义进展的动力学机制，并区分人类认知与算法模型生成的模式

Method: 将文本语义进展视为高维状态空间中的随机轨迹，使用精密计量学中的Allan偏差工具，将有序句子嵌入作为位移信号进行分析

Result: 发现两种不同的动态机制：短时幂律尺度（区分创意文学与技术文本）和长时交叉到稳定性受限的噪声基底。大型语言模型能成功模仿人类文本的局部尺度统计，但表现出稳定性视野的系统性降低

Conclusion: 语义连贯性是可测量的物理属性，该框架能区分人类认知的细微动态与算法模型生成的模式，为理解语言演进提供了新的量化方法
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法在探索解决方案空间时，通常将每次推理过程视为一次性样本，宝贵的中间洞察在每次尝试后被丢弃。这种系统性无记忆性导致大量计算冗余，模型反复重新推导已发现的结论并重访已知的死胡同。

Method: 提出RSE策略，通过主动将原始推理轨迹提炼到共享经验库中，实现正回收（中间结论）来减少冗余推导，负回收（失败模式）来修剪遇到的死胡同，将测试时搜索从孤立试验转变为累积过程。

Result: 在HMMT24、HMMT25、IMO-Bench和HLE等基准测试上的广泛实验表明，RSE在可比计算成本下始终优于强基线方法，实现了最先进的扩展效率。

Conclusion: RSE通过回收搜索经验，有效解决了测试时扩展中的计算冗余问题，将搜索从独立采样转变为累积学习过程，显著提升了大型语言模型在复杂推理任务中的效率。
...
TL;DR: TAPPA框架通过时间连续性视角统一解释LLM中的各种注意力模式，将其分为可预测和不可预测两类，并基于此指导KV缓存压缩和模型剪枝等推理加速任务。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然识别了检索头、汇头、对角线轨迹等个别注意力模式，但这些观察是碎片化的，缺乏统一的理论解释框架。需要建立一个能够统一解释各种注意力模式的数学框架。

Method: 提出TAPPA（时间注意力模式可预测性分析）框架，从时间连续性视角分析注意力模式的数学公式。将注意力模式分为可预测模式（具有清晰规律性）和不可预测模式（看似随机），并通过查询在时间维度上的自相似性来解释这种区别。特别关注可预测模式，通过查询、键和RoPE的联合效应进行详细数学分析。

Result: 在KV缓存压缩和LLM剪枝任务中验证TAPPA框架的有效性。基于TAPPA启发的简单指标在这些任务中持续优于基线方法。

Conclusion: TAPPA框架不仅深化了对注意力行为的理解，还能指导推理加速方法。该框架为理解LLM中的各种注意力模式提供了统一的理论基础，并在实际应用中展示了实用价值。
...
TL;DR: TACLer是一个基于课程强化学习的框架，通过渐进式训练和混合思考/非思考推理范式，在降低计算成本的同时提升大语言模型在复杂数学问题上的推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂推理任务中通常需要大规模强化学习训练来生成长链式思维，但这会导致计算成本高且容易产生冗余的中间步骤（过度思考）。需要一种既能提高学习推理效率，又能保持甚至增强性能的方法。

Method: 提出TACLer框架，包含两个核心组件：1) 定制化课程学习，根据模型在不同阶段的熟练程度渐进增加数据复杂度；2) 混合思考/非思考推理范式，通过启用或禁用思考模式来平衡准确性和效率。

Result: TACLer在学习和推理方面具有双重优势：1) 降低计算成本，相比长思考模型减少50%以上的训练计算量，相比基础模型减少42%以上的推理token使用；2) 提升准确性，在四个复杂数学数据集上比基础模型提升超过9%，持续优于最先进的非思考和思考基线模型。

Conclusion: TACLer通过课程强化学习和混合推理范式，有效解决了大语言模型在复杂推理任务中的效率与性能平衡问题，为高效推理提供了有前景的解决方案。
...
Method: 结合对比学习和序数排序目标来结构化LLM潜在空间，捕捉具体行动和模糊声明之间的分级区别；采用门控特征调制过滤披露噪声，使用MetaGradNorm稳定多目标优化。

Result: 在跨类别设置中的实验表明，该方法在鲁棒性方面优于标准基线，同时揭示了表示刚性和泛化之间的权衡关系。

Conclusion: 提出的参数高效框架能够有效提高ESG报告分析的鲁棒性，通过结构化LLM潜在空间来更好地区分具体行动和模糊声明，为可持续性报告评估提供了更可靠的方法。
...
TL;DR: 该研究探索了在语言模型预训练中先使用抽象结构化数据（如形式语言生成的程序性数据）的方法，发现仅用0.1%的程序性数据预训练就能显著提升模型性能，减少所需训练数据量，并在注意力机制和MLP层中形成有益的结构。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型主要在网页规模语料上进行预训练，研究者希望探索一种类似人类学习过程的替代方案：先接触抽象结构化数据（如逻辑和数学），再学习丰富的语义知识，以提升模型的学习效率和推理能力。

Method: 研究使用形式语言和其他简单算法生成的程序性数据作为预训练材料，分析了不同程序性数据对算法技能的提升效果。在1.3B参数规模的模型上进行实验，比较了程序性数据预训练与标准自然语言、代码和数学数据集（C4、CodeParrot、DeepMind-Math）预训练的效果差异。

Result: 程序性预训练显著提升了模型性能：在上下文回忆任务上，使用Dyck序列（平衡括号）预训练后准确率从10%提升到98%；仅使用0.1%程序性数据预训练就能超越标准预训练；达到相同损失值所需数据量减少到原来的55%、67%、86%；程序性预训练在注意力层和MLP层都形成了非平凡的结构。

Conclusion: 程序性预训练是一种简单轻量的方法，能够提升语言模型性能并加速预训练过程，为实现知识获取与推理能力解耦的LLM发展提供了有前景的方向。
...
TL;DR: 提出CE-GOCD方法，通过学术知识图谱中的语义子结构建模来增强LLM在科学问答中的表现，在三个NLP文献问答数据集上优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法通常依赖孤立的文本块或概念，忽略了论文之间更深层的语义连接，这损害了LLM对科学文献的理解，影响了回答的全面性和特异性。
...
Result: ChatGPT在医学信息抽取任务上的表现落后于微调基线模型；能提供高质量决策解释但过度自信；大多数情况下对原文有较高忠实度；生成不确定性导致信息抽取结果不确定性

Conclusion: ChatGPT在医学信息抽取任务中存在局限性，生成不确定性可能阻碍其在医学信息抽取任务中的应用，需要进一步改进
...
TL;DR: Zonkey是一个分层扩散模型，通过完全可训练的字符到文档级表示管道，解决了传统LLMs中固定、不可微分分词器的限制。
...
Method: 提出Zonkey模型，包含可微分分词器（Segment Splitter）、概率注意力机制、分层压缩序列、去噪扩散混合模型（DDMM）和Stitcher组件，实现从原始字符到文档级表示的端到端训练。

Result: 在维基百科上训练的Zonkey能够从噪声生成连贯、可变长度的文本，展示了涌现的分层结构，并在数据分布对齐方面优于基于熵的可学习分词器。

Conclusion: Zonkey推动了完全基于梯度的LLMs发展，具有更好的领域适应性和可扩展生成潜力，为端到端优化的语言模型提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统将密集检索器和语言模型结合，但组件间交互的不透明性给高风险领域部署带来挑战，需要可解释性框架来理解和审计这种交互。

Method: 提出RAG-E框架：1) 为检索器分析调整集成梯度方法；2) 引入PMCSHAP（蒙特卡洛稳定Shapley值近似）进行生成器归因；3) 引入WARG（加权归因-相关性差距）指标衡量生成器文档使用与检索器排名的对齐程度。
...
Conclusion: RAG-E能够审计RAG系统中检索器与生成器之间的交互，揭示关键错配问题，为高风险领域部署提供必要的可解释性工具。
...
TL;DR: DARE提出了一种分布感知的奖励估计方法，替代传统多数投票机制，通过利用完整的经验分布、探索奖励和分布剪枝来提供更稳健的奖励信号，显著提升了测试时强化学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有测试时强化学习方法依赖多数投票机制估计奖励，但这种假设很脆弱：多数投票将分布简化为单一结果，丢弃了非多数但正确的动作信息，导致系统性偏差的奖励估计。

Method: 提出分布感知奖励估计（DARE），将奖励估计从单一多数结果转向完整的经验分布。DARE进一步通过探索奖励和分布剪枝机制增强这种基于分布的奖励，用于非多数rollout探索和奖励去噪。

Result: 在具有挑战性的推理基准测试中，DARE相比现有基线方法显著提升了优化稳定性和最终性能，在AIME 2024上实现25.3%的相对改进，在AMC上实现5.3%的相对改进。

Conclusion: DARE通过利用完整的经验分布而非单一多数结果，提供了更信息丰富和稳健的奖励估计，有效解决了现有测试时强化学习方法中奖励估计的局限性。
...
TL;DR: MilSCORE是首个基于军事规划场景的专家编写多跳问答数据集，用于评估大语言模型在长上下文、多模态信息下的战术空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型处理更长更复杂任务的需求增长，需要真实的长上下文基准测试来评估模型对异构多模态信息的筛选和整合能力，特别是在军事规划等需要快速准确推理地图、命令、情报报告等分布式数据的领域。

Method: 提出了MilSCORE数据集，包含专家编写的基于复杂模拟军事规划场景的多跳问题，涵盖七类问题类型，针对事实回忆和多步推理能力，并提供了评估协议和多种视觉语言模型的基线结果。

Result: 当前系统在MilSCORE上表现不佳，显示出在真实场景级长上下文规划方面存在显著改进空间，表明该基准测试具有挑战性。
...
TL;DR: 本文提出了一种基于重写的自适应距离学习算法，用于检测LLM生成的文本，相比固定距离方法在多种场景下表现更优。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（如GPT、Claude、Gemini）能生成高度类人文本，引发了关于错误信息和学术诚信的严重担忧，迫切需要可靠的算法来检测LLM生成的内容。

Method: 首先提出几何方法解析基于重写的检测算法，揭示其基本原理并展示泛化能力；然后引入新颖的自适应学习距离的基于重写检测算法，理论上证明自适应学习距离函数比固定距离更有效。

Result: 在超过100种设置下进行广泛实验，发现该方法在大多数场景下优于基线算法，针对不同目标LLM（如GPT、Claude、Gemini），相对于最强基线的相对改进从57.8%到80.6%。

Conclusion: 自适应学习距离的基于重写检测算法能有效检测LLM生成的文本，为解决错误信息和学术诚信问题提供了可靠的技术方案。
...
TL;DR: SONIC是一个基于学习的KV缓存压缩框架，通过将历史对话段压缩为紧凑的Nexus令牌来优化多轮LLM部署，在80%和50%压缩率下优于现有基线方法。
...
Result: 在四个多轮基准测试中，SONIC在80%和50%压缩率下持续优于H2O和StreamingLLM等基线方法；在MTBench101基准上平均得分提升35.55%；推理过程整体加速50.1%。
...
TL;DR: 该研究提出了一种基于GPT的临床文本分类架构，采用选择性微调策略，通过冻结大部分GPT-2主干参数，仅训练最后Transformer块、层归一化和轻量分类头，显著减少可训练参数，在MIMIC-IV-Note放射学报告上实现了高效准确的疾病分类。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中非结构化临床叙述的日益增多为自动化疾病表征、队列识别和临床决策支持创造了新机会，但建模长领域特定临床文本面临标注数据有限、类别严重不平衡以及大型预训练语言模型适应计算成本高的挑战。

Method: 提出基于GPT的临床文本分类架构，采用选择性微调策略：冻结大部分GPT-2主干参数，仅训练最后Transformer块、最后层归一化和轻量分类头，显著减少可训练参数。在MIMIC-IV-Note放射学报告数据集上评估，使用基于报告文本的不确定性感知CheXpert风格标签。

Result: 在不同数据集规模下，模型表现出稳定的收敛行为和强大的分类性能，特别是在非提及和否定发现占主导的设置中。实验涵盖多标签放射学发现分类、不同不确定性假设下的二元每标签分类以及聚合疾病结果预测。
...
TL;DR: OVD是一种基于轨迹匹配的在线知识蒸馏框架，用离散语言评分替代token级概率匹配，显著降低内存消耗，提升学生模型在问答和数学推理任务上的表现。
...
Method: 提出在线语言蒸馏(OVD)框架，用教师模型的离散语言评分(0-9)进行轨迹匹配，替代token级概率匹配，避免token级对齐，允许学生模型自由探索输出空间。

Result: 在Web问答任务上平均EM提升高达12.9%，数学推理基准上提升高达25.7%(仅使用一个随机样本训练)，同时展现出优越的训练效率。

Conclusion: OVD通过轨迹匹配和离散语言评分，有效解决了现有在线蒸馏方法的内存瓶颈和探索限制问题，显著提升了学生模型的推理能力。
...
TL;DR: Token-Guard：基于自检解码的token级幻觉控制方法，通过内部验证、潜在空间评估和迭代修剪再生来减少LLM幻觉
...
Method: Token-Guard采用token级幻觉控制，通过自检解码在每个推理步骤进行内部验证，在潜在空间中对候选片段进行显式幻觉风险评分，并通过迭代修剪和再生动态纠正检测到的错误

Result: 在HALU数据集上的实验表明，Token-Guard显著减少了幻觉并提高了生成准确性

Conclusion: Token-Guard提供了一个可扩展、模块化的解决方案，能够可靠地控制LLM输出中的幻觉问题
...
TL;DR: MDA框架通过影响函数将可解释电路溯源到训练数据，发现重复结构化数据是机制催化剂，干预诱导头形成会影响上下文学习能力，并提出加速电路收敛的数据增强方法。


<details>
  <summary>Details</summary>
Motivation: 尽管机制可解释性已在LLMs中识别出可解释电路，但这些电路在训练数据中的因果起源仍然难以捉摸。需要建立训练数据与模型内部机制之间的因果联系。

Method: 提出机制数据归因（MDA）框架，使用影响函数将可解释单元溯源到特定训练样本。在Pythia模型家族上进行实验，通过移除或增强高影响力样本来进行针对性干预，并与随机干预对比。

Result: 针对性干预显著调节可解释头的出现，而随机干预无效；重复结构化数据（如LaTeX、XML）是机制催化剂；干预诱导头形成会同时改变模型的上下文学习能力；提出的数据增强管道能加速电路收敛。

Conclusion: MDA提供了训练数据与模型内部机制之间的因果证据，验证了诱导头与上下文学习的功能联系假设，并为引导LLM发展轨迹提供了原则性方法。
...
TL;DR: CARD是一个新颖的框架，将自回归模型（ARMs）的训练效率与扩散模型的高吞吐量推理相结合，通过因果注意力掩码重新表述扩散过程，实现单次前向传播中的密集监督。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练效率和推理速度之间存在权衡：自回归模型训练高效但推理速度慢，扩散模型推理速度快但训练效率低。需要一种能同时兼顾两者优势的方法。

Method: 1. 在严格因果注意力掩码内重新表述扩散过程；2. 引入软尾掩码方案以保留局部上下文；3. 基于信噪比原理设计上下文感知重加权机制；4. 支持动态并行解码，利用KV缓存自适应生成变长token序列。

Result: CARD在性能上优于现有的离散扩散基线方法，同时将训练延迟降低了3倍（相比块扩散方法）。实现了ARM级别的数据效率，同时获得了并行生成的延迟优势。

Conclusion: CARD建立了一个稳健的范式，为下一代高效大型语言模型提供了解决方案，成功统一了训练效率和推理速度的优势。
...
Result: 在Pile数据集上，Leviathan始终优于标准的LLaMA风格架构；通过经验幂律拟合，Leviathan表现出显著优越的有效参数容量，在研究的范围内，其表现相当于参数多1.47到2.11倍的密集模型

Conclusion: Leviathan架构通过重新设计嵌入表示方式，显著提升了小语言模型的参数效率，为资源受限环境下的语言模型设计提供了新思路
...
Conclusion: 指令跟随会干扰LLMs的任务解决能力，SUSTAINSCORE能有效量化这种干扰，为当前对齐策略提供实证观察，促进进一步研究
...
TL;DR: MasalBench是一个评估LLMs对波斯谚语上下文和跨文化理解的基准测试，发现LLMs在识别波斯谚语方面表现良好（准确率>0.90），但在识别等效英语谚语方面表现显著下降（最佳模型0.79准确率），揭示了当前LLMs在文化知识和类比推理方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型已成为日常生活的重要组成部分，需要掌握会话语言规则以有效与用户沟通。先前研究主要评估LLMs对高资源语言中比喻语言的理解，但对低资源语言中的表现仍缺乏探索。波斯语作为低资源语言，其谚语是会话的关键组成部分，需要专门评估LLMs在这方面的能力。

Method: 研究者开发了MasalBench基准测试，专门用于评估LLMs对波斯谚语的上下文和跨文化理解。该基准测试了8个最先进的LLMs，主要评估两个任务：1）在上下文中识别波斯谚语；2）识别与波斯谚语等效的英语谚语。

Result: LLMs在识别波斯谚语方面表现良好，准确率超过0.90；但在识别等效英语谚语方面表现显著下降，最佳模型仅达到0.79准确率。这表明当前LLMs在文化知识和类比推理方面存在明显局限性。

Conclusion: 研究揭示了当前多语言LLMs在低资源语言文化理解和跨文化类比推理方面的不足。MasalBench为评估其他低资源语言的跨文化理解提供了框架，有助于推动LLMs在文化敏感性和跨语言理解方面的改进。
...
TL;DR: VTC-R1是一种新的高效推理范式，通过将中间推理段渲染为紧凑图像作为"光学记忆"，实现3.4倍token压缩和2.7倍推理加速


<details>
  <summary>Details</summary>
Motivation: 长上下文推理虽然增强了LLMs处理复杂任务的能力，但带来了严重的效率瓶颈。现有方法通常需要复杂的额外训练或外部模型压缩，限制了可扩展性并丢弃了关键细粒度信息。

Method: 提出VTC-R1范式，将视觉-文本压缩集成到推理过程中。将中间推理段渲染为紧凑图像，迭代反馈到视觉语言模型中作为"光学记忆"。基于OpenR1-Math-220K构建训练数据集，并对Glyph和Qwen3-VL等代表性VLM进行微调。
...
TL;DR: 本文提出Fed-MedLoRA和Fed-MedLoRA+框架，通过联邦学习在医疗领域高效适配大语言模型，解决传统联邦学习在通信开销和数据异质性方面的限制，并在临床信息提取任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前医疗大语言模型通常基于单一机构数据训练，存在泛化性和安全性限制。传统联邦学习应用于大语言模型面临通信开销大（需传输完整模型）和数据异质性（临床数据在不同机构间差异显著）两大挑战。

Method: 提出模型无关且参数高效的联邦学习框架：Fed-MedLoRA仅传输低秩适配器参数以减少通信和计算开销；Fed-MedLoRA+进一步引入自适应、数据感知的聚合机制以改善跨站点异质性下的收敛性能。

Result: 在临床信息提取任务中，该框架在五个患者队列上进行了评估，包括域内训练测试、外部验证和低资源新站点适应场景。与BERT、LLaMA-3、DeepSeek-R1和GPT-4o等模型相比，展现了良好的性能。

Conclusion: Fed-MedLoRA和Fed-MedLoRA+框架能够有效解决医疗大语言模型联邦学习中的通信开销和数据异质性挑战，为跨医疗机构协作模型开发提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有CoT提示的LLM存在"盲目自思考"局限，在关键信息缺失或模糊时仍进行大量内部推理。需要让LLM能够主动与用户交互澄清不确定性，而不是被动地猜测或忽略信息缺口。

Method: PIR包含两个核心组件：1）不确定性感知的监督微调，赋予模型交互推理能力；2）基于用户模拟器的策略优化框架，使用复合奖励函数使模型行为与用户意图对齐。

Result: 在数学推理、代码生成和文档编辑任务上，PIR显著优于基线方法，准确率提升达32.70%，通过率提升22.90%，BLEU分数提升41.36，同时减少近一半的推理计算和不必要的交互轮次。

Conclusion: PIR通过主动交互推理有效解决了LLM在前提和意图层面的不确定性问题，在多个任务上展现出强大的泛化能力和鲁棒性，为LLM推理提供了新的范式转变。
...
TL;DR: 该论文提出了一种将互联网规模的预训练文档转化为数十亿合成指令-答案训练对的方法，创建了FineInstructions数据集，使LLM可以从头开始仅使用指令调优目标进行预训练。


<details>
  <summary>Details</summary>
Motivation: 由于监督训练数据有限，大型语言模型通常通过自监督的"预测下一个词"目标在大量非结构化文本数据上进行预训练。为了让模型对用户有用，还需要在更少的"指令调优"数据上进行进一步训练。为了克服监督数据有限的问题，需要一种方法将预训练文档中的知识转化为合成指令训练数据。

Method: 提出了一种将互联网规模预训练文档转化为合成指令-答案训练对的程序。使用约1800万个从真实用户查询和提示创建的指令模板，将这些模板与来自非结构化预训练语料库的人工编写源文档匹配并实例化，生成大规模的"监督"合成训练数据。

Result: 在FineInstructions上进行预训练比标准预训练和其他提出的合成预训练技术在衡量自由形式响应质量的标准基准测试中表现更好。模型可以从头开始仅使用指令调优目标进行预训练，这更符合LLM的下游使用预期。

Conclusion: 通过将预训练文档知识转化为大规模合成指令数据，可以克服监督数据有限的瓶颈，使LLM能够更有效地进行指令调优预训练，从而更好地响应用户提示。
...
TL;DR: 提出HALO蒸馏流程将Transformer转换为RNN-注意力混合模型，以及HypeNet混合架构，通过少量数据（2.3B tokens）实现性能相当但长上下文性能更优的转换


<details>
  <summary>Details</summary>
Motivation: 现有混合Transformer架构（结合softmax注意力块和RNN）在长上下文建模中表现出良好的性能-吞吐量权衡，但大规模从头预训练成本过高。现有转换方法需要大量训练数据且长上下文性能不佳，而混合模型在长上下文场景下相比Transformer有显著推理加速优势

Method: 提出HALO（Hybrid Attention via Layer Optimization）蒸馏流程，将Transformer模型蒸馏为RNN-注意力混合模型；提出HypeNet混合架构，采用新型位置编码方案HyPE和各种架构修改以实现优越的长度泛化能力

Result: 使用HALO将Qwen3系列转换为HypeNet，仅需2.3B tokens（不到其预训练数据的0.01%），在保持与原始Transformer模型相当性能的同时，获得了优越的长上下文性能和效率

Conclusion: HALO蒸馏流程和HypeNet架构为Transformer到混合模型的转换提供了高效解决方案，显著降低了转换成本，同时保持了模型性能并提升了长上下文处理能力
