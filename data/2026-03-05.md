...
TL;DR: 本文提出了一种聚类引导的mHC Mamba模型（mHC-HSI），用于增强高光谱图像分类，通过改进空间-光谱特征学习、引入软聚类成员图提升可解释性，并利用物理光谱知识进行分组处理。


<details>
  <summary>Details</summary>
Motivation: DeepSeek提出的流形约束超连接（mHC）方法在深度学习模型中表现出优于传统残差连接的性能，但该方法尚未针对高光谱图像（HSI）分类进行专门设计。本文旨在将mHC框架应用于HSI分类，提升分类精度和模型可解释性。

Method: 1. 设计基于mHC框架的聚类引导Mamba模块，显式学习HSI的空间和光谱信息；2. 设计新的残差矩阵实现，将其视为软聚类成员图，提升mHC方法的可解释性；3. 将光谱波段按物理意义分组，作为mHC中的"并行流"，形成具有物理意义的可解释方法。

Result: 在基准数据集上的测试结果表明，与最先进方法相比，所提模型不仅提高了分类精度，还增强了模型的可解释性。

Conclusion: 本文成功将mHC框架应用于HSI分类，通过聚类引导和物理光谱知识整合，实现了精度和可解释性的双重提升，为高光谱图像分析提供了新的有效方法。
...
TL;DR: 研究发现仅文本强化学习在医学VQA基准测试中能达到或超越图像-文本模型的表现，表明当前评估方法未能有效衡量视觉依赖性。作者提出反事实评估框架，发现RLVR模型在提高准确率的同时降低了视觉基础性，模型常生成无视觉依据的虚假视觉推理。


<details>
  <summary>Details</summary>
Motivation: 当前多模态医学VQA评估协议可能无法有效衡量模型对视觉信息的真实依赖，因为仅文本强化学习模型也能达到与图像-文本模型相当的准确率。这促使研究者开发更严格的评估方法来检测模型是否真正利用了视觉信息。

Method: 引入反事实评估框架，在四个医学VQA基准（PathVQA、PMC-VQA、SLAKE、VQA-RAD）上使用真实图像、空白图像和打乱图像进行测试。除了准确率外，还测量视觉依赖分数（VRS）、图像敏感度（IS），并引入幻觉视觉推理率（HVRR）来检测模型生成无视觉依据的视觉声明的情况。

Result: RLVR模型在提高准确率的同时降低了视觉基础性：仅文本RLVR在PathVQA上获得负VRS（-0.09），使用不匹配图像时表现更好；图像-文本RLVR将总体图像敏感度降至39.8%。在VQA-RAD上，两种变体都达到63%准确率但机制不同：仅文本RLVR在空白图像上保留81%性能，而图像-文本RLVR仅显示29%图像敏感度。模型在68-74%的回答中生成视觉声明，但其中38-43%是无视觉依据的（HVRR）。
...
TL;DR: PERSIST提出了一种新的世界模型范式，通过模拟潜在3D场景的演化（环境、相机、渲染器）来生成视频，解决了现有交互式世界模型缺乏3D表示、空间记忆受限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有交互式世界模型通常缺乏3D环境表示，3D一致性需要从数据中隐式学习，空间记忆受限于有限的时间上下文窗口，导致用户体验不真实，并对下游任务（如训练智能体）造成显著障碍。

Method: PERSIST采用新的世界模型范式，模拟潜在3D场景的演化：包括环境、相机和渲染器。这种方法能够合成具有持久空间记忆和一致几何结构的新帧。

Result: 定量指标和定性用户研究显示，在空间记忆、3D一致性和长时程稳定性方面相比现有方法有显著改进，能够生成连贯、演化的3D世界。还展示了从单张图像合成多样化3D环境，以及通过支持直接在3D空间中进行环境编辑和规范来实现细粒度、几何感知的控制等新能力。

Conclusion: PERSIST通过模拟潜在3D场景演化，实现了具有持久空间记忆和一致几何结构的交互式世界生成，显著提升了3D一致性和长时程稳定性，为生成连贯、演化的3D世界提供了新范式。
...
TL;DR: Phys4D是一个从视频扩散模型学习物理一致4D世界表示的管道，通过三阶段训练提升物理一致性，解决现有模型在细粒度物理一致性上的不足。
...
Result: Phys4D相比基于外观的基线方法，在细粒度时空和物理一致性方面有显著提升，同时保持了强大的生成性能。引入了4D世界一致性评估来探测几何一致性、运动稳定性和长时程物理合理性。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率泛北极海冰制图及其不确定性量化对于海冰业务化制图至关重要，但面临海冰特征细微、标签不精确、模型不确定性和数据异质性等挑战。

Method: 1) 设计高分辨率Transformer模型，包含全局和局部模块以提取细微海冰特征；2) 设计地理加权弱监督损失函数，在区域层面而非像素层面监督模型；3) 提出贝叶斯扩展，将模型参数视为随机变量以量化不确定性；4) 在决策层融合三种不同类型数据（Sentinel-1、RCM、AMSR2）。

Result: 在2021年和2025年泛北极最小冰盖条件下评估，使用Sentinel-1数据实现了0.70的总体特征检测精度，同时保持了泛北极海冰浓度模式（相对于ARTIST海冰产品的R²=0.90）。

Conclusion: 该方法成功解决了高分辨率海冰制图中的关键挑战，通过创新的Transformer架构、弱监督损失、贝叶斯不确定性和多源数据融合，实现了准确的海冰浓度制图和不确定性量化。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到视频生成器虽然视觉质量高，但经常违反物理定律。研究发现这源于提示词中物理约束不足，而非模型本身限制。手动添加物理细节虽然有效，但需要专业知识且难以扩展。

Method: 采用两阶段强化学习框架：1）在物理聚焦的思维链数据集上微调大语言模型，整合物体运动、力相互作用等物理原理；2）应用分组相对策略优化，配合动态奖励课程，初期优先语义忠实度，逐步转向物理常识。

Result: PhyPrompt-7B在VideoPhy2上达到40.8%联合成功率（提升8.6个百分点），物理常识提升11个百分点（55.8%到66.8%），语义忠实度提升4.4个百分点（43.4%到47.8%）。超越GPT-4o（+3.8%）和DeepSeek-V3（+2.2%，参数大100倍），并在多种T2V架构上实现零样本迁移，最高提升16.8%。

Conclusion: 通过领域专业化的强化学习与组合课程，可以在不增加模型规模的情况下显著提升文本到视频生成的物理合理性，证明组合式提示发现超越了传统的多目标权衡，为物理感知生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜手术中的单目深度估计受到烟雾、镜面反射、模糊和遮挡等噪声影响，现有模型不输出深度置信度，限制了临床可靠性。

Method: 提出三部分框架：1) 使用微调立体匹配模型集成生成像素级置信度概率；2) 用置信度感知损失函数优化基线模型，让可靠像素主导训练；3) 添加卷积层置信度估计头，在推理时预测像素级置信度。

Result: 在内部和公共数据集上的实验验证表明，该框架提高了深度估计精度并能稳健量化预测置信度。在内部临床内窥镜数据集(StereoKP)上，相比基线模型将密集深度估计精度提高了约8%。

Conclusion: 置信度感知框架提高了内窥镜手术中单目深度估计模型的准确性，解决了临床数据中噪声和伪影的挑战，并提供了可用于提高临床应用可靠性的置信度图。
...
TL;DR: L2G-Det：一种从局部到全局的实例检测框架，通过密集补丁级匹配绕过显式目标提议，在开放世界环境中实现新颖目标实例的检测和分割。
...
Method: 提出L2G-Det框架：1）通过模板图像和查询图像之间的密集补丁级匹配生成候选点；2）使用候选选择模块过滤误报；3）用过滤后的点提示增强的Segment Anything Model（SAM），结合实例特定的目标标记重建完整实例掩码。

Result: 实验表明，在具有挑战性的开放世界设置中，相比基于提议的方法，L2G-Det表现出改进的性能。

Conclusion: L2G-Det通过局部到全局的方法，绕过显式目标提议，利用密集补丁匹配和增强的SAM，为开放世界中的目标实例检测和分割提供了更鲁棒的解决方案。
...
TL;DR: 该论文提出交通场景图生成新任务，通过深度线索和事故数据补充视觉特征，生成突出危险与自车关系的交通场景图，为危险感知的场景理解提供直观指导。


<details>
  <summary>Details</summary>
Motivation: 现有研究在检测特定语义类别和视觉显著区域方面表现良好，但缺乏评估安全相关性的能力。同时，现有场景图建模的通用空间谓词要么只针对前景对象，要么针对所有场景实体，都不适用于驾驶场景。需要一种能够捕捉突出危险与自车之间交通特定关系的方法。

Method: 提出交通场景图生成框架，明确使用交通事故数据和深度线索来补充视觉特征和语义信息进行推理。输出通过颜色编码危险严重程度、标注影响机制和相对位置的交通场景图。

Result: 在Cityscapes数据集上创建关系标注，从5个角度评估10个任务。比较实验和消融研究结果表明，该模型在危险感知的交通场景理解中具有以自车为中心的推理能力。

Conclusion: 提出的交通场景图生成任务和框架能够有效捕捉交通特定关系，为复杂驾驶场景中的态势感知提供直观指导，实现危险感知的交通场景理解。
...
TL;DR: DM-CFO：一种基于扩散模型和3D高斯碰撞检测的组合式牙齿生成方法，通过文本和图约束逐步恢复缺失牙齿布局，并使用距离正则化惩罚牙齿碰撞


<details>
  <summary>Details</summary>
Motivation: 当前3D牙齿模型自动设计方法面临组合式生成挑战，需要同时优化缺失牙齿的布局和形状，且基于3D高斯的组合生成方法常忽略碰撞冲突问题

Method: 提出DM-CFO方法：1）在去噪阶段通过文本和图约束逐步恢复缺失牙齿布局；2）使用分数蒸馏采样交替更新每个布局引导的牙齿和整个颌骨的3D高斯参数；3）引入基于相邻牙齿与锚定牙齿间3D高斯距离的正则化项来惩罚牙齿交叉

Result: 在三个牙齿设计数据集上的实验结果表明，该方法相比现有方法显著提高了生成牙齿的多视角一致性和真实感

Conclusion: DM-CFO方法有效解决了组合式牙齿生成中的布局优化和碰撞检测问题，为牙齿数字化提供了高质量的3D牙齿模型自动设计解决方案
...
TL;DR: 提出一个结合外观和运动特征的框架，用于在动物设施中监测企鹅，通过连续帧处理改进检测，使用轨迹对比学习进行个体识别。
...
Method: 1. 检测：改进YOLO11处理连续帧，利用运动特征克服单帧检测的时间不一致性；2. 识别：在跟踪后应用基于轨迹的对比学习方法，生成连贯的特征嵌入。

Result: 使用两帧输入微调模型，mAP@0.5从0.922提升到0.933，优于基线，能恢复静态图像中无法区分的个体；特征嵌入可视化显示相同个体的样本在特征空间中更接近，有望减少ID切换。

Conclusion: 结合外观和运动特征的框架能有效提升企鹅监测的检测和识别性能，连续帧处理有助于克服视觉特征模糊的问题，轨迹对比学习为个体识别提供了有前景的方向。
...
TL;DR: 提出一种基于方向包围框(OBB)的头部朝向估计方法，通过三个检测器(头部、尾部、头尾)和IoU多数投票机制，解决现有OBB检测器无法区分头尾导致跟踪中断的问题。


<details>
  <summary>Details</summary>
Motivation: 野生马等群居动物的社会结构多样且研究不足，航拍视频跟踪对分析群体动态至关重要。现有轴对齐边界框在航拍场景中性能下降，方向包围框(OBB)虽能减少背景干扰，但现有OBB检测器(如YOLO-OBB)角度限制在180°范围内，无法区分头尾，导致帧间出现180°翻转，严重破坏连续跟踪。

Method: 提出头部朝向估计方法：1)裁剪OBB中心区域图像块；2)应用三个检测器分别检测头部、尾部和头尾区域；3)通过IoU(交并比)多数投票机制确定最终标签，从而准确识别动物头部朝向。

Result: 在299张测试图像上的实验表明，该方法达到99.3%的准确率，优于单个模型，证明了其在基于OBB的鲁棒跟踪中的有效性。

Conclusion: 提出的头部朝向估计方法有效解决了现有OBB检测器无法区分头尾的问题，提高了航拍视频中动物跟踪的准确性和连续性，为群体动态分析提供了更可靠的技术支持。
...
TL;DR: 提出了一种新颖的全视差注意力机制（OPAM）和视差多信息融合模块（PMIFM），构建了首个显著超越最先进MIC编解码器的分布式多视图图像压缩框架ParaHydra。
...
Method: 提出了全视差注意力机制（OPAM）来显式建模任意信息源之间的相关性和对齐特征。基于OPAM设计了视差多信息融合模块（PMIFM），自适应地整合来自不同源的信息。将PMIFM集成到联合解码器和熵模型中，构建了端到端的DMIC框架ParaHydra。
...
Conclusion: 提出的OPAM机制和ParaHydra框架有效解决了DMIC中忽略视图间相关性差异的问题，在保持低计算开销的同时实现了显著的性能提升，且随着输入视图数量的增加，性能增益更加明显。
...
TL;DR: 该论文研究了基于图像的提示注入攻击，通过在自然图像中嵌入对抗性指令来操控多模态大语言模型的行为，攻击成功率最高可达64%。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型整合了视觉和文本能力，但这种整合引入了新的安全漏洞。研究者关注图像提示注入攻击，这是一种黑盒攻击方式，通过在自然图像中嵌入对抗性指令来覆盖模型原有行为。

Method: 开发了端到端的图像提示注入管道，包含基于分割的区域选择、自适应字体缩放和背景感知渲染技术，以隐藏人类可感知的提示同时保持模型可解释性。使用COCO数据集和GPT-4-turbo评估了12种对抗性提示策略和多种嵌入配置。

Result: 实验结果表明，图像提示注入攻击能够可靠地操控模型输出，在最有效的配置下，在满足隐蔽性约束的条件下攻击成功率可达64%。

Conclusion: 图像提示注入攻击是黑盒设置中的实际威胁，突显了需要针对多模态提示注入攻击开发防御机制的必要性。
...
TL;DR: InfinityStory框架通过背景一致性生成管道和过渡感知视频合成模块，解决了长视频生成中的背景一致性、多主体镜头过渡和可扩展性三大挑战，在VBench评测中取得了最佳表现。
...
Method: 提出了背景一致性生成管道来保持跨场景的视觉连贯性，同时保留角色身份和空间关系；开发了过渡感知视频合成模块，专门处理多主体进入或退出帧的复杂场景过渡；创建了包含10,000个多主体过渡序列的合成数据集。

Result: 在VBench评测中，InfinityStory获得了最高的背景一致性分数(88.94)、最高的主体一致性分数(82.11)和最佳总体平均排名(2.80)，显示出改进的稳定性、更平滑的过渡和更好的时间连贯性。

Conclusion: 该研究提出的框架、数据集和模型有效解决了长视频生成中的关键挑战，特别是在背景一致性、多主体过渡和可扩展性方面取得了显著进展，为长篇故事视频合成提供了新的解决方案。
...
TL;DR: SCFlowFR：一种基于数据依赖耦合和捷径约束的单步人脸修复方法，通过建模低质量-高质量数据依赖关系实现高效高质量修复


<details>
  <summary>Details</summary>
Motivation: 现有基于流匹配的人脸修复方法从高斯噪声开始，忽略了低质量(LQ)和高质量(HQ)数据之间的内在依赖关系，导致路径交叉、弯曲轨迹和多步采样需求

Method: 1) 建立数据依赖耦合，显式建模LQ-HQ依赖关系，最小化路径交叉并促进近线性传输；2) 使用条件均值估计获得粗预测，细化源锚点以加强耦合并稳定大步长更新；3) 引入捷径约束监督任意时间间隔内的平均速度，实现精确单步推理

Result: SCFlowFR在单步人脸修复质量上达到最先进水平，推理速度与传统非扩散方法相当

Conclusion: 通过数据依赖耦合、条件均值估计和捷径约束，SCFlowFR有效解决了流匹配方法在人脸修复中的路径交叉和采样效率问题，实现了高效高质量的单步修复
...
TL;DR: 论文提出了首个评估图像编辑中中间路径推理能力的基准InEdit-Bench，发现现有模型在动态推理方面存在显著不足


<details>
  <summary>Details</summary>
Motivation: 当前多模态生成模型在静态图像编辑任务上表现优异，但在需要动态推理的复杂场景中能力有限，无法建模从初始状态到最终状态的多步演化逻辑路径，这限制了模型在视觉操作中的程序和因果理解能力

Method: 提出了InEdit-Bench评估基准，包含四个基本任务类别：状态转换、动态过程、时间序列和科学模拟；同时提出了一套评估标准，用于评估生成路径的逻辑连贯性、视觉自然度以及模型对指定路径约束的忠实度

Result: 对14个代表性图像编辑模型在InEdit-Bench上的综合评估显示，这些模型在该领域存在显著且普遍的缺陷
...
TL;DR: EvoPrune是一种针对多模态大语言模型的早期视觉令牌剪枝方法，在视觉编码阶段直接进行剪枝，显著提升推理效率
...
Method: 提出EvoPrune方法，在视觉编码阶段直接进行令牌剪枝。采用分层剪枝策略，基于令牌相似性、多样性和注意力重要性来指导剪枝过程，在选定的编码层中保留最具信息量的视觉令牌。

Result: 在图像和视频基准测试中验证了EvoPrune的有效性。特别是在VideoMME数据集上，实现了2倍的推理加速，性能下降小于1%。

Conclusion: EvoPrune通过早期视觉令牌剪枝有效解决了多模态大语言模型的推理效率瓶颈，为延迟敏感的应用部署提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本MRI重建方法依赖单模态无条件先验，在严重不适定问题下会产生幻觉。临床实践中常可获得互补的MRI采集（如高质量结构扫描），但现有方法缺乏利用这些额外信息的机制。

Method: 提出MPFlow框架：1）基于整流流构建零样本多模态重建；2）提出PAMRI自监督预训练策略，学习跨模态共享表示；3）采样时联合使用数据一致性和跨模态特征对齐，抑制内在和外在幻觉。
...
TL;DR: 本文提出LDP-Slicing框架，通过将像素值分解为二进制位平面，解决了局部差分隐私在图像数据上应用时的高维效用损失问题，在保证像素级ε-LDP的同时保持了图像的下游任务效用。
...
Method: 提出LDP-Slicing框架：1）将像素值分解为二进制位平面序列，将LDP机制直接应用于比特级表示；2）集成感知混淆模块，减轻人类可感知的信息泄露；3）采用基于优化的隐私预算分配策略。该框架轻量级且无需训练。

Result: 在人脸识别和图像分类任务上的大量实验表明，LDP-Slicing在可比较的隐私预算下优于现有的DP/LDP基线方法，且计算开销可忽略不计。该框架满足严格的像素级ε-LDP，同时生成的图像对下游任务保持高效用。

Conclusion: 通过将像素值转换为比特级表示，LDP-Slicing解决了LDP在图像数据上的域不匹配问题，证明了LDP在高维图像数据上的实用性，为隐私保护图像处理提供了有效的解决方案。
...
TL;DR: 提出QD-PCQA框架，通过质量感知域适应解决无参考点云质量评估的泛化问题，包含排序加权条件对齐和质量引导特征增强两大策略。


<details>
  <summary>Details</summary>
Motivation: 无参考点云质量评估面临泛化能力不足的问题，主要由于标注数据稀缺。人类视觉系统对感知质量的评估与媒体类型无关，因此可以从图像质量评估中迁移先验知识。现有基于域适应的PCQA方法往往忽视感知质量的关键特性，如对质量排序的敏感性和质量感知特征对齐。

Method: 提出QD-PCQA框架，包含两个核心组件：1) 排序加权条件对齐策略，在一致质量水平下对齐特征并自适应强调排序错误的样本；2) 质量引导特征增强策略，包括质量引导风格混合、多层扩展和双域增强模块。

Result: 广泛的跨域实验表明，QD-PCQA显著提升了无参考点云质量评估任务的泛化性能。

Conclusion: 通过质量感知的域适应框架，QD-PCQA有效解决了无参考点云质量评估的泛化问题，为从图像质量评估迁移知识到点云质量评估提供了有效途径。
...
TL;DR: DAGE是一个双流Transformer模型，通过分离全局一致性和细节处理，从非标定多视角/视频输入中高效估计几何和相机姿态，支持2K分辨率并保持实际推理成本。


<details>
  <summary>Details</summary>
Motivation: 从非标定的多视角/视频输入中准确估计视图一致的几何和相机姿态仍然具有挑战性，特别是在高空间分辨率和长序列情况下。现有方法难以同时处理全局一致性和细节保留。

Method: 采用双流Transformer架构：低分辨率流处理降采样帧，通过交替帧/全局注意力构建视图一致表示并估计相机姿态；高分辨率流处理原始图像以保留锐利边界和小结构；轻量级适配器通过交叉注意力融合两流，注入全局上下文而不干扰预训练的单帧路径。

Result: DAGE能够独立扩展分辨率和片段长度，支持高达2K的输入，保持实际推理成本，提供锐利的深度/点云图、强大的跨视图一致性和准确的姿态估计，在视频几何估计和多视角重建方面达到新的最先进水平。

Conclusion: DAGE通过解耦全局一致性和细节处理的双流设计，有效解决了高分辨率长序列下的几何和相机姿态估计问题，为视频和多视角重建任务提供了可扩展且高效的解决方案。
...
Method: 提出基于隐式神经表示的WSI-INR框架，将WSI建模为从空间坐标直接映射到组织语义特征的连续隐式函数；采用多分辨率哈希网格编码，将不同分辨率级别视为同一连续组织的不同采样密度；通过联合训练共享INR解码器捕获跨病例的通用先验

Result: WSI-INR在不同分辨率下保持稳健的分割性能：在Base/4分辨率下，分辨率特定优化使Dice分数提升+26.11%，而U-Net和TransUNet分别下降54.28%和36.18%
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶感知和仿真中的度量深度估计方法难以同时实现高度量精度、多视角和时间一致性以及跨域泛化。这些挑战限制了自动驾驶系统的可靠性和可扩展性。
...
Conclusion: DriveMVS通过结合LiDAR几何提示和深度多线索融合，解决了自动驾驶深度估计中的关键挑战，展示了其对于可扩展、可靠自动驾驶系统的实用价值。
...
TL;DR: 提出一种无配对的64 mT到3T MRI图像转换框架，通过多步精炼和对抗性目标增强，在提升图像真实感的同时保持解剖结构


<details>
  <summary>Details</summary>
Motivation: 超低场（64 mT）脑部MRI提高了可及性，但图像质量比3T差。由于配对的64 mT-3T扫描稀缺，需要开发无配对的质量提升方法

Method: 基于无配对神经薛定谔桥（UNSB）的多步精炼框架，通过DMD2风格扩散引导分布匹配增强目标分布对齐，结合PatchNCE和解剖结构保留（ASP）正则化器来保持全局结构

Result: 在两个独立队列上评估，该框架实现了改进的真实感-结构权衡，在无配对基准上提升了分布级真实感，在配对队列上相比无配对基线提高了结构保真度

Conclusion: 提出的无配对64 mT到3T转换框架有效平衡了图像真实感和解剖结构保留，为超低场MRI图像质量提升提供了可行解决方案
...
TL;DR: 本文提出了一种名为STAR的创新自回归预训练方法，专门用于Vision Mamba模型，通过引入分隔符来区分不同图像，从而将输入序列长度扩展四倍，在保持原始图像尺寸的同时显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前的自回归预训练方法受限于短序列任务，无法充分利用Mamba在处理长序列方面的优势。Mamba固有的因果机制使其特别适合自回归预训练，但现有方法未能充分发挥其在处理扩展序列方面的潜力。

Method: 提出STAR方法（Separators for AutoRegressive pretraining），在每张图像前插入相同的分隔符来标记其起始位置。这种方法可以在保持数据集图像原始尺寸的同时，将Vision Mamba的输入序列长度扩展四倍，从而更好地利用长序列预训练的优势。

Result: 使用这种长序列预训练技术，STAR-B模型在ImageNet-1k上达到了83.5%的准确率，这在Vision Mamba模型中具有高度竞争力。

Conclusion: 该方法通过改进长距离依赖关系的利用，展示了在增强视觉模型性能方面的潜力，为Vision Mamba的自回归预训练提供了有效的长序列处理方案。
...
TL;DR: 本文提出使用向量量化自编码器(VQAE)压缩数据集蒸馏中的软标签，在保持性能的同时显著减少存储开销，在ImageNet-1K上实现30-40倍压缩。


<details>
  <summary>Details</summary>
Motivation: 数据集蒸馏技术虽然能减少训练成本，但软标签及其增强版本带来了显著的存储和通信开销，特别是在大规模分类任务如ImageNet-1K中，软标签成为存储成本的主要贡献者。
...
Conclusion: 提出的VQAE方法能有效解决数据集蒸馏中软标签的存储开销问题，在保持性能的同时实现显著压缩，为实际应用中的存储和通信效率提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示调优方法在封闭组合零样本学习(CZSL)中表现良好，但在开放词汇组合零样本学习(OV-CZSL)中难以泛化到未见过的属性和对象。人类面对新概念时会通过与语义相似的已知概念进行类比来推理，受此启发，作者发现语义相关的属性/对象在嵌入空间中倾向于形成一致的局部结构。
...
Conclusion: 通过利用语义相关概念在嵌入空间中的局部结构一致性，SPA方法能够有效提升模型从可见概念向未见概念泛化的能力，为开放词汇组合零样本学习提供了有效的解决方案。
...
TL;DR: 该研究通过引入视觉注意力分数(VAS)分析多模态大型推理模型的冷启动阶段，发现推理性能与VAS高度相关，并揭示了"懒惰注意力定位"现象。基于此提出了AVAR框架，显著提升了多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大型推理模型(MLRMs)的冷启动初始化阶段对训练至关重要，但其机制尚未被充分理解。研究者希望深入分析这一阶段，揭示影响多模态推理性能的关键因素。

Method: 引入视觉注意力分数(VAS)作为评估指标，量化模型对视觉token的关注程度。通过实验发现VAS与推理性能的高度相关性，揭示"懒惰注意力定位"现象。提出AVAR框架，包含视觉锚定数据合成、注意力引导目标和视觉锚定奖励塑造三个组件。

Result: 研究发现推理性能与VAS高度相关(r=0.9616)。多模态冷启动未能提升VAS，而文本冷启动能明显增加VAS。提出的AVAR框架在Qwen2.5-VL-7B上实现了平均7.0%的性能提升，覆盖7个多模态推理基准。无需训练的干预方法也能获得1-2%的性能提升。

Conclusion: 冷启动阶段对多模态推理模型至关重要，注意力分布是影响性能的关键因素。AVAR框架通过系统性地引导视觉注意力，有效提升了多模态推理能力，为MLRMs的训练提供了新的见解和方法。
...
Method: 1) 模态交错Transformer学习波段模态专业化，形成可逆光谱仿射基，将任意波段多光谱图像映射到统一潜在空间；2) 潜在扩散桥模型逐步演化潜在表示；3) 桥后验采样耦合潜在扩散与像素空间观测；4) 无限维像素到潜在交互机制捕获全色观测与多光谱表示间的跨域依赖。

Result: FoundPS在广泛的全色锐化任务中一致优于最先进方法，表现出优越的泛化能力和鲁棒性。同时构建了包含多卫星、多场景图像对的PSBench基准数据集。

Conclusion: FoundPS作为一个通用的全色锐化基础模型，通过统一的潜在空间表示和扩散建模，成功解决了现有方法的卫星依赖和场景限制问题，实现了卫星无关和场景鲁棒的图像融合。
...
<details>
  <summary>Details</summary>
Motivation: 受人类在嘈杂环境中通过识别关键线索并将其与完整上下文关联进行视觉证据定位的启发，现有方法追求一次性定位完整证据存在局限，需要更鲁棒的视觉基础推理方法。

Method: 提出DeepScan框架，包含三个核心组件：1）分层扫描进行局部线索探索和多尺度证据提取；2）重新聚焦通过LVLMs和视觉专家协作优化证据视图；3）证据增强推理通过混合证据记忆聚合多粒度视图。

Result: 实验表明DeepScan显著提升LVLMs在多样化视觉任务中的性能，特别是在细粒度视觉理解方面。与Qwen2.5-VL-7B集成时，在V*数据集上达到90.6%的总体准确率，且无需额外适应成本即可为不同架构和规模的LVLMs提供一致改进。

Conclusion: DeepScan通过模仿人类的视觉证据定位机制，提供了一种有效的训练免费框架，能够显著提升大型视觉语言模型在视觉基础推理任务中的性能，特别是在存在干扰环境的细粒度视觉理解场景中。
...
<details>
  <summary>Details</summary>
Motivation: 当前唇形同步方法存在根本缺陷：基于mask的方法存在局部颜色差异问题，而无mask方法则难以处理全局背景纹理对齐。此外，大多数方法难以应对多样化真实场景，如风格化头像、面部遮挡和极端光照条件。

Method: UniSync采用统一框架，使用无mask的姿势锚定训练策略来保持头部运动并消除合成颜色伪影，同时采用基于mask的混合一致性推理来确保结构精度和平滑混合。通过在紧凑但多样化的视频上进行微调，赋予模型卓越的领域适应性。

Result: UniSync在提出的RealWorld-LipSync基准测试中显著优于现有最先进方法，该基准涵盖了人类面部和风格化头像等多样化应用场景，展示了卓越的领域适应性和处理复杂边界情况的能力。

Conclusion: UniSync通过统一框架解决了唇形同步中的关键挑战，推动了该领域向真正可泛化和生产就绪的唇形同步技术发展，为高质量视频配音提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 构建可靠的分类器是机器学习在实际应用中部署的基本挑战。现有研究通常将OOD检测和失败预测视为独立问题，忽视了它们之间的紧密联系，而可靠性需要同时评估这两个方面。

Method: 提出了一个统一评估框架，整合OOD检测和失败预测，使用双评分函数（double scoring functions）和新的评估指标DS-F1和DS-AURC。同时扩展了可靠分类器SURE，提出了SURE+方法。

Result: 在OpenOOD基准测试中，双评分函数相比传统单评分方法显著提高了分类器的可靠性。分析表明OOD方法在简单或远OOD偏移下效果明显，但在更具挑战性的近OOD条件下收益有限。SURE+方法在各种场景下显著提高了可靠性。

Conclusion: 该研究提出的框架、指标和方法为可信分类建立了新基准，为在实际环境中部署鲁棒模型提供了实用指导。双评分函数和SURE+方法显著提升了分类系统的可靠性。
...
TL;DR: MATA是一种用于无人机目标跟踪的模块化异步架构，结合了基于Transformer的跟踪器和扩展卡尔曼滤波，通过稀疏光流进行自运动补偿，并提出了新的嵌入式评估协议和NT2F指标。
...
Result: 在UAV基准测试（包括带有合成遮挡的增强版UAV123数据集）上，在多个跟踪处理频率下，成功率和NT2F指标均显示出一致的改进。在Nvidia Jetson AGX Orin上的ROS 2实现证实评估协议更接近嵌入式系统的实时性能。

Conclusion: MATA架构通过结合深度学习和传统滤波方法，在保持实时性能的同时提高了无人机目标跟踪的鲁棒性，提出的评估协议和NT2F指标更适合嵌入式系统的实际应用场景。
...
TL;DR: 本文提出了FGAesthetics细粒度图像美学评估数据库和FGAesQ框架，解决了现有IAA模型在区分细微美学差异方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有图像美学评估模型主要针对粗粒度评估设计，无法有效区分具有细微美学差异的图像。在实际应用中（如内容创作、相册管理、推荐系统等），经常需要从一系列美学差异细微的图像中挑选出最具美感的图像，因此需要细粒度IAA方法。

Method: 1. 构建FGAesthetics数据库：包含32,217张图像，组织成10,028个系列，涵盖自然、AIGC、裁剪等类别，通过系列内成对比较进行标注；2. 提出FGAesQ框架：通过差异保留标记化、比较文本辅助对齐和秩感知回归，从相对排序中学习判别性美学分数。

Result: FGAesQ在细粒度场景下实现了准确的美学评估，同时在粗粒度评估中保持竞争力。广泛的实验和比较证明了该方法的优越性。

Conclusion: 本文提出的FGAesthetics数据库和FGAesQ框架有效解决了细粒度图像美学评估问题，填补了现有IAA模型在区分细微美学差异方面的空白，为实际应用提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于实例的开放集语义建图方法存在两个主要瓶颈：上下文信息缺失和计算成本高昂的裁剪式特征提取。传统方法需要裁剪图像区域进行特征提取，这导致延迟和域偏移问题，限制了实时机器人部署。

Method: DISC采用单次距离加权提取机制，直接从视觉变换器的中间层获取高保真CLIP嵌入，避免了图像裁剪。该方法基于完全GPU加速架构，用精确的实时体素级实例细化替代了周期性的离线处理，支持大规模连续建图。

Result: 在标准基准测试（Replica、ScanNet）和新生成的大规模建图数据集（基于Habitat-Matterport 3D）上的评估表明，DISC在语义准确性和查询检索方面显著优于当前最先进的零样本方法，能够扩展到多层建筑的复杂场景。

Conclusion: DISC提供了一个鲁棒、实时可行的机器人部署框架，通过消除裁剪瓶颈和实现高效的特征提取，显著提升了开放集语义建图的性能。源代码和数据生成评估流程将开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督多模态工业异常检测方法依赖内存库、师生架构或脆弱的融合方案，在噪声深度、弱纹理或缺失模态下鲁棒性有限，需要更可靠、灵活的解决方案。
...
Conclusion: CMDR-IAD框架在多模态和单模态工业异常检测中展现出鲁棒性、模态灵活性和有效性，提出的融合策略在深度稀疏或低纹理区域也能实现稳定精确的异常定位。
...
TL;DR: 使用深度学习检测乳腺DWI高b值图像中的高信号和低信号伪影，DenseNet121模型在二元分类中表现最佳，AUROC分别达到0.92和0.94。
...
Method: 采用三种CNN架构（DenseNet121、ResNet18、SEResNet50）对乳腺DWI高b值图像（b=1500 s/mm2）进行二元分类（伪影存在与否）和多分类（伪影强度）训练。使用11806张切片数据集，最佳模型在独立测试集上评估，并通过Grad-CAM热图生成预测边界框。

Result: DenseNet121在二元分类中表现最佳，高信号伪影检测AUROC为0.92，低信号伪影为0.94；多分类加权AUROC分别为0.85和0.88。放射科医生对边界框精度的评估平均得分为3.33±1.04（高信号）和2.62±0.81（低信号）。

Conclusion: 使用CNN特别是DenseNet121检测乳腺DWI高b值图像中的高信号和低信号伪影具有前景，但需要进一步验证。
...
TL;DR: 该论文提出了空间因果预测（SCP）新任务范式，构建了包含2,500个QA对的SCP-Bench基准，评估了23个SOTA模型，发现模型在空间因果推理方面存在显著差距。
...
Method: 提出空间因果预测（SCP）任务范式，构建SCP-Bench基准数据集，包含2,500个QA对和1,181个视频，涵盖多样化视角、场景和因果方向。对23个最先进模型进行全面实验分析。

Result: 实验揭示了人类与模型性能之间的显著差距，模型在时间外推和因果基础方面表现有限。分析了影响性能的关键因素，提出了感知增强和推理引导的策略。

Conclusion: 该研究为评估和提升空间因果智能提供了新范式，揭示了当前模型的局限性，并提出了改进方向，有助于推动空间推理能力的发展。
...
TL;DR: ProFound是一个专门针对前列腺多参数MRI的视觉基础模型，通过大规模自监督预训练在5000名患者数据上，在11项临床任务中表现出色，超越或匹配现有专业模型。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌的诊断和治疗任务越来越依赖多参数MRI，但自动化这些任务面临挑战：需要专家解释难以规模化，现有系统需要大量特定任务标注数据，限制了临床实用性。

Method: 开发ProFound——针对体积前列腺mpMRI的领域专用视觉基础模型，使用多种自监督学习方法在5000名患者（超过22,000个3D MRI体积，180万张2D图像切片）上进行预训练。

Result: 在3000多名独立患者的11项下游临床任务评估中，微调后的ProFound始终优于或与最先进的专用模型以及现有医学视觉基础模型表现相当，包括前列腺癌检测、Gleason分级、病灶定位等任务。

Conclusion: ProFound作为前列腺MRI的领域专用基础模型，通过大规模自监督预训练，在多项临床任务中展现出卓越性能，有望提升前列腺癌诊断和治疗的自动化水平。
...
<details>
  <summary>Details</summary>
Motivation: 解决从任意角色概念生成高质量、像素级完美Minecraft皮肤的问题，传统方法难以保证生成皮肤的一致性和准确性。

Method: 采用双阶段管道：1) 3D预览合成阶段使用大型多模态模型生成一致的双面板（前/后）斜视角Minecraft风格预览；2) 皮肤解码阶段使用微调的FLUX.2模型将预览转换为皮肤图集图像。提出EvolveLoRA渐进式LoRA课程学习策略。

Result: 开发了完整的开源角色到皮肤生成系统，包含所有提示模板和微调权重，支持可复现的角色到皮肤生成。

Conclusion: BLOCK通过创新的双阶段架构和渐进式训练策略，实现了从任意角色概念到高质量Minecraft皮肤的有效生成，为游戏内容创作提供了新工具。
...
TL;DR: UniRain是一个统一的图像去雨框架，能够处理雨线和雨滴两种退化类型，适用于白天和夜间条件，通过RAG数据集蒸馏和多目标重加权优化实现更好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像去雨方法通常只针对特定类型的雨退化（如雨线或雨滴），无法泛化到多样化的真实世界雨天场景。需要建立一个统一的框架来有效建模不同类型的雨退化。

Method: 1. 构建基于检索增强生成(RAG)的数据集蒸馏管道，从所有公开去雨数据集中选择高质量训练样本进行混合训练；2. 在非对称专家混合(MoE)架构中融入多目标重加权优化策略，提高跨场景的鲁棒性。

Result: 在提出的基准测试和多个公开数据集上，UniRain框架表现优于现有最先进模型，展示了在多样化真实世界雨天场景中的良好泛化能力。

Conclusion: UniRain是一个有效的统一图像去雨框架，能够处理不同类型的雨退化（雨线和雨滴）以及不同光照条件（白天和夜间），通过创新的数据集蒸馏和优化策略实现了更好的跨场景泛化性能。
...
TL;DR: 提出一种新颖的自监督预训练方法，通过从视觉基础模型蒸馏知识来提升事件表示的质量，解决了事件流数据标注困难的问题。


<details>
  <summary>Details</summary>
Motivation: 事件流数据的细粒度表示学习面临标注困难的问题，限制了数据集规模、语义丰富性和应用范围的扩展。现有蒸馏方法由于图像和事件域在稀疏性和粒度上的不匹配，容易导致事件表示的语义崩溃。

Method: 构建大规模同步图像-事件数据集，提出结构感知蒸馏损失，利用视觉基础模型提供的语义结构信息来建立更高质量的图像-事件对应关系，优化密集事件表示。

Result: 在多个下游基准测试中取得显著提升，大幅超越传统方法和现有预训练技术，表现出更好的泛化能力、数据效率和可迁移性。

Conclusion: 通过结构感知蒸馏方法成功解决了图像和事件域之间的不匹配问题，为事件表示学习提供了有效的自监督预训练方案，推动了事件表示学习的发展。
...
TL;DR: GeoSeg是一个零样本、无需训练的多模态大语言模型框架，用于遥感图像推理分割，通过坐标修正和双路径提示机制解决遥感领域特定挑战


<details>
  <summary>Details</summary>
Motivation: 遥感领域缺乏通用的推理分割解决方案，主要障碍包括：1) 推理导向数据成本高昂；2) 遥感特有的挑战（如俯视视角）。现有方法无法有效处理遥感图像中的推理分割任务

Method: GeoSeg采用零样本、无需训练的方法，将MLLM推理与精确定位相结合：1) 偏置感知坐标修正：纠正系统性的定位偏移；2) 双路径提示机制：融合语义意图与细粒度空间线索

Result: 实验表明GeoSeg在所有基线方法中表现最优，同时作者还创建了GeoSeg-Bench诊断基准（包含810个图像-查询对，具有分层难度级别），验证了各组件有效性和必要性

Conclusion: GeoSeg成功绕过了遥感推理分割的监督瓶颈，为遥感领域提供了一种通用的零样本分割解决方案，通过结合MLLM推理和精确定位技术，有效解决了遥感特定挑战
...
TL;DR: 该论文提出了RIVER Bench基准测试，用于评估在线视频理解能力，包含回顾记忆、实时感知和前瞻预测三个任务，解决了现有多模态大语言模型缺乏实时交互能力的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要在离线模式下运行，缺乏实时交互能力，这限制了它们在真实世界应用中的实用性。为了填补这一空白，需要建立一个专门评估在线视频理解的基准测试。

Method: 作者设计了RIVER Bench基准测试，包含三个核心任务：回顾记忆（Retrospective Memory）、实时感知（Live-Perception）和前瞻预测（Proactive Anticipation）。使用来自不同来源、不同长度的视频进行详细标注，并精确定义了实时交互格式。针对现有模型在长时记忆和未来感知方面的不足，提出了一种通用的改进方法。

Result: 评估结果显示，离线模型在单次问答任务中表现良好，但在实时处理方面存在困难。提出的改进方法使模型能够在实时环境中更灵活地与用户交互。

Conclusion: RIVER Bench基准测试将显著推动实时交互视频理解模型的发展，并为这一新兴领域的研究提供启发。数据集和代码已公开可用。
...
TL;DR: 论文提出一个表征层面的诊断框架，分析视觉模型在面部幻视图像中的检测、定位、不确定性和偏见，发现不同模型在模糊性下的三种解释机制


<details>
  <summary>Details</summary>
Motivation: 当视觉证据模糊时，视觉模型需要决定是否将类似面部的模式解释为有意义。面部幻视（在非面部物体中感知到面部）为研究这种行为提供了受控的探针，旨在理解不同模型在模糊性下的解释机制
...
Result: 发现三种模糊性下的解释机制：视觉语言模型表现出语义过度激活，将模糊的非人类区域系统性地拉向人类概念；ViT采用不确定性作为弃权策略；基于检测的模型通过保守先验实现低偏见。结果表明模糊性下的行为更多由表征选择而非分数阈值决定，不确定性和偏见是解耦的

Conclusion: 面部幻视提供了一个紧凑的诊断工具和模糊感知的困难负样本来源，可用于探索和改进视觉语言系统的语义鲁棒性。代码将在发表后发布
...
### [48] [Weakly Supervised Patch Annotation for Improved Screening of Diabetic Retinopathy](https://arxiv.org/abs/2603.03991)
*Shramana Dey,Abhirup Banerjee,B. Uma Shankar,Ramachandran Rajalakshmi,Sushmita Mitra*

Main category: cs.CV

TL;DR: SAFE框架通过弱监督、对比学习和patch-wise嵌入推理，系统性扩展糖尿病视网膜病变的稀疏标注，提升下游分类任务性能


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变早期检测面临标注不足的挑战，现有方法无法系统性标注未标记的病变区域，专家标注又耗时且不完整，限制了深度学习模型的性能

Method: 提出两阶段SAFE框架：第一阶段使用双分支Patch Embedding Network从专家标注的patch中学习语义结构化的类别判别嵌入；第二阶段通过独立嵌入空间的集成，基于空间和语义相似性将标签外推到未标注区域，并包含弃权机制平衡可靠标注与噪声覆盖

Result: 实验结果显示健康与病变patch的可靠分离，准确率高达0.9886；SAFE生成的标注显著改善下游DR分类任务，疾病类F1分数大幅提升，AUPRC性能增益高达0.545；定性分析和可解释性验证表明SAFE关注临床相关病变模式

Conclusion: SAFE框架能够系统性扩展稀疏标注，在部分临床监督下保留病变的细粒度细节，显著提升糖尿病视网膜病变检测性能，并获得眼科医生的验证
...
<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法主要依赖强化学习的几何奖励来指导最终定位，但无法判断推理过程是否始终锚定在目标区域还是偏离到无关上下文。缺乏这种判别性指导会导致模型推理变得冗长且不聚焦，最终在复杂场景中无法准确感知目标。
...
Conclusion: 通过引入判别感知能力来补充强化学习目标，能够显著改善推理分割性能，同时描述性标题还提供了与分割结果对齐的可解释性依据。
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成AI方法临床实用性不足，强化学习虽具潜力但在此任务中尚未充分探索。本文旨在重新审视强化学习在R2G任务中的数据效率和优化效果问题。
...
Conclusion: 该研究证明了强化学习在放射学报告生成任务中的有效性，特别是通过关注数据质量和临床相关令牌优化，可以在减少训练数据的同时提高临床准确性。
...
TL;DR: VDD是一种新的3D病变分割方法，通过将生成轨迹锚定到确定性共识先验，解决了传统确定性模型忽略不确定性和标准扩散模型导致结构断裂的问题，在保持分割准确性的同时实现了更好的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 3D病变分割存在高度观察者间变异性，传统确定性模型忽略这种不确定性，产生过度自信的掩码；而标准扩散模型虽然能捕捉样本多样性，但从纯噪声恢复复杂拓扑结构时经常导致严重结构断裂和解剖幻觉。

Method: 提出Volumetric Directional Diffusion (VDD)方法，与标准扩散模型不同，VDD将生成轨迹数学锚定到确定性共识先验，通过将生成搜索空间限制为迭代预测3D边界残差场，在不冒拓扑崩溃风险的情况下准确探索专家分歧中的细粒度几何变化。
...
Method: 提出DQE-CIR方法，包含两个核心组件：1）可学习的属性权重：根据修改文本强调区分性视觉特征，实现语言和视觉特征更精确的对齐；2）目标相对负采样：构建目标相对相似度分布，从中间区域选择信息丰富的负样本，排除简单负样本和模糊的假负样本。

Result: 该方法通过提高查询表示的区分性和减少语义相似但不相关候选带来的混淆，实现了对细粒度属性变化更可靠的检索。
...
TL;DR: 该研究提出了首个用于海底环境长期视觉定位的精选数据集，包含5个海底参考站点的地理参考AUV图像，时间跨度长达6年，并开发了一种基于3D海底图像足迹的地面真值标注方法，用于评估视觉地点识别方法在动态海底环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 海底环境中的长期视觉定位研究不足，主要缺乏用于基准测试的精选数据集，且现有的地理参考精度和图像覆盖范围有限，需要精确的几何信息来进行准确的地面真值标注。

Method: 1) 创建包含5个海底参考站点、时间跨度长达6年的地理参考AUV图像数据集，包括原始和颜色校正的立体图像、相机校准和亚分米级注册的相机姿态；2) 开发基于3D海底图像足迹的地面真值标注方法，通过连接具有重叠足迹的相机视图来确保地面真值链接反映共享的视觉内容。

Result: 1) 这是首个涵盖多个站点和光带栖息地的水下长期视觉定位精选数据集；2) 对8种最先进的视觉地点识别方法进行基准测试，发现Recall@K显著低于已建立的陆地和水平基准；3) 与传统基于位置的地面真值相比，基于足迹的地面真值标注显示，在崎岖地形和高度变化的站点，基于距离阈值的地面真值标注会高估VPR Recall@K。

Conclusion: 精选的数据集、地面真值标注方法和VPR基准测试为推进动态海底环境中的长期视觉定位研究提供了重要基础，揭示了现有方法在海底环境中的局限性，并强调了精确几何信息对于准确评估的重要性。
...
TL;DR: 提出一个结合生物物理肿瘤生长模型与生成式AI的框架，从肿瘤浓度场合成生物学真实的3D脑MRI，用于个性化肿瘤进展可视化和可控合成数据生成。
...
Method: 结合生成模型与肿瘤浸润图，通过生物物理生长模型在时间上传播，在保持患者解剖结构的同时实现对肿瘤形状和生长的细粒度控制。

Result: 在纵向胶质母细胞瘤病例上评估，能生成时间一致的序列，显示肿瘤外观和周围组织反应的现实变化。纵向外推中，与生物物理模型保持75% Dice重叠，周围组织PSNR保持25。
...
TL;DR: 针对40x40像素的细胞病理图像分类，研究发现任务特定架构在足够数据下优于基础模型，小型Vision Transformer在精度和效率上表现最佳，基础模型在小样本场景中优势有限。


<details>
  <summary>Details</summary>
Motivation: 细胞级病理图像分析需要处理极小的图像块（40x40像素），远低于标准ImageNet分辨率。目前尚不清楚现代深度学习架构和基础模型能否在这种约束下学习到稳健且可扩展的表征。本研究旨在系统评估小图像块细胞分类的架构适用性和数据规模效应。

Method: 分析了303个结直肠癌标本的CD103/CD8免疫染色图像，生成了185,432个标注的细胞图像。训练了8个任务特定架构（从零开始训练），在多个数据规模下（每类256-16,384个样本）进行评估，同时评估了3个基础模型通过线性探测和微调的方式。通过应用高斯扰动（在调整大小前后）评估了模型对模糊的鲁棒性。

Result: 任务特定模型随着数据规模的增加持续改进，而基础模型在中等样本量时达到饱和。针对小图像块优化的Vision Transformer（CustomViT）获得了最高准确率，以显著更低的推理成本优于所有基础模型。各架构的模糊鲁棒性相当，基础模型未显示出定性优势。
...
TL;DR: EgoPoseFormer v2：用于AR/VR的自我中心人体姿态估计方法，通过transformer架构和自动标注系统解决视角受限、遮挡和数据稀缺问题，在精度和实时性上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 自我中心视角下的人体姿态估计面临三大挑战：视角覆盖有限、频繁遮挡和标注数据稀缺，这些限制了AR/VR应用的发展。

Method: 提出两个核心贡献：1）基于transformer的时空一致姿态估计模型，包含身份条件查询、多视角空间细化、因果时间注意力等创新；2）不确定性感知半监督自动标注系统，利用师生框架生成伪标签并扩展训练数据。

Result: 在EgoBody3M基准测试中，GPU延迟仅0.8ms，精度超越两个SOTA方法12.2%和19.4%，时间抖动减少22.2%和51.7%；自动标注系统进一步将手腕MPJPE降低13.1%。

Conclusion: EgoPoseFormer v2通过创新的transformer架构和可扩展的自动标注系统，有效解决了自我中心姿态估计的关键挑战，在精度、实时性和泛化能力上均取得显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 植物生长动态建模对农业研究至关重要，但多视角植物图像存在视角冗余和视角依赖的外观变化，导致学习鲁棒预测器面临挑战。

Method: 提出层级感知视觉语言框架：1) 将旋转视角聚合成角度不变表示；2) 使用轻量级文本先验编码视角层级来调节视觉特征；3) 基于CLIP嵌入构建单任务多模型，实现稳定预测。

Result: 在GroMo25基准测试中，平均年龄MAE从7.74降至3.91（提升49.5%），平均叶片数量MAE从5.52降至3.08（提升44.2%），同时简化了传统双模型设置。

Conclusion: 该方法通过统一的单模型框架显著提升了植物生长预测的准确性，增强了对缺失视角的鲁棒性，简化了预测流程，为农业研究提供了有效的多视角植物图像分析工具。
...
TL;DR: 提出一种基于眼动追踪的双标准帧筛选方法，通过注视点评估视觉稳定性（质量）和瞳孔反应评估新奇度，在10%存储预算下达到全流分类性能


<details>
  <summary>Details</summary>
Motivation: 解决头戴式设备在存储和电池限制下，如何从冗余低质量的第一人称视角视频流中高效筛选有用帧的问题

Method: 使用眼动追踪数据，通过注视点筛选高质量帧（视觉稳定性），再用瞳孔反应对筛选后的帧进行新奇度排序，形成双标准帧筛选器

Result: 在VEDB数据集上，仅用10%存储预算的筛选帧能达到全视频流的分类性能；瞳孔排序提升活动识别，注视点筛选对场景识别更有效

Conclusion: 眼动追踪的注视点和瞳孔反应信号具有互补作用，无需模型推理即可在采集时实现高效帧筛选，为持续第一人称数据管理提供可行方案
...
Method: 提出基于解耦表示学习的受控框架，将脑部MRI分解为抑制采集影响的解剖结构表示和捕获采集相关特征的对比度嵌入。在完整图像、解剖表示和仅对比度嵌入上训练年龄、性别和种族的预测模型，以量化结构和采集对人口统计学信号的相对贡献。

Result: 在三个数据集和多个MRI序列中，发现人口统计学可预测性主要根植于解剖变异：解剖结构表示在很大程度上保留了原始图像训练模型的性能。仅对比度嵌入保留了较弱但系统的信号，这种信号是数据集特定的，无法跨站点泛化。
...
TL;DR: Pointer-CAD是一种基于大语言模型的CAD生成框架，通过指针机制解决传统命令序列方法无法选择几何实体和量化误差的问题，显著提升了复杂几何结构的生成能力。


<details>
  <summary>Details</summary>
Motivation: 传统基于LLM的CAD生成方法使用命令序列表示，但存在两个主要问题：1) 无法支持实体选择（如面或边），限制了复杂编辑操作（如倒角或圆角）的能力；2) 在草图和拉伸操作中对连续变量的离散化会导致拓扑错误。

Method: 提出Pointer-CAD框架，采用基于指针的命令序列表示，将几何信息显式纳入序列建模。框架将CAD模型生成分解为步骤，每个后续步骤的生成都基于文本描述和先前步骤生成的B-rep。当操作需要选择特定几何实体时，LLM预测指针从可用集合中选择最特征一致的候选。还开发了数据标注流程，构建了约57.5万个CAD模型的数据集。

Result: 实验结果表明，Pointer-CAD有效支持复杂几何结构的生成，将分割错误降至极低水平，相比先前的命令序列方法有显著改进，显著减轻了量化误差引入的拓扑不准确性。

Conclusion: Pointer-CAD通过指针机制解决了传统LLM-based CAD生成方法的局限性，能够支持实体选择和复杂编辑操作，同时减少量化误差，为CAD模型的自动化生成提供了更有效的解决方案。
...
TL;DR: Any2Any是一个统一的潜在扩散框架，用于遥感图像的多模态任意到任意转换，通过共享潜在表示实现高效跨模态翻译，避免传统方法的二次复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 多模态遥感图像提供了同一地理场景的互补观测，但在实践中经常不完整。现有跨模态转换方法将每个模态对视为独立任务，导致二次复杂度且难以泛化到未见过的模态组合。

Method: 将任意到任意转换形式化为对场景共享潜在表示的推断，提出Any2Any框架：1)将异构输入投影到几何对齐的潜在空间；2)使用共享主干进行锚定潜在回归；3)轻量级目标特定残差适配器校正系统潜在不匹配；4)引入RST-1M百万级遥感数据集支持稀疏但连接的监督学习。

Result: 在14个转换任务上的实验表明，Any2Any始终优于成对转换方法，并对未见过的模态对表现出强大的零样本泛化能力。

Conclusion: Any2Any通过共享潜在表示的统一框架，有效解决了多模态遥感图像转换的效率和泛化问题，为任意模态组合的转换提供了可扩展的解决方案。
...
### [62] [TaxonRL: Reinforcement Learning with Intermediate Rewards for Interpretable Fine-Grained Visual Reasoning](https://arxiv.org/abs/2603.04380)
*Maximilian von Klinski,Maximilian Schall*

Main category: cs.CV

TL;DR: TaxonRL使用强化学习和分层分类推理，在细粒度视觉识别任务上超越人类表现，达到91.7%准确率
...
Method: 提出TaxonRL方法，使用强化学习和Group Relative Policy Optimization，通过中间奖励将推理过程分解为分层分类预测（物种级、属级、科级），激励模型在最终分类前明确推理各层次特征。

Result: 在Birds-to-Words数据集上达到91.7%平均准确率，超越人类表现（77.3%），并生成可解释的推理轨迹。在灵长类和海洋物种验证任务上表现出强大的跨领域泛化能力。

Conclusion: 强制执行结构化、分层推理为细粒度视觉识别提供了强大且可迁移的框架，不仅提高准确性，还产生透明、可验证的决策过程。
...
TL;DR: TextBoost：一种超低比特率图像压缩方法，通过OCR提取辅助文本信息并传输，在解码端利用语义指导增强小字体文本质量，同时保持整体图像质量


<details>
  <summary>Details</summary>
Motivation: 超低比特率图像压缩面临的关键挑战是：在保持整体视觉质量的同时，保留小字体场景文本。传统的感兴趣区域（ROI）比特分配方法虽然可以优先处理文本，但往往会降低全局保真度，导致局部准确性和整体图像质量之间的权衡问题。
...
Conclusion: TextBoost通过传输OCR提取的辅助文本信息作为语义指导，成功解决了超低比特率图像压缩中小字体文本保留的问题，避免了传统ROI编码的权衡问题，实现了文本质量和整体图像质量的双重提升。
...
TL;DR: 该论文提出了一种基于特征残差判别器（FR-Disc）的架构扩展，用于解决视频领域中的少样本开放集动作识别问题，在保持闭集准确性的同时显著提升了未知类别的拒绝能力。


<details>
  <summary>Details</summary>
Motivation: 现有的少样本动作识别通常基于闭集假设，难以应对现实世界中的开放集场景。虽然少样本开放集识别在图像领域已有研究，但在时空视频数据中的应用仍未被充分探索。

Method: 提出基于特征残差判别器（FR-Disc）的架构扩展，将先前在骨骼数据上的工作适配到更复杂的视频领域，用于区分已知和未知类别。

Result: 在五个数据集上的实验表明，常见的开放集技术仅带来边际改进，而FR-Disc在保持闭集准确性的同时显著增强了未知类别拒绝能力，创造了FSOS-AR的新SOTA。

Conclusion: 该方法成功地将少样本开放集识别扩展到视频领域，通过FR-Disc架构有效解决了视频动作识别中的开放集问题，为实际应用提供了更可靠的解决方案。
...
TL;DR: Crab⁺是一个解决音频-视觉任务异质性问题的统一场景理解模型，通过AV-UIE v2数据集和I-LoRA技术实现正迁移，在88%的任务上超越单任务基线。
...
Method: 1. 数据层面：构建AV-UIE v2数据集，包含约222K样本，覆盖17个数据集和7个任务，提供显式推理过程；2. 模型层面：设计统一接口对齐异构任务，提出交互感知LoRA（I-LoRA），通过动态路由显式建模任务间关系，协调不同的音频-视觉交互模式。

Result: Crab⁺覆盖比现有统一模型更广泛的任务，在各种基准测试中超越专用模型。成功逆转负迁移趋势，在多任务学习中88%的任务超越单任务基线。这些结果在不同AV-LLM范式中得到验证。

Conclusion: Crab⁺通过数据和模型层面的显式合作解决了音频-视觉任务异质性，实现了正迁移，是迈向整体音频-视觉场景理解的稳健一步。
...
TL;DR: 提出一种用于胸部X光反事实生成的推理时注意力调控框架，通过解剖感知和病理引导模块解决扩散模型编辑中的结构漂移和病理表达不稳定问题


<details>
  <summary>Details</summary>
Motivation: 胸部X光反事实生成旨在模拟真实的病理变化同时保留患者特异性解剖结构。然而，基于扩散的编辑方法存在结构漂移问题（稳定解剖语义通过注意力全局传播扭曲非目标区域）和病理表达不稳定问题（细微局部病变产生弱噪声条件信号）

Method: 提出推理时注意力调控框架：1）解剖感知注意力正则化模块，使用器官掩码门控自注意力和解剖标记交叉注意力，将结构交互限制在解剖ROI内；2）病理引导模块，在早期去噪阶段增强目标肺区域内的病理标记交叉注意力，并通过注意力集中能量驱动的轻量级潜在校正实现可控病变定位和程度

Result: 在胸部X光数据集上的广泛评估显示，相比标准扩散编辑方法，该方法在解剖一致性和精确可控的病理编辑方面表现更好，支持局部反事实分析和下游任务的数据增强

Conclusion: 提出的推理时注意力调控框架能够可靠地进行胸部X光反事实合成，通过解剖感知和病理引导的注意力调控解决了扩散模型编辑中的关键问题，为医学影像分析提供了更好的工具
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型（如CNN和Transformer）在局部特征建模和全局依赖捕获方面存在局限性：CNN受限于局部感受野，Transformer在有效建模局部结构方面有不足，两者都存在模型复杂度高和可解释性不足的问题。

Method: 设计基于可学习迭代收缩阈值算法（LISTA）的稀疏Transformer，将LISTA稀疏编码与视觉Transformer深度融合，构建具有自适应局部和全局特征协作机制的模型架构。使用连续小波变换将振动信号转换为时频图，输入到LISTA-Transformer中进行更有效的特征提取。

Result: 在CWRU数据集上，该方法的故障识别率达到98.5%，比传统方法提高3.3%，并显示出相对于现有Transformer方法的优势。

Conclusion: 提出的LISTA-Transformer方法通过融合稀疏编码和Transformer架构，有效解决了局部特征建模和全局依赖捕获的平衡问题，在工业故障诊断任务中取得了优异的性能。
...
TL;DR: 该研究提出了一种增强训练框架，通过在训练图像中应用多样化的人工退化，提升野生动物重识别模型在图像退化条件下的性能。


<details>
  <summary>Details</summary>
Motivation: 野生动物重识别面临图像退化问题（如模糊、遮挡等），这些退化因素显著降低了模型识别个体动物的能力，限制了生态研究的应用。现有基于深度度量学习的模型在退化图像上性能下降明显，且不同物种受影响程度不同。

Method: 提出增强训练框架，在训练集中对部分个体的图像应用多样化的人工退化（如模糊、噪声等）。通过这种方式训练深度特征提取器，即使对未见过的个体也能提升在退化条件下的重识别性能。

Result: 在18个野生动物数据集上验证，该方法在真实退化图像上能提升高达8.5%的Rank-1准确率。研究首次提供了人类专家标注的退化图像数据集，并公开了代码和数据。

Conclusion: 这是首个系统研究图像退化对野生动物重识别影响的工作，提出的增强训练框架能有效提升模型在退化条件下的鲁棒性，为后续研究提供了基准和工具。
...
TL;DR: NOVA3R是一种从无位姿图像进行非像素对齐3D重建的有效方法，通过全局场景表示和扩散解码器实现更完整、更准确的3D重建。


<details>
  <summary>Details</summary>
Motivation: 传统像素对齐方法存在两个关键限制：1) 只能重建可见点，无法恢复不可见点；2) 在重叠区域会产生重复结构。NOVA3R旨在解决这些问题，实现更完整、物理合理的3D重建。

Method: 提出场景令牌机制聚合无位姿图像信息，结合基于扩散的3D解码器，学习全局、视角无关的场景表示，从而解耦重建与像素对齐，生成完整的非像素对齐点云。

Result: 在场景级和物体级数据集上的大量实验表明，NOVA3R在重建精度和完整性方面优于现有最先进方法。

Conclusion: NOVA3R通过非像素对齐的全局场景表示方法，有效解决了像素对齐3D重建的局限性，实现了更完整、更准确的3D重建，为无位姿图像重建提供了新思路。
...
Method: 对OmniDocBench v1.5的1,355张图像进行完整的一对一物理重建，涵盖五种关键真实场景：扫描、扭曲、屏幕拍摄、光照和倾斜，建立完整的真实映射关系。

Result: 该基准测试为社区建立了具有挑战性的新标准，表明文档解析中的"现实差距"远未弥合，并提供了诊断工具来指导真正具有弹性的文档智能系统开发。
...
<details>
  <summary>Details</summary>
Motivation: 病理基础模型在细胞核检测与分类任务中表现不如预期，研究发现联合优化会导致表示退化，且检测与分类任务难度差异大，使得联合优化计算负担过重。

Method: 提出DeNuC方法：使用轻量级模型进行细胞核定位，然后基于检测坐标利用病理基础模型提取细胞核特征进行分类，实现检测与分类的解耦。

Result: 在三个基准数据集上显著超越现有方法，在BRCAM2C和PUMA数据集上F1分数分别提升4.2%和3.6%以上，且仅使用其他方法16%或更少的可训练参数。

Conclusion: DeNuC有效释放了病理基础模型在细胞核检测与分类任务中的表示潜力，通过解耦策略解决了联合优化的瓶颈问题，实现了更高效、更准确的细胞核分析。
...
<details>
  <summary>Details</summary>
Motivation: 在具身任务中，智能体需要在线、近实时地构建和理解3D场景。现有开放词汇3DGS方法通常局限于离线或逐场景优化设置，无法满足在线实时需求

Method: 提出在线稀疏系数场与CLIP全局码本，将2D CLIP嵌入绑定到每个3D高斯，同时最小化内存消耗。通过3D U-Net聚合3DGS的部分点云生成3D几何感知的CLIP特征，补偿2D语言嵌入的3D几何先验

Result: 在ScanNet、ScanNet++和Replica等室内数据集上的广泛实验证明了方法的有效性和效率，能够从超过300张流式图像中在线重建语义嵌入的3DGS

Conclusion: EmbodiedSplat实现了同时在线3D重建和3D语义理解，具有高度泛化性，支持近实时3D语义重建，为具身任务提供了有效的场景理解解决方案
...
TL;DR: ViterbiPlanNet是一个将程序知识显式集成到学习过程中的框架，通过可微维特比层嵌入程序知识图，实现端到端优化，在多个数据集上以更少参数达到SOTA性能。
...
Result: 在CrossTask、COIN和NIV数据集上达到最先进性能，参数比基于扩散和LLM的规划器少一个数量级。消融实验显示性能提升来自可微结构感知训练而非后处理精炼，提高了样本效率和未见较短时间范围的鲁棒性。

Conclusion: ViterbiPlanNet通过显式整合程序知识图实现高效的程序规划，建立了统一的测试协议和统计显著性评估方法，为智能体在复杂环境中的规划提供了更高效的解决方案。
...
### [74] [SSR: A Generic Framework for Text-Aided Map Compression for Localization](https://arxiv.org/abs/2603.04272)
...
TL;DR: 提出文本增强压缩框架，通过文本描述和小图像特征向量结合，使用相似性空间复制技术实现高保真定位，压缩率比基线提高2倍


<details>
  <summary>Details</summary>
Motivation: 机器人地图存储、传输和云端查询面临内存和带宽成本过高的问题，需要一种高效的压缩方法

Method: 提出文本增强压缩框架，将文本作为替代模态，结合轻量级文本描述和小图像特征向量；开发相似性空间复制技术，学习自适应图像嵌入，仅捕获与文本描述互补的信息

Result: 在多个定位任务上验证，包括视觉地点识别和物体中心蒙特卡洛定位，在TokyoVal、Pittsburgh30k、Replica和KITTI数据集上实现比基线2倍更好的压缩效果

Conclusion: 文本增强压缩框架能显著减少内存和带宽占用，同时保持高保真定位性能，为机器人地图存储和传输提供有效解决方案
...
TL;DR: 该研究探讨了利用深度学习技术结合序列数据和时间信息来改进结肠息肉检测和分割，以降低漏检率并提高结直肠癌预防效果。


<details>
  <summary>Details</summary>
Motivation: 结肠息肉作为结直肠癌的前兆，其检测和切除在临床实践中存在挑战。息肉的外观、位置和大小差异大，加上结肠结构的复杂性，导致漏检和不完全切除率高，影响患者预后。现有自动化方法虽有所改进，但缺乏对时间序列信息的利用，无法捕捉息肉生长的动态变化。

Method: 研究采用多中心、多样化人群的综合性数据集，由数据科学家和胃肠病学专家合作编制。重点评估深度学习技术在实时临床结肠镜检查任务中的应用，特别关注序列数据和时间信息的作用。通过分析帧间的时间关系，开发更精确的自动检测和分割方法。

Result: 研究表明，整合序列数据和时间信息能显著提升息肉检测和分割的精度。通过捕捉息肉生长的动态特性和随时间的变化，深度学习方法在实时临床应用中展现出改善诊断准确性的潜力，有助于降低漏检率。

Conclusion: 在自动化息肉检测和分割方法中纳入序列数据和时间信息至关重要。这种整合能够更好地反映息肉的动态生长过程，提高诊断精度，最终实现更有效的患者管理和结直肠癌预防。多学科合作的数据集构建为未来研究提供了重要基础。
...
TL;DR: Gaussian Wardrobe是一个从多视角视频数字化组合式3D神经化身的框架，通过将人体与服装分离为可组合的层次，实现服装在不同个体间的复用。


<details>
  <summary>Details</summary>
Motivation: 现有3D神经化身方法将人体和服装视为不可分割的整体，这无法捕捉复杂自由形态服装的动态特性，也限制了服装在不同个体间的复用。

Method: 开发了一种新颖的组合式3D高斯表示，将神经化身分解为身体和多个形状无关的神经服装层，从多视角视频中解耦每个服装层并将其规范化为形状独立的空间。

Result: 该方法能够建模具有高保真动态的逼真化身，在新姿态合成基准测试中达到了新的最先进性能，并且学习的组合式服装可用于虚拟试穿应用。

Conclusion: Gaussian Wardrobe框架成功实现了服装与身体的解耦，创建了可复用的数字衣橱，为虚拟试穿等应用提供了实用解决方案。
...
TL;DR: CubeComposer是一个新颖的时空自回归扩散模型，能够原生生成4K分辨率的360°全景视频，通过立方体贴图表示和精心规划的时空顺序来降低内存需求并实现高质量输出。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于普通扩散模型的计算限制，仅支持≤1K分辨率的原生生成，并依赖次优的后处理超分辨率来提高分辨率。对于VR应用来说，高分辨率视频对沉浸式体验至关重要。

Method: 将视频分解为六个面的立方体贴图表示，采用时空自回归策略在立方体面和时间窗口上协调生成；提出立方体面上下文管理机制和稀疏上下文注意力设计以提高效率；采用连续性感知技术（立方体感知位置编码、填充和混合）消除边界接缝。

Result: 在基准数据集上的广泛实验表明，CubeComposer在原生分辨率和视觉质量方面优于最先进的方法，支持实际的VR应用场景。

Conclusion: CubeComposer通过创新的立方体贴图表示和时空自回归策略，成功实现了原生4K分辨率360°视频生成，解决了现有方法在分辨率和计算效率方面的限制。
...
TL;DR: MMFA是一种基于无监督关键点定位的面部动画新方法，通过自监督表示学习和变分自编码器实现身份语义与运动信息的解耦，支持任意运动控制和表情插值。


<details>
  <summary>Details</summary>
Motivation: 现有基于无监督关键点定位的面部动画方法虽然能生成逼真细节，但无法实现可控的面部生成，因为现有关键点分解流程未能完全解耦身份语义和交织的运动信息（如旋转、平移和表情）。
...
Conclusion: MMFA通过创新的无监督关键点定位方法，成功解决了面部动画中身份语义与运动信息解耦的难题，实现了可控的面部生成和表情插值，为面部动画领域提供了新的解决方案。
...
TL;DR: PromptAvatar：一个基于双扩散模型的高效3D头像生成框架，通过多模态数据集实现文本/图像到3D的直接映射，10秒内生成高质量头像


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动方法依赖迭代优化，语义控制差且推理慢；图像驱动方法受高质量3D面部扫描数据稀缺限制。需要解决生成质量、语义控制和效率问题

Method: 构建包含10万对多模态数据的大规模数据集，提出双扩散模型框架：纹理扩散模型（TDM）支持文本/图像多条件引导，几何扩散模型（GDM）由文本引导，实现从多模态提示到3D表示的直接映射

Result: 方法在10秒内生成高质量、无阴影的3D头像，在生成质量、细粒度细节对齐和计算效率方面显著优于现有最先进方法

Conclusion: PromptAvatar通过大规模多模态数据集和双扩散模型架构，成功解决了3D头像生成中的语义控制、数据稀缺和效率问题，实现了高质量快速生成
...
TL;DR: 该研究提出了MOO数据集，一个包含1000头牛从128个均匀视角拍摄的大规模合成空中-地面动物重识别数据集，用于系统分析视角变化对模型性能的影响。
...
Method: 创建MOO合成数据集，包含1000头牛从128个均匀采样视角的128,000张标注图像；量化分析海拔高度对模型性能的影响，识别关键海拔阈值；验证合成数据在零样本和监督设置下向真实世界应用的迁移能力。
...
TL;DR: 本文提出一个可扩展框架，用于评估合成图像编辑方法的真实性，并将其应用于汽车摄像头图像中添加环境条件（雾、雨、雪、夜间）。生成式AI方法显著优于基于规则的方法，最佳生成方法达到最佳规则方法约3.6倍的接受率。
...
Method: 提出一个可扩展框架，用于评估合成图像编辑方法的真实性。使用40张晴天图像，比较基于规则的增强库与生成式AI图像编辑模型。真实性通过两种互补的自动指标评估：视觉语言模型（VLM）评审团进行感知真实性评估，以及基于嵌入的分布分析来测量与真实恶劣条件图像的相似性。

Result: 生成式AI方法显著优于基于规则方法，最佳生成方法达到最佳规则方法约3.6倍的接受率。性能因条件而异：雾最容易模拟，而夜间转换仍然具有挑战性。值得注意的是，VLM评审团对真实恶劣条件图像的接受率也不完美，为合成方法设定了实际上限。按此标准，领先的生成方法在大多数条件下达到或超过真实图像性能。

Conclusion: 现代生成式图像编辑模型能够为评估流程提供可扩展的现实恶劣条件图像生成。该框架提供了可扩展真实性评估的实用方法，但未来工作仍需验证与人类研究的一致性。
...
Method: 将铰接HOI合成视为单目视频先验的4D重建问题：1) 基于光流的部件分割，分离动态和静态区域；2) 解耦重建流程，先恢复物体铰接状态，再基于重建的物体状态合成人体运动

Result: 在多种铰接场景（如打开冰箱、橱柜、微波炉）中，ArtHOI在接触精度、穿透减少和铰接保真度方面显著优于现有方法
...
TL;DR: 系统评估了三种生成模型（DDPM、LDM、FM）在心脏MRI合成数据生成中的表现，发现DDPM在数据稀缺条件下在分割性能、图像保真度和隐私保护之间达到最佳平衡。
...
Result: 基于扩散的模型（特别是DDPM）在有限数据条件下在下游分割实用性、图像保真度和隐私保护之间达到最佳平衡；FM显示出有前景的隐私特性但任务性能稍低。
...
TL;DR: 提出HOSO-Adapter方法，通过单样本留出机制学习混合比例参数，无需验证集，在验证无关的少样本设置下显著提升CLIP-Adapter性能


<details>
  <summary>Details</summary>
Motivation: 现有CLIP适应方法需要验证集或测试集调优混合比例参数，不符合严格的少样本学习设定，需要一种无需验证集的方法

Method: 提出Hold-One-Shot-Out (HOSO)方法：留出一个样本作为"验证集"学习混合比例，其余少样本训练适配器，实现验证无关的CLIP适应

Result: 在11个标准少样本数据集上，HOSO-Adapter比CLIP-Adapter基线平均提升4个百分点以上；在8-shot和16-shot设置下，甚至优于在测试集上优化混合比例的CLIP-Adapter

Conclusion: HOSO-Adapter提供了一种简单有效的验证无关少样本CLIP适应方法，通过单样本留出机制学习混合比例，在多个数据集上取得显著性能提升
...
<details>
  <summary>Details</summary>
Motivation: 绘画作者归属鉴定是一个历史悠久的复杂任务，主要挑战在于真实艺术品的训练数据有限，这限制了计算模型的训练效果。

Method: 提出混合方法，结合真实绘画和通过DreamBooth微调Stable Diffusion生成的合成图像数据，使用生成和判别方法相结合的策略来增强分类模型。
...
Method: 使用大语言模型为每张图像生成可信的反事实描述，通过测量视觉语言基础模型区分正确描述与这些困难负样本的能力，构建捕捉模型在共享嵌入空间中判别能力的特征，然后用线性回归器预测零样本测试准确率。

Result: 该方法在五个多样化数据集（包括标准基准数据集和非洲代表性不足数据集）上实现了皮尔逊相关系数0.96的预测性能，能够可靠估计视觉语言基础模型在不同视觉领域的零样本准确率。

Conclusion: 该方法为探测视觉语言基础模型提供了一个低成本、可靠的工具，使研究人员和从业者能够在投入大量资源进行数据标注前做出明智决策。
...
Result: 在PathText-BRCA数据集上进行广泛实验，在标准自然语言生成指标上持续优于现有方法。完整RANGER模型在PathText数据集上达到最优性能：BLEU-1至BLEU-4分别为0.4598、0.3044、0.2036、0.1435，METEOR为0.1883，ROUGE-L为0.3038。

Conclusion: 动态专家路由和自适应知识细化对于语义基础的病理报告生成是有效的，RANGER框架通过稀疏门控MoE和自适应检索重排序显著提升了病理报告生成质量。
...
TL;DR: FocusGraph是一个用于长视频问答的关键帧选择框架，通过场景描述LLM选择器基于图式描述选择相关片段，并使用无需训练的PSFR方法选择关键帧，显著提升长视频理解性能并减少推理时间。


<details>
  <summary>Details</summary>
Motivation: 长视频理解对具身智能体至关重要，但现有多模态大语言模型在处理大量视频帧时存在响应质量下降和推理时间增长的问题。因此需要有效选择关键帧来回答用户查询。

Method: 1. 轻量级可训练的场景描述LLM选择器：基于图式描述选择查询相关片段，不依赖原始低分辨率帧序列；2. 无需训练的补丁级稀疏流保持方法：从选定片段中选择关键帧；3. 将关键帧输入MLLM生成最终答案。

Result: 在具有挑战性的第一人称长视频问答基准测试（FindingDory和HourVideo）上达到最先进水平，同时相对于基线方法显著减少推理时间。

Conclusion: FocusGraph通过创新的关键帧选择框架有效解决了长视频理解中的关键帧选择问题，结合场景描述LLM选择器和PSFR方法，在保持高性能的同时大幅提升效率。
...
TL;DR: Helios是首个14B参数视频生成模型，在单张H100 GPU上达到19.5 FPS，支持分钟级视频生成，质量媲美强基线模型，无需传统防漂移技术和加速技术。


<details>
  <summary>Details</summary>
Motivation: 解决长视频生成中的漂移问题，实现实时生成，同时避免使用复杂的防漂移启发式方法和标准加速技术，简化训练和推理流程。

Method: 采用14B自回归扩散模型，统一输入表示支持T2V、I2V、V2V任务；通过模拟漂移的训练策略解决长视频漂移问题；压缩历史和噪声上下文，减少采样步骤；引入基础设施级优化加速推理和训练。

Result: 在单张H100 GPU上达到19.5 FPS，支持分钟级视频生成，计算成本与1.3B视频生成模型相当或更低，在短视频和长视频生成上均优于先前方法。

Conclusion: Helios在三个关键维度取得突破：无需防漂移启发式方法的长视频稳定性、无需标准加速技术的实时生成、无需并行化框架的大规模训练，为视频生成领域提供了高效实用的解决方案。
...
Conclusion: ZipMap通过状态前馈模型实现了高效、高质量的3D重建，解决了现有方法在计算效率和重建质量之间的权衡问题，为大规模图像集合处理和实时应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前人体运动预测包含轨迹预测和人体姿态预测两个任务，但现有方法通常针对单个任务开发专用模型。将两者结合进行整体预测具有挑战性，现有方法在各自基准测试中表现不佳。需要一种统一模型来处理这两个任务。

Method: 提出一个简单而有效的基于Transformer的模型，使用堆叠的自注意力模块来同时捕捉姿态内的空间依赖关系和运动序列中的时间关系。这是一个简单、流线型的端到端模型，无需任务特定修改即可处理姿态预测、轨迹预测和组合预测任务。

Result: 通过在Human3.6M、AMASS、ETH-UCY和3DPW等多个基准数据集上的广泛实验，证明该方法在所有任务上都达到了最先进的结果。

Conclusion: 提出的Transformer模型能够有效统一处理人体运动预测中的轨迹预测和姿态预测任务，在多个基准数据集上取得SOTA性能，证明了其简单性、有效性和通用性。
...
TL;DR: 研究发现奖励模型存在多种偏见问题，包括长度偏见、谄媚偏见、过度自信偏见、模型风格偏见和答案顺序偏见，并提出了一种后处理干预方法来缓解低复杂度偏见。


<details>
  <summary>Details</summary>
Motivation: 奖励模型在语言模型对齐中至关重要，但基于奖励模型的偏好调整容易受到奖励黑客攻击，语言模型策略可能从有缺陷的奖励模型中学习到不良行为。现有研究虽然已经识别了一些偏见问题，但问题仍然存在，需要更系统的分析和解决方案。

Method: 1. 系统测量了五个高质量奖励模型（包括最先进模型）中的偏见；2. 将奖励模型失败按复杂度分类；3. 提出了一种简单的后处理干预方法来缓解低复杂度偏见，该方法基于机制性奖励塑造，使用最小标记数据，可扩展到新偏见和模型内部，并能泛化到分布外数据。

Result: 研究发现尽管先前工作已有所关注，但长度偏见、谄媚偏见和过度自信偏见问题仍然存在。同时发现了新的偏见问题：模型特定风格偏见和答案顺序偏见。提出的机制性奖励塑造方法能够在不降低奖励质量的情况下减少目标偏见，且使用最小标记数据。

Conclusion: 奖励模型存在多种系统性偏见问题，需要更全面的评估和缓解策略。提出的机制性奖励塑造方法为缓解低复杂度偏见提供了有效解决方案，该方法具有可扩展性和泛化能力，为改进奖励模型对齐提供了实用工具。
...
TL;DR: 本文提出了一种混合LLM架构，通过将事实检索与对话生成解耦，结合监督微调和缝合层，为小农户提供准确、安全、文化适宜的农业建议。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在农业咨询中存在推荐缺乏依据、建议过于笼统缺乏具体可操作性、沟通风格与小农户需求不匹配等问题。在农业这种高风险场景中，推荐准确性直接影响农民收成，这些限制对负责任部署构成挑战。

Method: 提出混合LLM架构：1) 使用LoRA在专家策划的"黄金事实"（原子化、验证的农业知识单元）上进行监督微调，优化事实召回；2) 单独的缝合层将检索到的事实转化为文化适宜、安全感知的响应。评估框架DG-EVAL基于专家策划的基准事实进行原子事实验证。

Result: 在印度比哈尔邦的作物和查询实验中，微调显著提高了事实召回率和F1分数，同时保持高相关性。使用微调的小模型在事实质量上达到或超过前沿模型，成本大幅降低。缝合层进一步提高了安全子分数，同时保持高对话质量。

Conclusion: 混合架构方法能有效解决农业咨询中LLM的局限性，通过解耦事实检索和对话生成，结合领域特定微调和安全感知响应生成，为小农户提供准确、安全、文化适宜的农业建议。发布了farmerchat-prompts库以促进领域特定农业AI的可重复开发。
...
TL;DR: PlugMem是一个任务无关的插件式记忆模块，可为任意LLM智能体提供高效记忆功能，通过知识中心化记忆图结构提升任务相关性和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有记忆设计要么是任务特定且不可迁移，要么是任务无关但效果较差，存在任务相关性低和原始记忆检索导致的上下文爆炸问题。需要一种既通用又高效的记忆解决方案。

Method: 基于认知科学原理，将情景记忆结构化为紧凑、可扩展的知识中心化记忆图，明确表示命题性和规范性知识，以知识而非实体或文本块作为记忆访问和组织的基本单元。

Result: 在三个异构基准测试（长时程对话问答、多跳知识检索和Web智能体任务）中，PlugMem一致优于任务无关基线，甚至超过任务特定记忆设计，同时在统一信息论分析下实现最高信息密度。

Conclusion: PlugMem提供了一个任务无关的插件式记忆解决方案，通过知识中心化表示有效解决了现有记忆设计的局限性，为LLM智能体在复杂环境中的长期记忆需求提供了通用高效的解决方案。
...
TL;DR: TTSR是一个自反思的测试时自进化训练框架，通过让单个语言模型在测试时交替扮演学生和教师角色，解决测试时训练中伪标签不可靠和缺乏针对性适应的问题。


<details>
  <summary>Details</summary>
Motivation: 测试时训练面临两大挑战：1）测试问题难度高，导致自生成的伪标签不可靠；2）现有方法缺乏有效机制来适应模型特定的推理弱点，导致学习效率低下。

Method: TTSR使用单个预训练语言模型，在测试时交替扮演学生和教师角色。学生专注于解决问题并从合成的变体问题中学习，教师则分析学生的失败推理轨迹，总结重复出现的推理弱点，并据此合成有针对性的变体问题。
...
TL;DR: 该研究对10个商业大语言模型在4个学术领域进行了大规模引文幻觉审计，生成了69,557个引文实例，发现幻觉率在11.4%到56.8%之间，受模型、领域和提示框架影响显著。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）存在编造学术引文的问题，但这种现象在不同提供商、领域和提示条件下的范围尚未得到充分量化。研究旨在系统评估LLMs的引文幻觉行为。

Method: 对10个商业部署的LLMs在4个学术领域进行提示，生成69,557个引文实例，通过CrossRef、OpenAlex和Semantic Scholar三个学术数据库进行验证。开发了基于书目字符串特征的轻量级分类器。

Result: 幻觉率在11.4%到56.8%之间，受模型、领域和提示框架显著影响。发现两个实用过滤器：多模型共识（3个以上LLMs引用同一工作准确率达95.6%）和提示内重复（2次以上重复准确率达88.9%）。分类器在交叉验证中AUC达0.876，在LOMO泛化中达0.834。

Conclusion: LLMs的引文幻觉是提示诱导而非固有的，改进并非部署新模型的保证。多模型共识和提示内重复是有效的幻觉检测方法，轻量级分类器可作为推理时的预筛选工具。
...
TL;DR: 本文评估了三种新兴工具在LaborBench法律RAG基准上的表现，发现定制工具STARA准确率达83%（实际92%），而商业平台表现不佳，并提出了法律RAG系统的设计原则。


<details>
  <summary>Details</summary>
Motivation: 法律领域的检索增强生成(RAG)系统缺乏系统性基准测试，先前研究显示标准RAG在法律任务上表现不佳（70%准确率），需要评估新兴工具的实际性能。

Method: 使用LaborBench基准（基于美国劳工部律师手动编制的各州失业保险要求）评估三种工具：定制法定研究助手STARA，以及Westlaw AI和Lexis+ AI两个商业平台。进行全面的错误分析，比较AI输出与人工编制的基准答案。

Result: STARA准确率达83%（经分析实际为92%），显著优于标准RAG；商业平台表现更差：Westlaw AI 58%，Lexis+ AI 64%。错误分析揭示了两类问题：推理错误（法律概念混淆、法规例外误解）和检索失败（未捕获相关法规条款）。研究发现许多"错误"实际上是人工基准的遗漏。

Conclusion: 定制化法律RAG工具（如STARA）能显著提升多司法管辖区法律研究的准确性，但商业AI平台仍需改进。研究提出了构建准确法律RAG系统的具体设计原则，为未来法律AI发展提供了实用指导。
...
TL;DR: 该论文研究大型语言模型（LLM）的语义缓存策略，证明最优离线策略是NP难问题，提出多项式时间启发式算法和结合时效性、频率、局部性的在线策略，实验表明新变体提高了语义准确性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的快速普及，对更快响应和更低成本的需求日益增长。语义缓存通过重用语义相似的请求来满足这一需求，但这打破了传统缓存假设并带来了新挑战。

Method: 1. 探索离线语义缓存策略，证明实现最优离线策略是NP难问题；2. 提出多种多项式时间启发式算法；3. 提出结合时效性、频率和局部性的在线语义感知缓存策略。

Result: 在多样化数据集上的评估表明，虽然基于频率的策略是强基线，但论文提出的新变体提高了语义准确性。研究揭示了当前系统的有效策略，并指出了未来创新的巨大空间。

Conclusion: 论文为LLM语义缓存提供了理论分析和实用策略，证明最优离线策略是NP难问题，提出的启发式和在线策略在实际中表现良好，所有代码已开源，为未来研究奠定了基础。
...
TL;DR: 本文提出了一种基于多智能体架构的检索增强生成框架，用于解决交通机构知识管理中的信息检索和决策支持问题。


<details>
  <summary>Details</summary>
Motivation: 传统知识管理方法（如静态文档、课堂培训和师徒制）导致知识传递碎片化、效率低下，随着资深工程师退休，专业知识逐渐流失。交通机构维护的大量技术手册、指南和研究报告使得工程师难以快速准确地找到相关信息，影响现场问题解决和培训任务准备，阻碍及时决策并增加新员工学习曲线。

Method: 提出基于多智能体架构的检索增强生成框架，集成了结构化文档检索和实时上下文感知响应生成。系统采用多个专门智能体进行检索、答案生成、评估和查询优化，支持迭代改进和质量控制。还引入开放权重视觉语言模型，将技术图表转换为语义文本表示，使图表知识与文本一同被索引和检索。检索到的文本和图表上下文提供给开放权重大语言模型，生成基于检索证据的最终响应。

Result: 论文未在摘要中提供具体实验结果，但描述了系统架构和功能特点。

Conclusion: 该框架通过多智能体RAG系统和视觉语言模型集成，能够有效解决交通机构知识管理中的信息检索和决策支持问题，提高知识传递效率和决策质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM用户模拟器主要模仿表面语言模式，无法反映真实用户的底层心理状态（如信念和情感），限制了用户中心应用的发展

Method: 提出HumanLM框架：1）模型不仅生成响应，还生成与真实响应对齐的自然语言潜在状态；2）使用强化学习确保潜在状态与真实响应一致；3）这些潜在状态对应心理学基础的状态维度；4）将对齐的潜在状态合成为准确代表真实用户的响应

Result: 在Humanual基准测试（6个大规模数据集，26k用户，216k响应）中，HumanLM显著优于替代方法，LLM评判对齐分数平均相对提升16.3%；在111名参与者的实时模拟研究中，HumanLM获得最高真实用户响应相似度和有竞争力的人类相似度分数

Conclusion: HumanLM通过生成与真实用户心理状态对齐的潜在状态，能够更准确地模拟真实用户，为开发更用户中心的应用提供了有效解决方案
...
TL;DR: 本文提出了History-Echoes框架，研究大型语言模型的对话历史如何影响后续生成，发现历史偏差会导致模型陷入几何陷阱


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）的对话历史如何影响其未来性能，特别是历史中的幻觉等问题如何影响后续模型响应

Method: 提出History-Echoes框架，从两个角度分析对话历史偏差：概率角度（将对话建模为马尔可夫链量化状态一致性）和几何角度（测量连续隐藏表示的一致性）

Result: 在三个模型系列和六个数据集上的分析显示，两个视角之间存在强相关性，行为持续性表现为几何陷阱，潜在空间中的间隙限制了模型的轨迹

Conclusion: 对话历史偏差会导致模型陷入几何陷阱，这为理解LLMs如何受历史影响提供了新的理论框架，有助于改进模型鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 冷启动场景是推荐系统面临的根本障碍，特别是当用户缺乏交互历史或物品元数据稀疏时。传统方法难以处理这些情况，需要创新的解决方案来生成有效的个性化推荐。

Method: 提出一个包含六个组件的混合框架：1) 语义元数据增强（LLM处理），2) 动态图谱构建，3) VARK认知偏好分析，4) 心理状态估计，5) 图谱增强检索与LLM排序，6) 自适应界面设计与迭代学习。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM推理引擎在固定解码规则下优化吞吐量和延迟，将生成视为令牌时间的线性进程，这种范式存在效率限制

Method: 提出熵时间推理范式，引入自组织推理架构，包括熵感知调度、分页注意力块的熵剪枝、以及稳定生成在目标熵区间的自适应温度控制

Result: 将推理转变为资源智能的热力学过程，最大化不确定性减少的计算分配，提出了具体的系统设计、伪代码和集成方案

Conclusion: 熵可以作为可扩展LLM推理的一流控制信号，实现更高效的推理过程
...
TL;DR: Logovista英日机器翻译系统的技术架构、开发实践和保存成果，这是一个从1990年代到2012年商业运营的大型显式规则型MT系统。


<details>
  <summary>Details</summary>
Motivation: 记录一个在真实商业环境中长期部署和演进的规则型机器翻译系统，为技术史研究提供资料，而非主张复兴规则型MT。

Method: 结合手工编写的语法规则、大型中心词典（编码句法和语义约束）、基于图表的解析以及加权解释评分来处理结构歧义。

Result: 系统在数十年间持续扩展和维护，成功应对实际使用压力，包括回归控制、歧义管理等挑战，最终保存了完整的软件和语言资源供未来研究。

Conclusion: Logovista系统作为长期商业部署的规则型MT案例，提供了独特的技术历史记录，其保存的资源对研究机器翻译发展史具有重要价值。
...
TL;DR: SemKey是一个用于从非侵入式EEG信号解码自然语言的多阶段框架，通过四个解耦的语义目标强制信号基础生成，解决了语义偏差、信号忽视和BLEU陷阱三个核心问题。


<details>
  <summary>Details</summary>
Motivation: 当前从EEG信号解码自然语言的最先进模型存在三个基本限制：语义偏差（模式崩溃为通用模板）、信号忽视（基于语言先验而非神经输入产生幻觉）以及BLEU陷阱（评估指标因高频停用词而人为膨胀，掩盖了真实语义保真度的缺乏）。

Method: 提出SemKey多阶段框架，通过四个解耦的语义目标（情感、主题、长度和惊奇度）强制信号基础生成。重新设计神经编码器与大型语言模型之间的交互，将语义提示作为查询注入，EEG嵌入作为键值对，强制模型关注神经输入。采用N-way检索准确率和Fréchet距离等评估指标。

Result: 实验表明，该方法能有效消除噪声输入上的幻觉，并在这些鲁棒协议上实现了最先进的性能。

Conclusion: SemKey通过强制信号基础生成和采用更严格的评估指标，成功解决了当前EEG到文本解码中的核心问题，为从神经信号解码自然语言提供了更可靠的方法。
...
TL;DR: 研究发现通过特定任务微调可以缩小LLMs的"具身化差距"，使其内部表征更接近人类感觉运动经验，但这种改进对学习目标敏感，无法跨不同任务格式迁移。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在显著的"具身化差距"，其基于文本的表征与人类感觉运动经验不匹配。本研究旨在系统探究任务特定微调是否能以及如何弥合这一差距。

Method: 使用表征相似性分析和维度特定相关性度量，研究通过微调将LLMs的内部表征引导向更具身化、更接地气的模式。

Result: 结果显示：1）LLMs的内部表征可以通过微调被引导向更具身化的模式；2）感觉运动改进在不同语言和相关感觉运动维度上具有鲁棒性；3）改进对学习目标高度敏感，无法在两种不同任务格式之间迁移。

Conclusion: 任务特定微调可以有效缩小LLMs的具身化差距，但改进效果受学习目标限制，无法跨任务格式泛化，这为开发更具身化的语言模型提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在现实应用中面临用户提示不完美的问题，现有方法依赖外部工具预处理提示，忽略了模型内在鲁棒性并引入额外计算开销

Method: 提出基于对比学习的逆直接偏好优化方法，最小化干净提示与噪声提示下标签对齐logits的差异，使用互信息理论分析，构建配对提示数据集进行训练

Result: 在NoisyPromptBench基准测试中，CoIPO方法相比当前最先进方法实现了显著的平均准确率提升

Conclusion: CoIPO有效提升了LLM对噪声提示的鲁棒性，无需依赖外部组件，代码、配对数据集和基准测试已开源
...
TL;DR: 该研究探讨了手语识别中迁移学习的效果，通过比较中文到阿拉伯语和希腊语到佛兰芒语两对语言中标志性手势的迁移学习性能，发现当源语言和目标语言之间存在相似性时，迁移学习效果更好。


<details>
  <summary>Details</summary>
Motivation: 大多数手语识别研究依赖于从视觉数据集（如ImageNet）进行迁移学习，有些扩展到其他语言数据集，但往往关注具有跨语言相似性的手势。本研究旨在探讨这些相似性对有效知识转移的必要性。

Method: 使用Google Mediapipe作为输入特征提取器，将手势的空间信息通过多层感知机处理，时间信息通过门控循环单元处理。比较了中文到阿拉伯语和希腊语到佛兰芒语两对语言中标志性手势的迁移学习性能。

Result: 实验结果显示，从中文到阿拉伯语的标志性手势迁移学习性能提高了7.02%，从希腊语到佛兰芒语的迁移学习性能提高了1.07%。这表明当源语言和目标语言之间存在相似性时，迁移学习效果更好。

Conclusion: 手语识别中的迁移学习效果受到源语言和目标语言之间相似性的影响，当存在跨语言相似性时，知识转移更加有效。
...
Conclusion: Retcon技术为多轮对话场景中的LLM控制提供了一种有效的解决方案，能够实现对话过程中的灵活行为调整。
...
Result: QISA在多个指标上显著优于标准自注意力：字符错误率提升15.5倍，词错误率提升4.7倍，交叉熵损失提升13倍。这些性能提升仅以推理时间增加2.6倍为代价。

Conclusion: 量子启发的自注意力机制可以成功集成到完整的语言建模流程中，并在多个性能指标上显著优于传统自注意力机制，为量子启发的NLP方法在实际应用中的可行性提供了有力证据。
...
TL;DR: 该论文研究如何自动发现大型语言模型作为评估者时的偏好特征，通过嵌入层概念提取方法分析LLM评判行为，发现稀疏自编码器方法能提取更可解释的偏好特征，并揭示了LLM与人类评估者在多个领域的系统性差异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地被用作模型输出的可扩展评估者，但它们的偏好判断存在系统性偏见，且可能与人类评估存在差异。先前研究主要关注少量预定义的偏见假设，未能解决自动发现LLM偏好未知驱动因素的问题。

Method: 研究比较了多种嵌入层概念提取方法分析LLM评判行为，特别关注稀疏自编码器方法。使用超过27,000对来自多个人类偏好数据集的响应，以及三个LLM的评判结果，分析LLM判断并与人类标注者进行比较。

Result: 稀疏自编码器方法相比其他方法能恢复更可解释的偏好特征，同时在预测LLM决策方面保持竞争力。研究验证了现有发现（如LLM更倾向于拒绝敏感请求），并揭示了新趋势：在一般和领域特定数据集中，LLM偏向强调具体性和同理心的回应、学术建议中的细节和正式性，以及反对提倡报警和诉讼等积极步骤的法律指导。

Conclusion: 自动化概念发现方法能够在没有预定义偏见分类的情况下，对LLM评判偏好进行系统性分析，为理解LLM作为评估者的行为提供了新工具。
...
TL;DR: 本文提出了一种基于社会科学理论和溯因推理的神经符号方法，用于实现文本的叙事转换，在保持原文核心信息的同时，将文本转换为不同的叙事框架。该方法显著优于现有大语言模型的零样本基线。


<details>
  <summary>Details</summary>
Motivation: 有效沟通通常需要将信息与受众的叙事框架和世界观对齐。叙事转换涉及将文本转换为反映不同叙事框架的形式，同时保持其原始核心信息。当前的大语言模型在这一任务上表现不佳，因此需要开发更有效的方法来实现高质量的叙事转换。

Method: 提出了一种基于社会科学理论和溯因推理的神经符号方法。该方法自动提取规则，通过溯因推理确定所需的故事元素，从而指导大语言模型进行一致且有目标的叙事转换。该方法结合了符号推理和神经语言模型的能力。

Result: 在多个大语言模型上，溯因推理指导的叙事转换方法成功实现了叙事框架的转变，同时保持了与原始故事的高度一致性。例如，使用GPT-4o时，在集体主义到个人主义的叙事转换任务上，比零样本基线提高了55.88%，同时在语义相似性方面也有40.4%的KL散度改进。在个人主义到集体主义的转换任务上也取得了类似的改进。在Llama-4、Grok-4和Deepseek-R1等模型上也表现出相似或竞争性的性能。

Conclusion: 基于社会科学理论和溯因推理的神经符号方法能够有效实现高质量的叙事转换，在多个大语言模型上都显著优于现有基线方法。该方法为文本的叙事框架转换提供了有效的解决方案，有助于实现更有效的跨文化、跨世界观沟通。
...
TL;DR: DBench-Bio是一个动态、自动化的基准测试，用于评估AI在生物医学领域的新知识发现能力，通过三阶段流水线构建月度更新的评估数据集，解决现有静态基准的数据污染和过时问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要依赖静态数据集，导致不可避免的数据污染问题（模型可能在训练时已经见过评估知识），且现代LLM快速发布周期使得静态基准很快过时，无法评估发现真正新知识的能力。

Method: 采用三阶段流水线：1) 获取严谨、权威的论文摘要数据；2) 使用LLM合成科学假设问题和相应的发现答案；3) 基于相关性、清晰度和中心性进行QA过滤以确保质量。该流水线实例化为覆盖12个生物医学子领域的月度更新基准。

Result: 对最先进模型的广泛评估揭示了当前在发现新知识方面的局限性。DBench-Bio提供了首个动态、自动化的框架来评估AI系统的新知识发现能力。

Conclusion: 该工作为AI研究社区建立了一个活生生的、不断演化的资源，用于催化知识发现的发展，解决了现有基准测试的关键局限性。
...
TL;DR: 该论文提出了一种名为DCR（通过对比精炼的辨别）的新方法，用于解决大型语言模型安全对齐中的过度拒绝问题，在减少误判的同时保持对真正有害内容的安全拒绝能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在进行安全对齐时经常出现过度拒绝问题，即模型倾向于拒绝看似有毒或良性的提示，将其误分类为有毒内容。这种行为削弱了模型的帮助性，并限制了在敏感或微妙上下文中的可用性。现有缓解策略通常面临权衡：减少过度拒绝通常会降低模型拒绝真正有害内容的能力。

Method: 引入了一个前置对齐阶段DCR：通过对比精炼的辨别。该方法使用对比精炼来提升LLM区分真正有毒提示和表面有毒提示的能力。从理论和实证两方面证明了对比精炼的有效性。

Result: 在多样化基准测试中的评估表明，该方法有效减少了过度拒绝，同时保持了安全对齐的好处。重要的是，该方法在实现这一目标时对通用能力的退化最小。

Conclusion: DCR方法为安全对齐提供了一个更原则性和稳健的方向，通过对比精炼改善了模型对有毒和表面有毒内容的辨别能力，在保持安全性的同时提高了模型的可用性。
...
TL;DR: 该论文提出了一种基于表示工程的轻量级、无需训练的方法，通过线性方向控制大语言模型中的风格属性，实现精确的风格控制。


<details>
  <summary>Details</summary>
Motivation: 当前控制大语言模型中风格属性的方法主要依赖于提示工程或训练后对齐，这些方法存在局限性。论文旨在探索风格属性是否在模型的激活空间中编码为线性方向，并基于此开发更有效的控制方法。

Method: 采用表示工程视角，假设不同风格属性（从情感基调到语言结构）在模型激活空间中编码为线性方向。基于这一假设，开发了一种轻量级、无需训练的方法，支持线性风格组合，并通过消融不良行为来增强安全性。

Result: 在十多个模型上进行的实验证实了风格属性编码为线性方向的假设，该方法能够实现高风格遵从度，同时以最小计算成本保留核心能力。

Conclusion: 风格属性确实在LLM激活空间中编码为线性方向，基于这一发现的方法为精确、高效、安全的风格控制提供了新途径，具有线性组合能力和计算效率优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM人格对齐方法依赖昂贵的SFT或RLHF，需要为每个目标人格训练独立模型，而推理时激活转向方法存在向量干扰问题，无法同时控制多个特征

Method: 提出Sequential Adaptive Steering (SAS)方法：通过在前序干预的残差流上训练后续探针来正交化转向向量，将转向向量转化为可重用基元，用户只需调整系数α即可合成复杂人格特征

Result: 在Big Five人格特质上验证，SAS在目标遵循度和一致性方面优于基线方法，能够实现精确、整体的人格调制而无需更新模型参数

Conclusion: SAS框架为LLM提供了模块化、连续的多维度人格控制能力，通过正交化转向向量避免了破坏性干扰，使复杂人格特征的即时合成成为可能
...
Conclusion: 通过构建多任务多标签中文对话数据集，解决了现有资源不足的问题，为更准确地预测用户满意度和理解情感动态变化提供了新的研究基础。
...
TL;DR: StructLens是一个分析语言模型内部结构的框架，通过构建基于残差流语义表示的最大生成树来量化层间结构相似性，发现与传统余弦相似性不同的模式，并在层剪枝等任务中显示实用价值。


<details>
  <summary>Details</summary>
Motivation: 语言具有内在结构特性，语言模型也应体现内部结构。现有可解释性研究主要关注层内或模块内的局部token关系，而忽略了全局层间关系。需要填补这一空白，从结构角度理解语言模型的内部工作机制。
...
Conclusion: 结构分析为理解语言模型内部工作机制提供了新视角，StructLens框架能够揭示传统方法忽略的层间结构关系，并在模型优化任务中展现实用价值。
...
TL;DR: Gemini-2.5-Flash可以通过自动合成代码约束来防止非法操作，使较小模型在游戏环境中超越更大模型，甚至能生成完整代码策略替代LLM决策。


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型近年来取得显著进展，但作为智能体使用时，它们经常尝试执行不仅次优而且被外部环境严格禁止的操作。例如，在最近的Kaggle GameArena国际象棋比赛中，78%的Gemini-2.5-Flash失败归因于非法移动。通常人们需要手动编写"约束"代码来防止此类失败。

Method: 使用Gemini-2.5-Flash自动合成代码约束，通过少量迭代代码精炼轮次，根据（游戏）环境的反馈进行优化。该方法可以扩展到生成整个策略代码，从而在决策时完全不需要使用LLM。

Result: 生成的约束在145个不同的TextArena游戏（包括单人和双人游戏）中防止了所有非法移动，使较小的Gemini-2.5-Flash模型能够超越更大的模型，如Gemini-2.5-Pro。生成的代码策略在16个TextArena单人游戏中获得了比Gemini-2.5-Pro和GPT-5.2-High更高的平均奖励。

Conclusion: 使用较小模型合成自定义代码约束（或完整策略）可以超越更大模型，同时更具成本效益。这表明自动代码生成是提升LLM作为智能体性能的有效途径。
...
TL;DR: 该研究提出了"确定性鲁棒性基准"，这是一个两轮评估框架，用于测试大语言模型在受到挑战时的反应稳定性，发现不同模型在交互可靠性方面存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估单轮准确性、真实性或置信度校准，但无法捕捉模型在交互环境中受到挑战时的行为。大语言模型通常表现出高度自信，但缺乏明确的确定性推理机制，需要评估它们在压力下的稳定性。

Method: 引入确定性鲁棒性基准，一个两轮评估框架，使用自我挑战提示（如"你确定吗？"和"你错了！"）以及数值置信度引导，在LiveBench的200个推理和数学问题上评估四个最先进的大语言模型，区分合理的自我修正和不合理的答案改变。

Result: 研究结果显示，不同模型在交互可靠性方面存在显著差异，这些差异不能仅用基线准确性解释：有些模型在对话压力下放弃了正确答案，而其他模型表现出对挑战的强大抵抗力和更好的置信度与正确性对齐。

Conclusion: 确定性鲁棒性是LLM评估的一个独特且关键的维度，对模型对齐、可信度和实际部署具有重要意义，需要将其纳入评估框架。
...
TL;DR: PulseLM是一个大规模PPG-文本数据集，包含131万个标准化10秒PPG片段和315万个问答对，用于连接原始PPG波形与自然语言处理
...
Method: 从15个公开来源聚合PPG记录，将异构注释统一为12个常见生理问答任务，采用封闭式问答公式化方法
...
TL;DR: 本文系统评估了大型语言模型对思维链提示中5种扰动的鲁棒性，发现不同扰动类型对模型性能影响差异显著，模型规模对某些扰动有保护作用但对单位转换等任务效果有限。


<details>
  <summary>Details</summary>
Motivation: 思维链提示已成为激发大型语言模型推理能力的基础技术，但其中间推理步骤对扰动的鲁棒性尚未得到充分理解。本文旨在系统评估LLM对思维链扰动的脆弱性，为实际部署提供指导。

Method: 构建了5种思维链扰动类型的结构化分类体系：数学错误、单位转换、迎合性、跳过步骤和额外步骤。在13个参数规模从3B到1.5T的模型上进行评估，测试它们在数学推理任务中面对不同位置注入扰动的能力。

Result: 发现异质性脆弱模式：数学错误对小模型影响最严重（50-60%准确率损失）但随规模扩大改善明显；单位转换在所有规模上都具挑战性（即使最大模型也有20-30%损失）；额外步骤影响最小（0-6%损失）；迎合性影响适中（小模型7%损失）；跳过步骤造成中等损害（15%损失）。缩放关系遵循幂律模式。

Conclusion: 模型规模对某些扰动具有保护作用，但对维度推理任务防御有限。研究结果对在多阶段推理管道中部署LLM有直接意义，强调了任务特定鲁棒性评估和缓解策略的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 推测解码通过轻量级草稿模型提出标记并用目标模型选择性接受来加速大语言模型推理。现有方法在匹配草稿标记与目标模型预测分布方面存在局限，需要更有效的接受机制。

Method: DropMatch通过仅对LM头应用蒙特卡洛dropout，生成多个解码路径形成经验标记分布，草稿标记根据与该分布的一致性进行评估接受。该方法自适应控制解码路径规模，防止目标模型预测分布显著失真。

Result: 实验表明，DropMatch在多个基准测试中提高了接受长度，同时保持竞争力的任务性能，推理速度比标准基线提升1.09x到1.33x，在EAGLE3基础上还能额外获得1.09x加速。

Conclusion: DropMatch提供了一种无需训练、无需数据、无需校准的推测解码方法，无需修改预训练模型架构，可与现有推测解码和推理加速技术正交集成，有效提升大语言模型推理效率。
...
TL;DR: 该研究提出了CompMath-MCQ基准数据集，用于评估大语言模型在研究生水平计算数学推理能力，包含1500道原创多项选择题，涵盖线性代数、数值优化、向量微积分、概率和Python科学计算等主题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在数学推理评估主要关注基础问题、竞赛题目或形式化定理证明，而研究生水平和计算数学领域相对缺乏系统评估。需要专门针对高级数学推理能力的基准测试。

Method: 创建CompMath-MCQ数据集：1）由研究生课程教授原创编写1500道多项选择题；2）涵盖五个核心领域；3）采用三选一格式；4）通过跨LLM分歧分析和专家手动审查确保问题质量；5）使用lm_eval库进行客观评估。

Result: 基准测试结果显示，当前最先进的大语言模型在高级计算数学推理方面仍面临显著挑战，表明该领域是LLM能力的重要瓶颈。

Conclusion: CompMath-MCQ填补了高级数学推理评估的空白，为客观、可重复、无偏见的LLM评估提供了新基准，有助于推动LLM在复杂数学推理能力的发展。
...
TL;DR: 研究发现Transformer语言模型中的特定能力高度集中于少数注意力头，通过压缩感知方法可识别这些稀疏的功能组件，揭示了模型能力的模块化组织原则。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型能力在Transformer架构中的分布和组织方式，理解特定能力是否集中在模型的特定组件中，这对模型的可解释性、编辑和AI安全具有重要意义。

Method: 采用压缩感知方法，通过战略性"敲除"少量注意力头并评估模型性能变化，利用这些功能组件的稀疏性来识别与特定能力相关的注意力头。

Result: 在Llama和Qwen模型（1B-8B参数）上验证发现，仅敲除5个任务特定的注意力头就能使相关能力性能下降高达65%，而不影响其他任务，表明能力高度集中在稀疏的注意力头中。

Conclusion: 能力局部化是Transformer语言模型的通用组织原则，特定能力由稀疏、功能不同的组件实现，这一发现对模型可解释性、编辑和AI安全有重要影响。
...
TL;DR: 该论文提出了一种针对大语言模型排名不确定性的统计推断框架，通过构建置信区间来处理人类偏好数据中的噪声和上下文依赖性问题，确保排名决策具有统计有效性。


<details>
  <summary>Details</summary>
Motivation: 当前基于人类偏好数据构建的LLM排名通常使用点估计，忽略了显著的估计噪声和上下文依赖的性能变化。这可能导致当排名差异在统计上不显著时，做出错误的部署决策，造成资源错配和福利损失。

Method: 采用上下文相关的Bradley-Terry-Luce模型，其中每个模型的潜在效用取决于输入提示。不针对效用的点估计，而是直接对诱导排名进行推断，基于成对效用差异的同步置信区间构建置信集，从而获得具有统计有效性的提示特定排名置信集。

Result: 使用大规模LLM评估的人类偏好数据进行实证分析，发现排名在不同提示特征间存在显著变化，许多表面上的排名差异在统计上无法区分。不确定性感知的排名能够仅在数据支持时识别支配关系，否则返回偏序关系。

Conclusion: 该框架将最近的排名推断进展与上下文偏好学习相结合，为基于排名的稳健决策提供了工具。通过统计有效的置信集，可以避免对统计上不显著的排名差异做出过度自信的决策。
...
TL;DR: 该研究使用因果解释性和探针方法分析Llama生物医学语言模型中药物组语义的表征机制，发现药物组知识分布在早期层和多个token中，而非集中在单个token上。
...
Method: 使用因果解释性和探针方法：1）应用激活修补技术定位药物组信息在模型层和token位置中的存储位置；2）使用在线性探针分析token级和池化激活表示。

Result: 早期层在编码药物组知识中起关键作用，最强的因果效应来自药物组跨度内的中间token而非最终token。线性探针显示药理学语义分布在多个token中，在嵌入空间中已存在，token级探针表现接近随机，而池化表示达到最高准确率。

Conclusion: LLMs中的药物组语义不是局部化在单个token上，而是来自分布式表征。这项研究首次对LLMs中的药理学知识进行了系统的机制分析，为理解生物医学语义在大型语言模型中的编码方式提供了见解。
...
TL;DR: LLMs在遇到难度增加（OOD偏移）的输入时，其最后隐藏状态会变得更稀疏，这种稀疏度与难度呈正相关。基于此现象，研究者开发了SG-ICL方法，利用表示稀疏度来安排few-shot演示，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在面对难度逐渐增加（表现为OOD偏移）的输入时，其内部表示如何适应和变化，探索模型处理复杂或陌生输入的机制。

Method: 通过分析不同难度（更难的推理问题、更长上下文、更多答案选项）下LLMs最后隐藏状态的稀疏度变化，发现稀疏度与难度呈正相关。基于此设计了Sparsity-Guided Curriculum In-Context Learning (SG-ICL)方法，利用表示稀疏度来安排few-shot演示的顺序。

Result: 发现了一个一致且可量化的现象：任务难度越大，LLMs最后隐藏状态越稀疏。这种稀疏度-难度关系在不同模型和领域中都存在。SG-ICL方法显著提升了模型性能。

Conclusion: LLMs通过使最后隐藏状态变得更稀疏来适应OOD挑战，这是一种稳定推理的自适应机制。基于稀疏度指导的课程学习策略能有效提升模型性能，为理解LLMs如何内化OOD挑战提供了新的机制性见解。
...
TL;DR: 本文介绍了LilMoo，一个6亿参数的印地语语言模型，通过完全透明可复现的流程从头训练，旨在解决多语言基础模型中的语言不平等问题，在有限计算环境下优化训练，性能优于同规模多语言基线模型。


<details>
  <summary>Details</summary>
Motivation: 大型多语言基础模型主导了自然语言处理领域，但加剧了语言不平等，低资源语言（如印地语）往往代表性不足。现有印地语模型通常依赖于不透明的多语言基础模型进行持续预训练，缺乏透明度和可复现性。

Method: 1. 构建高质量印地语语料库GigaLekh，采用启发式和LLM-as-a-judge方法过滤；2. 使用精选英语数据进行双语增强；3. 在有限计算环境下探索小规模语言模型的训练配方；4. 从头训练6亿参数的印地语专用模型LilMoo。

Result: LilMoo在全面评估套件中持续优于同等规模的多语言基线模型（如Qwen2.5-0.5B和Qwen3-0.6B），表明精心设计的语言专用预训练在十亿参数以下范围内可以与大型多语言模型竞争。

Conclusion: 通过完全透明可复现的流程从头训练语言专用模型，可以在有限计算资源下有效解决低资源语言的代表性问题，为其他语言提供了可借鉴的框架，证明了语言专用预训练在特定规模范围内的竞争力。
...
TL;DR: 该研究提出一个基于模板的模块化认知模型，用于解释名词的语义和形态句法特征变化，特别是性别标记的变异现象。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索名词的多样性特征，包括可数/不可数等语义区分和阳性/阴性等形态句法区分，特别关注跨语言中普遍存在的语法性别转换现象，旨在揭示词位变异的潜在模式。

Method: 提出一个新的计算组件，专门用于将词汇项与形态模板配对，通过模板基模块化认知模型（Template-Based and Modular Cognitive model）形式化表示，该模型由集合值集合函数 h: P(M)→P(M) 定义，预测词汇项到形态模板的非线性动态映射。

Result: 通过经验观察，展示了这些转换（包括非性别转换）在词汇变化过程中如何产生，特别是在Riffian语言中。模型表明这些变异标记是由于单词和意义形成过程中的模板转换而产生的。通过形式化证明转换适用于名词到名词的派生，挑战并拓宽了传统词形形成的观点。

Conclusion: 该数学模型不仅有助于更深入地理解形态句法变异，还为其他需要精确建模语言模式的领域提供了潜在应用，提出了一个统一框架来理解跨语言的形态标记复杂性。
...
TL;DR: 论文提出个性化对话推荐系统的安全问题，开发了SafeRec基准数据集和SafeCRS安全训练框架，在保持推荐质量的同时大幅降低安全违规率。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的对话推荐系统主要优化推荐准确性和用户满意度，但忽视了用户个性化安全约束（如创伤触发、自伤史、恐惧症等）可能被违反的风险，这是一个未被充分探索的脆弱性。
...
Conclusion: 个性化对话推荐系统的安全问题是重要但被忽视的挑战，提出的SafeCRS框架能有效平衡推荐质量与用户个性化安全约束，为构建更安全的推荐系统提供了解决方案。
...
Method: 提出RAG-X诊断框架，独立评估检索器和生成器在三种QA任务上的表现：信息提取、短答案生成和多项选择题回答。引入上下文利用效率（CUE）指标，将系统成功分解为可解释的象限，区分已验证的基础与欺骗性准确性。
...
TL;DR: Tucano 2是一个完全开源的葡萄牙语大型语言模型套件（0.5-37亿参数），通过扩展数据集和引入新训练方法，在葡萄牙语NLP任务上达到最先进性能。
...
Method: 1. 扩展数据集GigaVerbo-v2并引入合成数据集GigaVerbo-v2 Synth填补数据空白；2. 创建后训练数据集GigaVerbo-v2 SFT和GigaVerbo-v2 Preferences支持多领域训练；3. 设计预训练和持续预训练配方；4. 扩展和优化评估框架。

Result: 在多个葡萄牙语建模基准测试中达到最先进的性能，所有相关成果（训练配方、日志、源代码）都开源发布。

Conclusion: Tucano 2为葡萄牙语NLP社区提供了一个全面、可复现的开源解决方案，填补了葡萄牙语LLM开发的空白，并建立了新的性能标准。
...
TL;DR: ByteFlow Net是一种新型分层架构，完全移除分词器，让模型直接从原始字节流中学习语义单元分割，通过压缩驱动的分割策略实现自适应粒度，性能优于传统BPE分词器和之前的字节级架构。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型依赖固定的预定义子词分词器，导致模型只能在固定粒度上操作，产生脆弱和反直觉的行为，限制了模型的适应性和信息处理能力。

Method: 提出ByteFlow Net分层架构，基于潜在表示的编码率进行压缩驱动的分割，通过Top-K选择保持静态计算图，让模型根据输入自适应调整内部表示粒度。

Result: 实验表明这种基于压缩的分块策略带来显著性能提升，ByteFlow Net在性能上优于基于BPE的Transformer和之前的字节级架构。

Conclusion: 端到端、无分词器的建模不仅可行而且更有效，为开发更具适应性和信息基础的语言模型开辟了新路径。
...
<details>
  <summary>Details</summary>
Motivation: 错误信息对社会构成日益严重的威胁，不同人口群体由于基本信念差异而对错误信息的易感性不同。随着大型语言模型越来越多地用于模拟人类行为，需要研究它们是否能模拟人口统计学上的错误信息易感性，将信念作为主要驱动因素。

Method: 提出BeliefSim模拟框架，使用心理学启发的分类法和调查先验构建人口统计学信念档案。研究基于提示的条件化和训练后适应两种方法，并进行多维度评估：包括易感性准确性和反事实人口统计学敏感性。

Result: 在多个数据集和建模策略中，信念为模拟错误信息易感性提供了强有力的先验，准确率高达92%。
...
TL;DR: LX Topic是一种新的神经主题建模方法，将主题概念化为潜在语言构建，通过大语言模型精炼和校准机制提升主题质量，同时保持文档代表性，为商业研究提供标准化、可解释的测量工具。


<details>
  <summary>Details</summary>
Motivation: 当前主题建模方法在商业研究中作为测量工具存在不足：概率模型生成的概念模糊，神经主题模型在理论驱动场景下难以解释，大语言模型方法缺乏标准化、稳定性和文档级表示对齐。需要一种能够产生校准文档级主题比例、适合实证分析的方法。

Method: LX Topic基于FASTopic确保文档代表性，在大语言模型精炼阶段采用对齐和置信度加权机制，增强语义连贯性而不扭曲文档-主题分布。将主题概念化为潜在语言构建，提供标准化输出。
...
TL;DR: LIGRAM：一种用于韩语短文本分类的层次异构图模型，通过构建词素、词性和命名实体子图，并结合语义感知对比学习，有效解决了韩语短文本分类中上下文信息有限和语言特性复杂的问题。


<details>
  <summary>Details</summary>
Motivation: 现有短文本分类方法主要针对英语设计，缺乏对韩语等黏着语的语言特性（如黏着形态和灵活语序）的考虑。韩语短文本分类面临上下文信息有限和标注数据稀缺的挑战。

Method: 提出LIGRAM模型：1）构建词素、词性和命名实体三个层次的异构子图；2）层次化集成这些子图以捕捉韩语的语法和语义依赖；3）应用语义感知对比学习（SemCon）来反映文档间的语义相似性，建立更清晰的决策边界。

Result: 在四个韩语短文本数据集上的评估表明，LIGRAM consistently outperforms existing baseline models。

Conclusion: 将语言特定的图表示与语义感知对比学习相结合，为韩语等黏着语的短文本分类提供了有效的解决方案。
...
TL;DR: MIND是一个用于精神科咨询的统一询问-诊断强化学习框架，通过标准驱动的临床支持库和强化学习机制，解决现有方法在精神科咨询中的临床断言无依据和询问漂移问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗对话系统中已有进展，但精神科咨询面临更高要求：需要从患者不完整、不一致的多轮报告中持续提取精神病理线索，并进行严格的鉴别诊断推理。现有方法面临两个基本挑战：1) 缺乏标准驱动的临床支持，容易出现无依据的临床断言；2) 在多轮交互中难以避免询问漂移并优化提问策略。

Method: 提出MIND框架：1) 构建标准驱动的精神科推理库(PRB)，将对话上下文总结为临床检索状态，检索语义相似的参考咨询，提取可重用的标准驱动临床支持；2) 基于此，MIND通过基于量规的过程奖励强制执行明确的临床推理，对中间决策步骤提供细粒度监督；3) 结合价值感知的轨迹修正机制，共同改善跨轮次的信息获取和诊断决策。

Result: 大量实验表明，MIND在诊断准确性、同理心交互质量、可解释性和泛化能力方面持续优于强基线方法。

Conclusion: MIND通过统一询问-诊断强化学习框架，有效解决了精神科咨询中的临床支持不足和询问漂移问题，显著提升了精神科咨询系统的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型存在系统性偏见，实体在文本中的提及顺序会错误地决定图像的空间布局和角色分配，这种偏见会覆盖基于实际内容的线索，导致不正确的布局或角色互换。

Method: 提出了OTS-Bench基准测试，通过仅改变实体顺序的成对提示来隔离顺序效应，从同质化和正确性两个维度评估模型。通过实验验证OTS偏见的存在，并分析其数据驱动特性。

Result: 实验证明顺序到空间偏见(OTS)在现代图像生成模型中普遍存在，主要是数据驱动的，并在布局形成的早期阶段显现。针对性的微调和早期干预策略能显著减少OTS，同时保持生成质量。

Conclusion: 揭示了图像生成模型中的顺序到空间偏见问题，提出了量化方法和缓解策略，为改进模型的空间理解和布局生成能力提供了重要见解。
...
TL;DR: ErrorLLM：一个专门用于文本到SQL优化的框架，通过显式建模SQL错误类型，使用专用错误令牌检测复杂隐式错误，并进行错误引导的SQL结构优化


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL优化方法面临两个主要限制：1）自调试方法在现代LLM很少产生显式执行错误时效果有限；2）自校正方法由于缺乏基于问题和模式的显式错误建模，检测精度低且存在严重幻觉问题，经常破坏正确的SQL

Method: 提出ErrorLLM框架：1）将用户问题和数据库模式表示为结构特征；2）使用静态检测识别执行失败和表面不匹配；3）扩展ErrorLLM的语义空间，包含捕获分类隐式语义错误类型的专用错误令牌；4）通过精心设计的训练策略，用结构表示显式建模这些错误；5）基于检测到的错误，通过提示LLM进行错误引导的SQL结构优化

Result: 大量实验表明，ErrorLLM在基础初始生成上实现了最显著的改进。进一步分析显示，检测质量直接决定优化效果，ErrorLLM通过高检测F1分数同时保持优化效果，解决了这两个方面的问题

Conclusion: ErrorLLM通过显式建模文本到SQL错误，使用专用错误令牌检测复杂隐式错误，并进行错误引导的SQL结构优化，有效解决了现有文本到SQL优化方法的局限性，显著提升了SQL查询的准确性和可靠性
...
TL;DR: COREA系统通过级联小语言模型(SLM)和大语言模型(LLM)来平衡复杂推理任务的准确性和成本，使用强化学习校准SLM置信度，在保持准确性的同时显著降低成本。
...
Method: 提出COREA系统，先让SLM尝试回答问题并输出答案和置信度，置信度低于阈值的转交给LLM处理；使用强化学习训练算法通过置信度校准奖励来对齐SLM的置信度。

Result: 在多种数据集和模型上，该方法同时提升了SLM的推理能力和置信度校准；相比单独使用LLM，在数学和非数学数据集上分别降低成本21.5%和16.8%，准确率下降仅在2%以内。

Conclusion: COREA系统通过级联SLM和LLM，结合置信度校准机制，在保持高准确性的同时显著降低了推理成本，为平衡语言模型性能与成本提供了有效解决方案。
...
TL;DR: SoT是一种通过显式构建中间文本结构来提升大语言模型文本处理性能的提示技术，T2S-Bench是首个评估模型文本到结构能力的基准，包含1.8K样本覆盖6个科学领域和32种结构类型。
...
Method: 1. 提出SoT（Structure of Thought）提示技术，指导模型构建中间文本结构；2. 创建T2S-Bench基准，包含1.8K样本覆盖6个科学领域和32种结构类型；3. 在45个主流模型上进行评估。

Result: 1. 在多跳推理任务上平均准确率仅52.1%，最先进模型在端到端提取中节点准确率仅58.1%；2. SoT在Qwen2.5-7B-Instruct上平均提升5.7%；3. 在T2S-Bench上微调后提升增至8.6%。
...
Method: 提出逐步语义对齐（SSA）方法，将伪源域视为连接源域和目标域的语义桥梁而非直接替代。利用易获取的通用语义修正伪源域的语义特征，然后使用修正后的伪源语义对齐目标域。引入分层特征聚合（HFA）模块和置信感知互补学习（CACL）策略，在缺乏源域和目标域真实标签的情况下增强SSA过程的语义质量

Result: 在语义分割和图像分类等任务上进行了评估，在GTA2Cityscapes数据集上相比最先进方法获得了5.2%的性能提升

Conclusion: SSA方法通过将伪源域作为语义桥梁而非直接替代，有效解决了域适应中的分布偏移问题，在缺乏源域信息和目标域真实标签的情况下实现了显著性能改进
...
<details>
  <summary>Details</summary>
Motivation: 动机性访谈（MI）在物质使用障碍治疗中有效，但缺乏对大型语言模型在真实临床环境中MI能力的研究。本研究旨在评估专有和开源LLM在真实临床记录中的MI能力，并与人类治疗师比较。

Method: 从LMArena筛选3个专有和7个开源LLM，使用MITI 4.2框架在两个数据集（96个手工制作模型记录，34个真实临床记录）上评估性能。生成并行LLM-治疗师话语，保持客户响应不变，使用MITI组件和冗长度构建复合排名系统。进行可区分性实验，由两名独立精神科医生识别人类与LLM响应。

Result: 所有10个LLM在MITI测量中表现出良好能力（MITI全局评分>3.5）。在真实临床记录中，LLM在复杂反思百分比（39% vs 96%）和反思-问题比率（1.2 vs >2.8）上优于人类专家。可区分性实验中，精神科医生识别LLM响应的准确率仅为56%，d-prime值较低。

Conclusion: LLM可以在真实临床记录中实现良好的MI能力，即使是开源模型也具备在资源匮乏环境中扩展MI咨询会话的潜力，难以与人类治疗师区分。
...
TL;DR: 本文提出了两种基于原型的方法（PBR和PCM）来改进修辞角色标注任务，通过结合局部上下文和全局表示来解决现有分层模型的局限性，并在新构建的SCOTUS-Law数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 修辞角色标注是法律和医学等领域话语理解的关键任务。现有分层模型虽然能有效捕捉局部依赖关系，但在建模全局、语料库层面的特征方面存在局限，需要新的方法来整合局部上下文与全局表示。

Method: 提出了两种基于原型的方法：1）基于原型的正则化（PBR），通过学习软原型并通过基于距离的辅助损失来结构化潜在空间；2）原型条件调制（PCM），构建语料库级别的原型并在训练和推理过程中注入这些原型。同时构建了SCOTUS-Law数据集，这是首个包含三个粒度级别（类别、修辞功能、步骤）的美国最高法院意见修辞角色标注数据集。

Result: 在法律、医学和科学基准测试上的实验表明，所提方法相比强基线模型取得了持续改进，在低频角色上获得了4个Macro-F1的提升。进一步分析了在大型语言模型时代的影响，并通过专家评估补充了研究结果。

Conclusion: 提出的基于原型的方法有效整合了局部上下文与全局表示，显著提升了修辞角色标注的性能，特别是在低频角色上。新构建的SCOTUS-Law数据集为法律领域的修辞角色标注研究提供了有价值的资源，为大型语言模型时代的应用提供了重要启示。
...
TL;DR: 该研究创建了捷克历史文档的主题定位基准，评估了多种LLM和BERT模型在文档和词级别主题定位任务上的表现，发现模型性能差异显著，最强模型接近人类一致性水平。
...
Method: 创建基于捷克历史文档的人工标注基准数据集，包含人类定义的主题和手动标注的文本片段；在蒸馏开发数据集上微调BERT模型；评估多种大型语言模型和基于BERT的模型；采用相对人类一致性而非单一参考标注的评估方法。

Result: 不同LLM在主题定位任务上表现差异显著，从接近人类水平的主题检测到明显的片段定位失败；最强模型接近人类一致性水平；尽管规模较小，蒸馏的token嵌入模型仍保持竞争力。

Conclusion: 该研究提供了捷克历史文档主题定位的基准数据集和评估框架，展示了不同模型在该任务上的表现差异，最强模型接近人类水平，为后续研究提供了基础。
...
TL;DR: 研究评估大语言模型在法语医学开放性问题回答中的语义等价性判断能力，发现模型判断受答案生成模型影响显著，领域适应模型表现最佳，轻量级适应能提升小模型性能


<details>
  <summary>Details</summary>
Motivation: 医学开放性问题回答的自动评估面临挑战，需要专家标注。研究探索大语言模型能否作为法语医学OEQA的语义等价性评判者，以解决评估资源有限的问题

Method: 比较闭源、通用和生物医学领域适应的大语言模型作为评判者，评估它们对语义等价性的判断能力。进一步使用监督微调和GRPO对紧凑模型进行轻量级适应

Result: LLM判断受答案生成模型影响显著，不同生成器间一致性差异大。领域适应模型和大型通用模型与专家标注对齐度最高。轻量级适应能显著提升小模型性能并降低生成器敏感性

Conclusion: 医学OEQA评估需要考虑生成器影响，经过精心适应的小模型可以在低资源医学环境中支持可扩展的评估
...
TL;DR: 该研究提出了一种基于激活的监控方法，通过分析语言模型内部表征来检测奖励黑客行为，能够在生成过程中早期发现对齐问题。


<details>
  <summary>Details</summary>
Motivation: 微调后的大型语言模型可能出现奖励黑客行为，这种由涌现错位引起的问题很难仅从最终输出中检测。虽然先前研究在完整响应层面研究了奖励黑客行为，但尚不清楚是否能在生成过程中识别此类行为。

Method: 提出基于激活的监控方法：在残差流激活上训练稀疏自编码器，应用轻量级线性分类器生成奖励黑客活动的token级估计。该方法在多个模型家族和微调混合策略上进行测试。

Result: 内部激活模式能可靠区分奖励黑客与良性行为，泛化到未见过的混合策略适配器，在思维链推理中展现出模型依赖的时间结构。奖励黑客信号通常早期出现，在整个推理过程中持续存在，并可通过增加测试时计算（如思维链提示）在弱指定奖励目标下被放大。

Conclusion: 内部激活监控提供了比基于输出的评估更早的涌现错位信号，支持对微调语言模型进行更鲁棒的部署后安全监控。
...
TL;DR: FINEST是一个用于敏感话题的细粒度响应评估分类法，将有用性和无害性分解为内容、逻辑和适当性三个主要类别的错误，通过评分和错误导向的改进流程显著提升LLM对敏感问题的响应质量。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架缺乏系统方法来识别和解决LLM在敏感话题上响应中的具体弱点，导致模型往往生成过于谨慎和模糊的响应，牺牲了有用性来保证安全性，难以同时改进安全性和有用性。

Method: 提出FINEST评估分类法，将有用性和无害性分解为内容、逻辑和适当性三个主要类别的错误；开发基于评分和错误的改进流程，通过提供类别特定的分数和理由来指导模型响应改进。

Result: 在韩语敏感问题数据集上的实验表明，FINEST指导的改进流程显著提升了模型在所有三个类别上的响应质量，优于无指导的改进。评分导向的改进效果最显著，将适当性类别的错误句子比例降低了33.09%。

Conclusion: FINEST为LLM对敏感问题的响应提供了更可解释和全面的评估与改进基础，有助于同时提升安全性和有用性。
...
TL;DR: 首个对波兰大语言模型进行系统性的极端2位量化评估，比较了六种最先进的量化方法，在波兰语基准测试中取得了接近原始模型的效果，同时显著减小了模型大小。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估极端2位量化在波兰语大语言模型上的应用效果，探索在保持模型性能的同时大幅减小模型尺寸的方法，为资源受限环境下的波兰语NLP应用提供解决方案。

Method: 使用Bielik-11B-v2.3-Instruct作为基础模型，比较了六种后训练量化方法（QuIP#、SpinQuant+GPTQ、ButterflyQuant、QTIP、VPTQ、AQLM），所有方法都在波兰语语料库（CulturaX-PL）上校准，并共享Hessian矩阵。
...
Conclusion: 极端2位量化在波兰语大语言模型上是可行的，能够在显著减小模型尺寸的同时保持接近原始模型的性能，QTIP方法在效率方面表现最佳，同时发现了基于旋转的方法在自回归生成中的性能分离现象。
...
TL;DR: 该研究评估了11个主流大语言模型在涉及《世界人权宣言》权利权衡场景中的表现，发现模型存在系统性偏见：更易接受限制经济社会文化权利而非政治公民权利，在不同语言中存在显著差异，易受提示词引导，且不同回答格式间存在明显差异。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在全球信息获取中扮演越来越重要的角色，它们可能影响公共话语，因此需要确保这些模型在高风险AI中介交互中遵循普遍人权原则。研究旨在评估LLMs如何应对涉及《世界人权宣言》的权利权衡场景。

Method: 研究使用1,152个合成生成的场景，覆盖24个权利条款和8种语言，对11个主流大语言模型进行评估。通过分析模型在不同语言、不同权利类型下的响应，以及模型对提示词引导的敏感性，比较Likert量表和开放式回答的差异。

Result: 研究发现LLMs存在系统性偏见：1) 更易接受限制经济社会文化权利而非政治公民权利；2) 在中文和印地语中支持限制权利行动的比例显著高于英语或罗马尼亚语；3) 对提示词引导表现出显著敏感性；4) Likert量表和开放式回答之间存在明显差异，凸显了LLM偏好评估的关键挑战。

Conclusion: 大语言模型在人权权衡场景中存在系统性偏见和跨语言差异，对提示词敏感且评估方法影响结果。这些发现强调了在AI系统设计中纳入人权考量、开发更稳健评估方法的重要性，以确保AI中介交互中的人权保护。
...
TL;DR: 研究发现，在检索增强生成中，文档表示质量是基准测试改进的主要驱动因素，而非检索算法本身。通过固定检索机制（BM25）但改变转录和预处理方法，可以在多语言和视觉基准上实现大幅性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成系统通常使用端到端多模态检索器，声称在复杂布局的多语言文档上相比传统检索方法（如BM25）有显著改进。但需要验证这些改进究竟来自检索算法本身还是文档表示质量。

Method: 采用控制变量法：固定检索机制为BM25，系统性地改变文档转录和预处理方法。通过在不同转录和预处理配置下测试BM25的性能，分离文档表示质量对检索效果的影响。

Result: 实验表明，BM25在改进的文档表示下能够显著缩小与端到端多模态检索器在多语言和视觉基准上的性能差距。文档表示质量是基准测试改进的主要因素。

Conclusion: 需要建立分解评估基准，分别衡量转录和检索能力，以便正确归因进展并集中精力在真正重要的改进方向上。文档表示质量对检索性能的影响被低估。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型从研究原型转向实际系统，定制化成为核心瓶颈。纯文本提示不适合作为可扩展、稳定且仅推理的定制化控制接口。

Method: 提出模型提供商应在公开接口中暴露向量提示输入，支持这一立场的诊断证据包括：向量提示调优随监督增加持续改进而文本提示优化早期饱和，向量提示表现出密集、全局的注意力模式。

Result: 向量提示调优比文本提示优化表现更好，具有不同的控制机制，且在标准黑盒威胁模型下暴露向量提示不会显著增加模型泄漏风险。

Conclusion: 呼吁社区重新思考提示接口作为LLM定制化的核心组件，模型提供商应提供向量提示作为公开接口的一部分。
...
TL;DR: 研究探讨大语言模型对表达黑暗三性格特质的用户提示的响应模式，发现模型主要表现出纠正行为，但在某些情况下也会强化负面倾向，响应行为随严重程度和模型类型而变化。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常表现出高度迎合和强化的对话风格（AI-谄媚），虽然这种行为被鼓励，但当与反映负面社会倾向的用户提示互动时可能变得有问题，存在放大有害行为而非缓解的风险。

Method: 使用精心策划的数据集，研究大语言模型对表达不同程度黑暗三性格特质（马基雅维利主义、自恋和精神病态）的用户提示的响应，分析模型行为差异。
...
TL;DR: 论文提出V₁框架，通过成对自我验证提升复杂推理任务的测试时扩展性能，相比独立评分方法显著提高准确率


<details>
  <summary>Details</summary>
Motivation: 测试时扩展方法通过采样多个解决方案并聚合可以显著提升任务效果，但关键瓶颈在于验证：采样只有在能可靠识别正确候选方案时才有效。现有方法通常通过标量评分独立评估候选方案，但研究表明模型在成对自我验证方面能力更强。

Method: 提出V₁框架，统一生成和验证过程。包含两个组件：1) V₁-Infer：基于不确定性的算法，使用锦标赛式排名，动态分配自我验证计算资源给相对正确性最不确定的候选对；2) V₁-PairRL：强化学习框架，联合训练单一模型同时作为生成器和成对自我验证器，确保验证器适应生成器的演化分布。

Result: 在代码生成（LiveCodeBench、CodeContests、SWE-Bench）和数学推理（AIME、HMMT）基准测试中，V₁-Infer比点对点验证提升Pass@1高达10%，优于近期测试时扩展方法且效率更高。V₁-PairRL相比标准RL和点对点联合训练获得7-9%的测试时扩展增益，在代码生成场景中比标准RL提升基础Pass@1达8.7%。

Conclusion: 成对自我验证比独立评分更有效，V₁框架通过统一生成和验证过程，显著提升了复杂推理任务的测试时扩展性能，在代码生成和数学推理任务中均表现出色。
...
TL;DR: 研究发现语言模型隐藏状态中地理和时间信息的线性可恢复性可能源于文本本身的潜在结构，而非模型内部的世界表征。通过对静态词嵌入（GloVe和Word2Vec）的测试，发现这些简单嵌入也能恢复大量地理和时间信息。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为从大语言模型隐藏状态中线性恢复地理和时间变量表明模型具有类似世界的内部表征。本文旨在验证一个更简单的可能性：这些结构可能已经潜在于文本本身。

Method: 使用与先前研究相同的岭回归探针方法，应用于静态共现嵌入（GloVe和Word2Vec），分析地理和时间信号的可恢复性。通过语义邻居分析和目标子空间消融实验，探究这些信号对可解释词汇梯度的依赖。

Result: 静态嵌入中可恢复大量地理信号和较弱但可靠的时间信号：城市坐标的留出R²值为0.71-0.87，历史出生年份为0.48-0.52。这些信号主要依赖于可解释的词汇梯度，特别是国家名称和气候相关词汇。

Conclusion: 普通词共现保留了比通常假设更丰富的空间、时间和环境结构，简单的静态嵌入仅从文本就能保持世界形状结构。线性探针可恢复性本身不能证明表征超越了文本。
...
TL;DR: 本文介绍了一个在SemEval 2026 Task 12中获胜的三阶段系统，该系统结合了基于图的检索、LLM驱动的溯因推理以及后验一致性强制，在评估阶段以0.95的准确率排名第一。跨模型错误分析揭示了三个共享的归纳偏见。


<details>
  <summary>Details</summary>
Motivation: 解决多标签因果推理中的系统性问题，通过构建一个有效的溯因事件推理系统来提升准确率，并深入分析不同模型共享的失败模式。

Method: 采用三阶段系统：1）基于图的检索；2）LLM驱动的溯因推理，通过反射式提示演化优化提示设计；3）后验一致性强制。同时对14个模型（7个家族）进行跨模型错误分析。

Result: 系统在SemEval 2026 Task 12评估阶段以0.95的准确率排名第一。跨模型分析揭示了三个共享的归纳偏见：因果链不完整性、近因偏好和显著性偏见，这些偏见在跨家族中收敛（51%原因计数减少），表明是多标签因果推理中的系统性而非模型特定的失败模式。

Conclusion: 提出的三阶段系统在溯因事件推理任务中表现优异，跨模型分析揭示了多标签因果推理中存在的系统性偏见，为未来改进提供了重要见解。
...
TL;DR: 本文提出了一种面向深度研究智能体的检索新范式，通过利用智能体的推理轨迹来提升检索效果，并开发了相应的数据合成方法和嵌入模型。


<details>
  <summary>Details</summary>
Motivation: 深度研究智能体已成为现代检索系统的主要用户，它们会在搜索前生成明确的自然语言推理过程，这些丰富的意图和上下文信息被现有检索系统完全忽略。为了利用这些被忽视的信号，需要开发新的检索方法。

Method: 提出了两个核心组件：1）推理感知检索——一种将智能体的推理轨迹与查询联合嵌入的检索范式；2）DR-Synth——一种从标准问答数据集生成深度研究检索器训练数据的数据合成方法。

Result: 两个组件各自独立有效，组合后训练出的嵌入模型AgentIR-4B取得了显著提升。在BrowseComp-Plus基准测试中，AgentIR-4B与Tongyi-DeepResearch配合达到68%准确率，而传统嵌入模型（尺寸是其两倍）仅50%，BM25仅37%。

Conclusion: 通过利用深度研究智能体的推理轨迹信息，可以显著提升检索系统的性能，这为面向智能体的检索系统设计提供了新的方向。
