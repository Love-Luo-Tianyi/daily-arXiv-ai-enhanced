...
TL;DR: 提出了一种名为“Gaussian See, Gaussian Do”的新方法，利用动态3D高斯泼溅技术和视图感知运动嵌入，实现了无需骨骼绑定的跨类别语义3D运动迁移。


<details>
  <summary>Details</summary>
Motivation: 为了实现无需骨骼绑定（rig-free）且跨类别的物体间语义3D运动迁移，克服现有方法在处理多视图视频和语义对应关系时的局限性。

Method: 该方法基于隐式运动迁移技术，首先通过条件反转从源视频中提取运动嵌入，将其应用于静态目标形状的渲染帧生成视频，进而利用这些视频监督动态3D高斯泼溅（3D Gaussian Splatting）的重建。核心创新包括引入基于锚点的视图感知运动嵌入机制以确保跨视图一致性并加速收敛，以及一个能整合噪声监督视频的鲁棒4D重建流程。

Result: 建立了首个语义3D运动迁移的基准测试，并证明了该方法在运动保真度和结构一致性方面均优于现有的调整后基线方法。

Conclusion: 该方法成功实现了高质量的语义3D运动迁移，通过提出的基准测试展示了其在保持结构和运动真实性方面的显著优势，为跨类别物体动画生成提供了新的解决方案。
...
TL;DR: 本文提出了一种名为BR-DF的新型几何表示法，结合多分支潜在扩散模型，实现了100%成功率的CAD B-Rep模型生成。


<details>
  <summary>Details</summary>
Motivation: 旨在解决CAD模型生成中难以保证生成有效的、水密的B-Rep结构的问题，寻求一种能够确保转换过程稳健且不失败的新型几何表示方法。

Method: 1. 提出BR-DF表示法：将曲面网格编码为有符号距离场（SDF），将点、边、面及拓扑信息编码为逐面无符号距离场（UDF）。
2. 使用扩展的Marching Cubes算法将BR-DF直接转换为水密CAD B-Rep模型。
3. 采用具有3D U-Net骨干的多分支潜在扩散模型来联合生成BR-DF模型的SDF和UDF。

Result: 该方法实现了与当前最先进技术（SOTA）相当的CAD生成性能，并在生成（面片化）B-Rep模型时达到了前所未有的100%成功率。

Conclusion: BR-DF结合扩散模型不仅保证了CAD生成的几何有效性（100%成功率），还保持了高质量的生成效果，为CAD生成任务提供了鲁棒的新方案。
...
TL;DR: 本文提出了InstructMix2Mix框架，通过将2D扩散模型的编辑能力蒸馏到预训练的多视图扩散模型中，解决了稀疏输入下的多视图图像编辑和一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多视图图像编辑方法（基于单场景神经场或时间注意力机制）在稀疏输入视图设置下表现不佳，经常产生伪影且难以保持跨视图的一致性。

Method: 提出了InstructMix2Mix (I-Mix2Mix) 框架。核心在于利用分数蒸馏采样 (SDS)，将传统神经场整合器替换为多视图扩散模型作为“学生”模型。具体改进技术包括：跨时间步的学生增量更新、防止退化的专用教师噪声调度器，以及无需额外成本即可增强跨视图连贯性的注意力机制修改。

Result: 实验结果表明，I-Mix2Mix在保持高质量单帧编辑效果的同时，显著提高了多视图之间的一致性。

Conclusion: 该方法通过利用数据驱动的3D先验，成功解决了稀疏视图下的编辑一致性难题，为多视图图像编辑提供了一种高效的新方案。
...
TL;DR: SkinR1是一种新型皮肤病诊断模型，通过结合教科书式的深度推理生成、监督微调以及层级感知的强化学习，解决了现有模型缺乏推理依据和泛化难的问题，显著提升了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）在皮肤病诊断中展现出潜力，但受限于三大因素：数据异质性（缺乏一致标签）、缺乏有依据的诊断推理（缺少可靠监督）、以及有限的可扩展性和泛化能力（难以从密集注释的小数据迁移到稀疏注释的大数据）。

Method: 提出了SkinR1框架，包含三个关键步骤：
1. **推理生成器**：设计基于教科书的推理生成器，合成包含层级感知和鉴别诊断（DDx）信息的高保真推理轨迹。
2. **监督微调（SFT）**：利用上述轨迹对模型进行微调，赋予其有依据的推理能力。
3. **强化学习（RL）**：开发一种结合疾病层级结构的新型RL范式，将推理模式有效迁移至大规模稀疏注释数据中。

Result: 在多个皮肤病数据集上的广泛实验表明，SkinR1实现了优于现有方法的诊断准确率；消融研究进一步证实了通过监督微调（SFT）建立推理基础的重要性。

Conclusion: SkinR1通过将基于教科书的深度推理与强化学习的泛化能力相结合，有效解决了皮肤病诊断中的数据异质性和推理依据缺失挑战，为临床辅助诊断提供了更可靠、可扩展且具备高准确率的解决方案。
...
TL;DR: 本文针对遥感CLIP模型细粒度感知不足的问题，构建了MGRS-200k数据集并提出了FarSLIP框架，通过patch-to-patch蒸馏和CLS token区域对齐策略，在保持语义连贯性的同时显著提升了多项遥感任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感（RS）CLIP变体缺乏细粒度的空间感知能力。这主要归因于：
1. 现有数据集将对象级标签生成为全局描述，浪费了对象级监督信息；
2. 直接应用通用领域的区域-文本对齐方法会导致CLIP的语义连贯性严重下降，进而导致性能降低。

Method: 1. 构建了首个多粒度遥感图像-文本数据集**MGRS-200k**，提供丰富的对象级文本监督。
2. 提出了**FarSLIP**框架：采用**patch-to-patch蒸馏**策略来对齐局部和全局视觉线索（而非通常的patch-to-CLS），并使用简单的**基于CLS token的区域-类别对齐**来有效利用区域文本监督，避免了显式patch级对齐带来的副作用。

Result: FarSLIP改善了遥感领域的细粒度视觉-语言对齐，并在以下任务中取得了新的**最先进（SOTA）**结果：
1. 遥感开放词汇语义分割；
2. 图像级任务（如零样本分类和图像-文本检索）。

Conclusion: 通过引入新的多粒度数据集和优化的蒸馏与对齐策略，FarSLIP有效解决了遥感视觉-语言预训练中细粒度细节捕捉和语义连贯性保持之间的矛盾，为该领域设定了新的基准。
...
TL;DR: nnMIL是一个用于计算病理学的通用多示例学习框架，通过随机采样和滑动窗口推理机制，显著提升了全切片图像分析在多种临床任务中的准确性、泛化能力和可靠性。


<details>
  <summary>Details</summary>
Motivation: 目前的计算病理学方法在将基础模型提取的图块级特征聚合为全切片级（WSI）预测时，存在设计上的局限性，这些局限性阻碍了模型的泛化能力和可靠性。

Method: 开发了nnMIL框架，这是一种简单且通用的多示例学习（MIL）方法。核心机制包括：在图块（patch）和特征（feature）层面引入随机采样，支持大批量优化和任务感知采样策略；使用轻量级聚合器执行滑动窗口推理，以生成全切片级的集成预测，并支持原则性的不确定性估计。

Result: 在涵盖35个临床任务、4种病理基础模型和40,000张全切片图像的测试中，nnMIL在疾病诊断、组织学分型、分子生物标志物检测和泛癌预后预测方面均优于现有的MIL方法。此外，它还展示了强大的跨模型泛化能力、可靠的不确定性量化以及在多个外部队列中稳健的生存分层能力。

Conclusion: nnMIL为将病理基础模型转化为具有临床意义的预测提供了一种实用且可泛化的解决方案，有助于推动可靠AI系统在现实世界医疗场景中的开发与部署。
...
TL;DR: 提出了一种名为X-WIN的胸部X光世界模型，通过从CT数据中学习3D结构知识并将其蒸馏到2D投影预测中，解决了CXR缺乏3D信息的问题，显著提升了诊断性能。


<details>
  <summary>Details</summary>
Motivation: 胸部X光（CXR）作为2D投影图像，受限于结构重叠，无法捕捉3D解剖信息，这使得表征学习和疾病诊断变得困难。为了解决这一挑战，需要将CT影像中的3D体积知识引入到CXR分析中。

Method: 1. 提出X-WIN模型，核心思想是具备3D解剖结构知识的世界模型可以在潜在空间预测不同变换下的2D投影；
2. 引入亲和力引导的对比对齐损失（affinity-guided contrastive alignment loss），利用同一体积不同投影间的相互相似性捕捉关联信息；
3. 通过掩码图像建模（MIM）将真实CXR纳入训练，并利用域分类器（domain classifier）使真实和模拟CXR的表征在统计上相似，以提高模型适应性。

Result: 1. 在线性探测和少样本微调等多种下游任务中，X-WIN的性能优于现有的基础模型；
2. 模型展示了渲染2D投影以重建3D CT体积的能力。

Conclusion: 通过内化3D解剖结构知识，X-WIN有效地解决了CXR的维度限制，增强了模型在医疗诊断中的适应性和准确性，并具备从2D投影重建3D体积的潜力。
...
TL;DR: 提出了一种名为CPSL的紧凑型2.5D视频表示方法，通过分层分解和深度信息实现了低成本、高质量的沉浸式视频体验。


<details>
  <summary>Details</summary>
Motivation: 现有的体积视频技术（如显式点云到隐式神经场）在采集、计算和渲染方面成本高昂，难以实现规模化应用和实时通信。

Method: 提出了内容增强场景层（CPSL）。利用单帧深度和内容显著性，将视频帧分解为几何一致的层，辅以软Alpha带和边缘深度缓存以处理遮挡和边界。通过深度加权变形和Alpha合成生成新视图，并利用运动引导传播和标准视频编解码器保持时间连贯性。

Result: 在多个基准测试中，CPSL相比基于层的方法和神经场基线，实现了更优越的感知质量和边界保真度，同时将存储和渲染成本降低了数倍。

Conclusion: 该方法有效地弥补了传统2D视频与沉浸式体验之间的差距，为实现可扩展的2.5D沉浸式媒体提供了一条切实可行的技术路径。
...
TL;DR: 本文针对长期且低对比度的周期性人类活动工作流，提出了首个多模态基准测试集及一种轻量级、无需训练的基线方法，并在多项任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 尽管具有隐式工作流的周期性人类活动在制造业、体育和日常生活中很常见，但现有研究主要集中在结构简单、模式对比度高的短期活动上，而具有低对比度模式的长期周期性工作流在很大程度上未被探索。

Method: 本文引入了首个包含580个多模态人类活动序列的基准测试，专注于长期周期性工作流，支持无监督检测、任务完成跟踪和异常检测三个评估任务；同时提出了一种轻量级、无需训练的基线方法来建模多样化的周期性工作流模式。

Result: 实验显示：(1) 该基准对现有无监督检测方法和基于LLM的零样本方法构成显著挑战；(2) 提出的基线方法在所有任务中大幅优于竞争方法；(3) 该基线在实际部署中具备与传统监督方法相当的优势，且消除了标注和重训练的需求。

Conclusion: 本文填补了长期周期性工作流研究的空白，通过提供具有挑战性的基准测试和一种高效、无需训练的基线方法，证明了在无需人工标注和重新训练的情况下，也能在实际应用中达到与传统监督方法相当的性能。
...
TL;DR: 提出了一种基于定制LED时钟的低成本通用同步方法，能够实现异构相机系统（RGB和IR）的毫秒级对齐，显著提升了下游视觉任务的性能。


<details>
  <summary>Details</summary>
Motivation: 多视点视频流的精确时空对齐对于3D重建和姿态估计等应用至关重要，但在混合使用不同类型设备（如专业与消费级、可见光与红外、有无音频）的异构设置中，往往缺乏通用的硬件同步功能，这使得在非受控现实环境下的多相机同步面临巨大挑战。

Method: 提出了一种低成本、通用的同步方法，使用定制的“LED时钟”通过红色和红外LED对时间进行编码。该方法通过视觉解码录制帧中的曝光窗口（开始和结束时间），实现对可见光（RGB）和红外（IR）相机系统的毫秒级时间对齐。

Result: 该方法实现了1.34毫秒的均方根误差（RMSE），优于基于光、音频和时间码的传统同步方法。实验表明，该方法显著改善了多视点姿态估计和3D重建等下游任务的性能，并在包含超过25个异构相机的大规模手术录像中成功进行了验证。

Conclusion: 该解决方案简化了同步流程，有效解决了异构相机系统的同步难题，为工业和临床等非受控环境下的先进视觉传感应用提供了可靠且低成本的技术支持。
...
### [11] [Kandinsky 5.0: A Family of Foundation Models for Image and Video Generation](https://arxiv.org/abs/2511.14993)
...
TL;DR: 本文介绍了Kandinsky 5.0，一套开源的高分辨率图像和视频生成基础模型，通过多阶段数据处理、SFT及RL后训练优化，实现了最先进的性能和生成速度。


<details>
  <summary>Details</summary>
Motivation: 旨在开发并开源一套最先进的基础模型系列，以提升高分辨率图像和10秒视频合成的质量、速度及可访问性，促进生成式AI社区的发展。

Method: 提出了包含Image Lite (6B)、Video Lite (2B)和Video Pro (19B)三个核心模型的框架。采用了全面的数据治理生命周期，实施了包含大规模预训练、自监督微调（SFT）和基于强化学习（RL）的后训练的多阶段训练管道，并引入了架构与推理优化。

Result: Kandinsky 5.0在多种任务中实现了最先进的性能和高生成速度，Video Pro模型表现出卓越的视频生成质量，并通过人类评估验证了其优越性。

Conclusion: Kandinsky 5.0作为大规模公开可用的生成框架，通过发布开源代码和模型权重，将有力推动高质量生成模型的研究与应用发展。
...
### [12] [FinCriticalED: A Visual Benchmark for Financial Fact-Level OCR Evaluation](https://arxiv.org/abs/2511.14998)
...
TL;DR: 本文推出了FinCriticalED，这是一个针对金融文档的视觉基准测试，旨在通过专家标注和LLM评估管道，解决传统OCR指标无法检测关键事实错误（如数值符号和日期）的问题。


<details>
  <summary>Details</summary>
Motivation: 金融文档通常包含视觉密集和大量表格的布局，其中的数值和时间信息与结构紧密耦合。在高风险场景下，微小的OCR错误（如符号反转或日期偏移）可能导致截然不同的解读，而传统的OCR指标（如ROUGE和编辑距离）仅能捕捉表面的文本相似度，无法检测关键的事实错误。

Method: 1. 构建数据集：包含500对图像-HTML对，涵盖超过700个由专家标注的数值和时间事实；
2. 专家标注：所有标注均由金融专家创建并验证，严格控制符号、量级和时间表达的质量；
3. 评估管道：开发了一个“大模型即裁判”（LLM-as-Judge）的评估流程，用于对视觉复杂的金融文档进行结构化事实提取和上下文验证。

Result: 对OCR系统、开源视觉语言模型和专有模型进行的基准测试显示，尽管最强的专有模型实现了最高的事实准确性，但在视觉复杂的数值和时间上下文中仍然存在大量错误。

Conclusion: FinCriticalED通过定量评估和专家案例研究，为提高金融及其他对精度要求极高的领域的视觉事实准确性奠定了坚实的基础。
...
### [13] [CKDA: Cross-modality Knowledge Disentanglement and Alignment for Visible-Infrared Lifelong Person Re-identification](https://arxiv.org/abs/2511.15016)
*Zhenyu Cui,Jiahuan Zhou,Yuxin Peng*

Main category: cs.CV

TL;DR: 本文提出了一种跨模态知识解耦与对齐（CKDA）方法，通过显式分离模态通用与特定知识并进行对齐，解决了可见光-红外终身行人重识别中的协同遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有的可见光-红外终身行人重识别（VI-LReID）方法虽然利用跨模态知识蒸馏缓解灾难性遗忘，但忽略了“模态特定知识获取”与“模态通用知识抗遗忘”之间的相互干扰。这种知识冲突导致了“协同遗忘”现象，从而影响模型性能。

Method: 本文提出了一种跨模态知识解耦与对齐方法（CKDA），主要包含以下组件：
1. **模态通用/特定提示模块（MCP & MSP）**：用于显式解耦和纯化模态共存的通用信息与模态独有的特定信息，避免两者相互干扰。
2. **跨模态知识对齐模块（CKA）**：基于双模态原型，在独立的模态间和模态内特征空间中，以平衡的方式将解耦后的新知识与旧知识进行对齐。

Result: 在四个基准数据集上进行的广泛实验验证了该方法的有效性，结果表明CKDA的性能优于当前最先进（SOTA）的方法。

Conclusion: 本文提出的CKDA方法通过显式分离和平衡保存模态特定与通用知识，有效解决了VI-LReID中的协同遗忘问题，实现了跨昼夜连续行人匹配性能的提升。
...
### [14] [Complex-Valued 2D Gaussian Representation for Computer-Generated Holography](https://arxiv.org/abs/2511.15022)
*Yicheng Zhan,Xiangjun Gao,Long Quan,Kaan Akşit*

Main category: cs.CV

TL;DR: 本文提出了一种基于二维高斯基元的高效全息图表示法，通过可微分光栅化实现了快速、低显存占用且高保真的全息重建与优化。


<details>
  <summary>Details</summary>
Motivation: 现有的逐像素信息存储方式导致全息图参数搜索空间过大，计算效率低下。为了解决这一问题，需要一种新的表示方法来减少参数空间，降低显存占用，并提高优化速度和重建质量。

Method: 1. 提出了一种基于结构化复数值二维高斯基元（2D Gaussian primitives）的全息图表示法，以替代传统的逐像素存储。
2. 开发了针对该表示法的可微分光栅化器，并集成了GPU优化的自由空间光传播核心，实现了端到端训练。
3. 引入了一种转换程序，将该表示法适配为实用的平滑及随机纯相位全息图格式。

Result: 1. 参数搜索空间减少了高达10:1。
2. 与现有方法相比，显存（VRAM）使用量降低了高达2.5倍，优化速度提升了50%。
3. 实现了更高保真度的全息重建，且转换程序有效抑制了此前方法中出现的噪声伪影。

Conclusion: 通过显著减少全息图参数搜索空间，该表示法实现了更具可扩展性的全息图估计，为下一代计算机生成全息系统提供了强有力的支持。
...
### [15] [Computer Vision Modeling of the Development of Geometric and Numerical Concepts in Humans](https://arxiv.org/abs/2511.15029)
*Zekun Wang,Sashank Varma*

Main category: cs.CV

TL;DR: 本研究通过分析ResNet-50模型，发现计算机视觉模型在学习几何、拓扑和数值概念的过程中，展现出了与人类儿童相似的认知发展轨迹。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明受过训练的计算机视觉（CV）模型能像成人一样学习几何和数值概念的潜在表征，但尚不清楚这些模型在训练过程中的学习轨迹是否也符合人类儿童的认知发展规律。

Method: 本研究以ResNet-50模型为案例，深入分析其在训练过程中性能的提升情况，并将其与儿童在几何、拓扑及数值能力上的发展轨迹进行对比，以验证是否存在“发展对齐”现象。

Result: 在几何与拓扑方面，模型在欧几里得几何、几何图形、度量属性和拓扑学等类别上表现出发展对齐，但在手性图形、几何变换和对称图形上未表现出对齐；在数值方面，随着训练经验的增加，模型涌现出了类似人类的“心理数轴”表征。

Conclusion: 计算机视觉模型有望成为理解人类数学认知发展的有效工具。这些发现为未来探索更多模型架构和构建更大规模的基准测试指明了方向。
...
### [16] [UniHOI: Unified Human-Object Interaction Understanding via Unified Token Space](https://arxiv.org/abs/2511.15046)
*Panqi Yang,Haodong Jing,Nanning Zheng,Yongqiang Ma*

Main category: cs.CV

TL;DR: UniHOI模型通过统一Token空间联合了HOI检测与生成任务，利用半监督学习实现了知识共享，在长尾检测和开放词汇生成上均大幅超越现有SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 在人-物交互（HOI）领域，检测和生成通常被作为两个独立的任务分别处理，这种割裂阻碍了模型对交互行为的全面理解与深入发展。

Method: 作者提出了UniHOI模型，通过统一的Token空间联合建模HOI检测与生成任务。关键技术包括引入对称的交互感知注意力模块，以及采用统一的半监督学习范式，从而在有限标注下实现图像与交互语义之间的有效双向映射。

Result: UniHOI在HOI检测和生成任务上均取得了最先进（SOTA）的性能。具体而言，在长尾HOI检测上准确率提升了4.9%，在开放词汇生成任务中交互指标提升了42.0%。

Conclusion: UniHOI通过联合建模检测与生成任务，有效地促进了跨任务的知识共享，显著增强了模型的泛化能力，证明了在统一框架下处理对偶任务的有效性。
...
### [17] [Hyperspectral Super-Resolution with Inter-Image Variability via Degradation-based Low-Rank and Residual Fusion Method](https://arxiv.org/abs/2511.15052)
*Yue Wen,Kunjing Yang,Minru Bai*

Main category: cs.CV

TL;DR: 提出了一种名为DLRRF的新型高光谱与多光谱图像融合模型，通过对光谱变异进行退化建模及引入残差分量来处理图像间的变异问题，并在PnP框架下实现了优越的融合效果。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像（HSI）与多光谱图像（MSI）的融合常因采集条件不同而面临“图像间变异”（包括光谱变异和空间局部变化）。现有方法通常直接对图像本身应用变换来处理这一问题，但这往往会加剧融合模型的病态性（ill-posedness），从而严重影响融合性能。

Method: 提出了基于退化的低秩和残差融合（DLRRF）模型。主要创新点包括：1. 将光谱变异建模为光谱退化算子的变化；2. 将目标HSI分解为低秩分量和残差分量，利用残差捕获因空间局部变化丢失的细节；3. 利用光谱相关性进行降维，并引入隐式正则化项利用空间先验信息。在求解上，采用即插即用（PnP）框架下的近端交替优化（PAO）算法，利用外部去噪器处理正则化子问题。

Result: 文章提供了算法的全面收敛性分析。广泛的数值实验结果表明，DLRRF模型在处理存在图像间变异的HSI和MSI融合任务时，相比现有方法取得了更优越的性能。

Conclusion: 该研究通过改进退化模型和引入残差分解策略，有效解决了多源图像融合中常见的图像间变异挑战，为提高高光谱图像的空间分辨率提供了一种鲁棒且高效的解决方案。
...
### [18] [ProPL: Universal Semi-Supervised Ultrasound Image Segmentation via Prompt-Guided Pseudo-Labeling](https://arxiv.org/abs/2511.15057)
*Yaxiong Chen,Qicong Wang,Chunlei Li,Jingliang Hu,Yilei Shi,Shengwu Xiong,Xiao Xiang Zhu,Lichao Mou*

Main category: cs.CV

TL;DR: 提出首个通用半监督超声图像分割框架ProPL及配套数据集，利用提示机制和不确定性校准技术，在多器官多任务场景下实现了优于SOTA的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的超声图像分割方法（无论是监督还是半监督）通常局限于特定的解剖结构或单一任务，缺乏通用性，这限制了它们在临床环境中的实际应用价值。

Method: 提出了ProPL框架，采用共享视觉编码器和提示引导的双解码器架构。利用“解码时提示”机制灵活适应不同任务，并通过“不确定性驱动的伪标签校准（UPLC）”模块进行可靠的自训练。此外，还引入了一个涵盖5种器官和8个分割任务的综合超声数据集。

Result: 大量实验表明，ProPL在各项评估指标上均优于当前最先进（SOTA）的方法，确立了通用超声图像分割的新基准。

Conclusion: 该论文开创了通用半监督超声图像分割任务，建立了新的基准，并证明了所提框架在多任务处理和利用未标记数据方面的有效性。
...
### [19] [Evaluating Multimodal Large Language Models on Vertically Written Japanese Text](https://arxiv.org/abs/2511.15059)
*Keito Sasagawa,Shuhei Kurita,Daisuke Kawahara*

Main category: cs.CV

TL;DR: 本文研究了MLLMs在竖排日语文本上的表现，发现现有模型能力不足，并通过合成数据集微调显著提升了模型对竖排日语的识别能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在视觉文档理解方面发展迅速，但针对日语竖排文本的研究仍然有限，而竖排支持对于处理日语文档至关重要。

Method: 研究者生成了一个包含横排和竖排日语文本的合成OCR数据集，并构建了一个包含竖排文本的真实世界文档评估数据集。利用这些数据对现有的MLLMs进行评估和微调。

Result: 现有MLLMs在竖排日语文本上的表现不如横排文本。通过在合成数据集上进行训练，可以显著提高模型处理竖排文本的能力。

Conclusion: 本研究揭示了现有MLLMs在竖排日语阅读上的短板，并证明了使用合成数据进行微调是提升其性能的有效方法，同时开源了相关数据集和代码。
...
### [20] [Reasoning via Video: The First Evaluation of Video Models' Reasoning Abilities through Maze-Solving Tasks](https://arxiv.org/abs/2511.15065)
...
### [21] [BokehFlow: Depth-Free Controllable Bokeh Rendering via Flow Matching](https://arxiv.org/abs/2511.15066)
...
### [22] [MambaTrack3D: A State Space Model Framework for LiDAR-Based Object Tracking under High Temporal Variation](https://arxiv.org/abs/2511.15077)
...
### [23] [TiCAL:Typicality-Based Consistency-Aware Learning for Multimodal Emotion Recognition](https://arxiv.org/abs/2511.15085)
...
### [24] [Jointly Conditioned Diffusion Model for Multi-View Pose-Guided Person Image Synthesis](https://arxiv.org/abs/2511.15092)
...
### [25] [A Comprehensive Study on Visual Token Redundancy for Discrete Diffusion-based Multimodal Large Language Models](https://arxiv.org/abs/2511.15098)
...
### [26] [Gaussian Blending: Rethinking Alpha Blending in 3D Gaussian Splatting](https://arxiv.org/abs/2511.15102)
...
### [27] [An Event-triggered System for Social Persuasion and Danger Alert in Elder Home Monitoring](https://arxiv.org/abs/2511.15117)
...
### [32] [DCL-SE: Dynamic Curriculum Learning for Spatiotemporal Encoding of Brain Imaging](https://arxiv.org/abs/2511.15151)
...
### [45] [Edge-Centric Relational Reasoning for 3D Scene Graph Prediction](https://arxiv.org/abs/2511.15288)
...
### [62] [A Dataset and Baseline for Deep Learning-Based Visual Quality Inspection in Remanufacturing](https://arxiv.org/abs/2511.15440)
...
### [66] [RS-CA-HSICT: A Residual and Spatial Channel Augmented CNN Transformer Framework for Monkeypox Detection](https://arxiv.org/abs/2511.15476)
...
### [67] [FunnyNodules: A Customizable Medical Dataset Tailored for Evaluating Explainable AI](https://arxiv.org/abs/2511.15481)
...
### [68] [Evaluating Low-Light Image Enhancement Across Multiple Intensity Levels](https://arxiv.org/abs/2511.15496)
...
### [70] [Multi-Text Guided Few-Shot Semantic Segmentation](https://arxiv.org/abs/2511.15515)
...
### [71] [A Hybrid CNN-ViT-GNN Framework with GAN-Based Augmentation for Intelligent Weed Detection in Precision Agriculture](https://arxiv.org/abs/2511.15535)
...
### [74] [From Low-Rank Features to Encoding Mismatch: Rethinking Feature Distillation in Vision Transformers](https://arxiv.org/abs/2511.15572)
...
### [75] [AVATAAR: Agentic Video Answering via Temporal Adaptive Alignment and Reasoning](https://arxiv.org/abs/2511.15578)
...
### [79] [LiveCLKTBench: Towards Reliable Evaluation of Cross-Lingual Knowledge Transfer in Multilingual LLMs](https://arxiv.org/abs/2511.14774)
*Pei-Fu Guo,Yun-Da Tsai,Chun-Chia Hsu,Kai-Xin Chen,Ya-An Tsai,Kai-Wei Chang,Nanyun Peng,Mi-Yen Yeh,Shou-De Lin*

Main category: cs.CL

TL;DR: 本文介绍了LiveCLKTBench，一种通过使用时效性强且经过验证的知识实体来生成问题，从而精准评估大型语言模型跨语言知识迁移能力并排除预训练记忆干扰的自动化基准测试框架。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLM）的跨语言知识迁移能力非常困难，因为很难区分目标语言中的正确回答是源于真正的知识迁移，还是因为模型在预训练期间已经接触过相关内容（即数据污染问题）。

Method: 文章提出了LiveCLKTBench，这是一个自动生成流水线，专门用于隔离和测量跨语言知识迁移。该方法识别现实世界中具有时效性的独立知识实体，基于时间进行过滤，并验证模型是否已知这些知识。随后，利用这些有效实体的文档生成事实性问题，并翻译成多种语言，以评估模型在不同语言间的知识迁移能力。

Result: 通过对多个LLM在五种语言上的评估，发现跨语言迁移受到语言距离的强烈影响，并且在不同语言方向上通常是不对称的。虽然更大的模型能提高迁移效果，但这种增益随规模扩大而递减，且在不同领域间存在差异。

Conclusion: 该研究为多语言迁移机制提供了新的见解，并证明了LiveCLKTBench可以作为一个可靠的基准，用于未来关于消除预训练数据污染干扰的跨语言能力研究。
...
### [80] [COMPASS: Context-Modulated PID Attention Steering System for Hallucination Mitigation](https://arxiv.org/abs/2511.14776)
*Snigdha Pandya,Rohan Nagale,Kenji Sahay,Anna Lin,Shikhar Shiromani,Kevin Zhu,Dev Sunishchal*

Main category: cs.CL

TL;DR: 本文提出了COMPASS框架，通过引入上下文依赖评分（CRS）和嵌入式PID控制器，在解码过程中动态调节LLM的注意力，从而在无需重新训练的情况下有效减少了事实幻觉并提升了可解释性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）即使在有相关证据的情况下，也常因在上下文知识（外部证据）和参数化知识（内部记忆）之间的注意力分配不当，而生成流畅但事实错误的陈述。为了实现可信部署和模型机制的科学解释性，迫切需要理解并引导这种内部行为。

Method: 本文引入了COMPASS（上下文调节PID注意力引导系统），这是一个轻量级且可解释的控制框架。它包含两个核心组件：
1. **上下文依赖评分（CRS）**：一种透明的度量标准，作为在线探针，量化注意力头如何基于证据生成内容。
2. **PID控制器**：利用CRS信号，在解码过程中动态调节注意力头，以在无需重新训练或多次解码的情况下维持事实一致性。

Result: 在多个基准测试（HotpotQA, XSum, HaluEval, RAGTruth）中，COMPASS持续降低了上下文幻觉率（绝对值降低了2.8%至5.8%），同时揭示了不同的注意力头在证据对齐中的具体贡献。

Conclusion: 结果表明，反馈驱动的可解释性方法不仅能提升模型性能，还是科学地理解大型语言模型（LLM）行为的有效途径。
...
### [81] [The Impact of Prosodic Segmentation on Speech Synthesis of Spontaneous Speech](https://arxiv.org/abs/2511.14779)
...
TL;DR: 本文探讨了显式的人工和自动韵律切分如何提升基于FastSpeech 2的巴西葡萄牙语自发语音合成的自然度和可懂度。


<details>
  <summary>Details</summary>
Motivation: 自发语音合成面临捕捉自然对话流（如轮替、停顿和不流利）的挑战。虽然现有系统通过隐式建模韵律特征取得了进展，但显式韵律切分数据集的构建及其对自发语音合成的具体影响仍未被充分探索。

Method: 本文使用非自回归模型FastSpeech 2，在巴西葡萄牙语语境下，对比评估了人工韵律切分标注和自动韵律切分标注对合成语音质量的影响。

Result: 实验结果显示，结合韵律切分训练能生成可懂度和声学自然度稍高的语音。自动切分倾向于产生规则片段，而人工切分引入了更多变异性，有助于实现更自然的韵律。分析表明，韵律模型在通过前核轮廓模拟自然语音方面表现更佳。

Conclusion: 显式韵律切分有助于提升语音合成质量，且人工切分更能捕捉自然韵律。为了支持可重复性和未来研究，作者根据CC BY-NC-ND 4.0许可公开了所有数据集、源代码和训练模型。
...
### [82] [Human or LLM as Standardized Patients? A Comparative Study for Medical Education](https://arxiv.org/abs/2511.14783)
*Bingquan Zhang,Xiaoxiao Liu,Yuchi Wang,Lei Zhou,Qianqian Xie,Benyou Wang*

Main category: cs.CL

TL;DR: 本文提出了EasyMED多智能体框架和SPBench基准，旨在解决传统标准化病人（SP）成本高、扩展难的问题，实验证明其能达到人类SP的教学效果且更具成本效益。


<details>
  <summary>Details</summary>
Motivation: 标准化病人（SP）在临床技能培训中至关重要，但存在成本高、灵活性差和难以扩展的问题。现有的基于大语言模型（LLM）的模拟器虽然有望降低成本，但表现出行为不一致，且缺乏与人类SP的严谨对比。

Method: 1. 提出了EasyMED多智能体框架，包含负责真实对话的患者智能体、确保事实一致性的辅助智能体，以及提供可操作反馈的评估智能体。
2. 引入了SPBench基准，包含涵盖14个专科和8项专家定义标准的真实SP-医生互动数据，用于系统性评估。

Result: 实验表明，EasyMED能达到与人类SP相当的学习成果，对于基础较弱的学生能带来更大的技能提升，同时提供了更好的灵活性、心理安全性和成本效率。

Conclusion: EasyMED证明了利用多智能体LLM进行临床技能培训的可行性和有效性，提供了一种可扩展、低成本且高质量的教学替代方案，尤其在提升基础薄弱学生能力和保障心理安全方面具有优势。
...
### [83] [Hierarchical Token Prepending: Enhancing Information Flow in Decoder-based LLM Embeddings](https://arxiv.org/abs/2511.14868)
*Xueying Ding,Xingyue Huang,Mingxuan Ju,Liam Collins,Yozen Liu,Leman Akoglu,Neil Shah,Tong Zhao*

Main category: cs.CL

TL;DR: 提出HTP方法，通过分块前置总结Token和平均池化，解决了因果LLM在生成嵌入时的信息流限制和过度压缩问题，显著提升了长文档表现。


<details>
  <summary>Details</summary>
Motivation: 现有的因果大语言模型（LLM）由于单向注意力机制，无法有效地从后文向前文传递信息；而目前仅在开头添加单个总结Token的方法会导致信息过度压缩，严重损害长文档的表示质量。

Method: 提出Hierarchical Token Prepending (HTP)方法：
1. 对抗注意力层面的压缩：将输入划分为多个块，并将块级总结Token前置到后续块中，建立多条反向信息流路径；
2. 对抗读出层面的过度挤压：基于理论分析，使用平均池化（mean-pooling）代替传统的末位Token池化（last-token pooling）。

Result: 在11个检索数据集和30个通用嵌入基准测试中取得了一致的性能提升，特别是在长上下文设置下表现优异。

Conclusion: HTP是一种简单且与架构无关的方法，能够同时增强零样本和微调模型的效果，为生成高质量的长文档嵌入提供了一条可扩展的路径。
...
### [84] [Mathematical Analysis of Hallucination Dynamics in Large Language Models: Uncertainty Quantification, Advanced Decoding, and Principled Mitigation](https://arxiv.org/abs/2511.15005)
*Moses Kiprono*

Main category: cs.CL

TL;DR: 本文提出了一个结合概率论、信息论和信号分析的数学框架，旨在通过改进的度量指标和解码策略，系统性地分析并减轻大型语言模型中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）虽然能力强大，但容易产生“幻觉”，即生成看似合理但实际上不正确或无根据的内容。为了解决这一问题，需要一个系统性的、数学上严谨的框架来理解和处理这些错误。

Method: 1. 引入概率建模、信息论、三角信号分析和贝叶斯不确定性估计构建数学基础。
2. 分析误差在自回归生成过程中的复合效应。
3. 提出改进的不确定性度量指标，包括语义和相位感知变体。
4. 开发对比解码、检索增强基础、事实对齐和拒绝回答（abstention）等缓解策略。

Result: 构建了一个统一的分析视角，成功将模型校准、检索增强和对齐方面的最新进展联系起来，并提出了一套用于度量和减轻幻觉的具体方法论。

Conclusion: 该框架通过数学原理将多种技术整合，为理解和解决LLM幻觉问题提供了坚实的理论基础，有助于构建更安全、更可靠的大型语言模型。
...
### [85] [Teaching According to Students' Aptitude: Personalized Mathematics Tutoring via Persona-, Memory-, and Forgetting-Aware LLMs](https://arxiv.org/abs/2511.15163)
*Yang Wu,Rujing Yao,Tong Zhang,Yufei Shi,Zhuoren Jiang,Zhushan Li,Xiaozhong Liu*

Main category: cs.CL

TL;DR: TASA是一个结合了学生画像、记忆机制和遗忘动态的个性化数学辅导框架，通过动态追踪学生掌握程度和遗忘曲线来提供适应性教学，效果优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型（LLM）辅导系统未能有效捕捉学生知识随熟练度、概念缺口和遗忘模式的动态演变。特别是在数学辅导中，缺乏根据学生掌握程度和认知保持力进行的精细化支架式教学。

Method: 提出了TASA（因材施教）框架，该框架：
1. 维护结构化的学生画像以捕捉熟练度概况。
2. 建立事件记忆以记录之前的学习交互。
3. 结合连续遗忘曲线与知识追踪，动态更新每位学生的掌握状态。
4. 基于上述信息生成上下文适宜且难度经过校准的问题和解释。

Result: 实证结果显示，TASA在学习成果和辅导行为的适应性方面均优于具有代表性的基线模型。

Conclusion: 该研究证明了在基于LLM的辅导系统中，对时间性遗忘和学习者画像进行建模对于提升辅导效果至关重要，TASA框架有效地解决了这一需求。
...
### [86] [HinTel-AlignBench: A Framework and Benchmark for Hindi-Telugu with English-Aligned Samples](https://arxiv.org/abs/2511.15183)
...
TL;DR: 提出针对印地语和泰卢固语的VLM评估基准HinTel-AlignBench，揭示了当前模型在印度语言上相比英语存在显著的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有的多语言视觉-语言模型（VLM）评估方法存在依赖未验证的自动翻译、任务覆盖面窄、样本量不足以及缺乏本土文化背景等局限性，尤其不利于像印度这样拥有大量人口和语言多样性的低资源语言区域的发展。

Method: 提出了一种结合回译、过滤和人工验证的半自动化数据集构建框架，创建了包含印地语和泰卢固语的“HinTel-AlignBench”基准。该基准涵盖改编的英语数据集（如VQAv2、RealWorldQA）和本土数据集（如JEE、VAANI），每种语言约含4000个问答对，并以此评估了多个SOTA模型。

Result: 在5项任务中，所有模型在4项任务上表现出从英语切换到印度语言时的性能倒退。印地语平均下降8.3分，泰卢固语平均下降5.5分。

Conclusion: 现有的VLM在处理印度语言时存在明显的性能衰退，通过分析常见失败模式，本研究为提升多语言多模态理解能力指明了具体的改进方向。
...
### [87] [Unveiling Intrinsic Dimension of Texts: from Academic Abstract to Creative Story](https://arxiv.org/abs/2511.15210)
...
TL;DR: 本文首次全面研究了LLM中内在维度（ID）的文本决定因素，发现ID与熵不相关，且科学文本表现出较低的ID（表示简单），而创意文本ID较高（表示复杂），揭示了不同文体对模型几何复杂性的影响。


<details>
  <summary>Details</summary>
Motivation: 内在维度（ID）是分析大型语言模型（LLM）训练动态、扩展行为和数据集结构的重要工具，但目前尚缺乏关于具体文本属性如何决定ID的深入研究。

Method: 本文采用交叉编码器分析、语言特征提取以及稀疏自动编码器（SAEs）进行了综合研究，并通过引导实验（steering experiments）验证了特征对ID的因果影响。

Result: 1. ID与基于熵的指标互补：在控制长度后两者不相关，ID捕捉的是独立于预测质量之外的几何复杂性。
2. ID表现出稳健的体裁分层：科学散文ID较低（~8），百科内容中等（~9），创意/观点写作ID较高（~10.5）。
3. 确定了因果特征：科学信号（正式语气、统计）降低ID，而人性化信号（个性化、情感、叙事）增加ID。

Conclusion: 对于现代模型而言，科学写作在表示上相对“简单”，而小说、观点和情感类内容则增加了表示的自由度。这项多方面的分析为正确使用内在维度（ID）及其结果的合理解读提供了实践指导。
...
### [88] [OEMA: Ontology-Enhanced Multi-Agent Collaboration Framework for Zero-Shot Clinical Named Entity Recognition](https://arxiv.org/abs/2511.15211)
*Xinli Tao,Xin Dong,Xuezhong Zhou*

Main category: cs.CL

TL;DR: OEMA是一个利用多智能体协作和SNOMED CT本体过滤的零样本临床NER框架，在无需标注数据的情况下达到了接近监督学习模型的性能。


<details>
  <summary>Details</summary>
Motivation: 临床命名实体识别（NER）对于处理电子健康记录至关重要，但监督模型（如CRF和BioClinicalBERT）依赖昂贵的标注数据。现有的零样本大语言模型（LLM）方法在样本选择粒度和自我改进的提示集成方面存在困难。

Method: 提出了OEMA框架，采用多智能体协作模式，包含三个核心组件：1. 自我标注器（Self-annotator）：生成示例；2. 判别器（Discriminator）：利用SNOMED CT本体对生成的示例进行过滤；3. 预测器（Predictor）：利用实体描述进行准确的推理。

Result: 在MTSamples和VAERS数据集上，OEMA取得了最先进的精确匹配（exact-match）性能。在相关匹配（related-match）指标下，其表现与监督式BioClinicalBERT相当，并优于CRF模型。

Conclusion: OEMA通过本体引导推理和多智能体协作，有效解决了零样本NER的关键挑战，实现了接近监督学习的性能，展示了在临床NLP应用中的巨大潜力。
...
### [89] [Context Cascade Compression: Exploring the Upper Limits of Text Compression](https://arxiv.org/abs/2511.15244)
*Fanfan Liu,Haibo Qiu*

Main category: cs.CL

TL;DR: 本文提出了C3（上下文级联压缩）方法，通过级联大小两个LLM将长文本压缩为潜在Token，在高达40倍的压缩率下仍保持高准确率，性能显著优于光学字符压缩方法。


<details>
  <summary>Details</summary>
Motivation: 应对百万级Token输入的长上下文任务给大语言模型带来的计算和内存巨大挑战。受DeepSeek-OCR光学压缩研究的启发，旨在探索文本压缩的极限，并解决现有光学压缩方法的不足。

Method: 提出了一种名为“上下文级联压缩”（Context Cascade Compression, C3）的方法，通过级联两个不同规模的大语言模型（LLM）来处理任务：
1. 第一阶段（压缩）：使用小型LLM将长上下文压缩为一组高压缩比的潜在Token（如长度为32或64）。
2. 第二阶段（解码）：使用大型LLM基于这些压缩后的潜在Token执行解码和生成任务。

Result: 1. 在20倍压缩率下，C3模型达到了98%的解码准确率，显著高于DeepSeek-OCR的约60%。
2. 在将压缩率进一步提高到40倍时，准确率仍能保持在93%左右。

Conclusion: C3方法在上下文压缩方面表现优于光学字符压缩，证明了纯文本流程在忽略布局和视觉损失情况下的优越性。该研究不仅提供了一种高效的压缩方案，也为未来OCR及相关领域的光学字符压缩率上限提供了参考基准。
...
### [90] [IndicGEC: Powerful Models, or a Measurement Mirage?](https://arxiv.org/abs/2511.15260)
*Sowmya Vajjala*

Main category: cs.CL

TL;DR: TeamNRC通过零样本/少样本提示技术在印度语言语法纠错任务中取得优异成绩，并分析了模型大小的影响及数据和指标的适用性问题。


<details>
  <summary>Details</summary>
Motivation: 参与BHASHA-Task 1共享任务，旨在探索大语言模型在5种印度语言语法纠错（GEC）任务中的表现，并深入审视数据质量和评估指标。

Method: 采用了针对不同规模（从4B参数到大型专有模型）语言模型的零样本（zero-shot）和少样本（few-shot）提示（prompting）策略。

Result: 团队在泰卢固语中获得第4名（GLEU 83.78），在印地语中获得第2名（GLEU 84.31），并将实验扩展到了泰米尔语、马拉雅拉姆语和孟加拉语。

Conclusion: 研究展示了小型语言模型在语法纠错任务中的潜力，同时强调了针对印度语言脚本创建高质量数据集和采用合适评估指标的必要性。
...
### [91] [MAPROC at AHaSIS Shared Task: Few-Shot and Sentence Transformer for Sentiment Analysis of Arabic Hotel Reviews](https://arxiv.org/abs/2511.15291)
*Randa Zarnoufi*

Main category: cs.CL

TL;DR: 该研究利用SetFit框架对阿拉伯方言（摩洛哥和沙特）的酒店评论进行情感分析，旨在解决数据稀缺问题，最终取得了73%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯方言具有语言多样性且缺乏标注数据，使得对其进行情感分析面临巨大挑战。

Method: 采用了SetFit（Sentence Transformer Fine-tuning）框架，这是一种数据高效的小样本学习（few-shot learning）技术。

Result: 在AHaSIS共享任务的官方评估集上，该系统实现了73%的F1分数，在26个参赛队伍中排名第12。

Conclusion: 研究结果突显了小样本学习在解决特定领域（如酒店业）中细微差别的阿拉伯方言文本处理及数据稀缺问题上的潜力。
...
TL;DR: 本文介绍了HEAD-QA v2，这是一个扩展后的西班牙语/英语医疗多选推理数据集，包含超过1.2万个问题，并评估了多个开源LLM在医疗推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 为了应对医疗领域对高质量数据集日益增长的需求，旨在更好地捕捉医疗推理中的语言和概念复杂性。

Method: 作者将数据集扩展至包含十年间的西班牙专业考试题目，总数超过12,000题；使用提示工程（Prompting）、检索增强生成（RAG）和基于概率的答案选择方法对多个开源大语言模型（LLM）进行了基准测试，并提供了额外的多语言版本。

Result: 基准测试表明，模型性能主要取决于模型的规模和内在推理能力，而复杂的推理策略所带来的性能提升有限。

Conclusion: HEAD-QA v2 被确立为推进生物医学推理研究和模型改进的可靠资源。
...
### [93] [DEPO: Dual-Efficiency Preference Optimization for LLM Agents](https://arxiv.org/abs/2511.15392)
*Sirui Chen,Mengshi Zhao,Lei Xu,Yuying Zhao,Beier Zhu,Hanwang Zhang,Shengjie Zhao,Chaochao Lu*

Main category: cs.CL

TL;DR: 论文提出了DEPO方法，通过优化“双重效率”（步骤级和轨迹级），在大幅减少Token消耗和操作步骤的同时，显著提升了LLM智能体的任务完成性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）智能体的推理能力虽然增强，但往往伴随着冗长的思维链（CoT），导致交互效率低下。此外，目前缺乏对LLM智能体效率的系统定义，阻碍了针对性的改进。

Method: 引入了“双重效率”定义，包含步骤级效率（最小化每步Token数）和轨迹级效率（最小化完成任务的步骤数）。基于此提出了DEPO（双重效率偏好优化）方法，联合奖励简洁的回复和更少的行动步骤。

Result: 在WebShop和BabyAI上的实验表明，DEPO减少了高达60.9%的Token使用量和26.9%的步骤数，同时性能提升了29.3%。此外，该方法在域外数学基准测试中表现出泛化能力，且仅用25%的数据训练也能保持效率收益。

Conclusion: DEPO不仅有效地定义了智能体效率，还通过联合优化显著降低了计算开销并提升了任务成功率，展现了在不同任务和数据量下的鲁棒性与泛化潜力。
...
### [94] [NAMeGEn: Creative Name Generation via A Novel Agent-based Multiple Personalized Goal Enhancement Framework](https://arxiv.org/abs/2511.15408)
*Shanlin Zhou,Xinpeng Wang,Jianxun Lian,Zhenghao Liu,Laks V. S. Lakshmanan,Xiaoyuan Yi,Yongtao Hao*

Main category: cs.CL

TL;DR: 本文提出了NAMeGEn，一个针对中文取名任务的多智能体优化框架，利用古诗词语料库，在无需训练的情况下解决了创意生成中的多目标约束和审美解释难题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在创意自然语言生成（CNLG）方面虽有潜力，但面临两大挑战：一是多目标灵活性，即难以同时满足用户个性化、细粒度和多元化的需求；二是解释复杂性，即难以理解和解释隐含意义以增强用户的感知。目前的短文本生成方法在生成兼具创意和深刻见解的内容方面受到限制。

Method: 文章提出了NAMeGEn，这是一个新颖的多智能体优化框架，通过迭代交替进行目标提取、名字生成和评估来满足多样化需求并生成准确解释。为支持该任务，作者还构建了一个包含1.7万多首古典诗词的语料库以增强美感，并引入了一个带有定制指标的新基准CBNames。

Result: 广泛的实验表明，NAMeGEn在无需任何额外训练的情况下，能够有效生成满足多样化、个性化需求的创意名字，并提供有意义的解释，其表现优于涵盖不同LLM骨干的六种基线方法。

Conclusion: 该研究表明，NAMeGEn框架通过多智能体协作机制及引入古诗文语料库，有效解决了短文本创意生成中满足复杂约束和提供审美解释的难题，在中文取名任务上展现了优越的性能，为创意自然语言生成提供了新的思路。
...
### [95] [Building Robust and Scalable Multilingual ASR for Indian Languages](https://arxiv.org/abs/2511.15418)
...
### [96] [LLM-MemCluster: Empowering Large Language Models with Dynamic Memory for Text Clustering](https://arxiv.org/abs/2511.15424)
...
### [97] [Standardising the NLP Workflow: A Framework for Reproducible Linguistic Analysis](https://arxiv.org/abs/2511.15512)
...
### [98] [Multimodal Evaluation of Russian-language Architectures](https://arxiv.org/abs/2511.15552)
...
### [99] [HSKBenchmark: Modeling and Benchmarking Chinese Second Language Acquisition in Large Language Models through Curriculum Tuning](https://arxiv.org/abs/2511.15574)
