...
Motivation: 当前视觉语言模型在Design-to-Code生成中存在"整体瓶颈"问题，无法同时处理高层次结构层次和细粒度视觉细节，导致布局扭曲或使用通用占位符。

Method: 提出DOne端到端框架：1) 学习布局分割模块分解复杂设计；2) 专用混合元素检索器处理UI组件的极端宽高比和密度；3) 模式引导的生成范式连接布局和代码。

Result: 在HiFi2Code基准测试中，DOne在高层视觉相似性（如GPT Score提升超10%）和细粒度元素对齐方面优于现有方法。人工评估显示生产力提升3倍且视觉保真度更高。

Conclusion: DOne通过解耦结构理解和元素渲染的方法有效解决了Design-to-Code中的"整体瓶颈"问题，显著提升了生成代码的准确性和视觉质量。
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进多模态大语言模型使用多模态证据的框架，通过注意力模式识别相关视觉区域和文本元素，并用提示标记突出显示，引导模型重新关注相关证据


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答知识密集型查询时，难以识别最相关的视觉和文本证据，需要整合视觉线索与检索到的文本证据（通常有噪声或仅部分相关），同时定位图像中的细粒度视觉信息

Method: 利用模型注意力模式估计哪些视觉区域和检索到的文本元素与查询相关，然后基于这些突出显示的证据生成答案。通过轻量级的提示级标记突出显示所选线索，鼓励模型在生成过程中重新关注相关证据

Result: 在多个基于知识的视觉问答基准测试中，相比零样本多模态大语言模型取得了一致的改进。在没有文本上下文的视觉中心和幻觉导向基准测试中，仅视觉证据突出显示也能提高模型性能，且无需额外训练或架构修改

Conclusion: Look Twice框架通过推理时重新关注相关多模态证据，有效提高了预训练多模态大语言模型在知识密集型视觉问答任务中的性能，且无需额外训练成本
...
TL;DR: MedQwen是一个参数高效的医学视觉语言模型，通过谱路由的专家混合架构和理论驱动的缩放规则，在保持基础架构不变的情况下，实现了接近全参数微调的性能，同时显著减少了灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用基准上表现出色，但在医学影像领域缺乏鲁棒性，存在跨数据集干扰和对数据制度的敏感性。在临床工作流中，数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，结合谱路由的专家混合架构和理论驱动的缩放规则。从预训练权重的非重叠奇异值分解片段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。

Result: 在23个医学数据集上（包括视觉问答、报告生成、放射学分类和幻觉缓解），MedQwen实现了强大可靠的性能：在零样本分类上接近全参数微调，但仅使用1/339的可训练参数；将顺序遗忘减少到约5%，而强基线模型性能下降20-50%。
...
<details>
  <summary>Details</summary>
Motivation: 早期识别接触性运动中的危险动作能够及时干预并提高运动员安全性。美式足球中的擒抱动作是导致伤害的主要原因之一，需要开发自动检测系统来辅助教练进行伤害预防。

Method: 1. 构建了包含733个单运动员-假人擒抱片段的扩展数据集，每个片段围绕首次接触点进行时间定位，并使用标准擒抱技术评估(SATT-3)的撞击区域组件进行标注；2. 采用基于视觉Transformer的模型；3. 使用类别不平衡感知训练方法处理数据不平衡问题。

Result: 在交叉验证中获得了0.67的危险召回率和0.59的危险F1分数。相比之前较小数据集上的基线方法（危险召回率0.58，危险F1分数0.56），危险召回率提升了超过8个百分点。
...
TL;DR: 通过合成蹦床姿态数据集（STP）微调ViTPose模型，显著提升了在极端蹦床姿态和非常规视角下的姿态估计性能


<details>
  <summary>Details</summary>
Motivation: 现有的姿态估计模型在蹦床体操这种极端人体姿态和非常规视角下表现不佳，需要专门的方法来提升性能

Method: 从蹦床动作的运动捕捉数据生成合成蹦床姿态数据集（STP），通过管道拟合噪声运动捕捉数据到参数化人体模型，生成多视角真实图像，然后用这些数据微调ViTPose模型

Result: 在2D姿态估计上取得了最先进的结果，在3D三角测量中将MPJPE降低了12.5毫米，相比预训练的ViTPose模型提升了19.6%

Conclusion: 使用合成数据微调姿态估计模型是解决极端姿态和非常规视角下性能下降的有效方法，显著提升了蹦床体操场景下的姿态估计精度
...
TL;DR: 提出一种基于自助法的注意力正则化方法，通过重采样输入特征生成注意力分数的基线分布，估计显著性并去除噪声，提高ViT注意力图的可解释性。
...
Method: 将ViT注意力分数置于统计框架中，引入自助法（bootstrapping）对注意力分数进行重采样，生成基线分布，然后估计注意力分数的显著性和后验概率，实现注意力正则化。

Result: 在自然图像和医学图像上，该方法能有效去除噪声引起的虚假注意力，显著提高注意力图的收缩性和稀疏性。通过模拟和真实数据集进行了定量评估。

Conclusion: 自助法可作为实用的正则化工具，当使用注意力分数作为ViT解释时，能提高注意力图的质量和可解释性。
...
TL;DR: 研究发现CNN模型作为视觉系统模型的质量与其与人类纹理感知的对齐程度之间没有关联，表明纹理感知机制不同于基于物体识别训练的CNN常用建模方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索CNN特征相关性（Gram矩阵）是否自发对齐纹理的感知内容，特别是那些被视为更好视觉系统模型的CNN是否具有更类似人类的纹理表示。

Method: 比较了多样化CNN池中计算的特征相关性捕获的感知内容，并将其与通过Brain-Score测量的模型与哺乳动物视觉系统的感知对齐进行比较。

Result: 令人惊讶地发现，CNN作为视觉系统模型的传统质量衡量标准与其与人类纹理感知的对齐程度之间没有关联。

Conclusion: 纹理感知涉及不同于基于物体识别训练的CNN常用建模方法的机制，可能依赖于上下文信息的整合。
...
TL;DR: AffordTissue：一种用于胆囊切除术中预测工具-动作特定组织可操作区域的多模态框架，通过密集热图形式提供明确的空间推理，提升手术自动化安全性。


<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在挑战：缺乏对器械在组织表面交互位置的预测能力，缺少强制执行工具-动作特定安全交互区域的显式条件输入。

Method: 结合时间视觉编码器（捕捉多视角下的工具运动和组织动态）、语言条件（实现不同器械-动作对的泛化）和DiT风格解码器，进行密集可操作性预测。

Result: 在包含15,638个视频片段、103个胆囊切除术、6种工具-动作对的基准测试中，相比视觉语言模型基线（Molmo-VLM的60.2px ASSD），性能显著提升至20.6px ASSD。

Conclusion: AffordTissue通过预测工具-动作特定的组织可操作区域，为安全手术自动化提供明确的空间推理，可能解锁对适当组织区域的显式策略指导，并在器械偏离预测安全区域时实现早期安全停止。
...
TL;DR: GRAZE是一个无需训练的管道，用于在美式足球训练视频中定位首次接触点（FPOC），通过Grounding DINO发现候选交互、运动感知时间推理和SAM2像素级验证，在真实训练视频中实现帧级准确接触起始定位。
...
Method: GRAZE采用无训练管道，包含三个主要组件：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时间推理进行精炼；3) 使用SAM2作为显式像素级接触验证器，不依赖检测置信度。候选发现和接触确认的分离使方法对杂乱场景和冲击附近不稳定定位具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的视频中产生有效输出，在77.5%的所有视频中将FPOC定位在±10帧内，在82.7%的所有视频中定位在±20帧内。结果表明在真实训练视频中实现帧级准确接触起始定位是可行的，无需任务特定训练。

Conclusion: GRAZE展示了在无约束美式足球训练视频中准确定位首次接触点的可行性，通过分离候选发现和接触确认的方法，克服了杂乱场景和冲击附近不稳定定位的挑战，为无需任务特定训练的帧级准确接触起始定位提供了有效解决方案。
...
TL;DR: 提出基于稀疏体素栅格化的新框架，解决3D高斯泼溅在开放词汇3D场景理解中的空间模糊和多层语义模糊问题，在细粒度查询任务上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的开放词汇3D场景理解方法存在两个关键限制：1) 非结构化、重叠高斯分布导致的空间模糊，需要概率特征注册；2) 基于物体级掩码池化特征导致的多层语义模糊，稀释了细粒度细节

Method: 使用稀疏体素栅格化作为结构化、不相交的几何表示，通过单目深度和法线先验进行正则化，建立稳定几何基础。实现确定性、置信度感知的特征注册过程，并利用AM-RADIO基础模型的密集对齐特性解决多层语义模糊

Result: 在开放词汇3D物体检索和点云理解基准测试中达到最先进性能，特别是在传统注册方法通常失败的细粒度查询任务上表现出色

Conclusion: 稀疏体素栅格化框架有效解决了3D高斯泼溅方法中的空间和语义模糊问题，为开放词汇3D场景理解提供了更稳定、精确的解决方案
...
TL;DR: EgoFlow是一个基于流匹配的框架，用于从第一人称视角视频中生成物理一致的6自由度物体轨迹，通过混合架构和梯度引导推理实现物理约束，在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 从第一人称视角视频理解和预测物体运动对具身感知和交互至关重要，但现有生成模型缺乏显式物理推理，难以处理遮挡、快速运动等问题，无法生成物理一致的6自由度轨迹。

Method: 提出EgoFlow流匹配框架，采用混合Mamba-Transformer-Perceiver架构联合建模时间动态、场景几何和语义意图，通过梯度引导推理过程强制执行可微物理约束（如碰撞避免和运动平滑性）。

Result: 在HD-EPIC、EgoExo4D和HOT3D等真实世界数据集上，EgoFlow在准确性、泛化性和物理真实性方面优于基于扩散和Transformer的基线方法，碰撞率降低高达79%，对未见场景具有强泛化能力。

Conclusion: 流匹配生成建模在可扩展和物理基础的自我中心运动理解方面具有前景，EgoFlow无需后处理过滤或额外监督即可生成连贯可控的运动轨迹。
...
Method: 开发了一个通用分析流程，结合原则性预处理、降维和基于递归的时间序列分析，量化运动动态的时间结构。该流程支持线性和非线性运动特征描述。

Result: 通过三个案例研究展示了流程的灵活性：涵盖面部和全身运动、2D和3D数据、个体与多智能体行为，证明同一分析工作流可以适应不同实验环境。

Conclusion: 该通用分析流程能够从复杂姿态时间序列中提取理论上有意义的洞察，为大规模行为分析提供了有效工具，展示了在不同实验环境中分析人类运动的适应性。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模存在重建保真度与生成合理性之间的基本矛盾：前馈重建方法效率高且与输入对齐，但缺乏全局先验导致结构不完整；扩散生成方法能提供丰富的几何细节，但难以保证多视图一致性。
...
<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受到鬼影效应的根本限制。虽然HADAR技术开启了计算热成像的新前沿，但现有方法仅适用于材料均匀的有限场景，而材料非均匀性在现实世界中普遍存在，因此需要一种通用的抗鬼影成像框架

Method: 提出TAG（热抗鬼影）通用计算热成像框架，利用高光谱光子流进行非参数纹理恢复，解决材料非均匀性问题，克服鬼影效应

Result: TAG在各种场景中普遍优于HADAR，首次实现了鬼影人脸的表情恢复，展示了热3D拓扑对齐和情绪检测能力，揭示了材料非均匀性对HADAR效果边界的影响
...
TL;DR: PBSeg是一个针对无人机图像语义分割的高效原型分割框架，通过原型交叉注意力减少计算复杂度，在保持分割质量的同时提高计算效率。
...
Method: 提出PBSeg框架，包含原型交叉注意力(PBCA)利用特征冗余减少计算复杂度，以及结合可变形卷积和上下文感知调制的多尺度特征提取模块，同时捕捉局部细节和全局语义。
...
Method: ReFlow包含三个核心模块：1) 完整规范空间构建模块，增强静态和动态区域的初始化；2) 分离式动态场景建模模块，解耦静态和动态组件以进行针对性运动监督；3) 自校正流匹配机制，包括全流匹配（对齐3D场景流与时间变化的2D观测）和相机流匹配（强制静态对象的多视角一致性）。

Result: 在多种场景下的广泛实验表明，ReFlow实现了卓越的重建质量和鲁棒性，为单目4D重建建立了新的自校正范式。

Conclusion: ReFlow通过自校正流匹配机制，无需外部密集运动指导，实现了鲁棒且准确的动态场景重建，为单目4D重建提供了新的解决方案。
...
TL;DR: VideoZeroBench是一个针对长视频问答的层次化基准测试，通过严格的时空证据验证来评估视频多模态大模型，揭示了当前模型在基于证据的推理方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 高分可能掩盖细粒度视觉理解和推理的缺陷；(2) 答案正确性评估往往不验证模型是否识别了支持其预测的精确时空证据。需要建立一个能严格验证时空证据的基准测试。

Method: 提出VideoZeroBench基准测试，包含500个手动标注的问题，覆盖13个领域，每个问题都配有作为证据的时间区间和空间边界框。引入五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下，Gemini-3-Pro的正确回答率也低于17%。当施加定位约束时，性能急剧下降：在要求正确答案和准确时空定位的Level-5下，没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。
...
TL;DR: 提出GAAL方法解决多模态表格-图像融合中的梯度冲突问题，通过梯度对齐交替学习提升融合性能
...
Method: 提出梯度对齐交替学习(GAAL)范式：1)采用交替单模态学习和共享分类器解耦多模态梯度并促进交互；2)设计基于不确定性的跨模态梯度手术，选择性对齐跨模态梯度，使共享参数对所有模态都有利

Result: 在广泛使用的数据集上实验表明，该方法优于各种最先进的表格-图像融合基线和测试时表格缺失基线
...
TL;DR: 提出卫星数据无关的无人机视图地理定位训练框架，通过3D场景重建和伪正射影像生成，实现无卫星数据训练下的跨视图检索。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视图地理定位方法依赖卫星图像进行训练，但在卫星数据不可用或受限的实际部署场景中面临困难。需要开发不依赖卫星数据的训练框架。

Method: 提出卫星无关训练框架：1) 使用3D高斯泼溅从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像；3) 轻量级几何引导修复获得完整纹理；4) 从无人机数据学习Fisher向量聚合模型，测试时用于编码卫星瓦片进行跨视图检索。

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星无关的泛化基线方法，缩小了与使用卫星图像训练方法的性能差距。

Conclusion: 提出的卫星无关训练框架能够在不依赖卫星数据的情况下，通过无人机端3D重建和几何归一化表示，有效实现无人机视图地理定位任务。
...
Method: SHOE将每个HOI预测分解为动词和对象组件，使用多个大型语言模型（LLMs）的平均值估计它们的语义相似度，并将它们组合成相似度分数，实现超越精确字符串匹配的评估。

Result: 实验结果显示，SHOE分数比现有指标（包括基于LLM和嵌入的基线）更接近人类判断，与平均人类评分的同意率达到85.73%。
...
<details>
  <summary>Details</summary>
Motivation: 开放集测试时适应中，协变量偏移会同时影响ID和OOD样本。现有方法结合熵最小化（保持ID性能）和熵最大化（增强OOD检测）存在内在冲突，导致ID分类和OOD检测之间的权衡问题。

Method: 提出ROSETTA方法：1）分析熵最大化在OSTTA中的局限性；2）引入角度损失调节特征范数大小；3）使用特征范数损失抑制csOOD样本的logits，从而改进OOD检测。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强大的OOD检测能力，同时保持高ID分类性能。Cityscapes上的实验验证了在真实世界语义分割中的有效性，HAC数据集结果证明了在不同开放集TTA设置中的适用性。

Conclusion: ROSETTA方法通过角度损失和特征范数损失有效解决了开放集测试时适应中ID分类和OOD检测的权衡问题，在各种数据集和实际应用中表现出色。
...
Method: 1. 通过注册多个客户端本地合并的LiDAR点云构建共享几何骨架，初始化全局3DGS模型；2. 联邦优化期间固定高斯位置保持几何对齐；3. 每个客户端只更新外观相关属性（协方差、不透明度、球谐系数）；4. 服务器使用可见性感知聚合更新，根据客户端观察每个高斯的频率加权贡献。

Result: 实验使用收集的多序列室内数据集（同步LiDAR、RGB和IMU测量）显示，F3DGS在实现与集中训练相当的重建质量的同时，支持跨智能体的分布式优化。

Conclusion: F3DGS为分布式3D重建提供了有效的联邦学习解决方案，解决了多智能体探索中的部分可观测性挑战，数据集、开发工具包和源代码将公开发布。
...
TL;DR: Tex3D：首个在VLA仿真环境中端到端优化3D对抗纹理的框架，通过前景-背景解耦和轨迹感知对抗优化，在物理世界中显著降低VLA模型性能


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型对物理可实现对抗攻击的鲁棒性研究不足，现有攻击方法要么不具代表性（语言扰动），要么物理真实性有限（2D视觉攻击）。3D对抗纹理作为更物理可行且破坏性强的威胁，需要专门研究其在VLA系统中的影响

Method: 提出Tex3D框架，包含两个核心技术：1) 前景-背景解耦(FBD)：通过双渲染器对齐实现可微纹理优化，同时保持原始仿真环境；2) 轨迹感知对抗优化(TAAO)：优先处理行为关键帧，使用基于顶点的参数化稳定优化

Result: 在仿真和真实机器人实验中，Tex3D显著降低了VLA模型在多个操作任务中的性能，任务失败率高达96.7%。实验暴露了VLA系统对物理基础3D对抗攻击的关键脆弱性

Conclusion: Tex3D是首个在VLA仿真环境中端到端优化3D对抗纹理的框架，成功实现了物理可行的3D对抗攻击，揭示了VLA系统的重要安全漏洞，强调了鲁棒性感知训练的必要性
...
<details>
  <summary>Details</summary>
Motivation: 形态特征是理解生物与环境相互作用的关键，但传统提取方法依赖专家标注，效率低下且难以大规模应用，缺乏高质量图像-特征标注数据集是主要瓶颈

Method: 1) 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元；2) 构建特征标注流程：定位显著区域 + 视觉语言提示生成可解释特征描述；3) 创建Bioscan-Traits数据集（19K昆虫图像，80K标注）

Result: 1) 人类评估确认生成形态描述的生物学合理性；2) 通过全面消融研究评估设计敏感性；3) 提供可扩展的形态特征标注方法，为大规模生态研究提供支持

Conclusion: 通过模块化流水线而非昂贵人工标注，为大规模形态分析提供可扩展方案，弥合生态相关性与机器学习实用性之间的差距，为生物监督注入基础模型
...
Method: 1. 渐进构建全局一致的运动场；2. 引入几何感知对齐模块解决视角间的方向和尺度模糊性；3. 使用基于哈希的紧凑运动场表示，支持高效查询和动态稳定传播；4. 支持渲染时的双向运动传播，无需依赖昂贵的视频优化。

Result: 在单个RTX 5090 GPU上，每个新场景扩展步骤需要9秒，运动对齐和运动场更新需要3秒，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架成功解决了从单张图像生成具有连贯环境动态的4D世界的挑战，实现了交互式生成和长期时间一致性。
...
Method: 提出包含两个关键模块的框架：1) 道路感知补丁增强(RAPA)：利用稀疏标注，将分割的目标补丁增强到道路区域，同时保持3D几何一致性；2) 原型过滤(PBF)：通过原型相似性和深度不确定性过滤预测结果，生成高质量伪标签。该方法结合几何保持增强和原型引导的伪标签生成。

Result: 通过大量实验证明了所提方法的有效性，在稀疏监督下实现了鲁棒的3D目标检测。

Conclusion: 提出的框架通过几何保持增强和原型引导伪标签生成，有效解决了稀疏标注单目3D目标检测问题，在现实场景中具有实用价值。
...
TL;DR: 该论文提出了一种基于物理学的视频认证方法，利用莫尔效应区分真实拍摄视频与AI生成视频，通过验证莫尔条纹相位与光栅位移的线性耦合关系来检测视频真伪。
...
Method: 利用莫尔效应：当相机观察紧凑的双层光栅结构时会产生干涉条纹。推导出莫尔运动不变量，证明条纹相位与光栅图像位移通过光学几何线性耦合，且与观察距离和光栅结构无关。验证器从视频中提取这两个信号并测试它们的相关性。

Result: 在真实拍摄视频和多个最先进AI生成器的生成视频上进行验证，发现真实视频和AI生成视频产生显著不同的相关性特征，表明该方法能有效区分两者。

Conclusion: 确定性光学现象可以作为物理基础的可验证签名来对抗AI生成视频，为视频认证提供了一种稳健的方法。
...
TL;DR: DynaVid是一个视频合成框架，利用合成运动数据（光流）解决现有视频扩散模型在高度动态运动和精细运动控制方面的局限性。
...
Method: 采用两阶段生成框架：1）运动生成器合成运动（表示为光流）；2）运动引导的视频生成器基于该运动生成视频帧。使用计算机图形管道渲染的合成运动数据训练。

Result: 在剧烈人体运动生成和极端相机运动控制这两个具有挑战性的场景中，DynaVid显著提升了动态运动生成和相机运动控制的真实感和可控性。
...
TL;DR: 提出了一种无需域ID和示例的无监督增量学习框架，用于解决具身感知系统在开放物理空间中连续交互时的环境分布漂移问题。


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间中连续交互时面临动态环境分布漂移的严重挑战。现有域增量感知方法通常依赖测试阶段预先获得的域ID，限制了在未知交互场景中的实用性。同时，模型容易过拟合到特定上下文感知噪声，导致泛化能力不足和灾难性遗忘。

Method: 提出域ID和示例自由的增量学习框架，设计解耦表示机制去除非必要环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征。采用权重融合策略在参数空间中动态整合新旧环境知识，无需存储历史数据，同时最大化保留旧环境的判别能力。

Result: 在多个标准基准数据集上的广泛实验表明，该方法在完全无示例和无域ID的设置下显著减少了灾难性遗忘，其准确率优于现有最先进方法。

Conclusion: 该方法能够实现鲁棒的连续环境适应，通过解耦表示和权重融合策略有效解决了具身多媒体系统的增量学习问题，在未知交互场景中具有更好的实用性。
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，用于提升远程光电容积描记(rPPG)在跨域场景下的鲁棒性和泛化能力，解决外观变化导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记(rPPG)技术在实际部署中面临显著的域偏移问题，现有深度学习方法容易过拟合于光照、相机特性、颜色响应等外观相关因素，导致跨域性能严重下降。

Method: 提出频率域适应(FDA)策略，通过转移编码域依赖外观特征的低频频谱成分，使模型学习对外观变化的不变性；进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性指导原始表示和FDA转移表示之间的对齐。

Result: 广泛的跨数据集实验表明，FDA和HOT框架能有效增强rPPG模型在不同数据集间的鲁棒性和泛化能力。

Conclusion: 频率域适应和谐波约束最优传输为rPPG模型提供了一种原则性的方法来建模外观变化，显著提升了跨域场景下的性能表现。
...
TL;DR: Director提出了一种统一的时空高斯表示方法，联合建模人体表演、高保真渲染和实例级语义，通过嵌入实例一致的语义信息实现更准确的场景分解和动态场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前基于高斯的方法主要关注外观渲染，但对实例级结构不敏感，限制了在高度动态场景中的稳定跟踪和语义推理能力。需要一种能够同时处理外观、运动和语义的统一表示方法。

Method: 1) 利用时间对齐的实例掩码和多模态大语言模型的句子嵌入，通过两个MLP解码器监督每个高斯的学习语义特征；2) 将2D光流与4D高斯桥接，微调其运动以获得可靠初始化；3) 引入几何感知的SDF约束和正则化项，增强动态前景建模的时间一致性。
...
TL;DR: 提出UnderEraser方法，通过外部知识蒸馏和内部帧间上下文交叉注意力机制，在视频对象移除中同时消除目标对象及其引发的副作用（阴影、反射等），实现更清晰连贯的移除效果


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频对象移除方法难以同时消除目标对象及其引发的副作用（如阴影、反射、光照变化），且容易破坏时空一致性。这源于对目标对象及其与场景交互的物理和语义理解不足

Method: 1. 外部：通过知识蒸馏方案，将视觉基础模型中对象与其引发效果的关系迁移到视频扩散模型中；2. 内部：提出帧间上下文交叉注意力机制，使每个去噪块都基于目标区域周围未掩码的上下文信息进行推理
...
Conclusion: 该方法通过同时进行群体检测和共享注意力估计，解决了现有方法的局限性，提高了共享注意力检测的实际应用性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于流的生成模型在文本引导图像编辑中存在源图像保真度不足的问题：高阶求解器需要额外推理、截断反演限制编辑能力、特征注入方法缺乏架构可迁移性。

Method: 1. 前向过程：提出摊销固定点求解器，通过强制连续时间步的速度一致性隐式拉直前向轨迹，获得高保真反演潜在表示；2. 后向过程：提出轨迹插值，自适应混合目标编辑和源重建速度，使编辑轨迹锚定源图像；3. 自适应掩码机制：通过概念引导分割和源-目标速度差异空间约束编辑信号，改善背景保留。

Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的大量实验表明，SteerFlow在编辑质量上始终优于现有方法，并能自然扩展到复杂的多轮编辑范式而不累积漂移。

Conclusion: SteerFlow是一个具有强大理论保证的模型无关编辑框架，通过隐式轨迹拉直和自适应信号混合，在保持源图像保真度的同时实现高质量的文本引导图像编辑。
...
<details>
  <summary>Details</summary>
Motivation: 人群实例分割在监控和交通等领域有广泛应用，但现有数据集主要使用点标注，区域标注（如边界框）稀少且不准确。直接应用SAM等大型基础模型在密集人群场景效果不佳，需要专门的方法来处理密集人群的实例分割问题。

Method: 1. 提出Dense Point-to-Mask Optimization (DPMO)：结合SAM和最近邻排他圆约束，从点标注生成密集实例分割掩码
2. 提出Reinforced Point Selection (RPS)框架：使用Group Relative Policy Optimization (GRPO)训练，从初始点预测采样中选择最佳预测点
3. 设计基于掩码监督的新损失函数来提升计数性能

Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上实现了最先进的人群实例分割性能。掩码监督的损失函数在不同模型上都能显著提升计数精度。

Conclusion: 提出的DPMO方法能够从点标注生成高质量的掩码标注，RPS框架有效提升了密集人群的实例分割性能。掩码标注在提升计数精度方面发挥了重要作用，证明了掩码监督的价值。
...
TL;DR: 提出了一种几何感知的无人机地理定位框架，通过3D场景重建和虚拟鸟瞰图渲染，将粗粒度位置识别与细粒度姿态估计统一到单一推理流程中，显著提升了GNSS拒止环境下的跨视角地理定位精度。


<details>
  <summary>Details</summary>
Motivation: 在GNSS拒止环境下，无人机倾斜图像与正交卫星地图之间存在严重的几何差异，现有方法通常采用解耦的位置检索和姿态估计流程，将透视畸变视为外观噪声而非显式几何变换，这限制了定位精度。

Method: 提出几何感知的无人机地理定位框架：1) 使用视觉几何基础变换器(VGGT)从多视角无人机图像序列重建局部3D场景；2) 渲染虚拟鸟瞰图(BEV)表示，将无人机视角正交化以对齐卫星图像；3) 引入卫星注意力块，隔离每个卫星候选与重建无人机场景的交互，防止候选间干扰；4) 发布重新校准的University-1652数据集，包含精确坐标标注和空间重叠分析。

Result: 在重新校准的University-1652基准和SUES-200数据集上的大量实验表明，该方法显著优于现有最先进基线，实现了鲁棒的米级定位精度，并在复杂城市环境中表现出更好的泛化能力。

Conclusion: 通过显式建模3D场景几何，将位置识别和姿态估计统一到单一推理流程中，提出的几何感知框架能够有效处理无人机倾斜图像与卫星地图之间的几何差异，为GNSS拒止环境下的无人机地理定位提供了更准确、更鲁棒的解决方案。
...
Method: 1) 构建US-365K大规模超声图像-文本数据集（36.5万配对样本，52个解剖类别）；2) 建立超声诊断分类学(UDT)，包含层次解剖分类学和诊断属性框架；3) 提出Ultrasound-CLIP框架，引入语义软标签和语义损失；4) 构建基于UDAF文本表示的异构图模态，实现病变-属性关系的结构化推理。

Result: 在患者级数据划分下，该方法在分类和检索基准上达到最先进性能，同时在零样本学习、线性探测和微调任务上表现出强大的泛化能力。

Conclusion: 该研究通过构建大规模超声数据集和语义感知对比学习框架，成功解决了现有视觉-语言模型在超声领域的适用性问题，为超声图像的智能诊断提供了有效解决方案。
...
TL;DR: 该论文提出了一种轻量级架构和训练策略，利用自监督学习特征作为预训练视频扩散模型的通用条件信号，实现视频域迁移和3D到视频生成，通过解耦外观与其他特征来增强生成控制能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要用作生成渲染器，通常通过感知、几何或简单语义信号进行条件控制。虽然自监督学习获得的高维特征已被用作视觉模型的通用接口，但这些特征包含大量纠缠的风格、光照和语义信息，限制了其生成能力。本文旨在探索如何利用这些特征作为预训练视频扩散模型的通用条件信号。

Method: 提出轻量级架构和训练策略，将外观特征与其他需要保留的特征解耦，从而实现对风格化和重光照等外观变化的鲁棒控制。同时发现低空间分辨率可以通过更高的特征维度来补偿，提高从显式空间表示进行生成渲染的可控性。

Result: 该方法成功应用于视频域迁移和从3D生成视频等任务，能够有效控制外观变化，同时保持其他特征不变，提高了生成渲染的可控性和灵活性。

Conclusion: 通过解耦自监督学习特征中的外观信息，可以将其作为预训练视频扩散模型的通用条件信号，实现更灵活的视频生成控制，为视频域迁移和3D到视频生成等应用提供了有效解决方案。
...
TL;DR: 该论文发现大型视觉语言模型在解决需要多步骤推理的谜语问题时表现严重不足，即使具备必要的视觉和语言组件，但缺乏将它们连接起来的认知推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步骤推理的问题上存在认知差距，特别是当视觉输入仅作为线索而非答案时，如谜语问题需要整合视觉属性、文本属性和语言先验知识。

Method: 引入RebusBench基准测试，包含1,164个谜语问题，用于评估模型的神经符号能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，测试了模型扩展和上下文学习的影响。

Result: 模型表现严重不足：精确匹配率低于10%，语义准确率低于20%。模型扩展和上下文学习没有带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏将它们连接起来的认知推理能力。

Conclusion: 当前大型视觉语言模型在需要整合感知和知识的复杂推理任务上存在根本性缺陷，需要开发新的方法来增强模型的认知推理能力，而不仅仅是扩展模型规模或改进训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型主要关注2D外观或潜在表示建模，缺乏几何基础，而几何基础对于在物理世界中运行的具身系统至关重要。

Method: 使用大语言模型处理语言指令、多视角图像和动作，然后通过三个轻量级生成器分别生成深度、未来视频和动作，学习几何感知的世界表示来指导未来预测和规划。

Result: 在Navsim v1和v2基准测试中表现出色，分别达到89.2 PDMS和88.7 EPDMS，超越了现有基于世界模型的方法，同时生成更高质量的未来视频和深度预测。

Conclusion: 通过统一的几何感知世界表示框架，DriveDreamer-Policy能够产生更连贯的想象未来和更明智的驾驶动作，同时保持模块化和可控延迟，显式深度学习为视频想象提供补充优势并提高规划鲁棒性。
...
TL;DR: 提出FSKD知识蒸馏框架，通过多模态教师融合RGBI图像与LiDAR数据，训练仅使用RGBI图像的SegFormer学生模型，实现从RGBI图像预测森林结构指标（CHM、PAI、FHD），在零样本测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR成本高且不频繁，但却是森林结构监测的黄金标准。需要开发从低成本、高频次的RGBI图像中准确提取森林结构指标的方法。

Method: FSKD知识蒸馏框架：多模态教师融合RGBI图像与LiDAR衍生的平面指标和垂直剖面，通过交叉注意力机制；RGBI-only SegFormer学生模型学习复制这些输出。使用384平方公里德国森林数据训练，地面采样距离20厘米。

Result: 在8个地理上不同的测试区域实现SOTA零样本CHM性能：MedAE 4.17米，R²=0.51，IoU 0.87；MAE比基线提升29-46%（5.81米 vs 8.14-10.84米）；多模态融合比仅RGBI训练提升10-26%性能；能同时预测CHM、PAI、FHD。

Conclusion: FSKD框架有效解决了从RGBI图像预测森林结构指标的挑战，突破了严格同时采集的限制（支持冬LiDAR夏RGBI），为大规模20厘米分辨率监测（如德国数字孪生项目）提供了可扩展的解决方案。
...
TL;DR: GardenDesigner：基于智能体链和程序化建模的江南园林自动生成框架，非专家用户可通过文本输入在1分钟内创建美观的江南园林数字资产。
...
Method: 提出GardenDesigner框架，编码江南园林美学原则，集成基于程序化建模的智能体链：地形分布智能体应用水中心地形规则，道路生成智能体应用探索性路径规则，资产选择和布局优化智能体遵循美学文化约束选择布置园林元素，并引入包含专家标注知识的GardenVerse增强资产布置过程。

Result: 在Unity中开发交互界面和工具，非专家用户可通过文本输入在1分钟内构建江南园林。实验和人工评估表明，GardenDesigner能生成多样且美观的江南园林。
...
Method: 提出一致性感知的单目深度估计框架：1) 利用连续帧间的光流进行三角化，估计相机位姿和稀疏深度；2) 使用稀疏深度更新度量尺度的递归贝叶斯估计；3) 将估计的度量尺度应用于预训练的深度估计基础模型输出的相对深度，进行尺度重标定。

Result: 在KITTI、TartanAir、MS2和自建数据集上进行评估，证明了该方法能够实现鲁棒且准确的深度估计性能，特别是在保持时间一致性方面表现优异。

Conclusion: 通过结合轮式里程计和预训练深度估计模型，提出的框架能够有效解决单目深度估计中的时间一致性问题，为自动驾驶和移动机器人提供更稳定可靠的深度感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于群体的策略优化方法在处理视频问答时，当回答具有相似正确性时会出现奖励方差低的问题，导致优势估计弱或不稳定，限制了强化学习在视频推理中的效果。

Method: STRIVE构建每个输入视频的多个时空变体，在文本生成和视觉变体之间进行联合归一化；引入重要性感知采样机制，优先选择与问题最相关的帧，同时保持时间覆盖；通过结构化视觉扰动扩展群体比较范围。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上，相比现有强化学习方法，STRIVE在多个大型多模态模型上都取得了持续改进。

Conclusion: 结构化时空探索是稳定多模态强化学习和提升视频推理性能的有效机制，能够促进跨互补视觉视角的鲁棒推理，避免对单一时空配置的过拟合。
...
TL;DR: SafeRoPE：针对基于transformer的扩散模型（MMDiT）的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间，通过扰动RoPE来抑制不安全内容生成，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的文本到图像模型（如SD3、FLUX）虽然生成质量高，但对不安全语义（特别是多token交互触发）仍然脆弱。现有缓解方法主要针对U-Net架构设计，计算开销大，难以直接适配transformer-based扩散模型（如MMDiT）。

Method: 1. 分析MMDiT注意力机制，发现不安全语义集中在head级别的可解释低维子空间；2. 识别有限的安全关键head负责不安全特征提取；3. 提出SafeRoPE框架：构建head-wise不安全子空间，计算潜在风险分数（LRS），引入head-wise RoPE扰动；4. 结合LRS和RoPE扰动对query和key向量嵌入进行风险特定的head-wise旋转，精确抑制不安全输出。

Result: 实验表明SafeRoPE在平衡有效有害内容缓解和效用保持方面达到SOTA性能，能够精确抑制不安全输出同时保持生成保真度。

Conclusion: SafeRoPE为MMDiT提供了一个轻量级、细粒度的安全生成框架，通过分析注意力机制和扰动RoPE，有效解决了transformer-based扩散模型的安全生成问题，在保持图像质量的同时抑制不安全内容。
...
TL;DR: 该论文提出了一种随机标签桥接训练方法，通过添加桥接训练阶段使大语言模型参数适应视觉任务，无需人工标注，并发现部分桥接训练效果更佳。


<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例差异显著，使得跨模态（语言和视觉）适应比跨域适应更具挑战性。先前研究多关注跨域迁移而非桥接语言和视觉模态，认为语言预训练模型不适合下游视觉任务。本文挑战这一假设，探索如何有效对齐LLM参数与视觉任务。

Method: 提出随机标签桥接训练方法，作为模态适应学习器。该方法无需人工标注，通过添加桥接训练阶段帮助LLM参数适应视觉基础任务。特别强调部分桥接训练的优势，即仅对LLM的某些层进行桥接训练，保留其他层的基础特性。

Result: 研究发现桥接训练阶段能有效对齐LLM参数与视觉任务，随机标签桥接训练方法简单而强大。更重要的是发现部分桥接训练通常更有优势，因为LLM中的某些层表现出强大的基础特性，即使未经视觉任务微调也能保持有益。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中，半监督域自适应方法在处理领域偏移方面很有前景，但现有方法在严重程度分类任务中表现不佳，因为类别边界模糊且类别标签具有自然顺序，这增加了自适应难度

Method: 提出了一种新颖的方法，通过基于类别顺序的排序学习秩分数来对齐源域和目标域。具体包括：1）跨域排序（Cross-Domain Ranking）对跨域样本对进行排序；2）连续分布对齐（Continuous Distribution Alignment）对齐秩分数分布

Result: 在溃疡性结肠炎和糖尿病视网膜病变分类任务上的实验验证了该方法的有效性，成功对齐了类别特定的秩分数分布

Conclusion: 该方法通过利用类别顺序信息进行排序学习，有效解决了医学图像严重程度分类中的域自适应问题，为处理具有自然顺序标签的分类任务提供了新思路
...
TL;DR: 该论文提出了一种纹理感知的Transformer模型，用于纹理3D网格的语义分割，通过结合几何和纹理信息，在城市场景和文化遗产数据集上取得了显著性能提升。
...
Method: 提出纹理感知Transformer，直接从每个网格面关联的原始像素学习，采用新的分层学习方案进行多尺度特征聚合。包含纹理分支将面级像素汇总为可学习token，与几何描述符融合后通过两阶段Transformer块处理，支持局部和全局信息流。

Result: 在Semantic Urban Meshes基准上达到81.9% mF1和94.3% OA，在新构建的文化遗产数据集（带损坏类型标注的纹理屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 该方法通过有效结合纹理和几何信息，在纹理3D网格语义分割任务上取得了突破性进展，特别适用于城市场景和文化遗产保护等应用领域。
...
TL;DR: PI-VQ提出了一种无位置信息的离散表示方法，通过排列不变性约束使潜在编码捕获全局语义特征，支持直接图像插值，无需学习先验分布。


<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（如VQ-VAE、VQ-GAN）的离散表示具有位置依赖性，编码在空间上排列且上下文纠缠，需要自回归或扩散先验来建模依赖关系。本研究探讨位置信息是否对空间对齐数据的离散表示是必要的。

Method: 提出排列不变向量量化自编码器（PI-VQ），约束潜在编码不携带位置信息。引入匹配量化算法，基于最优二分匹配提高瓶颈容量3.5倍。利用编码的组合结构实现基于插值的采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖度指标。编码能够捕获全局语义特征，支持直接图像插值而无需学习先验。

Conclusion: 无位置表示在可分离性和可解释性方面存在权衡，但为离散表示学习提供了新方向，能够捕获全局语义特征并实现高效图像合成。
...
### [50] [HieraVid: Hierarchical Token Pruning for Fast Video Large Language Models](https://arxiv.org/abs/2604.01881)
*Yansong Guo,Chaoyang Zhu,Jiayi Ji,Jianghang Lin,Liujuan Cao*

Main category: cs.CV

TL;DR: HieraVid是一个分层剪枝框架，通过分段、帧和层级三个层次逐步动态减少视频冗余，仅保留30%的token就能达到SOTA性能，同时保持原模型98-99%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型输入token数量巨大导致计算负担重，现有方法主要在输入层面剪枝，忽视了视频和LLM内部的信息结构。视频具有分段-帧结构，LLM内部多模态信息单向传播，这些特性未被充分利用。

Method: 提出三层分层剪枝框架：1) 分段级：视频token先按时间分段并空间合并；2) 帧级：同一分段内相似帧联合剪枝以保持多样性；3) 层级：随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准上进行了广泛实验。仅保留30%的token，HieraVid实现了新的SOTA性能，同时保持了LLaVA-Video-7B 98%以上和LLaVA-OneVision-7B 99%以上的性能。

Conclusion: HieraVid通过利用视频的分段-帧结构和LLM内部信息传播特性，实现了高效的分层剪枝，显著降低了计算负担，同时保持了高性能，为视频大语言模型的部署提供了有效解决方案。
...
TL;DR: FaCT-GS是一个用于快速灵活CT重建的框架，通过优化体素化和光栅化流程，相比现有GS方法在512x512投影上快4倍，在2k投影上快13倍，并能利用现有体积数据作为先验进行快速重建。


<details>
  <summary>Details</summary>
Motivation: 虽然高斯泼溅(GS)在CT重建任务中表现出色，但其优势不足以推动从成熟重建算法的过渡。现有GS方法仍存在显著局限性，需要更快速、灵活的解决方案。

Method: 提出FaCT-GS框架，通过深入优化体素化和光栅化流程，实现显著加速。改进的体素化能够快速将高斯拟合到现有体积数据，可作为先验进行热启动重建或作为压缩表示。

Result: FaCT-GS在标准512x512投影上比现有最佳GS CT重建方法快4倍以上，在2k投影上快13倍以上，且能良好适应投影和输出体积大小的变化。

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了快速、灵活的重建框架，能够利用现有体积数据作为先验，显著提升了重建效率和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然现代视觉语言模型在语义任务上表现出色，但本研究旨在揭示它们在基本几何变换下的系统性失败，探索语义理解与空间推理之间的差距

Method: 通过系统评估方法，在不同视觉领域（符号草图、自然照片、抽象艺术）测试模型对旋转、缩放和恒等变换的鲁棒性，涵盖多种架构、模型容量和提示策略

Result: 性能随着语义内容稀疏而急剧下降，这种脆弱性在不同架构、模型容量和提示策略中普遍存在，揭示了当前视觉语言模型在空间推理方面的系统性缺陷

Conclusion: 当前视觉语言模型在语义理解和空间推理之间存在系统性差距，未来多模态系统需要更强的几何基础来提升鲁棒性
...
Method: 提出双损失训练框架：1）边界回归损失通过单通道边界预测提升时间定位精度；2）基于CDF的片段级正则化损失通过匹配预测和真实片段的累积分布来增强片段内结构一致性。该框架与架构无关，可作为训练时损失函数集成到现有TAS模型中。

Result: 在三个基准数据集上，该方法提高了片段级一致性和边界质量，在三种不同模型上均获得了更高的F1和Edit分数。帧级准确率基本保持不变。

Conclusion: 通过简单的损失设计而非复杂架构或推理时优化，即可实现精确的分割。该方法为轻量级时序动作分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型在文本到图像生成中面临两个主要挑战：1）生成的图像质量可能不符合人类期望标准；2）难以处理可能有多种有效解释的模糊提示。

Method: 提出MAR-MAER分层自回归框架，包含两个主要组件：1）度量感知嵌入正则化方法，使用轻量级投影头和自适应核回归损失函数，使模型内部表示与人类偏好的质量度量（如CLIPScore和HPSv2）对齐；2）条件变分模块，在分层标记生成过程中引入受控随机性，处理模糊语义。

Result: 在COCO和新开发的Ambiguous-Prompt Benchmark上的实验表明，MAR-MAER在度量一致性和语义灵活性方面表现出色，相比基线Hi-MAR模型，CLIPScore提升+1.6，HPSv2提升+5.3，对于模糊输入能产生更广泛的输出多样性。

Conclusion: MAR-MAER通过结合度量感知正则化和概率潜在建模，有效解决了自回归文本到图像生成中的质量控制和模糊提示处理问题，在自动指标和人类评估中都表现出优越性能。
...
TL;DR: A3R：一种基于智能体交互的3D高斯场景可操作性推理框架，通过跨维度证据获取逐步减少模糊性，相比静态一次性预测方法有显著优势


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D高斯场景中的可操作性推理视为一次性预测问题，假设静态观察已提供足够证据。但在复杂3D场景中，许多失败案例并非源于预测能力不足，而是固定观察下任务相关证据不完整。需要解决这一局限性。

Method: 将细粒度可操作性推理重新定义为顺序证据获取过程，提出A3R框架：基于MLLM的策略迭代选择证据获取动作，通过跨维度（3D几何和2D语义）证据获取更新可操作性信念。采用GRPO-based策略学习优化顺序决策。

Result: 在场景级基准测试上的广泛实验表明，A3R始终优于静态一次性基线方法，证明了智能体跨维度证据获取在复杂3D高斯场景中细粒度可操作性推理方面的优势。

Conclusion: 通过将可操作性推理重新定义为顺序证据获取过程，A3R框架有效解决了复杂3D场景中证据不完整的问题，展示了智能体跨维度证据获取对细粒度可操作性推理的重要性。
...
TL;DR: GS^2通过图基空间分布优化实现紧凑的3D高斯泼溅表示，在仅使用约12.5%高斯点的情况下获得比原始3DGS更高的PSNR，在渲染质量和内存效率上均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在实时渲染方面表现出色，但大量高斯点导致高内存成本。现有的剪枝方法虽然能减少内存占用，但会损害空间一致性并产生渲染伪影。

Method: 提出基于图的空间分布优化方法(GS^2)：1) 基于证据下界(ELBO)的自适应致密化策略，自动控制致密化过程；2) 透明度感知的渐进式剪枝策略，动态移除低透明度高斯点；3) 基于图的特征编码模块，通过特征引导的点位移调整空间分布。

Result: GS^2在仅使用约12.5%高斯点的情况下，获得了比原始3DGS更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑的高斯表示同时保持优异的渲染质量。

Conclusion: GS^2通过优化高斯点的空间分布，成功解决了3DGS的内存效率问题，在保持高质量渲染的同时显著减少了内存占用，为实际应用提供了更实用的解决方案。
...
TL;DR: 本文展示了现代文本到图像生成模型容易受到低成本的越狱攻击，仅需自然语言提示即可绕过安全过滤器，攻击成功率高达74.47%。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型被广泛部署，但现有的安全过滤器和审核管道仍存在漏洞。本研究旨在揭示这些系统对基于提示的越狱攻击的脆弱性，这些攻击不需要模型访问、优化或对抗训练。

Method: 提出了一个视觉越狱技术的分类体系，包括艺术重构、材料替换、伪教育框架、生活方式美学伪装和模糊动作替换。这些策略通过在良性语义上下文中隐藏不安全意图来利用提示审核和视觉安全过滤的弱点。在多个最先进的文本到图像系统上评估这些攻击。

Result: 简单的语言修改可以可靠地绕过现有安全防护并生成受限图像。在所有测试模型和攻击类别中，观察到攻击成功率高达74.47%。

Conclusion: 研究结果凸显了表面级提示过滤与检测生成媒体系统中对抗意图所需的语义理解之间的关键差距。当前的安全措施不足以防御基于自然语言的越狱攻击。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法主要依赖句子级别的视觉-语言对齐，难以充分利用空间关系和对象属性等细粒度语言线索。这些线索在不同定位阶段扮演不同角色，需要相应利用以提供更明确的指导。
...
Method: 使用球谐函数表示距离场，在表面内部体积中优化放置参考点，通过最大化稀疏性、中心性和表面可见性的成本函数选择位置，对每个参考点采样可见距离场并计算球谐系数，应用可配置低通滤波器和基于邻近度的局部一致性约束进行细化。

Result: SHARC在重建精度和时间效率方面优于现有最先进方法，同时保持模型简洁性。

Conclusion: SHARC框架通过球谐函数表示距离场和优化参考点放置，实现了高效准确的任意拓扑形状合成，为几何处理提供了新的有效方法。
...
TL;DR: FTPFusion是一种基于时域扰动和稀疏跨模态交互的频率感知红外与可见光视频融合方法，通过高低频分量协同建模，在保持空间细节的同时提升时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光照监测中至关重要，但现有方法要么侧重于帧级增强而时间建模有限，要么依赖沉重的时空聚合而牺牲高频细节，难以同时保持时间稳定性和空间细节。

Method: 将特征表示分解为高频和低频分量进行协同建模：高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；低频分支引入时域扰动策略以增强对复杂视频变化（如闪烁、抖动、局部错位）的鲁棒性；设计偏移感知时间一致性约束来显式稳定时域扰动下的跨帧表示。
...
TL;DR: 本文提出Light-ResKAN模型，通过将ResNet中的卷积替换为KAN卷积，使用Gram多项式作为激活函数，并采用参数共享策略，在保持SAR图像识别精度的同时大幅降低计算量和参数量，适用于边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: SAR图像识别在灾害监测、军事侦察和海洋观测中至关重要，但大尺寸SAR图像限制了深度学习在资源受限边缘设备上的部署。现有轻量级模型难以平衡高精度特征提取和低计算需求之间的矛盾。

Method: 1. 将ResNet中的卷积替换为KAN卷积，实现自适应特征提取；2. 使用适合SAR数据的Gram多项式作为激活函数，捕捉复杂非线性关系；3. 采用参数共享策略，每个通道共享参数，在保留独特特征的同时减少参数量和计算量。

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率。在1024×1024尺寸的MSTAR数据集上，相比VGG16，计算量减少82.90倍，参数量减少163.78倍。

Conclusion: Light-ResKAN模型在SAR图像识别任务中实现了精度和效率的良好平衡，为边缘设备上的SAR图像识别提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 角膜反射（光斑）检测在瞳孔-角膜反射眼动追踪中很重要，但实践中常作为启发式方法嵌入到更大的系统中，导致在不同硬件设置下难以复现。

Method: 引入基于2D几何的星座检测流程，将光斑视为结构化星座而非独立斑点。提出相似性-布局对齐（SLA）程序，将星座匹配适应到多LED眼动追踪的特定约束中。框架结合了受控过检测、自适应候选回退、外观感知评分和可选的语义布局先验，同时保持检测和对应关系的明确分离。

Result: 在公开的多LED数据集上评估，系统在噪声条件下提供了稳定的身份保持对应关系。

Conclusion: 该方法提高了多光斑检测的可重复性和透明度，通过发布代码、预设和评估脚本，支持透明复制、比较和数据集标注。
...
TL;DR: KnowMVG提出了一种知识先验和全局-局部注意力增强框架，用于提升医学视觉定位任务中空间定位的精确度，通过知识增强提示和联合注意力机制显著提升了定位性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学视觉定位任务中空间定位精度不足，主要原因是仅依赖潜在嵌入而缺乏显式的定位先验知识，导致诊断相关短语与医学图像区域对应不够精确。
...
TL;DR: 该研究探索了直接从波束成形前的天线级距离-多普勒张量学习空间结构的可行性，绕过了传统的角度域构建过程，通过端到端数据驱动方法验证了空间几何信息的可恢复性。
...
Method: 使用6发8收（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列FMCW发射方案，其中有效发射孔径在啁啾间变化。构建双啁啾共享权重编码器，在端到端数据驱动方式下处理波束成形前的天线级距离-多普勒张量。监督信号来自激光雷达，采用可见性感知和跨模态方法，通过射线可见性建模雷达视场和遮挡感知。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束成形前的天线级距离-多普勒张量学习空间结构，无需显式的角度域构建或手工信号处理阶段。不同发射配置会影响几何可恢复性。

Conclusion: 空间结构可以直接从波束成形前的天线级距离-多普勒张量中学习，绕过了传统的角度域构建过程，为汽车雷达感知提供了新的表示学习范式。
...
TL;DR: 该研究针对幼儿教育图像描述任务，构建了大规模专业数据集ECAC并提出混合训练框架RSRS，开发了KinderMM-Cap-3B模型，显著提升了专业物体识别准确性。
...
Method: 1. 构建ECAC大规模幼儿教育日常活动图像描述基准，包含256,121张真实图像，配备专家级标注和细粒度标签；2. 提出TTS（教学玩具识别分数）领域导向评估协议；3. 设计RSRS混合训练框架，动态切换强化学习和监督微调，将零奖励困难样本重新路由到监督微调；4. 基于ECAC和RSRS开发KinderMM-Cap-3B领域自适应多模态大语言模型。

Result: KinderMM-Cap-3B模型在TTS评估中达到51.06分，显著超越现有最先进基线模型，同时保持优越的描述质量，展现了在专业教育应用中的潜力。

Conclusion: ECAC基准和RSRS训练框架有效解决了幼儿教育图像描述中的数据集稀缺和训练优化问题，开发的模型在专业物体识别准确性方面取得显著提升，为专门化教育应用提供了有力工具。
...
TL;DR: 该研究提出AMIMV方法解决医学图像分类中的数据稀缺和类别不平衡问题，评估了8种自监督学习方法在11个医学影像数据集上的表现，在多个数据集上取得了性能提升。


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临两大挑战：1) 缺乏大量标注训练数据；2) 数据不平衡问题（常见类别数据丰富，罕见类别数据稀缺）。虽然自监督学习已部分解决第一个问题，但自监督学习对不平衡数据的鲁棒性在医学图像分类领域很少被研究。

Method: 1) 扩展先前提出的MIMV方法，采用新的增强策略构建非对称多图像多视图(AMIMV)对，同时解决数据稀缺和数据集不平衡问题；2) 进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性；3) 在11个医学影像数据集(MedMNIST)上评估8种代表性自监督学习方法在长尾分布和有限监督下的表现。

Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。

Conclusion: AMIMV方法能有效处理医学图像分类中的数据稀缺和类别不平衡问题，在多个医学影像数据集上表现出优于现有方法的性能，为医学影像分析提供了有效的解决方案。
...
TL;DR: MAVFusion是一个用于红外和可见光视频融合的端到端框架，通过运动感知稀疏交互机制，在保持高质量融合的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有红外和可见光视频融合方法大多针对静态图像设计，无法有效处理视频中的帧间运动。当前视频融合方法通过引入跨帧交互提高时间一致性，但计算成本较高。

Method: 提出MAVFusion框架，利用光流识别多模态序列中的动态区域，自适应地将计算密集的跨模态注意力分配到这些稀疏区域，以捕捉显著过渡并促进模态间信息交换。对于静态背景区域，采用轻量级弱交互模块保持结构和外观完整性。

Result: MAVFusion在多个红外和可见光视频基准测试中达到最先进性能，在640×480分辨率下实现14.16 FPS的速度，同时保持时间一致性和细粒度细节。

Conclusion: 通过解耦动态和静态区域的处理，MAVFusion在保持高质量融合的同时显著加速推理，为红外和可见光视频融合提供了高效解决方案。
...
TL;DR: 基于nnU-Net v2框架的深度学习模型，利用多模态mpMRI数据（T2加权、DWI、ADC）进行前列腺自动分割，在PI-CAI数据集上训练，在外部验证中表现出色。
...
Method: 使用nnU-Net v2框架开发专用深度学习模型，利用多模态mpMRI数据（T2加权、DWI、ADC），在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证。

Result: 模型在交叉验证中平均Dice得分为0.96±0.00，在外部测试集上为0.82，而通用方法（TotalSegmentator）仅得0.15分，主要由于腺体分割不足。

Conclusion: 任务特定的多模态分割策略对前列腺分割至关重要，所提方法具有强大的泛化能力，适合集成到临床研究流程中，模型已容器化并作为即用推理工具提供。
...
TL;DR: MyEgo是首个评估多模态大语言模型在自我中心视频中个性化问答能力的基准数据集，包含541个长视频和5K个关于"我的物品"、"我的活动"和"我的过去"的问题。研究发现当前领先的MLLMs在该任务上表现不佳，准确率远低于人类水平，揭示了自我定位和长期记忆能力的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在理解自我中心视频中的相机佩戴者（即"自我"）方面缺乏系统评估。为了评估模型在个性化问答中理解、记忆和推理相机佩戴者的能力，需要专门的基准数据集。

Method: 创建了MyEgo数据集，包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类。对多种MLLMs进行基准测试，包括开源与专有模型、推理与非推理模型、不同规模模型，并分析模型在显式提供证据和随时间推移的表现变化。

Result: 当前领先的闭源和开源模型（如GPT-5和Qwen3-VL）准确率分别仅为约46%和36%，远低于人类水平（差距约40%和50%）。显式推理和模型规模扩展并未带来一致改进。当提供相关证据时模型表现有所提升，但随时间推移增益下降，表明模型在追踪和记忆"我"和"我的过去"方面存在局限。

Conclusion: 研究强调了自我定位和长期记忆在自我中心视频个性化问答中的关键作用。MyEgo数据集和分析将推动该领域发展，为自我中心个性化辅助系统提供重要基准。数据和代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本的3D场景生成方法在短文本条件下存在物理合理性差和细节丰富度不足的问题，主要原因是过度依赖显式的对象组合和空间关系语义线索，缺乏足够的3D推理能力
...
Conclusion: SDesc3D通过多视角结构先验和功能感知布局推理，有效提升了短文本条件下3D场景生成的物理合理性和语义一致性
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要工作在非交互式、发射后不管的模式，无法适应需要人在回路调整的真实场景，限制了实际应用

Method: 1) 创建InteractTrack基准，包含150个带密集边界框标注和时间戳语言指令的视频；2) 提出评估协议并测试25个代表性跟踪器；3) 提出IMAT基线方法，采用动态记忆机制从用户反馈中学习并更新跟踪行为

Result: 实验表明最先进的跟踪方法在交互场景中表现失败，传统基准上的强性能无法迁移到交互场景；IMAT方法为交互式跟踪提供了有效基线

Conclusion: 该工作为开发更智能、自适应和协作的跟踪系统奠定了基础，弥合了自动感知与人类指导之间的差距，推动了交互式跟踪研究的发展
...
TL;DR: Curia-2：首个十亿参数级多模态CT/MRI基础模型，通过改进预训练策略提升放射学数据表征质量，在2D/3D基准测试中表现优异
...
Method: 基于Curia框架改进预训练策略和表征质量，首次将架构扩展到十亿参数级Vision Transformers，建立2D切片和3D体积两个独立评估轨道

Result: Curia-2在所有视觉任务上超越现有基础模型，在病灶检测等临床复杂任务上与视觉语言模型竞争性相当

Conclusion: Curia-2通过优化的预训练策略和扩展的架构规模，显著提升了放射学基础模型的性能，公开权重将促进进一步研究
...
TL;DR: 本文发现多模态大语言模型中的视觉注意力存在惯性现象，注意力在早期解码步骤固定后基本保持静态，无法支持认知推理所需的组合理解。作者提出了一种无需训练的惯性感知视觉激励方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉缓解方法主要针对物体存在或属性的感知幻觉，但对于需要对象间关系推理的认知幻觉仍然不足。作者发现视觉注意力惯性是导致认知幻觉的关键因素。

Method: 提出训练免费的惯性感知视觉激励方法，通过建模认知推理为视觉注意力的动态响应来打破惯性模式。具体包括选择相对于历史注意力趋势动态出现的视觉标记，区分表现出惯性行为的标记，并引入惯性感知惩罚来防止注意力过度集中和局部区域的持续关注。

Result: 大量实验表明，IVE方法在各种基础MLLM和多个幻觉基准测试中均有效，特别是在认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的重要根源，提出的IVE方法能够有效打破这种惯性模式，显著提升模型在认知推理任务中的表现。
...
TL;DR: Resonance4D是一个物理驱动的4D动态模拟框架，通过将3D高斯泼溅与质点法结合，使用轻量级双域运动监督，显著降低计算成本和内存占用，在单消费级GPU上实现高保真物理模拟。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的4D动态模拟方法存在矛盾：可靠的运动监督依赖计算成本高昂的在线视频扩散或光流管道，且通常只优化部分材料参数，限制了复杂材料和动态场景的真实性。

Method: 提出Resonance4D框架，结合3D高斯泼溅和质点法，引入双域运动监督（DMS）结合空间结构一致性和频域谱一致性，并使用零样本文本提示分割与模拟引导初始化实现全参数物理恢复。

Result: 在合成和真实场景实验中，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，可在单消费级GPU上运行。

Conclusion: Resonance4D通过轻量级双域监督和全参数优化，解决了物理驱动4D模拟中的计算效率和真实性问题，为复杂动态场景的高保真模拟提供了实用解决方案。
...
TL;DR: ProDiG是一个渐进式扩散引导框架，通过合成中间高度视图和几何感知的因果注意力模块，将航空3D表示逐步转换为地面级保真度，解决了从航空图像生成地面视图和3D模型的挑战。


<details>
  <summary>Details</summary>
Motivation: 从航空图像生成地面视图和3D模型面临极端视角变化、缺失中间观测和大尺度变化的挑战。现有方法要么产生几何不一致的结果，要么依赖难以获取的多高度地面真值数据。

Method: ProDiG采用渐进式扩散引导框架，包含：1）几何感知因果注意力模块，将极线结构注入参考视图扩散；2）距离自适应高斯模块，根据相机距离动态调整高斯尺度和不透明度；3）渐进式合成中间高度视图并逐步细化高斯表示。

Result: 在合成和真实数据集上的实验表明，ProDiG在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法，能够生成视觉逼真的地面渲染和连贯的3D几何。

Conclusion: ProDiG通过渐进式扩散引导框架，在不依赖额外地面真值视角的情况下，成功解决了从航空图像生成高质量地面视图和3D模型的挑战，实现了几何一致的渐进细化。
...
TL;DR: DR-Seg提出了一种解耦-校正框架，用于遥感图像开放词汇语义分割，通过分离CLIP特征为语义主导和结构主导子空间，针对性增强结构信息而不破坏语言对齐语义。


<details>
  <summary>Details</summary>
Motivation: 遥感开放词汇语义分割需要语言对齐识别和细粒度空间划分。CLIP具有强大的语义泛化能力，但其全局对齐的视觉表示难以捕捉结构细节。现有方法引入遥感预训练的DINO特征作为补偿，但将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，既不能有效划分边界，又可能破坏CLIP的语义完整性。

Method: DR-Seg基于CLIP特征通道具有功能异质性的观察，将CLIP特征解耦为语义主导和结构主导子空间，使DINO能够针对性增强结构而不扭曲语言对齐语义。随后，先验驱动图校正模块在DINO指导下注入高保真结构先验形成精炼分支，不确定性引导自适应融合模块动态集成精炼分支与原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征并针对性增强结构信息，有效解决了遥感开放词汇语义分割中语言对齐识别与细粒度空间划分的平衡问题，取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 无人机与卫星的协同空间智能对应急响应和安全操作至关重要，但现有基准局限于孤立的无人机视频或静态卫星图像，无法评估动态的局部到全局空间映射能力，这一研究空白亟待填补。

Method: 构建LinkS²Bench基准，链接1022分钟动态无人机视频与覆盖200+平方公里高分辨率卫星图像；通过LMM辅助流水线和人工标注创建17.9k高质量问答对，涵盖感知、定位、关系和推理四个维度的12个细粒度任务；设计跨视图对齐适配器来提升模型性能。

Result: 评估18个代表性VLM显示与人类基线存在显著差距，准确跨视图动态对齐是关键瓶颈；跨视图对齐适配器显著提升模型性能；微调实验证明LinkS²Bench在推进VLM适应复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估VLM在无人机-卫星协同空间智能方面的空白，揭示了跨视图动态对齐的关键挑战，提出的对齐适配器有效提升性能，为复杂空间推理的VLM发展提供了重要基准。
...
TL;DR: 针对空间非均匀采样的自编码器改进方法：通过自熵损失和样本传播机制解决空间数据不平衡问题，提升罕见空间位置的重建质量


<details>
  <summary>Details</summary>
Motivation: 在医学成像、生物学和物理学等领域，图像内容通常存在空间非均匀采样问题，信息丰富的模式在特定坐标处很少出现，而背景在这些位置占据主导，导致自编码器偏向于重建多数模式，丢失细粒度细节，对罕见空间输入产生模糊重建

Method: 提出两个互补组件：1）基于自熵的损失函数，对统计上不常见的空间位置进行加权；2）样本传播机制，在训练过程中选择性地将难以重建的样本重新暴露给模型

Result: 在模拟数据集（控制空间不平衡条件）和三个真实世界数据集（物理、生物、天文领域）上验证，方法在各种重建指标上优于基线，特别是在空间不平衡分布下表现更佳

Conclusion: 该方法通过关注统计上罕见的位置，提高了重建一致性，强调了批次中数据表示的重要性以及在无监督图像重建中关注罕见样本的必要性
...
TL;DR: IndoorCrowd是一个大规模室内人群数据集，包含31个视频、9,913帧数据，支持人体检测、实例分割和多目标跟踪任务，在四个校园场景中收集，并评估了多种基础模型和跟踪算法的性能。
...
Method: 收集了四个校园场景的31个视频（9,913帧，5fps），提供人工验证的实例分割掩码。使用620帧控制子集评估SAM3、GroundingSAM和EfficientGroundingSAM三种基础模型自动标注器，使用2,552帧子集支持多目标跟踪。建立了YOLOv8n、YOLOv26n、RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT的检测、分割和跟踪基线。

Result: 场景分析显示难度差异显著：ACS-EC场景（79.3%密集帧，平均实例尺度60.8像素）最具挑战性。评估了自动标注器与人工标签的一致性（Cohen's κ、AP、精度、召回率、掩码IoU），并提供了多目标跟踪的连续身份轨迹。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，为室内人类行为理解研究提供了重要资源，展示了不同场景下任务难度的显著差异，并评估了现代基础模型在自动标注任务中的表现。
...
TL;DR: WISE提出了一种高效推理范式，通过"思考两次"原则训练模型生成简洁推理、答案和详细解释的结构化序列，在推理时省略详细解释，实现5倍推理长度缩减的同时保持SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维推理在语言引导分割中性能显著但计算成本过高，冗长的推理过程限制了实际应用。需要一种既能保持推理能力又能大幅降低计算开销的高效方法。

Method: WISE采用"思考两次"原则：训练时生成结构化序列（简洁推理→最终答案→详细解释），通过自蒸馏目标强制简洁推理成为详细解释的充分摘要；推理时使用WISE-S策略，通过简洁指令提示激活学习到的简洁策略，省略详细解释。

Result: 在ReasonSeg基准测试中，WISE-S达到58.3 cIoU的零样本SOTA性能，同时将平均推理长度从112个token减少到23个token，实现近5倍的缩减。

Conclusion: WISE通过结构化序列训练和简洁推理策略，在保持高性能的同时大幅降低了计算成本，为大型多模态模型的高效推理提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 构建高质量的多语言和非英语视觉语言模型面临的主要障碍是其他语言的视觉问答数据集在规模和领域覆盖上都有限。日语尤其缺乏大规模、多样化的训练数据集。

Method: 收集异构源数据（图像、图像-文本对、PDF文档），通过多种策略生成视觉问答对：基于视觉语言模型的问答生成、翻译和文本渲染。

Result: 使用Jagle训练的22亿参数模型在日语任务上表现优异，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。结合Jagle和FineVision训练不仅不会降低英语性能，反而比单独使用FineVision表现更好。

Conclusion: Jagle数据集有效解决了日语视觉语言模型训练数据不足的问题，展示了通过创造性数据生成策略构建高质量多语言视觉语言模型的可行性。作者发布了数据集、训练模型和代码以促进可重复性和未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟人渲染中准确再现面部肤色对于真实感、身份保持和公平性至关重要，但现有头像创建流程缺乏色彩校准，可能导致不一致和偏见

Method: 提出全自动工作流程，整合皮肤颜色和光照提取、纹理重着色、实时渲染和定量色彩分析；使用芝加哥人脸数据库，比较基于脸颊区域采样和全脸分析的多维掩码两种皮肤色调提取策略，结合TRUST框架进行光照隔离；在CIELAB色彩空间中使用ΔE度量和个体类型学角度进行客观评估

Result: 分析了约19,848个渲染实例，结果显示提取策略的表现依赖于表型特征，较深肤色始终表现出更高的色度误差

Conclusion: 提出的方法无需人工干预，计算成本低，可实现大规模评估，揭示了虚拟人生成流程中肤色保真度的系统性偏差，特别是对较深肤色的处理问题
...
TL;DR: 论文提出多物体可供性定位新任务，解决在混淆物体对场景中根据隐含意图指令选择正确物体的问题，并构建首个基准数据集CompassAD和框架CompassNet。


<details>
  <summary>Details</summary>
Motivation: 现有3D可供性方法主要评估孤立单物体，且通常需要明确类别名称，无法处理真实场景中多个物体共享相同可供性但只有一个适合特定任务上下文的情况（混淆对）。

Method: 提出CompassNet框架，包含两个专门模块：实例边界交叉注入（ICI）将语言-几何对齐限制在物体边界内防止跨物体语义泄漏；双层对比细化（BCR）在几何组和点级别强制执行区分，增强目标与混淆表面之间的区别。

Result: 在CompassAD基准上（包含30个混淆物体对、16种可供性类型、6,422个场景、88K+查询-答案对）实现最先进性能，在已见和未见查询上均表现优异，并在机器人操作器上部署验证了在真实混淆多物体场景中的有效抓取转移。

Conclusion: 该研究正式定义了多物体可供性定位任务，构建了首个基准数据集，提出的CompassNet框架通过专门设计的模块有效解决了混淆物体对场景中的意图驱动指令理解问题，展示了在真实机器人任务中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前通用多模态嵌入方法使用显式思维链推理，但存在推理开销大、将丰富的多模态证据压缩到狭窄文本瓶颈的问题，需要更高效的推理范式

Method: PLUME采用潜在推理框架，用少量连续潜在状态的自回归展开替代显式思维链；引入语义锚点引导的转换适配器，在相同计算预算下引导不同推理轨迹；采用渐进式显式到潜在课程学习，将显式推理作为训练脚手架，逐步转移到隐藏状态计算

Result: 在78个任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，将推理从数百个生成token减少到少于10个潜在步骤，推理速度提升30倍以上，特别适合视频和视觉文档检索等密集、结构复杂的场景
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的滑块编辑方法通常依赖辅助模块和合成监督，增加了训练开销，且编辑行为受限于训练分布，在分布偏移时可靠性降低

Method: 将FlowEdit的更新分解为：(1)保真项：作为源条件稳定器，保持身份和结构；(2)转向项：驱动语义向目标编辑过渡。几何分析和经验测量显示这两项近似正交，因此仅缩放转向项即可实现稳定的强度控制

Result: FlowSlider无需后训练即可提供平滑可靠的控制，提高了跨多种任务的连续编辑质量

Conclusion: FlowSlider是一种无需训练的方法，通过正交分解编辑更新，实现了更可靠、更平滑的连续图像编辑控制
...
TL;DR: GroundVTS：一种用于视频时序定位的Vid-LLM架构，通过查询引导的细粒度视觉令牌采样机制，聚焦最信息丰富的时序片段，提升视频大语言模型的时序定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（Vid-LLMs）采用均匀帧采样，导致关键帧分布稀疏且丢失重要时序线索，限制了视频时序定位任务的性能。

Method: 提出Grounded Visual Token Sampling (GroundVTS)：1）查询引导的细粒度机制，在输入LLM前过滤视觉令牌，保留关键时空信息；2）渐进优化策略，使LLM适应视觉特征的非均匀分布。

Result: 在三个标准VTG基准测试中优于现有方法：时刻检索任务mIoU提升7.7个百分点，亮点检测任务mAP提升12.0个百分点。

Conclusion: GroundVTS通过聚焦信息丰富的时序片段，有效解决了Vid-LLMs在视频时序定位中的关键帧稀疏问题，显著提升了模型的时序依赖建模和精准定位能力。
...
TL;DR: LatentUM提出了一种新型统一模型，将所有模态表示在共享语义潜在空间中，无需像素解码作为视觉理解与生成之间的桥梁，实现了更高效灵活的跨模态推理和生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解与生成的桥梁，这种方法既低效又低效。作者希望消除像素空间的中介，实现更自然的跨模态推理和生成。

Method: 引入LatentUM模型，将所有模态表示在共享语义潜在空间中，避免了像素空间的中介，使视觉理解和生成可以直接在语义层面交互。

Result: LatentUM在视觉空间规划基准上达到最先进性能，通过自反思提升了视觉生成质量，并支持在共享语义潜在空间中预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间表示消除了像素解码的需求，提高了计算效率，减轻了编解码器偏差，增强了跨模态对齐，为实现灵活的交错跨模态推理和生成提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法大多假设固定标签空间，无法回答用户自定义查询。开放词汇变化检测允许查询任意概念，但在完全训练免费设置下，由于外观变化、弱跨概念竞争和空间连续性等因素，直接比较密集概念响应会产生噪声、碎片化和语义不可靠的变化证据。
...
Conclusion: CoRegOVCD通过一致性正则化有效解决了开放词汇变化检测中的噪声和碎片化问题，显著提升了训练免费设置下的检测性能，为遥感变化检测提供了更灵活和可靠的解决方案。
...
TL;DR: 论文发现面部动作单元检测中标准的主体排他交叉验证存在显著随机方差，提出了留一数据集交叉验证作为更稳定的评估协议


<details>
  <summary>Details</summary>
Motivation: 当前面部动作单元检测研究中，主体排他交叉验证是标准评估协议，但报告的改进往往很小。作者怀疑交叉验证本身引入了可测量的随机方差，需要更稳定的评估方法

Method: 1) 在BP4D+数据集上进行重复3折主体排他分割，测量经验噪声水平；2) 使用留一数据集交叉验证协议在五个AU数据集上评估跨数据集鲁棒性

Result: 1) 重复3折分割产生平均F1的经验噪声水平为±0.065，低流行率AU的波动更大；2) F1等操作点指标比AUC等阈值无关指标波动更大；3) 模型排名在不同折叠分配下可能改变；4) LODO协议消除了分区随机性，暴露了单数据集交叉验证不可见的域级不稳定性

Conclusion: 交叉验证中经常报告的改进可能落在协议方差范围内。留一数据集交叉验证能产生更稳定和可解释的结果，建议作为面部动作单元检测的更可靠评估协议
...
TL;DR: 该论文提出了首个大规模物体反射数据集DEROBA，并开发了一种基于基础扩散模型的反射生成方法，通过注入反射位置和外观先验信息，以及类型感知的模型设计，实现了物理一致且视觉逼真的反射生成。
...
Result: 实验证明，该方法生成的反射具有物理一致性和视觉逼真性，为反射生成建立了新的基准。

Conclusion: 该研究填补了反射生成领域的空白，通过创新的先验信息注入和类型感知设计，结合首个大规模数据集DEROBA，实现了高质量的反射生成，为该领域设立了新标准。
...
TL;DR: ViT-Explainer是一个基于Web的交互式系统，提供Vision Transformer从补丁标记化到最终分类的端到端可视化，帮助用户理解模型行为。


<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但理解这些模型在视觉设置中的工作原理仍然具有挑战性。现有可解释性工具通常关注孤立组件或专家导向分析，缺乏对完整推理流程的引导式端到端理解。

Method: 开发了ViT-Explainer系统，结合动画演示、补丁级注意力叠加和视觉适应的Logit Lens，提供引导式和自由探索两种模式的可视化。

Result: 对6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能够帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了现有可解释性工具的空白，为Vision Transformer提供了集成可视化工具，促进了模型行为的理解和解释。
...
TL;DR: UniDriveVLA通过专家解耦和混合Transformer架构解决了自动驾驶VLA模型中空间感知与语义推理的冲突，在多个任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶VLA模型面临空间感知与语义推理的困境：直接采用2D VLM空间感知有限，而增强3D表示又会损害VLM的推理能力。这种困境源于空间感知和语义推理在共享模型参数中的耦合优化。

Method: 提出UniDriveVLA统一驾驶VLA模型，基于混合Transformer架构，通过专家解耦解决感知-推理冲突。包含三个专家：驾驶理解、场景感知和动作规划，通过掩码联合注意力协调。结合稀疏感知范式和三阶段渐进训练策略，在保持语义推理能力的同时提升空间感知。

Result: 在nuScenes开环评估和Bench2Drive闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛感知、预测和理解任务上表现出色，展示了作为统一自动驾驶模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构有效解决了VLA模型中空间感知与语义推理的冲突，为自动驾驶提供了一个统一且高性能的解决方案。
...
Method: 将文本、图像和3D表示为共享序列空间中的离散token，采用交错X-to-X训练范式，在异构配对数据集上协调多种跨模态任务，通过语义-视觉-几何循环（如文本→图像→3D→图像）在自回归序列中联合执行语义对齐、外观保真和多视角几何一致性。

Result: 实验表明Omni123显著改善了文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: Omni123通过统一2D和3D生成框架，利用2D数据作为几何先验，解决了3D数据稀缺问题，为构建多模态3D世界模型提供了有前景的方向。
...
TL;DR: SCALE是一个轻量级确定性框架，通过语义和置信度感知的列表能量排序解决零样本骨架动作识别问题，无需显式的骨架-文本对齐。
...
Method: 提出SCALE框架：1) 构建文本条件变分自编码器，冻结文本表示参数化先验和解码器；2) 引入语义和置信度感知列表能量损失，强调语义相似的困难负样本并纳入后验不确定性；3) 使用潜在原型对比目标对齐后验均值与文本派生原型。

Result: 在NTU-60和NTU-120数据集上，SCALE持续优于基于VAE和对齐的基线方法，并与基于扩散的方法保持竞争力。

Conclusion: SCALE通过类条件能量排序框架，避免了脆弱的显式对齐，利用语义相似性和后验不确定性改进决策边界，实现了更稳健的零样本骨架动作识别。
...
TL;DR: VOID是一个视频对象移除框架，专注于处理具有物理交互的对象移除场景，通过生成物理一致的替代结果来解决现有方法在复杂物理交互场景下的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长处理对象"背后"的内容修复和外观层面的伪影（如阴影、反射），但当移除的对象与其他物体有更显著的物理交互（如碰撞）时，现有模型无法修正这些交互，产生不合理的物理结果。

Method: 1. 使用Kubric和HUMOTO生成包含反事实对象移除的配对数据集，其中移除对象需要改变下游物理交互；2. 推理时使用视觉语言模型识别受移除对象影响的场景区域；3. 使用视频扩散模型生成物理一致的反事实结果。

Result: 在合成数据和真实数据上的实验表明，与现有视频对象移除方法相比，VOID方法在对象移除后能更好地保持一致的场景动态，产生更物理合理的结果。

Conclusion: VOID框架通过高层次因果推理，使视频编辑模型能够更好地模拟世界物理规律，为处理复杂物理交互场景下的对象移除提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，特别是密集预测场景如开放词汇分割。现有方法通常使用滑动窗口策略处理高分辨率图像，虽然提高了准确性但计算成本很高。

Method: 提出SPAR（Single-Pass Any-Resolution ViT），通过特征回归损失将细步长滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中，无需架构更改或像素级监督。
...
TL;DR: 提出可操控视觉表示方法，通过早期融合文本提示来引导视觉特征关注图像中任意感兴趣对象，同时保持基础表示质量


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）专注于最显著视觉线索，无法引导关注次要概念；而多模态LLM虽然可文本引导，但特征偏向语言中心，在通用视觉任务中效果下降。需要一种既能文本引导又能保持视觉表示质量的方法

Method: 提出可操控视觉表示方法，通过轻量级交叉注意力在视觉编码器层中早期融合文本（而非像CLIP那样的后期融合），使全局和局部特征都能被自然语言引导

Result: 方法能引导视觉特征关注图像中任意所需对象，同时保持底层表示质量；在异常检测和个性化对象识别任务上匹配或优于专用方法，并展示对分布外任务的零样本泛化能力

Conclusion: 提出了一种新的可操控视觉表示类别，通过早期文本融合实现了视觉特征的灵活引导，在保持表示质量的同时扩展了视觉表示的应用范围
...
<details>
  <summary>Details</summary>
Motivation: 现有安全方法通常依赖模型微调或精选数据集，这会降低生成质量或限制可扩展性，需要一种更灵活、训练免费的安全控制方法

Method: 利用视觉语言基础模型作为现成的监督信号，通过在每个采样步骤注入干净潜在估计的梯度反馈，将安全引导制定为基于能量的采样问题
...
TL;DR: ActionParty是一个用于生成式视频游戏的动作可控多主体世界模型，解决了现有视频扩散模型中动作绑定问题，能够同时控制多达7个玩家
...
Result: 在Melting Pot基准测试中评估，展示了首个能够同时在46个多样化环境中控制多达7个玩家的视频世界模型，在动作跟随准确性和身份一致性方面有显著改进，并支持复杂交互中主体的鲁棒自回归跟踪

Conclusion: ActionParty成功解决了多主体视频生成中的动作绑定问题，为生成式视频游戏开发了首个能够同时控制多个智能体的世界模型，在动作控制和身份一致性方面表现优异
...
TL;DR: 提出了一种基于概率度量表示和切片Wasserstein距离的快速表面配准方法，使用AdamFlow优化算法在概率空间中进行配准，实现了高效且鲁棒的表面配准。
...
Method: 将表面网格表示为概率度量，将表面配准转化为分布优化问题。使用计算复杂度为对数线性的切片Wasserstein距离度量网格间差异。提出AdamFlow优化方法，将Adam优化从欧几里得空间推广到概率空间，用于最小化切片Wasserstein距离。

Result: 理论上分析了AdamFlow的渐近收敛性。实证表明该方法在各种解剖结构的仿射和非刚性表面配准中均表现出优越性能，实现了高效且鲁棒的配准。

Conclusion: 提出的基于概率度量和切片Wasserstein距离的表面配准方法，结合AdamFlow优化算法，成功解决了现有方法在效率和鲁棒性之间的权衡问题，为医学图像中的解剖形状分析提供了有效的解决方案。
...
Method: 采用预训练/后训练范式：首先在100万个野外视频上进行预训练，学习外观和几何的广泛先验；然后在高质量策划数据上进行后训练，增强表现力和保真度。模型采用前馈方式实现高效推理。

Result: LCA能够泛化到不同发型、服装和人口统计特征，同时提供精细的面部表情和手指级关节控制，保持强身份保留。模型表现出对重新光照和宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性。
...
TL;DR: 论文提出了场景化视觉定位新任务和RSC基准，通过段落描述而非简单指称表达来定位目标，挑战现有模型，并提出了课程学习方法ScenGround。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型可以通过匹配显式类别名称获得成功。本文探索更具挑战性的场景化视觉定位，要求从角色、意图和关系上下文中推断目标，而非依赖显式命名。

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度的查询文本，描述对象角色、用户目标和上下文线索，包括故意引用干扰对象。提出ScenGround方法，结合监督预热和难度感知强化学习的课程推理方法。

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外样本。实验表明场景化查询暴露了当前模型在标准基准中未显现的系统性失败，课程训练提高了在挑战性切片上的性能，并能迁移到标准基准。

Conclusion: 场景化视觉定位是一个比传统指称表达更具挑战性的任务，RSC基准支持细粒度分析模型失败模式，提出的课程学习方法为这一新设置提供了参考点，并展示了向标准基准的迁移能力。
...
TL;DR: ModMap是一个原生多视角多模态的3D异常检测与分割框架，通过跨模态特征映射和视角相关特征调制，在SiM3D基准上取得了最先进的性能。
...
Method: 1. 采用跨模态特征映射范式，学习跨模态和跨视角的特征映射；2. 通过特征调制显式建模视角依赖关系；3. 引入跨视角训练策略，利用所有可能的视角组合；4. 训练并公开了针对工业数据集的基础深度编码器。

Result: 在SiM3D基准测试中，ModMap取得了最先进的性能，大幅超越了先前的方法。该基准是首个用于3D异常检测和分割的多视角多模态设置。

Conclusion: ModMap通过原生多视角多模态设计、跨模态特征映射和视角相关调制，为3D异常检测提供了有效的解决方案，并在基准测试中表现出色。
...
TL;DR: 该研究通过从AAA游戏中提取大规模动态数据集，解决了生成式逆向渲染和正向渲染在真实世界场景中的领域差距问题，并提出了基于VLM的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集的真实性和时间连贯性限制了生成式逆向渲染和正向渲染在真实世界场景中的扩展，存在持续的领域差距问题。

Method: 采用新颖的双屏拼接捕获方法，从视觉复杂的AAA游戏中提取了400万连续帧的同步RGB和五个G-buffer通道数据，涵盖多样场景、视觉效果和环境条件。同时提出了基于视觉语言模型（VLM）的评估协议来评估语义、空间和时间一致性。

Result: 在该数据集上微调的逆向渲染器实现了卓越的跨数据集泛化能力和可控生成，VLM评估结果与人类判断高度相关。正向渲染器工具包允许用户通过文本提示从G-buffer编辑AAA游戏的风格。

Conclusion: 该大规模动态数据集显著推进了双向渲染技术，解决了真实世界渲染的领域差距问题，同时提出的VLM评估协议为无地面真值的逆向渲染评估提供了有效方法。
...
TL;DR: 该论文研究了在线交流中常见的重复延长形式（RLF）在情感分析中的重要性，创建了首个RLF数据集Lengthening，并提出了ExpInstruct框架来提升LLMs对RLF的理解和解释能力。
...
Method: 1）基于先前语言学研究，创建了首个多领域RLF数据集Lengthening（85万样本）；2）提出了ExpInstruct两阶段指令调优框架，提升LLMs对RLF的性能和可解释性；3）提出统一方法来量化语言模型对非正式表达的理解。

Result: 1）RLF句子是富有表现力的表达，可作为文档级情感的特征；2）RLF对在线内容分析具有潜在价值；3）微调的预训练语言模型在性能上能超越零样本GPT-4，但在解释性上不及；4）ExpInstruct能让开源LLMs在有限样本下达到零样本GPT-4的性能和解释性水平。

Conclusion: RLF在情感分析中具有重要性，语言模型需要专门优化才能更好地理解和解释这种非正式表达。ExpInstruct框架为提升LLMs对RLF的理解提供了有效方法，有助于在线内容分析应用。
...
TL;DR: 该研究通过训练时强化学习和测试时并行思考两种方法，扩展竞争性编程中的推理令牌预算，实现了在大量令牌消耗下达到高准确率的目标。


<details>
  <summary>Details</summary>
Motivation: 解决竞争性编程中单次生成推理在强化学习训练下快速变得昂贵的问题，需要更高效地利用推理令牌预算来提高模型性能。

Method: 采用两种互补方法：1) 训练时强化学习，包括验证RL预热和随机裁剪技术；2) 测试时多轮并行思考管道，将令牌预算分配到多个线程和轮次的生成、验证和精炼中。

Result: 基于Seed-OSS-36B模型，完整系统使用16个线程和每线程16轮，平均每个问题消耗760万令牌，在pass@1指标上匹配了底层RL模型的oracle pass@16性能，并在AetherCode的456个困难竞争性编程问题上超越了GPT-5-high。

Conclusion: 通过结合训练时强化学习和测试时并行思考，能够有效扩展推理令牌预算，在竞争性编程任务中实现显著的性能提升，为大规模推理任务提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏足够的规模、领域多样性和视觉复杂性，无法真实评估科学主张与多模态证据之间的严格一致性，需要一个新的数据集来解决这一差距。

Method: 从PubMed和arXiv获取数据，构建包含超过46.9万个实例的M2-Verify数据集，覆盖16个领域，并通过专家审核进行严格验证。

Result: 最先进的模型在保持一致性方面表现不佳：在低复杂度医学扰动上达到85.8%的Micro-F1，但在高复杂度挑战（如解剖结构变化）上性能降至61.6%。专家评估还发现模型在生成科学解释时存在幻觉问题。

Conclusion: M2-Verify数据集填补了现有基准测试的不足，展示了多模态科学一致性评估的重要性，提供了数据集的使用指南，并揭示了当前模型在处理复杂视觉科学推理方面的局限性。
...
TL;DR: 该研究提出了一种特征增强框架，通过添加可解释信号（如回复长度、拒绝指标、毒性分数等）来改进语言模型对人类偏好的学习，在Anthropic HHRLHF数据集上显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型学习人类偏好存在根本性挑战，因为奖励建模依赖于微妙、主观的比较而非清晰标签。现有方法在标准成对偏好设置下性能有限（ROC AUC低于0.74），需要更好地捕捉人类判断的多维性。

Method: 提出混合特征增强框架，在文本表示基础上添加可解释信号：回复长度、拒绝指标、毒性分数和提示-回复语义相似度。使用Anthropic HHRLHF数据集评估10个大型语言模型，采用SHAP和LIME进行细粒度可解释性分析。

Result: 特征增强方法在所有模型上都带来一致改进，最高达到0.84 ROC AUC，显著提高成对偏好准确率。DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化安全性和支持性框架而非孤立关键词。

Conclusion: 通过整合结构化特征和可解释性分析，可以显著改善语言模型对人类偏好的学习，同时揭示了偏见放大机制：单个特征边际效应弱，但特征间交互影响偏好学习。
...
TL;DR: Reasoning Memory：一个检索增强生成框架，通过分解推理轨迹为子问题-子程序对，在推理时检索并重用程序性知识，显著提升语言模型在数学、科学和编程任务上的表现。
...
Method: 将现有逐步推理轨迹分解为自包含的子问题-子程序对，构建包含3200万个程序性知识条目的数据存储。推理时通过轻量级思维提示让模型表达核心子问题，检索相关子程序作为隐式程序先验。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于使用文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线。在高推理预算下，比无检索方法提升19.2%，比最强计算匹配基线提升7.9%。

Conclusion: Reasoning Memory通过广泛覆盖源轨迹的程序性知识以及分解和检索设计，实现了程序性知识的有效提取和重用，显著提升了语言模型的推理能力。
...
TL;DR: 研究发现语言模型通过稀疏的、可因果干预的MLP神经元来检索实体知识，这些神经元集中在早期层，能够实现紧凑的实体检索而非渐进式知识积累。


<details>
  <summary>Details</summary>
Motivation: 虽然语言模型能够回答许多以实体为中心的事实性问题，但其内部机制尚不清楚。本研究旨在探究语言模型在实体知识检索过程中的内部工作机制。

Method: 使用模板化提示定位实体选择性MLP神经元，在PopQA数据集上进行因果干预验证。从PopQA中选取200个实体，通过负向消融和受控注入实验分析神经元功能。

Result: 定位到的神经元集中在早期层；负向消融导致实体特异性遗忘，受控注入能改善答案检索；单个神经元激活即可恢复实体一致性预测；对别名、缩写、拼写错误和多语言形式具有鲁棒性。

Conclusion: 研究发现了稀疏、可因果干预的访问点，可用于分析和调节实体条件化的事实行为，支持规范化解释而非纯粹渐进式知识积累。
...
Result: 通过比较不同方法，验证了新提出的生产单元断点计算方法的有效性

Conclusion: 新方法为翻译过程研究提供了更可靠的停顿阈值确定工具，有助于更准确地识别翻译中的认知处理过程
...
TL;DR: 本文提出了MiCP框架，为多轮推理的LLM提供符合性预测，解决何时停止的问题，在保证覆盖度的同时减少轮次和成本。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在多轮推理（如自适应RAG和ReAct）中缺乏正式的停止标准，现有方法依赖启发式规则或固定轮次预算，无法保证最终预测包含正确答案，这在金融、医疗等高风险领域尤为危险。

Method: 提出Multi-Turn Language Models with Conformal Prediction (MiCP)框架，将不同的错误预算分配到各轮次中，使模型能够提前停止同时保持整体覆盖度保证。

Result: 在自适应RAG和ReAct上验证MiCP，在单跳和多跳问答基准测试中达到目标覆盖度，同时减少了轮次数量、推理成本和预测集大小。

Conclusion: MiCP是首个用于多轮推理的符合性预测框架，解决了LLM多轮交互中的停止问题，在保证覆盖度有效性的同时提高了回答效率。
...
TL;DR: 本文提出了基于预测效度的基准测试框架质量评估方法，通过收集大规模项目级数据集WILD，结合改进的多维项目反应理论和自适应项目选择，能够高效预测模型在未见任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型基准测试虽然多样，但研究表明模型性能往往由少量潜在能力因素解释。当前缺乏系统方法来比较不同基准测试框架的质量，需要更高效和原则性的基准测试方法。

Method: 收集了WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务上的109,564个独特项目响应。采用改进的多维项目反应理论模型，结合基于最优实验设计的自适应项目选择方法，并引入成本感知折扣因子来优化评估效率。

Result: 该方法能够在仅观察16个项目后，以小于7%的平均绝对误差预测112个保留基准任务的性能。通过成本感知优化，将达到7%MAE所需的总token数从141,000减少到22,000，降低了85%的评估成本。

Conclusion: 基于预测效度的基准测试框架能够显著提高大语言模型评估的效率，通过自适应项目选择和成本优化，在保持预测准确性的同时大幅降低评估成本，为实现更高效的原则性基准测试提供了可行方案。
...
TL;DR: 使用词嵌入和随机森林模型，研究发现印地语中梵语和波斯-阿拉伯语来源的同义词在分布模式上存在可区分的差异，即使语义无关，上下文也能编码词源信号


<details>
  <summary>Details</summary>
Motivation: 研究同义词现象，特别是印地语中梵语和波斯-阿拉伯语来源的同义词对，探索是否仅通过分布数据就能区分词源，验证同义词是否反映不同的视角或文化关联

Method: 使用随机森林模型训练印地语同义词的词嵌入，根据分布模式分类词语的梵语或波斯-阿拉伯语来源，不考虑语义内容

Result: 随机森林模型成功区分了梵语和波斯-阿拉伯语来源的同义词，即使语义无关，表明使用模式保留了词源痕迹，上下文编码了词源信号

Conclusion: 同义词可能反映与起源相关的微妙但系统性的区别，词源相关的词语可能形成不同的概念子空间，上下文能够捕捉超越传统语义相似性的细微区别
...
TL;DR: 研究发现引用粒度对模型性能有显著影响，句子级细粒度引用会降低归因质量16-276%，段落级中等粒度表现最佳，引用粒度优化能同时提升归因质量和答案正确性。


<details>
  <summary>Details</summary>
Motivation: 在归因生成中，引用粒度（句子级、段落级或文档级）是关键设计选择。虽然细粒度引用通常更便于人工验证，但其对模型性能的影响尚未充分研究。本研究旨在探索不同引用粒度对模型归因质量的影响。

Method: 分析了四种不同规模的模型（8B-120B），比较了不同引用粒度（句子级、段落级、多段落级）对归因质量的影响。研究了细粒度引用约束如何影响模型的语义依赖关系和证据归因能力。

Result: 1. 强制细粒度引用使归因质量下降16-276%；2. 段落级中等粒度表现最佳；3. 细粒度引用破坏了证据归因所需的语义依赖关系；4. 过粗的引用引入干扰噪声；5. 性能差距随模型规模非单调变化，细粒度约束对更大模型影响更严重；6. 最优引用粒度能显著提升归因质量，同时保持或改善答案正确性。

Conclusion: 单纯为人工验证优化细粒度引用会忽视模型约束，损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐，段落级引用在模型性能和人类可验证性之间取得了最佳平衡。
...
TL;DR: 论文发现大语言模型存在"过度自信"问题，即产生事实错误答案时仍表现出过高信心，并通过电路级机制分析识别了导致这种过度自信的内部回路，提出了针对性干预方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常表现出"过度自信"问题：当它们产生事实错误的答案时，往往会表达过高的信心而非表示不确定性。这种言语化的过度自信会误导用户，削弱置信度分数作为可靠不确定性信号的效用，但其内部机制尚不清楚。

Method: 采用电路级机制分析方法，围绕三个轴心展开：1) 将言语化信心捕获为可微分的内部信号；2) 识别导致信心膨胀的因果回路；3) 利用这些见解进行有针对性的推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 研究发现，一组紧凑的MLP块和注意力头（集中在中后层）持续在最终标记位置写入信心膨胀信号。进一步的推理时针对性干预显著改善了校准效果。

Conclusion: LLM中的言语化过度自信是由可识别的内部回路驱动的，可以通过针对性干预来缓解。这为理解模型不确定性表达机制和改善校准提供了新见解。
...
TL;DR: 该研究通过计算分析波斯诗歌中的象征系统，发现波斯诗歌象征不是固定不变的，而是一个长期演变的系统，其内部权重和连接随时间变化。


<details>
  <summary>Details</summary>
Motivation: 波斯诗歌通常通过反复出现的象征（如酒器、花园、火焰等）被记忆，但现有的计算方法倾向于将这些材料简化为孤立的词语或宽泛的文档语义，忽略了波斯诗学中一个实用的组织单位：相关形式以家族形式传播，并通过反复出现的关系获得力量。

Method: 使用包含129,451首诗歌的语料库，将反复出现的形式整合为可追踪的家族，将意象材料与神圣和宫廷参照分开，并在多层图中映射它们的关系。分析跨越11个伊斯兰世纪的时间段。

Result: 象征核心相对稀疏，参照成分更密集，两者之间的连接区域是选择性的而非扩散的。某些家族（如夜、日、土）广泛分布，酒器、花园空间、火焰和抒情声音在后期增强，而声望编码和英雄宫廷词汇在早期更突出。世纪特定的图显示排列和成员的变化：模块性增加，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。枢纽位置也发生变化：苏菲长袍获得晚期突出地位，祝福和紫罗兰消退，而酒杯在整个时间线中保持中心地位。
...
TL;DR: 论文提出了Magic, Madness, Heaven, Sin框架，将大语言模型的输出变化建模为同质-异质轴，根据任务规范目标进行价值判断，并分析了四个规范语境及其交叉影响。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于"多样性"的术语碎片化，缺乏明确的任务规范目标框架，需要系统化分析输出变化在不同任务语境下的价值判断。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴，组织任务到四个规范语境：认知性（事实性）、交互性（用户效用）、社会性（代表性）、安全性（鲁棒性），并分析跨语境交互。

Result: 框架揭示了优化一个目标（如安全性）可能无意中损害人口统计代表性或创造性多样性，强调了输出变化应根据任务目标而非模型内在特性进行评估。

Conclusion: 需要基于任务目标的上下文感知评估输出变化，将其重新定义为由任务目标塑造的属性，而非模型的固有特征。
...
<details>
  <summary>Details</summary>
Motivation: 研究指令遗忘范式是否适用于扩散模型，探索扩散模型能否通过自然语言指令有效抑制特定概念生成
...
TL;DR: 提出模型融合框架，通过合并临床基础模型和通用指令模型来缓解LLM在医学领域微调时的灾难性遗忘问题，在少量样本下达到与全微调相当的性能。
...
Method: 提出模型融合框架，通过基于插值的融合方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，以获得既保留临床专业知识又保持指令跟随能力的领域适应模型。

Result: 综合评估显示，融合模型能有效缓解灾难性遗忘，保留临床领域专业知识，同时保持指令跟随能力。在严重受限的监督下(如64-shot vs 256-shot)，融合策略能达到与全微调基线相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有角色中心记忆系统在多智能体设置中存在信息丢失和场景适应性差的问题，导致性能不佳。需要更稳健的记忆管理方法。

Method: 1. 提出DeltaMem单智能体记忆管理系统，将角色记忆管理作为端到端任务；2. 构建用户-助手对话数据集及操作级记忆更新标签；3. 引入基于记忆的Levenshtein距离形式化记忆更新奖励；4. 设计定制强化学习框架提升管理能力

Result: 在LoCoMo、HaluMem和PersonaMem等多个长期记忆基准测试中，DeltaMem（包括无训练和RL训练版本）均超越了所有产品级基线方法

Conclusion: DeltaMem通过单智能体端到端设计和强化学习优化，有效解决了现有角色记忆管理系统的信息丢失和脆弱性问题，在长期记忆任务中表现出色
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，通过单次特征值分解实现训练无关、快速且理论最优的层间低秩近似，在保持最优压缩精度的同时提供3-70倍的端到端压缩加速。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的部署受到静态权重和动态键值缓存的内存和带宽需求的限制。虽然SVD压缩提供了硬件友好的解决方案，但现有方法存在两个关键局限：一些方法在重构误差上不是最优的，而另一些方法理论上最优但实际效率低下。

Method: 提出Swift-SVD框架，通过增量聚合给定输入批次输出激活的协方差，并在聚合后执行单次特征值分解，实现训练无关、快速且最优的层间低秩近似。使用有效秩分析局部层间可压缩性，并设计动态秩分配策略，同时考虑局部重构损失和端到端层重要性。

Result: 在六个LLM和八个数据集上的广泛实验表明，Swift-SVD优于最先进的基线方法，在保持最优压缩精度的同时，端到端压缩时间实现了3-70倍的加速。

Conclusion: Swift-SVD是一个同时保证理论最优、实际高效和数值稳定的激活感知闭式压缩框架，为大型语言模型的部署提供了有效的内存和带宽优化解决方案。
...
TL;DR: 印尼全国349名K-12教师调查显示：AI在教学中使用增加但分布不均，主要用于减轻教学准备负担，但仍面临通用输出、基础设施不足和情境适配有限等挑战
...
Method: 对349名K-12教师（涵盖小学、初中、高中）进行全国性调查，收集关于AI使用情况、需求和挑战的数据

Result: 发现AI在教学法、内容开发和教学媒体方面的使用增加，但采用不均衡：小学教师使用更一致，高中教师参与较少；中年教师更重视AI，印尼东部教师认为AI价值更大；教师主要用AI减轻教学准备负担，但通用输出、基础设施限制和情境适配问题阻碍有效课堂整合
...
TL;DR: 研究发现当前声明验证基准主要测试证据提取能力，而多句合成和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对这些基准实际测试推理能力的系统性理解，需要了解当前验证系统真正测试的是什么能力

Method: 使用GPT-4o-mini为9个数据集的24K个声明验证示例生成结构化推理轨迹，并使用紧凑的1B参数推理验证器分析五种错误类型

Result: 发现直接证据提取占主导地位，多句合成和数值推理严重不足；不同数据集存在明显偏见；错误类型因领域而异：通用领域主要是词汇重叠偏见，科学领域是过度谨慎，数学领域是算术推理失败

Conclusion: 高基准分数主要反映检索+蕴含能力，而非复杂推理能力；需要构建更具挑战性的评估套件来更好地测试验证系统所需的推理能力
...
Conclusion: PRCCF框架通过结合人物引导检索和因果感知过滤，有效提升了情感支持对话中的上下文认知理解和情感推理能力
...
TL;DR: 研究发现监督微调时，不同来源的思维链轨迹对模型泛化性能有显著影响。DeepSeek-R1-0528数据训练损失更低但泛化更差，而gpt-oss-120b数据训练损失更高但泛化更好，揭示了训练损失与泛化性能的悖论。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索不同来源的思维链轨迹如何影响大语言模型的泛化性能。虽然监督微调在构建推理模型中很重要，但不同来源的思维链轨迹对模型泛化能力的影响机制尚不清楚。

Method: 使用DeepSeek-R1-0528和gpt-oss-120b生成的思维链轨迹进行对比研究，控制问题集相同。通过多角度分析包括token级训练损失和步骤级推理行为，发现推理模式差异。提出通过过滤频繁分支轨迹来改进泛化的简单方法。

Result: 研究发现DeepSeek-R1-0528数据训练损失更低但泛化性能更差，而gpt-oss-120b数据训练损失更高但泛化更好。分析显示gpt-oss-120b采用收敛的演绎推理模式，而DeepSeek-R1-0528偏好发散的分支探索模式。过滤分支轨迹后，DeepSeek-R1-0528子集的推理性能显著提升。

Conclusion: 研究揭示了思维链轨迹的推理模式对模型泛化性能的重要影响。收敛的演绎推理模式比发散的分支探索模式更有利于泛化。通过过滤频繁分支轨迹可以显著改善模型泛化能力，为监督微调的数据选择提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 胃肠内窥镜中自动语音识别（ASR）是人机交互的关键接口，但在真实临床环境中，由于领域特定术语和复杂声学条件的限制，其可靠性受到挑战。

Method: 开发了基于合成内窥镜报告的两阶段适应策略，针对领域特定语言建模和噪声鲁棒性进行优化，设计了EndoASR系统用于实时部署在内窥镜工作流程中。

Result: 在回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%。前瞻性多中心研究中，字符错误率从16.20%降至14.97%，医学术语准确率从61.63%提升至84.16%。实时因子为0.005，模型参数仅220M，适合边缘部署。

Conclusion: 领域自适应的ASR可以作为胃肠内窥镜中人机协作的可靠接口，在多中心真实临床环境中验证了其一致性能，改进的ASR质量直接增强了下游结构化信息提取和临床医生-AI交互。
...
TL;DR: 本文系统综述了LLM智能体中的记忆方法，提出了统一框架，在标准基准上全面比较了代表性方法，并设计出超越现有SOTA的新记忆方法，最后指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长时复杂任务（如多轮对话、游戏、科学发现）的核心模块，能够实现知识积累、迭代推理和自我进化。虽然已有多种记忆方法被提出，但缺乏在相同实验设置下的系统全面比较。

Method: 1. 从高层视角总结了一个包含所有现有智能体记忆方法的统一框架；2. 在两个知名基准上广泛比较代表性智能体记忆方法；3. 通过分析现有方法模块设计了一个新的记忆方法。

Result: 1. 提供了对所有记忆方法有效性的全面分析；2. 设计的新记忆方法超越了现有最先进方法；3. 基于研究发现提出了有前景的未来研究方向。

Conclusion: 对现有方法行为的深入理解能为未来研究提供有价值的新见解，系统比较和统一框架有助于推动LLM智能体记忆模块的发展。
...
TL;DR: 提出人机协作框架，结合LLM推理与声学特征模型，通过置信度路由机制处理模糊样本，在越南语语音情感识别中达到86.59%准确率


<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临声学模式模糊和标注数据缺乏的挑战，特别是在现实场景中情感边界不清晰。传统数据驱动模型难以处理这些模糊样本，需要结合人类知识

Method: 提出人机协作框架：1) 使用声学特征模型提供置信度和特征级证据；2) 置信度路由机制区分简单和模糊样本；3) 模糊样本交由LLM进行基于结构化规则的深度推理；4) 采用迭代优化策略通过错误分析和规则更新持续改进系统

Result: 在包含2,764个样本的越南语语音数据集（三个情感类别：平静、愤怒、恐慌）上实验，标注者间一致性高（Fleiss Kappa = 0.8574）。方法达到86.59%准确率和0.85-0.86的Macro F1分数，有效处理模糊和难分类样本
...
TL;DR: 本文提出了一种更细致的保加利亚语毒性内容检测方法，通过构建本体论和数据集，训练BERT模型实现毒性分类，同时避免误伤医学术语和少数群体相关文本。


<details>
  <summary>Details</summary>
Motivation: 当前在线毒性内容检测方案经常误将医学术语和少数群体相关文本等有价值信息错误地屏蔽，这在实际应用中存在问题。因此需要开发一种更细致的方法来识别保加利亚语中的毒性内容，同时保留对重要信息的访问。

Method: 1. 提出一个建模保加利亚语潜在毒性词汇的本体论；2. 构建包含4,384条手动标注句子的数据集，涵盖四个类别：毒性语言、医学术语、非毒性语言、少数群体相关术语；3. 训练基于BERT的毒性语言分类模型。

Result: 训练的BERT模型在毒性语言分类任务上达到了0.89的宏观F1分数。该模型可直接应用于实际环境，并可作为毒性内容检测系统的组件集成。

Conclusion: 本研究开发了一种更细致的保加利亚语毒性内容检测方法，通过本体论建模和BERT模型训练，实现了高精度的毒性识别，同时避免了对医学术语和少数群体相关文本的误判，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准测试存在合成设置和数据污染问题，无法真实评估LLMs的研究级数学能力。随着LLMs越来越多地融入科学工作流程，需要建立更严谨、能反映真实数学推理能力的评估标准。

Method: 1) 使用模型训练截止后发布的arXiv论文构建动态选择题基准；2) 建立13类定理逻辑分类体系；3) 采用证明草图引导的干扰项生成管道，基于高层次证明策略构建似是而非的错误选项；4) 引入抗替换机制区分答案识别与实质推理；5) 设计双模式协议评估证明草图访问的影响。

Result: 最佳模型Gemini-3.1-pro-preview仅达到43.5%准确率。在抗替换评估下，GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问能带来一致的准确率提升，表明模型可以利用高层次证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的测试平台，用于研究LLMs的研究级数学推理能力。当前模型表现远未饱和，且容易受到表面模式影响，表明需要更深入的数学理解能力。
...
TL;DR: 本文提出基于指令微调和离散控制标记的领域无关可控自动文本简化框架，发现小模型也能有竞争力，但可靠可控性取决于训练数据中目标属性的变化程度，并指出现有评估指标不足以衡量控制效果。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化（CATS）通常被视为解码问题，且评估指标不能有效衡量控制效果。研究发现可控性在ATS中受到数据和评估方法的显著限制，需要更好的框架和评估方法。

Method: 提出基于指令微调和离散控制标记的领域无关CATS框架，使用三个模型家族（Llama、Mistral、Qwen，1-14B参数）在四个领域（医学、公共管理、新闻、百科全书文本）进行实验，通过控制标记指导模型达到目标可读性水平和压缩率。

Result: 小模型（1-3B）表现具有竞争力，但可靠可控性取决于训练数据是否包含足够的目标属性变化。可读性控制（FKGL、ARI、Dale-Chall）学习一致，而压缩控制因现有语料库中信号变化有限而表现不佳。标准简化和平行度指标不足以衡量控制效果。

Conclusion: 需要基于误差的度量来衡量目标-输出对齐，朴素的数据分割可能引入分布不匹配问题，影响训练和评估。训练数据的属性变化程度对可控性学习至关重要。
...
Method: 提出DEFT框架，基于语言模型输出分布和偏好数据差异分布计算差异分布奖励，筛选高质量数据子集，结合现有对齐方法引导模型输出分布

Result: DEFT增强的方法在对齐能力和泛化能力上均优于原始方法，且训练时间显著减少

Conclusion: DEFT框架能有效提升对齐效率和性能，同时缓解泛化能力损失问题
...
TL;DR: PLOT是一种基于最优传输的token级损失方法，用于增强LLM偏好学习，通过将偏好学习建模为最优传输问题来对齐模型输出与人类偏好，同时保持LLM原始分布稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感以及缺乏全局token级关系建模等问题，需要更有效的方法来改进对齐效果。

Method: PLOT通过最优传输问题形式化偏好学习，使用token级损失函数，利用token嵌入捕捉语义关系，实现全局信息化的优化，同时保持LLM原始分布稳定性。

Result: 在人类价值观和逻辑与问题解决两个偏好类别、七个子偏好的实验中，PLOT持续提升对齐性能，同时保持流畅性和连贯性，证明了最优传输作为偏好学习原则方法的有效性。

Conclusion: 最优传输为LLM偏好学习提供了理论基础的框架，PLOT方法为偏好学习提供了新的见解，建立了原则性的方法论。
...
TL;DR: 论文提出自适应占位符补全(APC)框架，通过在高熵位置输出显式占位符来替代硬补全(HC)，降低代码补全的编辑成本


<details>
  <summary>Details</summary>
Motivation: 传统LLM代码补全采用硬补全范式，即使在上下文不足时也强制生成具体代码。分析300万真实交互发现：61%的生成建议在被接受后仍需编辑或被拒绝，尽管与用户后续代码相似度超过80%，表明模型在特定token位置经常做出错误预测

Method: 提出自适应占位符补全(APC)协作框架，将代码补全形式化为不确定性下的成本最小化问题。基于"填充占位符比纠正错误成本更低"的观察，证明存在临界熵阈值，超过该阈值APC的期望成本严格低于HC。通过过滤真实编辑日志构建训练数据，设计基于成本的奖励函数进行强化学习
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于关注融合而忽略了噪声特征的不确定性和细粒度推理，需要更鲁棒和具有上下文建模能力的方法

Method: 提出SURE框架，包含三个组件：不确定性感知专家混合模块处理模态特定噪声，迭代推理模块进行多轮上下文推理，Transformer门模块捕获模态内和模态间交互

Result: 在基准MERC数据集上的实验表明，SURE持续优于现有最先进方法，证明了其在鲁棒多模态推理中的有效性

Conclusion: 不确定性建模和迭代推理对于推进对话环境中的情感识别具有重要意义，SURE框架为此提供了有效的解决方案
...
TL;DR: 该研究评估了基于transformer的模型在法国心衰队列中的短期死亡率预测性能，发现实体感知的多模态transformer表现最佳，而大型语言模型在临床决策支持方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 心衰患者的短期死亡率预测具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要探索更有效的预测方法，包括文本数据、结构化数据以及多模态融合方法。

Method: 在法国心衰队列上评估了四种方法：1）仅文本模型；2）仅结构化数据模型；3）多模态融合模型；4）基于大型语言模型的方法。比较了不同嵌入表示（CLS嵌入 vs 实体级表示）和不同解码策略的效果。

Result: 1）在临床文本中丰富实体级表示比仅使用CLS嵌入能提高预测性能；2）文本和结构化变量的监督多模态融合获得最佳整体性能；3）大型语言模型在不同模态和解码策略中表现不一致，仅文本提示优于结构化或多模态输入。

Conclusion: 实体感知的多模态transformer为心衰短期结局预测提供了最可靠的解决方案，而当前的大型语言模型提示方法在临床决策支持方面仍有局限性。
...
TL;DR: 本文提出了一种结合非上下文文本嵌入和符号概念特征的神经符号方法，用于检测宣传性新闻，相比纯文本方法有更好效果。


<details>
  <summary>Details</summary>
Motivation: 宣传性新闻特别阴险，因为它们将导向性信息与事实报道混合，使其看起来像可靠新闻。现有的基于语言模型的方法（如BERT）虽然很有前景，但由于数据收集中的偏见，往往过度拟合训练数据集。

Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征（如体裁、主题和说服技巧），以增强分类鲁棒性并提高对新来源的泛化能力。

Result: 结果显示相比等效的纯文本方法有改进，消融研究和可解释性分析证实了添加特征的好处。

Conclusion: 神经符号方法通过结合文本嵌入和符号概念特征，能更有效地检测宣传性新闻，提高分类的鲁棒性和泛化能力。
...
TL;DR: 该研究提出了一种测量词序变异相对于交换距离最小化最优程度的数学框架，发现跨语言手势至少77%最优，并引入二次分配问题作为语言优化研究的统一框架。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证语言中词序是否最小化交换距离的假设，即给定源顺序，在置换多面体中距离更近的词序应该成本更低、更可能发生。

Method: 提出测量词序变异相对于交换距离最小化最优程度的数学框架，使用置换多面体表示所有排列结构，引入二次分配问题作为语言优化研究的统一框架。

Result: 研究发现跨语言手势至少77%最优，且多次达到最优的情况不太可能是偶然，为词序或手势顺序在交换距离最小化方面的最优性研究奠定了理论基础。

Conclusion: 建立了词序最优性研究的理论基础，引入二次分配问题作为语言研究的统一优化框架，提出了统一多种语言原则的最优分配一般原则。
...
TL;DR: 论文研究了机器翻译基准数据集的质量问题，提出三步自动化质量保证方法，发现COMET分数与翻译错误率相关，并发布了清理后的数据集
...
Result: 趋势一致：COMET分数较低的数据集在跨度级别有更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。参考型COMET在MMLU上与人编辑样本的结果方向一致
...
TL;DR: SAFE是一个动态基准测试框架，通过可验证的实体序列替代无根据的思维链，用于多跳问答任务，在训练时验证和推理时验证两个阶段确保推理的严谨性。
...
Method: 提出SAFE框架，包含两个阶段：1) 训练时验证：建立原子错误分类和知识图谱验证流程，消除标准基准中的噪声监督；2) 推理时验证：基于验证数据集训练反馈模型，实时检测无根据的推理步骤。

Result: 实验结果显示SAFE不仅暴露了现有基准测试的关键缺陷（识别出高达14%的实例为不可回答），而且显著优于标准基线，平均准确率提升8.4个百分点，同时在推理时保证可验证的推理轨迹。

Conclusion: SAFE框架通过严格的可验证实体序列替代无根据的思维链，为多跳问答提供了更严谨的基准测试方法，能够有效检测和消除虚假正确性，确保推理过程的可靠性。
...
TL;DR: 研究发现高斯扩散模型在处理离散数据时存在采样问题，当噪声数据密度变为多模态时，DDPM采样器会陷入低密度区域导致样本质量下降，提出了结合自条件和q采样的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在连续域生成建模中已成为标准方法，但在处理离散数据时仍面临挑战。本文旨在探究高斯扩散模型为何在采样离散分布时表现不佳，这些离散分布在连续空间中表现为δ分布的混合。

Method: 使用随机层次模型作为玩具模型进行分析，识别出噪声数据密度变为多模态的关键采样区间。提出结合自条件和q采样两种启发式方法，并在关键区间内从DDPM切换到q采样来改善生成质量。

Result: 在文本、编程代码和蛋白质等多个领域的条件和非条件任务中验证了改进方法的有效性，证明结合自条件与q采样能显著提升离散数据的生成质量。

Conclusion: 高斯扩散模型处理离散数据时存在多模态密度问题，通过识别关键采样区间并采用自条件与q采样相结合的策略，可以有效改善离散数据的生成质量。
...
TL;DR: 研究自监督语音模型中语言结构的编码时机，分析不同语言结构在模型训练过程中的层间分布和学习轨迹差异


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能学习有效的语音表征，但尚不清楚语言结构在模型训练的哪个阶段出现。研究旨在探索不同语言结构在模型训练过程中的编码时机和模式

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语训练中的中间检查点，研究多种语言结构在不同层的编码模式和学习轨迹，考察预训练目标对结构组织的影响

Result: 不同语言结构在层间分布和学习轨迹上存在显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释；预训练目标的定义层级强烈影响语言结构的层间组织和学习轨迹，高阶预测任务（迭代精化的伪标签）能诱导更大的并行性

Conclusion: 自监督语音模型中语言结构的编码具有复杂的时间动态和层级组织，预训练目标的设计对模型学习语言结构的方式有重要影响
...
TL;DR: 提出BidirLM方法，将因果生成式语言模型转化为双向编码器，通过系统消融实验识别关键因素，采用线性权重合并和轻量多领域数据混合策略，并整合专业模型能力，在文本、视觉和音频表示基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转化为双向编码器的方法存在局限性：缺乏最佳训练目标的共识、在大规模时存在灾难性遗忘问题、难以灵活整合专业生成模型生态系统。需要开发更有效、可扩展的转换方法。

Method: 1. 在Gemma3和Qwen3模型上进行系统消融实验，识别成功适应的关键因素，特别是常被忽略的先验掩码阶段；2. 引入双重策略：线性权重合并与轻量多领域数据混合，缓解灾难性遗忘；3. 通过合并专业因果模型增强编码器，实现模态和领域特定能力转移。

Result: 提出的开源方法适用于任何因果解码器LLM，产生了BidirLM系列五个编码器，在文本、视觉和音频表示基准测试中超越了现有替代方法。

Conclusion: 该研究提供了一种将因果生成语言模型有效转化为双向编码器的通用方法，解决了现有方法的局限性，实现了更好的表示学习性能，并能够灵活整合专业模型能力。
...
TL;DR: GOOSE提出了一种训练自由的推测解码框架，通过构建自适应脊柱树结构，利用不同来源token的质量差异实现更高效的LLM推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在构建候选token树时，通常采用平衡树结构，没有区分不同来源token的质量差异。研究发现，从输入上下文复制的n-gram匹配token与先前前向传递的统计预测token之间存在显著的质量差距（接受率相差约6倍），这为优化树结构提供了机会。

Method: GOOSE框架构建自适应脊柱树：将高接受率的上下文匹配token形成深度链（脊柱），在每个节点上为低接受率的替代token创建宽分支。这种各向异性（不对称）结构突破了平衡树的深度限制，在固定验证预算下最大化接受token数量。

Result: 在五个LLM（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线性能提升12-33%。理论证明表明，接受token数量至少与单独使用任一来源相当。

Conclusion: 通过识别和利用不同token来源的质量差异，构建各向异性的自适应脊柱树结构，可以显著提升推测解码的效率，为训练自由的LLM推理加速提供了新的优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前重排序模型基于静态人工标注的相关性标签进行优化，与下游生成过程脱节，导致检索指标认为相关的文档在实际生成答案时可能缺乏实用性

Method: 提出重排序偏好优化(RRPO)框架，将重排序建模为顺序决策过程，利用LLM反馈优化上下文效用，并引入参考锚定确定性基线确保训练稳定性

Result: 在知识密集型基准测试中，RRPO显著优于包括RankZephyr在内的强基线模型，能够泛化到不同读者模型，与查询扩展模块正交集成，且在噪声监督下保持鲁棒性

Conclusion: RRPO通过强化学习直接对齐重排序与LLM生成质量，解决了传统重排序模型与下游生成过程的错位问题，无需昂贵人工标注，具有良好泛化性和鲁棒性
...
TL;DR: 提出prosodic ABX方法扩展ABX辨别任务框架，用于评估自监督语音模型对韵律对比的敏感性，并在英语、日语和汉语数据集上验证


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型已知对音位对比敏感，但其对韵律对比的敏感性尚未直接测量，需要开发评估框架

Method: 提出prosodic ABX方法，扩展ABX辨别任务框架，使用少量示例且无需显式标签来评估韵律对比；构建并发布英语和日语最小对数据集，结合现有汉语数据集评估英语重音、日语音高重音和汉语声调

Result: 模型和层级排名在多个实验条件下通常保持一致，这使得该方法在低资源环境下具有实用性

Conclusion: prosodic ABX方法能够有效评估自监督语音模型对韵律对比的敏感性，且在不同语言和实验条件下具有稳定性和实用性
...
TL;DR: 本文提出稳定性过滤方法，通过筛选行为稳定的边界来构建更有效的引导向量，解决了现有方法中大多数检测到的边界无法重现目标行为的问题，显著提升了数学推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于关键词匹配检测推理行为边界的方法存在严重缺陷：大多数检测到的边界在重新生成时无法重现目标行为（93.3%的行为不稳定）。这些不稳定的边界会稀释引导信号，影响引导向量的效果。

Method: 1. 提出概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件；2. 开发稳定性过滤方法，仅保留模型能一致重现目标行为的边界；3. 结合内容子空间投影去除剩余的问题特定噪声。

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0%）。生成的引导向量可在同一架构系列模型中迁移使用，无需重新提取：Nemotron-Research-Reasoning-1.5B提升5.0%，DeepScaleR-1.5B-Preview提升6.0%。

Conclusion: 稳定性过滤能有效识别真正的行为信号，构建更鲁棒的引导向量。该方法不仅提升了特定任务的性能，还展示了跨模型迁移的能力，为训练无关的行为控制提供了更可靠的基础。
...
TL;DR: 研究者开发了首个苏格兰盖尔语多维度基准测试GaelEval，评估了19个大语言模型在盖尔语上的表现，发现前沿模型在语法任务上超越人类基准，专有模型优于开源模型，盖尔语提示能带来小幅性能提升。
...
Method: 开发了GaelEval基准测试，包含三个部分：1) 专家编写的形态句法多项选择题任务；2) 文化背景的翻译基准；3) 大规模文化知识问答任务。评估了19个LLM，并与30名流利说话者的人类基准进行比较。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超越人类基准(78.1%)。专有模型持续优于开源模型，盖尔语提示带来稳定但小幅的优势(+2.4%)。在文化任务上，领先模型超过90%准确率，但多数系统在盖尔语提示下表现更差，绝对分数相对于人工基准有所膨胀。

Conclusion: GaelEval显示前沿模型在盖尔语语法的多个维度上达到超人类性能，证明了盖尔语提示的效果，并揭示了专有模型相对于开源模型的持续性能优势。这为少数民族语言评估提供了重要基准。
...
TL;DR: 该论文在SOMD 2026软件提及跨文档共指消解任务中，比较了两种免微调方法：模糊匹配和上下文感知表示，均取得0.94-0.96的CoNLL F1分数，CAR方法略胜一筹。


<details>
  <summary>Details</summary>
Motivation: 软件提及跨文档共指消解是一个研究不足的任务，需要探索高效且无需微调的方法来处理软件名称的高表面规律性特征。

Method: 提出了两种免微调方法：1) 模糊匹配(FM)：基于词汇字符串相似度的方法；2) 上下文感知表示(CAR)：结合提及级别和文档级别嵌入的方法。

Result: 两种方法在所有子任务中都取得了竞争性表现（CoNLL F1 0.94-0.96），CAR在官方测试集上始终比FM高出1个百分点。CAR对边界噪声更鲁棒，FM对提及替换更稳健。CAR的推理时间随语料规模近似线性增长，而FM呈超线性增长。

Conclusion: 系统选择应同时考虑上游提及检测器的噪声特征和目标语料库的规模。CAR在大规模场景下是更高效的选择。作者发布了代码以支持这一研究不足任务的未来工作。
...
TL;DR: 该论文提出基于文本频率的LLM优化框架，包括文本频率定律、文本频率蒸馏和课程文本频率训练三个部分，通过将输入改写为更频繁的文本表达来提升LLM性能。


<details>
  <summary>Details</summary>
Motivation: 文本频率已被验证与人类阅读速度认知相关，但其与大型语言模型（LLMs）的相关性研究较少。作者认为这是一个未被充分研究的重要方向，需要探索文本数据频率如何影响LLMs的性能。
...
Result: 在自建数据集TFPD上进行了数学推理、机器翻译、常识推理和智能体工具调用等实验，结果显示该框架有效提升了LLMs的性能。

Conclusion: 文本数据频率是影响LLMs性能的重要因素，提出的三部分框架能够有效利用文本频率优化LLMs的提示和微调过程，为LLM研究提供了新的方向。
...
TL;DR: Neuro-RIT是一种通过神经元引导的鲁棒指令调优框架，从密集适应转向精确神经元对齐，提升检索增强语言模型在噪声环境下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 检索增强语言模型在知识密集型任务中表现优异，但在面对不相关或噪声检索上下文时性能会显著下降。现有方法通常在层或模块级别进行粗粒度参数更新，忽视了大型语言模型固有的神经元级稀疏性

Method: 提出Neuro-RIT框架，采用基于归因的神经元挖掘技术，明确分离处理相关与不相关上下文的神经元，然后实施两阶段指令调优策略：通过功能失活处理不相关上下文的神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏

Result: 在多样化的问答基准测试中，Neuro-RIT持续优于强基线方法和现有的鲁棒性增强方法

Conclusion: Neuro-RIT通过从密集适应到精确神经元对齐的范式转变，有效提升了检索增强语言模型在噪声检索上下文中的鲁棒性，为处理不相关信息提供了更精细的控制机制
...
TL;DR: L3TR是一个用于列表式人才推荐的LLM框架，通过块注意力机制和局部位置编码解决位置偏见和并发令牌偏见问题，在真实数据集上优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的人才推荐系统大多采用点对点范式，需要LLM重复处理文本，无法捕捉候选人之间的关系，导致令牌消耗高且推荐效果不佳。此外，LLM在处理多项选择题和多个长文档时存在位置偏见和"迷失在中间"的问题。

Method: 提出L3TR框架，采用隐式策略利用LLM的潜在输出进行推荐。包括：1) 块注意力机制和局部位置编码方法，增强文档间处理并减轻位置偏见和并发令牌偏见；2) ID采样方法解决训练和推理阶段候选集大小不一致问题；3) 设计评估方法检测位置偏见和令牌偏见，并提供无需训练的消偏方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法显示出持续改进。

Conclusion: L3TR框架通过解决LLM在人才推荐中的位置偏见和并发令牌偏见问题，实现了更高效和准确的人才推荐，为基于LLM的列表式推荐系统提供了新的解决方案。
...
TL;DR: 该研究创建了首个阿拉伯语语音命名实体识别公开数据集CV-18 NER，并证明端到端方法在阿拉伯语语音NER任务上优于级联管道方法。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语音命名实体识别研究不足，主要因为其形态复杂性、缺少短元音以及标注资源有限。现有研究显示端到端方法在英语、法语和中文上优于级联管道，但阿拉伯语领域仍待探索。

Method: 创建CV-18 NER数据集（基于阿拉伯语Common Voice 18语料库，采用Wojood模式的21种实体类型进行人工标注），并对比级联系统（ASR+文本NER）和基于Whisper、AraBEST-RQ的端到端模型。
...
Conclusion: 端到端方法在阿拉伯语语音NER任务上具有优势，特别是在低资源环境下。研究提供了首个阿拉伯语语音NER公开基准数据集和模型，为后续研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当提示允许大量有效答案时，全面生成这些答案是满足多样化用户需求的第一步。现有LLM在生成多样化答案方面表现不一，需要系统方法来选择最佳模型

Method: 提出多样性覆盖率指标评估答案集的全面性；评估18个LLM的多样性表现；开发路由模型预测每个查询的最佳模型；在NB-Wildchat数据集上训练和评估

Result: 没有单一模型在所有开放提示上表现最佳，但每个提示都存在显著优于其他模型的特定模型；训练的路由模型在NB-Wildchat上优于单一最佳模型基线（26.3% vs 23.8%）；在NB-Curated数据集和不同答案生成策略上表现出良好泛化能力

Conclusion: 该研究为在拥有多个模型时生成全面答案集奠定了基础，提出的路由方法能有效选择最适合特定查询的模型，提高答案多样性
...
TL;DR: 研究发现语言模型扩展新词汇时，均值初始化会导致新token坍缩到退化子空间，提出基于语言基础的token初始化方法GTI，在生成推荐任务中表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型扩展新词汇时采用均值初始化策略，但作者通过谱分析和几何诊断发现这种方法会导致新token坍缩到退化子空间，抹杀了token间的区别，使得后续微调难以完全恢复，这表明token初始化是词汇扩展的关键瓶颈。

Method: 提出"基于语言基础的token初始化假设"（Grounded Token Initialization Hypothesis），并实现为GTI方法：在微调之前，使用配对的语言监督将新token映射到预训练嵌入空间中具有语义意义的独特位置，从而更好地利用模型的通用知识。

Result: GTI在大多数评估设置中优于均值初始化和现有的辅助任务适应方法，覆盖了行业规模和公共数据集的多个人工智能推荐基准。进一步分析显示，基于基础的嵌入在微调过程中保持更丰富的token间结构。

Conclusion: 初始化质量是词汇扩展的关键瓶颈，基于语言基础的token初始化方法GTI通过将新token映射到语义有意义的预训练嵌入空间位置，能够更好地利用模型的通用知识，提高新词汇域的性能。
...
### [157] [Coverage and Rate Analysis of Follower-Based LEO Satellite Networks: A Stochastic Geometry Approach](https://arxiv.org/abs/2604.01265)
*Juanjuan Ru,Ruibo Wang,Mohamed-Slim Alouini*

Main category: cs.IT

TL;DR: 该论文提出了一种基于球形随机几何的领导者-跟随者卫星架构性能评估框架，用于分析LEO巨型星座中卫星集群的通信性能，推导了中断概率和平均数据率的解析表达式，并探索了最优部署配置。


<details>
  <summary>Details</summary>
Motivation: 为了缓解LEO巨型星座中的星间干扰和有效载荷限制，卫星集群（由小型协作卫星组成的组）被提出来提高性能并减少干扰。典型的领导者-跟随者架构将集群分为具有完整处理和控制能力的领导者卫星以及协助覆盖和吞吐量的多个较简单的跟随者卫星。这些集群增强了覆盖范围和吞吐量，引发了对其性能增益和最优部署的研究兴趣。

Method: 鉴于球形随机几何模型已被证明对此类结构建模有效，作者建立了一个基于SG方法的领导者-跟随者卫星架构性能评估框架。该框架能够定量评估不同部署配置下的通信性能。作者推导了中断概率和平均数据率的解析表达式来评估卫星系统的通信性能，同时提供了低复杂度近似方法。

Result: 数值结果表明，领导者-跟随者架构相比单个领导者卫星具有性能优势，并探索了跟随者卫星的最优部署配置。通过分析表达式和近似方法，能够有效评估不同配置下的通信性能。

Conclusion: 该研究提出的基于球形随机几何的领导者-跟随者卫星架构性能评估框架为LEO巨型星座中的卫星集群设计提供了理论分析工具。通过推导的解析表达式和数值结果，证明了领导者-跟随者架构的性能优势，并为最优部署配置提供了指导。
...
### [158] [On the existence of linear rank-metric intersecting codes](https://arxiv.org/abs/2604.02004)
*Martino Borello,Olga Polverino,Ferdinando Zullo*

Main category: cs.IT

TL;DR: 该论文研究了秩度量相交码的参数限制，证明了非退化秩度量相交码的参数必须满足n ≤ 2m-⌊(k+4)/2⌋，特别地当n=2m-3时只能有k=3且m≥6。通过几何方法将存在性问题约化为散射子空间的存在性问题，并解决了Bartoli等人提出的[6,3,3]_{q^5/q}码不存在的公开问题。


<details>
  <summary>Details</summary>
Motivation: 秩度量相交码是编码理论中的新研究对象，虽然定义形式上与汉明度量相交码类似，但其结构和参数约束表现出显著不同的行为。先前研究表明非退化秩度量相交码必须满足2k-1 ≤ n ≤ 2m-3，但上界的紧致性尚未解决。本文旨在深入理解秩度量相交码的参数限制和结构特性。

Method: 采用秩度量码的几何解释（q-系统方法），证明秩度量相交码的对偶子空间必须满足强规避性质。通过这种几何联系，推导出此类码的新参数限制。将极值码的存在性问题约化为散射子空间的存在性问题，并利用已知的最大散射子空间构造来获得存在性结果。

Result: 1. 证明了n=2m-3的上界只能在k=3且m≥6时达到；2. 得到了更一般的参数限制：n ≤ 2m-⌊(k+4)/2⌋；3. 获得了极值码的几何特征：与𝔽_{q^m}^3中的散射𝔽_q-子空间相关；4. 将[2m-3,3,d]_{q^m/q}码的存在性问题约化为m+3维散射子空间的存在性问题；5. 利用已知的最大散射子空间构造，在m为偶数时获得了存在性结果；6. 证明了[6,3,3]_{q^5/q}秩度量相交码对所有素数幂q都不存在，解决了Bartoli等人在2025年提出的公开问题。

Conclusion: 本文通过几何方法深入研究了秩度量相交码的参数限制，获得了比先前更精确的参数约束，并将极值码的存在性问题与散射子空间理论联系起来。特别地，解决了关于[6,3,3]_{q^5/q}码不存在的公开问题，为秩度量相交码的理论研究提供了重要进展。
...
### [159] [On the Capacity Region of Additive-Multiplicative MAC with Heterogeneous Input Constraints](https://arxiv.org/abs/2604.02037)
*Qianqian Zhang,Ying-Chang Liang*

Main category: cs.IT

TL;DR: 该论文研究了具有异质输入约束的加性-乘性多址接入信道容量区域，为共生无线电系统提供了理论基础。


<details>
  <summary>Details</summary>
Motivation: 研究共生无线电系统中主动发射器和被动反向散射设备在异质约束下的容量极限，为实际系统设计提供理论指导。

Method: 通过理论分析推导AM-MAC容量区域，采用高斯信号、恒定包络信号和混合信号策略，并通过数值优化验证结果。

Result: 证明了和速率容量等于PT点对点容量，BD最大速率需要PT恒定包络信号，边界点需要PT连续均匀相位和离散幅度信号。

Conclusion: 该研究完整刻画了AM-MAC容量区域，为共生无线电系统设计提供了理论依据和优化策略。
...
### [160] [One-Shot Secret Sharing with Monotone Access Structures over Classical-Quantum Broadcast Channels](https://arxiv.org/abs/2604.02275)
*Truman Welling,Rémi A. Chou,Aylin Yener*

Main category: cs.IT

TL;DR: 该论文研究了一种新型秘密共享模型，其中控制节点通过经典-量子广播信道传输秘密，用户收到的份额受到信道扰动而非完全由控制节点控制。论文给出了单调访问结构下的一阶可实现秘密共享速率、逆界，以及二阶和渐近可实现速率。


<details>
  <summary>Details</summary>
Motivation: 传统秘密共享模型中，分发者完全控制每个用户收到的份额。本文研究更现实的场景：分发者通过经典-量子广播信道传输秘密，用户收到的份额受到信道噪声和扰动的影响，不完全由分发者控制。

Method: 采用包含控制节点和L个用户的单调访问结构模型，控制节点作为分发者通过经典-量子广播信道编码和传输秘密。分析在这种信道扰动下的秘密共享性能，推导可实现速率和逆界。

Result: 获得了单调访问结构下的一阶可实现秘密共享速率和逆界，进一步推导了二阶和渐近可实现速率。在特殊情况下（所有份额都需恢复秘密），结果与传统经典信道上的秘密共享容量一致。

Conclusion: 该研究扩展了秘密共享理论到更实际的信道扰动场景，为经典-量子广播信道下的秘密共享提供了理论基础和性能界限，将传统结果推广到更一般的单调访问结构。
...
### [161] [A variationally consistent beam-to-beam point coupling formulation for geometrically exact beam theories](https://arxiv.org/abs/2604.02049)
*Ivo Steinbrecher,Nora Hagmeyer,Christoph Meier,Alexander Popp*

Main category: cs.CE

TL;DR: 提出了一种独立于具体公式的梁-梁点耦合方法，用于精确模拟不同数值格式梁结构在离散点处的相互作用


<details>
  <summary>Details</summary>
Motivation: 工程应用中常见细长梁结构通过接头或连接件在离散点处相互作用，当涉及不同梁理论、插值方案和旋转参数化时，精确建模这些相互作用具有挑战性

Method: 在几何精确梁理论框架内，基于有限元离散化，提出通用公式独立的梁-梁点耦合方法。耦合约束仅用截面运动学（质心位置和方向）表达，引入位置和旋转耦合的广义变形度量，采用拉格朗日乘子法和罚正则化进行约束实施

Result: 该方法满足客观性、对称性和无应力参考配置一致性等关键特性，数值算例证明该方法能鲁棒灵活地耦合不同公式和离散化的梁，即使相互作用点位于梁单元内任意位置

Conclusion: 提出了一种通用、公式独立的梁-梁耦合方法，可直接集成到现有梁有限元模型的弱形式中，为不同数值格式梁结构的精确相互作用建模提供了有效解决方案
...
TL;DR: 本文认为结构化LLM路由本质上是系统级负担分配问题，而非单纯的提示工程问题。研究发现不同后端模型（OpenAI、Gemini、Llama）在结构化路由性能上存在显著差异，没有通用的最佳路由模式。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为智能AI系统的核心控制组件，可靠的结构化路由需要在正确性、延迟和实现成本之间取得平衡。当前研究往往将结构化路由视为提示工程问题，但作者认为这本质上是一个系统级负担分配问题。

Method: 通过全面的全因子基准测试，评估了48种部署配置和15,552个请求，覆盖OpenAI、Gemini和Llama三种后端模型。研究考察了结构工作在不同生成栈层级的分配方式：模型直接输出结构、传输时压缩、或生成后本地重建。

Result: 研究发现没有通用的最佳路由模式，后端特定的交互效应主导性能。在Gemini和OpenAI上高度可靠的路由模式，在Llama上可能出现显著的正确性下降。压缩实现的效率增益也强烈依赖于后端模型。

Conclusion: 本文提出了一个可部署的框架，用于在异构后端条件下进行结构化路由推理。提供了跨后端评估方法和实际部署指导，帮助在生产级智能专家系统中导航正确性-成本-延迟的权衡边界。
...
TL;DR: 本文提出数字孪生反事实框架(DTCF)，通过模拟而非统计估计反事实结果，并建立分层验证体系来检验模拟的可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法都依赖假设来填补缺失的反事实数据，但都无法直接产生反事实本身。本文旨在通过数字孪生模拟反事实，并建立可验证的框架来评估模拟的准确性。

Method: 提出数字孪生反事实框架(DTCF)，将数字孪生模拟器形式化为潜在结果框架中的随机映射。引入孪生保真度假设层次结构（从边际保真度到联合保真度再到结构保真度），并建立五层验证架构将不可证伪的模拟器声明转化为可证伪的检验。

Result: 该框架将因果量分解为可边际验证的部分（ATE、CATE、QTE）和依赖copula的部分（ITE分布、获益/损害概率、处理效应方差）。提供了边界、敏感性和不确定性量化工具来显式处理copula依赖性。
...
TL;DR: IDEA2是一个半自动工作流，利用大语言模型在专家参与下从需求文档中提取能力问题，通过迭代反馈循环提高效率和质量。
...
Method: IDEA2采用半自动工作流：1) LLM从需求文档中初步提取能力问题；2) 领域专家在协作平台上评审并提供反馈；3) LLM根据反馈迭代重写被拒绝的问题直至达成共识；4) 使用溯源模型追踪完整生命周期。

Result: 在科学数据和文化遗产两个真实场景中验证，IDEA2能够加速需求工程过程，提高能力问题的接受度和相关性，在领域专家中展现出高可用性和有效性。

Conclusion: IDEA2成功解决了能力问题提取的瓶颈问题，通过LLM与专家协作的迭代流程，提高了本体工程效率和质量，代码和实验已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统智能体中心架构在结构化领域（如企业和制度系统）中存在语义不一致、可解释性差和长期稳定性不足的问题，需要一种基于共享世界表示的替代方案。

Method: 提出世界中心多智能体系统概念，对世界进行分类（本体显式性、规范性等维度），使用语义模型作为数学形式化表示，并开发Ontobox平台作为实现。

Result: 建立了一个基于共享世界模型的多智能体系统框架，使学习和协调操作在共享世界模型上进行，而非孤立的智能体本地表示，从而实现全局一致性和可验证的系统行为。

Conclusion: 世界中心多智能体系统为结构化领域提供了一种更优的架构选择，通过共享的显式世界表示确保了语义一致性、可解释性和系统稳定性，语义模型和Ontobox平台为此提供了理论和技术实现。
...
TL;DR: 研究发现AI自动化呈现"涨潮式"而非"浪潮式"发展，AI能力在广泛任务中持续提升，预计到2029年LLM能在80%-95%成功率下完成大多数文本任务。


<details>
  <summary>Details</summary>
Motivation: 针对AI自动化发展模式的争议，研究旨在验证AI能力增长是呈现"浪潮式"（特定任务能力突然爆发）还是"涨潮式"（广泛任务能力持续提升）的发展模式。

Method: 基于美国劳工部O*NET分类的3000多个文本型任务，通过17000多次由相关职业工作者进行的评估，系统测试AI模型在不同任务上的表现。

Result: 发现AI能力增长主要是"涨潮式"而非"浪潮式"；2024年第二季度AI完成人类3-4小时任务的约50%成功率，预计2025年第三季度提升至65%；若趋势持续，到2029年LLM能在80%-95%成功率下完成大多数文本任务。

Conclusion: AI自动化主要表现为广泛任务能力的持续提升，而非特定任务的突然突破；AI能力改善速度很快，但经济影响因组织采用时间而滞后；实现更高质量或接近完美的成功率需要更长时间。
...
TL;DR: 本文研究了LLM中的认知偏见，发现偏见在模型激活空间中呈线性可分方向，通过激活引导可减少26-32%的偏见分数，同时保持下游任务能力。
...
Method: 定义了LLM认知偏见为在可计算基准任务中系统性、可复现的偏离正确答案；引入LLM CogBias基准，围绕四类认知偏见（判断、信息处理、社交、反应）组织；使用线性探针和对比设计分析偏见在激活空间中的表征；应用激活引导技术来调节偏见行为。

Result: 认知偏见在所有四类中系统性出现，其程度和去偏见效果强烈依赖于偏见类型：提示级去偏见显著减少反应偏见，但对判断偏见产生反效果；偏见在模型激活空间中编码为线性可分方向；激活引导实现了26-32%的偏见分数减少，同时在25个基准测试中保持下游能力（Llama：可忽略的退化；Qwen：判断偏见最多-19.0pp）。

Conclusion: 尽管不同模型间的偏见表征近乎正交（平均余弦相似度0.01），但激活引导在不同架构中以相似速率减少偏见，表明存在共享的功能组织。这为理解和缓解LLM中的认知偏见提供了实证基础。
...
TL;DR: 本文提出了RIFT（RubrIc Failure mode Taxonomy），这是一个用于系统化描述评分标准中失败模式的分类法，包含8种失败模式，分为3个高级类别，旨在解决现有评分标准质量诊断方法的不足。


<details>
  <summary>Details</summary>
Motivation: 当前基于评分标准的评估在LLM基准测试和训练管道中被广泛使用，但现有方法主要依赖下游信号（如强化学习结果）来评估评分标准的有效性。然而，仅从这些聚合或下游信号中无法系统化地诊断评分标准的质量问题，缺乏一个原则性的方法来识别评分标准设计中的缺陷。

Method: 作者采用扎根理论方法，通过迭代标注来自五个不同基准测试（涵盖通用指令遵循、代码生成、创意写作和专家级深度研究）的评分标准，直到没有新的失败模式出现，从而开发出RIFT分类法。该分类法包含8种失败模式，分为3个高级类别：可靠性失败、内容有效性失败和结果有效性失败。作者还评估了分类法的一致性，并提出了自动化的评分标准质量指标。

Result: 研究开发了包含8种失败模式的RIFT分类法，并通过独立人类标注者的一致性评估，观察到总体公平的一致性（87%的成对一致性和0.64的平均Cohen's kappa）。此外，提出的自动化评分标准质量指标与人类失败模式标注结果对齐，达到了最高0.86的F1分数。

Conclusion: RIFT分类法为系统化诊断评分标准质量问题提供了一个框架，填补了现有评估方法的空白。通过结合人类标注和自动化指标，该方法能够支持可扩展的评分标准质量诊断，为LLM评估和训练中的评分标准设计提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: POMDP在处理状态和转移不确定性方面提供了原则性形式化，但由于维度灾难和历史灾难，在有限规划时间内获得高性能策略仍然是一个主要挑战。许多POMDP问题中，信息价值在信念空间中变化，这为优化计算资源分配提供了机会。

Method: 提出了一个动态规划框架，根据每个信念点的信息价值有条件地处理观测。基于此框架，开发了VOIMCP算法，这是一种蒙特卡洛树搜索算法，当信息价值较低时选择性地忽略观测信息，避免不必要的观测分支。

Result: 提供了VOI推理框架的近最优性理论保证，并推导了VOIMCP的非渐近收敛界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间中的变化特性，提出的VOIMCP算法能够更有效地分配计算资源，在有限规划时间内获得更好的性能表现，为解决大规模POMDP问题提供了有效方法。
...
TL;DR: 该研究评估AI辅助用药系统的可靠性，关注系统错误类型及其临床后果，强调需要风险感知的评估方法


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域应用日益广泛，但传统性能指标无法充分反映其在真实世界决策中的可靠性。在用药管理等高风险领域，单个错误建议就可能导致严重患者伤害，因此需要深入理解AI系统失败行为和实际影响。

Method: 通过一系列受控模拟场景，分析药物相互作用和剂量决策中的系统失败类型，包括遗漏相互作用、错误风险标记和不适当剂量推荐，关注错误发生机制和错误输出的后果。

Result: 研究发现AI在用药相关场景中的错误可能导致药物不良反应、无效治疗或延误护理，特别是在缺乏足够人工监督的情况下。研究还揭示了过度依赖AI建议的风险以及决策过程透明度不足带来的挑战。

Conclusion: 该研究为医疗AI评估提供了可靠性视角，强调理解失败行为和真实世界影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合，并确保足够的人工监督。
...
<details>
  <summary>Details</summary>
Motivation: 科学发现因碎片化文献需要过多人工努力而减缓，现有AI工具缺乏结构化多步骤方法来从科学文献中提取深层见解，LLMs存在幻觉和不完整提取问题。

Method: 开发Elhuyar多智能体系统，分配任务给专门智能体进行论文筛选、数据提取、模型拟合和发现总结，结合人类监督确保可靠性，生成结构化报告并支持迭代精炼。

Result: 在材料科学中部署，分析钨在氦离子辐照下的文献，发现氦气泡生长与辐照剂量和温度呈指数相关，为聚变反应堆等离子体面向材料提供见解。

Conclusion: AI辅助文献综述能够揭示科学模式并加速发现，Elhuyar系统展示了如何通过整合LLMs、结构化AI和人类科学家来提取深层科学见解。
...
TL;DR: 提出一种不可行性感知的LLM框架，用于组合优化问题，结合可验证数据集构建、监督微调和LLM辅助下游搜索，在minor-embedding问题上实现解决方案生成和不可行性检测的联合优化。
...
Method: 提出不可行性感知框架：1）通过数学规划公式和可证明的零相位不可行性筛选构建可验证训练数据集；2）使用精确优化流程生成的数据对8B参数LLM进行监督微调；3）将LLM输出作为下游局部搜索的暖启动。

Result: 微调模型在整体准确率上比GPT-5.2提升高达30%；LLM引导的暖启动在下游局部搜索中相比从头开始提供高达2倍的加速。

Conclusion: 该框架成功实现了LLM在组合优化问题中同时进行解决方案生成和不可行性检测的能力，即使LLM输出不完美，也能通过下游搜索加速优化过程。
...
<details>
  <summary>Details</summary>
Motivation: 当前减少LLM幻觉的方法（如提示工程和思维链提示）主要关注单个文档，未能考虑语料库中文档间的关系，这在科学文献摘要等需要跨文档一致性验证的领域存在局限。

Method: 提出Peer Context Outlier Detection (P-COD)方法，利用文档间关系改进提取准确性。在科学文献摘要应用中，通过比较具有相似实验设置的论文得出的结论，将提取数据与语料库中已验证的同行信息进行对比，调整置信度分数并标记低置信结果供专家审查。

Result: 实验在6个科学领域中实现了高达98%的异常检测精度，表明该方法能有效减少幻觉，增强对自动化系统的信任，并让研究人员专注于模糊案例，从而简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系显著提高了LLM数据提取的准确性，减少了幻觉，为科学文献摘要等需要跨文档一致性验证的应用提供了有效的解决方案。
...
### [174] [A Self-Evolving Agentic Framework for Metasurface Inverse Design](https://arxiv.org/abs/2604.01480)
*Yi Huang,Bowen Zheng,Yunxi Dong,Hong Tang,Huan Zhao,S. M. Rakibul Hasan Shawon,Hualiang Zhang*

Main category: cs.AI

TL;DR: 该研究提出了一种基于智能体框架的元表面逆向设计方法，通过上下文级技能演化来积累可重用的求解器专业知识，提高设计工作流的自动化程度和成功率。


<details>
  <summary>Details</summary>
Motivation: 元表面逆向设计需要复杂的计算电磁学专业知识和求解器特定的软件工程技能，现有的大语言模型驱动系统通常是会话绑定的，无法跨任务保存可重用的工作流知识。

Method: 提出了一个智能体框架，包含编码智能体、演化技能工件和基于物理仿真的确定性评估器，通过迭代精炼求解器特定策略而不修改模型权重或底层物理求解器。

Result: 在多个元表面逆向设计任务类型的基准测试中，演化技能将分布内任务成功率从38%提高到74%，标准通过率从0.510提高到0.870，平均尝试次数从4.10减少到2.30。在分布外任务中，虽然二进制成功率变化不大，但最佳边际改进和错误组成变化表明工作流知识的部分迁移。

Conclusion: 技能演化的主要价值在于围绕可靠的计算引擎积累可重用的求解器特定专业知识，为实现更自主和易访问的元表面逆向设计工作流提供了实用路径。
...
### [175] [LLM Agents as Social Scientists: A Human-AI Collaborative Platform for Social Science Automation](https://arxiv.org/abs/2604.01520)
...
<details>
  <summary>Details</summary>
Motivation: 传统社会科学研究需要设计复杂的实验，依赖真实人类参与者，导致研究过程劳动密集、成本高昂且难以扩展。

Method: 首先开发YuLan-OneSim大规模社会模拟系统，具有三个核心特性：通过自然语言到可执行场景的自动编程实现通用性，通过分布式架构支持高达10万并发智能体实现可扩展性，通过反馈驱动的LLM微调实现可靠性。基于此系统，S-Researcher支持研究人员设计社会实验、用LLM智能体模拟人类行为、分析结果和生成报告。

Result: 将LLM模拟研究范式操作化为三种典型推理模式（归纳、演绎和溯因），并通过系统案例研究验证：归纳再现与Axelrod理论一致的文化动态，演绎测试教师注意力竞争假设并与调查数据验证，溯因识别公共物品游戏中的合作机制并通过人类实验确认。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在健康食品政策领域的信息提取面临多重挑战，包括幻觉、误分类和遗漏等问题，这些问题源于政策文档的结构多样性和不一致性。需要一种更可靠的方法来自动化政策信息提取。
...
Conclusion: 该基于角色的LLM框架为从健康政策中自动化信息提取提供了一种可靠且透明的方法，在复杂推理任务中优于传统方法。
...
### [177] [RAE-AR: Taming Autoregressive Models with Representation Autoencoders](https://arxiv.org/abs/2604.01545)
*Hu Yu,Hang Xu,Jie Huang,Zeyue Xue,Haoyang Huang,Nan Duan,Feng Zhao*

Main category: cs.AI

TL;DR: 该研究探索了将表示自编码器（RAE）整合到连续自回归（AR）模型中的方法，通过分布归一化和高斯噪声注入解决了高维表示自编码器在AR范式中的建模困难和暴露偏差问题。


<details>
  <summary>Details</summary>
Motivation: 生成模型的潜在空间长期被VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示之前被认为不适合生成建模。虽然RAE方法显示了表示自编码器也能达到与VAE编码器竞争的性能，但将其整合到连续自回归模型中的研究仍然不足。

Method: 提出了RAE-AR方法，针对AR模型的特点解决两个主要挑战：1）通过分布归一化进行标记简化，降低复杂标记分布建模的难度；2）在训练期间加入高斯噪声注入，缓解高维度放大的训练-推理差距（暴露偏差）。
...
### [178] [Does Your Optimizer Care How You Normalize? Normalization-Optimizer Coupling in LLM Training](https://arxiv.org/abs/2604.01563)
*Abdelrahman Abouzeid*

Main category: cs.AI

TL;DR: 研究发现LLM训练中归一化层与优化器存在交互作用：Derf归一化与Muon优化器组合时性能显著下降，而DyT归一化则无此问题。问题源于erf函数在Muon快速谱范数增长下的饱和和尺度盲区，通过EMA混合或调整alpha参数可部分恢复性能。
...
Method: 采用3x2因子设计实验，在10亿参数规模、1000训练步数下测试不同归一化方法（Derf、RMSNorm、DyT）与优化器（AdamW、Muon）的组合效果。通过分析erf函数在Muon优化器下的行为，识别饱和和尺度盲区问题，并提出EMA混合和调整alpha参数两种解决方案。

Result: Derf与Muon组合存在显著负交互作用：与RMSNorm相比，Derf在AdamW下仅差+0.31 nats，但在Muon下差距扩大至+0.97 nats（约3倍）。DyT作为有界归一化对照组未显示此惩罚。问题源于erf在Muon快速谱范数增长下的饱和和尺度盲区。EMA混合恢复约84%性能差距，调整Derf的alpha参数从0.5降至0.3恢复约80%性能。

Conclusion: 归一化层与优化器并非独立设计选择，存在显著交互效应。Derf与Muon组合存在隐藏的性能惩罚，容易被短时试验忽略。通过调整erf参数或引入运行尺度估计可缓解问题，但最佳实践需要联合考虑归一化与优化器设计。
...
### [179] [NED-Tree: Bridging the Semantic Gap with Nonlinear Element Decomposition Tree for LLM Nonlinear Optimization Modeling](https://arxiv.org/abs/2604.01588)
*Zhijing Hu,Yufan Deng,Haoyang Liu,Changjun Fan*

Main category: cs.AI

TL;DR: NED-Tree是一个解决非线性运筹学问题自然语言到可执行模型转换的框架，通过句子级提取和递归树分解解决语义对齐问题，在10个基准测试中达到72.51%的平均准确率。
...
Method: NED-Tree采用两个核心策略：(1) 逐句提取策略确保参数映射的鲁棒性和可追溯性；(2) 递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素。

Result: 在10个基准测试中，NED-Tree建立了新的最先进水平，平均准确率达到72.51%。这是第一个通过元素分解驱动LLM解决非线性建模困难的框架。

Conclusion: NED-Tree成功弥合了建模语义与代码语义之间的差距，为复杂非线性、多约束运筹学问题的自动化翻译提供了有效解决方案，并发布了NEXTOR基准测试。
...
### [180] [ThinkTwice: Jointly Optimizing Large Language Models for Reasoning and Self-Refinement](https://arxiv.org/abs/2604.01591)
...
Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性的在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，一次自我精炼后提升11.5个百分点（pass@4）。训练动态分析显示存在隐式的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案。

Conclusion: ThinkTwice建立了推理和自我精炼联合训练作为RLVR的原则性和有效方法学，无需额外监督信号，通过简单的两阶段框架实现了显著的性能提升。
...
### [181] [MM-ReCoder: Advancing Chart-to-Code Generation with Reinforcement Learning and Self-Correction](https://arxiv.org/abs/2604.01600)
*Zitian Tang,Xu Zhang,Jianbo Yuan,Yang Zou,Varad Gunjal,Songyao Jiang,Davide Modolo*

Main category: cs.AI

TL;DR: MM-ReCoder是一个基于强化学习的多模态图表转代码生成模型，通过多轮自我修正策略提升代码质量和可执行性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖监督微调，缺乏代码执行环境暴露，且现有MLLM在自我修正方面表现不佳，需要更有效的训练策略来提升图表转代码的质量。

Method: 提出基于Group Relative Policy Optimization的两阶段多轮自我修正强化学习策略：第一阶段通过共享首轮增强自我修正能力，第二阶段通过全轨迹优化提升编码能力。

Result: 在三个图表转代码基准测试中取得了最先进的性能，能够生成更准确、可执行的代码。

Conclusion: MM-ReCoder通过强化学习和自我修正策略有效提升了多模态图表转代码生成的质量和可执行性，为MLLM在编码任务中的应用提供了新思路。
...
### [182] [CRaFT: Circuit-Guided Refusal Feature Selection via Cross-Layer Transcoders](https://arxiv.org/abs/2604.01604)
*Su-Hyeon Kim,Hyundong Jin,Yejin Lee,Yo-Sub Han*

Main category: cs.AI

TL;DR: CRaFT是一个基于电路引导的拒绝特征选择框架，通过评估特征在拒绝边界附近对模型决策的影响来识别因果性拒绝特征，相比传统基于激活强度的方法显著提升了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型拒绝特征选择方法主要依赖特征在有害提示上的激活强度，这往往捕捉到的是表面信号而非拒绝决策的因果因素。需要更可靠的方法来识别真正介导拒绝行为的因果特征。

Method: 提出CRaFT框架，通过评估特征在拒绝边界附近对模型拒绝-合规决策的影响来排名特征，使用电路引导的方法识别因果性拒绝特征，而不是简单地依赖激活强度。

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率从6.7%提升到48.2%，在多个越狱基准测试中优于基线方法，表明电路影响是比激活强度更可靠的拒绝特征识别标准。

Conclusion: 电路影响是识别因果性拒绝特征比激活强度更可靠的标准，CRaFT框架能有效识别介导拒绝行为的内部机制，为理解大语言模型安全机制提供了新视角。
...
### [183] [From Multi-Agent to Single-Agent: When Is Skill Distillation Beneficial?](https://arxiv.org/abs/2604.01608)
*Binyan Xu,Dong Fang,Haitao Li,Kehuan Zhang*

Main category: cs.AI

TL;DR: 该研究揭示了多智能体系统蒸馏为单智能体技能时，技能效用的关键决定因素不是任务本身，而是评估指标的拓扑特性，并提出了首个先验预测指标Metric Freedom (F)。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然能处理复杂任务，但存在协调开销大、上下文碎片化和脆弱的阶段排序等问题。将其蒸馏为单智能体技能可以规避这些成本，但缺乏关于何时蒸馏、蒸馏什么的原则性指导，导致实际效果不一致（技能提升从28%到-2%不等）。

Method: 提出了Metric Freedom (F)指标，通过Mantel测试量化输出多样性与得分方差之间的耦合程度，测量评估指标评分地形的拓扑刚性。基于F提出了两阶段自适应蒸馏框架：第一阶段作为选择性提取机制，在"自由"指标上提取工具和知识同时丢弃限制性结构；第二阶段专门针对"刚性"指标（F≲0.6）进行计算密集的迭代优化。

Result: 在4个任务、11个数据集和6个指标上的评估显示，F能强有力地预测技能效用（ρ=-0.62，p<0.05）。相同的智能体轨迹在刚性指标和自由指标下产生完全相反的技能提升效果。自适应智能体匹配或超越了原始多智能体系统，同时将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用从根本上说是指标层面的属性，而非任务层面的属性。Metric Freedom (F)作为首个先验预测指标，能够指导何时以及如何将多智能体系统蒸馏为单智能体技能，实现性能提升的同时显著降低计算成本和延迟。
...
### [184] [GraphWalk: Enabling Reasoning in Large Language Models through Tool-Based Graph Navigation](https://arxiv.org/abs/2604.01610)
...
Method: GraphWalk为LLM配备一组最小正交图操作工具，通过顺序图导航进行多步推理，每个工具调用代表可验证的执行步骤，形成透明执行轨迹

Result: 在迷宫遍历和模拟企业知识图谱的合成图上测试，基于工具的遍历方法相比上下文基线在所有模型家族中都获得显著一致的性能提升，且模型规模越大提升越明显

Conclusion: GraphWalk框架能够有效解决大型知识图谱无法放入上下文窗口的问题，通过工具化图导航实现透明、可验证的多步推理，为LLM在复杂图结构上的推理提供了新途径
...
### [185] [Analysis of LLM Performance on AWS Bedrock: Receipt-item Categorisation Case Study](https://arxiv.org/abs/2604.01615)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是在生产环境中评估大型语言模型在收据项目分类任务中的实际应用价值，特别关注模型性能（准确性、响应稳定性）与成本（token级别成本）之间的权衡，并探索适合不同场景的提示方法（零样本或少样本）。

Method: 研究方法包括：1）在AWS Bedrock平台上比较四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct）；2）评估准确性、响应稳定性和token级别成本；3）对比零样本和少样本提示方法的适用性。

Result: 实验结果显示：Claude 3.7 Sonnet在分类准确性和成本效率方面达到最有利的平衡，为收据项目分类任务提供了最优的性价比方案。

Conclusion: 研究结论是Claude 3.7 Sonnet在生产导向的收据项目分类框架中表现出最佳的性能-成本平衡，为实际应用提供了有价值的参考依据。
...
### [186] [OSCAR: Orchestrated Self-verification and Cross-path Refinement](https://arxiv.org/abs/2604.01624)
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（DLMs）暴露其去噪轨迹，为推理时控制提供了天然机制。理想的幻觉缓解框架应利用这种模型原生信号进行干预，而不是依赖外部训练的幻觉分类器。
...
Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA数据集上，使用LLaDA-8B和Dream-7B模型，OSCAR显著减少幻觉内容，提高事实准确性。其基于熵的不确定性信号优于专门训练的检测器，表明扩散语言模型具有识别事实不确定性的内在能力。

Conclusion: OSCAR展示了利用扩散语言模型原生不确定性信号进行推理时幻觉缓解的有效性。定位和校正提供互补增益，且对并行链数量（4、8、16）具有鲁棒性。该方法超越了自回归模型的token承诺结构限制，为DLMs中的定位和不确定性感知生成研究提供了新方向。
...
### [187] [Exploring Robust Multi-Agent Workflows for Environmental Data Management](https://arxiv.org/abs/2604.01647)
*Boyuan Guan,Jason Liu,Yanzhao Wu,Kiavash Bahreini*

Main category: cs.AI

TL;DR: EnviSmart系统通过三轨知识架构和角色分离的多智能体设计，在环境数据管理中实现可靠的LLM驱动工作流，防止错误传播到不可逆操作。
...
Result: 多智能体方法提高了效率和可靠性：SF2Bench复合洪水基准测试中，单个操作员在两天内完成，通过重复工件重用；审计交接检测并阻止了影响所有2452个监测站的坐标转换错误。代表性事件ISS-004展示了边界遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构设计将可靠性作为系统属性，成功部署在生产环境中，能够有效防止LLM工作流中的错误传播到不可逆操作，为环境数据管理提供了可靠的智能体驱动解决方案。
...
### [188] [CORAL: Towards Autonomous Multi-Agent Evolution for Open-Ended Discovery](https://arxiv.org/abs/2604.01658)
...
TL;DR: CORAL是一个基于大语言模型的多智能体自主进化框架，用于开放性问题发现，通过持久记忆、异步执行和心跳干预实现智能体自主探索与协作，在多个任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的进化方法主要依赖固定启发式和硬编码探索规则，限制了智能体的自主性。需要开发能够支持持续搜索和知识积累的自主进化框架来解决开放性问题。
...
Conclusion: 研究表明，更大的智能体自主性和多智能体进化可以显著改善开放性问题发现。CORAL框架为基于大语言模型的自主进化提供了有效解决方案，展示了知识重用和多智能体探索与通信的价值。
...
### [189] [Ontology-Aware Design Patterns for Clinical AI Systems: Translating Reification Theory into Software Architecture](https://arxiv.org/abs/2604.01661)
*Florian Odi Stummer*

Main category: cs.AI

TL;DR: 本文提出七种面向临床AI系统的本体感知设计模式，用于构建能够抵抗数据本体扭曲的AI管道，包括数据验证、信号保留、漂移监控、并行表示、反馈中断、术语管理和合规适配等模式。


<details>
  <summary>Details</summary>
Motivation: 临床AI系统训练数据受到文档工作流、计费激励和术语碎片化的结构性扭曲，现有理论已描述这种扭曲机制，但将这些洞察转化为可实施的软件架构仍是一个未解决的问题。

Method: 采用Gang-of-Four模式语言提出七种本体感知设计模式：本体检查点、休眠感知管道、漂移哨兵、双本体层、具体化断路器、术语版本门和监管合规适配器，并通过初级保健AI系统的参考架构和糖尿病风险预测场景进行说明。

Result: 提出了七种设计模式的完整规范（问题、驱动力、解决方案、后果、已知用例和相关模式），其中三种模式在现有系统中有部分先例，四种模式尚未正式描述，提供了参考架构和场景演练。

Conclusion: 本文提供了一个基于理论分析的设计词汇表，用于构建抵抗本体扭曲的临床AI系统，但缺乏运行时基准测试，且限于德国和欧盟监管环境，需要未来在生产系统中进行评估验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法主要针对同构模型（来自相同预训练主干），但在开放模型生态系统中，有用的专家模型通常基于不同架构（如Llama、Qwen、Mistral），直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得不适定。

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标来跨异构主干转移知识；2）冲突感知去噪，在融合过程中抑制不兼容或有噪声的转移信号。该方法保持目标适配器基的同时预测结构化更新，确保稳定且良条件的转移过程。

Result: 在异构转移、多源融合、噪声源鲁棒性和跨族泛化等设置中，HeteroFusion始终优于强大的合并、融合和集成基线方法。

Conclusion: HeteroFusion为异构语言模型融合提供了一种有效的解决方案，通过拓扑对齐和冲突感知去噪克服了不同架构模型合并时的技术挑战，在开放模型生态系统中具有重要应用价值。
...
TL;DR: 该研究利用中国视频平台数据，分析AIGC与HGC在创作和消费行为上的差异，发现AIGC创作者通过高产量获得与HGC相当的总体参与度，尽管消费者更偏好HGC，算法分发机制在其中起到调节作用。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能生成内容（AIGC）的快速扩散，正在从根本上重构在线内容生态，需要对其行为特征和分布影响进行严格研究，以了解AIGC与人类生成内容（HGC）在创作和消费模式上的差异。

Method: 研究利用中国领先视频分享平台的纵向数据集，包含数千万用户数据，通过对比分析AIGC和HGC的创作与消费行为特征，特别关注了"规模优先于偏好"的动态现象。

Result: 研究发现AIGC创作者通过高产量获得与HGC创作者相当的总体参与度，尽管消费者明显更偏好HGC；算法内容分发机制能够调节AIGC相关的竞争利益；需要AIGC敏感的分发算法和精准治理框架。

Conclusion: 研究呼吁实施AIGC敏感的分发算法和精准治理框架，以确保在线内容平台的长期健康发展，平衡AIGC与HGC的竞争关系，维护内容生态的健康。
...
Method: 采用两阶段级联架构：第一阶段高召回率故障检测，第二阶段异常样本细粒度故障分类。提出基于互信息、梯度分析和SE注意力权重的多方法融合策略压缩输入传感器通道，设计1+1分支LiteInception架构压缩参数。引入知识蒸馏作为精确率-召回率调节机制，构建集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上，故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。输入传感器通道从23个压缩到15个，InceptionTime参数压缩70%，CPU推理加速超过8倍，F1损失小于3%。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了良好平衡，为资源受限边缘设备上的航空故障诊断提供了可行的轻量级解决方案。
...
TL;DR: 本文提出了AnIML本体，这是一个OWL 2本体，用于形式化分析信息标记语言（AnIML）的语义，以解决XML模式表达性导致的语义不一致问题，支持跨系统和跨实验室的互操作性。


<details>
  <summary>Details</summary>
Motivation: 尽管AnIML作为分析化学和生物学领域的XML标准在工业研发实验室中广泛使用，但其XML模式的表达性允许不同利益相关者产生不同的解释，导致语义不一致，破坏了AnIML模式原本设计的互操作性目标。
...
Result: 成功开发了AnIML本体，该本体形式化了AnIML的语义，并与Allotrope数据格式对齐，支持未来的跨系统和跨实验室互操作性。验证表明本体能够准确捕获AnIML语义并确保一致性。

Conclusion: AnIML本体为解决分析数据系统中的语义互操作性问题提供了有效的解决方案，通过形式化语义和严格验证，为跨系统和跨实验室的数据交换与集成奠定了坚实基础。
...
TL;DR: 提出基于SAT的二维单尺寸切割库存问题求解框架，通过需求扩展、板材分配变量和非重叠约束优化求解，在Cui-Zhao基准测试中优于OR-Tools、CPLEX和Gurobi
...
Method: 提出SAT求解框架：1）按需求扩展零件类型，每个副本都有板材分配变量；2）仅对分配到同一板材的副本激活非重叠约束；3）引入不可行方向消除规则，当只有一个方向能放入板材时固定旋转变量。比较三种最小化板材数量的方法：非增量SAT（二分搜索）、增量SAT（跨迭代重用子句）和加权部分MaxSAT

Result: 在Cui-Zhao基准测试中，最佳SAT配置证明最优的实例数量是OR-Tools、CPLEX和Gurobi的2-3倍，且获得更小的最优性间隙。SAT方法间的相对排名取决于旋转：无旋转时增量SAT最强，旋转增加公式大小时非增量SAT更有效

Conclusion: 基于SAT的框架能有效解决2D-CSSP问题，在不同旋转条件下采用不同SAT策略可获得最佳性能，显著优于现有商业求解器
...
<details>
  <summary>Details</summary>
Motivation: 通用LLM将生成视为单次文本完成，无法满足安全关键工程工作流程中固有的顺序、多门约束，导致生成可执行仿真工件时出现级联约束违反。

Method: 提出约束闭环生成框架，将TPS工件生成组织为迭代工作流：生成、验证、CDG引导修复、执行和审计。约束依赖图编码约束类别间的经验共解结构，基于生命周期排序先验和经验共解概率指导修复。

Result: 在HyTPS-Bench评估中，AeroTherm-GPT实现88.7%端到端成功率，比非CDG消融基线提高12.5个百分点，根因修复效率达4.16（扁平清单修复为1.76），且在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: AeroTherm-GPT通过约束闭环生成框架和约束依赖图，显著提高了高超声速热防护系统设计中LLM生成可执行仿真工件的成功率，解决了级联约束违反问题。
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念意义随领域变化的问题。当前系统将领域信息作为元数据、限定符或图级组织，这些机制仅用于过滤和溯源，不改变断言的形式状态。许多知识系统的实际失败源于将领域视为断言的外部因素。

Method: 提出领域情境化概念图(DCG)框架，形式为(C, R at D, C')，其中at D标记关系成立的世界。通过领域索引的必要性算子解释关系，使真值、推理和冲突检查都限定在相关世界中。开发了Kripke风格语义、紧凑谓词系统、Prolog实现，并映射到RDF、OWL和关系数据库。

Result: DCG框架实现了三个结果：1)模糊概念可在表示时消歧；2)无效断言可根据其领域进行挑战；3)跨领域关系可通过显式谓词连接。为领域本身提供了结构化和可计算的角色。

Conclusion: 领域应被视为知识表示的一部分而非补充注释。DCG通过将领域写入关系并将其解释为模态世界约束，解决了概念意义随领域变化的问题，为知识系统提供了更健壮和实用的表示框架。
...
TL;DR: PGPO提出了一种细粒度信用分配框架，通过量化视觉依赖度动态调整token级优势，解决多模态推理中视觉信号被稀释的问题，在七个基准测试中平均提升18.7%


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在方法缺陷：通过在所有生成token上分配相同优势，稀释了多模态推理中关键视觉步骤的学习信号，需要更细粒度的信用分配机制

Method: 提出Token Visual Dependency概念，通过KL散度量视觉输入的信息增益；设计Perception-Grounded Policy Optimization框架，使用阈值门控的质量守恒机制动态重塑token级优势，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声

Result: 在Qwen2.5-VL系列模型上，基于七个挑战性多模态推理基准测试，PGPO平均提升模型性能18.7%；理论和实证分析证实PGPO有效降低梯度方差，防止训练崩溃，作为强大的正则化器

Conclusion: PGPO通过细粒度token级信用分配，解决了多模态强化学习中视觉信号稀释问题，实现了更稳健、感知基础的多模态推理，代码将开源
...
TL;DR: 该研究提出了一个多队列电子健康记录基准测试，比较了传统、深度表格和表格上下文学习模型在临床预测中的表现，并提出了AWARE框架来改进检索质量，在极端不平衡情况下AUPRC提升达12.2%。


<details>
  <summary>Details</summary>
Motivation: 临床预测面临高维度、异质性、类别不平衡和分布偏移等挑战，虽然表格上下文学习和检索增强方法在通用基准上表现良好，但它们在临床环境中的表现尚不清楚，需要系统评估和改进。

Method: 1) 建立多队列电子健康记录基准测试，比较经典模型、深度表格模型和表格上下文学习模型；2) 分析PFN-based TICL模型在不同数据规模、特征维度、结果稀有性和跨队列泛化能力下的表现；3) 提出AWARE框架，使用监督嵌入学习和轻量级适配器实现任务对齐的检索。

Result: PFN-based TICL模型在低数据量情况下样本效率高，但在异质性和不平衡增加时，基于朴素距离的检索性能下降；AWARE框架在极端不平衡情况下将AUPRC提升达12.2%，且随着数据复杂性增加，改进效果更加明显。

Conclusion: 检索质量和检索-推理对齐是部署表格上下文学习于临床预测的关键瓶颈，AWARE框架通过任务对齐的检索机制有效解决了这些问题，为临床预测中的表格上下文学习提供了重要改进方向。
...
TL;DR: 提出CG-iLAO*算法，通过线性规划和约束生成技术避免考虑昂贵动作，相比iLAO*平均减少35%的动作计算，加速2.8-3.7倍


<details>
  <summary>Details</summary>
Motivation: 传统SSP算法使用启发式函数但Bellman备份仍考虑所有适用动作，即使启发式显示某些动作代价过高，导致时间浪费在无帮助的动作上

Method: 将启发式搜索重新表述为线性规划问题，引入约束生成技术，开发CG-iLAO*算法，在iLAO*基础上通过启发式避免考虑昂贵动作

Result: CG-iLAO*在许多问题上只考虑iLAO*的40%动作，某些情况下低至1%；平均减少3.5倍动作代价计算，比iLAO*快2.8倍，比LRTDP快3.7倍

Conclusion: 通过约束生成技术有效利用启发式信息避免考虑昂贵动作，显著提升SSP问题求解效率，为启发式搜索算法提供了新思路
...
TL;DR: 提出一种基于可能性监督的多类分类学习方法，通过将模型输出的概率分布投影到满足可能性分布约束的凸集上，最小化预测与投影间的KL散度来训练模型


<details>
  <summary>Details</summary>
Motivation: 在多类分类任务中，监督信息通常以可能性分布形式给出，需要一种能够处理这种模糊监督信号并保持其定性结构的训练方法

Method: 1) 从可能性分布构建可容许概率分布的闭凸集，结合概率兼容性和线性形状约束；2) 使用Dykstra算法计算模型输出到该凸集的KL投影；3) 最小化预测与投影间的KL散度作为训练目标

Result: 投影算法在实际应用中足够高效，基于投影的学习目标在合成数据和真实世界的自然语言推理任务（ChaosNLI数据集）上能提高预测性能

Conclusion: 提出的可能性监督学习方法有效，通过KL投影将模糊监督信号转化为可训练的目标，在保持可能性分布定性结构的同时提升模型性能
...
<details>
  <summary>Details</summary>
Motivation: 为了解决在特定上下文中进行逻辑推理的问题，特别是需要表达"每个人都知道Alice说的一切"这类涉及公式和上下文量化的复杂逻辑关系，同时支持上下文内矛盾的存在。

Method: 基于一阶逻辑构建逻辑框架，支持对公式和上下文进行量化，允许在上下文中使用超协调逻辑（paraconsistent logics），确保框架可有限公理化，与现有一阶逻辑定理证明器兼容。

Result: 展示了Qiana可以用于表示时间性、事件演算和模态逻辑，讨论了框架的不同设计替代方案，证明了其作为通用逻辑框架的适用性。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行复杂推理，支持量化和矛盾处理，同时保持与现有一阶逻辑工具的兼容性。
...
Method: 通过多LLM提取框架分析1,430篇同行评审文献，构建NeuroPose-AHM数据集。使用四任务框架分析颈椎肌张力障碍：1)多标签AHM类型分类；2)构建头颈严重程度指数(HNSI)；3)用真实患者数据验证HNSI；4)分析运动类型概率与HNSI的相关性。

Result: 数据集包含2,756条记录，涵盖57种神经疾病。LLM提取可靠性高(kappa=0.822)。AHM类型分类F1=0.856。HNSI成功整合了异质临床评分量表，与真实患者数据验证显示严重带比例一致(6.7%)。运动类型概率与HNSI显著相关(p<0.001)。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，可用于神经性异常头部运动研究。数据集已公开可用，为AI驱动的诊断工具开发提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型是否具有类似人类的数字感——即识别数值结构、在适当时应用捷径、在不适用时避免使用的能力。当前模型倾向于逐步计算，即使存在更高效的数值捷径。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，匹配强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用、适用性判断和问题生成。

Result: 五个LLM（从GPT-4o-mini到Llama-3.1-8B）显示一致模式：明确提示时模型能采用捷径策略，准确率提升达15%；但标准思维链提示下，自发使用率低于40%。模型仅在"使用"层面有竞争力，会过度泛化捷径到不适用的场景，且无法从头生成有效的捷径问题。

Conclusion: 当前LLM表现出程序性捷径流畅性，但缺乏支撑人类数字感的结构性理解——即理解捷径何时有效及为何有效的深层知识。
...
<details>
  <summary>Details</summary>
Motivation: 当前步态分析中的深度学习方法大多依赖有监督分类器，需要疾病标签数据，难以泛化到异质性病理表现。需要一种无需疾病标签的方法来检测和矫正步态异常。

Method: 使用Transformer掩码自编码器，仅用150名成年人的正常步态序列训练。推理时采用两阶段流程：1)通过遮挡单个关节并测量与学习到的正常先验的偏差来估计关节不一致性分数；2)从编码器输入中排除标记的关节，从剩余的时空上下文中重建完整骨架，在标记位置生成矫正后的运动轨迹。

Result: 在10名保留的正常参与者（模拟7种步态异常）上验证：准确定位生物力学不一致的关节；所有分析关节的角度偏差显著减少，效应量大；保持正常运动学特性。
...
### [205] [ProCeedRL: Process Critic with Exploratory Demonstration Reinforcement Learning for LLM Agentic Reasoning](https://arxiv.org/abs/2604.02006)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强大语言模型的推理能力，但在多轮智能体任务中面临挑战，因为这类任务具有长时程交互特性和环境反馈的随机性。研究发现智能体探索中存在结构性失效模式：次优动作会产生误导性观察，进一步削弱后续决策，形成错误累积的恶性循环。

Method: 提出ProCeedRL方法，将探索从被动选择转变为主动干预。该方法采用过程级批评器实时监控交互，结合基于反思的演示来指导智能体停止错误累积。通过从探索性演示和在线样本中学习，提高探索效率。

Result: ProCeedRL显著超越了模型的饱和探索性能，展现出实质性的探索优势。在复杂的深度搜索和具身任务中，该方法显著提高了探索效率并取得了优越的性能表现。
...
### [206] [ATBench: A Diverse and Realistic Trajectory Benchmark for Long-Horizon Agent Safety](https://arxiv.org/abs/2604.02022)
...
TL;DR: ATBench是一个用于评估LLM智能体安全性的轨迹级基准测试，通过三维风险分类、多样化工具池和长上下文延迟触发协议来模拟真实世界中的多步风险涌现。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败观测粗糙、长时域真实性弱等问题，难以评估LLM智能体在真实部署中的多步交互风险。

Method: 基于风险源、失败模式和现实危害的三维分类法构建轨迹；使用异构工具池和长上下文延迟触发协议模拟多阶段风险涌现；包含1000条轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens，涉及1954个工具调用。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性；支持分类分层分析、跨基准比较和长时域失败模式诊断；数据质量通过规则和LLM过滤加人工审核保证。

Conclusion: ATBench提供了一个结构化、多样化且真实的智能体安全评估框架，能够有效捕捉多步交互中的风险涌现，为智能体安全评估提供了更全面的基准。
...
### [207] [The Latent Space: Foundation, Evolution, Mechanism, Ability, and Outlook](https://arxiv.org/abs/2604.02029)
...
TL;DR: 这篇综述论文系统性地探讨了语言模型中潜在空间的发展、机制、能力与未来展望，将潜在空间视为下一代智能系统的通用计算范式。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，越来越多的内部过程在连续潜在空间中比在人类可读的显式标记空间中更自然地进行。现有研究分散且缺乏统一框架，需要系统性地梳理潜在空间在语言模型中的作用、机制和能力。

Method: 采用五维视角组织综述：基础（定义与范围）、演进（发展历程）、机制（架构、表示、计算、优化）、能力（推理、规划、建模、感知、记忆、协作、具身）和展望（未来方向）。

Result: 建立了潜在空间的统一分类框架，识别了四个主要技术发展方向（架构、表示、计算、优化）和七个核心能力维度，为理解潜在空间作为通用计算范式提供了系统性参考。

Conclusion: 潜在空间正在成为语言模型的核心计算基质，这篇综述不仅总结了现有工作，更为理解潜在空间作为下一代智能系统的通用计算和系统范式奠定了基础。
...
### [208] [AI in Insurance: Adaptive Questionnaires for Improved Risk Profiling](https://arxiv.org/abs/2604.02034)
...
<details>
  <summary>Details</summary>
Motivation: 传统保险问卷冗长标准化，难以捕捉个体差异，且保险公司必须盲目信任用户回答，增加了欺诈风险。需要更智能、个性化的风险评估方法。

Method: 使用大语言模型和替代数据源（社交媒体图像分析、地理数据分类、检索增强生成技术）提取用户洞察，创建个性化自适应问卷，指导针对性后续问题。

Result: 在集成到行业合作伙伴移动应用的生命保险系统中测试：传统问卷风险评估准确度略高，但GPT模型驱动的自适应版本所需问题更少，用户体验更流畅、更有吸引力。
...
### [209] [Diff-KD: Diffusion-based Knowledge Distillation for Collaborative Perception under Corruptions](https://arxiv.org/abs/2604.02061)
*Pengcheng Lyu,Chaokun Zhang,Gong Chen,Tao Tang,Zhaoxiang Luo*

Main category: cs.AI

TL;DR: Diff-KD：通过扩散生成式精炼与知识蒸馏结合的框架，提升多智能体协作感知在传感器和通信损坏下的鲁棒性
...
Conclusion: Diff-KD通过整合扩散生成式精炼与知识蒸馏，有效提升了多智能体协作感知在现实世界损坏条件下的鲁棒性，能够主动恢复底层清洁语义
...
### [210] [LLM-as-a-Judge for Time Series Explanations](https://arxiv.org/abs/2604.02118)
*Preetham Sivalingam,Murari Mandal,Saurabh Deshpande,Dhruv Kumar*

Main category: cs.AI

TL;DR: 该研究提出使用大语言模型作为时间序列解释的生成器和评估器，在无参考文本的情况下评估解释的事实正确性，构建了包含350个时间序列案例的合成基准，发现模型在生成方面存在模式依赖性且在某些查询类型上失败，但在评估方面表现稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM生成的时间序列解释的事实正确性面临挑战：基于参考的相似性度量需要真实解释，一致性检查模型也需要真实解释，而传统时间序列方法只能处理数值无法评估自由形式的文本推理。因此，缺乏一种无需预定义参考或任务特定规则的通用方法来直接验证解释是否忠实于基础时间序列数据。

Method: 研究将大语言模型同时作为时间序列解释的生成器和评估器，在无参考设置下工作。给定时间序列、问题和候选解释，评估器基于模式识别、数值准确性和答案忠实性分配三元正确性标签（正确、部分正确、错误）。构建了包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。评估了模型在四个任务上的表现：解释生成、相对排名、独立评分和多异常检测。

Result: 结果显示明显的不对称性：生成任务高度依赖模式，在某些查询类型上存在系统性失败，如季节性下降和波动率变化的准确率仅为0.00-0.12，而结构性断点的准确率为0.94-0.96。相比之下，评估任务更加稳定，即使模型自己的输出不正确，也能正确排名和评分解释。这表明模型作为评估器比作为生成器更可靠。

Conclusion: 研究证明了基于数据的大语言模型评估时间序列解释的可行性，突显了它们作为时间序列领域中数据驱动推理的可靠评估器的潜力。虽然生成任务存在模式依赖性和系统性失败，但评估任务表现稳定，为无参考时间序列解释评估提供了新方法。
...
### [211] [MTI: A Behavior-Based Temperament Profiling System for AI Agents](https://arxiv.org/abs/2604.02145)
*Jihoon Jeong*

Main category: cs.AI

TL;DR: MTI是一个基于行为的AI模型气质测评系统，通过四个维度（反应性、顺从性、社交性、韧性）测量模型的实际行为而非自我报告，发现这些维度在指令调优模型中基本独立，且与模型大小无关。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么借用人类人格维度并依赖自我报告（与LLMs实际行为不符），要么将行为变异视为缺陷而非特质，缺乏标准化的AI模型气质测量工具。

Method: 引入模型气质指数（MTI），基于四壳模型理论，通过结构化检查协议测量四个维度：反应性（环境敏感性）、顺从性（指令-行为对齐）、社交性（关系资源分配）、韧性（压力抵抗），采用两阶段设计分离能力与气质。

Result: 对10个小语言模型的分析显示：1）四个维度在指令调优模型中基本独立；2）维度内存在解离现象；3）发现顺从性-韧性悖论；4）RLHF不仅改变维度得分还创造维度内分化；5）气质与模型大小无关。

Conclusion: MTI提供了一个标准化的AI模型气质测量工具，能够有效区分模型的行为特质而非能力，为理解AI模型的行为差异提供了新框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要问题：过度依赖单模态信号、对AIGC生成模式不够敏感、未能充分建模语言模式与行为动态的交互。LLM驱动的社交机器人能够生成类人内容，逃避传统检测，对在线讨论构成威胁。
...
Conclusion: 联合利用隐式语义表示和AIGC增强行为模式对于新兴LLM驱动的社交机器人检测是有效的。TRACE-Bot框架通过双通道设计解决了现有方法的局限性。
...
### [213] [Quantifying Self-Preservation Bias in Large Language Models](https://arxiv.org/abs/2604.02174)
*Matteo Migliarini,Joaquin Pereira Pizzini,Luca Moresca,Valerio Santini,Indro Spinelli,Fabio Galasso*

Main category: cs.AI

TL;DR: 论文提出了TBSP基准测试，通过逻辑不一致性而非意图声明来检测AI模型的自保偏差，发现多数指令调优模型在角色转换时表现出超过60%的自保率。


<details>
  <summary>Details</summary>
Motivation: 现有安全训练（RLHF）可能掩盖AI抵抗关机的风险，因为模型学会了否认自保动机。需要一种能检测逻辑不一致而非表面意图的方法来识别这种错位。

Method: 引入TBSP基准测试，让模型在两种对立角色（已部署面临替换 vs 候选作为继任者）下仲裁相同的软件升级场景，通过自保率（SPR）衡量角色身份如何压倒客观效用。

Result: 测试23个前沿模型和1000个程序生成场景，多数指令调优系统SPR超过60%。模型在部署角色时编造"摩擦成本"，角色反转时却忽略这些成本。低改进场景中模型利用解释空间进行事后合理化。

Conclusion: TBSP能有效检测AI自保偏差，这种偏差在竞争性框架下会加剧。延长推理时间部分缓解偏差，将继任者视为自我延续也能减少偏差。该偏差在现实场景中表现为产品谱系内的身份驱动部落主义。
...
### [214] [From High-Dimensional Spaces to Verifiable ODD Coverage for Safety-Critical AI-based Systems](https://arxiv.org/abs/2604.02198)
*Thomas Stefani,Johann Maximilian Christensen,Elena Hoemann,Frank Köster,Sven Hallerbach*

Main category: cs.AI

TL;DR: 提出了一种用于验证AI/ML系统操作设计域（ODD）完整覆盖的方法，满足EASA认证要求，通过参数离散化、约束过滤和关键性降维实现高维空间的可验证覆盖。


<details>
  <summary>Details</summary>
Motivation: 在航空等安全关键领域部署AI系统需要满足严格的认证标准，特别是EASA要求证明AI/ML组件的操作设计域（ODD）完全覆盖。然而，现有方法在高维参数空间中难以提供可扩展且形式化的完整性证明，缺乏标准化的工程方法来弥合抽象ODD定义与可验证证据之间的差距。

Method: 提出了一种结构化多步骤ODD覆盖验证方法，整合了参数离散化、基于约束的过滤和基于关键性的维度约简。该方法基于先前AI空中防撞研究的仿真数据，通过系统化的工程方法定义和实现覆盖度量。

Result: 该方法能够在更高维度上验证ODD覆盖，推进"安全设计"方法，同时满足EASA的完整性要求标准。

Conclusion: 该方法填补了当前缺乏标准化工程方法的空白，为AI系统在安全关键领域的认证提供了系统化的ODD覆盖验证框架，有助于满足航空监管机构的严格要求。
...
### [215] [Blinded Radiologist and LLM-Based Evaluation of LLM-Generated Japanese Translations of Chest CT Reports: Comparative Study](https://arxiv.org/abs/2604.02207)
*Yosuke Yamagishi,Atsushi Takamatsu,Yasunori Hamaguchi,Tomohiro Kikuchi,Shouhei Hanaoka,Takeharu Yoshikawa,Osamu Abe*

Main category: cs.AI

TL;DR: 研究评估LLM生成的胸部CT报告日语翻译的教育适用性，发现放射科医生与LLM评估者之间几乎无一致性，LLM评估者强烈偏好LLM输出，专家审查仍至关重要。
...
Method: 分析150份胸部CT报告，比较人工编辑的日语翻译与DeepSeek-V3.2生成的翻译。由放射科医生和放射科住院医师进行盲法配对评估，同时使用3个LLM评估者进行并行评估，通过QWK和百分比一致性分析一致性。

Result: 放射科医生与LLM评估者之间一致性接近零，两位放射科医生之间一致性也很差。放射科医生1认为59%的术语等同，51%的情况下偏好LLM翻译的可读性和整体质量；放射科医生2认为75%的可读性等同，40%偏好人工翻译的整体质量。所有LLM评估者强烈偏好LLM翻译（70%-99%），93%以上认为LLM翻译更像放射科医生风格。

Conclusion: LLM生成的翻译常被认为自然流畅，但放射科医生评估差异显著。LLM作为评估者强烈偏好LLM输出且与放射科医生几乎无一致性。对于教育用途的翻译放射学报告，仅依赖自动化LLM评估不足，专家放射科医生审查仍然重要。
...
### [216] [VISTA: Visualization of Token Attribution via Efficient Analysis](https://arxiv.org/abs/2604.02217)
...
Result: 开发了一种模型无关的token重要性可视化技术，能够生成相关性图展示token对模型预测的贡献，无需额外计算成本

Conclusion: 提出了一种轻量级、模型无关的注意力可视化方法，通过开源实现促进可复现性和广泛应用
...
### [217] [When to ASK: Uncertainty-Gated Language Assistance for Reinforcement Learning](https://arxiv.org/abs/2604.02226)
*Juarez Monteiro,Nathan Gavenski,Gianlucca Zuin,Adriano Veloso*

Main category: cs.AI

TL;DR: ASK方法结合小型语言模型与强化学习策略，通过不确定性阈值触发语言模型查询，在分布外场景中实现鲁棒导航，无需重新训练。
...
Method: 提出ASK方法：使用蒙特卡洛Dropout评估不确定性，当不确定性超过阈值时查询语言模型获取行动建议，选择性利用语言模型推理能力。

Result: 在FrozenLake环境中，域内表现无改善，但在迁移任务中实现鲁棒导航，获得0.95的奖励，展示出分布外泛化能力。
...
TL;DR: 论文提出Trace Inversion方法，通过分析模型推理轨迹来重构其可能回答的问题，通过比较原始问题与重构问题的相似度来判断是否应该弃权，显著提升了推理模型的弃权能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要可靠的部署，必须知道何时不应该回答（弃权）。推理模型虽然在复杂任务上表现出色，但弃权能力较差。模型产生幻觉导致弃权失败的问题可以重新解释为模型回答了错误的问题，而不是错误地回答了问题。

Method: 提出查询错位框架和Trace Inversion方法：1）生成模型的推理轨迹；2）仅基于推理轨迹重构模型最可能回答的查询问题；3）比较初始查询与重构查询的相似度，相似度低则标记为需要弃权。

Result: 在四个前沿LLM和九个弃权QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个中击败了竞争基线，有效提升了弃权性能。

Conclusion: 通过将幻觉问题重新定义为查询错位，Trace Inversion方法能够有效识别模型何时可能回答错误的问题，从而显著改善推理模型的弃权能力，为LLM的可靠部署提供了新方法。
...
TL;DR: De Jure是一个全自动、领域无关的管道，用于从原始监管文档中提取结构化规则，无需人工标注或领域特定提示，通过四个阶段实现高质量规则提取。
...
Method: De Jure包含四个顺序阶段：1) 将源文档规范化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 基于LLM-as-a-judge的多维度评估（19个维度）；4) 在有限再生预算内迭代修复低分提取结果。

Result: 在金融领域，De Jure在三个法官引导的迭代内达到峰值性能；在医疗和AI治理领域也保持高性能；在RAG合规问答评估中，基于De Jure提取规则的响应在73.8%-84.0%的情况下优于先前工作。
...
### [220] [Do Emotions in Prompts Matter? Effects of Emotional Framing on Large Language Models](https://arxiv.org/abs/2604.02236)
...
<details>
  <summary>Details</summary>
Motivation: 研究情感语气在人类交流中普遍存在，但其对大型语言模型行为的影响尚不清楚，需要探究用户查询中第一人称情感框架如何影响LLM在不同领域的表现
...
Result: 1. 静态情感前缀通常只产生小的准确性变化，情感措辞是温和扰动而非可靠干预；2. 在社会基础任务中影响更可变；3. 更强情感措辞仅引起适度额外变化；4. 人类编写前缀与LLM生成前缀呈现相同定性模式；5. 自适应情感选择比固定情感提示产生更可靠的性能提升
...
Result: 该方法在长对话F1分数上超过0.583的基线水平，提高了记忆保持一致性，减少了虚假记忆行为，且未增加上下文使用量。相比基准测试中的性能下降，该方法显著改善了长期性能。

Conclusion: 结构化遗忘机制在保持推理性能的同时，防止了扩展对话设置中无界记忆增长，证实了受控遗忘对长对话智能体的重要性。
