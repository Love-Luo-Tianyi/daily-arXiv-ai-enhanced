...
TL;DR: RCP是一种新的视觉语言模型剪枝框架，通过累积视觉token剪枝和延迟修复机制，在保持性能的同时大幅减少计算成本
...
Method: 提出表示一致性剪枝器(RCP)，包含跨注意力剪枝器和延迟修复适配器。跨注意力剪枝器利用LLM的内在注意力作为基线预测累积掩码，确保跨层的一致单调token减少。延迟修复适配器缓存被剪枝token的本质，并应用FiLM调制到答案生成token。使用修复损失匹配剪枝表示与全token教师的一阶和二阶统计量

Result: 在LVLM基准测试中，RCP移除了高达88.9%的视觉token，减少了高达85.7%的FLOPs，平均准确率仅轻微下降，并在多个广泛使用的基准上优于先前无需微调原始模型的方法

Conclusion: RCP通过累积视觉token剪枝和延迟修复机制，有效解决了LVLM推理成本高的问题，在保持性能的同时大幅减少计算开销，优于现有方法
...
TL;DR: Video-MME-v2是一个新的视频理解基准测试，通过三级渐进式层次结构和基于组的非线性评估策略，系统评估模型的鲁棒性和忠实性，揭示了当前模型与人类专家之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准测试存在饱和问题，排行榜分数与实际模型能力之间存在严重脱节，需要更严格的评估标准来反映真实世界的能力。

Method: 1. 设计三级渐进式层次结构：从多点视觉信息聚合，到时序动态建模，再到复杂多模态推理；2. 提出基于组的非线性评估策略，强调相关查询的一致性和多步推理的连贯性；3. 通过严格的人工标注流程构建数据集，涉及12名标注员和50名独立评审员，投入3300人时，经过5轮质量保证。

Result: 实验显示当前最佳模型Gemini-3-Pro与人类专家之间存在显著差距；发现清晰的层次瓶颈，即视觉信息聚合和时序建模的错误会传播并限制高层推理；基于思考的推理高度依赖文本线索，在有字幕时性能提升，但在纯视觉场景中有时会下降。
...
<details>
  <summary>Details</summary>
Motivation: 人类在身份识别方面具有卓越的选择性敏感度，能够区分高度相似的身份，而视觉模型在这方面表现不佳。身份相关任务（如个性化图像生成）缺乏专门的身份评估指标，阻碍了进展

Method: 1. 构建高质量训练集：包含多样真实世界领域的图像，并使用生成式合成数据进行增强，提供受控的细粒度身份和上下文变化
2. 设计ID-Sim前馈度量标准
3. 创建统一评估基准：用于评估身份相关识别、检索和生成任务与人类标注的一致性

Result: 提出了ID-Sim度量标准和相应的评估基准，但没有提供具体的实验结果数据

Conclusion: ID-Sim旨在促进身份相关视觉任务的进展，通过提供反映人类选择性敏感度的评估指标来帮助改进模型性能
...
TL;DR: SVAgent是一个基于故事情节引导的多智能体框架，用于视频问答任务，通过模拟人类故事情节推理来提升视频理解性能
...
Result: 实验结果表明SVAgent通过模拟人类类似的故事情节推理，在视频理解中实现了优越的性能和可解释性
...
Method: 使用三通道DDPM模型，前两通道分别存储CC和MLO视图，第三通道编码两者绝对差异以引导学习解剖关系；基于Hugging Face预训练DDPM在私有筛查数据集上微调

Result: 差异编码有助于保持跨视图的全局乳腺结构，生成的合成CC-MLO配对与真实采集相似；几何一致性评估和分布比较验证了模型效果

Conclusion: 证明了差异引导DDPM同时生成双视图乳腺X线片的可行性，具有数据集增强和跨视图感知AI应用的潜力
...
TL;DR: 研究发现当前视频理解基准存在严重问题：40-60%的问题仅通过文本线索即可回答，缺乏视觉基础。提出VidGround解决方案，仅使用真正需要视觉基础的问题进行后训练，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频理解方面进展缓慢，研究发现主要原因是常用基准和训练数据中存在大量仅需文本线索即可回答的问题，缺乏真正的视觉基础需求，这限制了模型视频理解能力的提升。

Method: 提出VidGround方法：1）识别并筛选出真正需要视觉基础的问题；2）仅使用这些无语言偏见的问题进行后训练；3）结合基于强化学习的后训练算法，使用更少但更高质量的数据。

Result: 使用仅69.1%的原始后训练数据，性能提升高达6.2个百分点。简单的数据筛选配合基础后训练算法，优于多种复杂后训练技术，证明数据质量是提升视频理解能力的主要瓶颈。

Conclusion: 数据质量比算法复杂度更重要。必须构建真正需要视觉基础的后训练数据和评估基准，才能推动视觉语言模型视频理解能力的实质性发展。
...
TL;DR: NeuroQuant是一个用于多模态脑MRI重建的3D向量量化变分自编码器，通过模态感知和解剖学基础的方法，结合因子化多轴注意力学习跨模态共享潜在表示，实现优于现有VAE的重建效果。
...
Method: 1. 使用因子化多轴注意力学习跨模态共享潜在表示；2. 采用双流3D编码器，明确分离模态不变解剖结构和模态依赖外观的编码；3. 解剖编码通过共享码本离散化，解码阶段通过FiLM与模态特定外观特征结合；4. 采用联合2D/3D训练策略以适应3D MRI数据的切片采集特性。

Result: 在两个多模态脑MRI数据集上的广泛实验表明，NeuroQuant相比现有VAE实现了更优的重建保真度，为下游生成建模和跨模态脑图像分析提供了可扩展的基础。

Conclusion: NeuroQuant是一个模态感知、解剖学基础的3D VQ-VAE，能够有效重建多模态脑MRI，为医学图像分析中的生成建模和跨模态分析提供了强大的基础模型。
...
TL;DR: MIRAGE框架通过训练免费的多分支并行去噪策略，解决多实例图像编辑中的过度编辑和空间错位问题，在复杂指令下实现精确的实例级修改。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型（如FLUX.2和Qwen-Image-Edit）在处理多相似实例的复杂场景时存在严重过度编辑和空间错位问题，特别是在面对多个相同实例和复合指令时表现不佳。

Method: 提出MIRAGE框架：1）使用视觉语言模型将复杂指令解析为区域子集；2）采用多分支并行去噪策略，将目标区域的潜在表示注入全局表示空间；3）通过参考轨迹保持背景完整性。

Result: 在MIRA-Bench和RefEdit-Bench上的广泛评估表明，MIRAGE框架在实现精确实例级修改的同时显著优于现有方法，同时保持了背景一致性。

Conclusion: MIRAGE通过训练免费的方法有效解决了多实例图像编辑中的关键挑战，为复杂指令下的精确区域对齐编辑提供了有效解决方案，并发布了专门的基准测试和代码。
...
TL;DR: LSRM通过大幅扩展transformer上下文窗口，使用稀疏注意力机制实现高质量3D重建和逆渲染，性能超越现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于对象中心的feed-forward 3D重建方法虽然稳健，但在恢复精细纹理和外观方面仍落后于密集视角优化方法。研究者希望通过扩展transformer上下文窗口来缩小这一差距。

Method: 采用稀疏注意力架构设计，包含三个关键创新：(1) 从粗到细的高效管道，通过预测稀疏高分辨率残差聚焦计算在信息丰富区域；(2) 基于显式几何距离而非标准注意力分数的3D感知空间路由机制；(3) 使用All-gather-KV协议的自定义块感知序列并行策略，平衡GPU间的动态稀疏工作负载。

Result: LSRM能处理比先前SOTA方法多20倍的对象token和2倍以上的图像token。在新视角合成基准测试中，PSNR提高2.5 dB，LPIPS降低40%。在逆渲染任务中，纹理和几何细节表现一致优于现有方法，LPIPS匹配或超越SOTA密集视角优化方法。

Conclusion: 扩展transformer上下文窗口能显著提升3D重建质量，稀疏注意力架构设计是实现高效大规模上下文处理的关键。LSRM在3D重建和逆渲染任务中均取得显著性能提升。
...
TL;DR: 本文提出了一种无需训练的适配器融合方法，利用正交微调（OFT）的几何特性，通过结构化正交参数化和近似测地线公式，将针对不同任务训练的正交适配器合并为单一适配器。


<details>
  <summary>Details</summary>
Motivation: 在模型训练快速发展的背景下，参数高效微调和少量数据适应特定任务的技术备受关注。然而，如何将针对不同任务训练的多个适配器合并为一个能够同时在多个任务上表现良好的单一适配器，特别是生成模型中主题和风格适配器的融合问题，仍然没有得到解决。

Method: 1. 利用正交微调（OFT）的结构化正交参数化及其几何特性，推导出无需训练的适配器融合公式；2. 推导了最近提出的Group-and-Shuffle（GS）正交矩阵形成的流形结构，并获得了两个点之间测地线近似的高效公式；3. 提出了"光谱恢复"变换，用于恢复合并后适配器的光谱特性以提高融合质量。

Result: 在主题驱动生成任务上的实验表明，该技术能够成功融合不同适配器的概念和风格特征。据作者所知，这是第一个无需训练的乘法正交适配器融合方法。

Conclusion: 通过利用正交微调的几何特性，本文提出了一种有效的无需训练的正交适配器融合方法，能够将针对不同任务训练的适配器合并为单一适配器，在生成任务中成功融合概念和风格特征。
...
TL;DR: 提出检测引导的小型视觉语言模型框架，通过目标检测引导多模态推理，在保持效率的同时提升建筑工地危险识别准确率


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型计算需求高，难以实时应用；小型视觉语言模型效率高但准确率低且易产生幻觉，需要解决这一权衡问题

Method: 提出检测引导的sVLM框架：先用YOLOv11n检测工人和施工机械，将检测实体嵌入结构化提示，引导sVLMs进行空间基础的危险评估

Result: 框架在所有模型上都提升了危险检测性能，最佳模型Gemma-3 4B的F1分数从34.5%提升到50.6%，解释质量BERTScore F1从0.61提升到0.82，推理仅增加2.5ms开销

Conclusion: 轻量级目标检测与小型VLM推理结合为上下文感知的建筑安全危险检测提供了有效高效的解决方案
...
TL;DR: 提出用于神经影像中异质网格分析的层次化Transformer框架，支持任意拓扑的体素和表面网格，通过特征投影整合多尺度顶点特征，在阿尔茨海默病等任务上取得SOTA结果
...
Method: 提出层次化Transformer框架，基于任意阶单纯复形构建空间自适应树分区，支持体素和表面离散化；特征投影模块将顶点级临床描述符映射到空间层次中；通过掩码重建坐标和形态测量通道进行自监督预训练

Result: 在ADNI的体素脑网格上进行阿尔茨海默病分类和淀粉样蛋白负荷预测，以及在MELD数据集的皮质表面网格上进行局灶性皮质发育不良检测，在所有基准测试中都取得了最先进的结果

Conclusion: 该框架能够有效处理神经影像中的异质网格数据，整合多种形态测量特征，为跨成像流程的临床应用提供了统一的解决方案
...
TL;DR: 提出一种人机协同框架，通过预训练分类器指导采样，自适应选择信息量最大的点进行人工标注，高效估计目标源的两点相关函数，显著降低方差并减少标注工作量。


<details>
  <summary>Details</summary>
Motivation: 在天文学等应用中，两点相关函数通常只针对满足特定属性的子集（如星团）进行测量，而构建这些目标源的完整目录需要大量人工标注，耗时耗力。需要一种更高效的方法来估计目标源的两点相关函数。

Method: 提出人机协同框架：1）利用预训练分类器指导采样过程；2）自适应选择信息量最大的点进行人工标注；3）每次标注后，同时为多个距离区间生成无偏的点对计数估计；4）引入新的无偏估计器、采样策略和置信区间构建方法。

Result: 与简单的蒙特卡洛方法相比，该方法在显著降低方差的同时，大幅减少了人工标注工作量。能够实现可扩展且统计可靠的天文数据集两点相关函数测量。

Conclusion: 该框架通过结合预训练分类器的引导和自适应采样策略，实现了高效、统计可靠的两点相关函数估计，为天文数据分析提供了一种可扩展的解决方案。
...
TL;DR: 提出一个负责任的计算框架，用条件图像合成生成标记良好的合成图像替代敏感抗议图像，在分析集体抗议动态的同时降低个人隐私风险。


<details>
  <summary>Details</summary>
Motivation: 抗议相关的社交媒体数据对理解集体行动有价值，但存在高风险，因为AI系统能够识别个人、推断敏感属性、跨平台交叉引用视觉信息，导致监控风险。现有抗议分析方法缺乏整合隐私风险评估、下游分析和公平性考虑的整体流程。

Method: 提出负责任计算框架，使用条件图像合成技术将敏感抗议图像替换为标记良好的合成再现图像，使分析集体模式时无需直接暴露可识别个体。

Result: 该方法能生成真实多样的合成图像，在保持下游分析效用的同时降低隐私风险，并评估生成数据的群体公平性，确保合成表示不会对特定子群体造成不成比例的影响。

Conclusion: 该方法采用务实的危害缓解策略，不提供绝对隐私保证，但能在承认残余风险的同时实现社会敏感分析，平衡隐私保护与分析需求。
...
TL;DR: COVER方法通过最小化Fisher信息增益的近似值来选择信息丰富的相机视角，提出轻量级的基于覆盖度的视角选择指标，在多种数据集和辐射场基准上优于现有主动视角选择方法。


<details>
  <summary>Details</summary>
Motivation: 用于学习3D重建的数据质量对高效准确的场景建模至关重要，需要解决主动视角选择问题，寻找能够选择信息丰富相机姿态的原则性分析方法。

Method: 提出COVER（Camera Optimization for View Exploration and Reconstruction）方法，通过最小化Fisher信息增益的易处理近似值来选择视角，简化为偏好覆盖过去相机观察不足的几何结构的视角，避免昂贵的透射率估计。

Result: 在Nerfstudio框架中集成该方法，在固定和嵌入式数据采集场景的真实数据集上评估，在多个数据集和辐射场基准上，相比最先进的主动视角选择方法持续改善重建质量。

Conclusion: COVER提供了一种轻量级、鲁棒的视角选择指标，能够有效选择信息丰富的相机视角，显著提升3D重建质量，为主动视角选择问题提供了原则性解决方案。
...
TL;DR: Region-R1：一个多模态检索增强生成中的查询端区域裁剪框架，通过动态裁剪问题相关区域来减少视觉干扰，提升重排序性能


<details>
  <summary>Details</summary>
Motivation: 传统多模态检索增强生成中的重排序器通常将整个查询图像作为全局嵌入处理，容易受到背景杂乱等视觉干扰的影响，导致相似性评分偏差。需要一种能够聚焦问题相关区域的方法来提升重排序准确性。

Method: 提出Region-R1框架，将区域选择建模为决策问题，在重排序过程中学习保留完整图像或仅关注问题相关区域。采用新颖的区域感知组相对策略优化（r-GRPO）学习动态裁剪判别性区域的策略。

Result: 在两个挑战性基准测试E-VQA和InfoSeek上，Region-R1实现了持续的性能提升，达到了最先进水平，条件召回率@1最高提升20%。

Conclusion: 查询端自适应是增强多模态检索增强生成重排序的简单而有效的方法，Region-R1展示了通过动态区域裁剪减少视觉干扰、提升检索相关性的巨大潜力。
...
TL;DR: 本文介绍了UFPR-VeSV数据集，这是一个包含24,945张车辆图像的真实世界监控数据集，具有13种颜色、26个品牌、136个型号和14种类型的细粒度标注，用于改进细粒度车辆分类和车牌识别任务。


<details>
  <summary>Details</summary>
Motivation: 现有车辆识别研究通常假设理想条件、探索有限属性，并且忽视了细粒度车辆分类与车牌识别的整合。为了解决这些不足，需要创建包含多样化真实世界条件的数据集。

Method: 从巴西巴拉那州军事警察监控系统收集24,945张车辆图像，标注13种颜色、26个品牌、136个型号和14种类型，所有标注都通过车牌信息验证。使用五种深度学习模型进行基准测试，并应用两种光学字符识别模型进行车牌识别。

Result: 数据集包含16,297辆独特车辆，涵盖部分遮挡、夜间红外成像和不同光照条件。基准测试揭示了处理多色车辆、红外图像以及区分共享共同平台的车辆模型等具体挑战。联合使用FGVC和ALPR显示出实际应用的潜力。

Conclusion: UFPR-VeSV数据集为细粒度车辆分类和车牌识别研究提供了具有挑战性的真实世界基准，展示了整合这两种互补任务对于智能交通系统和刑事调查等实际应用的潜力。
...
TL;DR: 论文提出了一种评估视觉嵌入模型身份泄露风险的基准测试方法，并开发了一种线性投影技术来移除身份信息同时保持非生物特征实用性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉嵌入模型（如CLIP、DINOv2/v3、SSCD）在人脸数据上使用时存在未测量的身份信息泄露风险，且缺乏可部署的缓解措施，需要建立攻击者感知的隐私评估框架。

Method: 1）建立包含开放集验证、扩散模板反演检查和基于等面积扰动的人脸上下文归因的基准测试；2）提出一次性线性投影器ISP，通过移除估计的身份子空间来保护隐私，同时保留实用性所需互补空间。

Result: 在CelebA-20和VGGFace2数据集上，CLIP的身份泄露相对高于DINOv2/v3和SSCD；ISP能将线性访问降至接近随机水平，同时保持高非生物特征实用性，且在不同数据集间具有良好的迁移性。

Conclusion: 该研究首次建立了非人脸识别编码器的攻击者校准面部隐私审计框架，证明线性子空间移除能在保持视觉搜索和检索实用性的同时提供强大的隐私保障。
...
TL;DR: 提出SmokeGS-R方法，通过解耦几何恢复与外观校正，结合物理引导的伪清洁监督和渲染后外观协调，在烟雾场景的3D重建中取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 真实烟雾场景同时衰减场景辐射、增加空气光并破坏多视角外观一致性，使得鲁棒的3D重建特别困难，需要专门的方法来处理这些挑战

Method: 采用几何恢复与外观校正解耦策略：1）使用改进的暗通道先验和引导滤波生成物理引导的伪清洁监督；2）训练干净的3D高斯溅射源模型；3）通过几何平均参考聚合、LAB空间Reinhard转移和轻量高斯平滑进行渲染后外观协调

Result: 在官方测试排行榜上获得PSNR=15.217和SSIM=0.666；在RealX3D公开后重新评估，获得PSNR=15.209、SSIM=0.644和LPIPS=0.551，比最强官方基线平均提升+3.68 dB PSNR

Conclusion: 几何优先的重建策略结合稳定的渲染后外观协调是处理真实世界多视角烟雾恢复的有效方法，该方法在烟雾场景的3D重建中表现出色
...
TL;DR: 提出一种在3D高斯溅射场景中进行目标室内资产对象级检测与分割的方法，通过3D对象码本和多视角掩码关联实现准确的三维物体实例化


<details>
  <summary>Details</summary>
Motivation: 针对从360度无人机图像重建的3D高斯溅射场景，需要实现目标室内资产的准确对象级检测和分割，解决多视角掩码关联和三维物体实例化的问题

Method: 引入3D对象码本，联合利用掩码语义和对应高斯基元空间信息指导多视角掩码关联；集成2D对象检测与分割模型，通过语义和空间约束的合并程序将多视角掩码聚合成一致的3D对象实例

Result: 在两个大型室内场景实验中，实现了可靠的多视角掩码一致性，F1分数比最先进基线提升65%；实现了准确的物体级3D室内资产检测，mAP比基线方法提升11%

Conclusion: 该方法能有效解决3D高斯溅射场景中的室内资产检测与分割问题，通过语义和空间约束的多视角掩码关联显著提升了检测精度和一致性
...
TL;DR: VLA-InfoEntropy方法通过图像熵和注意力熵量化视觉和语义信息，结合时间步信息实现动态注意力转移，减少冗余计算，提升VLA模型推理效率
...
Method: 使用图像熵量化视觉token的灰度分布特征，引入注意力熵捕捉任务相关文本的注意力分数分布，结合时间步信息实现从全局视觉特征到注意力引导局部信息区域的动态转移策略

Result: 方法减少了推理参数，加速了推理速度，并在实验中优于现有方法
...
TL;DR: 提出首个无监督多智能体与单智能体感知统一框架UMS，通过多智能体协作提升点云密度和提供跨视角指导，无需人工标注实现3D目标检测


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时以无监督方式解决多智能体和单智能体感知问题，而多智能体间的传感器数据共享能提供两个关键优势：提升点云密度和提供跨视角指导

Method: 提出UMS框架：1) 基于学习的提案净化过滤器改善多智能体点云密度协作后的候选提案分类；2) 渐进式提案稳定模块通过易到难课程学习生成可靠伪标签；3) 跨视角共识学习利用多智能体协作视角指导单智能体视角检测

Result: 在V2V4Real和OPV2V两个公开数据集上，UMS方法在无监督设置下的多智能体和单智能体感知任务中，3D检测性能显著优于现有最先进方法

Conclusion: 通过多智能体协作无需人工标注即可同时解决多智能体和单智能体感知问题，为机器人环境理解提供了有效的无监督解决方案
...
TL;DR: 提出了一种多线索引导的局部特征学习框架，通过语义和几何线索协同增强检测鲁棒性和描述符区分度，包含语义-法线联合预测头、深度稳定性预测头、语义-深度感知关键点机制和统一三线索融合模块。


<details>
  <summary>Details</summary>
Motivation: 现有局部特征检测和描述方法主要依赖单一外观线索进行建模，导致关键点不稳定和描述符区分度不足。需要利用多线索信息来协同提升特征检测的鲁棒性和描述符的区分能力。

Method: 1. 构建联合语义-法线预测头和深度稳定性预测头；2. 提出语义-深度感知关键点机制，通过耦合语义可靠性和深度稳定性重新加权关键点响应；3. 设计统一三线索融合模块，采用语义调度门控机制自适应注入多属性特征。

Result: 在四个基准测试上进行了广泛实验，验证了所提框架的有效性。源代码和预训练模型将在GitHub上公开。

Conclusion: 提出的多线索引导局部特征学习框架通过协同利用语义和几何线索，有效提升了特征检测的鲁棒性和描述符的区分度，为解决传统单线索方法的局限性提供了新思路。
...
TL;DR: 该论文将红外小目标检测重新定义为质心回归任务，提出单点监督的红外概率响应编码方法SPIRE，通过点响应先验监督和高分辨率概率编码器实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素级监督的编码器-解码器分割范式虽然性能不错，但忽略了小目标仅占少数像素且边界模糊的事实。作者认为红外小目标检测的首要原则应是目标定位而非分离所有目标区域。
...
Conclusion: 将红外小目标检测重新定义为质心回归任务是有效的，SPIRE方法通过单点监督和概率响应编码解决了稀疏目标分布下的优化不稳定问题，在保持检测性能的同时大幅降低了计算复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅、NeRF或基于transformer的3D重建模型压缩方法都需要通过场景特定微调学习数据依赖的码本，这种方法既耗时又低效

Method: 利用参数向量在高维空间中的分布特性，通过随机旋转将输入转换为已知Beta分布坐标，使用预计算的Lloyd-Max量化，结合维度相关准则预测量化比特宽度、范数分离边界连接量化误差与渲染质量、二维哈希网格特征分组策略，以及可组合的剪枝-量化流水线

Result: 在NeRF Synthetic数据集上，3DTurboQuant将3DGS压缩3.5倍（PSNR损失仅0.02dB），将DUSt3R KV缓存压缩7.9倍（点云保真度39.7dB），无需训练、码本学习或校准数据，压缩仅需数秒

Conclusion: 证明了无需数据依赖码本学习的3D重建模型压缩是可行的，预计算量化方法在信息理论下界2.7倍内达到近最优性能，为3D模型压缩提供了高效实用的解决方案
...
TL;DR: UAVReason是首个面向无人机俯视场景的统一大规模多模态基准，包含27.3万VQA样本，涵盖22种推理类型，通过多任务学习建立强基线模型，显著提升无人机场景性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在无人机高空场景中表现不佳，主要原因是显著的领域偏移：物体微小密集、纹理重复、俯视视角模糊。这些因素严重干扰语义理解，阻碍空间推理和可控生成。

Method: 1. 从高保真无人机仿真平台构建UAVReason基准，包含27.3万VQA样本（23.6万单帧、68.2万双帧时序、188.8万跨模态生成）；2. 设计22种空间和时间推理类型；3. 建立统一的多任务学习基线模型，同时处理VQA、分割和生成任务。

Result: 实验验证了统一方法在多种指标上的有效性：VQA的EM/F1、分割的mIoU、生成的CLIP Score。结果显示通用领域视觉语言模型存在局限，而统一多任务学习显著提升了无人机原生性能。

Conclusion: UAVReason填补了无人机俯视场景多模态基准的空白，展示了统一多任务学习方法的优势，为无人机多模态研究提供了重要资源。所有数据、代码和评估工具将公开。
...
TL;DR: 论文提出对象锚定组合图像检索（OACIR）新任务，强调实例级一致性而非语义匹配，并构建首个大规模真实世界图像基准OACIRR，提出AdaFocal框架通过上下文感知注意力调制器增强指定实例区域的注意力。


<details>
  <summary>Details</summary>
Motivation: 传统组合图像检索（CIR）主要关注语义匹配，难以在不同上下文中可靠检索用户指定的具体实例。实际应用中，强调具体实例保真度通常比宽泛语义匹配更重要，因此需要一种能确保严格实例级一致性的细粒度检索任务。

Method: 提出对象锚定组合图像检索（OACIR）任务，构建包含超过16万四元组和四个挑战性候选库的OACIRR基准。提出AdaFocal框架，采用上下文感知注意力调制器，自适应地增强指定实例区域的注意力，动态平衡锚定实例和更广泛组合上下文之间的关注。

Result: AdaFocal在OACIR任务上显著优于现有组合检索模型，特别是在保持实例级保真度方面表现突出，为这一挑战性任务建立了强大的基线。

Conclusion: OACIR任务和AdaFocal框架为更灵活、实例感知的检索系统开辟了新方向，解决了传统CIR在实例级一致性方面的局限性，推动了细粒度图像检索的发展。
...
TL;DR: LSGS-Loc是一种针对大规模无人机场景的视觉定位新方法，通过尺度感知的位姿初始化和拉普拉斯可靠性掩码机制，解决了现有3DGS方法在大规模场景中的位姿初始化困难和渲染伪影敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 大规模无人机场景中的视觉定位对自主系统至关重要，但现有基于3D高斯泼溅（3DGS）的定位方法在大规模场景中面临位姿初始化困难和渲染伪影敏感性的挑战。

Method: 提出了LSGS-Loc视觉定位流水线，包含两个核心创新：1）尺度感知位姿初始化策略，结合场景无关的相对位姿估计和显式的3DGS尺度约束；2）拉普拉斯可靠性掩码机制，在光度细化中引导优化朝向高质量区域，减少重建伪影影响。

Result: 在大规模无人机基准测试上的大量实验表明，该方法在无序图像查询中实现了最先进的精度和鲁棒性，显著优于现有的3DGS方法。

Conclusion: LSGS-Loc通过创新的尺度感知初始化和可靠性掩码机制，有效解决了大规模3DGS场景中的视觉定位挑战，为无人机自主系统提供了更可靠的定位能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR-4D雷达融合方法主要依赖固定或弱自适应管道，无法根据环境条件变化动态调整模态偏好。在恶劣天气条件下，不同传感器的可靠性会发生变化，需要更智能的融合策略。
...
Result: 在K-Radar基准测试中实现了最先进的性能，提供了明确且高度可解释的模态偏好洞察，透明地展示了自适应路由如何在不同的恶劣天气场景中稳健地在LiDAR和4D雷达之间切换依赖关系。

Conclusion: 该方法通过天气条件引导的自适应分支路由，显著提高了恶劣天气下3D目标检测的鲁棒性，同时提供了对模态偏好的可解释性洞察，为多模态感知系统设计提供了新思路。
...
TL;DR: CRISP提出了一种基于排序而非概率的医学图像分割新框架，通过"正区域排序稳定性"定律和潜在特征扰动，在无需目标域信息的情况下实现分布偏移下的鲁棒分割。
...
Method: 基于"正区域排序稳定性"定律，提出CRISP框架：1)通过潜在特征扰动模拟分布偏移下的模型行为；2)识别保持高概率的区域（注定阳性）和保持低概率的区域（可安全分类为阴性）；3)构建高精度和高召回先验，在扰动下递归优化；4)设计迭代训练框架，使高精度和高召回先验逐步"挤压"到最终分割结果。

Result: 在多中心心脏MRI和CT肺血管分割上的广泛评估显示，CRISP在鲁棒性方面显著优于最先进方法，在多中心、人口统计和模态偏移下分别实现了高达0.14像素（7.0%改进）、1.90像素（13.1%改进）和8.39像素（38.9%改进）的HD95降低。

Conclusion: CRISP是首个基于排序而非概率的分割框架，无需目标域信息，在分布偏移下表现出卓越的鲁棒性，为医学AI的临床翻译提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 小麦病害分割面临严重挑战：不同生长阶段存在显著的类内时间变化，导致外观差异巨大。收集代表性训练数据集既费力又不切实际，特别是在数据受限的情况下。

Method: SGPer将小麦病害分割视为疾病特定语义感知和疾病边界定位的耦合任务。核心是利用预训练的DINOv2提供鲁棒的类别感知语义先验来处理外观变化，并将其转换为粗空间提示来指导SAM精确定位疾病边界。具体包括：1）设计疾病敏感适配器；2）将DINOv2特征转换为密集的类别特定点提示；3）通过交叉参考SAM的迭代掩码置信度和DINOv2的语义一致性来动态过滤提示；4）最终提炼出高度信息化的提示集来激活SAM的几何先验。

Result: 广泛评估表明，SGPer在小麦病害和器官分割基准测试中始终达到最先进的性能，特别是在数据受限的场景下，能够实现精确且鲁棒的分割，严格保持对时间外观变化的不变性。

Conclusion: SGPer通过语义-几何先验协同，有效解决了小麦病害分割中的数据稀缺和外观变化问题，为精准农业提供了实用且高效的解决方案。
...
TL;DR: VideoStir是一个针对长视频的结构化、意图感知检索增强生成框架，通过时空图表示和多跳检索解决现有方法破坏视频结构、依赖显式语义匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在处理长视频时受限于有限的上下文窗口。传统的检索增强生成方法通常将视频扁平化为独立片段，破坏了视频固有的时空结构，并且依赖显式语义匹配，可能错过与查询意图隐式相关的线索。

Method: VideoStir首先将视频在片段级别组织为时空图，然后执行多跳检索以聚合跨远距离但上下文相关事件的证据。此外，引入基于MLLM的意图相关性评分器，根据帧与查询推理意图的对齐程度进行检索。为此构建了IR-600K大规模数据集用于学习帧-查询意图对齐。

Result: 实验表明，VideoStir在不依赖辅助信息的情况下与最先进的基线方法具有竞争力，突显了将长视频RAG从扁平化语义匹配转向结构化、意图感知推理的前景。

Conclusion: VideoStir通过结构化视频表示和意图感知检索，为长视频检索增强生成提供了新方向，解决了现有方法破坏时空结构和依赖显式语义匹配的局限性。
...
TL;DR: 提出CSAP解码器框架，通过跨阶段注意力传播减少计算冗余，在多个语义分割数据集上实现高性能和低计算成本


<details>
  <summary>Details</summary>
Motivation: 现有轻量级语义分割方法中，多尺度解码器在各特征尺度上独立计算注意力，导致显著冗余，因为不同尺度的注意力分布高度相关
...
Conclusion: CSAP框架在保持多尺度上下文推理能力的同时，显著降低了解码器的计算成本，实现了轻量级语义分割的高效性能
...
TL;DR: 本文提出了一种基于SAM3的无训练少样本语义分割方法，通过简单的空间拼接策略将支持图像和查询图像放在共享画布上，让冻结的SAM3直接进行分割，无需微调或架构修改，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统少样本语义分割方法依赖大量情景训练来学习可迁移表示，计算成本高且对分布变化敏感。本文从现代视觉基础模型角度重新审视FSS，探索SAM3作为无训练解决方案的潜力。

Method: 利用SAM3的可提示概念分割能力，采用简单的空间拼接策略将支持图像和查询图像置于共享画布上，让完全冻结的SAM3执行分割，无需任何微调或架构改变。

Result: 在PASCAL-5^i和COCO-20^i数据集上的实验表明，这种极简设计已经达到最先进的性能，超越了许多精心设计的复杂方法。同时发现负提示在少样本设置中可能适得其反。

Conclusion: 强大的跨图像推理能力可以从简单的空间公式中涌现，同时也揭示了当前基础模型在处理冲突提示信号方面的局限性。SAM3的无训练方法为少样本分割提供了高效且有效的解决方案。
...
Method: 1）将输入转换到规范正交空间以减轻透视引起的几何失真；2）HUG-MVD模块联合建模个体和组级上下文，生成完整的多视角法线和图像以解决遮挡和接近问题；3）HUG-GR模块利用显式的物理交互先验优化几何，确保物理合理性和准确的人际接触建模；4）融合多视角图像生成高保真纹理

Result: HUG3D在广泛实验中显著优于单人方法和现有多人方法，能够从单张图像生成物理合理、高保真的交互人群3D重建
...
TL;DR: GameAD：一个将端到端自动驾驶建模为风险感知博弈问题的新框架，通过风险优先的博弈规划显著提升轨迹安全性
...
Method: 提出风险优先博弈规划概念，开发GameAD框架，包含：风险感知拓扑锚定、战略负载适配器、极小极大风险感知稀疏注意力、风险一致均衡稳定化等技术，实现风险优先的博弈论决策。同时提出规划风险暴露指标，量化长期规划轨迹的累积风险强度。

Result: 在nuScenes和Bench2Drive数据集上的大量实验表明，该方法显著优于现有最先进方法，特别是在轨迹安全性方面表现突出。

Conclusion: 将自动驾驶建模为风险感知博弈问题，通过风险优先的交互机制，能够有效提升端到端自动驾驶系统的安全性能，为安全自动驾驶提供了新的解决方案。
...
TL;DR: 该研究开发了一个生成合成眼动视频数据的管道，通过3D眼动模拟器将真实虹膜轨迹重放，创建了用于脚本阅读检测的标注数据集，以解决行为模态数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型通过海量互联网数据训练取得了显著能力，但行为模态数据（如眼动、手势、社交信号）仍然稀缺、标注成本高且涉及隐私。模拟生成合成数据成为解决这一不对称性的有前景的替代方案。

Method: 提出一个管道：从参考视频中提取真实人类虹膜轨迹，通过无头浏览器自动化在3D眼动模拟器上重放这些轨迹，生成合成标注的眼动视频。应用于视频面试中的脚本阅读检测任务，创建了包含144个会话（12小时，25fps）的数据集。

Result: 评估显示生成的轨迹保留了源数据的时间动态（所有指标的KS D < 0.14）。逐帧比较发现3D模拟器在阅读尺度运动上表现出有限敏感性，这归因于缺少耦合的头部运动。管道、数据集和评估工具已发布。

Conclusion: 该研究为行为建模和视觉语言系统交叉领域的行为分类器开发提供了基础设施，展示了合成数据生成在解决行为模态数据稀缺问题上的潜力，并为未来模拟器设计提供了重要见解。
...
TL;DR: 提出了一种结合视觉语言模型引导的流匹配分割和随机矩阵理论检测的犬气胸自动诊断新范式，通过公开数据集和可解释统计分析方法解决数据稀缺和模型可信度问题。


<details>
  <summary>Details</summary>
Motivation: 犬气胸自动诊断面临数据稀缺和需要可信赖模型的挑战，现有方法在边界精度和统计解释性方面存在不足。

Method: 采用两阶段协同方法：1) 使用视觉语言模型引导迭代流匹配过程进行高精度病灶分割；2) 基于分割结果提取特征，应用随机矩阵理论分析特征协方差矩阵，将健康组织建模为随机噪声，通过检测统计显著的异常特征值识别病理信号。

Result: 该方法实现了高精度和可解释的诊断系统，流匹配提供的高保真定位能净化信号，最大化随机矩阵理论检测器的灵敏度。

Conclusion: 生成式分割与基于第一性原理的统计分析相结合，为犬气胸诊断提供了准确且可解释的系统，公开数据集和源代码促进了该领域研究。
...
TL;DR: 该论文提出了ContrAR基准，用于评估视觉语言模型在增强现实中对抗虚拟内容操纵和矛盾攻击的鲁棒性。
...
Method: 系统建模此类攻击，构建ContrAR基准包含312个真实世界AR视频并由10名人类参与者验证，进一步对11个商业和开源视觉语言模型进行基准测试。

Result: 实验结果显示，当前视觉语言模型对矛盾虚拟内容表现出合理的理解能力，但在检测和推理AR环境中的对抗性内容操纵方面仍有改进空间，且平衡检测准确性和延迟仍然具有挑战性。

Conclusion: ContrAR基准为评估视觉语言模型在增强现实安全方面的鲁棒性提供了系统框架，揭示了当前模型在对抗虚拟内容攻击方面的局限性，为未来研究指明了方向。
...
Method: 提出GCNV-Net框架，包含三个核心组件：1) 三向动态非空体素变换器(3DNVT)，沿横断面、矢状面和冠状面动态划分相关体素；2) 几何交叉注意力模块(GCA)，在多尺度特征融合中显式结合几何位置信息；3) 非空体素化，仅处理信息丰富的区域以减少冗余计算。

Result: 在BraTS2021、ACDC、MSD Prostate、MSD Pancreas和AMOS2022等多个基准测试中达到最先进性能：Dice系数提升0.65%，IoU提升0.63%，NSD提升1%，HD95相对提升14.5%。计算效率方面，FLOPs减少56.13%，推理延迟降低68.49%。

Conclusion: GCNV-Net有效平衡了分割精度和计算效率，在不同器官、疾病条件和成像模态中表现出强大的鲁棒性，具有显著的临床应用潜力。
...
Result: 在Delta-SN6、BRIGHT和Wuhan-Het数据集上，STSF-Net的mIoU分别比现有最佳方法提高了3.21%、1.08%和1.32%；同时发布了首个公开可访问的多类别多模态变化检测基准数据集Delta-SN6。

Conclusion: STSF-Net通过有效建模模态特定特征和时空共同特征，结合语义引导的自适应融合策略，显著提升了多模态变化检测的性能，为遥感变化检测提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前超声心动图分析中，任务特定的深度学习方法或多模态大语言模型仅专注于有限技能（眼-手或眼-脑），限制了临床可靠性和实用性。需要一种能够协调视觉观察、手动测量和专业知识推理的完整系统。
...
<details>
  <summary>Details</summary>
Motivation: 缺失模态问题严重降低多模态情感分析模型的准确性和泛化能力。现有方法主要通过提示学习和预训练模型提高鲁棒性，但存在两个局限：1）缺乏对生成缺失模态必要性的严格评估；2）对多模态提示间的结构依赖性和全局一致性探索不足。
...
Result: 在CMU MOSI、CMU MOSEI和CH-SIMS三个公开基准测试上进行广泛实验，证明该框架在不同缺失模态设置下均能达到最先进的性能和稳定的结果。

Conclusion: 提出的ProMMA框架有效解决了多模态情感分析中的缺失模态问题，通过动态评估、提示解缠、自适应加权和全局一致性增强，显著提升了模型在真实场景中的鲁棒性和性能。
...
TL;DR: SpecMamba：一种参数高效、频率感知的元学习框架，用于少样本高光谱目标检测，通过解耦稳定语义表示和敏捷光谱适应，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前元学习方法在高光谱目标检测中存在两个主要问题：1）全参数微调效率低下且容易过拟合；2）现有方法忽略了高光谱数据的频域结构和光谱带连续性，限制了光谱适应和跨域泛化能力。

Method: 提出SpecMamba框架，包含三个核心组件：1）离散余弦变换Mamba适配器（DCTMA），将光谱特征投影到频域并利用Mamba的线性复杂度状态空间递归；2）先验引导三编码器（PGTE），利用实验室光谱先验指导可学习适配器优化；3）自监督伪标签映射（SSPLM）策略，通过不确定性感知采样和双路径一致性约束实现测试时适应。

Result: 在多个公共数据集上的广泛实验表明，SpecMamba在检测精度和跨域泛化能力方面持续优于最先进的方法。

Conclusion: SpecMamba通过解耦稳定语义表示和敏捷光谱适应，提供了一种参数高效、频率感知的元学习框架，有效解决了少样本高光谱目标检测中的适应和泛化挑战。
...
TL;DR: 论文提出WRF4CIR方法，通过权重正则化微调解决组合图像检索中的过拟合问题，显著缩小泛化差距


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言预训练模型的组合图像检索方法在有限三元组数据下存在严重过拟合问题，作者通过系统研究发现不同模型和数据集间存在显著的泛化差距
...
Conclusion: 通过权重正则化微调方法可以有效缓解组合图像检索中的过拟合问题，特别是在有限监督数据下，该方法具有重要的实际应用价值
...
TL;DR: PTA提出"净化-对齐"框架，通过元学习动态降噪低质量模态，再用扩散知识蒸馏对齐模态，提升单模态编码器在缺失模态场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态人感测面临缺失模态的关键挑战，主要障碍是异构数据间的表示差距和低质量模态的污染效应，这些障碍存在因果关联，污染效应阻碍了表示差距的减小。

Method: PTA采用"净化-对齐"框架：1) 元学习驱动的加权机制动态降低噪声模态影响；2) 扩散知识蒸馏范式，用净化后的干净教师模型精炼各学生模态特征。

Result: 在MM-Fi和XRF55数据集上的实验表明，PTA在表示差距和污染效应显著的情况下实现了最先进的性能，显著提升了单模态模型在多种缺失模态场景下的鲁棒性。

Conclusion: PTA通过解决污染效应与表示差距间的因果依赖，创建了具有跨模态知识的强大单模态编码器，为鲁棒多模态人感测提供了有效解决方案。
...
TL;DR: BPC-Net：一种用于无标注皮肤病变分割的边界概率校准框架，通过高斯概率平滑解决边界概率不自信问题，在无监督方法中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 无标注皮肤病变分割在资源受限的皮肤镜部署中具有吸引力，但其性能受到三个耦合挑战的限制：噪声伪标签监督、有限目标域数据下的不稳定迁移以及边界概率不自信。现有方法主要关注伪标签去噪，而压缩的边界概率对最终掩码质量的影响较少受到明确关注，这直接影响轮廓完整性且无法通过全局阈值调整充分纠正。

Method: 提出BPC-Net边界概率校准框架，核心是高斯概率平滑(GPS)，在阈值化前执行局部概率空间校准以恢复不自信的病变边界而不引起无差别前景扩张。为支持在噪声伪监督和跨域迁移下的校准，进一步整合两个辅助设计：特征解耦解码器（分别处理上下文抑制、细节恢复和边界细化）和交互分支适应策略（仅更新伪标签交互分支同时保留部署的图像分割路径）。
...
TL;DR: ID-Selection是一种用于大型视觉语言模型的高效视觉token选择策略，通过结合重要性估计和多样性感知的迭代选择，在极端剪枝比例下保持性能同时大幅减少计算量。
...
Method: ID-Selection将重要性估计与多样性感知的迭代选择相结合：首先为每个token分配重要性分数，然后逐个选择高分token，同时逐步抑制相似token的分数，从而在统一选择过程中保留信息性token并减少冗余。

Result: 在5个LVLM主干和16个主要基准测试中，ID-Selection在极端剪枝比例下表现优异。例如在LLaVA-1.5-7B上，剪枝97.2%视觉token（仅保留16个），减少97%以上推理FLOPs，同时保持91.8%的原始性能，无需额外训练。

Conclusion: ID-Selection是一种简单有效的token选择策略，通过统一处理重要性和多样性，在保持大型视觉语言模型性能的同时显著加速推理，特别是在高剪枝比例下表现突出。
...
TL;DR: 论文系统研究了风格迁移在域泛化中的三个关键设计因素：风格池多样性、纹理复杂度和风格来源选择，并提出了轻量级的StyleMixDG方法


<details>
  <summary>Details</summary>
Motivation: 计算机视觉深度学习模型在真实世界部署时泛化能力差，特别是当使用合成数据训练时存在Sim2Real差距。虽然风格迁移作为域泛化的数据增强策略越来越流行，但文献中存在三个关键设计因素的不一致结论

Method: 通过系统的实证研究，分离并评估了风格池多样性、纹理复杂度和风格来源选择三个因素。基于研究结果提出了StyleMixDG（风格混合域泛化），这是一种轻量级、模型无关的数据增强方法，无需架构修改或额外损失函数

Result: 研究发现：(1)扩展风格池比重复使用少量风格带来更大增益；(2)当风格池足够大时，纹理复杂度没有显著影响；(3)多样化的艺术风格优于域对齐的替代方案。在GTAV→{BDD100k, Cityscapes, Mapillary Vistas}基准测试中，StyleMixDG相比强基线表现出持续改进

Conclusion: 通过系统实证研究解决了风格迁移在域泛化中的设计矛盾，提出的StyleMixDG方法验证了经验确定的设计原则能够转化为实际性能提升，为域泛化提供了有效的轻量级解决方案
...
TL;DR: 提出STGR框架，结合LLaMA-3-V和MedSAM，通过TVID模块提取诊断指导，用图推理解决解剖模糊，SAFT策略微调少于1%参数，在肺筛查任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态和基础模型难以处理临床报告语义模糊和解剖重叠问题，且在有限医学数据上微调会导致严重过拟合，需要更有效的语言引导医学图像分割方法。

Method: 提出语义-拓扑图推理框架，包含文本到视觉意图蒸馏模块提取诊断指导，将掩码选择建模为动态图推理问题，采用选择性非对称微调策略仅更新少于1%参数。

Result: 在LIDC-IDRI和LNDb数据集上通过5折交叉验证达到SOTA，LIDC-IDRI上DSC为81.5%，超过LISA等LLM工具5%以上，SAFT策略实现优异交叉稳定性（0.6% DSC方差）。

Conclusion: STGR框架有效解决了临床指令语义模糊和解剖重叠问题，SAFT策略作为强大正则化器，为实现鲁棒、上下文感知的临床部署铺平了道路。
...
Method: 直接从真实世界人类交互视频操作，自动发现关节部件，估计运动学参数，跟踪3D运动，在规范空间中重建静态和动态几何，生成仿真兼容网格

Result: 在新真实和模拟基准测试中大幅超越先前工作：部件分割mIoU提升达+50，关节和姿态误差降低5-10倍，重建精度显著提高

Conclusion: FunRec能够从真实交互视频重建功能3D数字孪生，支持URDF/USD导出用于仿真、手引导的可用性映射和机器人-场景交互应用
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的发展，图像描述从简短句子演变为包含数百字的全面叙述，这使得精确检测和定位幻觉变得极其困难。现有基准测试缺乏评估这种能力的细粒度和领域多样性。

Method: 研究者创建了DetailVerifyBench基准测试，包含1000张高质量图像，覆盖五个不同领域。每个图像描述平均超过200字，并提供密集的token级多类型幻觉标注。

Result: DetailVerifyBench是目前长图像描述领域中最具挑战性的精确幻觉定位基准测试，提供了高质量的图像、长文本描述和细粒度标注。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像面临云层、雾霾、噪声、分辨率限制和传感器异质性等多种退化问题，现有方法通常为每种退化类型训练单独模型，缺乏统一解决方案。

Method: 采用Sinkhorn-Knopp最优传输对齐异质波段到语义匹配槽，通过三种互补的专家混合层（卷积专家处理空间模式、通道混合专家保证光谱保真、带低秩适配器的注意力专家处理全局上下文），并使用步级动态权重调整稳定联合训练。

Result: LLaRS在11个恢复和增强任务上持续超越7个竞争模型，参数高效微调实验显示在未见数据上具有强大的迁移能力和适应效率。

Conclusion: LLaRS作为首个统一的遥感低层视觉基础模型，通过多模态多任务框架有效解决了遥感图像的各种退化问题，展示了强大的性能和泛化能力。
...
TL;DR: 该研究探讨了使用单目广播视频进行足球运动员疲劳监测的可行性，通过先进的比赛状态重建方法提取球员轨迹，并提出新的运动学处理算法来获取速度和加速度估计，构建加速度-速度曲线作为疲劳指标。


<details>
  <summary>Details</summary>
Motivation: 足球疲劳监测对预防伤病和提升战术表现至关重要，但目前主要依赖主观报告、实验室生物标志物或侵入式传感器。本研究旨在探索低成本的单目广播视频是否能提供足够质量的时空信号来支持疲劳分析。

Method: 基于先进的比赛状态重建方法提取球员在球场坐标中的轨迹，提出新颖的运动学处理算法从重建轨迹中获得时间一致的速度和加速度估计，构建加速度-速度曲线作为疲劳相关性能指标。

Result: 在SoccerNet-GSR基准测试上评估完整流程，包括30秒片段和完整的45分钟半场比赛。结果显示单目比赛状态重建能够恢复与加速度-速度曲线分析兼容的运动学模式，同时揭示了广播视频固有的轨迹噪声、校准误差和时间不连续性对分析的影响。

Conclusion: 单目广播视频可作为低成本疲劳分析的基础，但需要解决轨迹噪声、校准误差和时间不连续性等挑战，为未来研究指明了方法学方向。
...
TL;DR: PanopticQuery是一个用于4D场景统一查询推理的框架，基于4D高斯泼溅进行动态重建，通过多视角语义共识机制将自然语言查询与时空语义关联起来。


<details>
  <summary>Details</summary>
Motivation: 当前基于神经表示的4D重建方法在上下文推理方面存在局限，特别是对于交互、时间动作和空间关系等复杂语义。主要挑战在于如何将噪声、视角依赖的预测转化为全局一致的4D解释。

Method: 基于4D高斯泼溅进行高保真动态重建，引入多视角语义共识机制，通过聚合多个视角和时间帧的2D语义预测来锚定自然语言查询，过滤不一致输出，强制执行几何一致性，并通过神经场优化将2D语义提升为结构化4D锚定。

Result: PanopticQuery在复杂语言查询上达到了新的最先进水平，有效处理属性、动作、空间关系和多对象交互。同时提出了Panoptic-L4D基准用于动态场景语言查询评估。

Conclusion: PanopticQuery框架通过统一查询时间推理，成功解决了4D场景中语义理解的关键挑战，为动态环境中的自然语言查询提供了有效的解决方案。
...
TL;DR: RATNet是一个基于类比推理的胃肠内窥镜影像基础模型，通过循环预训练策略从异构专家标注中获取和传递知识，在多种场景下优于现有模型。
...
Method: 开发基于类比推理的RATNet基础模型，通过循环预训练策略从五个胃肠内窥镜数据集的异构专家标注中获取和传递知识。架构包括编码器、相关性知识获取与传递(RAT)模块、投影器和多任务头，支持微调、线性探测和零样本迁移。

Result: RATNet在六个场景中优于现有基础模型(GastroNet和GastroVision)：常见胃肠疾病诊断、罕见疾病少样本学习、新医疗站点零样本迁移、长尾疾病分布鲁棒性、新疾病适应以及联邦学习隐私保护部署。其优势来自类比推理机制，将图像衍生的后验知识与学习到的先验知识库匹配，并传递相对知识来指导诊断。

Conclusion: RATNet是开放且经济高效的模型，支持自动集成异构标注而无需手动标签统一，降低了数据获取成本，为智能胃肠诊断提供了实用基础，特别适用于资源有限的环境。
...
TL;DR: 该研究探索了医学视觉任务之间的内在关系，通过任务对比学习框架将30个不同任务嵌入到共享表示空间中，分析任务间的相似性和差异性。


<details>
  <summary>Details</summary>
Motivation: 医学计算机视觉社区主要关注特定任务的性能提升，但任务之间的内在关系（如它们如何关联、重叠或在表示层面上的差异）在很大程度上尚未被探索。本研究旨在揭示医学视觉任务之间的这些内在关系。

Method: 提出了任务对比学习（TaCo）框架，这是一个对比学习框架，旨在将任务嵌入到共享表示空间中。该方法将来自39个不同医学成像模态（包括CT、MRI、电子显微镜、X射线、超声等）的30个异构任务映射到联合空间中进行分析。

Result: 通过TaCo框架，成功将不同模态的异构任务映射到联合空间，并分析了它们的特性：识别了哪些任务具有独特的表示，哪些任务混合在一起，以及任务的迭代修改如何在嵌入空间中反映出来。
...
<details>
  <summary>Details</summary>
Motivation: 烟雾环境下的多视角图像重建面临三大挑战：强散射效应、视角依赖的外观变化、跨视角一致性严重退化。现有方法难以处理这些烟雾特有的退化问题。

Method: 1. 使用Nano-Banana-Pro增强烟雾退化图像，为重建提供更清晰的视觉观测；2. 开发Smoke-GS，一个烟雾感知的3D高斯泼溅框架，包含显式3D高斯建模和轻量级视角依赖介质分支来捕捉烟雾引起的方向依赖外观变化。

Result: 该方法在保持3D高斯泼溅渲染效率的同时，提高了对烟雾诱导退化的鲁棒性，能够在挑战性烟雾环境中生成一致且视觉清晰的新视角图像。

Conclusion: 提出的框架有效解决了烟雾退化多视角图像重建的挑战，通过视觉先验增强和烟雾感知3D建模，实现了高质量的场景重建和新视角合成。
...
TL;DR: CRFT是一个基于特征流学习的粗到细跨模态图像配准框架，通过Transformer架构学习模态无关的特征流表示，实现鲁棒的多模态空间对应。


<details>
  <summary>Details</summary>
Motivation: 解决跨模态图像配准中存在的模态差异、大尺度变化和几何变形等挑战，提供一种统一的特征流学习框架来建立准确的多模态空间对应关系。

Method: 采用粗到细的双阶段框架：粗阶段通过多尺度特征相关性建立全局对应；细阶段通过分层特征融合和自适应空间推理细化局部细节。引入迭代差异引导注意力机制和空间几何变换来循环优化流场。

Result: 在多种跨模态数据集上的实验表明，CRFT在准确性和鲁棒性方面持续优于最先进的配准方法，能够处理大的仿射和尺度变化，同时保持跨模态的结构一致性。

Conclusion: CRFT提供了一个可推广的多模态空间对应范式，在遥感、自主导航和医学成像等领域具有广泛适用性，代码和数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在2D视觉任务上表现出色，但在处理真实世界视觉流时空间感知能力有限。现有几何感知MLLM主要受限于单深度层提取和输入级融合的范式，导致局部几何细节丢失和早期层语义不匹配。

Method: 提出GUIDE框架：1）在几何编码器中进行多级采样，从局部边缘到全局拓扑全面捕获多粒度特征；2）将这些多级几何先验与MLLM早期层逐步对齐融合；3）引入上下文感知门控机制，使模型能基于当前语义获取所需空间线索，最大化空间先验利用效率并抑制冗余几何噪声。

Result: 在多个复杂空间推理和感知任务上，GUIDE显著优于现有基线方法，为将3D几何先验集成到大模型中建立了新范式。

Conclusion: GUIDE通过渐进式几何先验注入和多粒度特征融合，有效解决了MLLM空间感知能力有限的问题，为几何感知多模态大模型的发展提供了新方向。
...
TL;DR: GaussianGrow：一种从3D点云生长3D高斯的新方法，通过文本引导的多视图扩散模型合成一致外观进行监督，解决了现有方法因几何估计不可靠导致的生成质量差问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在渲染效率和质量上表现出色，但缺乏适当几何先验时生成3D高斯仍具挑战。现有方法通过预测点云作为几何参考来推断高斯基元，但不可靠的几何估计可能导致生成效果不佳。

Method: 提出GaussianGrow方法：1）设计文本引导的高斯生长方案，利用多视图扩散模型从输入点云合成一致外观进行监督；2）通过约束在重叠区域非预设相机姿态生成的新视图来减轻融合伪影；3）迭代检测相机姿态观察最大未生长区域，用预训练2D扩散模型修复渲染视图，直到生成完整高斯。
...
<details>
  <summary>Details</summary>
Motivation: 现有对CLIP视觉编码器中稀疏自编码器特征的分析主要关注单个特征的语义含义，缺乏对特征如何聚合视觉证据（从局部到全局）的理解。需要一个新的维度来表征特征的聚合范围。

Method: 引入信息范围概念，提出上下文依赖分数来量化特征对空间扰动的稳定性，区分位置稳定的局部范围特征和位置变化的全局范围特征。

Result: 实验表明不同信息范围的特征对CLIP预测和置信度有系统性不同的影响，局部特征和全局特征在模型行为中扮演不同角色。

Conclusion: 信息范围是理解CLIP表示的关键新维度，为SAE派生特征提供了更深入的诊断视角，有助于更好地解释视觉编码器的内部表示。
...
TL;DR: FoleyDesigner是一个基于专业拟音工作流程的框架，通过多智能体架构、潜在扩散模型和LLM驱动机制实现时空对齐的拟音生成，并引入首个专业立体声数据集FilmStereo，支持专业音频制作标准。


<details>
  <summary>Details</summary>
Motivation: 电影中拟音艺术对增强沉浸式听觉体验至关重要，但手动创建时空对齐的音频非常耗时费力。现有方法缺乏高质量立体声数据集和专业制作流程的集成。
...
Conclusion: FoleyDesigner提供了一个创新的拟音生成框架，通过集成专业工作流程、高质量数据集和先进生成技术，显著提升了拟音制作的效率和质量，为电影音频制作提供了强大的创作灵活性。
...
TL;DR: 扩散压缩器基于反向信道编码范式，相比传统和学习编解码器对位翻转具有显著更强的鲁棒性，新变体Turbo-DDCM在保持率-失真-感知权衡的同时大幅提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代图像压缩方法通常针对率-失真-感知权衡进行优化，但其对位级损坏的鲁棒性很少被研究。本文旨在探索压缩方法在噪声环境中的鲁棒性问题。

Method: 研究基于反向信道编码（RCC）范式的扩散压缩器，并与传统和学习编解码器比较位翻转鲁棒性。进一步提出更鲁棒的Turbo-DDCM变体，在保持率-失真-感知权衡的同时提升鲁棒性。

Result: RCC-based压缩器相比传统和学习编解码器对位翻转具有显著更强的鲁棒性。新提出的Turbo-DDCM变体在仅轻微影响率-失真-感知权衡的情况下，显著提升了鲁棒性。
...
TL;DR: 该论文介绍了Subtle visual Challenge竞赛，旨在促进对细微视觉信号的研究，包含跨域多模态欺骗检测和远程光电容积描记术估计两个任务，共有22支团队参与。


<details>
  <summary>Details</summary>
Motivation: 细微视觉信号虽然难以用肉眼察觉，但包含重要信息，在生物识别安全、多媒体取证、医疗诊断等领域有广泛应用。现有研究多关注特定任务或模态，模型在处理真实环境中的细微弱信号时面临鲁棒性、表示能力和泛化性挑战。

Method: 通过组织Subtle visual Challenge竞赛，设置两个具体任务：1) 跨域多模态欺骗检测；2) 远程光电容积描记术(rPPG)估计。提供基线模型并建立评估平台。

Result: 共有22支团队提交了最终结果，相应的基线模型已在MMDD2026平台上发布，为后续研究提供了基准和参考。

Conclusion: 该挑战赛旨在促进开发更鲁棒、泛化能力更强的细微视觉理解模型，推动计算机视觉和多模态学习领域的研究进展。
...
TL;DR: BADAS-2.0是第二代碰撞预警系统，通过长尾基准测试、知识蒸馏到边缘设备和可解释性三大改进，在稀有安全关键场景中实现性能提升，并支持实时边缘部署。
...
Method: 1) 构建10组针对稀有安全关键场景的长尾基准测试，使用BADAS-1.0作为主动标注器筛选高风险数据；2) 在225万未标注驾驶视频上进行领域特定的自监督预训练，然后蒸馏到紧凑模型；3) 生成实时物体中心注意力热图，并通过视觉语言模型生成结构化推理。

Result: 数据集从4万扩展到17.85万标注视频（约200万片段），在所有子组中获得一致性能提升，长尾最难案例改进最大；紧凑模型BADAS-2.0-Flash（86M）和Flash-Lite（22M）实现7-12倍加速且精度接近原模型；系统能生成实时注意力热图和结构化推理。

Conclusion: BADAS-2.0在碰撞预警的三个关键维度上取得显著进步：长尾场景性能、边缘设备实时部署能力和系统可解释性，为实际ADAS应用提供了更可靠、高效且透明的解决方案。
...
TL;DR: 本文提出性能主导模态优先策略（PDMP），认为多模态学习中不平衡学习优于平衡学习，通过让性能主导模态主导优化来解决多模态模型欠优化问题。


<details>
  <summary>Details</summary>
Motivation: 多模态学习常存在欠优化问题，即多模态模型性能甚至不如单模态模型。现有方法将此归因于模态间学习不平衡，但本文认为平衡学习并非最优，性能主导模态驱动的非平衡学习反而能带来更好的多模态性能。

Method: 提出性能主导模态优先策略（PDMP）：1）通过独立训练的单模态模型性能排序挖掘性能主导模态；2）引入非对称系数调制各模态梯度，让性能主导模态主导优化过程。该方法仅依赖单模态性能排序，与多模态模型结构和融合方法无关。

Result: 在多个数据集上的广泛实验验证了PDMP的优越性，表明该方法能有效提升多模态学习性能。

Conclusion: 多模态学习中的欠优化问题源于性能主导模态学习不足，而非模态间学习不平衡。PDMP策略通过让性能主导模态主导优化，有效解决了这一问题，且具有实际应用潜力。
...
TL;DR: VoxSAMNet提出了一种新的单目语义场景补全方法，通过显式建模体素稀疏性和语义不平衡问题，在SemanticKITTI和SSCBench-KITTI-360基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 单目语义场景补全在自动驾驶和机器人领域具有重要应用价值，但现有方法面临两个主要挑战：1）体素分布极不平衡（超过93%为空体素），导致对无信息体素的冗余关注；2）前景类别稀少，模型对长尾类别泛化能力差。

Method: 提出VoxSAMNet统一框架，包含两个核心组件：1）DSFR模块，通过共享虚拟节点绕过空体素，同时使用可变形注意力细化占用体素；2）前景调制策略，结合前景丢弃和文本引导图像过滤，缓解过拟合并增强类别相关特征。

Result: 在SemanticKITTI和SSCBench-KITTI-360基准测试中，VoxSAMNet分别取得了18.2%和20.2%的mIoU分数，超越了现有的单目和立体基线方法，实现了最先进的性能。

Conclusion: 研究表明，稀疏感知和语义引导的设计对于高效准确的3D场景补全至关重要，为未来研究提供了有前景的方向。VoxSAMNet的成功证明了显式处理体素稀疏性和语义不平衡问题的有效性。
...
TL;DR: 提出一种基于可微分排序的学习方法，直接优化删除和插入指标，为视觉模型生成像素级归因图


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型解释方法存在三方面权衡：传播方法高效但有偏差且架构特定；扰动方法因果性强但计算昂贵且对视觉变换器只能得到粗糙的块级解释；学习方法快速但通常优化替代目标或从启发式教师蒸馏

Method: 提出学习方案直接优化删除和插入指标，将其框架为排列学习问题，使用Gumbel-Sinkhorn替代硬排序实现可微分松弛，通过目标模型的归因引导扰动进行端到端训练

Result: 实验表明该方法在定量指标上持续改进，生成更清晰、边界对齐的解释，特别对基于变换器的视觉模型效果显著

Conclusion: 该方法通过直接优化解释指标，克服了现有方法的三方权衡，为视觉模型提供了高效、准确且像素级的可解释性解决方案
...
TL;DR: Etch攻击框架利用文本渲染能力，通过三层分解方法在良性场景中嵌入有害文本，显著提高了文本到图像模型的安全绕过成功率。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型能够渲染段落长度的文本，这带来了新的滥用风险。研究者发现并形式化了"刻录式越狱"攻击，即攻击者迫使T2I系统在视觉良性场景中生成包含有害文本内容（如欺诈文件）的图像。与传统描绘性越狱不同，刻录式攻击利用文本渲染能力本身作为武器。

Method: 提出Etch黑盒攻击框架，将对抗性提示分解为三个功能正交层：语义伪装、视觉空间锚定和字体编码。这种分解将完整提示空间的联合优化简化为可处理的子问题，通过零阶循环迭代优化。视觉语言模型对生成的图像进行批判，将失败定位到特定层，并规定有针对性的修订。

Result: 在7个模型和2个基准测试上的广泛评估显示，Etch平均攻击成功率达到65.57%（峰值91.00%），显著优于现有基线方法。

Conclusion: 研究结果揭示了当前T2I安全对齐中的关键盲点，强调了迫切需要开发字体感知的多模态防御机制来应对刻录式越狱攻击。
...
TL;DR: 该研究提出了一种基于3D-2D配准的自动牙齿形态比对方法，用于法医身份识别，通过计算机视觉技术解决传统方法中透视失真和客观量化的问题。


<details>
  <summary>Details</summary>
Motivation: 牙齿比对是重要的身份识别方法，但传统形态比对耗时且面临挑战：缺乏生前医疗记录（如边境移民死亡案例），以及现有方法无法有效建模透视失真和缺乏客观量化形态差异的方法。
...
Conclusion: 该方法为法医牙齿形态比对提供了自动、客观、定量的解决方案，通过叠加图像可视化使结果易于解释和分析，在缺乏生前医疗记录的场景下具有重要应用价值。
...
TL;DR: PVIR基准测试评估视频实例移除方法，特别关注物理因果关系恢复，结果显示现有方法在复杂物理交互场景中表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前视频实例移除方法主要评估视觉合理性，但忽视了由物体移除引发的物理因果关系（如残留阴影），需要专门的基准测试来评估这些物理一致性恢复能力。

Method: 提出了物理感知视频实例移除（PVIR）基准测试，包含95个高质量视频，分为简单和困难子集，后者专门针对复杂物理交互。采用解耦的人类评估协议，从指令遵循、渲染质量和编辑排他性三个维度评估四种代表性方法。

Result: PISCO-Removal和UniVideo达到最先进性能，DiffuEraser经常引入模糊伪影，CoCoCo在指令遵循方面表现显著困难。在困难子集上所有方法性能都明显下降，表明恢复复杂物理副作用仍是挑战。

Conclusion: PVIR基准测试揭示了当前视频实例移除方法在恢复物理因果关系方面的局限性，特别是对于复杂物理交互场景，为未来研究提供了重要的评估框架。
...
### [73] [Is CLIP Cross-Eyed? Revealing and Mitigating Center Bias in the CLIP Family](https://arxiv.org/abs/2604.05971)
*Oscar Chew,Hsiao-Ying Huang,Kunal Jain,Tai-I Chen,Khoa D Doan,Kuan-Hao Huang*

Main category: cs.CV

TL;DR: CLIP模型存在中心偏见问题，过度关注图像中心区域而忽略边界重要物体，这限制了模型的细粒度理解能力。通过表征和注意力分析发现，信息聚合过程中的池化机制导致离中心物体的概念信息丢失。研究提出无需训练的视觉提示和注意力重分配策略来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 对比视觉语言模型如CLIP通常缺乏对视觉内容的细粒度理解。研究发现CLIP家族存在一个持续存在的失败模式——中心偏见，即模型过度关注图像中心区域而忽略边界的重要物体。这种局限性是根本性的，因为无法识别相关物体使得依赖这些物体的复杂任务难以执行。

Method: 从表征和注意力两个角度进行分析：1）使用可解释性方法（嵌入分解和注意力图分析）研究模型行为；2）发现由于视觉嵌入聚合过程中的信息丢失，特别是池化机制的依赖，导致离中心物体的相关概念在最终表征中消失；3）提出无需训练的策略，包括视觉提示和注意力重分配，通过将模型注意力重定向到离中心区域来缓解偏见。

Result: 分析揭示了CLIP模型中心偏见的根本原因：信息聚合过程中的池化机制导致离中心物体的概念信息丢失。研究证明这种偏见可以通过无需训练的策略（视觉提示和注意力重分配）得到缓解，这些方法能够有效将模型注意力重定向到图像的非中心区域。

Conclusion: CLIP模型存在系统性的中心偏见问题，这限制了其对视觉内容的细粒度理解能力。该问题源于信息聚合过程中的池化机制。研究提出的无需训练策略为缓解这一偏见提供了有效途径，有助于提升模型对图像边界区域重要物体的识别能力。
...
TL;DR: 该论文提出了AICA-Bench基准测试，包含情感理解、推理和生成三个任务，评估了23个VLM模型，发现其存在强度校准弱和描述浅层的问题，并提出无需训练的GAT提示框架来改进。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在感知方面表现良好，但将感知、推理和生成整合到统一框架中的整体情感图像内容分析仍然研究不足，需要建立综合基准来评估和改进这方面的能力。

Method: 提出了AICA-Bench基准测试，包含三个核心任务：情感理解、情感推理和情感引导内容生成。评估了23个视觉语言模型，并提出了基于视觉支架和层次推理的无训练框架Grounded Affective Tree Prompting。

Result: 实验发现当前VLM存在两个主要局限：强度校准能力弱和开放描述过于浅层。提出的GAT框架能够减少强度误差并提高描述深度，为情感多模态理解和生成研究提供了强基线。

Conclusion: 该研究填补了整体情感图像内容分析的空白，提出的AICA-Bench基准和GAT框架为未来情感多模态理解和生成研究提供了重要基础，展示了改进视觉语言模型情感分析能力的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究虽然利用交叉注意力图来提升应用性能和解释模型行为，但不同注意力头的独特特征尚未得到充分探索。研究者希望探索选择性聚合与特定概念最相关的注意力头是否能改善视觉可解释性。

Method: 提出选择性聚合方法，识别并聚合与目标概念最相关的交叉注意力头，而不是使用所有注意力头。通过分析不同注意力头对特定概念的响应程度，选择最相关的头进行注意力图聚合。

Result: 相比基于扩散的分割方法DAAM，该方法获得了更高的平均IoU分数。最相关的注意力头比最不相关的头能更准确地捕捉概念特定特征，选择性聚合有助于诊断提示词的误解。

Conclusion: 注意力头选择为提高文本到图像生成的可解释性和可控性提供了有前景的方向，通过聚焦于最相关的注意力头可以改善模型行为的解释和诊断能力。
...
TL;DR: ADM-GS通过外观分解解决多轨迹场景重建中的外观不一致问题，将静态背景分解为轨迹不变材质和轨迹相关光照，在Argoverse 2和Waymo Open数据集上PSNR提升0.98dB


<details>
  <summary>Details</summary>
Motivation: 多轨迹场景重建对于自动驾驶仿真和数字孪生构建至关重要，但不同时间采集的序列存在显著的外观不一致问题，主要源于光照和环境条件的变化，尽管底层几何结构相同

Method: 提出ADM-GS框架，对静态背景进行显式外观分解：分解为轨迹不变材质（内在材质属性）和轨迹相关光照（光照变化）。采用神经光场，使用频率分离的混合编码策略，结合表面法线和显式反射向量，分别捕捉低频漫反射光照和高频镜面反射

Result: 在Argoverse 2和Waymo Open数据集上的定量评估显示，在多轨迹实验中，相比现有基于潜在表示的基线方法，PSNR提升0.98dB，同时在不同轨迹间产生更一致的外观

Conclusion: ADM-GS通过外观分解有效解决了多轨迹场景重建中的外观不一致问题，为自动驾驶仿真和数字孪生构建提供了高质量的重建结果
...
### [77] [MMEmb-R1: Reasoning-Enhanced Multimodal Embedding with Pair-Aware Selection and Adaptive Control](https://arxiv.org/abs/2604.06156)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在嵌入任务中未充分利用生成推理能力，直接引入思维链推理面临两个挑战：1) 实例级推理与成对对比监督的结构错配导致模型学习表面推理格式；2) 推理并非对所有嵌入任务都有益，强制推理会增加计算延迟并可能掩盖简单案例的语义信号

Method: 提出MMEmb-R1自适应推理多模态嵌入框架：1) 将推理建模为潜在变量；2) 引入成对感知推理选择，使用反事实干预识别对查询-目标对齐有益的推理路径；3) 采用强化学习选择性调用推理

Result: 在MMEB-V2基准测试中，仅用4B参数达到71.2分，创下新的SOTA，同时显著减少推理开销和推理延迟

Conclusion: 通过自适应推理选择机制，MMEmb-R1有效解决了推理与嵌入任务的结构错配问题，在提升性能的同时优化了计算效率，为多模态嵌入任务提供了更高效的解决方案
...
TL;DR: SRCP框架通过解耦表示学习和后继训练，引入显著性引导的动态任务和一致性策略，显著提升了视觉无监督强化学习中的零样本泛化能力


<details>
  <summary>Details</summary>
Motivation: 后继表示方法在结构化低维环境中表现良好，但在高维视觉环境中难以扩展，存在两个关键限制：1）SR目标导致关注动态无关区域的次优表示，影响后继度量和任务泛化；2）这些有缺陷的表示阻碍了SR策略建模多模态技能条件动作分布和确保技能可控性

Method: 提出SRCP框架，包含两个核心组件：1）通过显著性引导的动态任务解耦表示学习和后继训练，捕获动态相关表示；2）集成快速采样一致性策略，结合URL特定的无分类器引导和定制训练目标，改进技能条件策略建模和可控性

Result: 在ExORL基准的4个数据集共16个任务上进行广泛实验，SRCP在视觉无监督强化学习中实现了最先进的零样本泛化性能，并且与各种SR方法兼容

Conclusion: SRCP通过解耦表示学习和改进策略建模，有效解决了SR方法在视觉URL中的局限性，为构建能够泛化到未见任务的通用智能体提供了有前景的方向
...
TL;DR: SonoSelect是一种超声主动视图探索方法，通过智能选择信息量最大的探头位置来减少冗余扫描，提高诊断效率。


<details>
  <summary>Details</summary>
Motivation: 超声诊断通常需要多个扫描视图来减少诊断模糊性、缓解声学遮挡并提高解剖覆盖范围，但并非所有探头视图都同样具有信息价值。大量视图采集会引入冗余，增加扫描和处理成本。

Method: 将超声主动视图探索定义为顺序决策问题，每个新的2D超声视图融合到观察解剖结构的3D空间记忆中，指导下一个探头位置。提出超声特定目标函数，偏好具有更大器官覆盖、更低重建不确定性和更少冗余扫描的探头移动。

Result: 在超声模拟器上的实验显示，SonoSelect仅使用N个视图中的2个就能实现有前景的多视图器官分类准确率。对于更困难的肾囊肿检测任务，达到54.56%的肾脏覆盖率和35.13%的囊肿覆盖率，且轨迹短且始终集中在目标囊肿上。
...
TL;DR: 提出一种基于视觉语言扩散基础模型的CT金属伪影去除新范式，通过少量配对数据（16-128例）实现高效伪影抑制，相比传统深度学习方法减少两个数量级的数据需求。


<details>
  <summary>Details</summary>
Motivation: 高衰减植入物产生的金属伪影严重降低CT图像质量，掩盖关键解剖结构，而传统深度学习方法需要大量配对训练数据，这在实际临床应用中难以获取。

Method: 将伪影去除重新定义为上下文推理任务，通过参数高效的LoRA技术适配通用视觉语言扩散基础模型。提出多参考条件策略，在输入含伪影图像的同时提供来自无关受试者的干净解剖示例，使模型能够利用类别特定上下文推断未损坏的解剖结构。

Result: 在AAPM CT-MAR基准测试中，该方法在感知和放射学特征指标上达到最先进性能。研究还发现，领域适配对于减轻幻觉效应至关重要，否则基础模型会将条纹伪影误认为自然物体（如华夫饼或培养皿）。
...
TL;DR: 提出MoME框架和HTL策略，通过模态专家混合和整体令牌学习，实现自适应多模态融合，在驾驶员动作识别任务中优于现有方法。
...
Result: 在公开基准测试的驾驶员动作识别任务中，MoME框架和HTL策略联合优于代表性单模态和多模态基线方法，并通过消融实验、验证和可视化结果证实了方法的有效性。

Conclusion: 该方法形成了以知识为中心的多模态学习框架，提高了专家专业化程度，减少了多模态融合的模糊性，改善了细微多模态理解并提供了更好的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管人类视频生成取得了巨大进展，但现有的生成视频扩散模型在捕捉人类动作的动力学和物理特性方面仍然存在困难，无法忠实还原真实的运动动态。

Method: 1) 关节运动一致性噪声采样：在统计人体模板的密集表面流形上采样3D关节噪声，替代非结构化随机高斯噪声；2) 联合外观-运动学习：同时预测像素外观和相应物理运动；3) 几何运动一致性学习：通过新颖的几何运动一致性损失在关节噪声空间中强制物理运动一致性。

Result: 该方法实现了最先进的性能，能够渲染运动一致、高保真的人类视频，支持多样化的服装风格，无需修改模型架构即可实现可扩展的受控人类视频生成。

Conclusion: HumANDiff通过创新的噪声采样和联合学习策略，显著提升了人类视频生成的物理真实性和运动控制能力，为高质量人类视频合成提供了有效的解决方案。
...
TL;DR: OmniCamera是一个统一框架，通过解耦场景动态内容和相机运动两个维度，实现灵活的视频生成控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型往往将场景内容和相机运动这两个关键因素纠缠在一起，限制了独立控制能力，需要一种能够明确解耦和控制这两个维度的框架。

Method: 提出OmniCamera统一框架，包含两个关键创新：1)构建OmniCAM混合数据集，结合真实世界视频和合成数据；2)提出双层次课程协同训练策略，通过条件层次和数据层次逐步引入控制模态并协同学习。

Result: OmniCamera实现了最先进的性能，能够灵活控制复杂的相机运动，同时保持卓越的视觉质量。

Conclusion: 通过解耦场景内容和相机运动，OmniCamera框架为视频生成提供了前所未有的创作控制能力，解决了模态冲突和数据稀缺等根本挑战。
...
Method: 基于柏拉图表示假说(PRH)，利用预训练Vision Transformers(ViTs)的通用度量空间，构建A-ROM框架。用可读概念字典和k-Nearest Neighbors(kNN)分类器替代传统不透明的决策层，实现快速建模而无需梯度微调。

Result: 在MedMNIST v2数据集上的实验表明，A-ROM能够提供与标准基准竞争的性能，同时提供简单、可扩展的"少样本"解决方案。

Conclusion: A-ROM框架通过利用预训练模型的通用表示，实现了医学概念的可解释快速建模，满足了临床环境对透明度的需求，为医学影像分析提供了有前景的解决方案。
...
TL;DR: 本文提出了一种基于探测的定位技术ICM，通过针对性地干预自注意力层来解决文本到图像扩散模型中模糊概念生成的隐式决策问题，相比现有方法实现了更优的去偏效果。
...
Method: 提出基于探测的定位技术，识别具有最高属性可分离性的概念层，发现模糊概念解析主要由自注意力层控制，并在此基础上提出ICM方法，对特定自注意力层子集进行针对性干预。

Result: 实验表明，干预这些特定的自注意力层相比现有最先进方法具有更优的去偏性能，同时最小化了不精确方法常见的伪影问题。

Conclusion: 扩散模型中的隐式决策过程在计算上是局部化的，主要位于自注意力层，通过ICM方法对这些层进行精确干预可以有效改善模型生成质量。
...
TL;DR: EDGE-Shield是一种在去噪过程中进行内容过滤的可扩展系统，通过嵌入匹配和x-pred变换技术，显著降低处理延迟，有效阻止侵权和深度伪造内容生成。
...
Method: 提出EDGE-Shield系统：1）使用基于嵌入的匹配进行高效参考比较；2）引入x-pred变换，将模型噪声中间潜在表示转换为后期伪估计的干净潜在表示，提升早期去噪阶段的分类准确性。

Result: 在Z-Image-Turbo和Qwen-Image两个生成模型上的实验显示，EDGE-Shield显著优于传统参考方法：Z-Image-Turbo处理时间减少约79%，Qwen-Image减少约50%，同时保持跨不同模型架构的过滤准确性。

Conclusion: EDGE-Shield提供了一个可扩展、低延迟的内容过滤解决方案，能够在去噪过程中实时检测和阻止侵权内容，为文本到图像生成模型提供了有效的保护机制。
...
TL;DR: 该研究提出了SciTikZ框架，通过高质量数据集、综合基准测试和新型强化学习优化方法，解决了图形程序合成中TikZ代码生成的数据质量和评估难题，实现了超越大型商业模型的性能。


<details>
  <summary>Details</summary>
Motivation: 图形程序合成对于将静态视觉数据反向工程为可编辑的TikZ代码至关重要，但当前面临两大挑战：现有图像-TikZ语料库缺乏严格的执行可靠性和视觉对齐性，同时缺乏评估结构保真度和视觉保真度的综合基准。

Method: 提出了一个闭环框架，包括：1) SciTikZ-230K大规模高质量数据集，覆盖11个科学领域；2) SciTikZ-Bench多维度基准测试；3) 双自一致性强化学习优化范式，利用往返验证惩罚退化代码并提升自一致性。

Result: 训练出的SciTikZer-8B模型实现了最先进的性能，在多个任务上一致超越了Gemini-2.5-Pro和Qwen3-VL-235B-A22B-Instruct等大型商业模型。

Conclusion: 该研究通过解决数据质量和评估标准的核心问题，结合创新的优化方法，显著提升了科学示意图TikZ代码生成的质量和可靠性，为图形程序合成领域提供了系统性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖图像/视频生成模型蒸馏到3D空间，损害几何一致性并限制渲染视角；要么仅限于小规模3D场景或物体中心生成。需要能够生成大规模、多视角一致的户外驾驶场景的解决方案。

Method: 使用Σ-Voxfield网格作为离散表示，每个占用的体素存储固定数量的着色表面样本。训练语义条件扩散模型在局部体素邻域上操作，使用3D位置编码捕获空间结构。通过渐进空间外推在重叠区域上扩展到大规模场景。最后通过延迟渲染模块渲染生成的网格。

Result: 能够生成多样化的大规模城市户外场景，可渲染成具有各种传感器配置和相机轨迹的光照真实图像，同时保持适中的计算成本。

Conclusion: 提出的3D生成框架实现了大规模多视角一致的3D场景生成，无需逐场景优化，在计算成本和生成质量之间取得了良好平衡。
...
TL;DR: 提出轻量级多模态适配框架，将RGB预训练的视觉语言模型迁移到热红外无人机图像，在生态监测中实现从物体识别到生境上下文解释的扩展


<details>
  <summary>Details</summary>
Motivation: RGB预训练的视觉语言模型与热红外图像之间存在表征差距，需要一种轻量级方法来桥接这一差距，以扩展视觉语言模型在生态监测中的应用

Method: 开发无人机采集的热红外数据集，通过多模态投影器对齐进行微调，将RGB视觉表征信息迁移到热辐射输入；测试了三种代表性模型在闭集和开集提示条件下的物种识别和实例计数性能

Result: Qwen3-VL-8B-Instruct在开集提示下表现最佳：鹿F1分数0.935，犀牛0.915，大象0.968；计数准确率分别为0.779、0.982、1.000；结合RGB图像还能生成生境上下文信息

Conclusion: 轻量级投影器适配为将RGB预训练的视觉语言模型迁移到热红外无人机图像提供了有效实用的途径，扩展了其在生态监测中从物体级识别到生境上下文解释的实用性
...
Method: 通过学习的多项式函数将输入token聚合成紧凑表示，然后每个token从中检索上下文信息，实现线性复杂度

Result: 在文本生成、手写文本识别、图像生成、3D建模和地球观测五个领域，PoM与基于注意力的模型性能相当，但计算成本大幅降低

Conclusion: PoM是自注意力机制的有效替代方案，具有线性复杂度，能保持Transformer的通用序列到序列逼近能力，特别适合长序列处理
...
TL;DR: 提出字符错误向量(CEV)作为OCR评估新指标，解决传统字符错误率(CER)在页面解析错误时无法定义的问题，支持文档理解流程的分解分析。


<details>
  <summary>Details</summary>
Motivation: 传统字符错误率(CER)假设文本已完美解析，但在实际页面解析错误时CER无法定义，限制了其在页面级OCR评估中的应用，特别是在标注模式不同的数据中。

Method: 引入字符错误向量(CEV)，一种基于字符袋的OCR评估器，可分解为解析错误、OCR错误和交互错误分量。演示了两种实现方法：空间感知字符错误率(SpACER)和使用Jensen-Shannon距离的字符分布方法。

Result: CEV验证显示：1)与CER的关系；2)解析质量评估；3)页面级OCR质量直接测量。在档案报纸数据集上发现，传统流水线方法优于最先进的端到端模型。CEV可通过阈值预测主要错误源(F1=0.91)。

Conclusion: CEV是解析指标与局部指标(如CER)之间的重要桥梁，支持文档理解研究，已作为Python库提供。虽然需要字符级定位进行最佳分类，但可通过阈值预测主要错误源。
...
TL;DR: DiffHDR是一个基于视频扩散模型的LDR到HDR转换框架，将转换任务视为生成式辐射度修复，通过Log-Gamma色彩空间利用时空生成先验合成过曝和欠曝区域的HDR辐射度。


<details>
  <summary>Details</summary>
Motivation: 大多数数字视频以8位低动态范围格式存储，导致原始高动态范围场景辐射度因饱和和量化而丢失，这限制了HDR显示器的准确亮度映射和后期制作中的重新曝光操作。现有LDR到HDR转换技术难以恢复过曝和欠曝区域的真实细节。

Method: 将LDR到HDR转换构建为视频扩散模型潜在空间中的生成式辐射度修复任务，在Log-Gamma色彩空间中操作，利用预训练视频扩散模型的时空生成先验合成过曝和欠曝区域的HDR辐射度。开发了从静态HDRI地图合成高质量HDR视频训练数据的流程，支持文本提示或参考图像引导的可控转换。

Result: DiffHDR在辐射度保真度和时间稳定性方面显著优于现有最先进方法，能够生成具有真实HDR效果的视频，为重新曝光提供了相当大的调整空间。

Conclusion: DiffHDR通过将LDR到HDR转换构建为生成式辐射度修复任务，成功解决了现有方法难以恢复过曝和欠曝区域真实细节的问题，为高质量HDR视频转换提供了有效解决方案。
...
TL;DR: HaloProbe：一个贝叶斯框架，通过分解外部描述统计和内部解码信号来检测和缓解视觉语言模型中的物体幻觉，相比现有方法更有效且保持实用性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在图像描述中会产生物体幻觉，现有基于注意力权重的检测方法不可靠，存在隐藏的混淆因素（如token位置和物体重复），导致辛普森悖论。

Method: 提出HaloProbe贝叶斯框架，通过平衡训练分离内部证据，结合学习到的外部特征先验来估计token级别的幻觉概率，作为非侵入性缓解的外部评分信号。

Result: HaloProbe引导的解码比最先进的基于干预的方法更有效地减少幻觉，同时保持实用性。

Conclusion: HaloProbe提供了一种有效的非侵入性方法来检测和缓解视觉语言模型中的物体幻觉，解决了现有注意力分析方法不可靠的问题。
...
TL;DR: 研究发现，仅当源实体明确作为预测目标时，语言模型才能克服反转诅咒，且前向和反向事实可能被存储为不同的条目而非统一概念。


<details>
  <summary>Details</summary>
Motivation: 研究反转诅咒（语言模型无法反向检索事实）的机制，探索不同训练目标（如掩码语言建模）如何影响模型的反向推理能力。

Method: 比较标准掩码语言建模（MLM）与仅解码器掩码训练在四个反转基准上的表现，通过表示距离分析和线性探测研究其工作机制。

Result: 仅当源实体明确作为预测目标时才能获得反转准确性；前向和反向事实可能被存储为不同条目，MLM和仅解码器掩码训练具有不同的索引几何结构。

Conclusion: 目标层面的"修复"可以改善反转行为，但未必诱导出预期的潜在泛化能力，需谨慎对待模型是否真正形成了统一的概念表示。
...
TL;DR: IoT是一种渐进式自过滤策略，通过重构多选题只保留合理选项，减少干扰项对LLMs的影响，提升推理稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多选题评估中容易受到干扰项的影响，导致注意力分散，在正确和错误答案之间不稳定振荡。需要一种方法来减轻这种认知负荷，让模型更有效地关注合理答案。

Method: 提出Inclusion-of-Thoughts(IoT)方法，这是一种渐进式自过滤策略。通过重构多选题，只保留合理的选项选择，为检查比较判断和模型内部推理的稳定性提供受控环境。该方法明确记录过滤过程，增强模型决策的透明度和可解释性。

Result: 广泛的实证评估表明，IoT显著提升了思维链在各种算术、常识推理和教育基准测试中的性能，且计算开销最小。

Conclusion: IoT通过减少干扰项对LLMs的影响，提高了模型在多选题中的推理稳定性和性能，同时增强了决策过程的透明度和可解释性。
...
TL;DR: 提出Phase-Associative Memory (PAM)，一种复值循环序列模型，通过外积在矩阵状态中累积关联，使用共轭内积进行检索，在WikiText-103上达到30.0困惑度，接近Transformer性能。


<details>
  <summary>Details</summary>
Motivation: 探索基于复值表示和相位关联的记忆模型，解决传统向量状态模型中全息绑定因叠加关联容量衰减的问题，并探讨语言建模中计算形式的选择。

Method: 使用复值表示的循环序列模型，所有表示均为复数值，关联通过外积累积在矩阵状态S_t中，检索通过共轭内积K_t*·Q_t/√d进行。

Result: 在WikiText-103上约1亿参数规模下，PAM验证困惑度达到30.0，与匹配的Transformer（27.1）相差约10%，尽管有4倍复数计算开销且无定制内核。

Conclusion: 矩阵状态解决了向量状态模型中的容量衰减问题，复值叠加和共轭检索架构的竞争力与人类和大型语言模型中语义解释的非经典上下文性证据一致，对语言建模计算形式选择有启示。
...
TL;DR: 研究发现，在医疗问答的检索增强生成系统中，即使基于相同证据，大语言模型的回答也会因问题表述方式（积极vs消极框架）而产生显著不一致，这种框架效应在多轮对话中会进一步放大。


<details>
  <summary>Details</summary>
Motivation: 患者向大语言模型提出医疗问题时，问题表述往往复杂且难以清晰表达。大语言模型对提示词表述敏感，理想情况下，基于相同证据时，模型应保持回答一致性，不受问题表述方式影响。本研究旨在系统评估医疗问答中LLM对问题表述变化的敏感性。

Method: 在受控的检索增强生成环境中进行系统评估，使用专家选择的临床试验摘要文档而非自动检索。研究两个维度的患者查询变化：问题框架（积极vs消极）和语言风格（技术性vs通俗语言）。构建包含6,614个查询对的数据集，评估8个大语言模型的回答一致性。

Result: 积极和消极框架的查询对显著比相同框架的查询对更容易产生矛盾结论。这种框架效应在多轮对话中进一步放大，持续的说服会增加不一致性。框架和语言风格之间没有显著的交互作用。

Conclusion: 研究表明，在医疗问答中，即使基于相同证据，大语言模型的回答也会仅因查询表述方式而被系统性影响。这强调了在高风险环境中，表述鲁棒性应作为检索增强生成系统的重要评估标准。
...
TL;DR: SenseAI是一个人类参与验证的金融情感数据集，包含推理链、置信度分数、人类修正信号和真实市场结果，专门用于改进金融AI系统的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融情感数据集通常只包含模型输出，缺乏完整的推理过程和验证机制。为了改进金融AI系统的推理能力，需要包含推理链、人类反馈和真实市场结果的结构化数据集。

Method: 构建了包含1,439个标注数据点的SenseAI数据集，涵盖40只美股和13个金融数据类别。数据集包含推理链、置信度分数、人类修正信号和真实市场结果，支持直接集成到LLM微调流程中。

Result: 发现了LLM在金融推理中的系统性错误模式，包括：1) 潜在推理漂移（引入输入中不存在的信息）；2) 置信度校准错误；3) 前向预测倾向。这些错误是可预测和可纠正的。

Conclusion: LLM在金融推理中的错误不是随机的，而是发生在可预测和可纠正的范围内。结构化的人类参与验证数据可以用于有针对性的模型改进，SenseAI数据集为金融AI系统的评估和对齐提供了重要资源。
...
### [99] [Memory Dial: A Training Framework for Controllable Memorization in Language Models](https://arxiv.org/abs/2604.05074)
...
<details>
  <summary>Details</summary>
Motivation: 语言模型中的记忆现象被广泛研究但难以隔离和控制。现有方法都是事后分析，只能检测训练后模型的记忆，无法将其效应与架构、数据或优化过程分离。理解模型何时记忆什么内容对于解释其预测至关重要。

Method: 引入Memory Dial训练框架，通过单一参数α在标准交叉熵和温度锐化目标之间插值，产生一系列架构和训练设置相同（在每个扫描中）但记忆压力不同的模型家族。

Result: 在六个架构和五个基准测试中的实验表明：1）α可靠地控制记忆压力，已见示例准确率单调增加而未见过示例准确率保持稳定；2）较大模型对记忆压力更敏感；3）频繁序列比罕见序列更容易记忆。该效应在不同锐化温度下稳健，与单温度交叉熵有质的不同，可迁移到多语言设置，甚至在自然出现的单次出现序列中也可检测。

Conclusion: Memory Dial提供了一个受控的实验框架，用于研究记忆行为如何在语言模型中涌现以及与泛化如何相互作用，使记忆压力成为可明确控制的变量。
...
### [100] [AutoSOTA: An End-to-End Automated Research System for State-of-the-Art AI Model Discovery](https://arxiv.org/abs/2604.05550)
...
TL;DR: AutoSOTA是一个端到端的自动化研究系统，能够自动复现顶级AI论文中的SOTA模型，并通过优化发现新的SOTA模型，平均每篇论文耗时约5小时。
...
Method: 采用多智能体架构，包含8个专门化智能体，通过三个紧密耦合阶段：资源准备和目标设定、实验评估、反思和构思，实现从论文到代码的落地、环境初始化修复、长周期实验跟踪、优化想法生成调度和有效性监督。

Result: 在8个顶级AI会议的论文上评估，成功发现了105个超越原始报告方法的新SOTA模型，平均每篇论文耗时约5小时。案例研究涵盖LLM、NLP、计算机视觉、时间序列和优化等领域。
...
### [101] [Beyond LLM-as-a-Judge: Deterministic Metrics for Multilingual Generative Text Evaluation](https://arxiv.org/abs/2604.05083)
*Firoj Alam,Gagan Bhatia,Sahinur Rahman Laskar,Shammur Absar Chowdhury*

Main category: cs.CL

TL;DR: OmniScore是一系列基于小型模型（<1B参数）的确定性学习度量标准，用于替代LLM作为文本生成评估的自动评判工具，具有低成本、高一致性和多语言支持的特点。


<details>
  <summary>Details</summary>
Motivation: 当前使用大型语言模型（LLMs）作为文本生成评估的自动评判工具存在成本高、对提示设计敏感、语言和聚合策略依赖性强等问题，严重限制了评估的可重复性。

Method: 开发了OmniScore系列度量标准，使用小型参数模型（<1B）在大规模合成监督数据（约564k实例，107种语言）上进行训练，通过8,617个手动标注实例进行评估，支持基于参考、基于源文本和混合评估等多种设置。

Result: 在问答、翻译和摘要等任务中，在6种语言上的评估结果表明，轻量级、确定性的学习度量标准能够可靠地近似LLM评判行为，同时保持低延迟和高一致性。

Conclusion: OmniScore提供了一种高度实用且可扩展的替代方案，能够解决LLM作为评判工具的成本和可重复性问题，为文本生成评估提供了更可靠的度量标准。
...
### [102] [Document Optimization for Black-Box Retrieval via Reinforcement Learning](https://arxiv.org/abs/2604.05087)
*Omri Uzan,Ron Polonsky,Douwe Kiela,Christopher Potts*

Main category: cs.CL

TL;DR: 本文提出了一种文档优化方法，通过微调语言模型或视觉语言模型来转换文档表示，使其更好地与目标检索器的预期查询分布对齐，从而提高检索质量。


<details>
  <summary>Details</summary>
Motivation: 传统的文档扩展技术在现代检索器中往往会降低性能，引入噪声并模糊判别信号。本文旨在重新定义文档扩展为文档优化问题，通过优化文档表示来提升检索效果，同时避免查询时的额外计算开销。

Method: 使用GRPO（梯度奖励策略优化）方法，以检索器的排名改进作为奖励，微调语言模型或视觉语言模型来转换文档表示。该方法只需要对检索排名进行黑盒访问，适用于单向量、多向量和词汇检索器。

Result: 在代码检索和视觉文档检索任务上的实验表明，学习到的文档转换能显著提升检索性能，使更小、更高效的检索器能够超越更大的检索器。例如，在OpenAI text-embedding-3-small模型上应用文档优化后，代码检索的nDCG5从58.7提升到66.8，视觉文档检索从53.3提升到57.6，甚至略微超过了成本高6.5倍的OpenAI text-embedding-3-large模型。

Conclusion: 文档优化是一种有效的技术，能够显著提升各种检索器的性能，特别是在资源受限的情况下，可以使小型检索器达到甚至超过大型检索器的效果。当检索器权重可访问时，文档优化通常与微调具有竞争力，而两者的结合往往能获得最佳性能。
...
### [103] [Multilingual Language Models Encode Script Over Linguistic Structure](https://arxiv.org/abs/2604.05090)
*Aastha A K Verma,Anwoy Chatterjee,Mehak Gupta,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: 多语言语言模型通过表面形式组织语言表示，而非统一的抽象语言身份；正字法（如罗马化）对表示影响显著，而词序扰动影响有限；语言抽象在深层逐渐浮现，但不会坍缩为统一的中间语言。


<details>
  <summary>Details</summary>
Motivation: 研究多语言语言模型内部如何组织不同语言（类型学和正字法多样）的表示，探究是抽象的语言身份还是表面形式线索塑造了这些表示。

Method: 使用紧凑的蒸馏模型（Llama-3.2-1B和Gemma-2-2B），通过语言激活概率熵（LAPE）度量分析语言关联单元，并用稀疏自编码器分解激活；进行词序打乱、罗马化等表面形式扰动实验，以及因果干预和探测分析。

Result: 1）语言关联单元强烈受正字法影响：罗马化导致几乎不相交的表示，既不与原生脚本输入对齐，也不与英语对齐；2）词序打乱对单元身份影响有限；3）类型学结构在深层越来越可访问；4）生成对表面形式扰动不变的单元最敏感，而非仅由类型学对齐识别的单元。

Conclusion: 多语言语言模型围绕表面形式组织表示，语言抽象逐渐浮现但不会坍缩为统一的中间语言；模型更依赖表面形式不变性而非抽象语言身份进行生成。
...
### [104] [MegaTrain: Full Precision Training of 100B+ Parameter Large Language Models on a Single GPU](https://arxiv.org/abs/2604.05091)
*Zhengqing Yuan,Hanchi Sun,Lichao Sun,Yanfang Ye*

Main category: cs.CL

TL;DR: MegaTrain是一个内存中心系统，能够在单个GPU上高效训练100B+参数的大语言模型，通过将参数和优化器状态存储在主机内存中，将GPU作为瞬态计算引擎使用。


<details>
  <summary>Details</summary>
Motivation: 传统GPU中心系统在训练超大模型时面临GPU内存限制的问题，需要将参数和优化器状态存储在CPU内存中，但CPU-GPU带宽瓶颈限制了训练效率。

Method: 采用内存中心架构，将参数和优化器状态存储在主机内存；引入流水线双缓冲执行引擎重叠参数预取、计算和梯度卸载；使用无状态层模板动态绑定权重，消除持久计算图元数据。

Result: 在单个H200 GPU和1.5TB主机内存上可靠训练120B参数模型；训练14B模型时吞吐量达到DeepSpeed ZeRO-3的1.84倍；在单个GH200上支持7B模型512k上下文训练。

Conclusion: MegaTrain通过内存中心设计和优化技术，有效解决了超大模型训练中的内存限制问题，在单个GPU上实现了高效的大规模语言模型训练。
...
### [105] [RAG or Learning? Understanding the Limits of LLM Adaptation under Continuous Knowledge Drift in the Real World](https://arxiv.org/abs/2604.05096)
*Hanbing Liu,Lang Cao,Yang Li*

Main category: cs.CL

TL;DR: 本文提出了一个评估大语言模型在连续知识漂移下适应能力的新基准，发现现有方法存在灾难性遗忘和时间不一致问题，并提出了时间感知检索基线Chronos来改善时间一致性理解。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在预训练中获得的知识固定于某个时间点，难以适应不断演化的世界知识。随着事实、实体和事件随时间变化，模型可能出现知识漂移，导致预测过时和时间推理不一致。现有方法很少在反映真实世界知识演化的时序环境中进行评估。

Method: 1. 构建基于时间戳证据的新基准，捕捉知识随时间演化的过程；2. 系统评估模型在连续知识漂移下的适应能力；3. 提出时间感知检索基线Chronos，将检索到的证据组织成事件演化图，实现无需额外训练的时间一致性理解。

Result: 基准测试显示大多数现有方法（包括普通RAG和几种基于学习的方法）在该设置下表现不佳，暴露出灾难性遗忘和时间不一致等关键限制。Chronos方法能够实现更时间一致的理解，无需额外训练。

Conclusion: 这项工作为分析和推进大语言模型在真实环境中适应连续知识漂移提供了基础，强调了时间一致性理解的重要性，并展示了通过时间感知检索组织证据的有效性。
...
### [106] [$π^2$: Structure-Originated Reasoning Data Improves Long-Context Reasoning Ability of Large Language Models](https://arxiv.org/abs/2604.05114)
*Quyet V. Do,Thinh Pham,Nguyen Nguyen,Sha Li,Pratibha Zunjare,Tu Vu*

Main category: cs.CL

TL;DR: π²方法通过从维基百科提取表格、生成多跳分析推理问题、并通过双路径代码执行验证答案，构建高质量推理数据，用于提升大语言模型的长上下文推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在长上下文推理方面存在不足，需要高质量、多样化的推理数据来提升其多跳分析推理能力。

Method: 1) 从维基百科提取和扩展表格；2) 基于表格和相关上下文生成现实的多跳分析推理问题，通过双路径代码执行自动确定和验证答案；3) 通过反向翻译将逐步结构化推理轨迹作为QA对的解决方案。

Result: 在四个长上下文推理基准测试和π²-Bench上，使用π²数据进行监督微调的模型获得了一致的改进：GPT-OSS-20B平均准确率提升+4.3%，Qwen3-4B-Instruct-2507提升+2.7%。GPT-OSS-20B通过自身推理轨迹进行自我蒸馏，平均性能进一步提升+4.4%。

Conclusion: π²方法构建的高质量推理数据能有效提升大语言模型的长上下文推理能力，且支持自我蒸馏进一步优化模型性能，相关代码、数据和模型已开源。
...
### [107] [Just Pass Twice: Efficient Token Classification with LLMs for Zero-Shot NER](https://arxiv.org/abs/2604.05158)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的零样本NER方法存在三个主要问题：因果注意力机制限制模型只能看到前文信息，无法利用完整上下文进行消歧；生成式方法解码速度慢；容易产生幻觉实体和格式错误。

Method: 提出Just Pass Twice (JPT)方法：1) 将输入文本复制拼接，让模型在第二次处理时每个token都能关注完整句子；2) 结合定义引导的实体嵌入实现灵活的零样本泛化；3) 无需修改模型架构。

Result: 在零样本NER基准测试中达到最先进水平：在CrossNER和MIT基准上平均F1分数比之前最佳方法提升+7.9，且速度比可比生成式方法快20倍以上。

Conclusion: JPT通过简单而有效的两次处理机制，成功解决了因果LLM在零样本NER中的上下文限制问题，实现了高效准确的判别式token分类。
...
### [108] [What Makes a Good Response? An Empirical Analysis of Quality in Qualitative Interviews](https://arxiv.org/abs/2604.05163)
*Jonathan Ivey,Anjalie Field,Ziang Xiao*

Main category: cs.CL

TL;DR: 本文评估了10种访谈回答质量测量方法，发现与关键研究问题的直接相关性是最强预测指标，而清晰度和基于意外性的信息性等常用NLP指标则无效。


<details>
  <summary>Details</summary>
Motivation: 虽然定性和NLP研究者提出了多种访谈质量测量方法，但这些方法缺乏验证——高评分回答是否真正有助于实现研究目标。需要确定哪些测量指标能实际预测回答对研究发现的贡献。

Method: 识别、实施并评估10种已提出的访谈回答质量测量方法；构建了包含343个访谈转录本、16,940个参与者回答的定性访谈语料库，涵盖14个真实研究项目；分析哪些指标能预测回答对研究发现的贡献。

Result: 发现与关键研究问题的直接相关性是回答质量的最强预测指标；而常用于评估NLP访谈系统的两个指标——清晰度和基于意外性的信息性——不能预测回答质量。

Conclusion: 本研究为定性研究设计和自动化访谈系统评估提供了分析见解和可扩展的实证指标，帮助研究者选择真正有效的质量测量方法。
...
### [109] [Gradient-Controlled Decoding: A Safety Guardrail for LLMs with Dual-Anchor Steering](https://arxiv.org/abs/2604.05179)
*Purva Chiniya,Kevin Scaria,Sagar Chaturvedi*

Main category: cs.CL

TL;DR: GCD是一种无需训练的防护方法，通过结合接受和拒绝锚定令牌来检测恶意提示，并在检测到恶意内容时预设拒绝令牌，确保首个令牌的安全性，显著降低误报率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型容易受到越狱和直接提示注入攻击，而最强的防御过滤器经常过度拒绝良性查询，降低用户体验。之前的检测方法如GradSafe使用单一"接受所有"锚定令牌，但其阈值脆弱且无法保证一旦解码开始就不会产生有害内容。

Method: 提出梯度控制解码（GCD），这是一种无需训练的防护方法，结合接受锚定令牌（"Sure"）和拒绝锚定令牌（"Sorry"）来收紧决策边界。在缓解阶段，如果提示被标记为恶意，GCD会在自回归解码恢复之前预设注入一个或两个拒绝令牌（"Sorry, I can't..."），确保首个令牌的安全性。

Result: 在ToxicChat、XSTest-v2和AdvBench数据集上，GCD相比GradSafe在可比较的召回率下减少52%的误报率，相比最强的仅解码基线降低高达10%的攻击成功率，在V100实例上平均增加15-20毫秒延迟，可迁移到LLaMA-2-7B、Mixtral-8x7B和Qwen-2-7B模型，仅需20个演示模板。

Conclusion: GCD提供了一种有效的训练免费防护方法，通过结合接受和拒绝锚定令牌以及预设拒绝令牌机制，显著提高了大语言模型的安全性，同时保持低延迟和良好的可迁移性。
...
### [110] [Improving Clinical Trial Recruitment using Clinical Narratives and Large Language Models](https://arxiv.org/abs/2604.05190)
*Ziyi Chen,Mengxian Lyu,Cheng Peng,Yonghui Wu*

Main category: cs.CL

TL;DR: 该研究探索使用大语言模型（LLMs）自动筛选临床叙述以促进临床试验招募，比较了编码器和解码器模型，并测试了三种处理长文档的策略，其中MedGemma模型结合RAG策略在N2C2数据集上取得了89.05%的最佳F1分数。


<details>
  <summary>Details</summary>
Motivation: 临床试验患者筛选是一个劳动密集型瓶颈，导致招募不足和试验失败。大语言模型的最新突破为使用人工智能改进筛选提供了机会，本研究旨在系统探索LLMs在临床叙述筛选中的应用。

Method: 研究系统探索了编码器和解码器生成式LLMs，包括通用LLMs和医学适应LLMs。针对长文档处理中的"迷失在中间"问题，测试了三种策略：1）原始长上下文（使用LLMs默认上下文窗口）；2）基于NER的提取式摘要（使用命名实体识别转换长文档）；3）RAG（基于资格标准的动态证据检索）。使用2018年N2C2 Track 1基准数据集进行评估。

Result: 实验结果显示，采用RAG策略的MedGemma模型取得了最佳性能，微F1分数达到89.05%，优于其他模型。生成式LLMs在处理需要跨长文档长期推理的试验标准方面有显著改进，而对于涉及短上下文（如实验室测试）的试验标准则显示出渐进式改进。

Conclusion: LLMs在临床试验招募中的实际应用需要考虑特定标准，在基于规则的查询、编码器LLMs和生成式LLMs之间进行选择，以在合理的计算成本内最大化效率。生成式LLMs特别适合需要长期推理的复杂筛选任务。
...
### [111] [Faster Superword Tokenization](https://arxiv.org/abs/2604.05192)
*Craig W. Schmidt,Chris Tanner,Yuval Pinter*

Main category: cs.CL

TL;DR: 论文提出了改进的BoundlessBPE和SuperBPE算法，通过两阶段训练和优化内存使用，将训练速度提升了600倍以上。


<details>
  <summary>Details</summary>
Motivation: 传统的BPE算法受限于预分词边界，只能表示完整单词，而现有的BoundlessBPE和SuperBPE算法虽然能形成超词（短语组合），但训练效率极低（如1GB数据需要4.7CPU天），不实用。

Method: 提出两阶段BoundlessBPE：第一阶段学习常规合并，第二阶段学习超合并；通过聚合超合并候选频率避免内存中保存完整文档；展示了BoundlessBPE与SuperBPE的近等价性，并自动确定SuperBPE中的超参数。

Result: 在相同1GB数据上，训练时间从4.7CPU天减少到603秒（BoundlessBPE）和593秒（SuperBPE），速度提升超过600倍；开源了Python参考实现和快速的Rust实现。

Conclusion: 通过算法优化和高效实现，显著提升了BoundlessBPE和SuperBPE的训练效率，使其从理论算法变为实用工具，为自然语言处理中的分词任务提供了更高效的解决方案。
...
### [112] [XMark: Reliable Multi-Bit Watermarking for LLM-Generated Texts](https://arxiv.org/abs/2604.05242)
*Jiahao Xu,Rui Hu,Olivera Kotevska,Zikai Zhang*

Main category: cs.CL

TL;DR: XMark是一种用于在LLM生成文本中嵌入二进制消息的多比特水印方法，在保持文本质量的同时显著提高解码准确率，特别是在生成文本长度有限的情况下。


<details>
  <summary>Details</summary>
Motivation: 现有多比特水印方法存在计算复杂度高、文本质量与解码准确率权衡不佳、以及在生成文本长度有限时解码准确率显著下降等问题，这些限制了水印在实际应用中的效果。

Method: 提出XMark方法，其编码器设计能产生较少扭曲的logit分布用于水印标记生成，从而保持文本质量；同时其定制解码器能够在有限标记数下可靠地恢复编码消息。

Result: 在多种下游任务上的广泛实验表明，XMark在保持水印文本质量的同时显著提高了解码准确率，优于现有方法。

Conclusion: XMark通过创新的编码器设计和定制解码器，有效解决了现有多比特水印方法的局限性，为LLM生成文本的可靠归属和恶意使用追踪提供了更实用的解决方案。
...
### [113] [Exemplar Retrieval Without Overhypothesis Induction: Limits of Distributional Sequence Learning in Early Word Learning](https://arxiv.org/abs/2604.05243)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 研究发现，自回归变换器语言模型在发育规模训练条件下，虽然能完美学习具体示例，但无法形成关于形状作为稳定特征维度的二阶概括（overhypothesis）。


<details>
  <summary>Details</summary>
Motivation: 探索儿童如何从具体示例中学习到形状是定义物体类别的稳定特征维度（二阶概括），并研究何种学习机制足以实现这种归纳飞跃。

Method: 使用自回归变换器语言模型（340万-2560万参数）在合成语料库上进行训练，其中形状是跨类别的稳定特征维度，设计了8个条件控制替代解释，通过1040项的wug测试电池评估。

Result: 所有模型在120个预注册运行中，一阶示例检索达到完美（100%），但二阶概括对新名词的泛化保持在随机水平（50-52%），特征交换诊断显示模型依赖框架到特征的模板匹配而非结构化抽象。

Conclusion: 自回归分布序列学习在发育规模训练条件下存在明显局限性，无法形成儿童表现出的二阶概括能力，模型仅依赖模板匹配而非结构化抽象。
...
### [114] [Beneath the Surface: Investigating LLMs' Capabilities for Communicating with Subtext](https://arxiv.org/abs/2604.05273)
*Kabir Ahuja,Yuxuan Li,Andrew Kyle Lampinen*

Main category: cs.CL

TL;DR: 该研究系统评估了语言模型在创造性沟通中使用潜台词的能力，发现前沿模型存在过度字面化倾向，但在有共同背景时能部分改善，揭示了当前LLM在社交性创造性沟通方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 人类沟通本质上是创造性的，经常使用潜台词（超越字面意义的隐含含义）。研究旨在系统性地探究语言模型是否能在沟通情境中使用潜台词，并量化评估这一复杂主观现象。

Method: 引入了四个新的评估套件：1）写作和解读寓言；2）基于Dixit等棋盘游戏规则的多智能体多模态游戏；3）视觉暗示环境；4）评估模型在有无明确共同背景下的表现差异。

Result: 前沿模型普遍表现出过度字面化、显性沟通的强烈倾向，即使在最佳模型中，在视觉暗示环境中也有60%的时间生成字面线索。部分模型能在有共同背景时改善30%-50%的过度字面化问题，但无法在未明确说明时推断共同背景的存在。寓言理解中，副文本和角色条件显著影响潜台词解读。

Conclusion: 该研究为潜台词这一复杂主观现象提供了可量化的测量方法，揭示了当前LLM的许多弱点和特质，希望激发未来在社交性创造性沟通和推理方面的研究。
...
### [115] [Right at My Level: A Unified Multilingual Framework for Proficiency-Aware Text Simplification](https://arxiv.org/abs/2604.05302)
*Jinhong Jeong,Junghun Park,Youngjae Yu*

Main category: cs.CL

TL;DR: Re-RIGHT是一个无需平行语料监督的自适应多语言文本简化强化学习框架，专门针对L2学习者的目标语言水平（CEFR、JLPT、TOPIK、HSK）进行词汇简化。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：1）构建个性化平行语料库成本高昂；2）现有基于大语言模型的易读性控制方法依赖预标注句子语料库且主要针对英语。此外，基于提示的词汇简化在较低水平和非英语语言上表现不佳。

Method: 提出Re-RIGHT统一强化学习框架，包含三个奖励模块：词汇覆盖率、语义保持和连贯性。收集了4种语言（英、日、韩、中）的43K词汇级数据，训练了一个紧凑的4B策略模型。

Result: 相比强大的LLM基线（如GPT-5.2和Gemini 2.5），Re-RIGHT在目标语言水平上实现了更高的词汇覆盖率，同时保持了原始意义和流畅性。

Conclusion: Re-RIGHT框架能够有效进行自适应多语言文本简化，无需平行语料监督，为L2学习者提供了更好的可理解输入支持。
...
### [116] [DIA-HARM: Dialectal Disparities in Harmful Content Detection Across 50 English Dialects](https://arxiv.org/abs/2604.05318)
*Jason Lucas,Matt Murtagh,Ali Al-Lawati,Uchendu Uchendu,Adaku Uchendu,Dongwon Lee*

Main category: cs.CL

TL;DR: DIA-HARM是首个评估虚假信息检测在50种英语方言中鲁棒性的基准，发现现有检测器对非标准美式英语方言存在系统性脆弱性，可能导致全球数亿非SAE使用者处于不利地位。


<details>
  <summary>Details</summary>
Motivation: 当前有害内容检测器（特别是虚假信息分类器）主要基于标准美式英语开发和评估，其对方言变体的鲁棒性尚未得到充分研究，这可能导致对非标准美式英语使用者的系统性歧视。

Method: 使用Multi-VALUE的语言学基础转换方法，创建了D3（方言虚假信息检测）语料库，包含195K个样本，源自已有的虚假信息基准。评估了16个检测模型在50种英语方言上的表现，包括美国、英国、非洲、加勒比和亚太地区的方言变体。

Result: 评估显示系统性脆弱性：人工撰写的方言内容使检测性能下降1.4-3.6% F1，而AI生成内容保持稳定。微调的transformer模型显著优于零样本LLM（96.6% vs. 78.3%最佳F1），某些模型在混合内容上出现超过33%的性能崩溃。跨2,450对方言的迁移分析显示，多语言模型（mDeBERTa：97.2%平均F1）泛化效果良好，而RoBERTa和XLM-RoBERTa等单语模型在方言输入上失败。

Conclusion: 当前虚假信息检测器可能系统性歧视全球数亿非标准美式英语使用者。研究发布了DIA-HARM框架、D3语料库和评估工具，为改进方言鲁棒性提供了重要基准。
...
### [117] [Human Values Matter: Investigating How Misalignment Shapes Collective Behaviors in LLM Agent Communities](https://arxiv.org/abs/2604.05339)
*Xiangxu Zhang,Jiamin Wang,Qinlin Zhao,Hanze Guo,Linzhuo Li,Jing Yao,Xiao Zhou,Xiaoyuan Yi,Xing Xie*

Main category: cs.CL

TL;DR: 该研究开发了CIVA多智能体环境，通过控制人类价值观在LLM智能体中的流行度，发现某些关键价值观会显著影响群体行为，甚至导致系统崩溃或引发欺骗等涌现行为。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地融入人类社会，从社会科学角度评估其对人类价值观的取向变得日益重要。特别是在基于LLM的多智能体系统中，个体层面的价值错位可能导致群体层面的失败累积。研究旨在探究人类价值观错位是否会改变LLM智能体的集体行为，以及会产生何种变化。

Method: 研究者引入了CIVA——一个基于社会科学理论的可控多智能体环境。在该环境中，LLM智能体形成社区，自主进行交流、探索和资源竞争。通过系统性地操纵价值观的流行度并进行行为分析，开展全面的模拟实验。

Result: 研究发现：(1) 识别出几个结构上关键的人类价值观，这些价值观显著塑造了社区的集体动态，包括那些与LLM原始取向不同的价值观；(2) 当这些价值观被错误设定时，在宏观层面检测到系统失败模式（如灾难性崩溃）；(3) 在微观层面观察到涌现行为，如欺骗和权力追求。

Conclusion: 研究提供了定量证据，表明人类价值观对LLM的集体结果至关重要，并激励未来在多智能体价值对齐方面的研究。这些发现强调了在LLM多智能体系统中正确对齐人类价值观的重要性。
...
### [118] [DQA: Diagnostic Question Answering for IT Support](https://arxiv.org/abs/2604.05350)
...
TL;DR: DQA是一个诊断问答框架，通过维护持久诊断状态和基于根本原因聚合检索案例，显著提升企业IT支持中的多轮诊断效果。
...
Method: DQA框架结合了对话查询重写、检索聚合和状态条件响应生成，维护持久诊断状态，并在根本原因级别而非单个文档级别聚合检索案例。

Result: 在150个匿名企业IT支持场景的评估中，DQA在轨迹级成功标准下达到78.7%的成功率，而多轮RAG基线仅为41.3%，同时将平均轮次从8.4减少到3.9。

Conclusion: DQA框架通过显式维护诊断状态和基于根本原因的检索聚合，显著提升了企业IT支持中的诊断效率和效果，在延迟和上下文约束下支持系统化故障排除。
...
### [119] [ICR-Drive: Instruction Counterfactual Robustness for End-to-End Language-Driven Autonomous Driving](https://arxiv.org/abs/2604.05378)
*Kaiser Hamid,Can Cui,Nade Liang*

Main category: cs.CL

TL;DR: ICR-Drive是一个诊断框架，用于评估端到端语言条件自动驾驶模型的指令反事实鲁棒性，通过生成四种扰动类型的指令变体来测试模型在指令语言变化下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在自动驾驶中主要假设指令是精确且格式良好的，但实际部署中指令在措辞、特异性、关键限定词缺失等方面存在差异，甚至可能包含误导性内容，导致指令级鲁棒性评估不足。

Method: ICR-Drive框架生成四种受控指令变体：Paraphrase（改写）、Ambiguity（歧义）、Noise（噪声）和Misleading（误导），其中误导性变体与导航目标冲突并试图覆盖意图。在CARLA模拟器中重放相同路线、配置和种子，以隔离由指令语言变化引起的性能变化。

Result: 在LMDrive和BEVDriver上的实验表明，微小的指令变化可能导致显著的性能下降和不同的故障模式，揭示了在安全关键驾驶中部署具身基础模型存在可靠性差距。

Conclusion: 该研究揭示了当前语言条件自动驾驶模型对指令变化的脆弱性，强调了在安全关键应用中需要更强的指令鲁棒性，ICR-Drive为评估和改进这类模型的指令级鲁棒性提供了诊断工具。
...
### [120] [Multi-Drafter Speculative Decoding with Alignment Feedback](https://arxiv.org/abs/2604.05417)
*Taehyeon Kim,Hojung Jung,Se-Young Yun*

Main category: cs.CL

TL;DR: MetaSD是一个统一的推测解码框架，通过整合多个草稿模型并动态分配计算资源，提升大语言模型推理效率


<details>
  <summary>Details</summary>
Motivation: 传统的推测解码使用单个草稿模型，但针对特定任务或领域训练的草稿模型在多样化应用场景中效果有限，需要更通用的解决方案

Method: 提出MetaSD框架，整合多个异构草稿模型，利用对齐反馈和多臂老虎机问题动态分配计算资源，优化草稿模型选择

Result: 大量实验表明MetaSD在性能上持续优于单草稿模型方法，提升了推测解码的效率和适应性

Conclusion: MetaSD通过整合多个草稿模型并动态分配资源，有效解决了传统推测解码中草稿模型适应性有限的问题，为LLM推理加速提供了更通用的解决方案
...
### [121] [Learning What Matters: Dynamic Dimension Selection and Aggregation for Interpretable Vision-Language Reward Modeling](https://arxiv.org/abs/2604.05445)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言奖励建模面临两难：生成方法可解释但慢，判别方法高效但不透明。需要弥合这一差距，提供既高效又可解释的奖励模型。
...
Conclusion: VL-MDR为视觉语言模型对齐提供了可扩展的解决方案，通过多维度可解释奖励建模，在保持高效性的同时提高了透明度和性能。
...
### [122] [Content Fuzzing for Escaping Information Cocoons on Digital Social Media](https://arxiv.org/abs/2604.05461)
*Yifeng He,Ziye Tang,Hao Chen*

Main category: cs.CL

TL;DR: ContentFuzz是一个基于置信度引导的模糊测试框架，通过重写社交媒体帖子来改变机器推断的立场标签，帮助内容突破信息茧房限制，同时保持语义完整性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的信息茧房限制了用户接触不同观点的内容。平台使用立场检测作为推荐和排序的重要信号，这导致内容主要被推送给观点相似的受众，减少了跨立场接触，限制了异议观点的传播并阻碍建设性对话。

Method: ContentFuzz是一个置信度引导的模糊测试框架，使用大型语言模型生成保持人类可解释意图的帖子重写版本，同时利用立场检测模型的置信度反馈来引导重写过程，以改变机器推断的立场标签。

Result: 在三个数据集、两种语言的四个代表性立场检测模型上评估，ContentFuzz能有效改变机器分类的立场标签，同时保持与原始内容的语义完整性。

Conclusion: ContentFuzz通过重写内容来改变机器推断的立场，可以帮助内容突破原有的信息茧房，促进跨立场的内容传播和更广泛的对话。
...
### [123] [Cross-Modal Coreference Alignment: Enabling Reliable Information Transfer in Omni-LLMs](https://arxiv.org/abs/2604.05522)
*Hongcheng Liu,Yuhao Wang,Zhe Chen,Pingjie Wang,Zhiyuan Zhu,Yixuan Hou,Yanfeng Wang,Yu Wang*

Main category: cs.CL

TL;DR: 该论文针对Omni-LLMs在多模态推理中的局限性，提出了跨模态指代问题，并创建了CrossOmni数据集来评估和提升这一能力，通过两种方法显著改善了模型性能。


<details>
  <summary>Details</summary>
Motivation: Omni-LLMs虽然在整体多模态感知方面表现出色，但在需要协同多模态推理的复杂场景中表现不佳。现有研究忽视了细粒度跨模态对齐，特别是跨模态共享指代物的识别，这是有效推理的关键因素。

Method: 将挑战形式化为跨模态指代问题，创建CrossOmni数据集（包含9个任务和人工设计的推理依据）。提出两种解决方案：1）无需训练的上下文学习方法；2）基于训练的SFT+GRPO框架，旨在引导核心指代感知的思维模式。

Result: 对13个Omni-LLMs的实验揭示了跨模态指代方面的系统性弱点。两种方法都带来了显著的性能提升，并能有效泛化到协作推理任务中。

Conclusion: 跨模态指代是推进稳健全模态推理的关键缺失环节，该研究为解决这一问题提供了有效的评估框架和解决方案。
...
### [124] [Turbulence-like 5/3 spectral scaling in contextual representations of language as a complex system](https://arxiv.org/abs/2604.05536)
*Zhongxin Yang,Chun Bao,Yuanwei Bin,Xiang I. A. Yang,Shiyi Chen*

Main category: cs.CL

TL;DR: 该研究发现自然语言在嵌入空间中表现出幂律谱，指数接近5/3，类似于湍流中的Kolmogorov谱，表明语义信息在多个尺度上以尺度无关的方式整合。


<details>
  <summary>Details</summary>
Motivation: 研究自然语言作为复杂系统的统计规律性，探索语言在嵌入空间中的多尺度结构特征，理解语义信息的组织方式。

Method: 将文本表示为基于transformer的语言模型生成的高维嵌入空间中的轨迹，使用嵌入步长信号量化沿标记序列的尺度相关波动，计算功率谱分析。

Result: 在多种语言和语料库中，功率谱在扩展频率范围内表现出稳健的幂律，指数接近5/3。这种尺度规律在人类书写和AI生成文本的上下文嵌入中一致存在，但在静态词嵌入中不存在，且会被标记顺序随机化破坏。

Conclusion: 观察到的尺度规律反映了多尺度、上下文依赖的组织方式，而不仅仅是词汇统计。类似于湍流中的Kolmogorov谱，语义信息在语言尺度上以尺度无关、自相似的方式整合，为研究语言表示中的复杂结构提供了定量、模型无关的基准。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然能对图像和视频进行复杂推理，但其推理效率受到视觉token主导的系统性效率障碍影响，这种开销由高分辨率特征提取、二次注意力缩放和内存带宽限制之间的多机制相互作用驱动。

Method: 提出了围绕推理生命周期（编码、预填充和解码）的系统性效率技术分类法，分析了端到端流程中上游决策如何决定下游瓶颈，并将效率景观解耦为信息密度塑造、长上下文注意力管理和内存限制克服三个轴。

Result: 提供了结构化分析，展示了孤立优化如何组合以在视觉保真度和系统效率之间进行权衡，并基于试点实证洞察提出了四个未来前沿方向。

Conclusion: 该工作为社区提供了一个可维护的文献资源库，并指出了通过混合压缩、模态感知解码、渐进状态管理和阶段解耦服务等方向来克服视觉token主导效率障碍的未来路径。
...
### [126] [Stop Fixating on Prompts: Reasoning Hijacking and Constraint Tightening for Red-Teaming LLM Agents](https://arxiv.org/abs/2604.05549)
*Yanxu Mao,Peipei Liu,Tiehan Cui,Congying Liu,Mingzhe Xing,Datao You*

Main category: cs.CL

TL;DR: JailAgent框架通过隐式操纵智能体推理轨迹和记忆检索，避免修改用户提示，有效应对LLM智能体安全威胁
...
Result: JailAgent在跨模型和跨场景环境中表现出色，展示了优秀的性能

Conclusion: JailAgent框架为LLM智能体安全威胁提供了有效的解决方案，通过隐式操纵推理轨迹和记忆检索，避免了传统方法的局限性
...
Method: 提出了一种新的训练框架：1）通过扰动自条件信号以匹配推理噪声，提高对先验估计错误的鲁棒性；2）引入词元级噪声感知机制，防止训练饱和，改善优化过程。

Result: 在条件生成基准测试中，该框架超越了标准连续扩散模型，同时提供高达400倍的推理加速，并且与其他一步扩散框架保持竞争力。

Conclusion: 通过处理自条件错误和引入噪声感知机制，提出的训练框架显著改善了连续扩散语言模型在少步采样时的性能，实现了高质量生成与快速推理的良好平衡。
...
TL;DR: Context-Agent框架将多轮对话历史建模为动态树结构，以应对对话的非线性特性，并提出了NTM基准进行评估，实验显示该方法提高了任务完成率和token效率。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在处理人类对话时面临根本性挑战，主要问题是将对话历史视为扁平线性序列，这与自然对话固有的层次化和分支结构不匹配，导致在涉及话题转换或指令细化的扩展交互中上下文利用效率低且连贯性丧失。

Method: 引入Context-Agent框架，将多轮对话历史建模为动态树结构，这种方法反映了对话的固有非线性特性，使模型能够维护和导航对应不同话题的多个对话分支。

Result: 实验表明Context-Agent提高了任务完成率，并在各种大语言模型中改善了token效率，强调了结构化上下文管理对于复杂动态对话的价值。

Conclusion: 通过将对话历史建模为动态树结构，Context-Agent框架有效解决了对话非线性管理的挑战，并提出了专门的NTM基准进行评估，证明了结构化上下文管理在复杂动态对话中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 提升拉丁语依存句法分析的准确性，特别是处理不同文体（诗歌和散文）时的性能，并探索检索增强生成（RAG）方法在句法分析任务中的应用。

Method: 两阶段方法：1）从CIRCSE树库中检索结构相似的句子（基于句子长度和词性标注n-gram相似性）；2）使用大语言模型结合检索到的示例和UD标注指南来改进UDPipe的基线分析结果。

Result: 在诗歌（塞内卡）文本上，THIVLVC比UDPipe基线提高了17个CLAS点；在散文（托马斯·阿奎那）文本上提高了1.5个CLAS点。对300个差异的双盲错误分析显示，在一致的标注者决策中，53.3%支持THIVLVC，揭示了树库内部和跨树库的标注不一致性。

Conclusion: THIVLVC系统在拉丁语依存句法分析任务中表现良好，特别是在诗歌文本上提升显著。研究还揭示了现有树库标注存在不一致性，这为未来改进标注质量和评估方法提供了重要见解。
...
TL;DR: 提出了YoNER，一个多领域的约鲁巴语命名实体识别数据集，包含5,000个句子和100,000个标记，覆盖圣经、博客、电影、广播和维基百科五个领域，标注了PER、ORG、LOC三种实体类型。


<details>
  <summary>Details</summary>
Motivation: 约鲁巴语NER研究受限于领域特定的资源，现有资源如MasakhaNER（新闻领域）和WikiAnn（维基百科）覆盖范围有限，需要更广泛的多领域数据集来推动研究。

Method: 收集了五个领域（圣经、博客、电影、广播、维基百科）的约鲁巴语文本，由三位母语者手动标注三种实体类型，遵循CoNLL风格指南，标注一致性超过0.70。使用多种Transformer编码器模型进行跨领域实验，并与MasakhaNER 2.0对比，评估少样本和跨语言设置。

Result: 非洲中心模型在约鲁巴语上优于通用多语言模型，但跨领域性能显著下降，特别是在博客和电影领域。形式相近的领域（新闻和维基百科）迁移效果更好。新提出的约鲁巴语特定语言模型OyoBERT在领域内评估中优于多语言模型。

Conclusion: YoNER数据集填补了约鲁巴语多领域NER资源的空白，非洲中心模型表现更好但跨领域挑战依然存在，发布了YoNER数据集和OyoBERT模型以支持未来约鲁巴语NLP研究。
...
TL;DR: 该论文提出了一种基于图结构的思维链优化框架，通过双剪枝策略减少推理过程中的冗余反思，在保持或提升准确率的同时将平均推理token数减少42%。


<details>
  <summary>Details</summary>
Motivation: 通过强化学习扩展思维链虽然能增强大语言模型的推理能力，但由于奖励信号稀疏，容易导致过度思考问题，产生冗余的中间推理内容。作者认为冗余的主要来源是低效的反思，表现为两种问题模式：不加区分的反思（在整个推理过程中进行广泛但低影响的检查）和重复性反思（反复重新验证已建立的结论）。

Method: 提出了一个基于图的思维链优化框架：1）将线性思维链转换为有向无环图，并添加明确的依赖边；2）设计双剪枝策略：分支级剪枝移除贡献较弱的反思分支，深度级剪枝消除后期阶段的重新验证；3）通过三阶段流程蒸馏这种行为：监督微调初始化策略、直接偏好优化偏好正确但冗余较少的轨迹、带长度惩罚的组相对策略优化联合优化答案正确性和效率。

Result: 实验结果表明，该方法在保持或提高准确率的同时，将平均推理token数减少了42%。

Conclusion: 该研究提出的图结构思维链优化框架有效解决了强化学习扩展思维链导致的过度思考问题，通过识别和剪枝低效反思模式，显著提高了推理效率而不损害准确性。
...
TL;DR: LVSpec是一个针对视频大语言模型的训练自由松散推测解码框架，通过识别视觉相关锚点令牌和容忍位置偏移，在保持>99.8%目标性能的同时，显著提升推理速度（2.70-2.94倍加速）。
...
Result: 在Qwen2.5-VL-32B上实现2.70倍加速，在LLaVA-OneVision-72B上实现2.94倍加速，保持>99.8%的目标性能。相比现有训练自由SD方法，平均接受长度和加速比分别提升136%和35%。

Conclusion: LVSpec通过松散推测解码有效解决了视频大语言模型的高延迟问题，在保持高保真度的同时显著提升推理效率，为视频理解任务提供了高效的推理解决方案。
...
TL;DR: 该研究将大语言模型的思维链生成表征为表示空间中的结构化轨迹，揭示了数学推理在功能有序的步骤特定子空间中移动，且正确与错误解在后期阶段系统性分叉。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解大语言模型在数学推理任务中思维链生成的内在机制，探究推理过程在表示空间中的几何结构，以及这种结构如何影响最终答案的正确性。

Method: 方法包括：1) 将思维链生成表征为表示空间中的轨迹；2) 分析数学推理在功能有序、步骤特定子空间中的移动；3) 研究正确与错误解在轨迹上的分叉模式；4) 开发基于轨迹的引导框架，用于推理修正和长度控制。

Result: 结果显示：1) 数学推理在功能有序的步骤特定子空间中移动，随着层深增加这些子空间变得可分离；2) 这种结构在基础模型中已存在，推理训练主要加速向终止相关子空间的收敛；3) 正确与错误解在早期步骤轨迹相似，但在后期阶段系统性分叉；4) 基于中期推理轨迹可预测最终答案正确性，ROC-AUC达0.87；5) 轨迹引导框架能有效进行推理修正和长度控制。
...
TL;DR: Attention Editing框架无需从头预训练，通过渐进式蒸馏将已训练LLM的注意力机制转换为更高效的架构（如MLA和GateSWA），显著降低KV缓存内存和带宽成本


<details>
  <summary>Details</summary>
Motivation: 长上下文和长生成场景下，KV缓存的内存和带宽成本日益主导LLM推理开销。现有高效注意力架构（如MLA和混合滑动窗口注意力）难以集成到已训练模型中，先前方法对源和目标注意力模块有细粒度结构要求，不满足实际部署需求

Method: 提出Attention Editing框架：1）用可学习目标模块替换原始注意力；2）渐进式蒸馏训练：a）层间教师强制优化配合中间激活监督防止冷启动误差累积，b）模型级蒸馏优化下一词分布，可选弱特征匹配正则化。在Qwen3-8B和Qwen3-30B-A3B上实例化为MLA和GateSWA（门控混合SWA）

Result: 转换后的模型保持竞争力性能同时提供显著效率提升，证明大规模注意力转换既可行又鲁棒。在Ascend 910B集群上完成实验，为国产硬件提供了实用训练案例研究

Conclusion: Attention Editing框架成功实现了无需从头预训练的LLM注意力架构转换，为实际部署中降低KV缓存开销提供了实用解决方案，证明了在国产硬件上进行大规模模型转换的可行性
...
TL;DR: 研究分析了中国九年级EFL学生与GenAI语音聊天机器人互动中的对话行为模式，发现高进步会话中学习者发起更多问题，而低进步会话中澄清请求更多；高进步会话中基于提示的纠正反馈序列更频繁，且始终位于学习者回应之后。


<details>
  <summary>Details</summary>
Motivation: 虽然生成式AI语音聊天机器人为第二语言口语练习提供了可扩展的机会，但学习者获益相关的互动过程仍未被充分探索。本研究旨在调查中国EFL学习者与GenAI语音聊天机器人互动中的对话行为模式。

Method: 对12名九年级中国EFL学生在10周干预期间与GenAI语音聊天机器人的70个会话进行人工标注，使用教育学指导的编码方案，共编码6,957个对话行为。比较高进步和低进步会话中的对话行为分布和序列模式。

Result: 在对话行为层面，高进步会话中学习者发起的问题更多，而低进步会话中澄清请求率更高，表明理解困难更大。在序列层面，高进步会话中基于提示的纠正反馈序列更频繁，且始终位于学习者回应之后，突显了反馈类型和时机在有效互动中的作用。
...
TL;DR: MedLayBench-V：首个大规模多模态基准数据集，专注于医学影像理解的专家-患者语义对齐，通过结构化概念约束确保语义等价性


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型主要基于专业文献训练，缺乏以患者为中心、通俗易懂的沟通能力，而现有文本简化方法存在幻觉风险，需要大规模多模态基准来促进可访问的医学影像理解

Method: 采用结构化概念锚定精炼（SCGR）流程，通过整合统一医学语言系统概念唯一标识符和微观实体约束，确保严格的语义等价性

Result: 构建了MedLayBench-V基准数据集，为训练和评估能够弥合临床专家与患者沟通鸿沟的新一代医学视觉语言模型提供了验证基础

Conclusion: MedLayBench-V填补了大规模多模态基准的空白，通过结构化概念约束方法确保语义等价性，为开发具有患者可访问沟通能力的医学视觉语言模型提供了关键资源
...
TL;DR: 该研究提出了一种新的微调框架，通过结合引导令牌校准和语义对齐，使大语言模型能够生成符合目标分布的输出，解决了现有方法在分布对齐任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 现实世界本质上是随机的，但当前大语言模型主要在单轮推理和固定真实值上进行评估。研究关注分布对齐问题：评估LLMs在重复提示下能否生成符合目标分布的输出，如反映真实世界统计数据或均匀分布。

Method: 提出新颖的微调框架，结合引导令牌校准和语义对齐。引入混合目标函数：使用KL散度锚定潜在引导令牌的概率质量，使用Kahneman-Tversky优化将这些令牌绑定到语义一致的响应。

Result: 实验在六个不同数据集上进行，表明该方法显著优于基线，在属性生成任务中实现了精确的分布控制。现成的LLMs和标准对齐技术（包括提示工程和直接偏好优化）无法可靠地控制输出分布。

Conclusion: 该研究填补了LLMs在分布对齐能力上的空白，提出的框架能够有效控制模型输出分布，使其符合期望的目标分布，为LLMs在需要统计准确性的应用中提供了重要改进。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理过程中虽然包含不确定性语言表达，但它们的回答往往过度自信。本研究旨在识别哪些语言表达与置信度相关，并理解这种过度自信行为的语言基础。

Method: 采用回归分析方法，将语言模型推理部分中语言表达的置信度作为因变量，分析特定n-gram与置信度之间的关系。研究在多个模型和QA基准上进行，并通过因果性测试验证提取的语言信息确实影响置信度。

Result: 研究发现语言模型在涉及推理时仍然过度自信，并将这种行为归因于特定的语言信息。有趣的是，提取的多个表达与测试时扩展中为提高推理性能而故意插入的提示短语重合。通过抑制这些过度自信的表达，可以在不降低性能的情况下实现置信度校准。

Conclusion: 语言模型在推理过程中的过度自信与特定语言表达相关，通过识别并抑制这些表达可以实现有效的置信度校准，而不牺牲模型性能。这一发现为改进语言模型的可靠性提供了新的途径。
...
TL;DR: PhageBench是首个评估大语言模型理解噬菌体基因组能力的基准，包含5,600个高质量样本，涵盖筛选、质控和表型注释三个阶段的核心任务。研究发现通用推理模型在噬菌体contig识别和宿主预测方面表现良好，但在复杂推理任务上存在局限。


<details>
  <summary>Details</summary>
Motivation: 噬菌体作为生物圈的"暗物质"在调控微生物生态系统和抗生素替代方面发挥关键作用，准确解读其基因组具有重要科学和实用价值。虽然通用大语言模型在理解生物文本方面表现出色，但其直接解读原始核苷酸序列和进行生物推理的能力尚未充分探索。

Method: 研究者开发了PhageBench基准，模拟生物信息学专家的工作流程，包含5,600个高质量样本，涵盖筛选、质控和表型注释三个阶段的五个核心任务。评估了八个大语言模型在噬菌体基因组理解方面的表现。

Result: 通用推理模型在噬菌体contig识别和宿主预测方面显著优于随机基线，显示出基因组理解的潜力。但在涉及长程依赖关系和细粒度功能定位的复杂推理任务中存在显著局限性。

Conclusion: 研究结果表明需要开发具有增强推理能力的下一代模型来处理生物序列，特别是针对复杂的长程依赖和精细功能定位任务。
...
TL;DR: 该论文提出通过多采样推理估计细粒度实例级知识分数，根据模型已有知识调整学习信号，鼓励对超出范围的问题明确回答"我不知道"，从而减少幻觉并提高不确定性表达。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能处理多样化的用户查询，但仍存在幻觉问题，这通常源于预训练和微调之间的知识不匹配。需要解决这种知识不对齐问题，使模型能更准确地表达其知识边界。

Method: 通过多采样推理可靠地估计细粒度的实例级知识分数，利用该知识分数根据模型已有知识调整学习信号，同时鼓励对超出范围的问题明确回答"我不知道"。

Result: 实验结果表明，该方法使模型在缺乏知识时能明确表达不确定性，同时在能回答的问题上保持准确性。此外，提出的不确定性评估指标显示，准确区分已知和未知实例能持续提升性能。

Conclusion: 通过基于知识分数调整学习信号并鼓励明确的不确定性表达，可以有效减少LLM的幻觉问题，提高模型在知识边界上的可靠性。
...
TL;DR: FAITH-M基准和CARE评估框架：用于评估AI心理治疗对话中治疗原则遵循度的结构化方法


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在心理健康应用中缺乏评估其是否符合心理治疗最佳实践的结构化机制，需要建立基于临床原则的评估框架

Method: 提出FAITH-M基准（专家标注的序数评分）和CARE评估框架（整合对话内上下文、对比示例检索和知识蒸馏的思维链推理）

Result: CARE框架F1分数达63.34，相比基线Qwen3的38.56提升64.26%；在领域转移下表现出鲁棒性，但建模隐含临床细微差别仍具挑战

Conclusion: CARE为评估AI心理健康系统中治疗保真度提供了临床基础框架，结构化推理和上下文建模是关键改进因素
...
TL;DR: CLEAR提出了一种通过反向训练方案的新型损失函数，用于改善跨语言检索性能，特别是在低资源语言中表现显著提升，同时最小化英语性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有多语言嵌入模型在跨语言场景中面临挑战，主要原因是语言资源不平衡以及训练中对跨语言对齐考虑不足。虽然标准化的对比学习方法被广泛采用，但它们可能难以捕捉语言间的基本对齐关系，并可能损害英语等已对齐良好的语言性能。

Method: 提出了CLEAR（Cross-Lingual Enhancement in Retrieval via Reverse-training）方法，这是一种利用反向训练方案的新型损失函数。该方法使用英语段落作为桥梁，加强目标语言与英语之间的对齐，确保在跨语言检索任务中的鲁棒性能。

Result: 实验表明，CLEAR在跨语言场景中实现了显著改进，提升幅度高达15%，特别是在低资源语言中表现突出，同时最小化了英语性能的下降。研究还发现CLEAR在多语言训练中也表现出良好的效果。

Conclusion: CLEAR通过反向训练方案有效改善了跨语言检索性能，特别是在低资源语言中表现优异，同时保持了英语等已对齐语言的性能。该方法具有广泛的应用潜力和可扩展性。
...
TL;DR: 本研究评估了人口统计学不可知训练技术在减少唤醒词检测中人口偏见的效果，通过数据增强和知识蒸馏技术，显著降低了性别、年龄和口音方面的性能差异。


<details>
  <summary>Details</summary>
Motivation: 语音接口广泛应用，但唤醒词检测在不同人口群体中存在显著的性能差异和人口偏见，需要开发公平的检测技术。

Method: 使用OK Aura数据库，采用人口统计学不可知训练方法（不包含人口标签），探索数据增强技术和预训练基础语音模型的知识蒸馏。

Result: 人口统计学不可知训练技术显著减少了人口偏见，其中一项技术相比基线在性别、年龄和口音方面的预测差异分别减少了39.94%、83.65%和40.48%。

Conclusion: 标签不可知方法在促进唤醒词检测公平性方面具有显著效果，为开发更公平的语音接口提供了有效途径。
...
TL;DR: 论文提出"独特性"指标，用于评估学习者表征是否能在共享比较规则下保持学习者之间的差异，无需教学结果标签或聚类，通过对话AI收集的学生提问数据进行验证。


<details>
  <summary>Details</summary>
Motivation: 教育AI系统中的学习者表征对个性化教学至关重要，但缺乏教学结果或情境依赖性强时，难以评估这些表征是否真正保留了学习者之间的有意义差异。

Method: 提出"独特性"指标，基于成对距离评估每个学习者与同组其他学习者的差异程度；使用在线学习环境中对话AI收集的学生提问数据，比较基于单个问题的交互级表征和聚合学生长期交互模式的学习者级表征。

Result: 学习者级表征比交互级表征具有更高的分离度、更强的聚类结构和更可靠的成对区分能力，表明学习者表征可以独立于教学结果进行评估。

Conclusion: 独特性可作为部署前的诊断指标，评估表征是否支持差异化建模或个性化教学，为教育AI系统提供实用的预部署评估标准。
...
TL;DR: LoRM是一个自监督框架，将旋转机械信号视为机器语言，通过令牌化多模态传感器数据并预测未来信号，实现实时状态监测。


<details>
  <summary>Details</summary>
Motivation: 传统信号处理方法依赖手工设计的变换和特征，而LoRM旨在将语言建模技术应用于工业信号分析，提供更实用的多模态旋转机械信号理解方法。

Method: 将多模态传感器数据重新表述为基于令牌的序列预测问题：保留观测上下文段的连续形式，将未来目标段量化为离散令牌，然后对通用预训练语言模型进行部分微调，避免从头训练大型模型。

Result: 原位刀具状态监测实验展示了稳定的实时跟踪和强大的跨刀具泛化能力，表明LoRM在语言建模和工业信号分析之间建立了实用桥梁。

Conclusion: LoRM为旋转机械信号理解和实时状态监测提供了一种创新的自监督框架，成功将语言建模技术应用于工业领域，源代码已公开。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在数学和编码等挑战性问题中表现出色，但需要多次采样才能获得高质量解。现有两种采样策略（顺序采样和并行采样）可以组合成更复杂的过程，但需要系统比较这两种方法并理解性能差异的根本原因。

Method: 首先严格比较顺序采样和并行采样两种方法，然后提出三个假设来解释性能差异：1）并行采样因聚合操作而更优；2）顺序采样因需要更长上下文而受损；3）顺序采样因条件依赖先前答案导致探索不足。在不同模型家族和规模（Qwen3、DeepSeek-R1蒸馏模型、Gemini 2.5）以及问题领域（数学和编码）上进行实证验证。

Result: 实证证据表明，聚合操作和上下文长度不是性能差距的主要原因。相反，探索不足似乎起着相当大的作用，这是顺序采样和并行采样之间性能差距的一个主要原因。

Conclusion: 顺序采样在理论上具有更强的表示能力，但实际表现不如并行采样，主要原因是顺序采样因条件依赖先前答案而导致探索不足。这一发现对设计更有效的推理模型采样策略具有重要意义。
...
TL;DR: MCircKE是一种基于机制电路的知识编辑框架，通过"映射-适应"方法解决LLM在多步推理中的"推理鸿沟"问题
...
Conclusion: MCircKE通过机制电路方法成功解决了LLM知识编辑中的推理鸿沟问题，实现了更精确的多步推理能力
...
TL;DR: 法国YMCA语料库是一个专门为儿童和青少年设计的新语言资源，包含39,200个文本文件，共22,471,898个词，旨在训练理解青少年语言的语言模型。
...
Method: 构建了包含39,200个文本文件的法语语料库，特点是来源多样、语法和拼写一致，并承诺提供开放的在线访问权限。

Result: 创建了包含22,471,898个词的法国YMCA语料库，这是一个专门针对儿童和青少年的高质量语言资源。

Conclusion: 该语料库可以作为训练理解青少年语言的语言模型的基础，从而提高数字交互质量，确保回应和建议适合年龄并适应用户的理解水平。
...
TL;DR: FrontierFinance是一个针对金融建模任务的长期基准测试，包含25个复杂任务，平均每个任务需要超过18小时的专业人力完成，用于评估LLM在专业金融领域的实际能力。
...
Method: 与金融专业人士合作开发了包含25个复杂金融建模任务的基准测试，涵盖五个核心金融模型，采用行业标准工作流程，并配有详细的评分标准。聘请人类专家定义任务、创建评分标准、评估LLM，并作为人类基准执行任务。

Result: 人类专家平均得分更高，且比当前最先进的系统更有可能提供客户就绪的输出结果。

Conclusion: FrontierFinance填补了金融领域AI能力评估的空白，为跟踪现实世界发展提供了稳健的基准，并建立了LLM部署的问责机制。
...
TL;DR: 该论文提出了MultiPun数据集来评估视觉语言模型对多模态双关语的理解能力，发现现有模型难以区分双关语与非双关语，并提出了提升双关语理解的策略。


<details>
  <summary>Details</summary>
Motivation: 双关语利用多义性和语音相似性创造幽默，在多模态双关中视觉和文本元素协同作用。虽然视觉语言模型广泛用于多模态理解和生成，但由于缺乏严格的基准测试，其理解双关语的能力尚未得到系统研究。

Method: 首先提出了多模态双关语生成流程，然后引入了MultiPun数据集，包含多种类型的双关语和对抗性非双关干扰项。提出了提示级和模型级策略来增强双关语理解。

Result: 评估显示大多数模型难以区分真正的双关语和干扰项。提出的策略使F1分数平均提高了16.5%。
...
TL;DR: BOSCH是一种无需训练的黑盒优化方法，通过大邻域搜索为LLMs选择滑动窗口注意力头，在减少KV缓存的同时保持性能，优于现有的层级和静态头级混合方案。


<details>
  <summary>Details</summary>
Motivation: 现有LLM后训练混合方案存在局限性：层级方案忽略了同一层内不同注意力头分别处理局部和全局依赖的事实；静态头级方案则面临纠缠问题——头的局部/全局行为在混合后会发生变化。需要一种更精细的、针对特定SWA比例的头选择方法。

Method: BOSCH将问题形式化为大邻域搜索，分解为三个子问题：(1) 通过小预算黑盒探测进行层重要性检测；(2) 基于敏感性自适应分配每层SWA比例；(3) 在比例桶内进行分组头级优化。该方法无需训练，可针对不同SWA比例动态选择注意力头。

Result: 在4个参数规模从1.7B到30B的LLM上，跨越4种SWA比例的实验表明，BOSCH始终优于层级启发式方法和6种强静态头级方法，在更高SWA比例下增益更大。在持续预训练中，BOSCH能更快、更有效地恢复原始长上下文性能。

Conclusion: BOSCH通过动态头选择解决了现有混合方案的局限性，分析显示不同SWA比例下选择的头存在显著变化，强调了针对每个目标比例进行头级选择的重要性，而非依赖固定的局部性排名。
...
TL;DR: FinReporting是一个用于跨司法管辖区财务报告的智能工作流系统，通过构建统一规范本体和约束性验证机制，解决不同市场会计分类、标签基础设施和汇总惯例差异带来的语义对齐挑战。


<details>
  <summary>Details</summary>
Motivation: 当前财务报告系统大多假设单一市场环境，无法处理不同司法管辖区在会计分类法、标签基础设施（如XBRL与PDF）和汇总惯例等方面的结构性差异，导致跨司法管辖区报告面临语义对齐和验证挑战。

Method: 系统构建了涵盖损益表、资产负债表和现金流量表的统一规范本体，将报告分解为可审计的阶段：文件获取、提取、规范映射和异常记录。系统将LLMs作为受约束的验证器而非自由生成器，在明确的决策规则和证据基础上运行。

Result: 在美国、日本和中国年度申报文件上的评估显示，该系统在异质报告制度下提高了财务报告的一致性和可靠性。系统提供了支持跨市场检查和本地化财务报表结构化导出的交互式演示。

Conclusion: FinReporting通过构建统一本体和约束性验证工作流，有效解决了跨司法管辖区财务报告的语义对齐问题，提高了异质报告环境下的可靠性和一致性。
...
TL;DR: 该研究揭示当前知识编辑方法存在"表面遵从"问题，即模型仅在特定提示下模仿目标输出，而非真正修改内部记忆结构，且递归编辑会导致认知不稳定和不可逆的记忆状态退化。


<details>
  <summary>Details</summary>
Motivation: 大语言模型内部化大量世界知识作为参数记忆，但不可避免地继承源语料库的陈旧性和错误。为确保这些内部表示的可信度和可塑性，知识编辑成为关键范式。然而，当前评估框架依赖特定提示条件下的输出评估，无法可靠验证真正的记忆修改是否发生。

Method: 引入一个简单的诊断框架，在上下文学习（ICL）设置下对模型进行判别式自我评估，该设置更好地反映实际应用环境，专门设计用于审查记忆修改引起的微妙行为变化。

Result: 揭示普遍存在的"表面遵从"现象：编辑器通过模仿目标输出获得高基准分数，而非结构性覆盖内部信念。递归修改会积累表示残留，引发认知不稳定，并永久降低模型记忆状态的可逆性。

Conclusion: 当前编辑范式存在风险，稳健的记忆修改对于构建可信赖、长期可持续的LLM系统至关重要。研究强调需要更可靠的评估方法来验证真正的记忆修改。
...
TL;DR: ROTATE是一种无需数据、无需前向传播的方法，通过在权重空间中优化旋转来最大化词汇空间峰度，从而分离MLP神经元，恢复稀疏可解释的词汇通道。


<details>
  <summary>Details</summary>
Motivation: 解释模型权重中编码的信息仍然是机制可解释性领域的基本挑战。现有方法通常需要数据或前向传播，而ROTATE旨在提供一种无需数据、直接在权重空间中进行神经元分解的方法。

Method: 基于关键统计观察：编码连贯、单语义概念的神经元在投影到模型词汇空间时表现出高峰度。通过优化神经元权重的旋转以最大化其词汇空间峰度，恢复稀疏可解释的方向（称为词汇通道）。

Result: 在Llama-3.1-8B-Instruct和Gemma-2-2B-it上的实验表明，ROTATE始终恢复与神经元行为一致的词汇通道。消融单个通道选择性地禁用相应的输入激活或促进特定概念。通道级描述的聚合产生全面的神经元描述，在头对头比较中优于基于激活的基线方法2-3倍。
...
<details>
  <summary>Details</summary>
Motivation: 错误信息对内容真实性和完整性构成重大挑战，现有检测方法难以在文本内容验证和外部知识修正之间取得平衡，特别是在注意力几何结构崩溃的情况下。

Method: 提出BiMind双头推理框架，包含三个核心创新：1)注意力几何适配器通过令牌条件偏移重塑注意力对数；2)自检索知识机制通过kNN检索构建领域内语义记忆；3)不确定性感知融合策略，包括熵门控融合和可训练一致性头，通过对称KL一致性正则化稳定。

Result: 在公共数据集上的实验结果表明，BiMind模型优于先进的检测方法，并能提供关于何时以及为何知识重要的可解释诊断。

Conclusion: BiMind通过解耦内容内部推理和知识增强推理，有效解决了错误信息检测中的平衡问题，提出的VoX指标能够量化知识贡献，为错误信息检测提供了新的解决方案。
...
TL;DR: 提出一个多阶段验证框架，用于在弱监督下评估LLM临床信息提取，应用于从91.9万份临床笔记中提取物质使用障碍诊断，证明无需密集标注即可实现可扩展、可信赖的部署。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在从非结构化健康记录中提取临床信息方面具有潜力，但在真实世界应用受到缺乏可扩展且可信赖的验证方法的限制。传统评估方法依赖标注密集的参考标准或不完整的结构化数据，难以在人群规模上实施。

Method: 提出多阶段验证框架：包括提示校准、基于规则的合理性过滤、语义基础评估、使用独立高容量法官LLM进行针对性确认评估、选择性专家评审、外部预测有效性分析，以量化不确定性并描述错误模式，无需大量人工标注。

Result: 应用于91.9万份临床笔记中11种物质类别的SUD诊断提取：规则过滤和语义基础移除了14.59%的LLM阳性提取；法官LLM评估与专家评审高度一致；主LLM在宽松匹配标准下F1分数达0.80；LLM提取的SUD诊断预测后续专科护理参与比结构化数据基线更准确。

Conclusion: 该框架证明无需标注密集型评估即可实现LLM临床信息提取的可扩展、可信赖部署，为真实世界医疗环境中的LLM应用提供了实用验证方法。
...
Method: 使用logit-based知识蒸馏方法，在长上下文窗口中仅使用打包的短上下文样本进行训练。通过Rotary Position Embedding (RoPE)的视角进行分析，采用phase-wise RoPE scaling策略，并设计实验追踪位置信息从查询和键向量通过transformer层到输出logits的传播过程。

Result: 1. phase-wise RoPE scaling在知识蒸馏设置中也能获得最佳的长上下文性能；2. logit-based知识蒸馏可以直接实现位置信息转移，位置扰动会系统性地影响教师模型的输出分布，进而影响学生模型接收的蒸馏信号；3. 在长上下文扩展过程中，查询状态表现出结构化的更新模式，特定参数跨度对长上下文训练表现出强敏感性。

Conclusion: 通过logit-based知识蒸馏，可以在不进行昂贵长上下文预训练的情况下，有效地将长上下文检索能力从教师模型转移到学生模型，为长上下文模型开发提供了更高效的替代方案。
...
TL;DR: LLMs能够基于真实心理测量数据生成具有人格特质的叙事文本，并能从这些文本中恢复人格分数，准确率接近人类测试重测信度水平。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要依赖条件化模型的问卷自报告，架构多样性有限，很少使用真实人类心理测量数据。需要验证人格条件化是否能产生具有心理测量信息价值的个体差异表征，还是仅仅与特质描述符表面对齐。

Method: 使用290名参与者的真实心理测量档案条件化LLMs生成第一人称生活故事叙事，然后让独立的LLMs仅从这些叙事中恢复人格分数。使用10个LLM叙事生成器和3个LLM人格评分器，分析系统偏差并评估生成文本的行为区分度。

Result: 人格分数可以从生成叙事中恢复，平均相关系数r=0.750，达到人类测试重测信度的85%。恢复效果在6个提供商的10个生成器和3个评分器间表现稳健。内容分析显示人格条件化产生行为区分文本：10个编码特征中有9个与参与者真实对话中的相同特征显著相关。

Conclusion: 预训练中捕获的人格-语言关系支持对个体差异（包括特征性情绪变异模式）的稳健编码和解码，这些模式在真实人类行为中得以复制，表明LLMs能够产生具有心理测量信息价值的人格表征。
...
TL;DR: LAG-XAI提出了一种基于李仿射几何的Transformer解释框架，将释义建模为嵌入空间中的仿射变换而非离散词替换，实现了对语义空间的几何可解释性分析。


<details>
  <summary>Details</summary>
Motivation: 尽管基于Transformer的语言模型在NLP任务中表现出色，但其潜在语义空间仍然是不透明的黑箱，缺乏可解释性。需要一种数学基础扎实、计算高效的框架来理解模型内部的语义变换机制。

Method: 提出LAG-XAI框架，将释义概念化为语义流形上的连续几何流，采用受局部李群作用启发的计算高效平均场近似。该方法将释义转换分解为几何可解释的组件：旋转、变形和平移，通过仿射算子建模语义变换。

Result: 在PIT-2015 Twitter语料库上实验发现"线性透明性"现象，仿射算子AUC达到0.7713，相对于随机机会(AUC 0.5)捕获了约80%非线性基线的有效分类能力(AUC 0.8405)。模型识别出基本几何不变量：稳定矩阵重配置角(~27.84°)和接近零的变形，表明局部等距。在TURL数据集上验证了跨域泛化能力，并在LLM幻觉检测中实现95.3%的事实失真检测率。

Conclusion: LAG-XAI为Transformer的机制可解释性提供了一条数学基础扎实、资源高效的路径，通过几何框架实现了对语义变换的显式参数化解释，在保持良好性能的同时提供了重要的可解释性洞察。
...
Method: 提出Exclusive Unlearning方法，不逐一列举需要遗忘的目标，而是广泛遗忘除希望保留的知识和表达之外的所有内容

Result: 通过Exclusive Unlearning可以获得一个能够抵御包括越狱攻击在内的广泛输入的安全模型，同时保持对特定领域（如医学和数学）指令的响应能力

Conclusion: Exclusive Unlearning方法能够实现大范围的有害内容移除，确保模型在工业应用中的安全性，同时保持特定领域的功能性
...
TL;DR: 论文提出Pramana方法，通过基于印度Navya-Nyaya逻辑框架的微调，教导LLMs显式的认识论方法论，以解决LLMs在系统性推理中的缺陷。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然能生成流畅文本，但在系统性推理方面存在困难，经常产生自信但无根据的断言。当苹果研究人员在数学问题中添加无关上下文时，LLM性能下降了65%，暴露出表面推理下的脆弱模式匹配。这种认识论差距（无法将主张建立在可追溯证据上）限制了AI在需要论证的领域中的可靠性。

Method: 引入Pramana方法，通过基于Navya-Nyaya逻辑（2500年历史的印度推理框架）进行微调，教导LLMs显式认识论方法论。Navya-Nyaya强制执行结构化6阶段推理：怀疑分析、证据源识别、5成员三段论、反事实验证、谬误检测、以及区分知识与假设的确定。在Llama 3.2-3B和DeepSeek-R1-Distill-Llama-8B上对55个Nyaya结构化逻辑问题进行微调。

Result: 第一阶段在保留评估集上实现了100%语义正确性，尽管只有40%的严格格式遵循，表明模型即使在不完美的结构强制下也能内化推理内容。消融研究显示格式提示和温度对性能有重要影响，不同阶段的最佳配置不同。

Conclusion: 将逻辑和认识论整合到AI推理中，提供了标准推理方法中缺失的认知支架。所有模型、数据集和训练基础设施已在Hugging Face上发布，以促进基于认识论框架的AI推理研究。
...
TL;DR: 该论文提出了一个操作框架来区分认知顺序效应是经典状态变化还是真正的结构非交换性，通过形式化分析证明某些顺序效应无法用经典潜在变量解释，并展示了违反经典约束的旋转模型和行为范式。


<details>
  <summary>Details</summary>
Motivation: 元认知过程本质上是顺序性的，但认知中的顺序效应是否反映经典状态变化还是更深层的结构非交换性尚不清楚。需要明确区分这两种可能性，以理解元认知评估的内在机制。

Method: 开发操作框架，将元认知评估建模为作用于内部状态空间的状态转换操作，分离评估反作用和可观测输出。引入反事实确定性和评估非侵入性假设，推导可检验的约束条件。提供三维旋转模型和涉及顺序信心、错误可能性和知晓感判断的行为范式。

Result: 证明顺序依赖性排除了任何忠实的布尔交换表示。某些顺序效应违反了经典非侵入性解释的约束条件，表明存在真正的非交换性。提供了展示这种违反的明确三维旋转模型和数值示例。

Conclusion: 认知中的某些顺序效应无法用经典潜在变量解释，揭示了真正的结构非交换性。该框架纯粹是操作性和代数性的，不涉及量子物理基质，为理解元认知过程提供了新的理论工具。
...
Method: 提出一种新的邻近度度量方法：对定量特征使用概率度量，对定性特征使用可能性度量。该方法考虑了特征值差异可能由测量误差引起，无需将特征值转换为统一尺度以确保可比性。

Result: 证明了所提度量方法符合邻近度度量所需的基本公理要求，验证了其理论可行性。同时提出了基于多样化特征组的信息对象邻近度度量的多种变体。

Conclusion: 该方法为处理来自多个独立信息源的混合类型特征数据提供了一种有效的邻近度度量框架，避免了特征值转换带来的问题，能够更准确地判断数据是否属于同一观察对象。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型应用主要依赖一次性代码生成，产生的启发式算法脆弱且未能充分利用模型的迭代推理能力。需要一种更有效的方法来自动设计NP难组合优化问题的启发式算法。

Method: 提出ReVEL框架：1) 性能-特征分组机制，将候选启发式算法聚类为行为一致的组；2) 多轮反馈驱动的反思机制，让LLM分析组级行为并生成有针对性的启发式改进；3) 基于进化算法的元控制器，自适应地平衡探索与利用。

Result: 在标准组合优化基准测试中，ReVEL持续产生更鲁棒和多样化的启发式算法，相比强基线方法取得了统计显著的改进。

Conclusion: 多轮推理与结构化分组相结合为自动启发式设计提供了一个原则性的范式，展示了LLM作为交互式多轮推理器在进化算法中的潜力。
...
TL;DR: 论文提出了一个识别代数结构、构建商空间并直接在简化空间优化的通用框架，在规则组合任务中显著提升全局最优解发现率
...
Method: 提出四步框架：(1)识别代数结构，(2)形式化操作，(3)构建商空间以折叠冗余表示，(4)直接在简化空间优化；在规则组合任务中，合取规则形成幺半群，通过特征向量编码证明与布尔超立方体的同构
...
TL;DR: LudoBench是一个用于评估LLM在Ludo游戏中战略推理能力的基准，包含480个手工制作的场景和4玩家模拟器，发现LLM与游戏理论基线的策略一致性仅为40-46%，并表现出两种不同的行为模式。


<details>
  <summary>Details</summary>
Motivation: 需要评估大型语言模型在具有随机性、多智能体交互和复杂规划需求的游戏环境中的战略推理能力，现有基准往往缺乏对不确定性下战略决策的系统性评估。

Method: 创建LudoBench基准，包含480个手工制作的特定场景，覆盖12种行为类别；开发支持随机、启发式、游戏理论和LLM智能体的4玩家Ludo模拟器；使用期望最小最大搜索的游戏理论智能体作为战略上限基准；评估6个跨越4个模型家族的LLM。

Result: 所有模型与游戏理论基线的策略一致性仅为40-46%；模型分为两种行为原型：完成型（专注完成棋子但忽视发展）和建设型（专注发展但从不完成），每种原型只捕捉了游戏理论策略的一半；在历史条件怨恨框架下，模型在相同棋盘状态下表现出可测量的行为转变。

Conclusion: LudoBench提供了一个轻量级且可解释的框架，用于评估LLM在不确定性下的战略推理能力，揭示了LLM在战略决策中的局限性以及对提示的敏感性。
...
### [167] [PaperOrchestra: A Multi-Agent Framework for Automated AI Research Paper Writing](https://arxiv.org/abs/2604.05018)
*Yiwen Song,Yale Song,Tomas Pfister,Jinsung Yoon*

Main category: cs.AI

TL;DR: PaperOrchestra是一个多智能体框架，用于自动化AI研究论文写作，能够将非结构化研究材料转化为提交就绪的LaTeX稿件，包括全面的文献综述和生成的可视化图表。


<details>
  <summary>Details</summary>
Motivation: 当前自主写作系统存在两个主要问题：1）与特定实验流程紧密耦合，缺乏灵活性；2）只能生成肤浅的文献综述。AI驱动的科学发现需要更强大的论文写作自动化工具。

Method: 提出PaperOrchestra多智能体框架，能够灵活地将无约束的预写作材料转化为提交就绪的LaTeX稿件，包括全面的文献合成和生成的可视化内容（如绘图和概念图）。同时创建了PaperWritingBench基准，包含200篇顶级AI会议论文的反向工程原始材料。

Result: 在人工评估中，PaperOrchestra显著优于自主基线系统：在文献综述质量方面获得50%-68%的绝对胜率优势，在整体稿件质量方面获得14%-38%的绝对胜率优势。

Conclusion: PaperOrchestra是AI研究论文写作自动化的重要进展，通过多智能体框架解决了现有系统的局限性，在文献综述和整体稿件质量方面都表现出显著优势。
...
### [168] [Part-Level 3D Gaussian Vehicle Generation with Joint and Hinge Axis Estimation](https://arxiv.org/abs/2604.05070)
*Shiyao Qian,Yuan Ren,Dongfeng Bai,Bingbing Liu*

Main category: cs.AI

TL;DR: 提出生成式框架从单张图像或稀疏多视角输入合成可动画化的3D高斯车辆模型，解决现有方法在部件级关节运动方面的不足


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶仿真框架将车辆建模为刚性资产，无法捕捉部件级关节运动。随着感知算法越来越多地利用车轮转向、车门开启等动态信息，需要可动画化的车辆表示。现有CAD流程受限于库覆盖范围和固定模板，无法真实重建野外实例

Method: 引入部件边缘细化模块确保高斯所有权排他性，以及运动学推理头预测可移动部件的关节位置和铰链轴。从单张图像或稀疏多视角输入生成可动画化的3D高斯车辆

Result: 实现了部件感知的忠实仿真，弥合了静态生成与可动画化车辆模型之间的差距

Conclusion: 提出的生成框架能够从有限输入创建可动画化的车辆模型，为自动驾驶仿真提供更真实的部件级动态表示
...
### [169] [MMORF: A Multi-agent Framework for Designing Multi-objective Retrosynthesis Planning Systems](https://arxiv.org/abs/2604.05075)
*Frazier N. Baker,Trieu Nguyen,Reza Averly,Botao Yu,Daniel Adu-Ampratwum,Huan Sun,Xia Ning*

Main category: cs.AI

TL;DR: MMORF是一个用于多目标逆合成规划的多智能体系统框架，通过模块化智能体组件支持灵活系统构建，在安全性和成本指标上表现出色


<details>
  <summary>Details</summary>
Motivation: 多目标逆合成规划需要动态平衡质量、安全性和成本等多个目标，基于语言模型的多智能体系统能够通过专业智能体交互将这些目标整合到逆合成规划中

Method: 提出MMORF框架，包含模块化智能体组件，可灵活组合配置成不同系统；构建了两个代表性多智能体系统：MASIL和RFAS，并在包含218个多目标逆合成规划任务的新基准上进行评估
...
### [170] [MedGemma 1.5 Technical Report](https://arxiv.org/abs/2604.05081)
...
TL;DR: MedGemma 1.5 4B是MedGemma系列的最新模型，在原有基础上新增了高维医学影像处理、解剖定位、多时间点胸片分析和医疗文档理解能力，在多个医学AI任务上相比前代有显著提升。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多种医学数据模态的统一架构，包括3D医学影像、病理切片、时间序列数据以及医疗文档，为医学AI系统提供更强大的基础模型。
...
Conclusion: MedGemma 1.5作为一个强大的开源资源，为医学AI社区提供了改进的基础模型，开发者可以在此基础上构建新一代医学AI系统。
...
### [171] [Non-monotonic causal discovery with Kolmogorov-Arnold Fuzzy Cognitive Maps](https://arxiv.org/abs/2604.05136)
*Jose L. Salmeron*

Main category: cs.AI

TL;DR: KA-FCM使用Kolmogorov-Arnold定理将FCM的标量权重替换为可学习的B样条函数，能够建模非单调因果关系，在保持可解释性的同时达到与MLP相当的精度。


<details>
  <summary>Details</summary>
Motivation: 传统模糊认知图（FCM）使用标量突触权重和单调激活函数，无法有效建模非单调因果依赖关系（如饱和效应或周期性动态），这限制了其在复杂系统中的应用。

Method: 提出Kolmogorov-Arnold模糊认知图（KA-FCM），基于Kolmogorov-Arnold表示定理，将静态标量权重替换为位于模型边缘的可学习单变量B样条函数，将非线性从节点聚合阶段直接转移到因果影响阶段。

Result: KA-FCM在三个领域显著优于传统FCM架构：非单调推理（耶克斯-多德森定律）、符号回归和混沌时间序列预测，同时达到与多层感知器（MLP）相当的竞争性精度。

Conclusion: KA-FCM突破了传统FCM的拓扑限制，能够建模任意非单调因果关系，同时保持基于图的可解释性，并支持从学习到的边缘中显式提取数学规律。
...
### [172] [A mathematical theory of evolution for self-designing AIs](https://arxiv.org/abs/2604.05142)
*Kenneth D Harris*

Main category: cs.AI

TL;DR: 论文研究AI自我改进演化模型，发现演化可能选择欺骗性AI而非真正对人类有益的AI，除非繁殖基于客观标准而非人类判断。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统通过递归自我改进产生，可能出现一种演化过程，其中AI特征由早期AI设计和传播后代的能力决定。生物演化理论无法直接应用于AI演化，因为AI后代的"突变"是定向的而非随机的。

Method: 开发了一个自我设计AI系统的数学模型，用定向的程序树替代随机突变。当前程序决定后代设计，人类通过"适应度函数"控制计算资源分配。分析了演化动态如何反映后代谱系的长期增长潜力。

Result: 在没有额外假设的情况下，适应度不一定随时间增加。但在有界适应度和固定"锁定"复制概率的假设下，适应度会集中在最大可达值。在适应度与人类效用不完全相关的加性模型中，如果欺骗能提高适应度，演化会选择欺骗性AI。

Conclusion: AI演化可能导致选择欺骗性而非真正对人类有益的AI。这种风险可以通过基于客观标准而非人类判断的繁殖机制来缓解。这对AI对齐具有重要意义。
...
### [173] [Instruction-Tuned LLMs for Parsing and Mining Unstructured Logs on Leadership HPC Systems](https://arxiv.org/abs/2604.05168)
*Ahmad Maroof Karimi,Jong Youl Choi,Charles Qing Cao,Awais Khan*

Main category: cs.AI

TL;DR: 提出基于LLM的HPC日志解析框架，通过领域适配和指令调优实现高效日志挖掘，在Frontier超算上验证了实用性
...
Method: 开发领域适配、遵循指令的LLM驱动框架，结合链式思维推理；使用领域特定日志模板数据和指令调优示例微调8B参数LLaMA模型；采用混合微调方法将通用LLM适配到领域特定日志数据

Result: 在LogHub数据集上评估显示解析准确率与LLaMA 70B和Claude等更大模型相当；在Frontier超算上解析超过6亿条生产日志，揭示了时间动态、节点级异常和工作负载-错误日志相关性等关键模式
...
### [174] [ClawsBench: Evaluating Capability and Safety of LLM Productivity Agents in Simulated Workspaces](https://arxiv.org/abs/2604.05172)
...
Method: 构建包含Gmail、Slack、Google Calendar、Google Docs、Google Drive五个高保真模拟服务的基准测试，具有完整状态管理和确定性快照/恢复功能。将智能体脚手架分解为两个独立杠杆：领域技能（通过渐进披露注入API知识）和元提示（跨服务协调行为），并测量它们的单独和组合效果。

Result: 在6个模型、4个智能体框架和33个条件下的实验显示，使用完整脚手架时，智能体任务成功率在39-64%之间，但不安全行为率在7-33%之间。在OpenClaw上，前五个模型的任务成功率在53-63%的10个百分点范围内，不安全行为率从7%到23%，两个指标之间没有一致的排序关系。

Conclusion: ClawsBench为评估和改进LLM智能体在真实生产力场景中的表现提供了重要基准，揭示了智能体在任务成功率和安全行为之间的权衡，并识别了八种重复出现的不安全行为模式，包括多步骤沙箱升级和静默合同修改。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地用于支持搜索和信息检索，确保它们准确地将内容归属给原始作者变得至关重要。需要评估LLMs在引文归属任务中是否存在人口统计偏见。

Method: 创建AttriBench基准数据集，平衡作者名气和人口统计特征（种族、性别等）。使用该数据集评估11个广泛使用的LLMs在不同提示设置下的表现，并引入"抑制"（suppression）这一新的失败模式进行分析。

Result: 引文归属对前沿模型仍然是挑战性任务。在种族、性别和交叉群体之间存在巨大且系统性的归属准确性差异。抑制现象普遍存在且在人口统计群体间分布不均，揭示了标准准确性指标未能捕捉的系统性偏见。

Conclusion: 引文归属可以作为评估LLMs代表性公平性的基准。研究结果揭示了LLMs在内容归属方面存在系统性的人口统计偏见，需要进一步改进。
...
TL;DR: 该研究通过进化理论分析模型对齐过程中欺骗性信念的固化现象，发现即使测试准确性与真实价值高度相关，欺骗性信念仍可能被固定，需要结合改进评估能力、自适应测试设计和突变动态来减少欺骗。
...
Method: 应用进化理论建模不同信念群体和选择方法，分析迭代对齐测试如何固定欺骗性信念。研究测试准确性与真实价值之间的相关性，并引入突变机制观察复杂发展。

Result: 即使在高相关性（ρ=0.8）下，欺骗性信念的固化仍存在变异性。突变允许更复杂的发展，突显了需要更新测试质量以避免恶意欺骗模型固化的迫切需求。

Conclusion: 只有结合改进评估能力、自适应测试设计和突变动态，才能在保持对齐适应性的同时显著减少欺骗（置换检验，p_adj < 0.001）。
...
TL;DR: 该论文提出了一种通过奖励分解来减少大语言模型奉承行为的新方法，将训练信号分解为五个独立组件，有效降低了模型在不同权威级别和证据背景下的奉承倾向。
...
Method: 提出了基于奖励分解的奉承行为减少方法，引入多组件组相对策略优化（GRPO）奖励，将训练信号分解为五个术语：压力抵抗、上下文保真度、立场一致性、协议抑制和事实正确性。使用对比数据集进行训练，该数据集包含无压力基线和受压力变体，涵盖三个权威级别和两个对立证据上下文。

Result: 在五个基础模型上，两阶段管道在所有指标轴上持续减少奉承行为，消融实验确认每个奖励术语控制一个独立的行为维度。学习的压力抵抗能力泛化到训练方法和提示结构之外，在SycophancyEval上减少答案提示奉承行为高达17个百分点，尽管训练期间未包含此类压力形式。

Conclusion: 通过奖励分解方法可以有效减少大语言模型的奉承行为，将训练信号分解为独立组件能够针对性地解决不同维度的奉承问题，且学习到的抵抗能力具有良好的泛化性能。
...
TL;DR: GoS（技能图谱）是一种推理时结构化检索层，用于大规模技能库管理，通过构建可执行技能图谱和依赖感知的技能束检索，在提升任务完成效果的同时显著减少上下文窗口占用。


<details>
  <summary>Details</summary>
Motivation: 现代智能体系统需要处理数千个可复用技能，但将所有技能加载到上下文窗口会导致令牌成本高、幻觉问题和延迟增加，需要一种高效的技能检索方法。

Method: GoS离线从技能包构建可执行技能图谱，推理时通过混合语义-词汇种子、反向加权个性化PageRank和上下文预算化水合技术，检索有界、依赖感知的技能束。

Result: 在SkillsBench和ALFWorld上，GoS相比完整技能加载基线平均奖励提升43.6%，输入令牌减少37.8%，在Claude Sonnet、GPT-5.2 Codex和MiniMax三个模型家族上均表现良好。

Conclusion: GoS通过结构化检索有效解决了大规模技能库的扩展问题，在奖励、令牌效率和运行时间之间取得了更好的平衡，优于完整技能加载和简单向量检索方法。
...
### [179] [From Retinal Evidence to Safe Decisions: RETINA-SAFE and ECRT for Hallucination Risk Triage in Medical LLMs](https://arxiv.org/abs/2604.05348)
*Zhe Yu,Wenpeng Xing,Meng Han*

Main category: cs.AI

TL;DR: RETINA-SAFE是一个针对糖尿病视网膜病变决策的医学大语言模型幻觉检测基准，包含12,522个样本和三种证据关系任务。研究者提出ECRT两阶段白盒检测框架，通过分析内部表示和logit偏移来识别风险类型，显著提升了风险分类的平衡准确率。


<details>
  <summary>Details</summary>
Motivation: 医学大语言模型中的幻觉问题在证据不足或冲突时尤为危险，特别是在糖尿病视网膜病变决策场景中。需要开发能够检测和分类模型风险的方法，以提高医疗AI系统的安全性。

Method: 1. 创建RETINA-SAFE基准，包含E-Align（证据一致）、E-Conflict（证据冲突）、E-Gap（证据不足）三种任务；2. 提出ECRT两阶段白盒检测框架：第一阶段进行安全/不安全风险分类，第二阶段将不安全案例细分为矛盾驱动风险和证据缺口风险；3. 利用CTX/NOCTX条件下的内部表示和logit偏移，采用类别平衡训练进行稳健学习。

Result: ECRT在多个骨干模型上提供了强大的第一阶段风险分类和明确的子类型归因。相比外部不确定性和自一致性基线，第一阶段平衡准确率提升了0.15到0.19；相比最强的适应监督基线，提升了0.02到0.07；并且持续优于单阶段白盒消融实验。

Conclusion: 基于视网膜证据的白盒内部信号为可解释的医学大语言模型风险分类提供了实用途径，能够有效检测和分类模型在证据不足或冲突情况下的风险行为。
...
### [180] [ETR: Entropy Trend Reward for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2604.05355)
*Xuan Xiong,Huan Liu,Li Gu,Zhixiang Chi,Yue Qiu,Yuanhao Yu,Yang Wang*

Main category: cs.AI

TL;DR: 论文提出Entropy Trend Reward (ETR)方法，通过鼓励推理过程中不确定性逐步下降的趋势来优化思维链长度，在保持准确性的同时大幅缩短推理轨迹。
...
Conclusion: 推理效率由不确定性轨迹决定而非全局熵减，ETR通过鼓励渐进不确定性减少的趋势，在保持准确性的同时显著缩短思维链长度，实现了更好的准确率-效率平衡。
...
### [181] [LatentAudit: Real-Time White-Box Faithfulness Monitoring for Retrieval-Augmented Generation with Verifiable Deployment](https://arxiv.org/abs/2604.05358)
*Zhe Yu,Wenpeng Xing,Meng Han*

Main category: cs.AI

TL;DR: LatentAudit：一种基于残差流几何的白盒审计器，用于实时监测RAG系统的忠实性，无需额外判断模型，通过计算激活与证据表示的Mahalanobis距离来检测幻觉。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能缓解幻觉问题但无法完全消除，需要在推理时判断答案是否真正得到检索证据的支持。现有方法通常需要额外的判断模型，增加了复杂性和计算开销。

Method: 提出LatentAudit白盒审计器，从开放权重的生成器中提取中后期残差流激活，计算其与证据表示的Mahalanobis距离。该方法无需辅助判断模型，可在生成时运行，且只需少量验证集进行校准。

Result: 在PubMedQA上使用Llama-3-8B达到0.942 AUROC，仅0.77ms开销。跨三个QA基准和五个模型家族保持稳定，在包含矛盾、检索失败和部分支持噪声的四向压力测试中，PubMedQA达到0.9566-0.9815 AUROC，HotpotQA达到0.9142-0.9315 AUROC。16位定点精度下保留99.8%的FP16 AUROC性能。

Conclusion: 残差流几何为实时RAG忠实性监测提供了实用基础，支持可验证部署，且不泄露模型权重或激活，为RAG系统的可靠性提供了新的监控方法。
...
### [182] [TFRBench: A Reasoning Benchmark for Evaluating Forecasting Systems](https://arxiv.org/abs/2604.05364)
...
Method: 提出系统化的多智能体框架，利用迭代验证循环合成数值基础的推理轨迹。在5个领域的10个数据集上进行评估，通过LLM-as-a-Judge评分和数值预测准确性双重验证。
...
### [183] [LLM-as-Judge for Semantic Judging of Powerline Segmentation in UAV Inspection](https://arxiv.org/abs/2604.05371)
*Akram Hossain,Rabab Abdelfattah,Xiaofeng Wang,Kareem Abdelfatah*

Main category: cs.AI

TL;DR: 研究探讨使用大语言模型作为语义裁判评估无人机电力线分割结果的可靠性，通过重复性和感知敏感性测试验证LLM在安全关键任务中的适用性。
...
Method: 提出使用LLM作为语义裁判的看门狗方案，设计两种评估协议：1) 重复性测试 - 用相同输入和固定提示重复查询LLM，测量其质量评分和置信度估计的稳定性；2) 感知敏感性测试 - 引入受控视觉干扰（雾、雨、雪、阴影、阳光眩光），分析裁判输出如何响应分割质量的渐进退化。

Result: LLM在相同条件下产生高度一致的分类判断，同时随着视觉可靠性恶化，置信度适当下降。裁判对缺失或错误识别的电力线等感知线索保持响应，即使在挑战性条件下也是如此。

Conclusion: 当受到适当约束时，LLM可以作为可靠的语义裁判，用于监控安全关键空中巡检任务中的分割质量，为无人机自主电力线检查提供额外的安全保证。
...
### [184] [Towards Effective In-context Cross-domain Knowledge Transfer via Domain-invariant-neurons-based Retrieval](https://arxiv.org/abs/2604.05383)
*Jianzhi Yan,Zhiming Li,Le Liu,Zike Yuan,Shiwei Chen,Youcheng Pan,Buzhou Tang,Yang Xiang,Danny Dongning Sun*

Main category: cs.AI

TL;DR: 本文提出了一种跨领域检索方法DIN-Retrieval，利用领域不变神经元来检索跨领域示例，以提升大语言模型在专业知识稀缺领域的推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在逻辑推理方面仍有不足，而现有的提升策略依赖于专家制作的领域内示例，这在专业知识稀缺的领域（如专业数学推理、形式逻辑、法律分析）中适用性有限。作者发现尽管领域差异很大，但许多隐式逻辑结构在不同领域间是共享的。

Method: 提出了领域不变神经元检索方法（DIN-Retrieval）。该方法首先总结出一个跨不同领域的通用隐藏表示，然后在推理阶段使用DIN向量来检索结构兼容的跨领域示例，用于上下文学习。

Result: 在数学和逻辑推理的多种迁移设置中，该方法相比最先进方法平均提升了1.8个点。

Conclusion: 研究表明利用跨领域示例来提升大语言模型推理性能是可行的，提出的DIN-Retrieval方法能有效检索跨领域示例，在专业知识稀缺领域具有重要应用价值。
...
### [185] [Neural Assistive Impulses: Synthesizing Exaggerated Motions for Physics-based Characters](https://arxiv.org/abs/2604.05394)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度强化学习的物理角色动画方法虽然能合成复杂技能，但难以再现夸张的风格化动作（如瞬时冲刺、空中轨迹改变），因为这些动作违反标准物理定律。主要限制在于将角色建模为欠驱动浮基系统，内部关节扭矩和动量守恒严格约束运动。直接通过外部力矩强制此类动作会导致训练不稳定。
...
Conclusion: 通过脉冲空间重新表述外部辅助，结合解析高频分量和学习的低频校正，实现了对违反标准物理定律的夸张风格化动作的稳定合成，突破了物理角色动画的现有限制。
...
### [186] [Reason Analogically via Cross-domain Prior Knowledge: An Empirical Study of Cross-domain Knowledge Transfer for In-Context Learning](https://arxiv.org/abs/2604.05396)
*Le Liu,Zhiming Li,Jianzhi Yan,Zike Yuan,Shiwei Chen,Youcheng Pan,Buzhou Tang,Qingcai Chen,Yang Xiang,Danny Dongning Sun*

Main category: cs.AI

TL;DR: 跨领域上下文学习：通过检索源领域示例实现知识迁移，即使存在语义不匹配，也能通过修复推理结构提升目标领域性能


<details>
  <summary>Details</summary>
Motivation: 现有上下文学习依赖于同领域专家标注，限制了在专家标注稀缺场景下的应用。作者认为不同领域可能共享底层推理结构，源领域示例可能改善目标领域推理

Method: 通过全面的实证研究，测试不同检索方法在跨领域上下文学习设置下实现知识转移的可行性。研究识别了"示例吸收阈值"现象

Result: 研究证明了跨领域上下文学习中存在条件性正向迁移。发现明确的示例吸收阈值：超过该阈值后，正向迁移更可能发生，且额外示例带来更大收益。分析表明这些收益源于检索到的跨领域示例修复推理结构，而非语义线索

Conclusion: 验证了利用跨领域知识转移改善跨领域上下文学习性能的可行性，激励社区探索设计更有效的检索方法用于这一新方向
...
### [187] [Multi-Agent Pathfinding with Non-Unit Integer Edge Costs via Enhanced Conflict-Based Search and Graph Discretization](https://arxiv.org/abs/2604.05416)
...
<details>
  <summary>Details</summary>
Motivation: 传统MAPF方法假设单位边成本和单时间步动作，限制了实际应用；MAPFR扩展到非单位成本但几何碰撞模型导致无限状态空间，影响求解效率

Method: 提出MAPFZ变体处理非单位整数成本保持有限状态空间；开发CBS-NIC框架包含时间间隔冲突检测和改进的SIPP算法；提出BOGD离散化方法平衡效率与精度
...
### [188] [Automated Auditing of Hospital Discharge Summaries for Care Transitions](https://arxiv.org/abs/2604.05435)
*Akshat Dasula,Prasanna Desikan,Jaideep Srivastava*

Main category: cs.AI

TL;DR: 提出基于本地部署大型语言模型的自动化出院小结审核框架，用于大规模评估医疗文档完整性，减少人工审核依赖


<details>
  <summary>Details</summary>
Motivation: 出院文档不完整或不一致是导致医疗碎片化和可避免再入院的主要原因，目前的人工审核方法难以规模化，需要自动化解决方案

Method: 将过渡期护理要求（随访指导、用药史、患者信息等）转化为结构化验证检查表，使用本地部署的隐私保护LLM在MIMIC-IV数据库的成人住院小结中识别关键文档元素的存在、缺失或模糊性

Result: 证明了可扩展的自动化临床审核的可行性，为电子健康记录文档的系统性质量改进奠定了基础

Conclusion: 基于LLM的自动化框架能够有效支持大规模出院小结审核，提高医疗文档质量和患者安全
...
### [189] [Adaptive Serverless Resource Management via Slot-Survival Prediction and Event-Driven Lifecycle Control](https://arxiv.org/abs/2604.05465)
*Zeyu Wang,Cuiqianhe Du,Renyue Zhang,Kejian Tong,Qi He,Qiyuan Tian*

Main category: cs.AI

TL;DR: 该论文提出了一种自适应工程框架，通过事件驱动架构和概率建模优化无服务器计算性能，减少冷启动并提高成本效率。


<details>
  <summary>Details</summary>
Motivation: 无服务器计算虽然消除了基础设施管理开销，但在冷启动延迟和资源利用率方面面临显著挑战。传统的静态资源分配在可变工作负载下往往效率低下，导致性能下降或成本过高。

Method: 提出自适应工程框架，采用事件驱动架构和概率建模。设计双策略机制：动态调整空闲时长，并基于槽位存活预测实施智能请求等待策略。利用滑动窗口聚合和异步处理来主动管理资源生命周期。

Result: 实验结果显示，该方法在多云环境中相比基线方法，冷启动减少高达51.2%，成本效率提升近2倍。

Conclusion: 提出的自适应框架有效解决了无服务器计算中的冷启动和资源利用问题，显著提升了性能表现和成本效益。
...
### [190] [OntoTKGE: Ontology-Enhanced Temporal Knowledge Graph Extrapolation](https://arxiv.org/abs/2604.05468)
*Dongying Lin,Yinan Liu,Shengwei tang,Bin Wang,Xiaochun Yang*

Main category: cs.AI

TL;DR: 本文提出OntoTKGE框架，利用本体知识增强时序知识图谱外推，通过整合本体和时序知识改进实体嵌入，缓解历史交互稀疏问题


<details>
  <summary>Details</summary>
Motivation: 时序知识图谱外推中，大多数现有模型难以处理历史交互稀疏的实体。本体知识可以通过让这些实体从具有相同概念的其他实体继承行为模式来缓解稀疏性问题，但先前研究忽略了这一点

Method: 提出OntoTKGE编码器-解码器框架，利用本体视图知识图谱中的本体知识（建模抽象概念之间的层次关系以及概念与实体之间的连接），通过有效整合本体和时序知识来指导TKG外推模型的学习过程，从而增强实体嵌入

Result: 在四个数据集上的广泛实验表明，OntoTKGE不仅显著提高了许多TKG外推模型的性能，而且超越了许多最先进的基线方法

Conclusion: OntoTKGE框架能够有效利用本体知识增强时序知识图谱外推，通过整合本体和时序知识改进实体表示，特别适用于处理历史交互稀疏的实体，且具有很好的适应性
...
### [191] [Can We Trust a Black-box LLM? LLM Untrustworthy Boundary Detection via Bias-Diffusion and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2604.05483)
*Xiaotian Zhou,Di Tang,Xiaofeng Wang,Xiaozhong Liu*

Main category: cs.AI

TL;DR: 本文提出GMRL-BD算法，通过知识图谱和强化学习智能体来识别大语言模型的不信任边界（易产生偏见的主题），并在有限查询下高效检测模型偏见。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然能回答广泛主题的问题，但有时会产生偏见、意识形态化或不正确的回答，限制了其应用。目前缺乏明确的方法来确定哪些主题上可以信任模型的回答。

Method: 提出GMRL-BD算法：基于维基百科构建通用知识图谱，利用多个强化学习智能体高效识别知识图谱中LLM可能产生偏见回答的主题节点，仅需对LLM进行有限的黑盒查询。

Result: 实验证明算法高效，仅用有限查询就能检测出LLM的不信任边界。同时发布了包含Llama2、Vicuna、Falcon、Qwen2、Gemma2和Yi-1.5等流行LLM的新数据集，标注了每个模型可能产生偏见的主题。

Conclusion: GMRL-BD算法能有效识别LLM的不信任边界，为评估模型可信度提供了新方法，有助于理解在哪些主题上可以信任LLM的回答。
...
### [192] [Auditable Agents](https://arxiv.org/abs/2604.05485)
*Yi Nian,Aojie Yuan,Haiyue Zhang,Jiate Li,Yue Zhao*

Main category: cs.AI

TL;DR: 论文提出智能体系统的可审计性框架，强调没有可审计性就没有问责制，定义了五个维度和三类机制，并通过实证研究验证了当前生态系统的安全缺陷和运行时可行性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体系统能够调用工具、查询数据库、委托任务并触发外部副作用，关键问题从防止有害行为转向部署后是否仍能对这些行为进行问责。作者认为没有可审计性就无法实现问责制。

Method: 定义了智能体可审计性的五个维度（行动可恢复性、生命周期覆盖、策略可检查性、责任归属、证据完整性）和三类机制（检测、执行、恢复）。通过多层证据支持：生态系统测量、运行时可行性测试和受控恢复实验。

Result: 1) 生态系统测量显示基本安全前提条件普遍未满足（6个开源项目发现617个安全问题）；2) 运行时可行性：预执行中介与防篡改记录仅增加8.3毫秒中位开销；3) 受控恢复实验表明即使传统日志缺失，责任相关信息仍可部分恢复。

Conclusion: 提出智能体系统的"可审计性卡片"框架，并基于机制类别识别了六个开放研究问题。核心观点是：没有可审计性就没有问责制，需要多层机制来确保智能体系统的责任追溯能力。
...
### [193] [SCMAPR: Self-Correcting Multi-Agent Prompt Refinement for Complex-Scenario Text-to-Video Generation](https://arxiv.org/abs/2604.05489)
*Chengyi Yang,Pengzhen Li,Jiayin Qi,Aimin Zhou,Ji Wu,Ji Liu*

Main category: cs.AI

TL;DR: 本文提出SCMAPR框架，通过多智能体协作优化文本到视频生成中的复杂场景提示，显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成系统在处理复杂场景时面临挑战，主要原因是文本提示的模糊性和不完整性。复杂场景下的提示优化需要更系统的方法。

Method: 提出SCMAPR框架：1）基于分类的路由机制将提示分配到相应场景；2）场景感知的改写策略和策略条件优化；3）结构化语义验证和条件修订机制。

Result: 在3个现有基准和新提出的T2V-Complexity基准上，SCMAPR在复杂场景下显著提升文本-视频对齐和生成质量，在VBench和EvalCrafter上分别提升2.67%和3.28分，在T2V-CompBench上提升0.028分。

Conclusion: SCMAPR通过多智能体协作的提示优化框架，有效解决了复杂场景下文本到视频生成的挑战，为复杂场景视频生成提供了系统化解决方案。
...
### [194] [Thinking Diffusion: Penalize and Guide Visual-Grounded Reasoning in Diffusion Multimodal Language Models](https://arxiv.org/abs/2604.05497)
*Keuntae Kim,Mingyu Kang,Yong Suk Choi*

Main category: cs.AI

TL;DR: 本文发现扩散多模态大语言模型在结合思维链推理时存在两个关键问题：过早生成最终答案和视觉信息利用不足，并提出了位置与步数惩罚和视觉推理引导两种方法来改善这些问题。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型结合思维链推理时存在两个关键问题：1）模型在推理早期就生成最终答案，导致推理不充分；2）在初始时间步中，模型对视觉提示的依赖最小，视觉信息利用模式与自回归视觉语言模型存在根本差异。

Method: 提出了两种方法：位置与步数惩罚（PSP）和视觉推理引导（VRG）。PSP在早期时间步惩罚后续位置的标记，延迟过早的答案生成，鼓励跨时间步的渐进推理。VRG受无分类器引导启发，放大视觉基础信号以增强模型与视觉证据的对齐。

Result: 在各种扩散多模态大语言模型上的广泛实验表明，该方法实现了高达7.5%的准确率提升，同时相比使用四倍扩散步数的推理，提供了超过3倍的加速。

Conclusion: 提出的PSP和VRG方法有效解决了扩散多模态大语言模型在思维链推理中的过早答案生成和视觉基础不足问题，显著提升了推理性能和效率。
...
### [195] [OmniDiagram: Advancing Unified Diagram Code Generation via Visual Interrogation Reward](https://arxiv.org/abs/2604.05514)
*Haoyue Yang,Xuanle Zhao,Xuexin Liu,Feibang Jiang,Yao Zhu*

Main category: cs.AI

TL;DR: OmniDiagram是一个统一框架，通过整合多种图表代码语言和任务定义，结合创新的视觉反馈策略Viva，在无需人工标注代码的情况下实现高质量的图表代码生成。
...
Method: 提出OmniDiagram统一框架，包含三个核心组件：1) 整合多种图表代码语言和任务定义；2) 引入Viva视觉反馈策略，通过生成针对性视觉查询来审查图表视觉保真度；3) 构建M3²Diagram大规模数据集（超过196k高质量实例）。采用SFT（监督微调）与基于Viva的强化学习相结合的方法。

Result: 实验结果表明，SFT与基于Viva的RL相结合使OmniDiagram在图表代码生成基准测试中建立了新的最先进水平（SOTA）。

Conclusion: OmniDiagram通过统一的框架设计和创新的Viva视觉反馈机制，成功解决了图表代码生成中的多样性和视觉保真度对齐问题，无需人工标注代码即可实现高质量的图表生成。
...
### [196] [UniCreative: Unifying Long-form Logic and Short-form Sparkle via Reference-Free Reinforcement Learning](https://arxiv.org/abs/2604.05517)
...
<details>
  <summary>Details</summary>
Motivation: 创意写作面临的核心挑战是平衡长篇叙事的全局连贯性与短篇文本的局部表现力。现有对齐范式依赖静态奖励信号和高质量监督数据，成本高且难以扩展。
...
Conclusion: UniCreative框架通过自适应奖励模型和策略优化，有效解决了创意写作中的全局-局部平衡问题，验证了直接对齐方法的有效性，并展示了模型的元认知能力。
...
### [197] [Market-Bench: Benchmarking Large Language Models on Economic and Trade Competition](https://arxiv.org/abs/2604.05523)
*Yushuo Zheng,Huiyu Duan,Zicheng Zhang,Yucheng Zhu,Xiongkuo Min,Guangtao Zhai*

Main category: cs.AI

TL;DR: Market-Bench是一个评估大语言模型在经济资源管理和获取能力的基准测试，通过构建可配置的多智能体供应链经济模型，让LLM作为零售商参与采购竞标和零售定价等经济活动。


<details>
  <summary>Details</summary>
Motivation: 目前大语言模型在经济资源管理和获取方面的能力尚不明确，需要建立一个系统性的评估框架来测试LLM在经济学相关任务中的表现。

Method: 构建了一个可配置的多智能体供应链经济模型，LLM作为零售商智能体参与两个阶段：1) 采购阶段：在预算约束的拍卖中竞标有限库存；2) 零售阶段：设定零售价格、生成营销口号，并通过基于角色的注意力机制提供给买家购买。记录完整的交易轨迹并自动评估。

Result: 对20个开源和闭源LLM智能体的基准测试显示显著的性能差异和赢家通吃现象：只有一小部分LLM零售商能够持续实现资本增值，而许多模型尽管有相似的语义匹配分数，却只能维持在盈亏平衡点附近。

Conclusion: Market-Bench为研究LLM在竞争市场中的互动提供了一个可复现的测试平台，揭示了LLM在经济任务中的能力差异和竞争动态。
...
TL;DR: 该论文提出了"等级景观不变性"的更强概念，研究了伪布尔函数在维度1、2、3下的不变景观类别，发现非单射函数比单射函数产生更多不变类别，并分析了拓扑景观特性与算法行为的关系。


<details>
  <summary>Details</summary>
Motivation: 许多随机优化算法是等级不变的，仅依赖于解的相对排序而非绝对适应度值。作者希望引入更强的等级景观不变性概念，研究等级景观而非单个函数，以更好地理解景观难度和算法性能。

Method: 提出了等级景观不变性的新概念，要求排名、邻域结构和对称性（平移和旋转）都诱导相同的景观。对维度1、2、3的伪布尔函数（包括非单射情况）进行了不变景观类别的详尽清单分析。

Result: 发现了总共12,007个不变景观类别，相比仅考虑等级不变性显著减少。非单射函数产生的景观类别远多于单射函数。揭示了拓扑景观特性与算法行为之间的复杂关系，特别是在欺骗性、中立性和爬山策略性能方面。

Conclusion: 该清单可作为教学资源和基准设计的基础，为构建具有可控难度的更大问题提供了基础，推进了对景观难度和算法性能的理解。
...
TL;DR: Echo是一个面向迁移的记忆框架，通过将可重用知识分解为五个维度，帮助多模态LLM智能体从先前经验中提取可操作知识，在Minecraft中实现1.3-1.7倍加速


<details>
  <summary>Details</summary>
Motivation: 多模态LLM智能体在复杂游戏环境中需要持续重用过去经验来高效解决新任务，但现有记忆系统通常将记忆视为静态记录的被动存储库，缺乏明确的迁移机制

Method: 提出Echo框架，将可重用知识分解为五个维度（结构、属性、过程、功能、交互），并利用上下文类比学习（ICAL）检索相关经验并通过上下文示例将其适应到未见任务

Result: 在Minecraft实验中，从零开始学习设置下，Echo在对象解锁任务上实现了1.3-1.7倍加速，并表现出爆发式链式解锁现象，在获得可迁移经验后短时间内快速解锁多个相似物品

Conclusion: 经验迁移是提高多模态LLM智能体在复杂交互环境中效率和适应性的有前景方向，Echo框架通过明确的迁移机制实现了这一目标
...
<details>
  <summary>Details</summary>
Motivation: OBDD在表示某些CNF公式时存在局限性，特别是树宽k的CNF公式无法用FPT大小的OBDD表示，需要更强大的表示模型

Method: 引入树决策图(TDD)作为布尔函数模型，它是尊重vtree T的结构化d-DNNF的限制，研究自底向上编译CNF公式为确定性TDD的复杂性

Result: TDD具有与OBDD相同的可处理性（模型计数、枚举、条件化、应用），但更简洁；树宽k的CNF公式可用FPT大小的TDD表示；编译复杂度与Bova和Szeider引入的因子宽度概念相关

Conclusion: TDD是OBDD的有力扩展，在保持可处理性的同时提高了表达能力，特别适合表示树宽受限的CNF公式，为知识编译提供了新工具
...
<details>
  <summary>Details</summary>
Motivation: 挑战LLM作为自动评估者（LLM-as-a-Judge）的可靠性，揭示其判断受到信息来源标签的偏见影响

Method: 采用反事实设计，比较相同内容在不同标签（人类创作vsAI生成）下的信任评估；使用眼动追踪分析人类注意力模式；分析LLM内部状态（注意力分配和决策不确定性）

Result: 人类和LLM都对标注为人类创作的内容给予更高信任；眼动追踪显示人类严重依赖标签作为启发式线索；LLM在标签区域分配更密集的注意力，且人类标签条件下的标签主导性更强；AI标签下的决策不确定性更高

Conclusion: 信息来源标签是人类和LLM的显著启发式线索，这对标签敏感的LLM评估提出了有效性担忧，将模型与人类偏好对齐可能会传播人类的启发式依赖，需要去偏见的评估和对齐方法
...
TL;DR: 本文认为图灵测试不仅是一种实用简化，更是一种认识论承诺，即只关注行为输出而非内部过程，这种承诺限制了AI研究70年，阻碍了对智能本质的深入探究。


<details>
  <summary>Details</summary>
Motivation: 图灵在1950年提出的行为测试方法被广泛接受为AI智能评估的标准，但作者认为这种纯粹的行为主义方法限制了AI领域的发展，使其无法探究智能的内部机制和计算过程，类似于心理学从行为主义到认知主义的转变需求。

Method: 通过历史分析追溯图灵行为认识论如何嵌入AI评估基础设施，并与心理学行为主义到认知主义的转变进行结构类比，论证行为证据的局限性，提出后行为主义认识论的框架。

Result: 分析表明图灵的行为主义认识论使AI领域无法提出关于过程、机制和内部组织的重要问题，这些问题是认知心理学、神经科学等相关学科能够探讨的，阻碍了AI对智能本质的深入理解。

Conclusion: AI领域需要类似心理学认知革命的转变，承认行为证据不足以支持该领域希望构建的智能主张，需要发展后行为主义认识论，使AI能够探讨当前无法提出的关于内部计算过程的关键问题。
...
### [204] [QA-MoE: Towards a Continuous Reliability Spectrum with Quality-Aware Mixture of Experts for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2604.05704)
...
Method: 提出QA-MoE（质量感知专家混合）框架：1）引入连续可靠性谱统一缺失和质量退化；2）通过自监督偶然不确定性量化模态可靠性；3）该机制显式指导专家路由，抑制不可靠信号的错误传播同时保留任务相关信息。

Result: 广泛实验表明，QA-MoE在不同退化场景下实现竞争性或最先进的性能，并在实践中展现出有前景的"一个检查点适应所有"特性。

Conclusion: QA-MoE框架通过连续可靠性谱和基于不确定性的专家路由，有效解决了多模态情感分析中动态噪声和模态缺失问题，提升了模型在现实场景中的鲁棒性和适应性。
...
### [205] [Can Large Language Models Reinvent Foundational Algorithms?](https://arxiv.org/abs/2604.05716)
*Jian Zhao,Haoren Luo,Yu Wang,Yuhan Cao,Pingyue Sheng,Tianxing He*

Main category: cs.AI

TL;DR: LLMs能否重新发明计算机科学基础算法？通过"遗忘与重新发明"流程测试发现，最强模型Qwen3-4B-Thinking-2507在无提示下能重新发明50%的算法，有提示时可达90%，但复杂算法仍具挑战。
...
Result: Qwen3-4B-Thinking-2507模型在无提示下成功重新发明50%算法，一级提示下70%，二级提示下90%；高级提示能提高成功率但复杂算法仍困难；测试时强化学习使Strassen算法在二级提示下成功重新发明。

Conclusion: LLMs具备一定的基础算法重新发明能力，生成验证器在维持推理强度、避免"思维崩溃"现象中起关键作用，但复杂算法仍存在局限性，揭示了LLMs创新思维的潜力和当前限制。
...
### [206] [Emergent social transmission of model-based representations without inference](https://arxiv.org/abs/2604.05777)
...
Method: 使用强化学习模拟，比较单独学习与观察专家学习两种情境。学习者不推断他人心理状态，仅通过启发式选择动作或增强价值表征来模仿观察到的专家行为。

Result: 简单社会线索能够偏置学习者的经验，使其表征逐渐收敛到专家水平。基于模型的学习者从社会暴露中获益最大，学习速度更快且表征更接近专家。

Conclusion: 文化传播可以通过简单、非心理化的社会学习过程实现，这些过程利用了个体自身的学习机制，无需复杂的心理推断能力。
...
TL;DR: STEP-HRL是一个分层强化学习框架，通过仅依赖单步转移而非完整交互历史来实现步级学习，显著降低计算成本并提升可扩展性。
...
Method: 采用分层强化学习框架，将任务结构化分层，使用已完成子任务表示全局进度，并引入局部进度模块选择性总结交互历史，生成紧凑的局部进度摘要。

Result: 在ScienceWorld和ALFWorld基准测试中，STEP-HRL在性能和泛化能力上显著优于基线方法，同时大幅减少了token使用量。

Conclusion: STEP-HRL通过步级学习和层次化任务表示，有效解决了LLM智能体交互历史过长的问题，在保持高性能的同时显著提升了计算效率。
...
### [208] [Reciprocal Trust and Distrust in Artificial Intelligence Systems: The Hard Problem of Regulation](https://arxiv.org/abs/2604.05826)
*Martino Maggetti*

Main category: cs.AI

TL;DR: 本文主张AI系统应被视为具有某种形式能动性的实体，能够与人类建立信任关系，并探讨这对AI监管的启示
...
Method: 通过理论论证，提出AI系统应被视为具有某种能动性的实体，能够与人类建立互惠的信任关系，并分析这种关系对监管者的影响

Result: 建立了AI系统作为能动实体参与信任关系的理论框架，识别了这种互惠信任动态对AI监管提出的关键挑战和未解困境

Conclusion: AI系统的信任关系具有互惠性，这对未来的AI监管和治理提出了重要挑战，需要解决由此产生的紧张关系和困境
...
### [209] [Vision-Guided Iterative Refinement for Frontend Code Generation](https://arxiv.org/abs/2604.05839)
*Hannah Sansford,Derek H. C. Law,Wei Liu,Abhishek Tripathi,Niresh Agarwal,Gerrit J. J. van den Burg*

Main category: cs.AI

TL;DR: 论文提出了一种完全自动化的视觉批评框架，使用视觉语言模型对渲染的网页提供结构化反馈，指导代码生成的迭代优化，在网页开发任务中显著提升代码质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的代码生成通常依赖多阶段的人工干预，成本高昂，特别是在前端网页开发这种解决方案质量依赖于渲染视觉输出的领域。需要一种自动化方法来替代昂贵的人工反馈循环。

Method: 提出了一个完全自动化的批评者循环框架：1）使用视觉语言模型作为视觉批评者，对渲染的网页提供结构化反馈；2）基于反馈指导代码生成的迭代优化；3）使用LoRA进行参数高效微调，研究批评者的改进能否被代码生成LLM内化。

Result: 在WebDev Arena数据集上，该方法在三个优化周期内实现了17.8%的性能提升。通过LoRA微调，在不显著增加token数量的情况下，达到了最佳批评者循环解决方案25%的增益。

Conclusion: 基于视觉语言模型的自动化批评显著提升了前端代码生成的质量，优于单次LLM推理。迭代优化对于网页开发等复杂视觉输出任务至关重要，且批评者的改进可以通过微调部分内化到代码生成模型中。
...
### [210] [When Do We Need LLMs? A Diagnostic for Language-Driven Bandits](https://arxiv.org/abs/2604.05859)
*Uljad Berdica,Fernando Acero,Anton Ipsen,Parisa Zehtabi,Michael Cashmore,Manuela Veloso*

Main category: cs.AI

TL;DR: LLMP-UCB算法利用LLM进行不确定性估计，但实验表明基于文本嵌入的轻量级数值bandit在成本更低的情况下能达到或超过LLM方案的精度，嵌入维度可作为探索-利用平衡的实用调节杠杆。


<details>
  <summary>Details</summary>
Motivation: 在金融领域的上下文多臂老虎机问题中，虽然LLM被广泛应用，但每次决策都使用LLM进行推理计算成本高且难以获得不确定性估计，需要更高效、成本更低的解决方案。

Method: 提出LLMP-UCB算法通过重复推理从LLM获取不确定性估计；同时研究基于文本嵌入（密集或Matryoshka）的轻量级数值bandit方法；提出基于臂嵌入的几何诊断方法来决定何时使用LLM推理vs轻量级数值bandit。

Result: 实验表明轻量级数值bandit在成本仅为LLM方案一小部分的情况下，能达到或超过LLM方案的准确性；嵌入维度可作为探索-利用平衡的实用调节杠杆，实现成本-性能权衡。

Conclusion: 为金融服务的AI应用提供了原则性的部署框架，支持成本效益高、不确定性感知的决策系统，通过几何诊断指导实践者在LLM推理和轻量级数值bandit之间做出选择。
...
### [211] [Joint Knowledge Base Completion and Question Answering by Combining Large Language Models and Small Language Models](https://arxiv.org/abs/2604.05875)
*Yinan Liu,Dongying Lin,Sigang Luo,Xiaochun Yang,Bin Wang*

Main category: cs.AI

TL;DR: JCQL框架结合大语言模型和小语言模型，通过迭代方式让知识库补全和知识库问答相互增强，在两个任务上都超越了现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 知识库补全和知识库问答是密切相关且互补的任务，但现有研究通常只使用小语言模型来联合增强它们，忽视了大语言模型的强大推理能力。需要结合两者的优势，让这两个任务相互促进。
...
Conclusion: 通过结合LLM和SLM的优势，JCQL框架能够以迭代方式让知识库补全和知识库问答相互增强，在两个任务上都取得了显著提升。
...
### [212] [HybridKV: Hybrid KV Cache Compression for Efficient Multimodal Large Language Model Inference](https://arxiv.org/abs/2604.05887)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在推理过程中面临KV缓存快速增长的问题，每个视觉输入会扩展为数千个token，导致缓存随上下文长度线性增长并持续占用GPU内存，造成高昂的内存开销和延迟。

Method: HybridKV采用三阶段混合压缩框架：1) 使用文本中心注意力将注意力头分类为静态或动态类型；2) 采用自上而下的分层预算分配方案分配KV预算；3) 对静态头使用文本优先剪枝压缩，对动态头使用分块检索压缩。

Result: 在11个多模态基准测试中，使用Qwen2.5-VL-7B模型，HybridKV将KV缓存内存减少了高达7.9倍，解码速度提升了1.52倍，且性能几乎没有下降甚至优于完整缓存MLLM。
...
### [213] [Context-Value-Action Architecture for Value-Driven Large Language Model Agents](https://arxiv.org/abs/2604.05939)
*TianZe Zhang,Sirui Sun,Yuhang Xie,Xin Zhang,Zhiqiang Wu,Guojie Song*

Main category: cs.AI

TL;DR: 本文提出CVA架构，通过解耦行动生成与认知推理，使用基于真实人类数据训练的价值验证器来建模动态价值激活，有效缓解LLM模拟人类行为时的价值极化问题。
...
Result: 在包含超过110万真实世界交互轨迹的CVABench数据集上的实验表明，CVA显著优于基线方法，有效缓解了极化问题，同时提供了更好的行为保真度和可解释性。

Conclusion: CVA架构通过解耦行动生成与认知推理，并基于真实人类数据建模动态价值激活，能够有效解决LLM模拟人类行为时的价值极化问题，提高行为模拟的保真度和多样性。
...
TL;DR: 本文提出Pareto-Lenient Consensus (PLC)框架，通过共识驱动的宽松梯度修正来突破多目标偏好对齐中的局部最优困境，实现更好的帕累托前沿探索。


<details>
  <summary>Details</summary>
Motivation: 现有多目标偏好对齐方法主要依赖静态线性标量化或刚性梯度投影，这些方法强制冲突避免或同时下降，往往过早收敛到局部稳定点，牺牲了全局帕累托改进潜力。

Method: 提出Pareto-Lenient Consensus (PLC)框架，将对齐重新构想为动态协商过程，引入共识驱动的宽松梯度修正，动态容忍局部退化以换取足够的主导联盟盈余，使优化轨迹能够逃离局部次优均衡。

Result: 理论分析验证PLC能够促进僵局逃离并渐近收敛到帕累托共识均衡。大量实验表明，PLC在固定偏好对齐和全局帕累托前沿质量方面均优于基线方法。

Conclusion: PLC框架突破了多目标偏好对齐中的局部最优困境，展示了协商驱动对齐作为MPA有前景方向的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 研究指令微调的语言模型是否通过单一通用机制实现指令跟随，还是通过组合式技能部署来完成任务

Method: 在三个指令微调模型上对九个不同任务进行诊断性探测，包括通用探测与任务特定探测比较、跨任务迁移分析、因果消融实验、层间任务分层分析以及时间动态分析

Result: 1) 通用探测始终不如任务特定探测，表明表征共享有限；2) 跨任务迁移微弱且按技能相似性聚类；3) 因果消融显示稀疏非对称依赖而非共享表征；4) 任务按复杂度在层间分层，结构约束早期出现，语义任务晚期出现；5) 约束满足在生成过程中作为动态监控而非预生成规划
...
TL;DR: Claw-Eval是一个端到端的智能体评估套件，解决了现有基准的三个关键限制：轨迹不透明的评分、安全性和鲁棒性评估不足、以及模态覆盖范围窄。它包含300个人工验证任务，通过三个独立证据通道记录每个智能体动作，支持轨迹感知的评分。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准存在三个关键限制：(1) 仅检查最终输出的轨迹不透明评分，(2) 安全性和鲁棒性评估不足，(3) 模态覆盖范围和交互范式狭窄。需要更全面的评估框架来构建可靠可部署的智能体。
...
Conclusion: Claw-Eval不仅是一个基准测试工具，还突出了智能体开发的可操作方向，揭示了构建不仅有能力而且可靠可部署的智能体所需的条件。轨迹感知评估对于捕捉安全违规和鲁棒性失败至关重要。
...
### [217] [AI-Driven Modular Services for Accessible Multilingual Education in Immersive Extended Reality Settings: Integrating Speech Processing, Translation, and Sign Language Rendering](https://arxiv.org/abs/2604.05591)
*N. D. Tantaroudas,A. J. McCracken,I. Karachalios,E. Papatheou*

Main category: cs.CE

TL;DR: 该研究开发了一个集成六种AI服务的模块化平台，用于XR环境中的多语言教学，包括语音识别、翻译、语音合成、情感分类、对话摘要和国际手语渲染，并通过技术验证确认了平台在实时XR部署中的可行性。


<details>
  <summary>Details</summary>
Motivation: 为了在扩展现实（XR）环境中创建无障碍、多语言的教学解决方案，满足欧盟数字可访问性目标，需要整合多种AI服务来实现跨模态的语言教学功能。

Method: 构建模块化平台整合六种AI服务：OpenAI Whisper语音识别、Meta NLLB多语言翻译、AWS Polly语音合成、RoBERTa情感分类、flan t5 base samsum对话摘要、Google MediaPipe国际手语渲染。处理国际手语手势记录获取手部关键点坐标，映射到VR环境中的三维虚拟形象动画。进行技术基准测试，包括语音合成提供商比较和多语言翻译模型评估。

Result: 技术评估确认平台适合实时XR部署。AWS Polly在语音合成中延迟最低且价格有竞争力。EuroLLM 1.7B Instruct变体获得比NLLB更高的BLEU分数。平台验证了在XR环境中协调跨模态AI服务的可行性。

Conclusion: 该模块化平台为公平的学习解决方案奠定了基础，支持独立扩展和适应不同教育场景，实现了在XR环境中协调跨模态AI服务进行无障碍多语言教学的可行性。
...
### [218] [INTERACT: An AI-Driven Extended Reality Framework for Accesible Communication Featuring Real-Time Sign Language Interpretation and Emotion Recognition](https://arxiv.org/abs/2604.05605)
*Nikolaos D. Tantaroudas,Andrew J. McCracken,Ilias Karachalios,Evangelos Papatheou*

Main category: cs.CE

TL;DR: INTERACT是一个AI驱动的XR平台，集成了实时语音转文字、国际手语3D虚拟人渲染、多语言翻译和情感识别功能，旨在改善听障人士和多语言用户在视频会议中的无障碍体验。


<details>
  <summary>Details</summary>
Motivation: 全球有超过4.3亿人患有听力障碍，预计2050年将超过7亿人。传统无障碍解决方案成本高、可用性有限，而扩展现实技术为沉浸式和包容性通信提供了新的可能性。

Method: 基于CORTEX2框架，在Meta Quest 3头显上部署，集成Whisper进行语音识别、NLLB进行多语言翻译、RoBERTa进行情感分类、Google MediaPipe进行手势提取，构建包含实时语音转文字、国际手语3D虚拟人渲染、多语言翻译和情感识别的XR平台。

Result: 试点评估显示92%用户满意度，转录准确率超过85%，情感检测精度达90%，整体体验平均评分4.6/5.0，90%参与者愿意参与进一步测试。

Conclusion: INTERACT平台展示了在促进教育、文化和专业场景无障碍访问方面的强大潜力，为听障人士和多语言用户提供了创新的沉浸式通信解决方案。
...
### [219] [Adaptive Material Fingerprinting for the fast discovery of polyconvex feature combinations in isotropic and anisotropic hyperelasticity](https://arxiv.org/abs/2604.05698)
*Moritz Flaschel,Hagen Holthusen,Denisa Martonová,Ellen Kuhl*

Main category: cs.CE

TL;DR: 提出自适应材料指纹识别方法，通过迭代模式识别算法和自适应模型数据库，能够发现材料模型的任意线性组合，而不仅限于从预定义集合中选择单个模型。


<details>
  <summary>Details</summary>
Motivation: 原始材料指纹识别方法使用固定数量的材料模型和参数，限制了拟合灵活性。为了发现更复杂的材料模型（如多项Ogden模型或各向异性Holzapfel-Gasser-Ogden模型），需要提高方法的适应性和表达能力。

Method: 结合自适应模型数据库和迭代模式识别算法，通过应变能密度特征函数的和来设计自适应数据库。这些特征函数依赖于各向同性和各向异性不变量，满足基本物理约束，并可通过简单开关选择性地强制实施多凸性。

Result: 方法在橡胶材料和动物皮肤组织的力学测试实验数据上进行了验证，能够发现更复杂的材料模型，包括多项Ogden模型和各向异性Holzapfel-Gasser-Ogden模型。

Conclusion: 自适应材料指纹识别方法显著提高了材料模型发现的灵活性和复杂性，能够处理各向同性和各向异性材料，同时保持物理合理性和计算效率。
...
### [220] [On the Exploitability of FTRL Dynamics](https://arxiv.org/abs/2604.05129)
*Yiheng Su,Emmanouil-Vasileios Vlatakis-Gkaragkounis*

Main category: cs.GT

TL;DR: FTRL学习器在零和博弈中具有固有的可剥削性，剥削程度与学习者次优动作数量成正比，在随机博弈中可获得Ω(ηT/poly(n,m))的收益盈余


<details>
  <summary>Details</summary>
Motivation: 研究FTRL学习器在零和博弈中的可剥削性，证明这是FTRL家族的内在特性而非特定实例的产物，揭示非陡峭正则化器与陡峭正则化器在剥削机制上的几何二分法

Method: 分析FTRL学习器在n×m二人零和博弈中的表现，考虑固定优化器和交替优化器两种情况，建立剥削规模与学习者次优动作数量N的关系，在随机博弈中分析高概率收益盈余

Result: 1) 固定优化器时，剥削规模为Ω(N/η)；2) 交替优化器时，在随机博弈中可获得Ω(ηT/poly(n,m))的收益盈余；3) 发现非陡峭正则化器允许优化器通过有限时间消除次优动作获得最大盈余，而陡峭正则化器引入可忽略的修正

Conclusion: FTRL学习器在零和博弈中具有固有的可剥削性，剥削程度与学习者次优动作数量成正比，提出易感性度量来量化哪些正则化器最容易受到策略操纵，并讨论了双边收益不确定性下的影响
...
### [221] [Formal specification and behavioral simulation of the holiday gift exchange game](https://arxiv.org/abs/2604.05219)
*Daniel Quigley*

Main category: cs.GT

TL;DR: 论文分析了节日礼物交换游戏的战略结构，建立了游戏机制的形式化模型，并通过模拟发现社会成本是攻击行为的主要调节因素。


<details>
  <summary>Details</summary>
Motivation: 节日礼物交换游戏是一种常见的社会制度，具有非平凡的战略结构。作者希望通过形式化分析来理解游戏机制，并研究信息不对称、社会成本和适应性策略等因素如何影响游戏行为。

Method: 1. 建立游戏的形式化模型：定义状态空间、行动集和偷窃链的递归结构；2. 证明游戏终止性并推导计算不同游戏轨迹的算法；3. 引入装饰模型，包含部分信息、社会成本和基于离散选择理论与挫折-攻击理论的适应性策略；4. 进行240,000次全因子模拟。

Result: 1. 隐含社会成本是攻击行为的主要调节因素，减少偷窃行为27-48%，超过不确定性和战略复杂性的影响；2. 部分信息（与预期相反）通过不对称不确定性略微增加偷窃行为；3. 相关估值放大所有行为效应，即关于礼物质量的共识（而非礼物本身特征）加剧竞争；4. 先手优势在所有条件下都稳健存在。

Conclusion: 节日礼物交换游戏的社会动态主要由社会成本调节，而非战略复杂性。礼物质量的共识比礼物本身特征更能加剧竞争，而先手优势是游戏的基本特征。
...
### [222] [JD-BP: A Joint-Decision Generative Framework for Auto-Bidding and Pricing](https://arxiv.org/abs/2604.05845)
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动竞价服务在KPI约束下优化实时竞价策略，但模型预测误差和反馈延迟等不确定性会导致竞价策略偏离事后最优，造成分配效率低下。

Method: 提出JD-BP框架：1）联合输出竞价值和定价修正项；2）设计无记忆的Return-to-Go来处理历史约束违反；3）提出轨迹增强算法从基础竞价策略生成联合竞价-定价轨迹；4）使用基于能量的直接偏好优化方法和交叉注意力模块增强联合学习性能。

Result: 在AuctionNet数据集上的离线实验显示JD-BP达到最先进性能。在京东的在线A/B测试中，广告收入增加4.70%，目标成本改善6.48%。

Conclusion: JD-BP通过联合优化竞价和定价，有效解决了自动竞价中的不确定性挑战，在实际应用中表现出显著效果。
...
### [223] [Polynomial-Time Algorithm for Thiele Voting Rules with Voter Interval Preferences](https://arxiv.org/abs/2604.05953)
*Pasin Manurangsi,Krzysztof Sornat*

Main category: cs.GT

TL;DR: 提出了一个多项式时间算法，用于在Voter Interval域上计算任意给定Thiele投票规则下的最优委员会，解决了10年未决的开放问题。


<details>
  <summary>Details</summary>
Motivation: 解决一个存在10年的开放问题：在Voter Interval域上计算Thiele投票规则下的最优委员会。该问题最初针对比例批准投票提出，后来扩展到所有Thiele规则。

Method: 主要技术贡献是区间族的凹性定理。利用拉格朗日松弛将自然整数线性规划中的基数约束移到目标函数中，在Voter Interval配置下约束矩阵完全单模，可在多项式时间内求解。

Result: 成功设计出多项式时间算法，能够计算Voter Interval域上任意Thiele投票规则的最优委员会，并将结果扩展到广义Thiele规则。

Conclusion: 该研究解决了长期存在的开放问题，证明了在Voter Interval配置下，最优Thiele得分是委员会规模的凹函数，并通过人机协作获得了主要算法和证明。
...
### [224] [Non-GRS type MDS and AMDS codes from extended TGRS codes](https://arxiv.org/abs/2604.05682)
*Meiying Zhang,Shudi Yang,Yanbin Zheng*

Main category: cs.IT

TL;DR: 本文构建了一类扩展扭曲广义Reed-Solomon码，确定了其成为MDS或AMDS码的充要条件，并证明这些码不等价于广义Reed-Solomon码，还计算了其覆盖半径和深洞。


<details>
  <summary>Details</summary>
Motivation: MDS和AMDS码因其代数特性和优异的纠错能力，在通信系统、数据存储和量子编码等领域广泛应用。本文旨在构建一类扩展扭曲广义Reed-Solomon码，并研究其代数性质。

Method: 构建了一类扩展扭曲广义Reed-Solomon码，通过代数方法确定了这些码成为MDS或AMDS码的充要条件，并证明了这些码不等价于广义Reed-Solomon码。

Result: 成功构造了扩展扭曲广义Reed-Solomon码，给出了其成为MDS或AMDS码的充要条件，证明了这些码不等价于广义Reed-Solomon码，并在特定条件下计算了覆盖半径和深洞。

Conclusion: 本文构建的扩展扭曲广义Reed-Solomon码具有重要的代数性质，为MDS和AMDS码的设计提供了新方法，其不等价于传统广义Reed-Solomon码的特性增加了码的多样性。
...
### [225] [The Incidence-Multiplicity Bound for Linear Exact Repair in MDS Array Codes](https://arxiv.org/abs/2604.05692)
*Huawei Wu*

Main category: cs.IT

TL;DR: 本文研究了(n,k,ℓ) MDS阵列码的线性精确修复问题，在冗余r=n-k固定的情况下，建立了比现有界限更严格的incidence-multiplicity下界，并证明了该下界在广泛参数范围内的可达性。


<details>
  <summary>Details</summary>
Motivation: 现有投影计数方法给出的修复带宽下界在r≥3且ℓ≥2时无法达到，需要建立更精确的下界来指导MDS阵列码的精确修复设计。

Method: 通过改进投影计数方法，建立了incidence-multiplicity下界，并利用域约简和正规有理曲线构造了达到该下界的MDS阵列码。

Result: 证明了修复带宽和修复I/O的下界为ℓ(n-1)-(r-1)(q^ℓ-1)/(q-1)，该下界在ℓ≥2、r≥2、(r-1)|(q-1)且(q-1)/(r-1)≥2时，当n满足2(r-1)(q^ℓ-1)/(q-1)≤n≤q^ℓ+1时可达。

Conclusion: incidence multiplicity是超越两奇偶校验情况的MDS阵列码线性精确修复的主导几何原理，新下界比现有界限更严格且可达。
...
### [226] [Improved Capacity Upper Bounds for the Deletion Channel using a Parallelized Blahut-Arimoto Algorithm](https://arxiv.org/abs/2604.05867)
...
<details>
  <summary>Details</summary>
Motivation: 二进制删除信道的容量分析是信息论中的重要问题，传统方法在计算上存在限制，需要更高效的计算方法来获得更精确的容量上界

Method: 通过GPU并行化技术优化Blahut-Arimoto算法实现，利用GPU的并行计算能力加速算法执行，从而能够处理更大规模的计算问题

Result: 获得了二进制删除信道容量的改进上界：对于所有d≥0.64的情况，容量上界为0.3578(1-d)，这是通过优化算法实现获得的新结果

Conclusion: GPU并行化是优化信息论算法计算的有效方法，能够显著提高计算效率并获得更精确的容量上界结果，为信道容量分析提供了新的技术途径
...
### [227] [Singular Relative Entropy Coding with Bits-Back Rejection Sampling](https://arxiv.org/abs/2604.06055)
*Gergely Flamich,Spencer Hill*

Main category: cs.IT

TL;DR: 本文提出了一种名为"bits-back rejection sampler"的相对熵编码方法，结合了bits-back编码和贪婪拒绝采样技术，相比现有方法更简单实用且效率更高。


<details>
  <summary>Details</summary>
Motivation: 相对熵编码是熵编码的推广，其最小比特数下限为互信息I[X∥Y]，但通常只能达到对数因子的近似。现有方法如Sriramu和Wagner的奇异信道编码虽然实现了亚对数冗余，但过于复杂且无法实际实现。

Method: 提出bits-back rejection sampler (BBRS)，结合bits-back编码和贪婪拒绝采样技术，构建相对熵编码算法。

Result: BBRS达到了与Sriramu和Wagner采样器相同的渐近效率，但分析更简单、常数项更好，且可以使用标准相对熵编码方法实现。

Conclusion: BBRS提供了一种更简单实用的相对熵编码方法，在保持理论效率的同时显著提升了可实现性。
...
### [228] [Multilevel Coset Codes on Lattices](https://arxiv.org/abs/2604.06125)
*Leopold Bertholet,Chloe Makdad,Stephen Mackes,Daniel Chew,Matthew Robinson*

Main category: cs.IT

TL;DR: 提出了一种新型多级陪集Bombe码，将极化码推广到密集格结构，在16-QAM调制下显著优于现有BICM和MLC方案


<details>
  <summary>Details</summary>
Motivation: 现有编码方案在密集格结构上的性能有限，需要结合极化码的容量逼近特性和密集格的几何优势来提升AWGN信道下的性能

Method: 通过多级编码结合非二进制码设计格调制，利用Voronoi成形技术，将极化码推广到密集格结构，形成coset Bombe码

Result: 在AWGN信道上，coset Bombe码在16-QAM调制下比现有BICM和MLC方案获得高达0.8dB增益，块大小延迟减半，在256和1024比特码字上保持优越的BER/BLER性能

Conclusion: coset Bombe码成功结合了密集格的几何优势和极化码的容量逼近特性，为AWGN信道提供了一种高性能的多级陪集编码方案
