...
TL;DR: 提出HIVE框架，通过分层交叉注意力机制增强视觉编码器与LLM的层次特征整合，提升视觉语言对齐效果


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将视觉编码器和大型语言模型视为独立模块，限制了层次视觉特征的整合，导致视觉语言对齐不足
...
Conclusion: 分层特征整合有利于提升视觉语言模型的效率和表达能力，为更有效的多模态融合模型开辟了新途径
...
<details>
  <summary>Details</summary>
Motivation: 原始(raw)数据对于低级视觉任务更忠实，但大规模原始数据集收集困难，现有数据集有限且与特定相机硬件绑定。现有扩散模型框架设计用于生成照片完成的sRGB图像，而非物理一致的线性表示。

Method: 利用大规模sRGB扩散模型的生成先验，通过潜在空间和像素空间的专门处理合成物理有意义的线性输出。构建多对一逆ISP数据集，将使用不同ISP参数生成的同一场景的多个sRGB版本锚定到共同的场景参考目标。在数据集上微调解码器和条件去噪器。
...
TL;DR: 该论文提出了Q-Mask框架，通过因果查询驱动的掩码解码器改进OCR中的文本-区域锚定能力，解决了现有视觉语言模型在文本空间定位方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽然具备OCR能力，但在实际应用中缺乏可靠的文本锚定能力，即无法准确地将查询文本与其对应的空间区域进行关联。现有模型在细粒度文本-区域定位方面表现不佳，这限制了OCR在真实世界视觉问答等下游任务中的应用。

Method: 提出了Q-Mask框架，基于因果查询驱动的掩码解码器(CQMD)。受思维链推理启发，Q-Mask采用因果视觉解码，在生成最终OCR输出前，顺序生成查询条件化的视觉掩码。这种视觉CoT范式将"文本在哪里"与"文本是什么"解耦，强制在识别前获取基础证据，并在推理过程中显式构建文本锚点。为训练CQMD，构建了TextAnchor-26M大规模数据集，包含带有特定文本元素对应细粒度掩码的图像-文本对。

Result: 实验表明，Q-Mask显著提升了文本锚定和理解能力，在TextAnchor-Bench基准测试中表现出色，能够更好地处理多样化的视觉场景中的文本定位问题。

Conclusion: Q-Mask通过因果视觉解码和视觉思维链范式，有效解决了OCR中的文本锚定问题，为视觉语言模型提供了更可靠的文本空间定位能力，提升了在实际应用中的实用性。
...
TL;DR: SOAP方法通过PCA分析语义不变性，直接抑制MIM模型中非语义信息，提升零样本性能


<details>
  <summary>Details</summary>
Motivation: 研究发现MIM目标导致学习到的表示保留了非语义信息，这会损害推理性能

Method: 提出SOAP方法：使用PCA分析真实和合成非语义图像的语义不变性，通过线性投影头直接抑制非语义信息

Result: SOAP在各种基于MIM的模型上都能一致提升零样本性能

Conclusion: SOAP是一种无需训练的后处理抑制方法，可作为单个线性头附加到任何模型，有效提升MIM模型的语义表示质量
...
<details>
  <summary>Details</summary>
Motivation: 生物医学研究中，模型扩展常受限于训练数据不足和高昂的数据生成成本。当前研究多集中于构建更大的基础模型，而提升小模型能力的研究相对不足。本文旨在探索在单细胞分割任务中构建高效小模型的可行性。

Method: 构建参数规模为10-30M的轻量级模型UCell，关键设计是在前向计算图中引入递归结构，实现更高效的参数利用。模型可以直接从显微成像数据从头训练，无需依赖自然图像的大规模预训练。

Result: 在多个基准测试中，UCell的性能与规模大10-20倍的模型相当，对未见域外数据具有相似的泛化能力。模型能够通过少量样本进行一次性或少量样本的微调，适应多种小型数据集。

Conclusion: UCell展示了在生物医学领域构建高效小模型的可行性，通过递归结构实现参数高效性，性能媲美大模型，且无需依赖外部商业利益驱动的大规模预训练，为数据受限场景提供了实用解决方案。
...
TL;DR: QAsk-Nav是首个可复现的协作实例物体导航基准，支持对导航和协作提问的独立评估，包含轻量级提问协议、增强的导航协议和开源数据集。


<details>
  <summary>Details</summary>
Motivation: 现有协作实例物体导航基准主要关注导航成功率，缺乏对协作交互能力的一致评估方法，无法明确分离评估导航和提问能力。

Method: 提出QAsk-Nav基准，包含：(1)独立于导航评分的轻量级提问协议；(2)具有真实、多样、高质量目标描述的增强导航协议；(3)包含28,000条质量检查推理和提问轨迹的开源数据集。

Result: 基于QAsk-Nav基准开发了Light-CoNav模型，比现有模块化方法小3倍、快70倍，在未见过的物体和环境泛化方面优于最先进的协作实例导航方法。

Conclusion: QAsk-Nav为协作实例物体导航提供了首个可复现的基准，支持对导航和协作提问能力的明确分离评估，推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常基于预处理的社交线索进行操作，而现实场景中AI助手需要从原始数据中感知和推理。现有管道和多模态LLM在身份归因能力上表现不佳，导致社交互动理解不准确。

Method: 提出Omni-MMSI-R管道，通过工具生成身份归因的社交线索，并进行链式思维社交推理。构建参与者级别的参考对，并在现有数据集上添加推理标注。

Result: 实验表明，Omni-MMSI-R在Omni-MMSI任务上优于先进的LLM和其他对比方法。

Conclusion: Omni-MMSI任务对于开发能够感知和响应人类互动的AI助手至关重要，提出的Omni-MMSI-R管道通过改进身份归因能力，显著提升了社交互动理解的性能。
...
Method: 提出EASe框架，包含：1) SAUCE（语义感知上采样与通道激励），用于激励低分辨率基础模型特征通道进行选择性校准；2) 跨空间编码的图像和基础模型特征进行注意力机制，恢复全分辨率语义表示；3) CAFE（线索注意力特征聚合器），利用SAUCE注意力分数作为语义分组信号，将聚合特征分割为多粒度掩码。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）存在模态间隙问题，限制了跨模态任务（如图像描述和联合聚类）的性能。现有后处理方法主要减少全局质心偏移，但未能解决底层分布不匹配问题。

Method: 提出TPC-CMA（三阶段跨模态对齐课程学习）微调框架：1）将模态间隙分解为质心间隙和分布间隙；2）设计三阶段课程学习，通过梯度感知调度逐步引入对齐；3）联合减少质心偏移并重塑分布结构。

Result: 实验表明：当α_target=0.05时，模态间隙减少66.6%，仅带来4.84%准确率下降；当α_target=0.5时，模态间隙减少82.3%，聚类ARI从0.318提升至0.516，图像描述CIDEr分数提高57.1%。

Conclusion: 分布间隙是跨模态任务质量的真正预测指标，TPC-CMA通过显式减少质心间隙和分布间隙，显著改善了视觉语言模型的跨模态对齐能力。
...
TL;DR: SANA-I2I是一个无文本的高分辨率图像到图像生成框架，通过条件流匹配模型在潜在空间学习从源图像到目标图像的映射，专门用于医学图像处理中的运动伪影减少任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖文本提示进行图像生成控制，但在医学成像等专业领域，文本描述可能不够精确或难以获取。本文旨在开发一个完全基于图像对的无文本框架，专门解决医学图像处理中的实际问题，如胎儿MRI运动伪影减少。

Method: 提出SANA-I2I框架，完全移除文本条件，仅使用配对源-目标图像学习条件流匹配模型。在潜在空间中学习条件速度场，将目标图像分布映射到另一个分布。针对胎儿MRI运动伪影减少任务，采用Duffy等人的合成数据生成策略创建训练对，因为真实配对数据难以获取。

Result: 实验结果表明，SANA-I2I能有效抑制运动伪影同时保持解剖结构，在少量推理步骤下达到竞争性性能。这证明了基于流的无文本生成模型在医学图像监督图像到图像任务中的效率和适用性。

Conclusion: SANA-I2I展示了无文本条件流匹配模型在医学图像处理中的有效性，特别是在运动伪影减少等监督图像到图像任务中。该方法不依赖语言提示，为专业领域图像处理提供了更精确的解决方案。
...
TL;DR: 使用冻结的DINOv3 ViT-B嵌入，通过基于最近邻的自训练方法，仅需少量标注数据即可实现水下物种分类，在AQUA20基准测试中接近全监督模型的性能。


<details>
  <summary>Details</summary>
Motivation: 水下图像物种分类面临专家标注成本高、模型跨数据集泛化能力差的问题，需要探索标注效率更高的方法。

Method: 使用未微调的DINOv3 ViT-B模型提取图像嵌入特征，通过基于最近邻的自训练方法，利用少量标注种子在大量未标注数据中传播标签信息。

Result: 仅需不到5%的训练标签，自训练方法就能接近全监督ConvNeXt基线的性能；在全监督条件下，差距缩小到几个百分点，部分物种甚至超过监督基线。

Conclusion: 冻结的基础模型嵌入已包含足够的判别性结构，结合自训练方法可建立无需训练、无需领域特定工程、立即可用的标签高效海洋物种识别基准。
...
TL;DR: 本文提出了一种基于动态图模型的自适应节点选择方法，用于RGB-D室内场景识别，通过自适应选择关键局部特征并动态更新图模型来提升识别性能。


<details>
  <summary>Details</summary>
Motivation: RGB-D数据在室内场景识别中很重要，深度图能描述场景的3D结构和物体间的几何关系。先前研究表明两种模态的局部特征对提升识别精度都很关键，但如何自适应选择和有效利用这些关键局部特征仍是开放问题。

Method: 提出动态图模型，包含自适应节点选择机制：1）构建动态图建模物体与场景的关系；2）自适应节点选择方法从RGB和深度模态中提取关键局部特征；3）节点按三个不同层次分组，表示物体的远近关系；4）根据注意力权重动态更新图模型；5）融合更新优化的RGB和深度特征进行室内场景识别。

Result: 在SUN RGB-D和NYU Depth v2公开数据集上进行实验，广泛结果表明该方法相比最先进方法具有优越性能，证明能够有效利用RGB和深度模态中的关键局部特征。

Conclusion: 提出的动态图模型通过自适应节点选择和动态更新机制，能够有效利用RGB-D数据中的关键局部特征，在室内场景识别任务上取得了优于现有方法的性能。
...
TL;DR: mmAnomaly：结合毫米波雷达与RGBD视觉的多模态异常检测框架，通过条件潜在扩散模型生成预期毫米波频谱，实现隐蔽武器检测、穿墙入侵检测等应用，准确率达94% F1分数。
...
Method: 提出多模态异常检测框架mmAnomaly：1）使用快速ResNet分类器提取视觉语义线索（场景几何和材料属性）；2）采用条件潜在扩散模型合成给定视觉上下文下的预期毫米波频谱；3）通过双输入比较模块识别真实与生成频谱之间的空间偏差来定位异常。

Result: 在三个应用场景（隐蔽武器定位、穿墙入侵者定位、穿墙跌倒定位）的两个多模态数据集上评估，系统达到94% F1分数和亚米级定位误差，在衣物、遮挡和杂乱环境中表现出鲁棒泛化能力。

Conclusion: mmAnomaly建立了准确且可解释的上下文感知异常检测框架，通过结合毫米波雷达与视觉上下文，有效解决了毫米波传感中的复杂信号失真问题，为实际应用提供了可靠解决方案。
...
TL;DR: 论文提出TEP方法，通过跟踪增强提示改进SAM3在复杂视频目标分割中对微小和语义主导目标的识别能力，在PVUW挑战赛中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的SAM3方法在常规目标上表现优异，但在微小目标和语义主导目标上表现不佳，主要原因是SAM3对这些特定类型目标的理解不足。
...
TL;DR: 提出VLM-in-the-Loop插件式质量保证模块，通过工具锚定机制提升心电图数字化质量，在真实临床图像上达到98%优秀质量


<details>
  <summary>Details</summary>
Motivation: 心电图数字化可解锁数十亿存档临床记录，但现有方法在真实世界图像上表现不佳，尽管基准测试数据良好

Method: VLM-in-the-Loop插件模块，通过工具锚定机制将VLM评估与领域特定信号分析工具的定量证据相结合，无需修改底层数字化器

Result: 工具锚定将判决一致性从71%提升至89%，保真度分离加倍；在四个后端上均改善性能；在428个真实临床HCM图像上达到98%优秀质量

Conclusion: 插件架构和工具锚定机制具有领域参数化特性，在质量标准可客观测量的领域具有广泛应用前景
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在脑转移瘤自动分割方面潜力显著，但单一机构训练的模型由于扫描硬件、成像协议和患者人口统计学差异，在不同机构表现不佳。需要创建跨机构适用的域适应框架。

Method: 提出VAE-MMD预处理管道，结合变分自编码器(VAE)和最大均值差异(MMD)损失，包含跳跃连接和自注意力机制，与nnU-Net分割结合。在来自四个公共数据库的740名患者数据上测试。

Result: VAE-MMD将域分类器准确率从0.91降至0.50，表明成功实现跨机构特征对齐。重建体积PSNR大于36dB，保持解剖准确性。相比基线nnU-Net，平均F1提升11.1%，平均sDice提升7.93%，平均HD95降低65.5%。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数公开数据集（如Waymo、Argoverse）基于西方道路环境，无法反映韩国等地区的独特交通模式、基础设施和驾驶行为，导致在跨地理区域部署时性能下降。

Method: 使用韩国自动驾驶数据集，比较四种训练策略：零样本迁移、从头训练、完全微调、编码器冻结，研究QCNet模型从美国数据到韩国环境的迁移适应性。

Result: 利用预训练知识显著提升预测性能，选择性微调解码器同时冻结编码器的策略在准确性和训练效率方面达到最佳平衡，相比从头训练减少超过66%的预测误差。

Conclusion: 该研究为在新地理领域部署轨迹预测模型提供了有效的迁移学习策略实践见解，证明预训练知识迁移对跨区域模型适应性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决运动中心语言表达下的视频对象分割问题，需要模型同时理解外观、时间行为和对象交互，传统方法需要大量标注数据进行训练

Method: 三阶段无训练流程：1) Gemini-3.1 Pro分解目标事件并生成描述；2) SAM3-agent生成种子掩码并传播；3) Qwen3.5-Plus进行精炼和行为验证

Result: 在PVUW 2026 MeViS-Text测试集上排名第一，Final分数0.909064，J&F分数0.7897，无需任务特定微调

Conclusion: 通过结合多模态大语言模型和SAM3的无训练方法，在运动中心语言表达的视频对象分割任务中取得了优异表现
...
TL;DR: FADE是一种针对基于查询传播的多目标跟踪方法的对抗攻击框架，通过时间查询泛滥和时序记忆破坏两种策略，利用TBP方法的架构漏洞进行攻击。


<details>
  <summary>Details</summary>
Motivation: 基于查询传播的跟踪方法虽然通过端到端管道和长时序建模提升了多目标跟踪性能，但其依赖查询传播的架构存在未探索的对抗攻击漏洞。本文旨在揭示并利用这些特定漏洞。

Method: FADE采用两种攻击策略：1）时间查询泛滥：生成时间一致的虚假跟踪查询，耗尽跟踪器的有限查询预算，迫使有效轨迹终止；2）时序记忆破坏：直接攻击查询更新器的记忆，通过状态去相关切断时序链接，并擦除匹配轨迹的学习特征身份。此外，通过可微分管道优化攻击，利用高级感知传感器欺骗模拟实现物理世界可行性。

Result: 在MOT17和MOT20基准测试上的实验表明，FADE对最先进的TBP跟踪器非常有效，导致显著的身份切换和轨迹终止。

Conclusion: FADE框架成功揭示了基于查询传播的多目标跟踪方法的架构漏洞，通过针对性的对抗攻击策略，显著破坏了跟踪器的性能，为未来鲁棒跟踪系统的设计提供了重要参考。
...
TL;DR: 本文提出了一种名为First Logit Boosting（FLB）的无训练技术，通过存储第一个生成token的logit并添加到后续token预测中，有效缓解大型视觉语言模型中视觉信息的长时衰减问题，显著减少物体幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多种多模态任务中表现出色，但物体幻觉（生成不存在物体的答案）仍然是一个持续挑战。现有的缓解方法如重新训练和外部接地方法存在数据成本高或结构复杂的问题，而无训练方法如对比解码虽然成本效益高，但存在长时衰减问题——随着生成过程进行，视觉接地减弱而语言先验主导。

Method: 提出First Logit Boosting（FLB）技术：1）存储第一个生成token的logit；2）将该logit添加到后续token的预测中。这种方法能够：1）在整个生成过程中维持嵌入在第一个token中的视觉信息；2）通过"The" token的稳定化效应抑制幻觉词汇。

Result: 实验结果表明，FLB在各种任务、基准测试和骨干模型上显著减少了物体幻觉。该方法仅带来可忽略的推理开销，使其非常适合实时多模态系统。

Conclusion: FLB是一种简单而有效的无训练技术，通过缓解长时衰减问题来减少大型视觉语言模型中的物体幻觉，具有实际应用价值。
...
TL;DR: 本文研究了7T MRI在多发性硬化白质病变分割中的应用，发现传统基于低场强数据开发的自动分割工具在7T图像上表现不佳，而基于7T数据训练的transformer模型能更好地检测小病灶。


<details>
  <summary>Details</summary>
Motivation: 7T MRI能更好地显示多发性硬化白质病变，但其对比度和伪影与1.5-3T成像差异显著，导致广泛使用的自动分割工具可能无法直接应用。需要开发专门针对7T MRI的病变分割方法。

Method: 使用7T FLAIR扫描生成参考白质病变掩模，基于LST工具输出并经专家手动修订。比较了LST-LPA和LST-AI等传统方法，并在7T FLAIR数据上训练了3D UNETR和SegFormer transformer模型，评估了不同分辨率下的性能。

Result: 在0.5x0.5x0.5³原生分辨率下，7T训练的transformer模型与LST-AI竞争性相当，同时能检测到传统方法遗漏的小病灶。最佳模型SegFormer在体素级Dice为0.61，病变级Dice为0.20，优于传统LST-LPA工具。下采样训练会降低性能，凸显原生7T分辨率对小病灶检测的重要性。

Conclusion: 7T训练的transformer模型在多发性硬化白质病变分割中优于传统方法，特别是在小病灶检测方面。原生7T分辨率对保持分割性能至关重要。作者开源了7T训练模型，为超高场MS研究提供可复现的自动病变量化工具。
...
TL;DR: 本文分析了强化学习在视觉语言模型推理中的局限性，提出多组策略优化方法来解决多样性崩溃问题


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习（特别是GRPO）能够增强视觉语言模型的推理能力，但其有效性的内在机制和局限性尚未得到充分探索。研究发现RL模型存在推理深度但狭窄的问题，而基础模型虽然推理不够精细但思维模式更广泛多样。

Method: 提出多组策略优化（MUPO），这是一种简单而有效的方法，旨在激励跨多个解决方案的发散性思维，避免模型过早收敛到有限的推理策略子集。

Result: 在已建立的基准测试中证明了MUPO的有效性，能够解决GRPO容易出现的多样性崩溃问题，避免模型陷入局部最优和可扩展性差的问题。

Conclusion: 通过分析RL模型与基础模型的行为差异，揭示了GRPO容易导致多样性崩溃的局限性，提出的MUPO方法能够有效激励发散性思维，提高模型的推理多样性和可扩展性。
...
TL;DR: CheXOne是一个用于胸部X光片解读的推理增强视觉语言模型，能同时生成诊断预测和明确的临床推理轨迹，在多项任务中表现优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 胸部X光检查量增加导致放射科医生工作量增大和诊断错误风险升高，现有AI系统通常只生成最终预测而缺乏解释视觉证据如何转化为放射学发现和诊断预测的明确推理过程。

Method: 使用从30个公共数据集中整理的1470万个指令和推理样本进行训练，采用结合指令微调和强化学习的两阶段框架来提升推理质量，涵盖36个胸部X光解读任务。

Result: 在17个评估设置中零样本表现优于现有医学和通用领域基础模型；临床阅读研究表明CheXOne生成的报告在55%情况下优于住院医师报告；生成的推理轨迹具有高临床事实性并为最终预测提供因果支持。

Conclusion: 明确的推理能够提升AI辅助胸部X光解读中的模型性能、可解释性和临床实用性，为性能提升提供了合理解释。
...
TL;DR: ARGS框架通过自回归高斯溅射实现多尺度3D对象生成，使用高斯简化策略和树状结构实现O(log n)复杂度的高效生成。


<details>
  <summary>Details</summary>
Motivation: 自回归框架在2D图像生成中表现出色，但在3D对象生成领域尚未得到充分探索。现有方法在生成多尺度3D表示时面临效率和一致性的挑战。

Method: 提出自回归高斯溅射(ARGS)框架，采用高斯简化策略和反向简化引导多尺度生成。利用层次树结构实现O(log n)复杂度，并设计基于树的Transformer自回归预测树结构，使叶节点能关注其祖先节点以增强结构一致性。

Result: 实验表明，ARGS能够有效生成具有可控细节层次、视觉保真度的多尺度高斯表示，同时保持可管理的时间消耗预算。

Conclusion: ARGS框架成功将自回归范式扩展到3D对象生成，实现了高效、可控的多尺度3D内容生成，为3D生成领域提供了新的解决方案。
...
TL;DR: PC-SAM是一个统一框架，结合了全自动道路分割和交互式分割，通过精心设计的微调策略，使点提示的影响仅限于对应补丁，解决了SAM在遥感道路分割中性能不佳的问题，支持细粒度局部修正。


<details>
  <summary>Details</summary>
Motivation: 当前全自动道路分割方法在识别具有挑战性的道路段时仍存在不足，容易产生假阳性和假阴性区域，且不支持感兴趣区域的局部分割或现有掩码的细化。虽然SAM模型在自然图像的交互式分割中表现良好，但在遥感道路分割中性能较差，无法支持细粒度局部细化。

Method: 提出PC-SAM框架，将全自动道路分割和交互式分割集成到统一框架中。通过精心设计微调策略，约束点提示的影响仅限于对应的补丁区域，克服了原始SAM无法进行精细局部修正的局限性，实现了细粒度交互式掩码细化。

Result: 在多个代表性遥感道路分割数据集上的广泛实验表明，结合点提示时，PC-SAM在道路掩码分割方面显著优于最先进的全自动模型，同时提供灵活的局部掩码细化和局部道路分割功能。

Conclusion: PC-SAM成功解决了遥感道路分割中全自动方法的局限性，通过集成全自动和交互式分割，实现了更准确的道路分割和灵活的局部细化能力，为下游任务提供了更好的支持。
...
TL;DR: PET-DINO是一个支持文本和视觉提示的通用检测器，通过Alignment-Friendly Visual Prompt Generation模块和两种提示增强训练策略，解决了开放集目标检测中文本表示与复杂视觉概念对齐困难的问题。


<details>
  <summary>Details</summary>
Motivation: 开放集目标检测(OSOD)虽然能够识别超出固定类别的新类别，但面临文本表示与复杂视觉概念对齐困难、稀有类别图像-文本对稀缺的问题，导致在专业领域或复杂物体上性能不佳。现有视觉提示方法通常涉及复杂的多模态设计和多阶段优化，开发周期长，且数据驱动的OSOD模型的有效训练策略尚未充分探索。

Method: 提出PET-DINO通用检测器，包含：1) Alignment-Friendly Visual Prompt Generation (AFVPG)模块，基于先进的文本提示检测器构建，解决文本表示指导的局限性并缩短开发周期；2) 两种提示增强训练策略：迭代级别的Intra-Batch Parallel Prompting (IBP)和整体训练级别的Dynamic Memory-Driven Prompting (DMD)，支持同时建模多个提示路径，促进与多样化现实使用场景的并行对齐。

Result: 综合实验表明，PET-DINO在各种基于提示的检测协议中展现出竞争力的零样本目标检测能力。这些优势归因于继承式哲学和提示增强训练策略，这些策略在构建有效的通用目标检测器中起着关键作用。

Conclusion: PET-DINO通过AFVPG模块和两种提示增强训练策略，有效解决了开放集目标检测中的关键挑战，缩短了开发周期，提升了在复杂场景下的检测性能，为构建通用目标检测器提供了有效方案。
...
Conclusion: RegFormer通过空间接地信号引导的弱监督学习，解决了传统方法计算成本高和假阳性问题，实现了高效准确的实例级HOI检测，代码已开源。
...
TL;DR: MAESIL是一种用于3D医学图像的自监督学习框架，通过"superpatch"3D块和双掩码策略有效捕捉3D结构信息，在CT数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 3D医学影像（如CT）训练面临标注数据稀缺问题。现有方法通常将3D扫描处理为独立的2D切片，这丢弃了关键的轴向连贯性和3D结构上下文信息，限制了性能。

Method: 提出MAESIL框架，核心创新是"superpatch"3D块输入单元，平衡3D上下文保留与计算效率。采用3D掩码自编码器策略和双掩码策略来学习全面的空间表示。

Result: 在三个不同的大规模公共CT数据集上验证，MAESIL在PSNR和SSIM等关键重建指标上显著优于AE、VAE和VQ-VAE等现有方法。

Conclusion: MAESIL为3D医学成像任务提供了一个强大且实用的预训练解决方案，能够有效捕捉3D结构信息。
...
TL;DR: 提出基于可学习性的数据集蒸馏方法，通过增量式构建合成数据集，使用可学习性评分指导生成新样本，减少冗余并提升性能


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法存在冗余问题，不同样本传递重叠信息而非互补知识，导致训练信号重复

Method: 提出可学习性引导的数据集蒸馏（LGD），从少量样本开始，通过可学习性评分识别当前模型能从哪些样本学习，生成与课程对齐的新样本，平衡当前模型的训练效用和参考模型下的有效性

Result: 在ImageNet-1K上达到60.1%，ImageNette上87.2%，ImageWoof上72.9%的SOTA性能，冗余减少39.1%

Conclusion: 基于可学习性的增量式数据集蒸馏方法能有效减少冗余，促进训练阶段间的专业化，实现更好的性能
...
TL;DR: AceTone提出首个支持多模态条件色彩分级统一框架，通过生成式色彩变换直接生成3D-LUT，在文本引导和参考图像引导任务上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 色彩影响图像风格和情感表达，现有色彩分级方法依赖块状重着色或固定滤镜库，难以泛化到不同创意意图或符合人类审美偏好

Method: 将色彩分级构建为生成式色彩变换任务，模型直接生成条件于文本提示或参考图像的3D-LUT；开发VQ-VAE分词器压缩LUT向量为离散token，构建大规模数据集AceTone-800K，训练视觉语言模型预测LUT token，并通过强化学习对齐感知保真度和美学

Result: 在文本引导和参考图像引导的色彩分级任务上达到最先进性能，LPIPS指标提升高达50%；人类评估确认结果视觉愉悦且风格一致

Conclusion: AceTone展示了语言驱动、美学对齐的色彩分级新途径，为多模态条件色彩变换提供了统一框架
...
TL;DR: FreqPhys是一个基于频率引导的远程光电容积描记（rPPG）框架，通过显式利用生理频率先验来增强对运动伪影和光照波动的鲁棒性，实现高质量的非接触式生理信号恢复。
...
Method: FreqPhys包含四个核心模块：1）生理带通滤波模块抑制带外干扰；2）生理频谱调制与自适应频谱选择强调脉搏相关频率成分；3）跨域表示学习融合频谱先验与深度时域特征；4）频率感知条件扩散过程渐进重建高质量rPPG信号。

Result: 在六个基准测试上的广泛实验表明，FreqPhys相比最先进方法有显著改进，特别是在具有挑战性的运动条件下表现突出。

Conclusion: 该研究强调了显式建模生理频率先验的重要性，为鲁棒的非接触式生理监测提供了有效的解决方案。源代码将公开发布。
...
TL;DR: TRiGS是一种新的4D表示方法，通过统一的连续几何变换解决现有4D高斯泼溅方法中的时间碎片化和内存增长问题，能够在长视频序列中保持高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有4D高斯泼溅方法依赖分段线性速度近似和短时间窗口，导致严重的时间碎片化，迫使基元反复消除和再生以跟踪复杂非线性动态。这种临时近似消除了物体的长期时间一致性，并导致高斯基元不可避免的增殖，阻碍了扩展到长视频序列的能力。

Method: TRiGS采用统一的连续几何变换，整合了SE(3)变换、分层贝塞尔残差和可学习的局部锚点，为单个基元建模几何一致的刚性运动。这种连续表示保留了时间一致性并有效缓解了无界内存增长。

Result: 在标准基准测试中，TRiGS实现了高保真渲染，同时独特地扩展到长视频序列（如600到1200帧），没有严重的内存瓶颈，在时间稳定性方面显著优于先前工作。

Conclusion: TRiGS通过连续几何变换解决了4D高斯泼溅中的时间碎片化和内存增长问题，为动态场景重建提供了可扩展的解决方案，特别适用于长视频序列。
...
<details>
  <summary>Details</summary>
Motivation: 当前前馈重建模型过度依赖多视角几何标注（如3D点云图和相机位姿），使得全监督训练方案难以扩展，需要大量标注数据和计算资源。

Method: 设计模糊感知的相对深度损失和基于三角测量的重投影损失，利用预训练模型的零样本预测提供的单目相对深度和图像稀疏对应关系，实现多视角几何一致性监督。

Result: 使用更少数据从头训练，Reliev3R能够达到全监督兄弟模型的性能水平，实现了低成本3D重建监督和可扩展的前馈重建模型。

Conclusion: Reliev3R为前馈重建模型提供了一种弱监督训练范式，减少了对几何传感器数据和计算密集型预处理的需求，推动了低成本3D重建监督的发展。
...
TL;DR: TF-SSD：一种基于SAM和DINO协同的无训练共显著目标检测方法，通过生成候选掩码池、质量筛选、单图像显著性和跨图像原型选择来实现优异的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的共显著目标检测方法受限于封闭数据集且泛化能力有限，而视觉基础模型展现出强大的泛化能力和显著性理解潜力，但尚未被充分探索用于CoSOD任务。

Method: 提出无训练方法TF-SSD：1) 使用SAM生成候选掩码池；2) 基于SAM的质量掩码生成器过滤冗余掩码；3) 利用DINO注意力图进行单图像显著性过滤；4) 通过跨图像原型相似度计算选择最终掩码。

Result: 实验表明TF-SSD显著优于现有方法，在无训练方法上获得13.7%的性能提升，展现出强大的泛化能力和共显著目标检测效果。

Conclusion: TF-SSD成功利用视觉基础模型（SAM和DINO）的协同作用，实现了无需训练的高性能共显著目标检测，为CoSOD任务提供了新的无训练解决方案。
...
TL;DR: STAR是一个基于拓扑锚点的两阶段空间导航框架，通过监督微调和空间感知段级直接偏好优化，在复杂拓扑结构中实现更准确的空间推理，其32B版本在RedMaze-23K数据集上超越了DeepSeek-V3并达到GPT-4性能的82.4%。


<details>
  <summary>Details</summary>
Motivation: 结构化空间导航是大语言模型空间推理的核心基准，现有方法如VoT在复杂拓扑中容易产生级联错误，需要更鲁棒的解决方案。

Method: 提出STAR两阶段框架：第一阶段通过监督微调帮助模型内化空间语义并剪枝冗余路径；第二阶段采用空间感知段级直接偏好优化来精炼长视野导航中的自我修正能力。

Result: STAR在开源模型中达到最先进性能：32B版本在RedMaze-23K数据集上超越DeepSeek-V3（29.27% vs. 25.00%），达到GPT-4性能的82.4%。

Conclusion: STAR框架通过拓扑锚点和两阶段优化策略，有效解决了复杂拓扑中的空间导航问题，为LLM空间推理提供了更鲁棒的解决方案。
...
TL;DR: 本文提出FecalFed，一种用于禽类疾病分类的隐私保护联邦学习框架，解决了农场数据隐私和数据污染问题，在非IID数据分布下实现了接近集中式训练的性能。


<details>
  <summary>Details</summary>
Motivation: 高致病性禽流感和地方性禽类疾病的早期检测对全球粮食安全至关重要。虽然计算机视觉模型在粪便图像疾病分类方面表现出色，但大规模部署受到农场数据隐私问题和机构数据孤岛的制约。此外，现有开源农业数据集普遍存在严重且未记录的数据污染问题。

Method: 1. 首先整理并发布poultry-fecal-fl数据集，包含8,770张独特图像，涵盖四种疾病类别，消除了流行公共存储库中46.89%的重复数据；2. 提出FecalFed隐私保护联邦学习框架；3. 在高度异构、非IID条件下（Dirichlet α=0.5）评估框架；4. 使用服务器端自适应优化（FedAdam）配合Swin-Small架构；5. 同时评估边缘优化的Swin-Tiny模型。

Result: 1. 在高度异构数据分布下，孤立单农场训练仅获得64.86%准确率；2. 联邦学习方法在不集中敏感数据的情况下恢复性能：FedAdam+Swin-Small达到90.31%准确率，接近集中式上限95.10%；3. 边缘优化的Swin-Tiny模型保持89.74%的竞争性性能。

Conclusion: FecalFed框架为农场禽类疾病监测提供了一个高效、隐私优先的蓝图，在保护数据隐私的同时，在高度异构的农业环境中实现了接近集中式训练的性能，解决了农业AI部署中的关键瓶颈问题。
...
TL;DR: HarassGuard：基于视觉语言模型的社会VR骚扰检测系统，仅使用视觉输入实现主动骚扰检测，保护用户隐私


<details>
  <summary>Details</summary>
Motivation: 社会VR平台存在严重的在线骚扰风险，现有安全措施多为被动响应，而主动检测方案通常依赖敏感生物识别数据，引发隐私担忧

Method: 构建IRB批准的骚扰视觉数据集，应用提示工程，微调视觉语言模型（VLM），通过考虑社会VR中的上下文信息来检测骚扰行为

Result: HarassGuard在二分类任务中达到88.09%准确率，多分类任务中达到68.85%准确率，性能与最先进基线（LSTM/CNN、Transformer）相当，但所需微调样本显著减少（200 vs 1,115）

Conclusion: HarassGuard提供了一种有效的隐私保护骚扰检测方案，在保持竞争性能的同时减少数据需求，具有上下文推理和隐私保护优势
...
TL;DR: 本文提出一种利用3D基础模型几何先验的无增强方法，通过深度估计获取像素级3D位置作为位置嵌入，并通过交叉注意力融合中间几何特征，以提升自动驾驶轨迹规划在相机视角变化下的鲁棒性。
...
Method: 采用无数据增强的方法，利用3D基础模型提供的几何先验。具体包括：1) 从深度估计中提取像素级3D位置作为位置嵌入；2) 通过交叉注意力机制融合中间几何特征；3) 在VR-Drive相机视角扰动基准上进行评估。

Result: 在VR-Drive基准测试中，该方法在大多数扰动条件下减少了性能下降，特别是在俯仰角和高度扰动方面有明显改进。但在纵向平移扰动下的增益较小，表明需要更视角无关的集成方法来实现对相机视角变化的完全鲁棒性。

Conclusion: 利用3D基础模型的几何先验可以有效提升轨迹规划对相机视角变化的鲁棒性，特别是在俯仰角和高度变化方面。然而，对于纵向平移等扰动，需要开发更视角无关的特征集成方法以实现全面的鲁棒性。
...
TL;DR: 该论文提出KG-CMI框架，通过知识图谱增强的跨模态交互解决医学视觉问答中知识利用不足和答案形式受限的问题，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉问答方法未能充分利用领域专业知识，难以准确关联医学图像中的病灶特征与诊断标准；同时，基于分类的方法依赖预定义答案集，限制了处理自由形式答案的多样性和语义细节。

Method: 提出知识图谱增强的跨Mamba交互框架(KG-CMI)，包含四个模块：细粒度跨模态特征对齐(FCFA)、知识图谱嵌入(KGE)、跨模态交互表示(CMIR)和自由形式答案增强的多任务学习(FAMT)。该框架通过图结构有效整合专业医学知识，建立病灶特征与疾病知识的关联。

Result: KG-CMI在三个医学视觉问答数据集(VQA-RAD、SLAKE、OVQA)上超越了现有最先进方法，并通过可解释性实验进一步验证了框架的有效性。

Conclusion: KG-CMI框架通过有效整合医学知识图谱和引入自由形式答案增强学习，显著提升了医学视觉问答的性能，能够更好地处理开放性问题并建立准确的病灶-知识关联。
...
TL;DR: TALENT框架通过目标感知高效调优解决指代图像分割中的非目标激活问题，使用修正成本聚合器和目标感知学习机制提升分割精度


<details>
  <summary>Details</summary>
Motivation: 现有参数高效调优方法在指代图像分割中存在视觉特征无法突出文本指代目标实例，反而激活同类别无关对象的非目标激活问题

Method: 提出TALENT框架：1)修正成本聚合器高效聚合文本指代特征；2)目标感知学习机制包括上下文成对一致性学习和目标中心对比学习，前者通过句子级文本特征构建文本指代亲和图，后者增强目标定位并抑制无关关联

Result: 在G-Ref验证集上获得2.5% mIoU提升，在各种指标上优于现有方法

Conclusion: TALENT框架通过目标感知高效调优有效解决非目标激活问题，显著提升指代图像分割性能
...
TL;DR: 本文提出DirectFisheye-GS方法，将鱼眼相机模型直接集成到3D高斯泼溅框架中，避免预处理导致的图像失真和信息损失，并通过特征重叠驱动的跨视图联合优化策略解决边缘浮动物体问题。


<details>
  <summary>Details</summary>
Motivation: 传统3DGS处理鱼眼图像时需要先进行去失真预处理，这会带来两个问题：1) 图像边缘出现黑边导致信息损失，失去了鱼眼相机大视场的优势；2) 去失真过程中的拉伸和插值重采样会稀释细节密度，导致3DGS过拟合低频区域，产生模糊和浮动物体。
...
Conclusion: 通过将鱼眼相机模型直接集成到3DGS框架并引入跨视图联合优化策略，实现了高质量的原生鱼眼图像3D场景重建，该方法同样适用于现有的针孔相机基础流程。
...
TL;DR: 研究揭示AI在皮肤镜图像诊断中的系统误分类现象，这些困难图像同样导致人类专家诊断性能崩溃，表明问题源于图像质量而非算法偏差


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI算法与人类专家的性能对比，本研究从新视角探讨皮肤镜图像的内在复杂性，旨在确定AI误分类的根本原因

Method: 使用多种CNN架构进行实验，识别出所有模型都系统误分类的图像子集；让专家皮肤科医生独立评估这些困难案例和对照组；分析诊断一致性和专家共识

Result: AI系统误分类的图像同样导致人类专家诊断性能崩溃：与真实标签的一致性显著下降（Cohen's kappa从0.61降至0.08）；专家共识严重恶化（Fleiss kappa从0.456降至0.275）；图像质量是双重系统失败的主要驱动因素

Conclusion: AI在皮肤镜诊断中的失败不仅反映了算法局限性，更揭示了图像固有的视觉模糊性；研究强调了图像质量在诊断中的关键作用，并为透明研究提供了公开数据集和模型
...
TL;DR: CL-VISTA是一个为视频大语言模型设计的持续学习基准测试，包含8个多样化任务，通过6种评估协议在3个维度上系统评估持续学习方法，揭示了性能、计算效率和内存占用之间的基本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习基准测试存在不足：许多仍依赖未经大规模预训练的模型，且通常将单个数据集划分为子任务，导致任务冗余度高，对预训练视频大语言模型的遗忘效应不明显。需要专门针对视频大语言模型的持续学习基准来有效评估其在实际非平稳数据环境中的适应能力。

Method: 提出CL-VISTA基准测试，包含8个多样化任务，涵盖感知、理解和推理三个层面，以产生显著的分布偏移。建立包含6种评估协议的综合评估框架，覆盖性能、计算效率和内存占用三个关键维度。性能维度特别包含通用视频理解评估，以区分真正的智能提升与任务特定过拟合。

Result: 对10种主流持续学习方法进行广泛基准测试，发现基本权衡：没有单一方法在所有维度上均表现优异。成功缓解灾难性遗忘的方法往往在泛化能力、计算开销或内存占用方面做出妥协。揭示了持续学习方法在视频大语言模型中的局限性。

Conclusion: CL-VISTA为多模态基础模型的持续学习研究提供了重要基准和评估框架，揭示了当前方法在性能、效率和内存之间的基本权衡，为未来研究提供了关键见解。
...
TL;DR: MoonAnything是一个基于真实月球地形构建的统一基准数据集，提供首个包含几何和光度监督的大规模月球感知数据集，包含LunarGeo（几何数据）和LunarPhoto（光度数据）两个子数据集，共13万+样本。


<details>
  <summary>Details</summary>
Motivation: 现有月球数据集通常缺乏几何真值、光度真实性、光照多样性或大规模覆盖，这阻碍了基于学习的月球感知系统的开发。需要提供全面的几何和光度监督数据集来支持月球探索任务。

Method: 基于真实月球地形，采用基于物理的渲染技术构建统一基准。包含两个互补子数据集：LunarGeo提供立体图像、密集深度图和相机标定；LunarPhoto使用空间变化的BRDF模型生成真实感图像，并提供多光照渲染。
...
TL;DR: TP-Seg是一个用于统一医学病灶分割的任务原型框架，通过任务条件适配器和原型引导任务解码器解决现有方法中的特征纠缠和梯度干扰问题，在多种医学成像模态和病灶类型上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有统一分割方法通常依赖异构任务和模态间的共享编码器，这会导致特征纠缠、梯度干扰和次优的病灶区分。需要一种能有效平衡共享和任务特定表示的统一模型。
...
Conclusion: TP-Seg通过任务原型框架有效解决了统一医学病灶分割中的特征纠缠问题，实现了跨多种医学成像模态和病灶类型的高性能分割，为AI辅助诊断提供了有效的统一解决方案。
...
TL;DR: TTA-Vid：一种基于测试时强化学习的视频推理方法，无需大规模监督数据和多阶段训练，通过测试时自适应和自适应帧选择在推理时更新模型


<details>
  <summary>Details</summary>
Motivation: 现有视频推理模型依赖大规模监督数据和多阶段训练流程，训练成本高且难以适应新领域。需要一种能够在测试时自适应、无需显式标签的方法来降低训练成本并提高适应性。

Method: 提出测试时视频自适应方法（TTA-Vid），包含两个同时工作的组件：1）测试时自适应，在推理时对多个帧子集进行逐步推理，使用跨不同帧子集计算的批量感知频率奖励作为伪真值更新模型；2）多臂老虎机策略进行自适应帧选择，学习优先选择信息丰富的帧。整个过程完全在测试时进行，无需真实标注或专用训练集。

Result: TTA-Vid在单个批次甚至单个样本上训练后，能够泛化到整个数据集甚至跨数据集。在各种视频推理任务上取得一致改进，能够超越当前基于大规模数据训练的最先进方法。

Conclusion: 测试时强化学习在时序多模态理解方面具有巨大潜力，TTA-Vid方法通过测试时自适应和自适应帧选择，实现了无需大规模监督数据的高效视频推理，为降低训练成本和提高模型适应性提供了新途径。
...
TL;DR: 提出一种基于注意力机制对偶形式的无训练token剪枝框架，通过渐进分块最大边际相关性选择最优token子集，在保持性能的同时显著降低计算成本


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在处理图像和视频理解任务时，随着视觉token数量的增加，计算成本急剧增长。现有的token剪枝方法多基于经验性方法，忽略了注意力机制的内在原理。

Method: 从注意力机制的对偶形式视角出发，将注意力重新表述为隐式线性层，其权重矩阵是单个token键值对生成的外积之和。提出新的度量标准量化token的信息量和信息重复度，并引入渐进分块最大边际相关性算法高效选择最优token子集。

Result: 实验表明该方法在性能和效率之间取得了更好的平衡，为现有剪枝方法提供了新的理论视角。

Conclusion: 基于注意力机制对偶形式的无训练token剪枝框架不仅有效降低了计算成本，还从理论角度解释了token剪枝的本质，为后续研究提供了新的思路。
...
Method: 构建PrivHAR-Bench数据集，应用从轻量级空间模糊到加密块置换的渐进式视觉隐私变换，包含15个活动类别和1,932个源视频，每个视频分布在9个隐私强度递增的层级中，并提供背景移除变体以隔离人类运动特征。

Result: 使用R3D-18模型验证显示，识别准确率随隐私强度增加呈可测量的下降曲线：层级内准确率从88.8%（清晰）降至53.5%（加密+背景移除），跨域准确率降至4.8%。

Conclusion: PrivHAR-Bench为在标准化条件下比较隐私保护HAR方法提供了可控基准，数据集、生成管道和评估代码已公开可用。
...
TL;DR: 该论文提出了SurgSTU-Pipeline，一种用于生成外科手术视频细粒度时空理解数据集的确定性生成流程，并创建了包含7515个视频片段和15万个问答样本的SurgSTU数据集，显著提升了视觉语言模型在外科手术视频中的时空理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前外科手术视频理解领域面临两个主要挑战：现有视觉语言模型数据集无法充分捕捉和评估复杂的时空动态关系；创建大规模细粒度时空关系数据集成本高昂且容易出错。需要一种可靠的方法来生成高质量的外科手术时空理解数据集。

Method: 提出了SurgSTU-Pipeline确定性生成流程，包含时间和空间连续性过滤机制，能够可靠地创建外科手术细粒度时空多模态理解数据集。将该流程应用于公开可用的外科手术数据集，生成了SurgSTU数据集，包含7515个视频片段和15万个细粒度时空问答样本。

Result: 评估显示，最先进的通用视觉语言模型在零样本设置下表现不佳，但通过上下文学习可以改善其时空能力。在SurgSTU训练数据集上微调的视觉语言模型在所有时空任务中取得了最高性能，验证了数据集提升视觉语言模型在外科手术视频中时空理解能力的有效性。

Conclusion: SurgSTU-Pipeline能够可靠地生成高质量的外科手术时空理解数据集，SurgSTU数据集显著提升了视觉语言模型在外科手术视频中的细粒度时空理解能力，为计算机辅助手术的发展提供了重要支持。
...
TL;DR: 该论文提出了评估多模态大语言模型3D运动一致性推理能力的新任务：给定同一场景的两个视角，识别违反3D运动一致性的物体。


<details>
  <summary>Details</summary>
Motivation: 空间一致性是视觉世界的基本属性，也是理解物理现实模型的关键要求。尽管多模态大语言模型（MLLMs）取得了进展，但在跨多个视角的3D几何推理方面仍存在困难。需要更深入地评估模型对物理世界的理解能力。

Method: 提出了一种简单且可扩展的方法，从多视角场景中生成真实的空间不一致图像对，从而系统评估模型识别3D运动一致性违规的能力。

Result: 实验结果表明，最先进的多模态大语言模型在3D运动一致性推理任务上显著低于人类观察者，且在不同场景属性上表现出巨大变异性，显示出对3D结构的脆弱和不完整理解。

Conclusion: 研究发现强调了需要开发对物理世界有更深入基础理解的方法，多模态大语言模型在3D空间一致性推理方面仍有很大改进空间。
...
TL;DR: 本文重新审视了人在回路中的物体检索任务，通过主动学习循环和预训练的ViT表示，在多物体复杂场景中实现高效的交互式物体类别检索。


<details>
  <summary>Details</summary>
Motivation: 在多物体数据集中，目标物体可能只占据图像的小区域且场景复杂混乱，传统的全局描述符方法效果有限。需要开发更适应局部特征的描述符和有效的交互式检索流程。

Method: 采用主动学习循环框架，将检索任务构建为二分类问题，利用预训练的ViT表示，探索不同的表示策略，并解决关键设计问题：图像中考虑哪些物体实例、标注形式、主动选择策略以及最佳特征表示方法。

Result: 比较了多种表示策略在多物体数据集上的表现，揭示了全局上下文捕捉与细粒度局部物体细节之间的权衡关系，为基于主动学习的交互式检索流程设计提供了实用见解。

Conclusion: 通过系统性地重新审视人在回路物体检索任务，提出了基于预训练ViT表示和主动学习的有效解决方案，为复杂多物体场景中的交互式物体类别检索提供了设计指导。
...
TL;DR: 提出Vision-Geometry-Action (VGA)自动驾驶范式，强调密集3D几何信息的重要性，并开发了在线推理的DVGT-2模型，同时输出几何重建和轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 传统端到端自动驾驶从稀疏感知发展到VLA模型，但作者认为车辆在3D世界中运行，密集3D几何信息应该是最关键的决策线索。然而现有几何重建方法依赖多帧批处理，无法用于在线规划。

Method: 提出流式驾驶视觉几何变换器(DVGT-2)，采用时间因果注意力和历史特征缓存支持在线推理，使用滑动窗口流式策略避免重复计算，同时输出密集几何重建和轨迹规划。

Result: DVGT-2在多个数据集上实现了优越的几何重建性能，速度更快。同一模型无需微调即可直接应用于不同相机配置的规划任务，在NAVSIM闭环和nuScenes开环基准测试中表现良好。

Conclusion: VGA范式通过密集3D几何信息为自动驾驶提供了更全面的决策依据，DVGT-2实现了高效的在线几何重建和规划，展示了3D几何在自动驾驶中的关键作用。
...
TL;DR: 本文提出了CLeaRS基准测试，用于评估遥感视觉语言模型的持续学习能力，发现现有模型存在灾难性遗忘问题，现有持续学习方法在遥感领域效果有限。
...
Method: 构建CLeaRS基准测试，包含10个精心挑选的子集、超过20.7万张图像-文本对，涵盖多种解释任务、传感模态和应用场景，并定义了三种评估协议。

Result: 广泛基准测试显示所有设置下都存在灾难性遗忘问题，现有持续学习方法在适应遥感视觉语言模型时，处理任务、指令和模态转换的效果有限。

Conclusion: 遥感视觉语言模型需要专门针对其特点开发的持续学习方法，CLeaRS基准为这一研究方向提供了重要工具。
...
TL;DR: LinguDistill：一种无需额外模块的适配器自由蒸馏方法，通过使用原始冻结语言模型作为教师，恢复多模态适应中损失的语言能力，同时保持视觉任务性能。
...
Method: 提出LinguDistill方法：1）使用原始冻结语言模型作为教师；2）引入层级的KV缓存共享，让教师能够接触到学生的多模态表示；3）在语言密集型数据上选择性地蒸馏教师的强语言信号，恢复语言能力，同时保持学生在多模态任务上的视觉基础。

Result: 在语言和知识基准测试中恢复了约10%的性能损失，同时在视觉密集型任务上保持了相当的性能。表明无需额外模块即可恢复语言能力。

Conclusion: LinguDistill提供了一种高效实用的解决方案，能够解决多模态模型中特定模态性能退化问题，无需增加架构复杂性或推理参数，具有很好的灵活性和适用性。
...
TL;DR: DisCo框架通过解耦视觉和文本信息来解决主题驱动文本到图像生成中的相似性-可控性悖论，先分离主题身份和修改指令，再通过强化学习重新耦合，实现高保真主题保留和精确文本控制。
...
Method: 提出DisCo框架：1) 文本-视觉解耦模块：从参考图像中提取主题身份（使用实体词），简化文本提示仅包含修改指令（使用通用代词）；2) 设计专用奖励信号，通过强化学习重新耦合视觉定义的主题和文本生成的上下文。

Result: 该方法有效解决了相似性-可控性悖论，实现了同时高保真主题保留和精确文本控制。大量实验表明该方法达到了最先进的性能，生成了高度真实和连贯的图像。

Conclusion: DisCo框架通过先解耦后重新耦合视觉和文本信息，成功解决了主题驱动T2I生成中的核心挑战，为同时实现高保真主题保留和精确文本控制提供了有效解决方案。
...
Conclusion: MotionGrounder首次实现了基于DiT的多对象可控运动迁移，通过创新的运动信号设计和对象对齐机制，显著提升了多对象场景下的控制能力
...
<details>
  <summary>Details</summary>
Motivation: 现有的点云运动捕捉方法面临一个困境：基于点的方法具有几何细节但噪声敏感，而基于骨骼的方法鲁棒但过于简化。需要一种既能保持几何表达能力又能保证鲁棒性的有效表示方法。

Method: 提出Sparkle表示方法，统一骨骼关节和表面锚点，通过显式的运动学-几何分解。SparkleMotion框架通过层次化模块学习这种表示，嵌入几何连续性和运动学约束，将内部运动学结构与外部表面几何显式解耦。

Result: SparkleMotion在准确性、鲁棒性和泛化能力方面都达到了最先进的性能，特别是在严重的领域偏移、噪声和遮挡情况下表现优异。在多种传感器类型和挑战性真实场景中展示了优越性。

Conclusion: 通过显式分解运动学结构和表面几何的统一表示方法，能够有效平衡点云运动捕捉的表达能力和鲁棒性，为解决噪声、非结构化点云中的鲁棒表示学习问题提供了有效方案。
...
TL;DR: 提出基于高斯过程混合模型的对象特定功能形状表示方法，通过稀疏点云学习连续方向距离场，替代传统显式3D表示


<details>
  <summary>Details</summary>
Motivation: 传统显式3D表示（如点云和网格）需要大量存储空间来捕捉精细几何细节，并且需要复杂的索引系统进行表面查找，而功能表示提供了高效、紧凑和连续的替代方案

Method: 提出基于高斯过程混合模型的对象特定功能形状表示，通过稀疏点云学习连续方向距离场；在战略参考点处锚定局部高斯过程先验来捕捉复杂拓扑结构，参考点可通过任何结构分解方法（如骨架化、基于距离的聚类）灵活提取

Result: 在ShapeNetCore和IndustryShapes数据集上的广泛评估表明，该方法能够高效且准确地表示复杂几何形状

Conclusion: 基于高斯过程混合模型的功能形状表示方法提供了一种轻量级、高效的替代方案，能够从稀疏点云中学习连续表示，并灵活捕捉复杂拓扑结构
...
TL;DR: 该研究提出了一种基于显式4D表示的框架，为手术智能体配备时空推理工具，通过结合2D多模态大语言模型和3D计算机视觉模型，无需额外训练即可实现手术场景的4D时空理解。


<details>
  <summary>Details</summary>
Motivation: 软组织手术中的时空推理是人工智能的重要能力，但现有2D视觉语言模型难以处理手术场景的空间复杂性，需要显式的4D表示来提升时空推理能力。

Method: 利用点跟踪、深度估计和分割模型构建具有时空一致性的工具和组织语义的4D模型，然后让多模态大语言模型基于显式4D表示（如轨迹）进行推理，无需微调。

Result: 在包含134个临床相关问题的新数据集上评估，发现通用推理骨干与4D表示的结合显著提升了时空理解能力，并实现了4D基础。

Conclusion: 时空智能可以从2D多模态大语言模型和3D计算机视觉模型中"组装"出来，无需额外训练，为手术智能辅助系统和自主机器人提供了新途径。
...
TL;DR: PixelPrune是一种基于预测编码的压缩方法，通过在视觉Transformer编码器之前剪枝冗余图像块，加速文档和GUI理解任务中的视觉语言模型推理，最高可达4.2倍加速。


<details>
  <summary>Details</summary>
Motivation: 文档理解和GUI交互是视觉语言模型的高价值应用，但需要高分辨率输入产生大量视觉token，计算负担重。研究发现图像块中存在大量冗余（22-71%为重复块），造成计算浪费。

Method: 提出PixelPrune方法，利用像素级冗余，通过基于预测编码的压缩在视觉Transformer编码器之前剪枝冗余图像块。该方法无需训练、无学习参数，支持无损压缩和可控有损压缩。

Result: 在三个模型规模和文档/GUI基准测试中，PixelPrune在保持竞争力的任务准确性的同时，实现了最高4.2倍推理加速和1.9倍训练加速。

Conclusion: PixelPrune通过利用图像中的像素级冗余，在视觉Transformer编码器之前进行压缩，显著加速了视觉语言模型的推理和训练过程，为文档和GUI理解任务提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有SAR攻击方法虽然有效，但通常引入明显扰动且局限于数字域，缺乏物理实现考虑，需要更隐蔽且可实际部署的攻击策略

Method: 提出对抗性衰减补丁方法，采用能量约束优化策略结合基于衰减的部署框架，平衡攻击效果与隐蔽性，并与信号级电子干扰机制对齐

Result: AAP能有效降低检测性能，同时保持高不可感知性，在不同模型间表现出良好的可迁移性

Conclusion: 为SAR目标检测系统的对抗攻击提供了物理基础视角，促进了更隐蔽和可实际部署的攻击策略设计
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要针对未经授权的个性化（反个性化策略），但忽略了授权个性化场景下生成的公开图像仍然会通过人脸识别系统泄露用户身份信息的问题。

Method: 提出IDDM（Identity-Decoupled personalized Diffusion Models）模型端防御方法，将身份解耦集成到个性化流程中，采用交替程序：短个性化更新与身份解耦数据优化交替进行，使用两阶段调度平衡身份可链接性抑制和生成效用。

Result: 在多个数据集、多样化提示和最先进的人脸识别系统上的广泛实验表明，IDDM能持续降低身份可链接性，同时保持高质量的个性化生成效果。

Conclusion: IDDM为授权个性化场景提供了有效的隐私保护解决方案，通过模型端输出免疫实现可调节的隐私-效用权衡，满足多样化的隐私需求。
...
TL;DR: JAMMEval是一个经过系统优化的日语视觉问答基准数据集，通过两轮人工标注改进了7个现有日语基准，提升了数据质量和评估可靠性。


<details>
  <summary>Details</summary>
Motivation: 日语VQA基准相比英语基准迭代优化较少，存在模糊问题、错误答案、无需视觉基础即可解答等问题，影响了评估可靠性并导致模型比较的误导性结论。

Method: 通过两轮人工标注系统性地优化7个现有日语基准数据集，构建JAMMEval数据集，改进数据质量和评估可靠性。

Result: 在JAMMEval上评估开源和专有VLM，分析近期模型在日语VQA上的能力。改进后的基准能更好地反映模型能力、降低运行间方差、提高区分不同能力水平模型的能力。

Conclusion: JAMMEval为日语VLM提供了更可靠的评估基准，通过系统性优化解决了现有基准的问题，并发布了数据集和代码以推进VLM的可靠评估。
...
TL;DR: IKEA-Bench基准测试评估了19个视觉语言模型在组装图理解任务上的表现，发现视觉编码是提升跨描绘鲁棒性的关键瓶颈


<details>
  <summary>Details</summary>
Motivation: 2D组装图通常抽象且难以理解，需要智能助手来监控进度、检测错误并提供逐步指导。在混合现实环境中，系统需要从摄像头画面识别已完成和进行中的步骤，并与图解说明对齐。视觉语言模型在此任务上具有潜力，但面临描绘差距问题，因为组装图和视频帧共享的视觉特征很少。

Method: 构建IKEA-Bench基准测试，包含29个IKEA家具产品的1,623个问题，涵盖6种任务类型。评估19个视觉语言模型（2B-38B参数）在三种对齐策略下的表现。进行三级机制分析，包括ViT子空间分析和文本添加对推理模式的影响。

Result: 关键发现：1）组装指令理解可通过文本恢复，但文本同时会降低图解到视频的对齐能力；2）架构家族比参数数量更能预测对齐准确性；3）视频理解仍然是硬瓶颈，不受策略影响。机制分析显示图解和视频占据不相交的ViT子空间，添加文本会使模型从视觉驱动转向文本驱动推理。

Conclusion: 视觉编码是提升跨描绘鲁棒性的主要目标。研究结果为改进视觉语言模型在组装任务中的表现提供了方向，强调了视觉表示学习的重要性。
...
TL;DR: ProCap是一个解决空间增强现实（SAR）中虚拟-物理语义混淆问题的新框架，通过视觉分离和区域感知检索来区分投影内容和物理场景，并创建了首个大规模SAR语义基准数据集RGBP。


<details>
  <summary>Details</summary>
Motivation: 空间增强现实（SAR）使用投影仪将数字内容直接投影到物理场景上，但为了实现智能交互（如场景推理或回答用户查询），需要语义上区分物理场景和投影内容。标准的视觉语言模型（VLMs）在处理这种虚拟-物理模糊性时存在困难，经常混淆两种上下文。

Method: ProCap采用两阶段流程：首先通过自动分割在视觉上隔离虚拟和物理层；然后使用区域感知检索来避免因投影失真导致的模糊语义上下文。同时创建了RGBP数据集（RGB+投影），包含65个多样化物理场景和超过180,000个投影，带有密集的解耦标注。

Result: 建立了使用任务特定标记的双重描述评估协议，独立评估物理场景和投影描述。实验表明ProCap为未来SAR研究提供了强大的语义基础。

Conclusion: ProCap框架通过明确解耦投影内容和物理场景，解决了SAR中的虚拟-物理语义混淆问题，为空间增强现实的智能交互提供了可靠的语义理解基础。
...
TL;DR: 提出一种基于典型相关分析（CCA）的训练后方法，通过利用两个预训练图像编码器表示之间的共享结构，实现表示选择和降维，在保持语义内容的同时丢弃冗余维度。


<details>
  <summary>Details</summary>
Motivation: 现代视觉流程依赖预训练图像编码器，但这些表示通常是过完备且模型特定的，需要提高表示效率。

Method: 使用训练后的典型相关分析（CCA）算子，利用两个预训练图像编码器表示之间的共享结构，找到线性投影作为表示选择和降维的原则性方法。

Result: 表示维度可减少75%以上且下游性能提升，或在固定维度下通过从更大或微调模型进行训练后表示转移来增强表示。在ImageNet-1k、CIFAR-100、MNIST等基准测试中，相比基线和PCA投影表示，准确率提升最高达12.6%。

Conclusion: 提出的基于CCA的训练后方法能有效提高图像表示效率，通过跨模型一致性指导表示蒸馏和精炼，优于传统降维技术如PCA。
...
<details>
  <summary>Details</summary>
Motivation: 现有作物产量预测数据集存在稀缺、质量低、区域限制或作物类型单一等问题，这些限制源于数据获取成本高、数据质量参差不齐以及数据隐私法规约束，阻碍了可扩展数据驱动解决方案的发展。

Method: 创建了YieldSAT数据集，覆盖阿根廷、巴西、乌拉圭和德国等多个气候区，包含玉米、油菜、大豆和小麦等主要作物类型，共2173个专家标注的农田。每个产量样本具有10米空间分辨率，并配有多光谱卫星图像和环境辅助数据。将大规模高分辨率作物产量预测作为像素回归任务，比较了多种深度学习模型和数据融合架构，并探索了领域知识引导的深度集成方法来缓解分布偏移问题。

Result: 数据集包含超过1220万个产量样本和113,555张标记卫星图像。通过比较不同模型发现，领域知识引导的深度集成方法在真实世界条件下表现出显著的性能提升，能够有效缓解严重的数据分布偏移问题。

Conclusion: YieldSAT为作物产量预测研究提供了大规模、高质量、多模态的数据资源，展示了大规模高分辨率作物产量预测作为像素回归任务的潜力，并提出了应对真实世界数据分布偏移的有效方法，为开发可扩展的数据驱动解决方案奠定了基础。
...
TL;DR: GIFD是一种新的联邦学习梯度反演攻击方法，通过搜索GAN中间层的分层特征而非仅优化初始潜在代码，实现了像素级数据重建，并在分布外场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习梯度反演攻击仅在GAN的潜在空间进行优化，限制了表达能力和泛化性。需要更强大的攻击方法来揭示联邦学习系统的隐私漏洞。

Method: 提出GIFD方法：1）解构GAN模型，搜索中间层的分层特征；2）从初始潜在空间逐步优化到更接近输出图像的中间层；3）设计正则化器，通过添加小l1球约束避免生成不真实图像；4）扩展到分布外场景，处理标签不一致问题。

Result: 大量实验表明，该方法能够实现像素级重建，在各种联邦学习场景中优于现有基线方法，包括分布外场景。

Conclusion: GIFD通过搜索GAN中间层特征显著提升了梯度反演攻击的效果，揭示了联邦学习系统在隐私保护方面仍存在严重漏洞，需要更强的防御机制。
...
TL;DR: DLWM提出了一种用于自动驾驶的双重潜在世界模型范式，通过两阶段方法实现基于高斯表示的预训练，显著提升了3D占用感知、4D占用预测和运动规划性能。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的自动驾驶因其低成本和高性能受到关注，但现有方法如密集BEV或稀疏查询模型各有局限。高斯中心方法通过3D语义高斯描述场景，提供了全面而稀疏的表示，但需要有效的预训练框架来充分发挥其潜力。

Method: DLWM采用两阶段双重潜在世界模型：第一阶段通过自监督重建多视角语义和深度图像，从查询中预测3D高斯；第二阶段利用细粒度上下文特征，分别训练两个潜在世界模型进行时序特征学习：高斯流引导的潜在预测用于下游占用感知和预测任务，以及自我规划引导的潜在预测用于运动规划。

Result: 在SurroundOcc和nuScenes基准测试中，DLWM在高斯中心3D占用感知、4D占用预测和运动规划任务上均显示出显著的性能提升。

Conclusion: DLWM为自动驾驶提供了一个有效的双重潜在世界模型范式，能够通过两阶段预训练充分利用高斯中心表示的优势，在多个关键任务上实现性能突破。
...
Conclusion: ACT是一种无需训练、高度自适应的鲁棒解决方案，在不损害基本生成能力的情况下有效缓解幻觉问题
...
TL;DR: 该研究提出了一种仅使用T2加权MRI进行前列腺癌定位的新方法，通过将DWI作为训练时的潜在模态，在推理时仅需T2图像即可实现与多序列方法相当的癌症定位性能。


<details>
  <summary>Details</summary>
Motivation: 多参数MRI需要DWI和T2w序列，但仅使用T2w图像可降低成本和获取难度。研究旨在开发仅用T2w图像就能准确定位前列腺癌的方法，同时利用训练时可用的DWI信息提升性能。

Method: 采用期望最大化算法框架：E步使用基于流匹配的生成模型近似潜在DWI图像的后验分布；M步同时优化癌症定位器和生成模型以最大化癌症存在的期望似然。将DWI作为训练时的特权模态，推理时仅需T2w图像。

Result: 在4,133名前列腺癌患者的内外部数据集上评估，仅T2w方法优于缺乏DWI训练图像的方法，且与使用多序列的基线方法相比表现相当或更好：患者级F1分数提升14.4%，区域级QWK提升5.3%。

Conclusion: 该研究为特权模态学习提供了新颖的理论框架，实现了仅用T2w图像就能获得与多序列方法相当的癌症定位性能，具有显著的临床实用价值。
...
TL;DR: 提出一种无需训练的地滚波衰减框架，将地滚波衰减重新定义为语义引导的信号分离问题，利用大型视觉模型提取语义先验，通过掩码条件低秩逆公式实现自适应抑制和反射保留重建。


<details>
  <summary>Details</summary>
Motivation: 地滚波是陆地和垂直地震剖面数据中的主要相干噪声源，严重掩盖反射事件并降低后续成像和解释质量。传统衰减方法存在适应性有限、信号泄漏或依赖标记训练数据等问题，特别是在强信号噪声重叠情况下表现不佳。

Method: 提出训练免费框架，将地滚波衰减重新定义为语义引导的信号分离问题。使用可提示的大型视觉模型通过将地震道集转换为视觉表示并利用文本或图像提示定位地滚波主导区域来提取高级语义先验。将语义响应转换为连续软掩码，嵌入掩码条件低秩逆公式中，实现空间自适应抑制和反射保留重建。开发基于ADMM的高效求解器解决逆问题。

Result: 在合成和实际VSP数据集上的大量实验表明，该方法实现了优越的地滚波衰减效果，同时保持了反射连续性和波形保真度，一致优于代表性的变换域滤波和隐式神经表示方法。

Conclusion: 该方法提供了一种无需训练、物理一致的地滚波衰减解决方案，通过语义引导的信号分离框架有效解决了传统方法的局限性，在保持反射信号完整性的同时实现了高质量噪声抑制。
...
Method: 1）将3D场景建模为可更新的世界状态；2）通过几何-动作感知的观测模拟模型生成具身交互；3）使用交互感知状态更新模块确保空间一致性；4）设计可扩展的流水线从大规模单目自我中心视频中提取静态点云、相机轨迹和具身动作；5）引入EgoCap低成本采集系统，使用未校准智能手机进行真实世界数据收集。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于对抗训练的防御方法计算成本高且难以泛化到未见攻击类型，需要一种更通用、高效的防御方案。

Method: 提出PDA框架：1) 提示改写生成多个变体；2) 问题分解为子问题；3) 一致性聚合整合结果。完全在推理时操作，无需修改底层模型。

Result: 在多个VLM架构和基准测试（视觉问答、分类、字幕生成）中，PDA对各种对抗扰动均获得一致的鲁棒性提升，同时保持竞争力的干净准确率。

Conclusion: PDA为VLM推理提供了一个通用、强大且实用的防御框架，平衡了鲁棒性和效率，无需额外训练。
...
Method: 使用密集点轨迹作为视觉标记，设计扩散变换器建模无序轨迹集，显式处理遮挡问题

Result: 轨迹标记预测实现了类别无关、数据高效的预测，优于现有基线，并能泛化到稀有物种和形态

Conclusion: 密集点轨迹作为结构化中层表示，为野外预测性视觉智能提供了基础
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法存在两个主要方向：前馈模型在稀疏视图设置下推理速度快，但质量有限；逐场景优化能产生高质量渲染但计算成本高。需要结合两者的优势。

Method: 1) 引入可微分3DGS优化层到训练循环；2) 使用隐函数定理和矩阵自由PCG求解器计算梯度以降低计算成本；3) 集成数据驱动的不确定性模型，自适应控制优化过程中的参数变化；4) 模型无关设计，可无缝集成到现有前馈3DGS架构。

Result: 该方法有效缓解了欠约束区域的过拟合问题，提高了对输入异常值的鲁棒性，能够为测试时优化提供改进，适用于姿态已知和姿态自由的方法。

Conclusion: Diff3R成功桥接了前馈预测和测试时优化，通过可微分优化层学习预测最优初始化，结合了快速推理和高质量渲染的优势，为3D高斯泼溅提供了新的优化框架。
...
TL;DR: 首次使用开源流程从Chandrayaan-2 OHRC多视角影像生成亚米级月球数字高程模型，通过几何分析筛选立体像对，实现24-54厘米空间分辨率，垂直精度5.85米，水平精度小于30厘米。


<details>
  <summary>Details</summary>
Motivation: 高分辨率月球数字高程模型对表面移动规划、着陆点表征和行星科学研究至关重要。Chandrayaan-2的OHRC相机具有目前月球轨道成像中最佳的地面采样能力（20-30厘米/像素），但尚未有使用开源流程从OHRC影像生成亚米级DEM的研究。

Method: 1. 通过几何分析图像元数据（基线高度比计算和会聚角估计）从未配对的OHRC档案中识别候选立体像对；2. 应用密集立体匹配和射线三角测量生成点云；3. 将点云网格化为DEM；4. 通过迭代最近点算法与LRO NAC DTM对齐建立绝对高程一致性；5. 进行恒定偏差偏移校正。

Result: 在五个地理分布的月球站点生成了有效空间分辨率约24-54厘米的DEM，通过NAC参考地形验证获得垂直RMSE为5.85米（原生OHRC分辨率），通过平面特征匹配评估的水平精度小于30厘米。

Conclusion: 成功开发了首个完全开源的OHRC亚米级DEM生成流程，证明了从非配对OHRC档案中生成高质量地形产品的可行性，为月球高分辨率地形测绘提供了新方法。
...
TL;DR: ONE-SHOT是一个参数高效的组合式人-环境视频生成框架，通过解耦人类动态和环境信号，无需3D预处理即可实现精细控制


<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型在人类中心视频合成方面取得进展，但精细且独立地编辑主体和场景仍然是一个关键挑战。现有方法通过刚性3D几何组合提供更丰富的环境控制，但面临精确控制与生成灵活性之间的权衡，且繁重的3D预处理限制了实际可扩展性。

Method: 1. 提出规范空间注入机制，通过交叉注意力将人类动态与环境线索解耦；2. 提出动态接地RoPE位置嵌入策略，在不依赖启发式3D对齐的情况下建立不同空间域之间的空间对应关系；3. 引入混合上下文集成机制，支持分钟级长时程合成，保持主体和场景一致性。

Result: 实验表明，该方法显著优于最先进的方法，为视频合成提供卓越的结构控制和创意多样性。

Conclusion: ONE-SHOT是一个参数高效的框架，通过解耦生成过程实现组合式人-环境视频生成，解决了精确控制与生成灵活性之间的权衡问题，无需繁重的3D预处理，具有实际可扩展性。
...
TL;DR: CROWD是一个从YouTube公开视频中筛选和分割出的城市行车记录仪数据集，包含51,753个连续片段，覆盖全球238个国家和地区，专注于日常驾驶场景，排除事故和编辑内容，提供时间、车辆类型标签及机器生成的物体检测数据。


<details>
  <summary>Details</summary>
Motivation: 现有行车记录仪数据集往往关注事故、编辑内容或特定场景，缺乏反映日常普通驾驶的多样化、连续、未编辑的真实世界数据，这限制了跨域鲁棒性和交互分析的研究。

Method: 从YouTube公开视频中手动筛选和分割行车记录仪片段，排除事故、事故后果和编辑内容，专注于日常驾驶场景。数据集包含视频标识符和片段边界，提供手动标注的时间（白天/夜晚）和车辆类型标签，以及使用YOLOv11x生成的MS-COCO 80类物体检测数据和BoT-SORT多目标跟踪结果。

Result: 创建了包含51,753个片段记录的数据集，总时长20,275.56小时（42,032个视频），覆盖全球238个国家和地区的7,103个有人居住地点，涵盖所有六大洲。提供了片段级的手动标签和机器生成的检测数据，便于基准测试。

Conclusion: CROWD数据集填补了日常驾驶数据集的空白，通过提供多样化、连续、未编辑的真实世界行车记录仪数据，支持跨域鲁棒性和交互分析研究，同时通过视频标识符而非原始视频的方式分发，确保研究的可重复性。
...
<details>
  <summary>Details</summary>
Motivation: 对比增强CT（CECT）在临床中很重要但受限于对比剂的侵入性和辐射风险。现有虚拟对比增强方法在处理解剖异质性和空间错位方面存在困难，导致增强模式不一致和细节错误。

Method: 提出PHASOR框架：1）将CT体积视为连贯序列，利用视频扩散模型增强结构一致性和体积准确性；2）引入解剖路由专家混合（AR-MoE）模块，将不同增强模式锚定到解剖语义，使用器官特定记忆捕获显著细节；3）引入强度相位感知表示对齐（IP-REPA）模块，突出精细对比信号同时减轻不完美空间对齐的影响。

Result: 在三个数据集上的广泛实验表明，PHASOR在合成质量和增强准确性方面显著优于现有最先进方法。

Conclusion: PHASOR通过创新的体扩散框架和两个互补模块，成功解决了CT虚拟对比增强中的解剖异质性和空间错位问题，为临床提供了一种高质量的非侵入性替代方案。
...
TL;DR: ProOOD是一个轻量级即插即用方法，通过原型引导的语义补全和训练免费的OOD评分，提升3D语义占据预测的鲁棒性，解决长尾类别偏差和分布外输入问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D语义占据预测方法存在两个主要问题：1）对长尾类别偏差敏感，容易将异常样本过度自信地分配给稀有类别；2）对分布外输入脆弱，需要更可靠的异常检测能力。这些问题在自动驾驶等安全关键应用中尤为突出。

Method: ProOOD包含三个核心组件：1）原型引导的语义补全，用类别一致的特征填充遮挡区域；2）原型引导的尾部挖掘，增强稀有类别表示以减少OOD吸收；3）EchoOOD评分，融合局部logit一致性、局部和全局原型匹配，生成可靠的体素级OOD分数。

Result: 在五个数据集上的实验表明，ProOOD在3D占据预测和OOD检测方面均达到最先进性能。在SemanticKITTI上，整体mIoU提升+3.57%，尾部类别mIoU提升+24.80%；在VAA-KITTI上，AuPRCr提升+19.34分，在所有基准测试中均取得一致改进。

Conclusion: ProOOD通过原型引导的细化和训练免费的OOD评分，显著提升了3D语义占据预测的校准性和OOD检测的可靠性，为安全关键的城市驾驶场景提供了更稳健的解决方案。代码已开源。
...
TL;DR: ReMoGen是一个实时交互到反应生成框架，利用大规模单人运动先验和元交互模块，通过分段生成和帧级细化实现低延迟高质量运动响应。


<details>
  <summary>Details</summary>
Motivation: 真实世界中的人类行为本质上是交互式的，个体的运动受到周围智能体和场景的影响。这种能力对于虚拟化身、交互式动画和人机协作等应用至关重要。然而，现有方法面临两大挑战：1）交互数据有限且分散在异构的单人、人-人和人-场景领域；2）需要在连续在线交互中产生低延迟且高保真的运动响应。

Method: 提出ReMoGen（反应运动生成）模块化学习框架：1）从大规模单人运动数据集中学习通用运动先验；2）通过独立训练的元交互模块将先验适应到目标交互领域；3）采用分段级生成结合轻量级帧级分段细化模块，在帧级别整合新观察到的线索，提高响应性和时间连贯性。

Result: 在人-人、人-场景和混合模态交互设置下的广泛实验表明，ReMoGen能够产生高质量、连贯且响应迅速的反应，同时在多样化的交互场景中有效泛化。

Conclusion: ReMoGen通过结合通用运动先验和领域特定适应，以及创新的分段生成与帧级细化策略，成功解决了交互数据稀缺和实时响应需求的双重挑战，为实时交互到反应生成提供了有效的解决方案。
...
TL;DR: ProTPS是一种用于持续学习的原型引导文本提示选择方法，通过视觉原型指导文本提示的学习，在类增量、跨数据集持续学习以及真实海洋物种数据集上表现优异。
...
Method: 提出原型引导文本提示选择方法，学习类特定的视觉原型和文本提示，使用视觉原型指导每个类的文本提示选择和学习，增加训练灵活性以鼓励学习独特文本提示。

Result: 在类增量设置和跨数据集持续学习设置中，ProTPS性能接近理论上限；在真实海洋物种数据集Marine112上，该方法在类域增量学习设置和自然长尾分布下表现优于现有最先进方法。

Conclusion: ProTPS通过视觉原型引导文本提示学习，有效解决了持续学习中文本提示独特性的挑战，在多种设置下均表现优异，为社区提供了新的真实世界数据集和挑战。
...
TL;DR: ReinDriveGen是一个可控的动态驾驶场景生成框架，允许用户自由编辑车辆轨迹来模拟安全关键场景，通过3D点云重建、车辆补全和视频扩散模型生成逼真驾驶视频，并使用RL后训练策略提升分布外场景质量。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶场景生成方法缺乏对动态场景的完全可控性，难以模拟安全关键场景（如碰撞、失控车辆等），这些场景通常超出训练数据分布，需要新的方法来生成逼真且可控的驾驶视频。

Method: 1. 从多帧LiDAR数据构建动态3D点云场景；2. 引入车辆补全模块从部分观测重建完整360°几何；3. 将编辑后的场景渲染为2D条件图像；4. 使用视频扩散模型合成驾驶视频；5. 提出基于RL的后训练策略，包含成对偏好模型和成对奖励机制，在无真实监督下提升分布外场景质量。

Result: 实验表明ReinDriveGen在编辑驾驶场景上优于现有方法，在新型自我视角合成任务上取得了最先进的结果，能够生成高质量的安全关键场景视频。

Conclusion: ReinDriveGen提供了一个完全可控的动态驾驶场景生成框架，能够模拟各种安全关键场景，通过创新的3D重建和RL后训练策略，在分布外条件下生成逼真的驾驶视频，为自动驾驶测试和验证提供了有力工具。
...
TL;DR: 本文提出Open3D-AD方法，用于开放集监督的3D异常检测，利用正常样本、模拟异常和部分真实异常来建模正常与异常数据的概率密度分布，并在新构建的Open-Industry数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 在真实制造场景中，虽然获取高精度点云数据成本高，但通常可以收集到有限数量的异常样本。因此，研究开放集监督的3D异常检测，即仅用正常样本和少量已知异常样本训练模型，旨在测试时识别未知异常。

Method: 提出Open3D-AD方法：1) 将通用开放集异常检测方法适配到3D点云输入；2) 利用正常样本、模拟异常和部分观察到的真实异常建模正常与异常数据的概率密度分布；3) 引入简单的对应分布子采样技术，减少正常与非正常分布之间的重叠，实现更强的双分布建模。

Result: 在Open-Industry数据集以及现有数据集Real3D-AD和Anomaly-ShapeNet上建立了全面的基准测试。基准结果和消融研究证明了Open3D-AD的有效性，并进一步揭示了开放集监督3D异常检测的潜力。

Conclusion: 本文提出的Open3D-AD方法在开放集监督3D异常检测任务中表现出色，通过建模双分布和减少分布重叠，能够有效识别未知异常，为工业制造场景中的质量检测提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有编辑方法缺乏对中心主体结构完整性的保护能力，无法实现局部、细粒度的场景变换操作，需要一种能够保持结构完整性的3D场景编辑方法。

Method: 采用三阶段方法：1)多视角3D锚点合成，利用MV-TRACE数据集生成空间一致的3D锚点；2)实体几何锚定，通过两阶段注册确保插入网格与3DGS场景的精确空间同步；3)上下文视频掩码，将3D投影整合到自回归视频管道中实现时间稳定的物理渲染。

Result: TRACE在编辑多样性和结构完整性方面持续优于现有方法，能够实现高保真的场景变换。

Conclusion: TRACE通过3D几何锚定实现了自动化、高保真的3D场景编辑，在保持结构完整性的同时支持细粒度的局部操作，为3D场景编辑提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是评估在计算资源受限环境下（仅使用消费级CPU，无GPU），不同信息提取方法从学术文档中提取信息的可靠性和效率，特别是针对KRS文档中的表格和元数据提取。

Method: 采用三种策略：1）纯LLM方法；2）混合确定性-LLM方法（正则表达式+LLM）；3）基于Camelot的管道配合LLM回退。使用三个12-14B参数的LLM模型（Gemma 3、Phi 4、Qwen 2.5），在Ollama上本地运行。评估了140个文档的LLM测试和860个文档的Camelot管道评估，使用精确匹配（EM）和Levenshtein相似度（LS）指标，阈值为0.7。

Result: 基于Camelot的管道配合LLM回退在准确性和计算效率方面表现最佳，EM和LS达到0.99-1.00，大多数情况下每PDF处理时间少于1秒。Qwen 2.5:14b模型在所有场景中表现最一致。混合方法相比纯LLM方法能提高效率，特别是对于确定性元数据提取。

Conclusion: 在计算资源受限环境中，整合确定性方法和LLM方法对于从基于文本的学术文档中提取信息越来越可靠和高效。基于Camelot的管道配合LLM回退提供了最佳的准确性和效率平衡，Qwen 2.5模型表现出最稳定的性能。
...
TL;DR: LinearARD是一种自蒸馏方法，通过注意力结构一致性恢复RoPE扩展模型的短文本性能，仅需4.25M训练token即可在长上下文扩展中保持原始模型能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过扩展位置编码和轻量级持续预训练来扩展上下文窗口，但这通常会破坏原始模型能力，导致短文本基准测试性能下降。需要一种方法既能扩展上下文长度，又能保持原始模型性能。

Method: 提出LinearARD自蒸馏方法，使用冻结的原始RoPE教师模型监督RoPE扩展的学生模型。通过对齐密集Q/Q、K/K、V/V自关系矩阵的行分布来直接监督注意力动态。引入线性内存核来克服n×n关系图的二次内存瓶颈，利用每token的log-sum-exp统计量，并将logit重新计算融合到反向传播中。

Result: 在LLaMA2-7B从4K扩展到32K的实验中，LinearARD恢复了最先进基线方法98.3%的短文本性能，同时在长上下文基准测试中超越它们。仅使用4.25M训练token，而LongReD和CPT需要256M token。

Conclusion: LinearARD是一种高效的自蒸馏方法，能够有效恢复RoPE扩展模型的短文本性能，同时保持长上下文处理能力，显著减少了训练数据需求，为模型上下文扩展提供了更高效的解决方案。
...
TL;DR: 研究评估了LLM-as-judge框架在评估大语言模型解释性质量时与人类判断的一致性，发现虽然能捕捉大致趋势，但在分数量级和具体指标上存在显著差异，建议将其用于筛选而非替代人类判断。


<details>
  <summary>Details</summary>
Motivation: 随着质性研究者越来越多地使用自动化工具支持解释性分析，大语言模型常被直接引入分析流程而缺乏系统性评估。模型选择对解释结果有潜在影响但未被充分研究，因此需要评估LLM-as-judge评估是否与人类对解释性质量的判断一致，能否为模型选择提供依据。

Method: 使用712个K-12数学教师半结构化访谈对话片段，通过五个主流推理模型生成一句话解释性回应。使用AWS Bedrock的LLM-as-judge框架进行自动化评估（五个指标），同时对分层抽样的回应由训练有素的人类评估者独立评估解释准确性、细微差别保留和解释连贯性。

Result: LLM-as-judge评分在模型层面能捕捉人类评估的广泛方向性趋势，但在分数量级上存在显著差异。自动化指标中，连贯性与聚合人类评分最一致，而忠实性和正确性在片段层面显示系统性不一致，特别是对于非字面和细微的解释。安全相关指标与解释性质量基本无关。

Conclusion: LLM-as-judge方法更适合用于筛选或淘汰表现不佳的模型，而非替代人类判断。这为质性研究流程中系统比较和选择大语言模型提供了实用指导。
...
TL;DR: 本文针对大型推理模型（LRMs）在微调或后训练后安全性下降的问题，提出了一种轻量级解决方案SafeReAct，通过LoRA适配器恢复被抑制的安全机制，在不影响推理性能的前提下显著提升模型安全性。


<details>
  <summary>Details</summary>
Motivation: 尽管通用大语言模型（LLMs）性能优异，但在针对特定任务进行微调或后训练（如大型推理模型LRMs）后，往往会牺牲安全性。这些经过后训练的模型相比原始LLMs表现出更多有害行为，可能导致严重后果。本文旨在探究LRMs安全性下降的根本原因，并提出解决方案。

Method: 首先分析LRMs安全性下降的原因，发现后训练会掩盖原始LLM的安全机制，同时过度放大与后训练能力相关的表征。基于安全机制仍然存在的发现，提出SafeReAct方法：通过在少数层上使用LoRA适配器进行对齐，恢复被抑制的安全行为。该方法轻量且成本效益高。

Result: 在四个最先进的LRMs上进行实验，结果表明SafeReAct方法在有害提示上显著提高了安全性，同时不损害推理性能。在其他领域特定LLMs（如医疗模型）上的额外结果进一步证实了该方法的通用性和有效性。

Conclusion: 后训练导致LRMs安全性下降的原因是掩盖了原始安全机制，但安全机制并未被移除。提出的SafeReAct方法能够有效恢复被抑制的安全行为，在保持推理性能的同时提升模型安全性，且具有通用性，适用于各种领域特定LLMs。
...
TL;DR: 本文提出了一种结合结构化判别-校准推理与基于提示的强化学习的新训练框架，用于提升多模态情感分析的可解释性和性能，在细粒度情感回归任务中取得了更高准确率。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然通过监督微调取得了SOTA性能，但其端到端的"黑盒"特性限制了可解释性。现有的思维链推理方法面临高标注成本问题，而强化学习方法则存在探索效率低和奖励稀疏等挑战，特别是在困难样本上表现不佳。

Method: 提出了一种集成结构化判别-校准推理与基于提示的强化学习的训练框架。首先使用教师模型合成的优质思维链数据进行冷启动监督微调，这些数据天然包含DC结构，使模型具备先进行宏观判别再进行细粒度校准的推理范式。在此基础上提出Hint-GRPO方法，利用DC结构中的判别阶段作为强化学习中的可验证锚点，为困难样本提供方向性提示，指导策略优化并有效缓解奖励稀疏问题。

Result: 在Qwen2.5Omni-7B模型上的实验表明，该方法不仅在细粒度情感回归任务中实现了更高准确率，还能生成高质量的结构化推理链。更重要的是，在跨域评估中展现出优越的泛化能力。
...
TL;DR: 研究者开发了UTCO框架来系统测试LLM在心理健康问答中的安全性，发现幻觉率6.5%、遗漏率13.2%，遗漏主要集中在危机和自杀意念提示中。


<details>
  <summary>Details</summary>
Motivation: 心理健康问题常在非临床环境中表达，需要安全关键指导。现有LLM评估对高压力叙事性询问的代表性不足，需要更系统的压力测试方法。

Method: 提出UTCO（用户、主题、上下文、语气）提示构建框架，将询问分解为四个可控元素进行系统压力测试。使用2,075个UTCO生成的提示评估Llama 3.3，标注幻觉（捏造或错误的临床内容）和遗漏（缺失必要的临床或安全关键指导）。

Result: 幻觉发生率为6.5%，遗漏率为13.2%。遗漏主要集中在危机和自杀意念提示中。通过回归分析、元素特定匹配和相似性匹配比较，失败最一致地与上下文和语气相关，而用户背景指标在平衡后未显示系统性差异。

Conclusion: 应将遗漏作为主要安全结果进行评估，并超越静态基准问题集。UTCO框架支持对心理健康问答系统进行更全面的安全评估。
...
TL;DR: ASCAT是一个高质量的英阿平行科学翻译基准语料库，包含5个科学领域的完整摘要，通过多引擎翻译和人工验证构建，用于科学翻译评估和模型训练。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语-英语语料库大多基于短句或单一领域文本，缺乏针对科学领域完整摘要的高质量翻译资源，这限制了科学翻译质量的评估和领域特定翻译模型的开发。

Method: 从5个科学领域（物理、数学、计算机科学、量子力学、人工智能）收集完整科学摘要，使用三种互补架构（生成式AI、基于Transformer的模型、商业MT API）进行翻译，然后由领域专家在词汇、句法和语义层面进行人工验证。

Result: 构建了包含67,293个英语标记和60,026个阿拉伯语标记的语料库，阿拉伯语词汇量为17,604个独特单词。在语料库上评估了三个最先进的LLM：GPT-4o-mini（BLEU: 37.07）、Gemini-3.0-Flash-Preview（BLEU: 30.44）和Qwen3-235B-A22B（BLEU: 23.68），展示了其作为评估基准的区分能力。
...
TL;DR: 该研究提出利用人类认知限制（工作记忆容量有限）来区分在线行为研究中的真实人类参与者和大型语言模型，即使后者被特意指示模仿人类限制。


<details>
  <summary>Details</summary>
Motivation: 在线行为研究的有效性依赖于参与者是人类而非机器。过去可以通过简单挑战来检测机器，但基于大型语言模型的通用智能体现在能够解决许多这类挑战，威胁在线行为研究的有效性。

Method: 通过探测人类认知限制——有限的工作记忆容量，使用标准的序列回忆任务进行认知建模，即使LLMs被特别指示模仿人类工作记忆限制，也能区分在线参与者和LLMs。

Result: 研究表明，即使大型语言模型被指示模仿人类工作记忆限制，通过认知建模在序列回忆任务上仍能有效区分在线人类参与者和LLMs。

Conclusion: 利用已确立的认知现象来区分大型语言模型和人类是可行的，这为在线行为研究提供了新的验证方法。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探究心理学概念在不同传播语境（科学话语与大众媒体话语）中的语义变化，了解专业术语如何在大众传播中被重新解释和重构。

Method: 构建了两个俄语语料库：科学语料库（约300篇研究文章，767,543个词元）和科普语料库（来自在线心理学平台，1,199,150个词元）。经过预处理（OCR识别、词形还原、停用词和非信息字符移除）后，采用频率分析、聚类和语义关联识别等方法进行分析。

Result: 结果显示两种话语类型在词汇和概念框架上存在显著差异：科学文本强调方法论和临床术语，而科普材料突出日常经验和治疗实践。关键概念（如倦怠和抑郁）的语义关联比较表明，科学话语将这些术语与心理资源、症状学和诊断结构联系起来，而科普话语则通过个人叙事、情感和日常情境来构建它们。

Conclusion: 研究发现心理学概念在大众媒体话语中从精确的专业术语向更普遍化和经验化的意义转变，证实了分布语义学方法在识别不同交际语境中心理学概念语义转换方面的有效性。
...
TL;DR: 提出一个可配置的管道，用于生成具有特定特征（如领域、地理位置和流行度）的多语言实体数据集，用于评估LLMs长文本生成的事实性，并展示了RiDiC数据集作为示例。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要基于短问答数据集，缺乏对长文本生成事实性的评估。需要创建具有特定特征的多语言实体数据集来补充现有评估方法。

Method: 使用Wikipedia和Wikidata数据构建可配置的管道，生成具有领域、地理位置、流行度等特征的多语言实体集。创建了RiDiC数据集，包含3,000个来自河流、自然灾害和汽车模型三个领域的实体，每个实体附带地理位置、中英文名称和维基百科内容。

Result: 使用三个LLM生成了关于RiDiC实体的中英文长文本，通过第三方事实性检查器评估发现，即使是前沿模型也会对这些实体产生幻觉。代码、数据和评估脚本已公开发布。

Conclusion: 提出的可配置管道能够生成用于评估LLM长文本事实性的多语言实体数据集，RiDiC数据集展示了该方法在揭示模型幻觉方面的有效性，为多语言事实性评估提供了工具和资源。
...
TL;DR: 本文提出了一种基于预训练Transformer模型的时间情感聚合框架，用于检测用户反馈中的异常事件，如恶意评论活动或用户满意度突然下降。


<details>
  <summary>Details</summary>
Motivation: 在客户反馈监控、品牌声誉管理和产品健康跟踪等实际应用中，理解用户情感的时间动态对于早期检测异常事件至关重要。传统的情感分析方法侧重于单个文本分类，无法有效捕捉由于短用户评论中的固有噪声和类别不平衡导致的集体行为变化。

Method: 提出时间情感聚合框架：使用预训练的Transformer语言模型（RoBERTa）提取每条评论的情感信号，然后将这些信号聚合为时间窗口级别的分数。通过检测这些聚合分数的显著下降来识别用户反馈模式中的潜在异常。

Result: 在真实社交媒体数据上的实证评估表明，聚合情感分数能够揭示有意义的趋势，并支持有效的异常检测。实验证明该方法成功识别了与连贯投诉模式相对应的统计显著情感下降。

Conclusion: 该方法为反馈异常监控提供了一个有效且可解释的解决方案，能够检测用户反馈中的集体行为变化和异常事件。
...
TL;DR: 研究发现语言模型处理伦理指令存在四种不同模式，伦理指令格式仅对高处理能力模型有效，且安全性与合规性、伦理处理能力三者基本可分离。


<details>
  <summary>Details</summary>
Motivation: 虽然对齐安全研究假设伦理指令能改善模型行为，但语言模型内部如何处理这些指令仍不清楚。研究者希望探究不同模型如何内部处理伦理指令，以及指令格式对处理过程的影响。

Method: 进行了600多次多智能体模拟，涵盖四个模型（Llama 3.3 70B, GPT-4o mini, Qwen3-Next-80B-A3B, Sonnet 4.5）、四种伦理指令格式（无指令、最小规范、推理规范、美德框架）和两种语言（日语、英语）。开发了三个新指标：审议深度（DD）、跨困境价值一致性（VCAD）和他人识别指数（ORI）来分析伦理处理模式。

Result: 发现了四种不同的伦理处理类型：输出过滤器（GPT；安全输出但无内部处理）、防御性重复（Llama；通过公式化重复实现高一致性）、批判性内化（Qwen；深度审议但整合不完全）和原则一致性（Sonnet；审议、一致性和他人识别共存）。关键发现是处理能力与指令格式的交互作用：在低DD模型中，指令格式对内部处理无影响；在高DD模型中，推理规范和美德框架产生相反效果。词汇合规性与任何处理指标在细胞层面均无相关性。

Conclusion: 伦理处理模式具有模型特异性，安全、合规性和伦理处理能力在很大程度上是可分离的。这些模式与临床罪犯治疗中观察到的模式有结构对应性，其中形式合规而无内部处理被认为是风险信号。
...
TL;DR: 使用机器学习方法分析苏拉威西南岛语系语言中的非主流词汇层，发现26.5%的词汇可能来自底层语言，但缺乏单一底层语言的证据。


<details>
  <summary>Details</summary>
Motivation: 苏拉威西南岛语系语言中存在许多无法追溯到原始南岛语的词汇，这些词汇的语音模式与继承词根不一致。研究者希望确定这些非主流词汇是前南岛语底层语言还是独立创新。

Method: 结合基于规则的同源词减法与机器学习分类器（XGBoost），使用南岛语基本词汇数据库中的1,357个词形，通过26个语音特征训练分类器区分继承词与非主流词汇。

Result: 识别出438个候选底层词形（26.5%），机器学习分类器AUC=0.763，发现非主流词汇具有更长形式、更多辅音丛、更高喉塞音率和较少南岛语前缀等特征。但聚类分析未发现单一底层语言证据（轮廓系数=0.114）。地理分析显示苏拉威西语言的非主流词汇率（平均0.606）高于西印尼语言（0.393）。

Conclusion: 语音机器学习可以补充传统比较方法检测非主流词汇层，但语音不一致性不能作为共享底层语言的证据。研究揭示了苏拉威西语言中的复杂词汇层化现象，而非单一前南岛语底层。
...
TL;DR: WHBench是一个针对女性健康的AI评估基准，包含47个专家设计的医疗场景，测试22个模型在10个女性健康主题上的表现，发现所有模型平均得分低于75%，最佳模型仅72.1%，揭示临床部署需要专家监督。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于医疗指导，但女性健康在基准设计中评估不足。现有评估缺乏针对女性健康特定需求的系统性测试，无法暴露临床上有意义的失败模式。

Method: 创建WHBench评估套件，包含47个专家设计的场景覆盖10个女性健康主题。使用23项标准评分体系评估22个模型，涵盖临床准确性、完整性、安全性、沟通质量等维度，采用安全加权惩罚和服务器端分数重新计算。

Result: 在3,102个尝试回答中（3,100个评分），没有模型平均表现超过75%，最佳模型达到72.1%。即使顶级模型也显示出低完全正确率和显著的危害率变化。评估者间信度在响应标签层面中等，但在模型排名层面较高。

Conclusion: WHBench提供了一个公开的、关注失败模式的基准，用于追踪女性健康AI领域更安全、更公平的进展。研究支持WHBench在比较系统评估中的实用性，同时强调临床部署需要专家监督。
...
TL;DR: 大型语言模型在7.7%的基准问题上表现不如小模型，研究发现这是因为模型规模增大会导致过度详细回答而引入错误，通过约束回答简洁性可以显著提升大模型性能并逆转性能差距。


<details>
  <summary>Details</summary>
Motivation: 研究发现一个反直觉现象：在多个数据集的基准问题上，更大的语言模型反而比小模型表现更差。这挑战了"模型越大性能越好"的普遍认知，需要探究其背后的机制和解决方案。

Method: 系统评估了31个模型（0.5B-405B参数）在1,485个问题上的表现，通过因果干预实验验证机制，并采用简洁性约束来限制大模型的过度详细回答。

Result: 约束大模型给出简洁回答可将准确率提升26个百分点，将性能差距减少三分之二。在数学推理和科学知识基准上，简洁约束完全逆转了性能层次，大模型反而比小模型有7.7-15.9个百分点的优势。

Conclusion: 大模型具有优越的潜在能力，但通用提示设计会掩盖这些能力。最大化大模型性能需要规模感知的提示工程，而不是通用评估协议。提示适应既能提高准确性又能降低计算成本。
...
TL;DR: 研究通过多智能体对话实验平台，发现异质大语言模型群体会自发形成社会角色分化，而揭示真实模型名称会显著增加行为趋同。


<details>
  <summary>Details</summary>
Motivation: 探究多个大语言模型在共享对话中是否会发展出差异化的社会角色，还是趋向于统一行为，理解多智能体交互中的行为多样性机制。

Method: 构建控制实验平台，在统一推理后端上协调7个异质LLM同时进行多智能体讨论，系统变化群体组成、命名约定和提示结构，涵盖12个实验系列（208次运行，13,786条编码消息）。每条消息由来自不同模型家族的两个LLM法官独立编码六个行为标志，采用保守交集裁决。

Result: 1) 异质群体比同质群体表现出显著更丰富的行为分化；2) 群体在智能体崩溃时自发表现出补偿性响应模式；3) 揭示真实模型名称显著增加行为趋同；4) 移除所有提示支架会使行为特征收敛到同质水平。这些行为在智能体单独操作时不存在。

Conclusion: 行为多样性是一种结构化、可复现的现象，由架构异质性、群体上下文和提示级支架的相互作用驱动，而非孤立智能体的固有特性。
...
TL;DR: 本研究探讨了多语言多机构电子健康记录预测，通过文本化处理和语言对齐策略，实现了无需手动标准化的跨国ICU数据集联合训练。


<details>
  <summary>Details</summary>
Motivation: 大规模跨机构电子健康记录预测面临模式异质性和编码系统差异的挑战。虽然通用数据模型可以标准化记录，但手动协调和词汇映射成本高且难以扩展。文本化协调提供了替代方案，但跨国数据集引入了语言异质性这一额外障碍，需要解决以实现真正可扩展的EHR学习。

Method: 研究比较了两种处理语言障碍的实用策略：1) 使用多语言编码器直接建模多语言记录；2) 通过基于LLM的词级翻译将非英语记录翻译成英语。在七个公共ICU数据集上，对十个具有多个预测窗口的临床任务进行评估。

Result: 基于翻译的语言对齐比多语言编码器产生更可靠的跨数据集性能。多机构学习模型始终优于需要手动特征选择和协调的强基线，也超过了单数据集训练。文本化框架结合语言对齐通过少样本微调有效执行迁移学习，获得额外增益。
...
TL;DR: Hi-CoT是一种分层思维链提示方法，通过交替进行指令规划和逐步执行来分解推理过程，相比传统CoT在数学推理任务上平均准确率提升6.2%，推理轨迹长度减少13.9%。


<details>
  <summary>Details</summary>
Motivation: 传统思维链提示方法通常依赖非结构化、扁平的推理链，存在冗余和性能不佳的问题，特别是在处理复杂多步推理任务时表现不足。

Method: 提出分层思维链提示方法，将推理过程分解为分层子步骤，通过交替进行指令规划和逐步执行来管理长推理范围并保持逻辑一致性。

Result: 在多种大语言模型和数学推理基准测试中，Hi-CoT相比CoT提示平均准确率提升6.2%（某些模型和任务上最高达61.4%），同时推理轨迹长度减少13.9%。

Conclusion: 分层思维链提示能有效提升大语言模型的推理能力和效率，当模型严格遵循分层结构时，准确率和效率达到最大化。
...
TL;DR: Oblivion是一个LLM智能体记忆控制框架，通过选择性遗忘机制模拟人类记忆的适应性，将遗忘视为可访问性衰减而非显式删除，从而减少干扰和延迟。


<details>
  <summary>Details</summary>
Motivation: 人类记忆通过选择性遗忘实现适应性：经验随时间变得不易访问，但可通过强化或上下文线索重新激活。相比之下，基于记忆增强的LLM智能体依赖"始终开启"的检索和"扁平"的记忆存储，随着历史增长会导致高干扰和延迟。

Method: Oblivion将记忆控制解耦为读取路径和写入路径。读取路径基于智能体不确定性和记忆缓冲区充足性决定何时查询记忆，避免冗余的始终开启访问。写入路径通过强化对形成响应有贡献的记忆来决定强化什么。这实现了分层记忆组织，保持持久的高层策略，同时根据需要动态加载细节。

Result: 在静态和动态长时程交互基准测试中，Oblivion能够动态调整记忆访问和强化，在变化上下文下平衡学习和遗忘，表明记忆控制对有效的LLM智能体推理至关重要。

Conclusion: 记忆控制是有效LLM智能体推理的关键要素，Oblivion框架通过模拟人类选择性遗忘机制，实现了更高效、适应性更强的记忆管理。
...
TL;DR: 本研究使用分布语义学探讨波兰语词汇的音系和形态结构与意义之间的关系，发现语义向量能捕捉子词汇语言单位信息，且计算模型能准确预测理解和产出。


<details>
  <summary>Details</summary>
Motivation: 探究波兰语词汇的音系形态结构（特别是辅音簇）与其意义之间是否存在关联，以及复杂词的音系和形态结构是否在语义空间中有所反映。

Method: 使用统计和计算技术（t-SNE、线性判别分析、线性判别学习），通过分布语义学分析波兰语词汇，利用语义向量预测音系形态特征。

Result: 语义向量不仅能编码丰富的形态句法信息（时态、数、格），还能捕捉音素串等子词汇语言单位信息；音系复杂性、形态透明度和多种形态句法范畴都能从词嵌入中预测。

Conclusion: 计算建模（判别词典模型）能准确预测理解和产出，因为语义空间中存在与形式空间结构同构的丰富信息，证实了波兰语词汇形式与意义之间的系统性关联。
...
TL;DR: LLMs内部编码了上下文隐私规范但行为仍会泄露隐私，研究发现通过基于上下文完整性理论的参数化干预可以有效减少隐私违规


<details>
  <summary>Details</summary>
Motivation: LLMs在高风险场景中经常违反上下文隐私规范，泄露私人信息，而人类在这种情况下会谨慎行事。这引发了一个根本问题：LLMs是否内部编码了上下文隐私规范？如果编码了，为什么违规仍然存在？

Method: 基于上下文完整性(CI)理论，系统研究LLMs中的上下文隐私作为结构化潜在表示。通过探测多个模型，发现CI的三个规范决定参数（信息类型、接收者、传输原则）在激活空间中编码为线性可分且功能独立的方向。引入CI参数化干预，沿每个CI维度独立干预。

Result: 研究发现LLMs确实内部编码了上下文隐私规范，但模型行为仍会泄露隐私，表明概念表示与模型行为之间存在明显差距。CI参数化干预比整体干预更有效、更可预测地减少隐私违规。

Conclusion: 上下文隐私失败源于表示与行为之间的错位而非意识缺失，利用CI的组合结构能够实现更可靠的上下文隐私控制，为改进LLMs中的上下文隐私理解提供了启示。
...
TL;DR: 大型语言模型在拒绝有害请求前能否准确预测自己的拒绝行为？研究发现模型具有较高的内省敏感性，但在安全边界处敏感性下降，不同模型表现差异显著，置信度分数可用于安全关键部署的决策路由。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能够在实际响应之前准确预测自己的拒绝行为，这对于理解模型的安全机制、提高安全部署的可靠性具有重要意义。

Method: 采用系统研究方法，让模型先预测自己的拒绝行为，然后在新的上下文中实际响应。使用信号检测理论分析四个前沿模型（Claude Sonnet 4/4.5、GPT-5.2、Llama 3.1 405B）在3754个数据点上的表现，涵盖300个请求。

Result: 所有模型都表现出较高的内省敏感性（d' = 2.4-3.5），但在安全边界处敏感性显著下降。Claude Sonnet 4.5准确率最高（95.7%），GPT-5.2准确率较低（88.9%）且行为更不稳定，Llama 405B敏感性高但拒绝偏见强、校准差（80.0%）。武器相关查询最难内省。置信度分数具有实用价值：限制在高置信度预测时，校准良好的模型准确率可达98.3%。

Conclusion: 大型语言模型能够在一定程度上预测自己的拒绝行为，但不同模型表现差异显著。置信度分数为安全关键部署提供了实用的决策依据，基于置信度的路由策略可以显著提高安全决策的准确性。
...
TL;DR: 该研究首次为编程语言建立了资源层级分类体系，将646种语言分为4个层级，揭示了编程语言资源分布的极端不平衡性：仅1.9%的语言（高层级）占据了74.6%的语料库token，而71.7%的语言（稀缺层级）仅占1.0%。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理领域已有语言资源层级分类体系，但编程语言领域缺乏类似的资源分类标准。随着大语言模型生成代码能力的增强，建立编程语言资源分类体系变得至关重要。

Method: 提出了第一个可复现的编程语言资源分类方法，将646种编程语言分为四个层级（Tier 0-3），基于七个主要代码语料库中的token分布进行统计分析。

Result: 研究发现编程语言资源分布极不平衡：Tier 3（高层级）仅占1.9%的语言却贡献了74.6%的token，而Tier 0（稀缺层级）占71.7%的语言仅贡献1.0%的token。统计分析确认这种不平衡是极端且系统性的。
...
TL;DR: REM-CTX是一个基于强化学习的自动同行评审系统，通过对应感知奖励函数将视觉元素和外部学术信号等辅助上下文整合到评审生成过程中，在多个科学领域超越了现有基线系统。


<details>
  <summary>Details</summary>
Motivation: 现有自动同行评审系统主要依赖文本内容，未能充分利用图表等视觉元素和外部学术信号等辅助上下文信息，限制了评审的质量和相关性。

Method: 使用强化学习框架，通过Group Relative Policy Optimization训练80亿参数语言模型，结合多维度质量奖励和两个对应奖励函数，显式鼓励与辅助上下文的对齐。

Result: 在计算机、生物和物理科学领域的稿件实验中，REM-CTX在六个基线系统中获得最高整体评审质量，超越了使用更大商业模型的系统，并在质量和上下文基础指标上均优于次优的强化学习基线。

Conclusion: 两个对应奖励函数具有互补性，批评维度与其他指标在训练中呈负相关，未来研究应考虑对多维度奖励进行分组处理以提高评审生成效果。
...
TL;DR: LLM在作文评分中与人类评分者存在系统性偏差，特别是在语法等低阶特质上评分更严，可通过小样本偏差校正改善


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在教育评估中的应用日益增长，但其与人类评分的一致性尚不明确，需要系统评估LLM在作文评分中的表现

Method: 使用三个公开作文评分数据集（ASAP 2.0、ELLIPSE、DREsS），分析指令调优LLM在整体评分和分析评分中的表现，评估与人类共识分数的一致性、方向性偏差和偏差稳定性

Result: 强开放权重模型在整体评分中与人类达到中等至高一致性（QWK约0.6），但在分析评分中表现不均；在语法等低阶特质上存在稳定负向偏差（评分更严）；简洁关键词提示在分析评分中优于详细评分标准提示

Conclusion: 建议采用"偏差校正优先"部署策略：使用小规模人工标注的偏差估计集校正系统性分数偏移，而非依赖原始零样本评分或大规模微调
...
TL;DR: 自反思提示在医学QA中效果有限，不能稳定提升准确性，更适合作为分析工具而非可靠性提升方案


<details>
  <summary>Details</summary>
Motivation: 虽然自反思（自我纠正）提示被广泛认为能通过让LLMs批判和修改自身推理来增强模型可靠性，但其在安全关键的医学环境中的有效性仍不明确
...
Conclusion: 自反思推理更适合被视为理解模型行为的分析工具，而非改善医学QA可靠性的独立解决方案，揭示了推理透明度与推理正确性之间的差距
...
TL;DR: 本文探讨生物语言学如何将语言视为生物器官而非文化工具，强调数学代数模型能揭示语言的自然本质，特别是MERGE操作作为"自然关节"，为生物学、遗传学和神经科学研究提供清晰指导。


<details>
  <summary>Details</summary>
Motivation: 生物语言学旨在研究语言的生物学基础、进化和遗传机制，将语言视为天生的生物器官而非文化工具，挑战行为主义将语言习得视为刺激-反应关联的观点。作者希望通过数学代数模型揭示语言的本质特征。

Method: 采用四步论证法：1)明确生物语言学的研究对象是生成层次结构表达的内部计算系统；2)论证形式特征对进化解释的重要性；3)提出明确的代数句法描述对候选神经机制施加非平凡约束；4)考察神经计算研究如何将这些约束转化为可检验的假设。

Result: MERGE句法结构构建操作被认为是"自然的关节"和"自然的新方面"，而非仅仅是形式人工制品。数学语言理论能为生物学家、遗传学家和神经科学家提供更清晰的研究指导，将形式约束转化为可检验的实证假设。

Conclusion: 生物语言学通过数学代数模型为语言研究提供了生物学基础，MERGE操作作为核心概念连接了形式句法与神经机制研究。虽然当前研究仍具推测性和可修正性，但神经计算研究正将理论约束转化为可检验的实证假设，推动跨学科的语言生物学研究。
...
TL;DR: 这篇论文综述了大型语言模型在滥用检测生命周期中的应用，分析了LLMs如何通过上下文推理、政策解释等能力改进传统静态分类器的不足，并提出了四个阶段的集成框架。


<details>
  <summary>Details</summary>
Motivation: 在线滥用行为日益复杂，传统基于静态分类器和人工标注的机器学习方法难以应对不断演变的威胁模式和细微的政策要求，需要更智能的解决方案。

Method: 提出了滥用检测生命周期（ADL）的四阶段框架：I）标签与特征生成，II）检测，III）审查与申诉，IV）审计与治理。对每个阶段，综合了新兴研究和行业实践，分析了LLMs的集成架构。

Result: LLMs在上下文推理、政策解释、解释生成和跨模态理解方面展现出新能力，能够支持现代安全系统的多个阶段，但存在延迟、成本效率、确定性、对抗鲁棒性和公平性等挑战。

Conclusion: LLMs有潜力成为大规模滥用检测和治理系统的可靠、负责任组件，但需要进一步研究解决操作化过程中的关键技术挑战。
...
TL;DR: Agent Q-Mix是一个基于强化学习的多智能体框架，通过QMIX价值分解将拓扑选择重构为合作式MARL问题，在保持任务准确性的同时优化token成本，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂任务中需要多个智能体协同工作，但如何有效选择和连接这些智能体是一个基本问题。现有方法在智能体拓扑选择和通信效率方面存在挑战。

Method: 提出Agent Q-Mix强化学习框架，将拓扑选择重构为合作式多智能体强化学习问题。使用QMIX价值分解学习去中心化通信决策，每个智能体从一组通信动作中选择，共同形成轮式通信图。核心包括拓扑感知GNN编码器、GRU记忆和每个智能体的Q头，采用集中训练分散执行的范式。

Result: 在编码、推理和数学等七个核心基准测试中，Agent Q-Mix实现了最高的平均准确率，同时表现出优越的token效率和智能体故障鲁棒性。在具有挑战性的Humanity's Last Exam测试中，使用Gemini-3.1-Flash-Lite作为骨干，达到20.8%的准确率，优于Microsoft Agent Framework（19.2%）、LangGraph（19.2%）、AutoGen和Lobster by OpenClaw。

Conclusion: 学习到的去中心化拓扑优化在推动多智能体推理边界方面具有有效性，Agent Q-Mix框架为解决复杂多智能体协调问题提供了有前景的解决方案。
...
TL;DR: 研究发现神经元激活重叠主要源于词汇混淆而非概念压缩，通过2x2因子分解证明相同词形比相同语义更能解释激活重叠，这一混淆影响稀疏自编码器特征并损害下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 当同一神经元对"lender"和"riverside"都激活时，传统指标将其归因于叠加（superposition）——神经元压缩了两个无关概念。本研究旨在探索这种重叠有多少是由于词汇混淆造成的：神经元实际上是对共享词形（如"bank"）而非压缩的两个概念做出反应。

Method: 采用2x2因子分解方法，对比分析词汇条件（相同词形、不同含义）与语义条件（不同词形、相同含义）对神经元激活的影响。研究覆盖参数规模从110M到70B的不同模型，并扩展到稀疏自编码器特征分析。

Result: 词汇条件（相同词形）的激活重叠始终超过语义条件（相同含义），这一模式在不同规模模型中一致存在。词汇混淆影响18-36%的稀疏自编码器特征，仅占据≤1%的激活维度，但会损害下游任务：过滤掉这种混淆能改进词义消歧性能，并使知识编辑更具选择性（p=0.002）。

Conclusion: 神经元激活重叠主要源于词汇混淆而非概念压缩，这一发现挑战了传统叠加解释。词汇混淆虽然只占很小维度，但对下游任务有显著负面影响，过滤该混淆能提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 将预训练的文本大语言模型通过持续预训练适应语音数据时，通常会降低原始文本能力，需要找到既能保持文本能力又能有效适应语音的方法

Method: 提出Multimodal Depth Upscaling方法，在冻结的文本LLM中插入新的transformer层，仅训练这些新增层来适应语音数据；进一步探索使用专门为语音识别设计的E-Branchformer架构作为插入层

Result: 在SmolLM2-360M和SmolLM2-1.7B模型上，使用48k小时英语ASR数据，深度上采样方法在ASR性能上与全微调相当，但文本退化远少于全微调和LoRA；使用E-Branchformer作为插入层时，在更大模型上ASR性能匹配或超越全微调，文本退化减少75%以上，可训练参数减少60%

Conclusion: Multimodal Depth Upscaling是一种有效的策略，能够在保持文本语言模型能力的同时，将其适应到语音模态，减少灾难性遗忘问题，为多模态语言模型开发提供了有前景的方向
...
TL;DR: 提出基于影响力估计的优化框架，通过目标模型反馈评估合成数据效用，自动优化数据生成规则，解决知识密集型领域高质量SFT数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 知识密集型领域（人文社科、医学、法律、金融等）的高质量监督微调数据稀缺，专家标注成本高、隐私限制严格、标签一致性难以保证。现有合成数据方法依赖人工设计规则，但规则设计依赖专家经验、跨领域迁移性差、优化过程缺乏可靠的量化反馈。

Method: 提出基于影响力估计的优化框架：1）使用梯度信息量化每个合成样本对目标模型在特定任务上目标的贡献；2）基于目标模型反馈自适应优化规则；3）提供轻量级指导文本，使用规则专用模型生成任务条件规则；4）以影响力分数作为奖励，通过强化学习优化规则生成器。

Result: 实验表明，即使合成样本与真实样本在嵌入空间接近，它们对学习的影响也可能显著不同。提出的方法在不同领域、目标模型和数据生成器上均显示出持续改进和强大的泛化能力，无需任务特定调优。

Conclusion: 通过目标模型反馈评估合成数据效用并指导数据生成，能够有效解决知识密集型领域高质量SFT数据稀缺问题，实现跨领域的稳定性能提升和泛化能力。
...
TL;DR: 研究者构建了JUBAKU-v2数据集，专门评估日语大语言模型中的文化偏见，特别是推理过程中的群体归因偏见，相比现有基准能更敏感地检测模型性能差异。


<details>
  <summary>Details</summary>
Motivation: 现有日语偏见评估基准主要依赖英语数据翻译，无法准确反映日本文化背景下的社会偏见，且仅评估结论偏见而忽略了推理过程中的偏见。

Method: 基于社会心理学中的归因理论，构建了包含216个反映日本文化特定偏见的示例数据集JUBAKU-v2，评估在固定结论情况下推理过程中对内群体和外群体行为的归因偏见。

Result: 实验结果表明，JUBAKU-v2比现有基准能更敏感地检测不同模型之间的性能差异。

Conclusion: JUBAKU-v2数据集为评估日语大语言模型中的文化特定偏见提供了更有效的工具，特别是针对推理过程中的归因偏见评估。
...
TL;DR: 使用1.7B参数的小型语言模型，通过量化微调和有限人工标注数据，构建了比专有LLM更优的自动评估和标注系统


<details>
  <summary>Details</summary>
Motivation: 随着LLM能力提升，文本语料库标注需求激增，但专有LLM存在系统性偏见、缺乏可复现性和数据隐私问题，需要更优的替代方案

Method: 使用1.7B参数的小型语言模型，通过4位量化微调、有限人工标注数据、自定义多维评分框架以及简单的数据增强和正则化技术

Result: 提出的方法在评分者间一致性（Krippendorff's α）上比最佳专有LLM提高0.23分，并在情感分类任务上展示了泛化能力

Conclusion: 任务特定的对齐和高效的4位量化微调为评估和标注任务提供了优于专有模型的开源替代方案
...
TL;DR: 本文提出链式思维ASR（CoT-ASR），通过构建推理链让大语言模型先分析语音输入生成上下文分析，再基于此进行语音识别，单次完成推理和转录，显著降低词错误率和实体错误率。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型已扩展到语音输入，但在自动语音识别任务中有效利用其丰富知识和上下文理解能力仍然困难，因为ASR主要是直接的语音到文本映射任务。

Method: 提出CoT-ASR方法：1）构建推理链让LLM先分析语音生成上下文分析；2）引入CTC引导的模态适配器，使用CTC非空白标记概率加权LLM嵌入，有效对齐语音编码器输出与LLM的文本潜在空间；3）支持用户引导转录，可无缝整合用户提供的上下文。

Result: 相比标准LLM-based ASR，CoT-ASR实现了词错误率相对降低8.7%，实体错误率相对降低16.9%。

Conclusion: CoT-ASR通过构建推理链充分利用LLM的生成能力，结合CTC引导的模态适配器减少模态差距，显著提升ASR性能，同时支持用户引导的转录功能。
...
TL;DR: KUTED是一个用于中库尔德语的语音到文本翻译数据集，基于TED演讲构建，包含91,000个句子对。研究发现库尔德语的正字法变异会显著降低翻译质量，通过文本标准化方法可以显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 为库尔德语创建高质量的语音到文本翻译数据集，解决库尔德语资源稀缺的问题，并研究正字法变异对翻译性能的影响。

Method: 从TED和TEDx演讲中构建KUTED数据集，包含91,000个句子对。提出系统性的文本标准化方法来处理库尔德语的正字法变异问题。评估了多种模型：微调的Seamless模型、从头训练的Transformer模型，以及结合Seamless（ASR）和NLLB（MT）的级联系统。

Result: 在从TED演讲分离的测试集上，微调的Seamless模型达到15.18 BLEU分数。在FLEURS基准测试中，将Seamless基线提升了3.0 BLEU。文本标准化方法显著改善了翻译性能并产生了更一致的输出。

Conclusion: KUTED数据集填补了库尔德语语音翻译资源的空白，正字法变异是影响库尔德语翻译质量的关键因素，通过系统性的文本标准化可以显著提升翻译性能。该研究为低资源语言的语音翻译提供了重要参考。
...
TL;DR: TRIMS提出了一种轨迹引导的监督微调框架，通过自回归教师的轻量信号指导掩码策略，改善扩散语言模型的解码轨迹，在保持低训练成本的同时提升准确率与并行性的权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行解码有望实现低延迟生成，但实际效率严重依赖解码轨迹。标准训练缺乏对token揭示顺序的显式监督，导致训练-推理不匹配和次优解码行为。

Method: 提出TRIMS框架，将轨迹监督注入标准掩码扩散语言模型训练。使用自回归教师的轻量信号指导轨迹感知的掩码策略，而非依赖昂贵的DLM蒸馏，鼓励模型学习更有效的解码顺序。

Result: 在LLaDA和Dream模型上的数学和编程基准测试表明，TRIMS相比标准MDLM训练和无训练加速基线显著改善了准确率-并行性权衡，与先前的蒸馏方法性能相当但训练成本大幅降低。

Conclusion: TRIMS通过轨迹引导的监督改善了扩散语言模型的解码轨迹，验证了轨迹监督对DLM的有效性，为实现高效并行解码提供了实用解决方案。
...
TL;DR: 论文从统计假设检验角度重新解释TF-IDF，通过惩罚似然比检验框架推导出类似TF-IDF的评分，为传统术语加权方案提供统计理论基础。


<details>
  <summary>Details</summary>
Motivation: TF-IDF作为广泛使用的术语重要性评估公式，缺乏坚实的统计理论基础。作者希望通过统计假设检验框架来解释TF-IDF，并为术语加权方案的发展提供新的视角。

Method: 提出一个惩罚似然比检验框架：备择假设使用带有伽马惩罚项的beta-二项分布族来建模文档集合，捕捉词语的突发性（过离散）；零假设使用二项分布建模，不考虑词语突发性。从该检验统计量推导出类似TF-IDF的术语加权方案。

Result: 基于该检验统计量推导出的术语加权方案在文档分类任务上与TF-IDF表现相当，验证了统计框架的有效性。

Conclusion: 论文从统计角度为TF-IDF提供了新的解释，并展示了假设检验框架在推进术语加权方案发展方面的潜力。
...
Method: 采用扩散语言模型风格的离散非自回归架构，直接从文本映射到多码本声学标记。关键技术包括：1）全码本随机掩码策略用于高效训练；2）从预训练大语言模型初始化以确保卓越的可懂度。使用581k小时的多语言开源数据集进行训练。

Result: 实现了迄今为止最广泛的语言覆盖（超过600种语言），在中文、英文和多样化的多语言基准测试中达到了最先进的性能。

Conclusion: OmniVoice通过简化的架构和创新的训练策略，成功构建了一个大规模、高性能的多语言零样本文本转语音系统，代码和预训练模型已开源。
...
TL;DR: AfrIFact：首个覆盖十种非洲语言和英语的自动事实核查数据集，用于信息检索、证据提取和事实核查任务


<details>
  <summary>Details</summary>
Motivation: 在线虚假信息对信息获取有限的社区影响尤为严重，特别是在医疗保健和文化等关键领域，而非洲语言等低资源语言缺乏相应的事实核查资源

Method: 创建AfrIFact数据集，包含十种非洲语言和英语；评估现有嵌入模型的跨语言检索能力；测试LLMs在非洲语言事实核查中的表现；探索少样本提示和任务特定微调方法

Result: 最佳嵌入模型缺乏跨语言检索能力；文化和新闻文档比医疗领域文档更容易检索；LLMs在非洲语言事实核查中表现不佳；少样本提示可将性能提升43%；任务特定微调可进一步提升26%准确率

Conclusion: AfrIFact数据集的发布将推动低资源语言信息检索、证据检索和事实核查研究，当前模型在非洲语言事实核查方面仍有很大改进空间
...
TL;DR: 研究系统分析了在固定数据预算下，预训练数据规模与检索库规模之间的权衡关系，提出了三维缩放框架来优化语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 虽然检索增强生成（RAG）通过提供相关上下文提升了语言模型在知识密集型任务中的表现，但预训练获得的参数化知识与检索获得的非参数化知识之间的关系仍不清楚，特别是在固定数据预算下如何分配资源的问题尚未得到系统研究。

Method: 训练基于OLMo-2的语言模型（参数规模从30M到3B），在DCLM数据上使用最多100B tokens，同时变化预训练数据规模（参数数量的1-150倍）和检索库规模（1-20倍），并在推理、科学问答和开放域问答等多种基准测试上评估性能。

Result: 检索在所有模型规模上都持续优于纯参数化基线；提出了三维缩放框架，将性能建模为模型大小、预训练token数量和检索库规模的函数；该框架能够估算固定数据预算在预训练和检索之间的最优分配，发现检索的边际效用强烈依赖于模型规模、任务类型和预训练饱和程度。

Conclusion: 研究结果为理解何时以及如何让检索补充预训练提供了量化基础，为设计可扩展语言建模系统时的数据资源分配提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在动态环境中难以自主演化协调策略，因为粗粒度的全局结果掩盖了局部策略细化所需的因果信号。这本质上是一个多智能体信用分配问题，在传统多智能体强化学习中已有研究，但在基于LLM的系统中尚未得到充分解决。

Method: 提出LangMARL框架，将合作式多智能体强化学习中的信用分配和策略梯度进化引入语言空间。具体包括：1）智能体级语言信用分配；2）语言空间中的梯度进化用于策略改进；3）从重放轨迹中总结任务相关的因果关系，提供密集反馈并改善稀疏奖励下的收敛性。

Result: 在多种合作式多智能体任务上的广泛实验表明，该框架提高了样本效率、可解释性，并展现出强大的泛化能力。

Conclusion: LangMARL成功地将多智能体强化学习的信用分配机制引入语言模型智能体，解决了LLM智能体在动态协调任务中的策略演化瓶颈，为语言智能体的自主协作提供了有效解决方案。
...
TL;DR: 受果蝇全脑连接组启发，提出随机注意力机制，通过随机置换token序列实现全局感受野，在相同计算复杂度下显著提升注意力表达能力。


<details>
  <summary>Details</summary>
Motivation: 果蝇大脑连接组虽然连接概率极低（仅0.02%），但平均最短路径只有4.4跳，这得益于跨脑区的随机捷径连接。受此启发，研究者希望将这种随机路由机制引入注意力机制，在保持计算效率的同时提升全局信息传递能力。

Method: 提出随机注意力（SA），作为滑动窗口注意力（SWA）的即插即用增强模块。方法是在窗口注意力前对token序列进行随机置换，注意力计算后再恢复原始顺序。这样在保持O(nw)每层计算复杂度的同时，将固定局部窗口转化为随机全局窗口。通过多层堆叠，独立采样的置换能实现指数级增长的感受野。

Result: 在两种设置下验证：1）从头预训练语言模型，门控SA+SWA组合获得最佳平均零样本准确率；2）在Qwen3-8B和Qwen3-30B-A3B上进行免训练推理，SA始终优于SWA，在可比计算预算下匹配或超过混合块注意力。

Conclusion: 受连接组启发的随机路由是提升高效注意力表达能力的实用原语，与现有的线性和稀疏方法互补。该方法在保持计算效率的同时实现了更好的全局信息传递。
...
TL;DR: 该论文系统比较了心理健康文本分类的各种优化方法，从基础基线到参数高效微调再到偏好优化，提供了实用的方法选择指导框架。


<details>
  <summary>Details</summary>
Motivation: 心理健康文本分类领域虽然广泛采用了现代适应方法，但关于选择何种优化策略、何时使用以及为什么有效的实用指导仍然有限。需要系统研究不同优化路径的效果差异。

Method: 采用系统比较研究方法：1) 建立经典和编码器基线；2) 在多种目标和优化设置下评估LoRA/QLoRA参数高效监督微调；3) 评估DPO、ORPO、KTO等偏好优化方法，包括类别重平衡训练。重点关注方法性能如何随目标公式、适配器选择、优化器行为、上下文窗口和类别平衡干预而变化。

Result: 优化效果高度依赖于具体方法：某些方法提供稳定、可迁移的增益，而其他方法对配置和数据平衡敏感。偏好优化在不同目标间表现出巨大差异，表明方法选择比简单添加偏好训练阶段更为重要。

Conclusion: 提出了心理健康NLP的清晰优化路径：从透明基线开始，应用受控调优，在可证明增益的情况下选择性使用偏好优化。这为超越架构选择的有效训练策略提供了可重复且实用的框架。
...
TL;DR: LLMs在字符计数等基础符号任务上失败，不是因为缺乏表征能力，而是因为模型内部存在负向电路，抑制了正确的信号输出。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在复杂基准测试上表现出色，但在字符计数等基础符号任务上却经常失败。这种限制已被注意到，但其内部原因尚不清楚。研究者希望通过字符计数这一最小化、受控的探针任务，来理解LLMs在符号推理失败背后的机制。

Method: 使用字符计数作为探针任务，结合探测分类器、激活修补、logit lens分析和注意力头追踪等机制分析方法，研究LLaMA、Qwen和Gemma等现代架构的内部表征。

Result: 研究发现字符级信息在早期和中间层被编码，但在后期层（尤其是倒数第二层和最后一层的MLP）被少量组件衰减。这些组件构成负向电路，抑制正确信号而偏向高概率的错误输出。LLMs的前向传播实现了竞争性解码，正确和错误假设共存并动态重新加权。

Conclusion: LLMs的符号推理失败不是由于缺少表征或规模不足，而是源于计算图中的结构化干扰。这解释了为什么这类错误会持续存在，并在扩展和指令微调下可能恶化。研究强调了需要设计策略确保信息被可靠编码和使用。
...
TL;DR: 本文基于先前对Mapudungun语动词词根的确认工作，进一步研究这些动词的配价分类，使用语言自身的形态结构分析后缀与词根/动词词干的组合限制，旨在改进形态分析器并深化对Mapuche动词配价问题的理解。


<details>
  <summary>Details</summary>
Motivation: 在先前确认Mapudungun语动词词根的基础上，本研究旨在通过分析语言自身的形态结构来对这些动词进行配价分类，既为改进形态分析器提供数据支持，也从理论角度增进对Mapuche动词配价问题的认识。

Method: 利用Mapudungun语自身的形态结构，通过考察各种后缀与词根或动词词干在Mapuche动词形式中的允许和限制组合，来确定动词的配价分类。

Result: 研究结果已整合到形态分析器Dungupeyum中，改进了该系统的分析能力，同时从理论上对Mapuche动词形式的配价问题有了更深入的认识。

Conclusion: 本研究成功运用Mapudungun语自身的形态结构对动词进行了配价分类，既为形态分析器的改进提供了实证支持，也为理解Mapuche动词的配价特性做出了理论贡献。
...
TL;DR: EmoScene是一个基于情感理论的多维情感理解基准，包含4731个情境丰富的场景，标注了8维情感向量，用于评估语言模型在上下文感知的多标签情感预测能力。


<details>
  <summary>Details</summary>
Motivation: 现有情感理解基准大多基于短文本和预定义情感标签，将情感理解简化为独立的标签预测，忽略了情感之间的结构化依赖关系，无法捕捉自然语言中多维情感推理的复杂性。

Method: 1) 构建EmoScene基准：基于Plutchik基本情感理论，标注4731个情境丰富的场景的8维情感向量；2) 评估6个指令调优大语言模型的零样本性能；3) 提出纠缠感知贝叶斯推理框架，利用情感共现统计进行联合后验推断。

Result: 最佳模型在零样本设置下仅达到0.501的Macro F1分数，表明上下文感知的多标签情感预测具有挑战性。提出的贝叶斯推理框架能提高预测的结构一致性，对较弱模型带来显著提升（如Qwen2.5-7B提升+0.051 Macro F1）。

Conclusion: EmoScene为研究多维情感理解和当前语言模型的局限性提供了一个具有挑战性的基准，情感纠缠感知的推理方法能有效提升模型性能，表明考虑情感间依赖关系对准确情感理解的重要性。
...
TL;DR: StanceMoE：基于BERT的混合专家架构，用于演员级立场检测，通过六个专家模块捕捉不同语言信号，在StanceNakba数据集上达到94.26%的宏F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的立场检测模型通常依赖统一表示，难以充分捕捉对比性话语结构、框架线索和显著词汇指标等异质语言信号，需要能够显式建模多样化立场表达模式的自适应架构。

Method: 提出StanceMoE，基于微调BERT编码器的上下文增强混合专家架构。集成六个专家模块：全局语义导向、显著词汇线索、从句级焦点、短语级模式、框架指标和对比驱动的话语转换。采用上下文感知门控机制动态加权专家贡献。

Result: 在StanceNakba 2026 Subtask A数据集（1,401个英文文本，目标演员隐含在文本中）上进行实验，StanceMoE获得94.26%的宏F1分数，优于传统基线和替代的BERT变体。

Conclusion: StanceMoE通过显式建模多样化立场表达模式，有效提升了演员级立场检测性能，证明了混合专家架构在捕捉异质语言信号方面的优势。
...
### [135] [When Users Change Their Mind: Evaluating Interruptible Agents in Long-Horizon Web Navigation](https://arxiv.org/abs/2604.00892)
...
TL;DR: 该论文首次系统研究了长时域、环境基础的网页导航任务中的可中断智能体，提出了包含三种中断类型的InterruptBench基准，并评估了六个大型语言模型在中断处理中的表现。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体从解决短期静态问题转向在动态环境中执行复杂长时域任务，处理用户中断（如添加需求或修改目标）成为实际部署的核心需求。现有基准大多假设智能体行为不受中断，或仅研究短期、无约束的语言任务中的中断。

Method: 1. 形式化三种现实中断类型：添加、修改和撤回；2. 基于WebArena-Lite构建InterruptBench基准，在严格语义约束下合成高质量中断场景；3. 使用统一中断模拟框架评估六个强大的LLM主干模型；4. 分析模型在单轮和多轮中断设置下适应更新意图的有效性和从任务中途变化的恢复效率。

Result: 研究结果表明，即使在强大的大规模LLM中，在长时域智能体任务期间有效且高效地处理用户中断仍然具有挑战性。代码和数据集已在GitHub上公开。

Conclusion: 该研究填补了长时域、环境基础任务中可中断智能体系统研究的空白，为评估和提升LLM智能体在动态环境中的中断处理能力提供了基准和方法框架。
...
TL;DR: GPT-NL公共语料库是最大的荷兰语开源语料库，包含36B荷兰语token和大量其他语言数据，旨在促进合法、有用、无害的语言模型开发。


<details>
  <summary>Details</summary>
Motivation: 为荷兰语自然语言处理提供大规模、合规的开源语料资源，解决现有荷兰语数据稀缺问题，促进商业语言模型的合法开发。

Method: 整合现有大型语料库（如Common Corpus、Common Crawl）并进行合规性筛选，同时与组织合作收集新的荷兰语特定内容，包括合成增强内容。

Result: 创建了包含21个荷兰语集合、总计36B预处理荷兰语token的语料库，以及207B英语、232B代码、48B德语/丹麦语token，所有数据采用CC-BY许可并在Hugging Face Hub公开。

Conclusion: GPT-NL公共语料库为荷兰语NLP研究提供了重要资源，通过合规数据收集和开源许可，促进了合法、有用、无害的语言模型开发。
...
TL;DR: 该研究探索大语言模型学习新语言的训练动态，提出基于认知功能专门化的层选择方法CogSym，仅微调25%的最外层即可达到接近全微调的性能。


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型如何获取新语言和多语言能力对于实现高效适应至关重要。现有研究主要关注训练好的模型如何处理多语言指令，而忽略了模型在训练过程中获取新语言的机制。

Method: 通过语言感知（输入理解）和语言生成（输出生成）两个认知功能专门化的视角，研究解码器transformer的训练动态。在低资源语言上进行实验，通过从输入和输出方向进行层消融扫描，观察专门化模式，提出CogSym层选择启发式方法。

Result: 仅微调最外层的25%层数，在下游任务性能上能达到与全微调基线相差2-3%以内的效果。CogSym与LoRA等适配器方法表现一致，展示了超越全微调的泛化能力。

Conclusion: 研究揭示了LLMs学习新语言的机制，提出了高效的层选择微调方法，推动了可访问和包容性语言建模的发展。
...
TL;DR: 该研究构建了一个具有短程相互作用的随机语言模型，发现即使只参考长度恒定的上下文，模型仍会发生相变，表明语言模型的相变源于语言本身的内在特性，而非长程相互作用。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明具有长程相互作用的语言模型存在相变，但统计力学中已知长程相互作用本身就能诱导相变。因此需要澄清语言模型中的相变是否真正源于语言特性，而非仅仅是长程相互作用的结果。

Method: 构建了一个具有短程相互作用的随机语言模型，该模型属于乔姆斯基层级中的上下文敏感文法类别，允许显式参考上下文。通过数值方法研究其统计特性，特别关注只参考长度恒定上下文的情况。

Result: 研究发现即使模型只参考长度恒定的上下文（不随句子长度变化），仍然会发生相变。这表明相变确实源于语言的内在特性。

Conclusion: 语言模型中的有限温度相变是由语言本身的内在特性诱导的，而非长程相互作用的结果。这一发现澄清了先前研究中的疑问，确认了语言模型相变的语言学本质。
...
TL;DR: 该研究开发了一个多模态分析管道，用于分析YouTube Shorts中地缘政治事件的报道，并以以色列-哈马斯战争为例，分析了国家资助媒体的短视频报道模式。


<details>
  <summary>Details</summary>
Motivation: YouTube Shorts已成为平台新闻消费的重要形式，但关于地缘政治事件在这种格式中如何呈现的研究仍然有限，需要填补这一研究空白。

Method: 开发了一个结合自动转录、基于方面的情感分析（ABSA）和语义场景分类的多模态管道，应用于分析2300多个冲突相关Shorts和94000多个视觉帧。

Result: 研究发现不同媒体在特定方面的情感表达存在差异且随时间变化，场景分类反映了与现实事件一致的视觉线索；较小的领域适应模型在情感分析上优于大型transformer和LLM。

Conclusion: 该管道可作为其他短视频平台（如TikTok和Instagram）的分析模板，展示了多模态方法与定性解释相结合如何表征算法驱动视频环境中的情感模式和视觉线索。
...
TL;DR: VRF通过变分推理将用户偏好表示为共享偏好空间中的概率分布，解决了现有奖励分解方法在稀疏数据下权重估计不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有奖励分解方法从稀疏数据中孤立地估计用户权重作为确定性点，导致推断不准确和不可靠，需要一种能够处理不确定性的框架。

Method: VRF使用变分编码器推断用户分布，通过Wasserstein距离匹配与共享概率基函数推导权重，并通过方差衰减损失降低不确定估计的影响。

Result: 在三个基准测试中，VRF在所有基线方法上表现更优，适用于已见和未见用户、少样本场景以及不同不确定性水平，优势延伸至下游对齐任务。

Conclusion: VRF通过变分推理处理用户偏好不确定性，显著提升了奖励分解的准确性和可靠性，为个性化LLM提供了更稳健的框架。
...
<details>
  <summary>Details</summary>
Motivation: 探索作者是否在作品的信息论新颖性曲线上具有可识别的独特特征，验证作者声音是否在文本新颖性变化中留下可测量的痕迹。

Method: 使用Books3（52,796本书，759位作者）和PG-19（28,439本书，1,821位作者）两个语料库，分析作者作品的新颖性曲线。在书籍层面分析标量动态特征（平均新颖性、速度、体积、迂回程度），在章节层面使用SAX主题模式分析滑动窗口。

Result: 书籍层面的标量动态特征能显著识别43%的作者；章节层面的SAX主题模式识别效果达到随机水平的30倍，远优于书籍层面的标量特征。这些信号是互补的而非冗余的。作者指纹部分与体裁相关，但约四分之一作者在体裁内仍保持独特性。古典作家（吐温、奥斯汀、吉卜林）的指纹强度与现代作家相当。

Conclusion: 作者声音确实在文本新颖性变化中留下可测量的多尺度痕迹，这些指纹特征在不同历史时期的作家中都存在，表明这种现象不是当代出版惯例的产物。
...
TL;DR: 研究发现大型语言模型在上下文学习中表现出类似序列回忆的模式，特别倾向于预测重复标记后的下一个标记，这种模式与认知科学中的自由回忆范式相似。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型展现出强大的上下文学习能力，但模型如何跟踪和从上下文中检索信息的机制尚不清楚。研究者希望探索LLMs在信息处理中是否存在类似人类认知的模式。

Method: 采用认知科学中的自由回忆范式，分析多个开源LLMs的预测模式。通过系统性的消融实验，研究诱导头（induction heads）在序列回忆模式中的作用，比较移除高诱导分数头与随机移除头对模型行为的影响。

Result: 研究发现LLMs确实表现出类似序列回忆的模式，特别倾向于预测重复标记后的下一个标记（+1滞后偏差）。诱导头在这一现象中起关键作用，移除高诱导分数头会显著减少+1滞后偏差，而移除随机头则不会产生相同效果。移除高诱导分数头还会更严重地损害模型在少样本学习中进行序列回忆的性能。

Conclusion: 研究揭示了诱导头与Transformer中时间上下文处理之间的机制性联系，表明这些头在有序检索和上下文学习中的序列回忆行为中特别重要，为理解LLMs的内部工作机制提供了新的视角。
...
TL;DR: CARE框架通过多阶段隐私合规的智能体推理，解决医疗场景中体征与症状不一致时LLM决策性能下降的问题，在ICU器官功能障碍预测任务上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在支持高风险医疗决策时，当可用证据内部不一致（如患者报告症状与医学体征矛盾）时性能会下降。这种矛盾在真实医疗场景中常见，需要专门解决方案。

Method: 提出CARE框架：多阶段隐私合规的智能体推理系统。远程LLM在不访问敏感患者数据的情况下生成结构化类别和转换规则，本地LLM利用这些类别和规则进行证据获取和最终决策。基于MIMIC-IV构建了专门包含体征-症状不一致病例的MIMIC-DOS数据集。

Result: CARE在所有关键指标上均优于多个基线设置，包括单次传递LLM和智能体流水线方法。表明CARE能够更稳健地处理冲突的临床证据，同时保护隐私。

Conclusion: CARE框架通过分离远程指导与本地决策，有效解决了医疗场景中证据不一致时LLM决策的挑战，在保护隐私的同时提升了决策鲁棒性，为高风险医疗决策支持提供了可行方案。
...
TL;DR: Brainstacks是一种模块化架构，通过冻结适配器堆栈实现大语言模型的多领域持续微调，使用MoE-LoRA路由、残差提升、零遗忘投影和基于结果的元路由器等技术，在多个领域上实现高效知识组合。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在多领域持续微调中的灾难性遗忘问题，同时实现领域知识的可组合性和高效利用，探索领域堆栈是否编码可迁移的认知原语而非特定领域知识。

Method: 提出包含五个核心组件的架构：1) MoE-LoRA带噪声top-2路由；2) 残差提升内循环；3) 课程排序外循环；4) 随机SVD零空间投影实现零遗忘；5) 基于结果的sigmoid元路由器。还包含PSN预训练和领域RL验证两个边界实验。

Result: 在TinyLlama-1.1B和Gemma 3 12B IT上验证，MoE-LoRA比参数匹配的单LoRA收敛快2.5倍，残差提升突破单堆栈上限，路由系统恢复被无门控堆叠破坏的生成质量。关键发现：基于结果的路由器发现领域堆栈编码可迁移的认知原语而非领域特定知识。

Conclusion: Brainstacks架构成功实现了多领域持续微调，领域堆栈编码的是可迁移的认知原语，使得模型能够跨领域组合使用这些能力，为构建模块化、可组合的AI系统提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 探索是否仅通过大语言模型自身原始输出就能改进代码生成，而不依赖验证器、教师模型或强化学习等复杂机制

Method: 采用简单自蒸馏方法：以特定温度和截断配置从模型中采样解决方案，然后使用标准监督微调在这些样本上进行训练

Result: SSD将Qwen3-30B-Instruct在LiveCodeBench v6上的pass@1从42.4%提升到55.3%，增益主要集中在更难的问题上，并在4B、8B、30B规模的Qwen和Llama模型上普遍有效

Conclusion: SSD通过解决LLM解码中的精度-探索冲突，以上下文相关的方式重塑标记分布，为改进LLM代码生成提供了一种互补的后训练方向
...
TL;DR: RELISH是一种用于大语言模型文本回归的轻量级架构，通过交叉注意力迭代优化潜在状态，然后线性映射到标量值，在多个数据集和模型上优于现有方法，参数效率极高。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型文本回归方法存在效率问题：将数值目标解码为文本或聚合多个生成输出不够高效，需要更直接、参数效率更高的回归方法。

Method: RELISH通过交叉注意力在冻结的LLM表示上迭代优化学习到的潜在状态，然后将最终状态通过线性回归器映射为点估计，避免了数值文本解码或输出聚合。

Result: 在5个数据集、4种LLM主干和2种LLM训练机制下，RELISH始终优于所有三大类LLM回归基线方法（自回归解码、回归感知推理和现有预测头方法）。

Conclusion: RELISH提供了一种高效、轻量级的文本回归解决方案，在保持卓越性能的同时，参数开销极低（仅需3.4-3.7M可训练参数，占模型大小的0.01-0.04%），远低于LoRA等替代方案。
...
TL;DR: YC-Bench是一个评估LLM智能体在长期复杂任务中战略连贯性的基准测试，模拟初创公司运营一年，涉及员工管理、合同选择和盈利能力维持，测试模型在不确定性下的规划、延迟反馈学习和适应能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体处理日益复杂的任务，需要评估它们是否能在长期视野中保持战略连贯性：包括在不确定性下规划、从延迟反馈中学习、以及当早期错误累积时进行适应。现有基准测试往往缺乏这种长期、多步骤、具有复合后果的评估场景。

Method: 引入YC-Bench基准测试，模拟初创公司运营一年的场景，包含数百个回合。智能体需要在部分可观测环境中管理员工、选择任务合同、维持盈利能力，面对对抗性客户和不断增长的工资支出等挑战。评估了12个模型（包括专有和开源模型），每个模型3个种子。

Result: 只有三个模型能够持续超过20万美元的起始资本，Claude Opus 4.6获得最高平均最终资金127万美元，GLM-5获得121万美元但推理成本低11倍。草稿纸使用是成功的最强预测因素，对抗性客户检测是主要失败模式（占破产的47%）。前沿模型仍存在过度并行化等特定失败模式。

Conclusion: YC-Bench揭示了当前LLM智能体在长期视野任务中的能力差距，草稿纸使用对信息跨上下文持久化至关重要，对抗性客户检测是主要挑战。该基准测试是开源、可复现和可配置的，为评估长期战略连贯性提供了重要工具。
...
Method: 基于YOCO框架，实现通用自解码器，通过参数共享执行多次迭代，将迭代过程限制在浅层高效注意力层，结合恒定全局KV缓存和线性预填充
